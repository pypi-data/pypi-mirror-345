...
async def get_response_from_messages(messages: List[Dict[str, str]], model: str) -> str:
    """
    Generate a response from messages without directly sending it to the user.

    This is used for capturing response text for automatic text-to-speech playback
    in voice-initiated conversations.
    ...
    """
    # Default settings
    temperature = get_setting(conf.SETTINGS_TEMPERATURE) or conf.DEFAULT_TEMPERATURE
    top_p = get_setting(conf.SETTINGS_TOP_P) or conf.DEFAULT_TOP_P

    logger.info(f"Generating response for TTS using model: {model}")
    ...
    try:
        ...
        logger.error(f"Error generating response for TTS: {e}")
        return ""
...
async def handle_thinking_conversation(
    ...
    async with safe_execution(
        ...
        async with cl.Step(name=APP_NAME) as step:
            thinking_step = step
