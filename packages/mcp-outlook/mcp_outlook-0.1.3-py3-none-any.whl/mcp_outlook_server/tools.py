...
def _handle_outlook_operation(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        if isinstance(result, (list, type(None))):
            return {"success": True, "data": result}
        elif isinstance(result, dict) and "success" not in result and func.__name__.startswith(('create', 'update', 'delete')):
...
def get_email_tool(message_id: str, user_email: str) -> Optional[Dict[str, Any]]:
    return resources.get_email_by_id(message_id, user_email, as_text=True)
...
def search_emails_tool(query: str, user_email: str, top: int = 10, folders: List[str] = None) -> List[Dict[str, Any]]:
    return resources.search_emails(query, user_email, top, folders, as_text=True)
...
@_handle_outlook_operation
def download_emails_by_date_tool(start_date: str, end_date: str, user_email: str, 
                              top: int = 100, folders: List[str] = None) -> List[Dict[str, Any]]:
    return resources.download_emails_by_date(start_date, end_date, user_email, top, folders, as_text=True)
...
def update_draft_email_tool(message_id: str, user_email: str, subject: Optional[str] = None, 
    ...
    # Apply changes for non-null fields
    for attr, value, transform in [
        ...
        if value is not None:
            msg.set_property(attr, transform(value) if transform else value)
