...
class AbstractButtonView(AbstractComponentView, ButtonViewMixin):
    ...
    def __init__(
            ...
            condition: Union[bool, Callable[[Optional[Dict[str, Any]]], bool]] = True,
    ...
    def _resolve_param_value(self, name: str, item: Optional[dict[str, Any]], default: Any) -> Any:
        ...
        if isinstance(source, str) and source.startswith("<") and source.endswith(">"):
            ...
            if item and key in item:
                ...
                return default if self.run_as_task else self._raise_missing(name, key, item)
...
class FunctionButtonView(AbstractButtonView):
    ...
    def process(self, item: Optional[Dict[str, Any]] = None, **kwargs) -> dict[str, Any]:
        ...
        url = f"/execute/{self.call.__module__}.{self.call.__qualname__}"
        # Generate the params
...
class AutoLinkView(AbstractButtonView):
    ...
    def process(self, item: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        path_params = {k: matched_params[k] for k in path_param_names if k in matched_params}
        query_params = {k: matched_params[k] for k in query_param_names if k in matched_params}
    ...
    def _match_params(self, param_names: List[str], item: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Match route parameters to item columns.

        Args:
            param_names (List[str]): Names of the parameters from the endpoint.
        ...
        for name in param_names:
            value = self._resolve_param_value(name, item, default=None)
            if value is not None:
