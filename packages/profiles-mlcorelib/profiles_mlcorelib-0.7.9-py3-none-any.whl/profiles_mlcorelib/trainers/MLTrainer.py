...
# Patch for psutil in environments where /proc/meminfo doesn't exist (like Snowflake UDFs)
def patched_virtual_memory():
    # Return a dummy svmem object with reasonable defaults
...
# Apply the patch only if we're likely in a Snowflake environment
if not os.path.exists("/proc/meminfo") and hasattr(psutil, "virtual_memory"):
