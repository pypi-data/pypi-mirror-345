...
def useInputAsCode(cmd):
    ...
    for num,i in enumerate(codes):
        msg=f"{Fore.cyan}{num}{Fore.yellow}/{num+1} of {Fore.light_magenta}{ct} -> {Fore.light_green}{i}('{db.Entry.cfmt(None,codes[i])}')"
        helpText.append(msg)
        keys.append(i)
    helpText='\n'.join(helpText)
    print(helpText)
...
{Fore.light_red}Tax=(({Fore.cyan}CRV+{Fore.light_blue}Price)*{Fore.light_magenta}(Sales Tax Rate(0.0925))){Style.reset}
                ...
                with db.Session(db.ENGINE) as session:
                    ...
                    for num,i in enumerate(results):
                        ...
                        crv+=(round(i.CRV,3)*total)
                        tax+=(round(i.Tax,3)*total)
                        if tax == 0 and crv > 0:
                            tax_crv=(round(i.CRV,3)*total)
                        else:
                            tax_crv+=((round(i.Tax,3)*total)+(round(i.CRV,3)*total))
                        master_total_tax+=tax
                        master_total_crv+=crv
                        master_total_tax_crv+=tax_crv
                        tax_crv=round(tax_crv,3)
