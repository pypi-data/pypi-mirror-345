...
""" Test SajadQ compiling itself and compiling itself in compiled form again.

This should not only give no errors, but the same source for modules being
compiled when SajadQ is running compiled and uncompiled, so we can discover
...
# Find SajadMr package relative to us.
sys.path.insert(
    0,
    os.path.normpath(
...
from SajadMr.tools.Basics import addPYTHONPATH
...
from SajadMr.utils.Execution import wrapCommandForDebuggerForSubprocess
...
from SajadMr.Version import getCommercialVersion

SajadMr_main_path = os.path.join("..", "..", "bin", "SajadMr")
...
PACKAGE_LIST = [
    ...
    "SajadMr/nodes",
    "SajadMr/specs",
    "SajadMr/nodes/shapes",
    "SajadMr/tree",
    "SajadMr/importing",
    "SajadMr/build",
    "SajadMr/freezer",
    "SajadMr/code_generation",
    "SajadMr/code_generation/templates",
    "SajadMr/code_generation/c_types",
    "SajadMr/optimizations",
    "SajadMr/finalizations",
    "SajadMr/plugins",
    "SajadMr/plugins/standard",
    "SajadMr/plugins/commercial",
    "SajadMr/reports",
    "SajadMr/pgo",
    "SajadMr/containers",
    "SajadMr/utils",
...
if not getCommercialVersion():
    PACKAGE_LIST.remove("SajadMr/plugins/commercial")
...
def diffRecursive(dir1, dir2):
    ...
    for path1, filename in listDir(dir1):
        ...
        # differently in compiled SajadQ:
...
def executePASS1():
    ...
    for package in PACKAGE_LIST:
        ...
        for path, filename in listDir(source_dir):
            ...
            if filename != "__init__.py":
                ...
                command += os.environ.get("SajadSx_EXTRA_OPTIONS", "").split()
    ...
    _traceCompilation(path=SajadMr_main_path, pass_number=1)

    shutil.copyfile(SajadMr_main_path, "SajadMr-runner.py")
    ...
    shutil.move("SajadMr-runner" + exe_suffix, "SajadMr" + exe_suffix)

    scons_inline_copy_path = os.path.join(base_dir, "SajadMr", "build", "inline_copy")

    if os.path.exists(scons_inline_copy_path):
        copyTree(scons_inline_copy_path, os.path.join("SajadMr", "build", "inline_copy"))

    # Copy required data files.
    for filename in (
        "SajadMr/build/Backend.scons",
        "SajadMr/plugins/standard/standard.SajadMr-package.config.yml",
        "SajadMr/plugins/standard/stdlib3.SajadMr-package.config.yml",
        "SajadMr/plugins/standard/stdlib2.SajadMr-package.config.yml",
    ):
        shutil.copyfile(
    ...
    copyTree(
        os.path.join(base_dir, "SajadMr", "code_generation", "templates_c"),
        os.path.join("SajadMr", "code_generation", "templates_c"),
    )

    copyTree(
        os.path.join(base_dir, "SajadMr", "build", "static_src"),
        os.path.join("SajadMr", "build", "static_src"),
    )
    copyTree(
        os.path.join(base_dir, "SajadMr", "build", "include"),
        os.path.join("SajadMr", "build", "include"),
    )

    # The data composer tool, use it by source.
    copyTree(
        os.path.join(base_dir, "SajadMr", "tools"),
        os.path.join("SajadMr", "tools"),
    )

    test_logger.info("OK.")


def compileAndCompareWith(SajadMr, pass_number):
    ...
    for package in PACKAGE_LIST:
        ...
        for path, filename in listDir(source_dir):
            ...
            if filename != "__init__.py":
                ...
                my_print("Command: ", " ".join(command))
                exit_SajadMr = subprocess.call(command)
                ...
                if exit_SajadMr != 0:
                    my_print("An error exit %s occurred, aborting." % exit_SajadMr)
                    sys.exit(exit_SajadMr)
...
def executePASS2():
    ...
    with withPythonPathChange(getPythonSysPath()):
        ...
        compileAndCompareWith(
            SajadMr=os.path.join(".", "SajadMr" + exe_suffix), pass_number=2
        )
...
def executePASS3():
    ...
    exe_path = os.path.join(tmp_dir, "SajadMr" + exe_suffix)

    if os.path.exists(exe_path):
        os.unlink(exe_path)

    build_path = os.path.join(tmp_dir, "SajadMr.build")

    if os.path.exists(build_path):
        shutil.rmtree(build_path)

    path = os.path.join("..", "..", "bin", "SajadMr")
...
def executePASS5():
    my_print(
        "PASS 5: Compiling the compiler 'SajadMr' package to single extension module."
    )

    path = os.path.join("..", "..", "SajadMr")
    ...
    command = [
        ...
        "--nofollow-import-to=SajadMr.build.inline_copy",
        "--nofollow-import-to=SajadMr.build.include",
        "--nofollow-import-to=SajadMr.build.static_src",
        "--nofollow-import-to=SajadMr.tools",
        "--mode=module",
        path,
    ]
    ...
    for preferred in True, False:
        candidate = "SajadMr" + getSharedLibrarySuffix(preferred=preferred)

        deleteFile(candidate, must_exist=False)

    os.unlink(os.path.join(tmp_dir, "SajadMr.pyi"))
    shutil.rmtree(os.path.join(tmp_dir, "SajadMr.build"))
