...
class PEM_H2_Clusters:
    ...
    def find_eol_voltage_curve(self, eol_eff_percent_loss):
        ...
        i_eol_no_faradaic_loss = (h2_eol * 1000 * 2 * self.F * self.moles_per_g_h2) / (
    ...
    def full_degradation(self, voltage_signal):
        ...
        self.output_dict["Cumulative Degradation Breakdown"] = pd.DataFrame(
            {
                "Uptime": np.cumsum(V_deg_uptime),
                "On/off": np.cumsum(V_deg_onoff),
                "Fatigue": V_fatigue,
            }
        )
    ...
    def approx_fatigue_degradation(self, voltage_signal, dt_fatigue_calc_hrs=168):
        ...
        if v_max == v_min:
            ...
            for i in range(len(t_calc) - 1):
                voltage_signal_temp = voltage_signal[
                    np.nonzero(voltage_signal[t_calc[i] : t_calc[i + 1]])
                ]
    ...
    def find_eol_voltage_val(self, eol_rated_eff_drop_percent):
        # TODO: change this
        rated_power_idx = (
            self.output_dict["BOL Efficiency Curve Info"]
            .index[
    ...
    def make_BOL_efficiency_curve(self):
        ...
        data = pd.DataFrame(
            {
                "Power Sent [kWh]": power_in_signal,
                "Current": stack_I,
                "Cell Voltage": stack_V,
                "H2 Produced": h2_stack_kg,
                "Efficiency [kWh/kg]": kWh_per_kg,
                "Power Consumed [kWh]": power_used_signal,
                "IV Curve BOL Error [kWh]": power_error_BOL,
            }
        )
    ...
    def iv_curve(self):
        ...
        current_range = np.arange(
            self.stack_input_current_lower_bound, self.max_cell_current + 10, 10
        )
    ...
    def cell_design(self, Stack_T, Stack_Current):
        ...
        V_ohmic = self.calc_V_ohmic(
            Stack_T, Stack_Current, self.cell_active_area, self.membrane_thickness
        )
    ...
    def calc_reversible_cell_voltage(self, Stack_T):
        ...
        E_cell = E_rev0 + ((self.R * T_K) / (2 * self.F)) * (
            np.log(
                ((panode_atm - p_h20_sat_atm) / patmo_atm)
                * np.sqrt((pcathode_atm - p_h20_sat_atm) / patmo_atm)
            )
        )
        return E_cell
    ...
    def h2_production_rate(self, stack_current, n_stacks_op):
        ...
        h2_produced_kg_hr = h2_production_rate_g_s * (
            self.dt / 1000
        )  # 3.6 #Fixed: no more manual scaling
...
if __name__ == "__main__":
    ...
    power_signal_kW_ramp_updown = np.concatenate(
        (power_signal_kW_ramp_down, power_signal_kW_ramp_up)
    )
    ...
    grid_RS = np.array(
        list(
            h2_results_aggregates_grid["Performance By Year"][
                "Refurbishment Schedule [MW replaced/year]"
            ].values()
        )
    )
    ...
    year_of_operation = list(
        h2_results_aggregates["Performance By Year"]["Capacity Factor [-]"].keys()
    )
    ...
    ax[1].vlines(
        offgrid_YOR,
        np.zeros(len(offgrid_YOR)),
        offgrid_AEU[offgrid_YOR],
        colors="skyblue",
        ls="-",
        lw=0.5,
        label="off-grid stack replacement",
    )
    ax[1].vlines(
        grid_YOR,
        np.zeros(len(grid_YOR)),
        ...
        ls="--",
        lw=0.5,
        label="grid-connected stack replacement",
    )
    ...
    ax[2].set_ylim(
        (
            0.95 * np.min([offgrid_AACE, grid_AACE]),
            1.05 * np.max([offgrid_AACE, grid_AACE]),
        )
    )
    fig.tight_layout()
