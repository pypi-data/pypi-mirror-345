...
class NormalizableValue:
    ...
    def __init__(
        ...
        if not value_is_normalized and not NumberValidator.is_number_between(value, range[0], range[1]):
            raise Exception('The provided "value" is out of the given "range".')
        elif value_is_normalized and not NumberValidator.is_number_between(value, 0, 1):
