...
class CosimSlaveInfo(Structure):
    ...
    _fields_ = [("name", c_char * CosimConstants.SLAVE_NAME_MAX_SIZE), ("index", c_int)]
...
class CosimSlaveVariableDescription(Structure):
    ...
    _fields_ = [
        ("name", c_char * CosimConstants.SLAVE_NAME_MAX_SIZE),
        ("reference", c_uint32),
        ("type", c_int),
        ("causality", c_int),
        ("variability", c_int),
    ...
    def __repr__(self):
        return "name: {0}, reference: {1}, type: {2}, causality: {3}, variability: {4}".format(
            self.name,
            self.reference,
            CosimEnums.CosimVariableType(self.type),
