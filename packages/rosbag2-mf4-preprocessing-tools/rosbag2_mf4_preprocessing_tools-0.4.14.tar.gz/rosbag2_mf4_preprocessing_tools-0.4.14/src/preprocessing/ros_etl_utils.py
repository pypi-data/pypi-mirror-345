...
def get_all_fields(typename: str, typestore: Any, current_prefix: str = '', visited: Optional[Set[str]] = None) -> List[Tuple[str, str, bool]]:
    ...
    try:
        if typename not in typestore.types:
            ...
            task_logger.warning(f"Type '{typename}' not found in typestore. Known types sample: {known_types_sample}...")
            return []

        field_defs = typestore.types[typename][1]

    except KeyError:
        task_logger.warning(f"Type '{typename}' definition not found in typestore during field recursion.")
        return []
    except IndexError:
        task_logger.warning(f"Unexpected structure for type '{typename}' definition in typestore. Cannot access fields.")
        return []
    except Exception as e:
        task_logger.error(f"Unexpected error accessing definition for type '{typename}': {e}", exc_info=True)
        return []


    for field_name, field_type_info in field_defs.items():
        flat_name = f"{current_prefix}{field_name}"
        is_array = False
        element_type_name = None

        if isinstance(field_type_info, tuple):
            if len(field_type_info) > 0:
                base_type_info = field_type_info[0]
                if isinstance(base_type_info, tuple):
                    if len(base_type_info)>0 and isinstance(base_type_info[0], str):
                        element_type_name = base_type_info[0]
                    is_array = True
                elif isinstance(base_type_info, str):
                    element_type_name = base_type_info
                    is_array = True
                elif field_type_info[0] == 'string' and len(field_type_info) > 1:
                    element_type_name = 'string'
                    is_array = False

        elif isinstance(field_type_info, str):
            element_type_name = field_type_info
            is_array = False

        if element_type_name is None:
            task_logger.warning(f"Could not determine element type for field '{field_name}' (type: {typename}, info: {field_type_info}). Skipping.")
            continue

        is_complex = element_type_name in typestore.types and hasattr(typestore.types[element_type_name][1], 'items')
        ...
        if is_complex:
            ...
            if nested_fields:
                if is_array:
                    task_logger.warning(f"Field '{flat_name}' is an array of complex type '{element_type_name}'. Storing arrays of complex types is not directly supported in this flattened structure. Skipping detailed fields.")
                else:
            ...
            ros_type_name = element_type_name
            fields_list.append((flat_name, ros_type_name, is_array))
