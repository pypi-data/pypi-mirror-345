...
class Logger(object):
    ...
    def log(self, message: str, *args: List[str], prefix: str = "", level: int = 1):
        ...
        if level > senchar.db.verbosity:
            return

        message = str(message)  # better for exceptions
        message = senchar.utils.dequote(message)
    ...
    def start_logging(
        ...
        if "2" in logtype:
            ...
            senchar.log(f"Logging to remote logger on port {port}")

        # rotating file handler
        if "3" in logtype:
            if logfile is None:
                if self.logfile is None:
                    raise senchar.exceptions.SencharError("no logfile specified")
            else:
