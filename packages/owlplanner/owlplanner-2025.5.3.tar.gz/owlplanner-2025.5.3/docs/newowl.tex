...
\chapter{Formulation optimizing asset allocation ratios}
This second case represents the case where the assets in each savings account $b_{ijkn}$ can
be optimized along a guiding allocation ratio.
If the allocation ratios $\alpha_{ijkn}$ are exactly prescribed,
no optimization is possible and this will result in simply adding the following constraint
\begin{equation}
	b_{ijkn} = \alpha_{ijkn} \sum_{k'} b_{ijk'n},
\end{equation}
which can be rewritten as
\begin{eqnarray}
	\label{Eq:Account}
	\sum_{k'}\delta(k, k') b_{ijkn} - \alpha_{ijkn} b_{ijk'n} &=& 0,\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall j \in \{0,\ldots, N_j-1\},\nonumber\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}.\nonumber
\end{eqnarray}
This allocation type will be termed {\it account}. A different approach is
to prescribe an allocation ratio for the sum of assets of a given individual.
This approach will be termed {\it individual} allocation.
In this case, we have $\alpha_{ikn}$ describing the allocation ratios
across all accounts $j$ of individual $i$. We then have
\begin{equation}
	\sum_{j} b_{ijkn} = \alpha_{ikn} \sum_{jk'} b_{ijk'n},
\end{equation}
which can be rewritten as
\begin{eqnarray}
	\label{Eq:Individual}
	\sum_{jk'}\delta(k, k') b_{ijkn} - \alpha_{ikn} b_{ijk'n} &=& 0,\\
	&&\qquad\forall i \in \{0,\ldots, N_i-1\},\nonumber\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}.\nonumber
\end{eqnarray}
Finally, a {\it coordinated} type prescribes the sum of all assets, irrespective
of the account type or spouse owner. This case involves ratios for type
of assets and year $\alpha_{kn}$ and is expressed as
\begin{equation}
	\sum_{ij} b_{ijkn} = \alpha_{kn} \sum_{ijk'} b_{ijk'n},
\end{equation}
which is rewritten as
\begin{eqnarray}
	\label{Eq:Coordinated}
	\sum_{ijk'}\delta(k, k') b_{ijkn} - \alpha_{kn} b_{ijk'n} &=& 0,\\
	&&\qquad\forall k \in \{0,\ldots, N_k-1\},\nonumber\\
	&&\qquad\forall n \in \{0,\ldots, N_n-1\}.\nonumber
\end{eqnarray}
These approaches have the inconvenience of needing many more decision variables
at the benefit of optimizing over allocation ratios over multiple accounts.

We are now ready to rewrite Eq.~\cite{Eq:C3b} as
\begin{eqnarray}
	\label{Eq:A_C3b}
	b_{ijk(n+1)} - b_{ikjn}(1 + \tau_{ijkn}) 
	&& \nonumber \\
	- \ [\delta(j, 0)d_{ikn} - w_{ijkn} + (\delta(j, 2) - \delta(j, 1))x_{ikn}] (1 + \mathcal{T}_{ijn})
	&=& \kappa_{ijn} (1 + \mathcal{T}_{ijn}/2).
\end{eqnarray}
...
The initial savings account balances are imposed through additional constraints as
\begin{eqnarray}
	\label{Eq:A_InitialBalance}
	\sum_k b_{ijk0} - \beta_{ij} = 0.
\end{eqnarray}
At this point, all accounts are balanced according to the desired
allocation ratios $\alpha_{ijkn}$ through the type of coordination selected above.
