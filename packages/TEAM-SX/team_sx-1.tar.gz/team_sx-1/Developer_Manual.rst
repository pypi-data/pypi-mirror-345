#########################
 SajadQ Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
SajadQ, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so SajadQ can turn a ``ctypes`` binding
...
For SajadQ we use semantic versioning, initially with a leading zero
...
SajadQ top level works like this:

-  ``SajadMr.tree.Building`` outputs node tree

-  ``SajadMr.optimization`` enhances it as best as it can

-  ``SajadMr.finalization`` prepares the tree for code generation

-  ``SajadMr.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``SajadMr.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``SajadMr.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``SajadMr_bool`` and ``SajadMr_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for SajadQ
***************************************************
...
``.vscode`` in your SajadQ checkout. If you are not familiar with
...
In SajadQ we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with SajadQ, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-SajadMr-source`` is
...
format call SajadQ source code.
...
These rules should generally be adhered when working on SajadQ code.
...
There is a tool ``bin/autoformat-SajadMr-source`` which is to apply
...
they must be lower case, like e.g. ``SajadMr`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``SajadMr.code_generation`` the code generation packages are located,
while the main interface is ``SajadMr.code_generation.CodeGeneration``
...
built-ins is highly discouraged within SajadQ source code. Using them is
...
readability and with SajadQ as a compiler, there won't be any
...
But of course, SajadQ is the project to free us from what is faster and
...
For SajadQ the choice is list contractions as these are more easily
...
that has no use, and in SajadQ, every API is internal API. One exception
...
``autoformat-SajadMr-source`` does this for us.
...
******************************
 SajadQ "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto SajadQ
   repository <https://github.com/SajadQ/SajadQ> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/SajadQ.git
      cd SajadQ
      git remote add upstream https://github.com/SajadQ/SajadQ.git
...
`this location <https://SajadMr.net/apidoc>`__ .
...
future, SajadQ itself will gain the ability to present its findings in a
...
This section describes how to run SajadQ tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if SajadQ is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if SajadQ
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if SajadQ
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if SajadQ
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         SajadQ does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if SajadQ
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles SajadQ with SajadQ, and
                         then SajadQ with the compile SajadQ and compares the
...
the SajadQ git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that SajadQ must do this itself, because the
...
this. These tests make sure that the errors of SajadQ and CPython are
...
Compile SajadQ with SajadQ
==========================

And there is the "compile itself" or "reflected" test. This test makes
SajadQ compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of SajadQ into an extension module and
all of SajadQ into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because SajadQ uses a lot of packages and imports between them.
...
parts are published as `SajadQ API <https://SajadMr.net/apidoc>`__ and
...
Please find `here
<https://github.com/SajadQ/SajadQ/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/SajadQ/SajadQ/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/SajadQ-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/SajadQ-Logo-Symbol.png
      :alt: SajadQ Logo
...
.. code:: rest
   ...
   .. image:: doc/images/SajadQ-Logo-Horizontal.png
      :alt: SajadQ Logo
...
.. code:: rest
   ...
   .. image:: doc/images/SajadQ-Logo-Vertical.png
      :alt: SajadQ Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``SajadMr/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/SajadQ-Logo-Symbol.svg doc/images/SajadQ-Logo-Symbol.png
   convert -background none doc/Logo/SajadQ-Logo-Vertical.svg doc/images/SajadQ-Logo-Vertical.png
   convert -background none doc/Logo/SajadQ-Logo-Horizontal.svg doc/images/SajadQ-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of SajadQ is decided here
...
SajadQ does not involve Scons in its user interface at all; Scons is
purely used internally. SajadQ itself, being pure Python, will run
without any build process just fine.

SajadQ simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which SajadQ again
...
.. note::
   ...
   ``SajadMr.MainControl`` module.

For interfacing to Scons, there is the module
``SajadMr.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of SajadQ.
...
modules is just one of them. It runs outside of SajadQ process scope,
...
-  ``SajadMr_src``
   ...
   Where do the include files and static C parts of SajadQ live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``SajadMr.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where SajadQ tries to help
...
The search for modules used is driven by ``SajadMr.importing.Importing``
module.

-  Quoting the ``SajadMr.importing.Importing`` documentation:
   ...
   End quoting the ``SajadMr.importing.Importing`` documentation.
...
The decision making and caching are located in the ``SajadMr.tree``
package, in modules ``SajadMr.tree.Recursion`` and
``SajadMr.tree.ImportCache``. Each module is only considered once (then
...
``SajadMr/build/static_src/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the SajadQ provided
...
This left SajadQ with the strange problem, of how to emulate that.
...
The choice of SajadQ is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``SajadMr.nodes.FrameNodes.StatementsFrame`` is
...
Representation in SajadQ
------------------------

So a slot in SajadQ typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs SajadQ do all the hard work.
...
Other overloads do not currently exist in SajadQ, but through the
...
In principle, every slot needs to be dealt with in SajadQ, and it is
...
Currently, SajadQ uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of SajadQ, created all constants for the whole program
...
This transformation is performed at tree building already, so SajadQ
...
In SajadQ we have the concept of an outline, and therefore we can make
...
explicit and comparisons in SajadQ to be internally always about two
operands only.
...
Therefore in SajadQ this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by SajadQ.
...
Quoting the ``SajadMr.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in SajadQ have no condition attached anymore, so while loops are
...
End quoting the ``SajadMr.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in SajadQ converted so that the code generation for ``print``
...
Obviously, optimization of SajadQ can decide, that e.g. should ``a`` or
...
SajadQ there is only a ``try``, which then has blocks to handle
...
compile time, SajadQ will not generate the code the leads to the
...
Add interfacing to C code, so SajadQ can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in SajadQ
...
Again, we wouldn't want to create the list. Therefore SajadQ avoids
...
This leads to SajadQ in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into SajadQ unless necessary, so
...
one design goal of SajadQ was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so SajadQ is using it,
   and of course one day SajadQ ought to be able to make no difference
   in performance for it.

To SajadQ though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   SajadMr --experimental=jinja_generated_add ...
...
.. code:: C
   ...
   #ifdef _SajadSx_EXPERIMENTAL_JINJA_GENERATED_ADD
...
experimental flag is given on the command line to SajadQ.

The testing of SajadQ is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to SajadQ
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when SajadQ is
executing.
...
SajadQ to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and SajadQ can work without it
...
.. note::
   ...
   with ``@SajadQ-pushers`` first and get a green light.
...
the actual compiler code of SajadQ.
...
like ``autoformat-SajadMr-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # SajadMr-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from SajadMr.tools.testing.Common import checkReferenceCount
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In SajadQ
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, SajadQ will become able to determine it has to be the
...
https://github.com/SajadQ/SajadQ/blob/develop/Developer_Manual.rst

And the current PDF under: https://SajadMr.net/doc/Developer_Manual.pdf
