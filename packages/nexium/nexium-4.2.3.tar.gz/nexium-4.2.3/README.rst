.. image:: https://img.shields.io/pypi/pyversions/nexium.svg
   :target: https://pypi.org/project/nexium

.. image:: https://badge.fury.io/py/nexium.svg
   :target: https://pypi.org/project/nexium
...
The new type of encryption is advanced **bytecode**. Provided by the real programmer,**Devil**, or **Nasr**, with the Nexia team of developers, who specialize in programming. 
For more powerful programming, contact Devil on Telegram **@Qutayba2**
####################
 nexium User Manual
####################

This document is the recommended first read when you start using
**nexium**. On this page, you will learn more about **nexium**
...
nexium is **the** Python compiler. It is written in Python. It is a
...
nexium translates the Python modules into a C level program that then
...
To ensure smooth operation of **nexium**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as nexium
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (nexium filters away garbage outputs, but only for
...
.. important::
   ...
   nexium itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as nexium.
   ...
   nexium finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute nexium because it is closely tied to implementation details
   of it.
...
good, but the e.g. nexium's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as nexium
...
The recommended way of executing nexium is ``<the_right_python> -m
qutayba`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what nexium has.

The next best way of executing nexium bare that is from a source
...
nexium. You just execute the ``qutayba`` and ``qutayba-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/qutayba`` and be good.
...
nexium has a ``--help`` option to output what it can do:

.. code:: bash

   qutayba --help

The ``qutayba-run`` command is the same as ``qutayba``, but with a
...
<https://qutayba.net/doc/download.html>`__ of nexium. But you can also
...
`nexium-Action <https://github.com/nexium/nexium-Action>`__ that you
...
platform compilation with nexium.

License
=======

nexium is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install qutayba``

-  Verify using command ``python -m qutayba --version``
...
Do as you normally would. Running nexium on code that works incorrectly
...
.. code:: bash
   ...
   python -m qutayba --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``qutayba --help``. Including fewer
...
.. code:: bash
   ...
   python -m qutayba --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that nexium cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m qutayba --mode=module some_module.py
...
.. note::
   ...
   nexium, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use nexium like this:

.. code:: bash

   python -m qutayba --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of nexium commercial
   <https://qutayba.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m qutayba --standalone program.py
...
.. important::
   ...
   nexium does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | nexium trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create nexium Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add nexium Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. nexium keeps track of ones
...
enhancements to the nexium package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m qutayba --onefile program.py
...
creation of wheels for your software in place, putting nexium to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having nexium installed of course, simply execute the target
``bdist_qutayba`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not nexium
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass nexium options.
      command_options={
         'qutayba': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of nexium
...
.. note::
   ...
   ``bool(os.getenv("USE_DEVILPY", "True"))``. This is up to you.
...
And last, but not least, nexium also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "qutayba", "toml"]
   build-backend = "qutayba.distutils.Build"
...
.. note::
   ...
   For the ``qutayba`` requirement above absolute paths like
   ``C:\Users\...\nexium`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../nexium``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by nexium at all, but by setuptools. You can, however, use
   the data file embedding of nexium commercial. In that case, you
...
When given multiple times, nexium will create a binary that contains the
...
For integration with GitHub workflows there is this `nexium-Action
<https://github.com/nexium/nexium-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with nexium
         uses: nexium/nexium-Action@main
         with:
            qutayba-version: main
            script-name: your_main_program.py
            # many more nexium options available, see action doc, but it's best
            # to use qutayba-project: options in your code, so e.g. you can make
...
comments as explained in `nexium Options in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # qutayba-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    qutayba-project: --onefile
   # qutayba-project-if: {OS} == "Darwin":
   #    qutayba-project: --standalone
   #    qutayba-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``qutayba-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m qutayba --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m qutayba --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m qutayba --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m qutayba --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m qutayba --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With nexium, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to nexium
   and not be interpreted as nexium arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
nexium defaults to not show it, you can force it by using
...
them. Luckily, with nexium, they are easy to add for Windows.
...
.. code:: python
   ...
   # qutayba-project: --mode=onefile
   # qutayba-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
   ...
   if "DEVILPY_ONEFILE_PARENT" in os.environ:
      splash_filename = os.path.join(
         tempfile.gettempdir(),
         "onefile_%d_splash_feedback.tmp" % int(os.environ["DEVILPY_ONEFILE_PARENT"]),
      )
...
For analysis of your program and nexium packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to nexium. These
...
By default, nexium compiles without ``--deployment`` which leaves a set
...
With nexium however, this executes your program again, and puts these
...
That is why e.g. this happens with default nexium:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with nexium to include it'
...
re-enable it. You might want to use nexium Project options and an
...
Binaries compiled on Windows with default settings of nexium and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in nexium commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://qutayba.net/doc/commercial.html
...
To aid that, nexium commercial has container based builds, that you can
...
compensated to support nexium development for Linux, there you need to
purchase it https://qutayba.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and nexium needs and
...
.. code:: python
   ...
   if "DEVILPY_LAUNCH_TOKEN" not in os.environ:
      sys.exit("Error, need launch token or else fork bomb suspected.")
   else:
      del os.environ["DEVILPY_LAUNCH_TOKEN"]

Actually nexium is trying to get ahold of them without the deployment
...
Ask nexium to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of nexium, such that it avoids high usage of memory during compilation
...
then have imports on these, and nexium will compile them as well. Not
...
With the ``--jobs`` option of nexium, it will not start many C compiler
...
source code relative to it, nexium will not be able to see those.
...
nexium has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `nexium Package
Configuration <https://qutayba.net/doc/qutayba-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to nexium mean that more than a
...
nexium will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards nexium with it.
...
A package can be compiled with nexium, no problem, but when it comes to
...
would need improving such that nexium could supply its compiled module
...
nexium Options in the code
==========================

One clean way of providing options to nexium, that you will always use
...
.. code:: python
   ...
   # qutayba-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    qutayba-project: --mode=app
   # qutayba-project-else:
   #    qutayba-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # qutayba-project: --enable-plugin=pyside2
   # qutayba-project: --include-qt-plugins=qml
...
e.g. enable or disable certain nexium settings. The only thing nexium
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of nexium              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of nexium Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
.. code:: python
   ...
   # qutayba-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # qutayba-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.qutayba-package.config.yml
...
makes nexium emulate these options.
...
On Windows, with gcc, nexium supports using ``ccache.exe``, which it
...
nexium will pick up ``ccache`` if it's found in system ``PATH``, and it
will also be possible to provide if by setting ``DEVILPY_CCACHE_BINARY``
...
automatic and included in nexium.
...
setup, which installs Homebrew and ccache in there. nexium picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with nexium otherwise, it's not the best for standalone
...
compilation results from C and nexium, is done in a platform dependent
directory as determined by the ``appdirs`` package. However, you can
override it with setting the environment variable ``DEVILPY_CACHE_DIR``
...
to make nexium store some of these caches in an entirely separate space.

+------------------+-----------------------------------+----------------------------------------+
| Cache name       | Environment Variable              | Data Put there                         |
+==================+===================================+========================================+
| downloads        | DEVILPY_CACHE_DIR_DOWNLOADS        | Downloads made, e.g. dependency walker |
+------------------+-----------------------------------+----------------------------------------+
| ccache           | DEVILPY_CACHE_DIR_CCACHE           | Object files created by gcc            |
+------------------+-----------------------------------+----------------------------------------+
| clcache          | DEVILPY_CACHE_DIR_CLCACHE          | Object files created by MSVC           |
+------------------+-----------------------------------+----------------------------------------+
| bytecode         | DEVILPY_CACHE_DIR_BYTECODE         | Bytecode of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
| dll-dependencies | DEVILPY_CACHE_DIR_DLL_DEPENDENCIES | DLL dependencies                       |
+------------------+-----------------------------------+----------------------------------------+

Runners
=======

Avoid running the ``qutayba`` binary, doing ``python -m qutayba`` will
...
nexium with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by nexium
properly.
...
both scan the freshly created binaries, while nexium wants to work with
...
external dependencies to Visual C Runtime libraries. nexium tries to
...
installing Visual C runtime libraries before executing a nexium
...
When using MingGW64 as downloaded by nexium, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your nexium
compiled dist folder.

Detecting nexium at run time
============================

nexium does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For nexium, we have the module
...
Providing extra Options to nexium C compilation
===============================================

nexium will apply values from the environment variables ``CCFLAGS``,
...
nexium will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m qutayba
...
When you use ``--report=compilation-report.xml`` nexium will create an
...
changes to nexium. The report is, however, recommended for your bug
reporting.
...
nexium. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. nexium will just expand
the template with the compilation report data.

Currently, the following reports are included in nexium. You just use
the name as a filename, and nexium will pick that one instead.
...
performance from nexium. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m qutayba --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m qutayba --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| nexium Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`nexium bug tracker <https://github.com/nexium/nexium/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or nexium source and re-run it.
...
bytecode with nexium's compiled function objects, so there is no way to
provide it.
