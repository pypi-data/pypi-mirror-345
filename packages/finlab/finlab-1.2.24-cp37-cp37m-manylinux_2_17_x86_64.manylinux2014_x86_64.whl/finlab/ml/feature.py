...
from finlab.market import MarketSharedMemory, Market
...
class Factory(Protocol):
    ...
    def generate_feature_names(self, func:str, lb:int, ub:int, n:int) -> List[str]:
        return []
...
def ta_names(lb:int=1, ub:int=10, n:int=1, factory:Optional[Factory]=None) -> List[str]:
    """
...
def create_feature(params):
    ...
    org_market = config.get_market()
    market = MarketSharedMemory.from_args(*params[6:])
    config.set_market(market)
    func, output, params = IndicatorName.decode(name)

    factory = factories[name.split('.')[0]]
    try:
        f = resampler(factory.calculate_indicator(func, output, params, adj=adj), resample, **kwargs).T.unstack()
...
def ta(feature_names:Optional[List[str]], 
    ...
    if cpu == 1:
        ...
        market = MarketSharedMemory(config.get_market(), adj=adj, start_time=start_time, end_time=end_time)
        config.set_market(market)

    test_f = resampler(TalibIndicatorFactory().calculate_indicator("RSI", 'real', {}, adj=adj), 
    ...
    def create_features() -> Generator[np.ndarray, None, None]:
        ...
        if cpu == 1:
            for name in feature_names:
                ...
                factory = factories[name.split('.')[0]]
                values = resampler(factory.calculate_indicator(func, output, params, adj=adj), resample, **kwargs).T.unstack()
