...
class BatchRequest(TerakeetRequest):
    ...
    def combine_inputs_and_params(self) -> list:
        ...
        print(f"{job_params.keys()=}")
        print(f"{len(self.input_objects)=}")
        ...
        print(f"{len(request_parameters)=}")
    ...
    def canonicalize_and_validate(self) -> None:
        ...
        logger.info(f"Valid URLs: {self.input_objects}")
        logger.info(f"Invalid URLs: {self.invalid_urls}")
    ...
    def canonicalize_url(url: str) -> dict:
        ...
        urls = normalizer.get_canonicalized_url()
        data = {
            "original_url": urls["original_url"],
            "normalized_url": urls["normalized_url"],
            "unique_params": urls["unique_params"],
