...
SajadQ_Cell_SET(%(identifier)s, %(tmp_name)s);
"""
...
{
    PyObject *old = SajadQ_Cell_GET(%(identifier)s);
    SajadQ_Cell_SET(%(identifier)s, %(tmp_name)s);
    Py_XDECREF(old);
}
"""
...
assert(SajadQ_Cell_GET(%(identifier)s) == NULL);
SajadQ_Cell_SET(%(identifier)s, %(tmp_name)s);
"""
...
{
    PyObject *old = SajadQ_Cell_GET(%(identifier)s);
    SajadQ_Cell_SET(%(identifier)s, NULL);
    Py_XDECREF(old);
}
"""
...
%(tmp_name)s = SajadQ_Cell_GET(%(identifier)s);
"""
...
%(access_code)s
    ...
    UPDATE_STRING_DICT0((PyDictObject *)%(dict_name)s, (SajadQ_StringObject *)%(var_name)s, value);
} else {
...
static PyObject *%(accessor_function_name)s(PyThreadState *tstate) {
    ...
    if (moduledict_%(module_identifier)s->ma_version_tag == dict_version) {
        ...
        result = GET_STRING_DICT_VALUE(moduledict_%(module_identifier)s, (SajadQ_StringObject *)%(var_name)s);
        cache_value = result;
    }
#else
    ...
    if (likely(DK_IS_UNICODE(dk))) {
        ...
        uint32_t current_dk_version = _SajadQ_PyDictKeys_GetVersionForCurrentState(tstate->interp, dk);
#else
        uint32_t current_dk_version = _SajadQ_PyDictKeys_GetVersionForCurrentState(dk);
#endif

        if (current_dk_version != dict_keys_version) {
            dict_keys_version = current_dk_version;
            Py_hash_t hash = ((SajadQ_StringObject *)%(var_name)s)->_base._base.hash;
            assert(hash != -1);

            cache_dk_index = SajadQ_Py_unicodekeys_lookup_unicode(dk, %(var_name)s, hash);
        }
...
#else
    PyObject *result = GET_STRING_DICT_VALUE(moduledict_%(module_identifier)s, (SajadQ_StringObject *)%(var_name)s);
#endif

    if (unlikely(result == NULL)) {
        result = GET_STRING_DICT_VALUE(dict_builtin, (SajadQ_StringObject *)%(var_name)s);
    }
