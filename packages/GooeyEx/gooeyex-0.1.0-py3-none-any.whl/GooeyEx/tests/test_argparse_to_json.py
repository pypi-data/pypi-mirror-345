...
from GooeyEx.python_bindings import argparse_to_json
from GooeyEx.util.functional import getin
from GooeyEx.tests import *
from GooeyEx.gui.components.options.options import FileChooser
from GooeyEx.gui.components.widgets import FileSaver
...
class TestArgparse(unittest.TestCase):
    ...
    def test_listbox_defaults_cast_correctly(self):
        ...
        parser = GooeyParser()
        parser.add_argument(
            "--foo", widget="Listbox", nargs="*", choices=[1, 2, 3], default=[1, 2]
        )
    ...
    def test_listbox_single_default_cast_correctly(self):
        ...
        parser = GooeyParser()
        parser.add_argument(
            "--foo", widget="Listbox", nargs="*", choices=[1, 2, 3], default="sup"
        )
    ...
    def test_version_maps_to_checkbox(self):
        ...
        for args, kwargs, expectedType in testcases:
            with self.subTest([args, kwargs]):
                ...
                result = argparse_to_json.convert(
                    parser, required_cols=2, optional_cols=2
                )
    ...
    def test_textinput_with_list_default_mapped_to_cli_friendly_value(self):
        ...
        for case in testcases:
            with self.subTest(case):
                parser = ArgumentParser(prog="test_program")
                parser.add_argument(
                    "--foo", nargs=case["nargs"], default=case["default"]
                )
                action = parser._actions[-1]
                result = argparse_to_json.handle_initial_values(
                    action, case["w"], action.default
                )
