...
class ExtractEnrollmentLabels:
    def __init__(self):
        self.data1={f'{v[0]}_{v[1]}':v[1:] for v in ReadData('专业、大类以及逻辑代码').data}
        self.data2={v[1]:v[1:] for v in ReadData('专业、大类以及逻辑代码').data}
...
class UpdateName():
    ...
    """
    def __init__(self):
        school_names_change = ReadData('院校库', ['院校名称', 'school_name_change']).data
        self.school_names_change = []
        for school_new, school_old in school_names_change:
            for school_old in school_old.replace('与','、').split('、'):
                if school_new != school_old:
                    self.school_names_change.append((school_old,school_new))
        self.school_names_change = sorted(self.school_names_change, key=lambda x: len(x[0]), reverse=True)
...
school_names = lambda :list_dupl([school_name for x in ReadData('院校库').data for school_name in [x[1]] + x[2].split('、')])
...
英文表头对应中文字典 = {'year': '年份', 'school_name': '院校名称', 'school_ljdm': '院校逻辑代码', 'major_name': '专业名称',
    ...
    'school_code': '院校代码', 'major_code': '专业代码', 'maxfen': '最高分', 'pinfen': '平均分', 'minfen': '最低分',
    ...
    'leibie': '类别', 'school_region': '院校所在城市代码', 'school_city': '院校所在城市', 'banxuexingzhi': '办学性质',
    ...
    'showtab': '展示位置 0：普通批次展示 1：提前批展示 2：都展示', 'batch_xh': '批次排序', 'xuanke': '选科/再选',
