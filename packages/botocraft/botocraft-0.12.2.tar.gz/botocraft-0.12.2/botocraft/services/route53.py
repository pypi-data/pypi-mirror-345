...
class CidrBlockSummaryManager(Boto3ModelManager):
    ...
    def list(
        self, CollectionId: str, *, LocationName: Optional[str] = None
...
class Route53QueryLoggingConfigManager(Boto3ModelManager):
    ...
    def list(
        ...
        results: List["Route53QueryLoggingConfig"] = []
        for _response in response_iterator:
            if list(_response.keys()) == ["ResponseMetadata"]:
                break
            if "ResponseMetadata" in _response:
...
class HostedZone(HostedZoneModelMixin, PrimaryBoto3Model):
    ...
    @cached_property
    def records(self) -> Optional[List["Route53ResourceRecordSet"]]:
        """
    ...
    def update_comment(self, Comment: str) -> "UpdateHostedZoneCommentResponse":
        ...
        return (
            cast(HostedZoneManager, self.objects)
            .using(self.session)
            .update_comment(
                cast(str, self.Id),
                Comment,
...
class Route53GeoProximityLocation(Boto3Model):
    ...
    *GeoproximityLocation resource record sets only:* A complex type that lets you control how Route 53 responds to DNS
    ...
    """
    Coordinates: Optional[Route53Coordinates] = None
    """
    Contains the longitude and latitude for a geographic region.
    """
...
class CreateCidrCollectionResponse(Boto3Model):
    Collection: Optional[Route53CidrCollection] = None
    """
