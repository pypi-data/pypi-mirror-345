...
Py_ssize_t SajadQ_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
    ...
    return SajadQ_PyMapping_Size(sequence);
}

PyObject *SajadQ_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!SajadQ_Index_Check(item))) {
...
PyObject *SajadQ_Number_IndexAsLong(PyObject *item) {
    PyObject *result = SajadQ_Number_Index(item);
