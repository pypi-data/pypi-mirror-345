"""
Core buffering, persistence, and flushing logic for ObservAI.
...
def configure(
    *,
    ...
    """
    Initialize ObservAI buffering and persistence settings.
    """
...
def _write_batch_to_file(batch: List[Dict[str, Any]]) -> bool:
    try:
        ...
        _logger.exception("[ObservAI] file error", exc)
        return False
...
def _send_sync(batch: List[Dict[str, Any]]) -> bool:
    try:
        ...
        _logger.exception("[ObservAI] network error", exc)
        return False
