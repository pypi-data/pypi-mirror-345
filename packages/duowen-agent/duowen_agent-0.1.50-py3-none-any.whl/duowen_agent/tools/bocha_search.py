...
class BochaParameters(BaseModel):
    ...
    def validate_freshness(cls, v):
        ...
        if ".." in v:
            dates = v.split("..")
            if len(dates) != 2:
                raise ValueError("日期范围格式错误，使用 'YYYY-MM-DD..YYYY-MM-DD'")
            ...
            try:
                ...
                raise ValueError("非法日期格式，使用 YYYY-MM-DD")
                ...
                return [
                    {
                        "url": res["url"],
                        "title": res["name"],
                        "content": res.get("summary", None) or res.get("snippet", None),
                        "date_published": res["datePublished"],
                        "content_with_weight": f'URL:{res["url"]}\nTITLE: {res["name"]}\nDATE PUBLISHED: {res["datePublished"]}\nCONTENT: {res.get("summary", None) or res.get("snippet", None)}\n\n',
                    }
                    for res in response.json()["data"]["webPages"]["value"]
                ]
