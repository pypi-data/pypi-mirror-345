...
class IBTWSTConnector(BrokerConnector):
			...
			try:
				...
				for leg in order.legs:
					...
					try:
						...
						logger.error("Not price data for strike {} available!", leg.strike)
						return False
	...
	async def _requestMissingOptionData(self, symbolData: IBSymbolData, expirationDate: dt.date, atmStrike: float):
		...
		for strikePrice in strikes_of_interest:
			try:
				...
				if len(list_of_details) == 0:
					logger.info(f'No Options available at strike {strikePrice}')
					continue
