...
from senchar.plot import plt, update
...
def centroids(filename: str = ".", threshold: float = 500.0) -> None:
    ...
    if filename == ".":
        ...
        if reply is None:
            return
        filename = reply[0]
        senchar.db.parameters.set_local_par("centroids", "filename", filename)

    filename = senchar.utils.make_image_filename(filename)

    # debias the image
    senchar.fits.colbias(filename)

    # open FITS file
    fe55im = pyfits.open(filename)

    # make data buffer from FITS file
    im1 = senchar.image.Image(filename)
    im1.assemble(1)
    data = im1.buffer
    maxrows = len(data)
    maxcols = len(data[0])
