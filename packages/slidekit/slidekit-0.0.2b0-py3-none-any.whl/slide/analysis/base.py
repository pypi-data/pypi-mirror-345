...
class Analysis:
    ...
    def __init__(
        ...
        Args:
            ...
            annotations (Annotations): Annotations object containing terms and their associated labels.
        ...
        self.annotations_dict = self.annotations.to_dict()
    ...
    def run(
        ...
        Args:
            test (str, optional): Statistical test to use ('hypergeom' or 'poisson').
                Defaults to 'poisson'.
            null_distribution (str, optional): Basis for the null model ('input' or 'annotations').
                Defaults to 'input'.
            ...
            if max_workers > 1:
                ...
                with ctx.Pool(processes=max_workers) as pool, tqdm(
                    ...
                    for ws, job in zip(self.window_size, jobs):
                        ...
                        pbar.update(1)
                return Results(
                    results, self.input_container, self.annotations, start_index=start_index
                )
            else:
    ...
    def _run_for_window_size(
        ...
        Args:
            ...
            null_distribution (str): Basis for the null model ('input' or 'annotations').
        ...
        ann_mat = self.annotations.to_csr(label_universe=full_labels)

        # Run Stats against that full-background matrix
        stats = Stats(window_matrix=mat, annotations_matrix=ann_mat)
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "num_terms": len(self.annotations_dict),
            "window_labels_preview": [self.labels[i] for i in range(min(3, len(self.labels)))]
            + (["..."] if len(self.labels) > 3 else []),
            "annotation_terms_preview": list(self.annotations_dict.keys())[:3]
            + (["..."] if len(self.annotations_dict) > 3 else []),
        }
