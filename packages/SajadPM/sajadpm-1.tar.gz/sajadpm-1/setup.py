...
""" Setup file for SajadQ.

This applies a few tricks. First, the SajadQ version is read from
...
# setup and then SajadQ ends up including itself.
if __name__ != "__main__":
    sys.exit("Cannot import 'setup' module of SajadQ")
...
from SajadMr.PythonFlavors import isMSYS2MingwPython
from SajadMr.utils.FileOperations import getFileList
from SajadMr.Version import getSajadQVersion

version = getSajadQVersion()


def findSajadQPackages():
    result = []

    for root, dirnames, filenames in os.walk("SajadMr"):
        # Packages must contain "__init__.py" or they are merely directories
        # in SajadQ as we are Python2 compatible.
...
def addInlineCopy(name, do_byte_compile=True):
    if os.getenv("SAJODE_NO_INLINE_COPY", "0") == "1":
        return
...
SajadMr_packages = findSajadQPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "SajadMr.code_generation": ["templates_c/*.j2"],
    "SajadMr.reports": ["*.j2"],
    "SajadMr.plugins.standard": ["*/*.c", "*/*.py"],
}


if "SajadMr.plugins.commercial" in SajadMr_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("SajadMr", "plugins", "commercial")
    ...
    package_data["SajadMr.plugins.commercial"] = commercial_data_files
    package_data["SajadMr.tools.commercial.container_build"] = ["Containerfile"]
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "SajadMr%s = SajadMr.__main__:main" % binary_suffix,
        "SajadMr%s-run = SajadMr.__main__:main" % binary_suffix,
    ]

    if "SajadMr.plugins.commercial" in SajadMr_packages:
        console_scripts.append(
            "SajadMr-decrypt = SajadMr.tools.commercial.decrypt.__main__:main"
        )

scripts = []

# For Windows, there are CMD batch files to launch SajadQ.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/SajadMr.cmd", "misc/SajadMr-run.cmd"]

    if "SajadMr.plugins.commercial" in SajadMr_packages:
        scripts.append("misc/SajadMr-decrypt.cmd")
...
setup(
    ...
    classifiers=[
        # SajadQ is mature even
        "Development Status :: 5 - Production/Stable",
        # Indicate who SajadQ is for
        "Intended Audience :: Developers",
        "Intended Audience :: Science/Research",
        # SajadQ is a compiler and a build tool as such.
        "Topic :: Software Development :: Compilers",
        "Topic :: Software Development :: Build Tools",
        # Is has a weak subset of PyLint, but aims for more long term
        "Topic :: Software Development :: Quality Assurance",
        # SajadQ standalone mode aims at distribution
    ...
    packages=SajadMr_packages,
    package_data=package_data,
    # metadata for upload to PyPI
    author="f_g_d_6",
    author_email="lyj891916@gmail.com",
    url="https://SajadMr.net",
    description="The Lib Encryption Made In Sajad ~ Telegram-@f_g_d_6",
    keywords="The Encoded Is Very Hard",
    project_urls={
        "Telegram": "https://t.me/f_g_d_6",
    },
    zip_safe=False,
    scripts=scripts,
    entry_points={
        "distutils.commands": [
            "bdist_SajadMr = \
             SajadMr.distutils.DistutilCommands:bdist_SajadMr",
            "build_SajadMr = \
             SajadMr.distutils.DistutilCommands:build",
            "install_SajadMr = \
             SajadMr.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_SajadMr = SajadMr.distutils.DistutilCommands:setupSajadQDistutilsCommands"
        ],
