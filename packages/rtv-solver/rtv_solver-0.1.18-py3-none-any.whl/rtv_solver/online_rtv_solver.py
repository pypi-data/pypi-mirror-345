...
class OnlineRTVSolver:
    ...
    def evaluate_insertion(i, j, remaining_stops, pickup_stop, dropoff_stop, start_time, start_node, load, state, objective):
    ...
    def insert_request_to_driver_run(self, driver_run, request, objective="vmt"):
        ...
        for i in range(len(remaining_stops) + 1):
            for j in range(i + 1, len(remaining_stops) + 2):
                cost, manifest = OnlineRTVSolver.evaluate_insertion(i, j, remaining_stops, pickup_stop, dropoff_stop, start_time, start_node, load, state, objective)
                results.append((cost, manifest))
