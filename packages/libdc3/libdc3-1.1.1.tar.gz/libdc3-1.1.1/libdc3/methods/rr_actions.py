...
class RunRegistryActions:
    ...
    def fetch_runs(
        ...
        filter_dict = {"class": {"=": self.class_name}, "oms_attributes.b_field": {">=": b_field}}
        if run_list:
    ...
    def multi_count_offline_rr_lumis(self, run_list: list[int]):
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = executor.map(self.count_offline_rr_lumis, run_list)
    ...
    def multi_count_online_rr_lumis(self, run_list: list[int]):
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = executor.map(self.count_online_rr_lumis, run_list)
            results = [res for res in results]
        return {run_list[idx]: results[idx] for idx in range(len(run_list))}

    def multi_count_oms_lumis(self, run_list: list[int]):
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = executor.map(self.count_oms_lumis, run_list)
            results = [res for res in results]
        return {
    ...
    def multi_fetch_rr_oms_joint_lumis(self, run_list: list[int]):
        lumisections = []
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = executor.map(rr.get_joint_lumisection_ranges, run_list, [self.dataset_name for _ in run_list])
            results = [res for res in results]
