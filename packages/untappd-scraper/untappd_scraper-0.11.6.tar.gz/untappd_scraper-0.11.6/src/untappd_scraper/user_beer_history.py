...
def parse_web_user_history_beer(beer_item: Tag) -> WebUserHistoryBeer:
    ...
    label = img.get("src", "") if (img := beer_item.select_one("a img")) else ""

    # Brewery
    brewery_el = beer_item.select_one(".beer-details p.brewery a")
    brewery = str(brewery_el.text.strip() if brewery_el else "")
    brewery_slug = str(brewery_el["href"]).split("/")[-1] if brewery_el else ""
    ...
    return WebUserHistoryBeer(
        ...
        brewery_id=0,  # doesn't appear in scrape
        brewery=brewery,
        brewery_slug=brewery_slug,
        style=style,
        url=url,
        first_checkin=first_checkin or datetime.min,  # noqa: DTZ901
        first_checkin_id=first_checkin_id or 0,
        recent_checkin=recent_checkin or datetime.min,  # noqa: DTZ901
