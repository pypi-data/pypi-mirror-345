...
def test_activity(user: User) -> None:
    ...
    assert all(r.brewery_slug and "tba" not in r.brewery_slug.casefold() for r in result)
...
def test_beer_history(wardy_user: User) -> None:
    ...
    assert all(r.brewery_slug and r.brewery_slug != "TBA" for r in result.results)
