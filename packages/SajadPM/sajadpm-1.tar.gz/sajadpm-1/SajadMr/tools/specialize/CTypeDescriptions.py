...
from SajadMr.__past__ import long
from SajadMr.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from SajadMr.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "SajadQ_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "SajadQ_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getTypeComparisonSpecializationCode(
        ...
        if helper_target is target:
            ...
            return "return %s(%s, %s) ? SajadSx_BOOL_TRUE : SajadSx_BOOL_FALSE;" % (
                helper_name,
                operand1,
                operand2,
            )
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = SajadQ_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = SajadQ_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "SajadQ_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "SajadQ_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "SajadQ_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "SajadQ_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "SajadQ_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "SajadQ_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "SajadQ_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "SajadQ C bool value"
    type_decl = "SajadMr_bool"
    ...
    @staticmethod
    def getToValueFromBoolExpression(operand):
        return "%s ? SajadSx_BOOL_TRUE : SajadSx_BOOL_FALSE" % operand

    @classmethod
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match SajadMr_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "SajadQ int/long/C long value"
    type_decl = "SajadMr_ilong *"
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "SajadQ C void value"
    type_decl = "SajadMr_void"
