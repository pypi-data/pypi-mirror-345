...
class BaseGenerativeModel:
    def __init__(
        ...
        if system_prompt:
            self.prompt = self.prompt_formatter(system_prompt=system_prompt)
    def _get_llm(self):
            try:
    ...
    def prompt_formatter(
        ...
        """
        Format prompts into PromptTemplates.

        Args:
            human_prompt (str): Prompt for the planner agent.
            system_prompt (str): Base system prompt.

        Returns:
            tuple: (router_prompt, evaluator_prompt, reflector_prompt, planner_prompt) as PromptTemplate instances.
        """
        input_variables = ['question', 'history', 'current_time', 'useful_info']
        template = """
