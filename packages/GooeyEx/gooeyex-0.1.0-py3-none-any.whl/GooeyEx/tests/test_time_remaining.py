...
from GooeyEx.util.functional import identity
...
class TestFooterTimeRemaining(unittest.TestCase):
    ...
    def test_time_remaining_visibility(self):
        for testdata in self.testcases():
            with self.subTest(testdata):
                with instrumentGooey(self.make_parser(), timing_options=testdata) as (
                    app,
                    frame,
                    gapp,
                ):
    ...
    def testcases(self):
        ...
        """
        iterable = product(
            ["show_time_remaining", "hide_time_remaining_on_complete"], [True, False]
        )
