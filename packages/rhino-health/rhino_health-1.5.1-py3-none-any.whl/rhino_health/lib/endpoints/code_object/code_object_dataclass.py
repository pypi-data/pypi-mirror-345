...
class CodeObjectCreateInput(RhinoBaseModel):
    ...
    inputs_config: Optional[List[Union[CodeObjectInputConfig, str, None]]] = []
    """@autoapi True A list of input configs this CodeObject expects input datasets to adhere to."""
    outputs_config: Optional[List[Union[CodeObjectOutputConfig, str, None]]] = []
    """@autoapi True A list of input configs this CodeObject expects input datasets to adhere to."""
    ...
    def input_configs_xor_input_schema_uids(self):
        if self.inputs_config is None and self.input_data_schema_uids is None:
            raise ValueError("Cannot provide both input_configs and input_data_schema_uids")
        if not self.inputs_config is not None and not self.input_data_schema_uids is not None:
            raise ValueError("Must provide either input_configs or input_data_schema_uids")
        return self

    @model_validator(mode="after")
    def output_configs_xor_output_schema_uids(self):
        if self.outputs_config is None and self.output_data_schema_uids is None:
            raise ValueError("Cannot provide both output_configs and output_data_schema_uids")
        if not self.outputs_config is not None and not self.output_data_schema_uids is not None:
            raise ValueError("Must provide either output_configs or output_data_schema_uids")
        return self
