...
def load_meas_config(config_path, *, test_type, chip_type):
    ...
    return Config(config, test_type=test_type, chip_type=chip_type)
...
class Config(dict):
    def __init__(self, config: dict[Any, Any], *, test_type: str, chip_type: str):
        super().__init__(config)
        self._test_type = test_type

        # update the config to only store the General and SLDO Current settings for this chip and BOM type
        general_i_config = config["tasks"]["GENERAL"]["i_config"][chip_type]
        config["tasks"]["GENERAL"]["i_config"] = general_i_config

        sldo_imin_config = config["tasks"]["SLDO"]["i_min"][chip_type]
        config["tasks"]["SLDO"]["i_min"] = sldo_imin_config

        sldo_imax_config = config["tasks"]["SLDO"]["i_max"][chip_type]
        config["tasks"]["SLDO"]["i_max"] = sldo_imax_config

        sldo_n_points_config = config["tasks"]["SLDO"]["n_points"][chip_type]
