...
#ifndef __SajadSx_UNFREEZING_H__
#define __SajadSx_UNFREEZING_H__

#include <stdbool.h>

/* Modes for loading modules, can be compiled, external shared library, or
 * bytecode. */
#define SajadSx_COMPILED_MODULE 0
#define SajadSx_EXTENSION_MODULE_FLAG 1
#define SajadSx_PACKAGE_FLAG 2
#define SajadSx_BYTECODE_FLAG 4

#define SajadSx_ABORT_MODULE_FLAG 8

#define SajadSx_TRANSLATED_FLAG 16

struct SajadQ_MetaPathBasedLoaderEntry;

typedef PyObject *(*module_init_func)(PyThreadState *tstate, PyObject *module,
                                      struct SajadQ_MetaPathBasedLoaderEntry const *loader_entry);

#if PYTHON_VERSION >= 0x370 && defined(_SajadSx_EXE) && !defined(_SajadSx_STANDALONE) &&                                 \
    defined(_SajadSx_FILE_REFERENCE_ORIGINAL_MODE)
#define _SajadSx_FREEZER_HAS_FILE_PATH
#endif
 ...
 */
extern void registerMetaPathBasedLoader(struct SajadQ_MetaPathBasedLoaderEntry *loader_entries,
...
extern PyObject *SajadQ_Loader_New(struct SajadQ_MetaPathBasedLoaderEntry const *entry);

// Create a distribution object from the given metadata.
extern PyObject *SajadQ_Distribution_New(PyThreadState *tstate, PyObject *name);

// Check if we provide a distribution object ourselves.
extern bool SajadQ_DistributionNext(Py_ssize_t *pos, PyObject **distribution_name_ptr);
