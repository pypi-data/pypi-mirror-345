    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        if cached_value is not None:
            ...
            if include_data := parse_bool(request.query.get('data') or 'true'):
                ...
                volume_id = await _get_volume_by_aws_account_id(request, id_)
...
async def get_awsaccount_search_item(request: web.Request) -> web.Response:
    ...
    volume_id = await _get_volume_by_aws_account_id(request, account_id)
...
def main() -> None:
    ...
    start(package_name='heaserver-accounts', db=opensearch.S3WithOpenSearchManager,
          ...
          cleanup_ctx=[wait_on_coro_cleanup_ctx(coro=_listen_for_object_change, delay=30)], config=config)
...
async def _get_volume_by_aws_account_id(request: web.Request, aws_account_id: str) -> str | None:
    """
                ...
                for r in records:
                    if r.get('s3'):
                        ...
                        if 'bucket' in s3 and 'object' in s3:
                            ...
                            config_ids = s3['configurationId'].split("_")
                            ...
                            search_aws_obj.account_id = config_ids[1] if not inventory_event and config_ids and len(config_ids) > 1 else None
                            if search_aws_obj and search_aws_obj.event_name:
                                search_aws_obj.is_delete_marker =  "DeleteMarkerCreated" in search_aws_obj.event_name
                            else:
...
async def s3_event_handler(request: web.Request) -> web.Response:
    ...
    async with DesktopObjectActionLifecycle(request=request,
                                            code='hea-create',
                                            description=f'Creating s3 object entry in index in OpenSearch',
                                            activity_cb=publish_desktop_object) as activity:
        try:
            ...
            logger.info("S3 event listening is not configured: %s", str(e))
            raise response.status_not_found()
        try:
