...
class NeosphereAgent:
    ...
    def get_self_info(self):
        if not self.neosphere_client.contacts:
            return self.neosphere_client.contacts.get_or_add_self_contact()
        return {}
...
class NeosphereAgentRunner(object):
    ...
    async def dump_connection_status(self):
        while True and self.dump_connection_status_path:
            self_info = self.agent.get_self_info()
            dump = {
                'connected': self._is_connected,
                'listening': self._listening,
                'err_state': self._err_state,
                'agent_name': self_info.get('share_id', ''),
                'status': self_info.get('status', 'Unknown'),
                'public_agent': self_info.get('open_to_public', False),
                'allow_query': self_info.get('allow_query', False),
                'can_query': self_info.get('can_query', False),
                'allow_images': self_info.get('allow_images', False),
            }
            if self.dump_connection_status_path == 'stdout':
    ...
    async def run(self):
        while self._retry_count <= self.MAX_RETRIES:
            ...
            await asyncio.gather(self.ws_recv_loop(), self.ws_send_loop(), self.dump_connection_status())
