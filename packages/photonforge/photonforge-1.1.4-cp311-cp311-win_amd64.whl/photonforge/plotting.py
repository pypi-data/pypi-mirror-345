...
def plot_s_matrix(
    ...
    x: Literal["wavelength", "frequency"] = None,
    ...
    Args:
        ...
        show_progress: If ``True``, show S matrix computation progress.
        model_kwargs: Keyword arguments passed to the model for S matrix
          computation. Each model type can accept a different set of
          arguments. Check the documentation for the specific model for more
          information.
    ...
    if len(input_ports) == 0:
        input_ports = sorted(set(k[0] for k in s_matrix.elements))
    else:
        input_ports = sorted(
            set(
                k[0]
                for k in s_matrix.elements
                if any(k[0].startswith(p + "@") for p in input_ports)
            )
        )
    if len(output_ports) == 0:
        output_ports = sorted(set(k[1] for k in s_matrix.elements))
    else:
        output_ports = sorted(
            set(
                k[1]
                for k in s_matrix.elements
                if any(k[1].startswith(p + "@") for p in output_ports)
            )
        )
...
def tidy3d_plot(
    obj: Union[Component, Port, FiberPort, PortSpec],
    frequency: float = None,
    technology: Technology = None,
    plot_type: Optional[Literal["3d", "eps", "structures"]] = None,
    ...
    **kwargs,
    ...
    if plot_type == "3d" and hasattr(tidy3d_obj, "plot_3d"):
        ...
        if plot_grid:
            plot_grid = False
            warnings.warn("Grid plotting is not supported for 'plot_type == \"3d\".'")
