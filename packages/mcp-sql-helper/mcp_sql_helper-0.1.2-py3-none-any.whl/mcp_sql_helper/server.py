...
def get_engine(readonly=True, connection_string:str=''):
    if not connection_string:
        connection_string = os.environ.get('DB_URL')
        if not connection_string:
            raise ValueError("数据库连接字符串未提供。请设置 DB_URL 环境变量或直接传入 connection_string")
        
    get_logger(__name__).info(f"get_engine: {connection_string}")
...
#     engine = get_engine(readonly=True,connection_string=connection_string)
#     with engine.connect():
#         url = engine.url
#         result = [
...
VERSION = "2025.5.4.210242"
# DB_INFO = get_db_info()
EXECUTE_QUERY_MAX_CHARS = 4000 # int(os.environ.get('EXECUTE_QUERY_MAX_CHARS', 4000))
CLAUDE_LOCAL_FILES_PATH = '' # os.environ.get('CLAUDE_LOCAL_FILES_PATH')

### MCP ###

mcp = FastMCP(name="MCP Alchemy",port=8004)
get_logger(__name__).info(f"Starting MCP Alchemy version {VERSION}")

@mcp.tool(description=f"Return all table names in the database separated by comma. ")
def all_table_names(connection_string:str='') -> str:
    get_logger(__name__).info(f"connection_string: {connection_string}")
...
@mcp.tool(
    description=f"Return all table names in the database containing the substring 'q' separated by comma. "
)
def filter_table_names(q: str,connection_string:str='') -> str:
    engine = get_engine(connection_string=connection_string)
    inspector = inspect(engine)
    return ", ".join(x for x in inspector.get_table_names() if q in x)

@mcp.tool(description=f"Returns schema and relation information for the given tables. ")
def schema_definitions(table_names: list[str],connection_string:str='') -> str:
...
def execute_query(query: str, params: dict = {},connection_string:str='') -> str:
    ...
    def save_full_results(full_results,connection_string:str=''):
    ...
    try:
        engine = get_engine(readonly=False,connection_string=connection_string)
        with engine.connect() as connection:
            ...
            if full_results_message := save_full_results(full_results,connection_string=connection_string):
