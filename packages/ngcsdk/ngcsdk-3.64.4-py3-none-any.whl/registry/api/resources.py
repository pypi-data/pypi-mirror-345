...
class ResourceAPI:  # noqa: D101
    ...
    def download_version(
        ...
        if not mrt.version:
            mrt.version = recipe_response.recipe.latestVersionIdStr
...
class GuestResourceAPI(ResourceAPI):  # noqa: D101
    ...
    def download_version(
        ...
        """Download the specified model version from a Model Registry based on the provided target identifier.
        This function checks for the version's availability and download status before proceeding with the download.
        ...
        mrt = ModelRegistryTarget(target, org_required=True, name_required=True, version_required=False)
        resource_response = self.get(mrt.org, mrt.team, mrt.name)
        if resource_response.recipe.licenseTerms:
            return resource_response.recipe.licenseTerms
        return []
