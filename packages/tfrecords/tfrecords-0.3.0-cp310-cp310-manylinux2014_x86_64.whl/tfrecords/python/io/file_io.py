...
from ...lib.tfrecords_lib import io as _pywrap_file_io
...
class FileIO(object):
  ...
  def __init__(self, name, mode, encoding="utf-8"):
    ...
    if mode not in ("r", "w", "a", "r+", "w+", "a+"):
      raise Exception("mode is not 'r' or 'w' or 'a' or 'r+' or 'w+' or 'a+'")
  ...
  def _preread_check(self):
    if not self._read_buf:
      if not self._read_check_passed:
        raise Exception("File isn't open for reading")
  ...
  def _prewrite_check(self):
    if not self._writable_file:
      if not self._write_check_passed:
        raise Exception("File isn't open for writing")
...
def list_directory_v2(path):
  ...
  if not is_directory(path):
    raise Exception("Could not find directory {}".format(path))
