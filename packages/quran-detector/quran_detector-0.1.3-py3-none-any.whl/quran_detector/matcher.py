...
class term:
    ...
    def __init__(self):
        ...
        self.verses = set(
            [])  # the verses in which this term appears in a specific position preceded by a specific sequence
...
class qMatcherAnnotater():
    ...
    def __init__(self):
        ...
        data_dir = os.path.join(module_dir, 'dfiles')
        q_index_f = os.path.join(data_dir, 'quran-index.xml')
        suras = buildSuraIndex(q_index_f)
        self.all = {}
        self.qOrig = buildVerseDics(suras)
        self.qNorm = buildVerseDics(suras)
        stops_f = os.path.join(data_dir, 'nonTerminals.txt')
        self.stops = loadStops(stops_f)
        self.ambig = set([])
        self.minLen = 3  # minimum acceptable match length
        ayat_f = os.path.join(data_dir, 'quran-simple.txt')
        addAyat(ayat_f, suras, self.all, self.qOrig, self.qNorm, self.ambig, self.minLen, self.stops)
        ...
        print("Matcher/Annotator created! ")
    ...
    def isValidRec(self, r, allowedErrPers=0.25, minMatch=3, forceMatch=False):
        '''
    ...
    def matchAll(self, inText, findErr=True, findMissing=True, allowedErrPers=0.25, minMatch=3, forceMatch=False,
                 return_json=False, d=globalDelimeters):
