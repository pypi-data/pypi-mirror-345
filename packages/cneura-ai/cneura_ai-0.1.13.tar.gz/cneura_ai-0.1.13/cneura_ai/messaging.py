...
class MessageWorker:
    ...
    def callback(self, ch, method, properties, body):
        ...
        try:
            ...
            target_queue = response.get("queue", self.output_queue)  
    ...
    def safe_publish(self, queue, message):
        """Ensures message is published even after connection loss."""
        while True:
            try:
                self.channel.basic_publish(exchange='', routing_key=queue, body=message)
                return
            except pika.exceptions.AMQPConnectionError:
                logger.error("[!] Connection lost during publishing, reconnecting...")
                self.reconnect()
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                logger.error(f"[!] Connection error during consumption: {e}, reconnecting...")
                self.reconnect()
            except Exception:
