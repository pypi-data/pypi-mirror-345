        ...
        for opendapi_project_info, dbt_cp in dbt_cps_with_projects:
            ...
            if not success_run:
                ...
                if run := fast_fail_runs_by_project.get(dbt_cp.project_id):
                    fail_run = dbt_cp.get_latest_ci_run(
                        match_git_sha=commit_sha,
                        statuses={
                    ...
                    if fail_run:
                        # this may print multiple times for now, which is fine
                        print_cli_output(
                            f"The opendapi ci job failed for project {dbt_cp.project_id}. "
                            f"Run URL: {DBTCloudProject.build_run_html_url(fail_run)}"
                        )
                        ...
                        #       goes to sentry.
                        error_message = (
                            f"The dbt Cloud job failed for project {dbt_cp.project_id} and run {fail_run['id']}. "
                            f"Check logs at {DBTCloudProject.build_run_html_url(fail_run)}"
                        )
                        if self._fast_fail:
                            print_encoded_breadcrumbs(error_message)
                            raise RuntimeError(highlight_message(error_message))

                        # if we are not fast failing, we want to check
                        # if there is a pending run on the same SHA
                        # and if there is not, we want to raise an error and short circuit
                        pending_run = dbt_cp.get_latest_ci_run(
                            match_git_sha=commit_sha,
                            statuses={
                                DBT_CLOUD_RUN_STATUSES["queued"],
                                DBT_CLOUD_RUN_STATUSES["starting"],
                        ...
                        if not pending_run:
                            error_message += " - No other fallback runs are running for this commit on dbt cloud."
                            print_encoded_breadcrumbs(error_message)
                            raise RuntimeError(highlight_message(error_message))

                        print_cli_output(
                            "Waiting for the fallback run to complete: "
                            f"{DBTCloudProject.build_run_html_url(pending_run)}"
                        )
