 ...
 * 
 * @requires shiki
 * @requires wagtailcodeblock-shiki
 * 
 * @author kawakin
 * @license MIT
 * 
...
/**
 * remove decoration according to the specified ranges from highlightWords.
 * 
 * @param {*} textCode source code (plain text)
 * @param {Array} highlightWords syntax highlighting definition array
 * @param {Array}removeSpanArray ranges of array to remove decoration
 * @returns 
 */
...
export function setCSSStyleSheets(decorationStyles) {
  ...
  for (let i = 0; i < document.styleSheets.length; i++) {
    ...
    if (sh.title === 'WCBSDecorationsStyleRules') {
      styleSheet = sh;
      break;
    }
  }
