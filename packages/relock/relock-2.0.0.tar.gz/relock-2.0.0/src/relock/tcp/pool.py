...
class Pool(list):
	...
	def __call__(self, host:str, port:int):
		if id := self.__cn__ + 1:
			self.__cn__ = id
			self.append(Socket(host, port, expire=self.expire, 
										   id=id))
			with self[-1] as conn:
				return conn
	...
	def __next__(self):
		...
		if conn := self[self.__id__ - 1]:
			...
			if len(self.shutdown(self[self.__id__ - 1])):
				return self(self.host, self.port)
		raise ConnectionRefusedError('Connection has been lost.')
	...
	def __call__(self, host:str, port:int):
		if not (host, port) in self:
			if _ := Server(host,
						   port,
						   self.ping,
						   Pool(host, port, self.pool, self.ping)):
				if abs(_):
					self.append(_)
					return _
