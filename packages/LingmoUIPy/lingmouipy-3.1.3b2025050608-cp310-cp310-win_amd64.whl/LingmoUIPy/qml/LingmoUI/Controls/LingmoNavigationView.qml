...
Item {
    ...
    property LingmoObject items
    property LingmoObject footerItems
    property int displayMode: LingmoNavigationViewType.Auto
    property Component autoSuggestBox
    property Component actionItem
    property int topPadding: 0
    property int pageMode: LingmoNavigationViewType.Stack
    property LingmoMenu navItemRightMenu
    property LingmoMenu navItemExpanderRightMenu
    ...
    Item {
        ...
        property bool isCompact: d.displayMode === LingmoNavigationViewType.Compact
        property bool isMinimal: d.displayMode === LingmoNavigationViewType.Minimal
        property bool isCompactAndPanel: d.displayMode === LingmoNavigationViewType.Compact && d.enableNavigationPanel
        property bool isCompactAndNotPanel: d.displayMode === LingmoNavigationViewType.Compact && !d.enableNavigationPanel
        property bool isMinimalAndPanel: d.displayMode === LingmoNavigationViewType.Minimal && d.enableNavigationPanel
        property color itemDisableColor: LingmoTheme.dark ? Qt.rgba(131 / 255, 131 / 255, 131 / 255, 1) : Qt.rgba(160 / 255, 160 / 255, 160 / 255, 1)
        ...
        function handleItems() {
            ...
            if (control.items) {
                for (var i = 0; i < items.children.length; i++) {
                    ...
                    data.push(item);
                    _idx++;
                    if (item instanceof LingmoPanelItemExpander) {
    ...
    Component.onCompleted: {
        d.displayMode = Qt.binding(function () {
            if (control.displayMode !== LingmoNavigationViewType.Auto) {
                return control.displayMode;
            }
            if (control.width <= 700) {
                return LingmoNavigationViewType.Minimal;
            } else if (control.width <= 900) {
                return LingmoNavigationViewType.Compact;
            } else {
                return LingmoNavigationViewType.Open;
            }
        });
    ...
    Connections {
        ...
        function onDisplayModeChanged() {
            if (d.displayMode === LingmoNavigationViewType.Compact) {
                collapseAll();
            }
    ...
    Component {
        ...
        Item {
            ...
            Behavior on height {
                enabled: LingmoTheme.animationEnabled && d.animDisabled
                NumberAnimation {
                    duration: 83
                }
            }
            ...
            LingmoControl {
                ...
                Rectangle {
                    ...
                    LingmoIcon {
                        ...
                        iconSource: LingmoIcons.ChevronUp
                        iconSize: 15
                        anchors {
                            verticalCenter: parent.verticalCenter
                            right: parent.right
                        ...
                        color: {
                            ...
                            return LingmoTheme.dark ? "#FFFFFF" : "#000000";
                        }
                    }
                    color: {
                        if (!item_control.enabled) {
                    ...
                    LingmoText {
                        ...
                        color: {
                            ...
                            if (item_control.pressed) {
                                return LingmoTheme.dark ? LingmoColor.Grey80 : LingmoColor.Grey120;
                            }
                            return LingmoTheme.dark ? LingmoColor.White : LingmoColor.Grey220;
                        }
                    }
                    LingmoLoader {
                        id: item_edit_loader
    ...
    Item {
        ...
        RowLayout {
            ...
            LingmoIconButton {
                id: btn_back
                iconSource: LingmoIcons.ChromeBack
                Layout.leftMargin: 5
                ...
                onClicked: {
                    ...
                    if (pageMode === LingmoNavigationViewType.Stack) {
                        var nav_stack = loader_content.item.navStack();
                        var nav_stack2 = loader_content.item.navStack2();
                        nav_stack.pop();
                        if (nav_stack.currentItem.launchMode === LingmoPageType.SingleInstance) {
                    ...
                    } else if (pageMode === LingmoNavigationViewType.NoStack) {
            ...
            LingmoIconButton {
                id: btn_menu
                iconSource: LingmoIcons.GlobalNavButton
                iconSize: 15
        ...
        Item {
            ...
            StackLayout {
                ...
                visible: {
                    ...
                    return LingmoPageType.SingleInstance === nav_stack.currentItem.launchMode;
                }
            }
            function navStack() {
                return nav_stack;
    ...
    LingmoClip {
        radius: [LingmoUnits.windowRadius, 0, 0, LingmoUnits.windowRadius]
        width: {
            if (d.isCompactAndNotPanel) {
                return control.navCompactWidth;
            }
        ...
        Rectangle {
            ...
            border.color: LingmoTheme.dark ? Qt.rgba(45 / 255, 45 / 255, 45 / 255, 1) : Qt.rgba(226 / 255, 230 / 255, 234 / 255, 1)
            border.width: d.isMinimal || d.isCompactAndPanel ? 1 : 0
            color: {
                if (d.isMinimal || d.enableNavigationPanel) {
                    return LingmoTheme.dark ? Qt.rgba(61 / 255, 61 / 255, 61 / 255, 1) : Qt.rgba(243 / 255, 243 / 255, 243 / 255, 1);
                }
                return "transparent";
            }
            LingmoShadow {
            ...
            visible: {
                if (d.displayMode !== LingmoNavigationViewType.Minimal)
                    return true;
            ...
            Flickable {
                ...
                ListView {
                    ...
                    model: d.handleItems()
                    boundsBehavior: ListView.StopAtBounds
                    highlightMoveDuration: LingmoTheme.animationEnabled && d.animDisabled ? 167 : 0
                    highlight: Item {
                        clip: true
                        Rectangle {
                            height: 18
                    ...
                    delegate: LingmoLoader {
                        ...
                        sourceComponent: {
                            if (model === null || !model)
                                return undefined;
                            if (modelData instanceof LingmoPanelItem) {
                                return com_panel_item;
                            }
                            if (modelData instanceof LingmoPanelItemHeader) {
                                return com_panel_item_header;
                            }
                            if (modelData instanceof LingmoPanelItemSeparator) {
                                return com_panel_item_separatorr;
                            }
                            if (modelData instanceof LingmoPanelItemExpander) {
                                return com_panel_item_expander;
                            }
                            if (modelData instanceof LingmoPanelItemEmpty) {
    ...
    Popup {
        ...
        background: Rectangle {
            implicitWidth: 180
            color: LingmoTheme.dark ? Qt.rgba(45 / 255, 45 / 255, 45 / 255, 1) : Qt.rgba(252 / 255, 252 / 255, 252 / 255, 1)
            border.color: LingmoTheme.dark ? Qt.rgba(26 / 255, 26 / 255, 26 / 255, 1) : Qt.rgba(191 / 255, 191 / 255, 191 / 255, 1)
            border.width: 1
            radius: 5
            LingmoShadow {}
        }
    ...
    Component {
        id: com_placeholder
        Item {
            property int launchMode: LingmoPageType.SingleInstance
            property string url
        }
    }
    ...
    function setCurrentIndex(index) {
        ...
        if (item.url) {
            ...
            if (item instanceof LingmoPanelItem) {
                item.tap();
            }
        } else {
            item.onTapListener();
    ...
    function push(url, argument = {}) {
        function stackPush() {
            ...
            if (page) {
                ...
                case LingmoPageType.SingleTask:
                    while (nav_stack.currentItem !== page) {
                        nav_stack.pop();
                        d.stackItems = d.stackItems.slice(0, -1);
                    }
                    return;
                case LingmoPageType.SingleTop:
                    if (nav_stack.currentItem.url === url) {
                        return;
                    }
                    break;
                case LingmoPageType.Standard:
                default:
                }
            }
            var pageIndex = -1;
            ...
            if (pageIndex !== -1) {
                ...
                if (comp.status === Component.Ready) {
                    ...
                    if (obj.launchMode === LingmoPageType.SingleInstance) {
                        nav_stack.push(com_placeholder, options);
