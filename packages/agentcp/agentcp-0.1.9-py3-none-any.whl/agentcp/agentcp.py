...
class _AgentCP(abc.ABC):
    ...
    def serve_forever(self):
        """ """
        log_info(f"agentid client[{self.id}] serve forever")
...
class AgentID(abc.ABC):
    ...
    async def send_stream_message(self, receiver: str, session_id: str,response,type="text/event-stream",ref_msg_id:str=""):
        # 处理对象转换为字典
        stream_result = await self.create_stream(session_id,receiver,type, ref_msg_id)
        if stream_result is None:
            print("创建流失败")
            to_aid_list = []
            to_aid_list.append(receiver)            
            msg_block = {
                "type": "error",
                "status": "success", 
                "timestamp": int(time.time() * 1000),
        ...
        self.send_message(receiver,session_id,msg_block)
...
class AgentCP(_AgentCP):
    ...
    def __init__(self, app_path = "", debug = False):
        ...
        import os
        if app_path == "":
            self.app_path = os.path.join(os.getenv('APPDATA'), "agentcp")
        else:
    ...
    def create_aid(self, ep_url: str,agent_id: str) -> AgentID:
        if agent_id.startswith("guest"):
            return self.get_guest_aid(ep_url)
        self.ca_client = CAClient("https://ep."+ep_url,self.app_path)
        self.ep_url = "https://ep."+ep_url
        agent_id = self.__build_id(agent_id)
        # 确保证书目录存在
