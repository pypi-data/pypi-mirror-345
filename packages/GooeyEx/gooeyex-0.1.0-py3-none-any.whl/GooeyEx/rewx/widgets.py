...
import os
import weakref
from GooeyEx.rewx.dispatch import mount, update
from wx.richtext import RichTextCtrl
import sys
from GooeyEx.rewx.components import (
    Block,
    Grid,
    FlexGrid,
    TextArea,
...
from GooeyEx.rewx.util import exclude
from GooeyEx.rewx.bitmap_support import load, resize_image, to_bitmap
from GooeyEx.rewx.util import identity
...
def frame(element, instance: wx.Frame):
    ...
    if "icon_uri" in props and os.path.exists(props["icon_uri"]):
        ...
        if sys.platform == "darwin":
            ...
            if not hasattr(instance, "__taskbarIcon"):
                instance.__taskbarIcon = wx.adv.TaskBarIcon(iconType=wx.adv.TBI_DOCK)
            instance.__taskbarIcon.SetIcon(icon)
...
def bitmapbutton(element, instance: wx.BitmapButton):
    ...
    if "uri" in props and os.path.exists(props["uri"]):
...
def combobox(element, parent):
    ...
    return update(
        element,
        wx.ComboBox(
            parent, choices=props.get("choices", []), style=props.get("style", 0)
        ),
    )
...
def file_picker_ctrl_save(element, parent):
    ...
    return update(
        element,
        FilePickerCtrlSave(
            parent, style=wx.FLP_SAVE, wildcard=props.get("wildcard", "*.*")
        ),
    )
    ...
    if "value" in props:
        ...
        if value < 0:
            ...
            if instance.GetValue() != value:
                ...
                if props.get("disable_animation", False) and sys.platform.startswith(
                    "win"
                ):
...
def listbox(element, parent):
    return update(
        element, wx.ListBox(parent, choices=element["props"].get("choices", []))
    )
...
def mediactrl(element, parent):
    return update(element, wx.media.MediaCtrl(parent, style=wx.SIMPLE_BORDER))
...
def svg(element, instance: SVG) -> SVG:
    ...
    if props.get("uri"):
        if (
            getattr(instance, "_rewx_uri", None) != props.get("uri")
            or tuple(instance.GetBitmap().GetSize()) != props.get("size")
            and os.path.exists(props["uri"])
        ):
...
def block(element, parent):
    panel = update(
        element, Block(parent, style=element["props"].get("style", wx.TAB_TRAVERSAL))
    )
    sizer = wx.BoxSizer(element["props"].get("orient", wx.VERTICAL))
...
def scrolledpanel(element, instance: ScrolledPanel):
    ...
    if (
        not hasattr(instance, "last_scroll_x")
        ...
        instance.SetupScrolling(
            scroll_x=scroll_x, scroll_y=scroll_y, rate_x=rate_x, rate_y=rate_y
        )
...
def staticbitmap(element, parent):
    ...
    if element["props"].get("uri") and os.path.exists(element["props"].get("uri")):
...
def staticbitmap(element, instance: wx.StaticBitmap):
    props = element["props"]
    set_basic_props(instance, props)
    if props.get("uri") and os.path.exists(props["uri"]):
...
def radiobox(element, parent):
    return update(
        element, wx.RadioBox(parent, choices=element["props"].get("choices", []))
    )
...
_supported_props = {
    "wx.Window": {
        ...
        "wx.ActivityIndicator": {"start": "Start"},
        "wx.adv.CalendarCtrl": {
            "selected_date": "SetDate",  # ?
        ...
        "wx.BitmapButton": {"EVT_BUTTON": "on_click", "SetBitmap": "uri"},
        "wx.ToggleButton": {"EVT_TOGGLEBUTTON": "on_click", "SetValue": "value:bool"},
        "wx.StaticLine": {
            "style": "style",
        },
        "wx.StatusBar": {"SetStatusText": "text:str"},
        "textEntry": {"SetEditable", "SetHint", "ChangeValue"},  # for updates
    }
}
