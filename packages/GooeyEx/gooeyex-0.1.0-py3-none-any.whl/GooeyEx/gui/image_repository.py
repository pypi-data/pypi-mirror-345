...
def getImageDirectory(targetDir):
    return getResourcePath("images") if targetDir == "default" else targetDir
...
def collectOverrides(targetDir, filenames):
    ...
    if not os.path.isdir(targetDir):
        raise IOError("Unable to find the user supplied directory {}".format(targetDir))

    return {
        varname: pathto(filename)
...
def resolvePaths(dirname, filenames):
    return {key: os.path.join(dirname, filename) for key, filename in filenames.items()}
