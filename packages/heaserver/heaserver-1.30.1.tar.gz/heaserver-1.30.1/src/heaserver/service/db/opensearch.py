...
import logging
from abc import ABC
from typing import Optional, TypeVar, List, Any, AsyncGenerator
...
from opensearchpy import AsyncOpenSearch, RequestError, OpenSearchException
...
class OpenSearch(Database):
    ...
    async def get(self, item_id: str, search_item_type: type[ItemTypeVar], index: str | None = None) -> ItemTypeVar | None:
        """
        ...
        try:
            ...
            logging.exception(f"Failed to get item {item_id}: {e}")
            raise e
            ...
            while total_results < max_results:
                paginated_query = {
                    **query,
                    "from": from_offset,
                    "size": size,
                    "sort": ["_id"]  # Ensures consistent ordering
                }
...
class S3WithOpenSearch(S3, OpenSearch):
    def __init__(self, config:  configparser.ConfigParser | None = None, **kwargs):
        super().__init__(config, **kwargs)
...
class OpenSearchManager(MicroserviceDatabaseManager):
    ...
    def get_database(self) -> OpenSearch:
        ...
        """
        return OpenSearch(config=self.config)
