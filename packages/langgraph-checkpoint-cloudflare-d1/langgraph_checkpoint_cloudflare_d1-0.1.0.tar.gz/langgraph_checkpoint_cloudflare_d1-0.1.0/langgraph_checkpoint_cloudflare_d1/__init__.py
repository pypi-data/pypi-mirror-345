...
from .utils import search_where, _metadata_predicate
...
class CloudflareD1Saver(BaseCheckpointSaver[str]):
    ...
    def _execute_query(self, query: str, params: Optional[Sequence[Any]] = None) -> D1Response:
        ...
        data = {
            "sql": query,
            "params": formatted_params if params else []
        }
        ...
        try:
            ...
            try:
                ...
                return D1Response(success=raw_data.get("success", False), result=raw_data.get("result"))
        except Exception as e:
    ...
    def get_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        with self.lock:
            ...
            if not result.result or not isinstance(result.result, list) or len(result.result) == 0 or not result.result[0].results:
                return None
            ...
            if writes_result.success and writes_result.result and isinstance(writes_result.result, list) and len(writes_result.result) > 0:
                ...
                for write_row in writes_rows:
                    ...
                    writes.append((task_id, channel, self.serde.loads_typed((type_, value))))
            ...
            try:
                ...
                deserialized_checkpoint = self.serde.loads_typed((type_, checkpoint))
    ...
    def list(
        ...
        *,
        ...
        with self.lock:
            ...
            # D1 API returns a list of results, each with their own "results" array
            if not response.result or not isinstance(response.result, list) or len(response.result) == 0 or not response.result[0].results:
                return
            ...
            for row in rows:
                try:
                    ...
                    if not writes_response.success:
                        ...
                        writes_result = writes_response.result
                        writes = []
                        
                        for write_row in writes_result.get("results", []):
                            task_id = write_row.get("task_id")
                            channel = write_row.get("channel")
                            ...
                            try:
                                writes.append((task_id, channel, self.serde.loads_typed((write_type, value))))
                            except Exception as e:
                                continue
                    ...
                    try:
                        metadata_dict = {}
                        if metadata is not None and metadata != "" and isinstance(metadata, (str, bytes)):
                            metadata_dict = self.jsonplus_serde.loads(metadata)
                    except Exception as e:
                        metadata_dict = {}
...
__all__ = ["CloudflareD1Saver", "AsyncCloudflareD1Saver", "search_where", "_metadata_predicate"] 