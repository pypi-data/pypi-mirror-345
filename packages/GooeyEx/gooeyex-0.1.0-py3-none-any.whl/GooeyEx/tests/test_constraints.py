...
class TestConstraints(unittest.TestCase):
    ...
    def test_visibility_constraint(self):
        ...
        with self.assertRaises(ValueError):
            parser = GooeyParser()
            parser.add_argument("one", default=None, gooey_options={"visible": False})

        # default = empty string. Still error
        with self.assertRaises(ValueError):
            parser = GooeyParser()
            parser.add_argument("one", default="", gooey_options={"visible": False})

        # default = valid string. No Error
        parser = GooeyParser()
        parser.add_argument("one", default="Hello", gooey_options={"visible": False})

        # No default, but custom validator: Success
        parser = GooeyParser()
        parser.add_argument(
            "one", gooey_options={"visible": False, "validator": {"test": "true"}}
        )
        ...
        parser.add_argument(
            "one",
            default="Hai",
            gooey_options={"visible": False, "validator": {"test": "true"}},
        )
