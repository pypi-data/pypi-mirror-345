...
def generate(model, text, audio_prompt=None, max_tokens=None, cfg_scale=3.0, temperature=0.7, top_p=0.95, use_cfg_filter=True, cfg_filter_top_k=35, seed=0, scan_batch_size=500, use_scan=False, use_lax_scan=False, use_jit=False):
    ...
    def decode_fn(c1, key):
        ...
        # logits_B1CxV = decoder(decoder_cross_attn_mask, cross_kv_caches, tok, self_cos, self_sin, cross_cos, cross_sin, jnp.where(buf_mask, 0, -1e9).astype(config.dtype))
        logits_B1CxV, (_, d1) = nnx.call((d0, d1))(decoder_cross_attn_mask, cross_kv_caches, tok, self_cos, self_sin, cross_cos, cross_sin, jnp.where(buf_mask, 0, -1e9).astype(config.dtype))
