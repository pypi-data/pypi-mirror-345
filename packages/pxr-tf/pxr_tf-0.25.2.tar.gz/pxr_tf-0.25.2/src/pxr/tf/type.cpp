...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
    // Python class handle.
    // We use handle<> rather than pxr::boost::python::object in case Python
    // has not yet been initialized.
    pxr::boost::python::handle<> pyClass;
#endif // PXR_PYTHON_SUPPORT_ENABLED
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
// Comparison for pxr::boost::python::handle.
struct Tf_PyHandleLess
{
    bool operator()(const pxr::boost::python::handle<> &lhs,
                    const pxr::boost::python::handle<> &rhs) const {
...
public:
    ...
    void SetPythonClass(TfType::_TypeInfo *info,
                        const pxr::boost::python::object & classObj) {
        // Hold a reference to this PyObject in our map.
        pxr::boost::python::handle<> handle(
            pxr::boost::python::borrowed(classObj.ptr()));
        ...
        if (!info->sizeofType) {
            info->sizeofType = TfSizeofType<pxr::boost::python::object>::value;
        }
    }
#endif // PXR_PYTHON_SUPPORT_ENABLED
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
    TfType::_TypeInfo *
    FindByPythonClass(const pxr::boost::python::object &classObj) const {
        pxr::boost::python::handle<> handle(
            pxr::boost::python::borrowed(classObj.ptr()));
...
private:
    ...
    typedef map<pxr::boost::python::handle<>,
...
TfType::GetPythonClass() const
{
    ...
    if (_info->pyClass.get()) {
        return TfPyObjWrapper(pxr::boost::python::object(_info->pyClass));
    }
