...
def survival_function(estimates: np.ndarray,
    ...
    an evaluation set, shape (n_samples * n_timepoints,)
    :param times: array of time points on which the hazard was estimated,
    shape (n_timepoints,)
    :return: the survival function for each sample at the fitted time-points,
...
def cumulative_hazard_function(estimates: np.ndarray,
    ...
    an evaluation set
    :param times: array of time points on which the hazard was estimated
    :return: the cumulative hazard function for the fitted time-points
    """
    surv_curve = survival_function(estimates, times)
    surv_curve[surv_curve == 0] = np.finfo(float).eps
    return -np.log(surv_curve)
