...
class Concept(ConstrainedValueType):
    ...
    def build_scalar_variable(self, args, kwargs):
        if len(args) == 0:
            raise Exception(f"Unexpected error in Concept {self.name()}")

        if len(kwargs) != 0:
            raise Exception(
                f"Cannot use keyword arguments when instantiating a type parameter for Concept {self.name()}")
    ...
    def build_relation_variable(self, args, kwargs):
        ...
        if len(args) > 1:
            raise Exception(f"Cannot declare type variable of Concept {self.name()} by supplying more than a type")
    ...
    def head_display(self):
        ...
        for v in self._params:
            ...
            if vt == self._parent:
                ...
                headvars.append(f"{v.display()} in {vt.name()}")
        head = ", ".join(headvars)
        if self._parent is None:
            return f"value type {self.name()}({head})"
        else:
            return f"value type {self.name()} <: {self._parent.name()}({head})"
    ...
    def root_unconstrained_type(self):
        if self.arity() == 1:
            ...
            raise Exception(f"Composite nominal type {self.name()} has no root unconstrained type")