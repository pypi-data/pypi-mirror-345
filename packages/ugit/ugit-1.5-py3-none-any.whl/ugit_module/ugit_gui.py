...
def view_log():
    with setup_git_context():
        try:
            # Robustly resolve HEAD to actual SHA
            ref = data.get_ref('HEAD')
            if ref.symbolic:
                resolved = data.get_ref(ref.value)
                start_oid = resolved.value
            else:
                start_oid = ref.value
            ...
            output = ""
            for oid in base.iter_commits_and_parents({start_oid}):
            ...
            messagebox.showerror("Log Error", f"Could not load log:\n{str(e)}")
            print("Error in view_log:", e)
