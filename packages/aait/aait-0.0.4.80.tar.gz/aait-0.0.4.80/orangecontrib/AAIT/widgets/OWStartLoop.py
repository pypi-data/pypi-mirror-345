...
class LoopStartWidget(OWWidget):
    ...
    def check_iter_column(self,in_data_orange):
        ...
        - 2 if 'iter' exists but is either not categorical or contains values other than "0" and "1".
        """
        ...
        # Check if "iter" is categorical (discrete)
        if not iter_var.is_discrete:
            return 2  # "iter" is not categorical

        # Check if "iter" only contains the values "0" and "1"
        if set(iter_var.values) != {"0", "1"}:
            return 2  # "iter" contains values other than "0" and "1"
