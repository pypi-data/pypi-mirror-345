"""
The wntrfr.morph.skel module contains functions to skeletonize water
network models.
"""
import logging
import copy
...
from wntrfr.network.elements import Pipe, Junction
from wntrfr.sim.core import WNTRSimulator
from wntrfr.sim import EpanetSimulator
...
def skeletonize(wn, pipe_diameter_threshold, branch_trim=True, series_pipe_merge=True, 
    ...
    for pair in skel.pipe_removed:
        ...
        if iFound == False:
            ...
            pipe_groups.append(new_group)
                
    if return_map:
        return skel.wn, skel.skeleton_map, pipe_groups
    else:
        return skel.wn, pipe_groups
...
class _Skeletonize(object):
    ...
    def series_pipe_merge(self, pipe_threshold):
        ...
        for junc_name in self.wn.junction_name_list:
            if (junc_name in self.junc_with_controls
                and self.wn.get_node(junc_name).demand_timeseries_list[0].base_value != 0):
                continue
            neighbors = list(nx.neighbors(self.G,junc_name))
            ...
            self.pipe_removed.append((pipe_name0, pipe_name1))
            #print((pipe_name0, pipe_name1))
