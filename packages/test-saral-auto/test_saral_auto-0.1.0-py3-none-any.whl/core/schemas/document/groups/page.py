...
from core.schemas.document.blocks import Block, BlockId, Text
...
from core.schemas.document.text import Line, Span
# Import from math_utils instead of util to avoid circular imports
from core.schemas.document.math_utils import matrix_intersection_area
...
class PageGroup(Group):
    ...
    def get_block(self, block_id: BlockId) -> Block | None:
        ...
        """
        logger.debug(f"Attempting to retrieve block with ID: {block_id.block_id}")

        # Check if the block_id is valid
        if block_id.block_id < 0 or block_id.block_id >= len(self.children):
            logger.warning(f"Block ID {block_id.block_id} is out of range. Total children: {len(self.children)}")
            return None
        
        # Get the block
        block: Block = self.children[block_id.block_id]
        logger.debug(f"Retrieved block with ID: {block.block_id}")

        # Double-check that the block we found has the expected ID
        if block.block_id != block_id.block_id:
            logger.error(f"Block ID mismatch: Expected {block_id.block_id}, but found {block.block_id}")
            return None

        logger.info(f"Successfully retrieved block with ID: {block_id.block_id}")
        return block
    ...
    def aggregate_block_metadata(self) -> BlockMetadata:
        ...
        # Convert dict to BlockMetadata if needed
        elif isinstance(self.metadata, dict):
            self.metadata = BlockMetadata(**self.metadata)
            
        # Aggregate metadata from all blocks
        for block in self.current_children:
            if block.metadata is not None:
                # Convert block metadata from dict if needed
                if isinstance(block.metadata, dict):
                    block_metadata = BlockMetadata(**block.metadata)
                    self.metadata = self.metadata.merge(block_metadata)
                else:
