...
from ML_management.s3.utils import get_bucket_size, get_upload_paths, get_upload_size
...
class BucketFilterSettings(BaseModel):
    ...
    creation_datetime_interval: Optional[TimestampInterval]
...
class S3Manager:
        ...
        *,
        ...
        if untar_data:
            return self._download_data_tar(
                local_path=local_path, bucket=bucket, remote_paths=remote_paths, verbose=verbose
            )

        return self._download_files(
            local_path=local_path, bucket=bucket, remote_paths=remote_paths, verbose=verbose, max_workers=max_workers
        )
    ...
    def _upload_files(
        ...
        bucket: str,
        prefix: str,
        new_bucket_visibility: VisibilityOptions,
