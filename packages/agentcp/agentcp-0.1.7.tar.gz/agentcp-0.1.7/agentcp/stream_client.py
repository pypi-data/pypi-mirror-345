...
class StreamClient():
    ...
    def send_chunk_to_stream(self,stream_url: str, chunk: str):
        if self.ws is None or self.ws.sock is None or not self.ws.sock.connected:
            print("WebSocket connection is not established @send_chunk_to_stream.")
        ...
        def on_message(ws, message):
            ...
            if isinstance(message, bytes):
                ...
                if txt_msg is not None and len(txt_msg) > 0:
                    print(f"Text_Stream收到消息收到二进制消息: {txt_msg}")
                    on_message(ws, txt_msg)
            elif isinstance(message, str):
                print(f"Text_Stream收到消息: {self.agent_id}\t{message}")
            

        def on_error(ws, error):
            """连接发生错误时的处理函数"""
            print(f"Text_Stream连接错误: {error}")

        def on_close(ws, close_status_code, close_msg):
            """连接关闭时的处理函数"""
            print("Text_Stream WebSocket 连接已关闭")

        def on_open(ws):
            """连接建立后的处理函数，用于发送初始消息"""
            print("Text_Stream WebSocket 连接已建立")
            if self.ws_chunks is not None and len(self.ws_chunks) > 0:
                self.send_chunk_to_stream(self.ws_url, self.ws_chunks)
                self.ws_chunks = ""
