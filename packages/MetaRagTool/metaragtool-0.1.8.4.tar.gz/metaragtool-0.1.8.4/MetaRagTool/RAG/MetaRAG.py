...
class MetaRAG:
    ...
    def __init__(self, encoder_model: MyEncoder, llm: LLMIdentity, reranker_model: MyReranker = None,
                ...
                for paragraph in myDoc.MyParagraphs:
                    ...
                    while currentSentence is not None:
                        ...
                        if token_len(currentChunkText + nextText) > self.chunk_size and currentChunkText.strip():
                            currentChunkText, current_chunk_sentences, prevChunk = self.__sent_merger_create_chunk(currentChunkText,
                                                                                                    current_chunk_sentences,
                                                                                                    myDoc, paragraph,
                                                                                                    prevChunk)
    ...
    def __sent_merger_create_chunk(self, currentChunkText, current_chunk_sentences, myDoc, paragraph, prevChunk):
    ...
    def retrieve_core(self, query_embedding, top_k):
        if top_k < 1:
    ...
    def retrieve(self, query, top_k=20, force_basics=False, sum_of_retrieved_token_length_limit=-1):
        ...
        retrieved_chunks = self.retrieve_core(query_embedding, initial_k)
        ...
        if self.breath_first_retrival and not force_basics:
            ...
            for chunk in top_k_retrieved_chunks:
                additional_chunks.extend(self.retrieve_core(chunk.Embeddings, self.additional_top_k))
            ...
            for _ in range(2):
                level_chunks = self.retrieve_core(current_chunk.Embeddings, initial_k // 2)
            ...
            for i, chunk in enumerate(retrieved_chunks):
                ...
                if chunk_top_k < 1:
                    break
                additional_chunks.extend(self.retrieve_core(chunk.Embeddings, chunk_top_k))
        ...
        if self.include_reflections:
            ...
            for chunk in retrieved_chunks:
                ...
                additional_chunks.extend(self.retrieve_core(reflection, 1))
            ...
            for chunk in retrieved_chunks:
                ...
                closest_chunk = self.retrieve_core(newV, 1)
    ...
    def __ask_notTool(self, query, top_k=1, include_prompt=True):
    ...
    def ask(self, query, top_k=1, include_prompt=True,useTool=False):
        if useTool:
            return self.__ask_tool(query)
        else:
            return self.__ask_notTool(query, top_k=top_k, include_prompt=include_prompt)




    def __ask_tool(self, query):
        print("using tool")
        if self.answer_mode=='none':
    ...
    def retrieve_interface(self, query:str, top_k:int) -> str:
        ...
        retrieved_chunks_text=self.LLMIdentity.merge_chunks(retrieved_chunks=retrieved_chunks)
