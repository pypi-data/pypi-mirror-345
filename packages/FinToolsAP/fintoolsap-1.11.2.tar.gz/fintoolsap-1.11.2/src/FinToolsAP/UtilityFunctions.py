...
def group_quantile(df: pandas.DataFrame | polars.DataFrame,
    ...
    set_index : bool, optional
        If True, sets the group column(s) as the index in the returned DataFrame.
    ...
    if not (isinstance(vr, dict) or isinstance(vr, list)):
        raise TypeError(f'vr: expected type str or list[str], got {type(vars).__name__!r}')
    
    grouped: bool = gr is not None
    if grouped:
        if not (isinstance(gr, dict) or isinstance(gr, list)):
            raise TypeError(f'gr: expected type str or list[str], got {type(vars).__name__!r}')
