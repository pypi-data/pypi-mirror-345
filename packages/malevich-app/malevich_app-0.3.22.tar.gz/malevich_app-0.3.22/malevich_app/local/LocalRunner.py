...
from malevich_app.ws.mapping import operations_mapping, ws_init
...
class LocalRunner:
    ...
    async def ws(self, id: Optional[str] = None, secret: Optional[str] = None, dm_url: str = "localhost:8000", override_core_host: Optional[str] = None, sync_objects: Optional[Any] = None, secure: bool = True):
        if id is not None:
            if secret is None:
        ...
        async with websockets.connect(f"{'wss' if secure else 'ws'}://{dm_url}/ws/app?id={id}&secret={secret}", ping_interval=3600, ping_timeout=5) as ws:
            ...
            try:
                self.__ws_set_internals(ws, id, override_core_host or init_msg.core_host, init_msg.core_port, init_msg.save_df_format)
