...
class DockerAuthSettings(custom_types.ConfigModel):
    ...
    def validate_secret_name(self) -> "DockerAuthSettings":
        if self.auth_method == DockerAuthType.GCP_SERVICE_ACCOUNT_JSON and self.secret_name is None:
            raise ValueError("secret_name must be provided when auth_method is GCP_SERVICE_ACCOUNT_JSON")
        return self
