"""
This module contains the schema for the polygon. It is used to represent the polygon of the document.
"""
...
class PolygonBox(BaseModel):
    ...
    def to_dict(self):
        """
        Convert the polygon to a dictionary for JSON serialization.
        
        Returns:
            dict: The polygon as a JSON-serializable dictionary
        """
        return {
            "polygon": self.polygon.tolist() if hasattr(self.polygon, 'tolist') else self.polygon,
            "bbox": self.bbox
        }