...
import  pyav.src.cmd_modules.Computation as Computation
...
class Connection():
    ...
    def __init__(self, devType, stConType='Usb', *args):
        ...
        self.usbDev             =   0
        if self.cType == 'Usb':
            self.usb                = ctypes.CDLL('../src/cmd_modules/PythonUSBnAny.dll');
            self.usb.UsbPyStart();
                ...
                if nargin >= 2:
                    ...
                    if nargin >= 3:
                        ...
                        if (isinstance(devIdx, str)):
                            ...
                            self.SetIndex(devIdx);
            if stConType == 'Usb':
                self.usb.UsbPyUseCustomDevice(int('064B',16), int('7823',16), 1, 1);
        ...
        if self.cType == 'Usb':
            ...
            if TxData > 0:
                if self.cDevType == 'RadarLog':
                    ...
                    if (self.usb.UsbPySend(self.usbDev, 1, Data2048, len(Data2048)) > 0):
                        print("ok");
    ...
    def CmdRecv(self, dispError = True, stopOnError = False):
        ...
        if self.cType == 'Usb':
            ...
            if TxData > 0:
                BufLen      =   128;
                ResBytes    =   (ctypes.c_ubyte*BufLen)();
                if (self.usb.UsbPyRecv(self.usbDev, 1, ResBytes, BufLen) == 0):
                    Len         =   ResBytes[2];
                    if Len < 32:
                        Ret     =   ResBytes[4:];
                        Ret     =   self.ToUint32(Ret);
            if self.ConNew > 0:
                self.UsbClose()
        elif self.cType == 'PNet':
            TxData          =   0
    ...
    def     UsbOpen(self):
        if self.hCon > -1: 
            self.usb.UsbPyClose(self.usbDev);
        self.usbDev     =   self.usb.UsbPyOpen()
        print("usb dev: ", self.usbDev);
        self.hCon       =   1
        self.hConDat    =   1
        if self.cDebugInf > 10:
            print('USB device pointer: ', self.usbDev)
        return self.usbDev;
