...
/**
 * This is responsible for profiling SajadQ using "vmprof".
 */
...
void startProfiling(void) {
    tempfile_profile = fopen("SajadMr-performance.dat", "w+b");
    ...
    PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, PyObject_GetAttrString(vmprof_module, "enable"),
                                                     SajadQ_PyInt_FromLong(fileno(tempfile_profile)));
...
void stopProfiling(void) {
    ...
    FILE *tempfile_times = fopen("SajadMr-times.dat", "wb");
