...
class BreakAndExec():
    ...
    def single_step(self, addr, fresh=True):
        ...
        Otherwise, if mon._safe is true, it means that we will make every effort to not end up in the
        ...
        # or, if a two-word instruciton, simulate the step.
        if addr in self._bp and self._bp[addr]['inflash']:
            ...
            # if there is a two word instruction, simulate
                ...
                self.dbg.program_counter_write(addr>>1)
                return SIGTRAP
            else: # one-word instructions are handled by offline execution in the OCD
