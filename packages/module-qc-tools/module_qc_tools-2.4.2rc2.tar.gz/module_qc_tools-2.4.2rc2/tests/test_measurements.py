...
def config(test_type: str, chip_type: str):
...
@pytest.mark.parametrize(("test_type", "chip_type"), [("ADC_CALIBRATION", "RD53B")])
def test_issue114(config, hardware, runner):
    ...
    data = runner(config, hardware.ps, hardware.hv, hardware.yr, layer, BOM, False)
...
@pytest.mark.parametrize(("test_type", "chip_type"), [("IV_MEASURE", "RD53B")])
...
@pytest.mark.parametrize(
    ("test_type", "chip_type"), [("LONG_TERM_STABILITY_DCS", "RD53B")]
)
...
def test_issue132(config, hardware, runner):
    ...
    hardware.ps.on(
        v=config["v_max"],
        i=config["i_config"][layer] + 1.0,  # enforce wrong current
    )
    hardware.hv.on(
        v=-120,
        i=1e-6,
    ...
    with pytest.raises(RuntimeError, match="Module is not properly powered."):
