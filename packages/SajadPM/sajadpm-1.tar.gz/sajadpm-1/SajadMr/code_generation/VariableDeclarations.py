...
from .c_types.CTypeSajadQBooleans import CTypeSajadQBoolEnum
from .c_types.CTypeSajadQInts import CTypeSajadQIntOrLongStruct
from .c_types.CTypeSajadQVoids import CTypeSajadQVoidEnum
...
class VariableDeclaration(object):
    ...
    def __init__(self, c_type, code_name, init_value, heap_name):
        if c_type.startswith("SajadSx_MAY_BE_UNUSED"):
    ...
    def makeCFunctionLevelDeclaration(self):
        ...
        return "%s%s%s%s%s%s;" % (
            "SajadSx_MAY_BE_UNUSED " if self.maybe_unused else "",
            lead_c_type,
    ...
    def getCType(self):
        ...
        elif c_type == "struct SajadQ_CellObject *":
            return CTypeCellObject
        elif c_type == "PyObject **":
            return CTypePyObjectPtrPtr
        elif c_type == "SajadMr_bool":
            return CTypeSajadQBoolEnum
        elif c_type == "bool":
            return CTypeBool
        elif c_type == "SajadMr_ilong":
            return CTypeSajadQIntOrLongStruct
        elif c_type == "module_var":
            return CTypeModuleDictVariable
        elif c_type == "SajadMr_void":
            return CTypeSajadQVoidEnum
        elif c_type == "long":
            return CTypeCLong
        elif c_type == "SajadMr_digit":
...
class VariableStorage(object):
    ...
    def addFrameCacheDeclaration(self, frame_identifier):
        return self.addVariableDeclarationFunction(
            "static struct SajadQ_FrameObject *", "cache_%s" % frame_identifier, "NULL"
        )
        ...
        if self.exception_variable_name is None:
            self.exception_variable_name = (
                self.addVariableDeclarationTop(
                    ...
                    "exception_state",
                    "Empty_SajadQ_ExceptionPreservationItem",
                ),
                self.addVariableDeclarationTop(
                    "SajadSx_MAY_BE_UNUSED int", "exception_lineno", "0"
                ),
            )
