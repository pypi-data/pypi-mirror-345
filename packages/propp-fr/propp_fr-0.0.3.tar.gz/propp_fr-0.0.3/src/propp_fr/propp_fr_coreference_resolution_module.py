...
def generate_coreference_matrix_with_cache(entities_df, mention_pairs_df, verbose=0, confidence_threshold=0):
    """
...
def assign_coreference_to_proper_mentions_if_A_is_part_of_B(entities_df):
    ...
    results = []
    for i, (idx_A, text_A, gender_A, num_A, prop_A) in enumerate(tqdm(mention_list, desc="Processing mentions A is part of B", leave=False)):
...
def get_propagated_proper_mentions_pairs(grouped_PROP_entities_df,
    ...
    merged_pairs = []
    for A_ids, B_ids in tqdm(ids_pairs, desc="Propagating Coref Decision to Global Scale"):
...
def postprocess_mentions_pairs(predicted_mention_pairs,
    ...
    #
        if propagate_coref == True:
            propagated_mention_pairs_df = get_propagated_proper_mentions_pairs(grouped_PROP_entities_df, predicted_mention_pairs)
