...
class MavSender(AbcSender):
    ...
    def get_missing_chunk_indexes(self, msg_id, chunk_count):  # AMAR
        ...
        ack_data = self.get_ack_data_received()
        for chunk_index in range(chunk_count):
            if not ack_data.get(msg_id, {}).get(str(chunk_index)):
                missing.append(chunk_index)
    ...
    def send_mav_message(self, message, destination_id):
        chunks, chunk_count = self.create_chunks(self.serialise_msg(message))
...
def main():
    ...
    # Example message
    test_message = String()
    # test_message.data = "Test ssageMe for communication"
