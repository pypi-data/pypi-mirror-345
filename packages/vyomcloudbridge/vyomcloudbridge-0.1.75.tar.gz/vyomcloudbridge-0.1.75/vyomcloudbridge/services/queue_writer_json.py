...
class QueueWriterJson:
    ...
    def write_message(
        self,
        message_data,
        data_type: str,  # json, ros, file, video, image, binary
        data_source: str,  # telemetry, drone_state, mission_summary, camera1, camera2, machine_state
        destination_ids,  # array of destination_id
        filename: str = None,  # optional, only in case of data type is json, can be created by timestamp
        mission_id: str = None,  # optional, only in case of data generated by current device
        priority: int = 1,  # will keep the priority 1 for all, 2 for live
        merge_chunks: bool = True,  # true for now
    ) -> tuple[bool, str]:
        """
        ...
        Args:
            ...
            data_type: Type of data (json, ros, file, video, image, binary)
            data_source: Source of the data (telemetry, drone_state, mission_summary, camera1, camera2, machine_state)
            destination_ids: List of destination IDs to send the message to
            filename: Optional filename for JSON data
            mission_id: Optional mission ID for data generated by current device
            priority: Message priority (1 for normal, 2 for live)
            merge_chunks: Whether to merge message chunks
        ...
        try:
            ...
            buffer_key = str(mission_id) if mission_id else data_buffer_key
            ...
            fileinfo_message = {
                ...
                "topic": f"{mission_upload_dir}/file_properties/{base_filename}.json",
                "message_type": "json",
            ...
            if should_chunk:
                ...
                self.rabbit_mq.publish(fileinfo_message, priority)
            ...
            if should_chunk:
                ...
                for i, chunk in enumerate(data_chunks):
                    try:
                        ...
                        self.rabbit_mq.publish(chunk_message, priority)
                ...
                try:
                    ...
                    self.rabbit_mq.publish(file_message, priority)
...
def main():
    ...
    #     data_type = "json"  # image, json, file, video
    #     mission_id = "111333"
    ...
    try:
        ...
        for i in range(loop_len):
            ...
            data_type = "binary"  # image, json, file, video
            mission_id = "34556"
            formatted_index = str(i + 1).zfill(padding_length)
    ...
    # Example 5 - send mission topics list
    from vyomcloudbridge.services.queue_writer_json import QueueWriterJson
