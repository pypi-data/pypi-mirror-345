    ...
    def hlan_checkurl_shutdown_bj(
        ...
        *,
        ...
        #某些pve中可能需要写 python3.10 -c xxx
            ...
            @reboot python3 -c "from dknovautils import *; HomeLanUtils.hlan_checkurl_shutdown_bj(cmd='pve_stop_all',verbose=True,dry_run=True);" >>/root/pve_stop_all.log
        ...
        def mypr(s: str) -> None:
            ...
            if AT.isLinux:
                ...
                syslog.syslog(syslog.LOG_KERN, f"{sname} {s}")
        ...
        if cmd == "pve_stop_all":
            ...
            if root_path:
                ...
                Path(fname).write_text(cmd, encoding="utf-8")
        ...
        def cb(rss: List[str | None]) -> bool:
            ...
            if shutdown:
                if verbose:
                    mypr(f"{ts} connect url:{url} shutdown {cmd}")
                # cmd = "shutdown -P now"
                if not dry_run:
