...
"""Python level PGO handling in SajadQ."""

import os
import struct

from SajadMr.__past__ import xrange
from SajadMr.Options import getPythonPgoUnseenModulePolicy
from SajadMr.Tracing import pgo_logger
...
def readPGOInputFile(input_filename):
    ...
    with open(input_filename, "rb") as input_file:
        ...
        if header != b"KAY.PGO":
            pgo_logger.sysexit(
                "Error, file '%s' is not a valid PGO input for this version of SajadQ."
                % input_filename
            )
