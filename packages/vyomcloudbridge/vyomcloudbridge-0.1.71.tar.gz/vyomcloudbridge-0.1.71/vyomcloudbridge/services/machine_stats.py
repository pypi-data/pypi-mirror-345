...
class MachineStats(ServiceAbstract):
    ...
    def __init__(self):
        ...
        self.stats_publish_interval = 5  # Seconds between stats publication
    ...
    def _setup_connection(self):
        ...
        try:
            ...
            self.channel.queue_declare(queue="machine_buffer", durable=True)
    ...
    def get_current_buffer(self):
        ...
        try:
            ...
            method_frame, _, body = self.channel.basic_get(queue="machine_buffer", auto_ack=False)
    ...
    def _set_current_buffer(self, data_size: int, data_size_uploaded: int):
        try:
            ...
            while True:
                method_frame, _, _ = self.channel.basic_get(queue="machine_buffer", auto_ack=True)
                if not method_frame:
                    break
            ...
            self.logger.debug(f"Set buffer: data_size={data_size}, data_size_uploaded={data_size_uploaded}")
        except Exception as e:
    ...
    def on_data_arrive(self, size: int):
        try:
            ...
            self.logger.debug(f"Data arrived: +{size} bytes, new data_size={new_data_size}")
        except Exception as e:
    ...
    def on_data_publish(self, size: int):
        try:
            ...
            self.logger.debug(f"Data published: +{size} bytes, total uploaded={new_uploaded}")
        except Exception as e:
