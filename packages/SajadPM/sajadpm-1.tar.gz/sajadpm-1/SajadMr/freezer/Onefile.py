...
from SajadMr.Options import getOnefileTempDirSpec, isOnefileTempDirMode
from SajadMr.OutputDirectories import getResultFullpath
from SajadMr.plugins.Plugins import Plugins
from SajadMr.PostProcessing import executePostProcessingResources
from SajadMr.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from SajadMr.Tracing import onefile_logger, postprocessing_logger
from SajadMr.utils.Execution import withEnvironmentVarsOverridden
...
from SajadMr.utils.SharedLibraries import cleanupHeaderForAndroid
from SajadMr.utils.Signing import addMacOSCodeSignature
...
from SajadMr.utils.WindowsResources import RT_RCDATA, addResourceToFile
...
def _runOnefileScons(onefile_compression, onefile_archive):
    ...
    env_values["_SajadSx_ONEFILE_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_SajadSx_ONEFILE_TEMP_BOOL"] = "1" if isOnefileTempDirMode() else "0"
    env_values["_SajadSx_ONEFILE_COMPRESSION_BOOL"] = "1" if onefile_compression else "0"
    env_values["_SajadSx_ONEFILE_BUILD_BOOL"] = "1" if onefile_compression else "0"
    env_values["_SajadSx_ONEFILE_ARCHIVE_BOOL"] = "1" if onefile_archive else "0"
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from SajadMr.tools.onefile_compressor.OnefileCompressor import (
        ...
        mapping = {
            "SajadSx_PACKAGE_HOME": os.path.dirname(
                os.path.abspath(sys.modules["SajadMr"].__path__[0])
            )
        }

        mapping["SajadSx_PROGRESS_BAR"] = "1" if Options.shallUseProgressBar() else "0"
