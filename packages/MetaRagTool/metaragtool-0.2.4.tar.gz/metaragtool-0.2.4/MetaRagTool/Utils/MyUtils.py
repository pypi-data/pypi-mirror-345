...
def Init(encoder_name, top_k, sample_size, qa_sample_ratio, local_mode=False, multi_hop_hardness_factor=0,
         judged=False, useTool=False, llm_name:str=Gemini.GEMINI_2_FLASH,multi_hop=True,use_Gemini=True):
    ...
    if judged:
        if use_Gemini:llm = Gemini(has_memory=False, RequestPerMinute_limit=15, model_name=llm_name)
        else:llm = OpenaiGpt(has_memory=False, RequestPerMinute_limit=15)
    ...
    ragConfig = MetaRagConfig(encoder_name=encoder_name, top_k=top_k, sample_size=sample_size,
                              ...
                              project_name=project_name,multi_hop=multi_hop,use_Gemini=use_Gemini)
