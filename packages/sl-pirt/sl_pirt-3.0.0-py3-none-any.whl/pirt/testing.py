...
for i in range(9):
    ...
    print("testing.py could not find project root dir, using testing.py directory instead.")
    ROOT_DIR = THIS_DIR
...
def run_tests_if_main(show_coverage=False):
    ...
    pytest.main(
        ["-v", "-x", "--color=yes", "--cov", PACKAGE_NAME, "--cov-config", ".coveragerc", "--cov-report", "html", fname]
    )
    if show_coverage:
        import webbrowser
