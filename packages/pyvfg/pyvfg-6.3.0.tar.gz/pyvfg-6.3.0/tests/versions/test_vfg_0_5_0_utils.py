...
from pyvfg import vfg_from_json, vfg_to_json_schema
...
def valid_vfg_1():
    with open(
        package_dir
        / "../gpil/tests/fixtures/models/sprinkler/sprinkler_vfg_0_3_0.json",
        "r",
    ) as f:
...
def valid_vfg_2():
    with open(
        package_dir
        / "../gpil/tests/fixtures/models/insurance/insurance_vfg_0_5_0.json",
        "r",
    ) as f:
        vfg_json = json.load(f)
    return VFG(**vfg_json)
...
def test_vfg_from_json(valid_vfg_2: VFG):
    vfg = vfg_from_json(valid_vfg_2.model_dump())
    assert not vfg.validate(), "check for validity"
    assert vfg == valid_vfg_2, "check for round trip"
    vfg = vfg_from_json(valid_vfg_2.model_dump_json())
...
def test_vfg_from_json_invalid():
    ...
    with pytest.raises(JsonSerializationError):
        vfg = vfg_from_json(invalid_vfg_json)
...
def test_vfg_upgrade_invalid():
    ...
    with pytest.raises(AttributeError):
        vfg = pyvfg.vfg_upgrade(invalid_vfg_json)
