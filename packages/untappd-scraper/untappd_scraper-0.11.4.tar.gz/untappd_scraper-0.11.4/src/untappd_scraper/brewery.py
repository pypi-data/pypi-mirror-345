...
from untappd_scraper.beer import extract_checkin_elements_bs4
from untappd_scraper.logging_config import configure_logging, logger
from untappd_scraper.web import end_of_href, get_text, make_soup, parsed_value, url_of
...
class Brewery:
    ...
    def __post_init__(self) -> None:
        """Initiate a Brewery object, storing the brewery ID and loading details.
    ...
    @classmethod
    def from_slug(cls, brewery_slug: str) -> Self | None:
        """Navigate via a brewery slug and return the brewery objecct.

        Args:
            brewery_slug (str): brewery slug to search

        Returns:
            brewery: populated brewery object for this brewery slug
        """
        url = url_of(page=brewery_slug)
        try:
            soup, _ = make_soup(url)
        except HTTPStatusError as e:
            logger.debug("Error getting brewery page for slug {}: {}", brewery_slug, e)
            return None
        ...
        if not href:  # pragma: no cover
            logger.debug("No brewery found for slug {}", brewery_slug)
            return None
...
def brewery_details(soup: BeautifulSoup, *, brewery_id: int) -> WebBreweryDetails:
    ...
    if canonical := soup.select_one('link[rel="canonical"]'):
        ...
        brewery_slug = url.split("/")[-1]
    else:  # pragma: no cover
        url = f"https://untappd.com/brewery/{brewery_id}"
        brewery_slug = str(brewery_id)
    ...
    for beer in soup.select("div.activity div.item"):
        ...
        activity.append(
            WebActivityBeer(
                ...
                brewery_id=brewery_id,
                brewery_slug=end_of_href(brewery_el) or "",
