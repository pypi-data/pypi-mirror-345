...
def main():
    ...
    print("\nWhich directory do you want to delete duplicates from?")
    print("1 - First directory (left)")
    print("2 - Second directory (right, listed above)")
    choice = input("Enter 1 or 2 (default 2): ").strip()
    if choice == '1':
        to_delete = [f1 for (f1, f2, files1, files2) in batch]
    else:
        to_delete = [f2 for (f1, f2, files1, files2) in batch]

    resp = input(f"Delete ALL {len(to_delete)} folders from the selected directory? [y/N]: ").strip().lower()
    if resp == 'y':
        for f in to_delete:
            shutil.rmtree(f)
            print(f"Deleted {f}")
