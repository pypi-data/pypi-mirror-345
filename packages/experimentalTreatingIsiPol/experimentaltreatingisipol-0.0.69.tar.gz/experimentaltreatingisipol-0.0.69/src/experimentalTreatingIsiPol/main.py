...
class SeveralMechanicalTestingFittingLinear():
    ...
    def __findOtherArchives(self, docConfig: str, archive_name :  str,
            ...
            if (each_k != 'stress' and each_k !='strain' and
                ...
                if not (testType != 'tensile' and each_k =='Poisson'):
                    ...
                    if self.save_results != '':
                        fig_boxplot.savefig(os.path.join(self.save_results, f'resultados_{testType}_{each_k}.png'), dpi=2000 , bbox_inches='tight')
        ...
        fig,ax, fig_raw, ax_raw = self.__plotStressStrain(direction, testType=testType)

        self.__createExcelReport()

        if self.save_results != '':

            fig.savefig(os.path.join(self.save_results, 'resultados_finais.png'), dpi=2000 , bbox_inches='tight')
            fig.savefig(os.path.join(self.save_results, 'resultados_finais.jpeg'), dpi=2000, bbox_inches='tight')
            fig_raw.savefig(os.path.join(self.save_results, 'resultados_brutos_finais.png'), dpi=2000, bbox_inches='tight')
            fig_raw.savefig(os.path.join(self.save_results, 'resultados_brutos_finais.jpeg'), dpi=2000, bbox_inches='tight')
    ...
    def __plotStressStrain(self, direction : str, testType : str):
        ...
        if testType == 'compression':
            ...
            several_plots_helper(ax=ax, xs=xs, ys=ys,labels=labels,xlabel=r'Deformação $[mm/mm]$', ylabel=r'$\sigma$ [MPa]', color_scheme= 'matplotlib_default')
            ax_raw.set_title('Dados brutos dos Ensaios')

            return fig,ax, fig_raw, ax_raw
