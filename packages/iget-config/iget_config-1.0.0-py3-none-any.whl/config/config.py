...
class DataConfig:
    ...
    def __init__(
        ...
        # Assign the value passed from YAML or the code default
        # If feature_bins is explicitly provided (from YAML), use it.
        # Otherwise, calculate a default based on feature_size.
        if feature_bins is not None:
            self.feature_bins = feature_bins
        else:
            # Calculate the default only if nothing was provided via YAML
            # Using a simplified default pattern as an example, adjust as needed
            default_feature_bins_pattern = [
                [-0.08, -0.03, -0.01, 0.01, 0.03, 0.08],
                [0.01, 0.05, 0.2, 0.5, 2, 10],
            ]
            num_bin_patterns = len(default_feature_bins_pattern)
            # Use self.feature_size which is already assigned
            self.feature_bins = [
                default_feature_bins_pattern[i % num_bin_patterns]
...
class OneCycleLRConfig:
    ...
    def __init__(
        ...
        if self.total_steps is None:
            warnings.warn(
                "OneCycleLRConfig: total_steps is not set in the config. "
                "It should be calculated and passed when creating the scheduler instance "
                "(e.g., using TrainConfig.num_epochs and len(dataloader))."
            )
...
class Config:
    ...
    def __init__(self, reload: bool = False):
        ...
        # 确定模式：优先使用已设置的类变量 Config.mode，
        # 否则尝试从 temp/mode.txt 读取，最后默认为 TEST。
            ...
            try:
                with open("temp/mode.txt", "r", encoding="utf-8") as f:
                    ...
                    if mode_text in [m.name for m in Mode]:
                        ...
                        # 文件内容无效，默认为 TEST
                        warnings.warn(
                            f"Invalid mode '{mode_text}' found in temp/mode.txt, defaulting to TEST."
                        )
    ...
    def _determine_device(self) -> list[str]:
        """确定所有可用的计算设备 (cuda:0, cuda:1, ..., mps, cpu)。"""
        devices = []
    ...
    def _load_hierarchical_config(self):
        ...
        if mode and mode != Mode.TEST:
            ...
            pass  # Keep the pass or remove else block
    ...
    def _load_from_file(self, filename: str):
        ...
        print(f"DEBUG [Load]: Attempting to load '{file_path}'...")
        if os.path.exists(file_path):
            print(f"DEBUG [Load]: File '{file_path}' exists. Reading...")
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    yaml_data = yaml.safe_load(f)
                    if yaml_data:
                        print(
                            f"DEBUG [Load]: YAML data loaded from '{file_path}'. Overriding attributes..."
                        )
                        self._override_attributes(self, yaml_data)
                        print(
                            f"DEBUG [Load]: Attributes overridden from '{file_path}'."
                        )
                    else:
                        print(f"DEBUG [Load]: File '{file_path}' is empty.")
                        pass  # Keep the pass or remove else block
            except yaml.YAMLError as e:
                warnings.warn(
                    f"Error loading configuration from {file_path}: YAML parsing error: {e}",
                    RuntimeWarning,
                )
                print(f"DEBUG [Load]: YAML error in '{file_path}'.")
            except Exception as e:
                warnings.warn(
                    f"Error loading configuration from {file_path}: {e}", RuntimeWarning
                )
                print(f"DEBUG [Load]: Other error in '{file_path}': {e}")
        else:
            print(
                f"DEBUG [Load]: File '{file_path}' does not exist (os.path.exists returned False)."
            )
        ...
        for key, value in data.items():
            if hasattr(obj, key):
                ...
                if isinstance(value, dict) and hasattr(target_attr, "__dict__"):
                    # Recursively override attributes for nested objects
                    self._override_attributes(target_attr, value)
                else:
                    # Handle potential None value for optional configs like ReduceLROnPlateau
                    try:
                        setattr(obj, key, value)
                    except AttributeError as e:
                        warnings.warn(
                            f"Could not set attribute '{key}' on {type(obj).__name__}: {e}",
                            RuntimeWarning,
                        )
            # If the attribute doesn't exist on the object, add it.
            # This allows adding new config parameters via YAML easily.
            # Be cautious with this, as typos in YAML keys will create new attributes.
            else:
                setattr(obj, key, value)
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if yaml_data:
                    ...
                    # 处理空文件情况
                    print(
                        f"  - 版本文件 {os.path.basename(file_path)} 为空，未做更改。"
                    )
                    return True  # 认为加载空文件成功
        except yaml.YAMLError as e:
            warnings.warn(
                f"加载版本文件时出错 {file_path}: YAML 解析错误: {e}", UserWarning
            )
            return False
        except Exception as e:
            ...
            try:
                if isinstance(item, dict):
                    ...
                    cleaned_item = {
                        ...
                        if k not in exclusions
                        and not k.startswith("__")  # 排除魔法方法和 exclusions
                    }
                elif isinstance(item, list):
                    # 处理列表
                    cleaned_item = [
                        clean_recursive(sub_item, visited_ids) for sub_item in item
                    ]
    ...
    try:
        ...
        def capture_config_state(cfg):
            ...
            state = {
                ...
                "train.num_epochs": (
                    cfg.train.num_epochs if hasattr(cfg.train, "num_epochs") else "N/A"
                ),
                "data.window_size": cfg.data.window_size,
                "Adam.lr": cfg.Adam.lr if cfg.Adam else "N/A",
                "OneCycleLR.max_lr": cfg.OneCycleLR.max_lr if cfg.OneCycleLR else "N/A",
                # 可以添加更多需要对比的配置项
            }
            return state
        ...
        def demonstrate_mode(mode_to_set: Mode, prev_cfg_dict: dict):
            ...
            compare_and_print(
                "train.num_epochs",
                current_state["train.num_epochs"],
                prev_cfg_dict,
                mode_to_set.name,
            )
            compare_and_print(
            ...
            compare_and_print(
                "Adam.lr",
                current_state["Adam.lr"],
                prev_cfg_dict,
                mode_to_set.name,
            )
            compare_and_print(
                "OneCycleLR.max_lr",
                current_state["OneCycleLR.max_lr"],
                prev_cfg_dict,
                mode_to_set.name,
            )
            print("--- 模式演示结束 ---")
