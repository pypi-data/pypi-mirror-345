...
#include "RAFourSteelPCPlaneStress.h"
...
static int numRAFourSteelRCPlaneStressMaterials = 0;

OPS_Export void * OPS_ADD_RUNTIME_VPV(OPS_RAFourSteelPCPlaneStressMaterial)
{
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nRAFourSteelRCPlaneStress tag: " << tag << endln;
    return 0;
  }
  ...
  theMaterial = new RAFourSteelPCPlaneStress (tag, 
					      rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "RAFourSteelRCPlaneStress tag: " << tag << endln;
    return 0;
  }
...
RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress ( int      tag, 
						     double   RHO,
						     UniaxialMaterial *t1,
						     UniaxialMaterial *t2,
						     UniaxialMaterial *s1,
  ...
  fpc(FPC), fy1(FY1), fy2(FY2), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for tendon1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[1] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for tendon2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[4] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for concrete1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[5] == 0 ) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for concrete2\n";
    exit(-1);
  }
    /* FMK */
	...
	(theResponses[6] == 0) || (theResponses[7] == 0)) {
      ...
      opserr << " RAFourSteelPCPLaneStress::RAFourSteelPCPlaneStress - failed to set appropriate materials tag:" << tag << endln;
      exit(-1);
    }
...
double RAFourSteelPCPlaneStress::getRho(void)
{
	return rho;
}


int RAFourSteelPCPlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int RAFourSteelPCPlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int RAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int RAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& RAFourSteelPCPlaneStress::getTangent(void)
{
	return tangent_matrix;
}

const Vector& RAFourSteelPCPlaneStress::getStress(void)
{
...
const Vector& RAFourSteelPCPlaneStress :: getStrain()
{
	return strain_vec;
}
    

const Vector& RAFourSteelPCPlaneStress::getCommittedStress(void)
{
    return stress_vec;
}


const Vector& RAFourSteelPCPlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int RAFourSteelPCPlaneStress::commitState(void)
{
    for (int i=0; i<6; i++)
	{
...
int RAFourSteelPCPlaneStress::revertToLastCommit(void)
{
...
int RAFourSteelPCPlaneStress::revertToStart(void)
{
  int i;
  for (i=0; i<6; i++)
    {
...
NDMaterial* RAFourSteelPCPlaneStress::getCopy(void)
{
  /*
    RAFourSteelPCPlaneStress *clone;
    clone = new RAFourSteelPCPlaneStress();
    *clone = *this;
    return clone;
  //*/
  
  RAFourSteelPCPlaneStress* theCopy =
    new RAFourSteelPCPlaneStress( this->getTag(), 
...
NDMaterial* RAFourSteelPCPlaneStress::getCopy(const char *type)
{
  /*
    RAFourSteelPCPlaneStress *clone;
    clone = new RAFourSteelPCPlaneStress();
    *clone = *this;
    return clone;
  //*/
  RAFourSteelPCPlaneStress* theModel =
    new RAFourSteelPCPlaneStress( this->getTag(), 
				  rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], theMaterial[4], theMaterial[5],
				  angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4, pstrain1, pstrain2, fpc, fy1, fy2, E0, epsc0 );
...
void RAFourSteelPCPlaneStress::Print(OPS_Stream &s, int flag )
{
...
int RAFourSteelPCPlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
    ...
    if (res < 0) {
      opserr << "WARNING RAFourSteelPCPlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
      return res;
	}	      
    ...
    if (res < 0) {
      opserr << "WARNING RAFourSteelPCPlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
      return res;
    }
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (res < 0) {
	opserr << "RAFourSteelPCPlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
	return res;
      }
    }	
...
int RAFourSteelPCPlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelPCPlaneStress::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelPCPlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[6];
    if (theMaterial == 0) {
      opserr << "RAFourSteelPCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "RAFourSteelPCPlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "RAFourSteelPCPlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "RAFourSteelPCPlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int RAFourSteelPCPlaneStress::determineTrialStress(void)
{   
	// Define i, j, k
	int i, j, k;
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "RAFourSteelPCPlaneStress::determineTrialStress: Failure to calculate citaR\n";
	...
	{
    	opserr << "RAFourSteelPCPlaneStress::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
	}
	...
	if ( v12*v21==1.0 )
	{
		opserr << "RAFourSteelPCPlaneStress::determineTrialStress: failure to get matrix [V]!\n";
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "RAFourSteelPCPlaneStress::determineTrialStress: Failure to calculate principal stress direction\n";
