...
async def fetch_hubspot_lead_info(
    ...
    async with aiohttp.ClientSession() as session:
        if hubspot_id:
            ...
            if linkedin_url:
                filters.append({
                    "propertyName": "linkedinbio", "operator": "EQ", "value": linkedin_url
                })
            if phone_number:
                filters.append({
            ...
            payload = {
                ...
                "properties": ["firstname","lastname","email","phone","linkedinbio"],
                "limit": 1
            }
...
async def update_crm_contact_record_function(
    ...
    """
    Create or update a HubSpot contact (NON-marketing contact).

    - If is_update=True, we attempt to find an existing contact (by ID or filters).
      If not found, we'll create a new one anyway (i.e., "upsert" behavior).
    - If is_update=False, always create a new contact.
    - If a property name in PROPERTY_MAPPING doesn't exist in your HubSpot portal,
      we skip it to avoid errors.
    - 'tags' (if provided) is stored in a custom property 'my_tags' (semicolon-separated).

    Additional custom logic:
      - We build a lead info "block" from first_name, last_name, email, user_linkedin_url,
        phone, job_title, organization info, etc.
      - If "dhisana_lead_information" is a valid property, we append or set it with that text.
      - Otherwise we create a note with that text.
    """
    HUBSPOT_API_KEY = get_hubspot_access_token(tool_config)
    headers = {
        "Authorization": f"Bearer {HUBSPOT_API_KEY}",
    ...
    # ----------------------------------------------------------------
    # 0) Build the appended string for lead info
    # ----------------------------------------------------------------
    lead_info_parts = []
    fname = contact_values.get("first_name", "")
    lname = contact_values.get("last_name", "")
    if fname or lname:
        lead_info_parts.append(f"Name: {fname} {lname}".strip())
    if contact_values.get("email"):
        lead_info_parts.append(f"Email: {contact_values['email']}")
    if contact_values.get("user_linkedin_url"):
        lead_info_parts.append(f"LinkedIn: {contact_values['user_linkedin_url']}")
    if contact_values.get("phone"):
        lead_info_parts.append(f"Phone: {contact_values['phone']}")
    if contact_values.get("job_title"):
        lead_info_parts.append(f"Job Title: {contact_values['job_title']}")
    if contact_values.get("organization_name"):
        lead_info_parts.append(f"Organization: {contact_values['organization_name']}")
    if contact_values.get("organization_domain"):
        lead_info_parts.append(f"Domain: {contact_values['organization_domain']}")
    if contact_values.get("research_summary"):
        lead_info_parts.append(f"Research Summary: {contact_values['research_summary']}")

    compiled_lead_summary = "\n".join(lead_info_parts)

    # ----------------------------------------------------------------
    # 1) Convert contact_values to valid HubSpot properties
    # ----------------------------------------------------------------
    ...
    for k, v in contact_values.items():
        ...
        if k in PROPERTY_MAPPING:
            mapped_name = PROPERTY_MAPPING[k]
            if mapped_name in valid_props:
                hubspot_props[mapped_name] = v

    # If user provided tags, store them in "my_tags" if valid
    if tags and "my_tags" in valid_props:
        hubspot_props["my_tags"] = ";".join(tags)

    # ----------------------------------------------------------------
    # 2) If is_update=True, try to find an existing contact
    # ----------------------------------------------------------------
    found_contact_id = None
    if is_update:
        found_contact_id = hubspot_contact_id
        if not found_contact_id:
            # Attempt to find a match by email, user_linkedin_url, or first/last/domain
            filters = []
            if email:
                filters.append({"propertyName": "email", "operator": "EQ", "value": email})
            elif user_linkedin_url and "linkedinbio" in valid_props:
            ...
            if filters:
                search_res = await search_hubspot_objects(
                    ...
                    limit=1,
                    tool_config=tool_config,
                    properties=["email", "firstname", "lastname", "domain", "dhisana_lead_information", "company", "jobtitle", "phone", "linkedinbio"]
                )
        ...
        if found_contact_id:
            # ----------------------------------------------------------------
            # 3) We found an existing contact. Retrieve properties
            # ----------------------------------------------------------------
            contact_data = await _get_contact_by_id(found_contact_id, headers)
            current_properties = contact_data.get("properties", {})
            existing_notes = current_properties.get("dhisana_lead_information", "")

            # If "dhisana_lead_information" is valid => append
            if "dhisana_lead_information" in valid_props:
                if compiled_lead_summary:
                    new_notes_value = existing_notes.strip()
                    if new_notes_value:
                        new_notes_value += "\n\n" + compiled_lead_summary
                    else:
                        new_notes_value = compiled_lead_summary

                    hubspot_props["dhisana_lead_information"] = new_notes_value
            else:
                # Not valid => create a note instead
                if compiled_lead_summary:
                    await create_hubspot_note_for_customer(
                        customer_id=found_contact_id,
                        note=compiled_lead_summary,
                        tool_config=tool_config
                    )

            # ----------------------------------------------------------------
            # 4) Update the contact with any normal property changes
            # ----------------------------------------------------------------
            update_url = f"https://api.hubapi.com/crm/v3/objects/contacts/{found_contact_id}"
            payload = {"properties": hubspot_props}
            async with aiohttp.ClientSession() as session:
                async with session.patch(update_url, headers=headers, json=payload) as resp:
                    result = await resp.json()
                    if resp.status != 200:
                        raise RuntimeError(f"Error updating contact: {resp.status}, response: {result}")
                    return result
        else:
        ...
        # ----------------------------------------------------------------
        # is_update=False -> always create new
        #   We'll do the same logic re: "dhisana_lead_information" property or note
        # ----------------------------------------------------------------
        return await _create_new_contact_with_note_or_property(
            hubspot_props,
            compiled_lead_summary,
...
# ---------------------------------------------------------------------
# Helper for creating a new contact and either setting property or note
# ---------------------------------------------------------------------
async def _create_new_contact_with_note_or_property(
    properties: Dict[str, Any],
    compiled_lead_summary: str,
    valid_props: List[str],
    headers: Dict[str, str],
    tool_config: Optional[List[Dict]]
) -> Dict[str, Any]:
    """
    Create a new contact. If "dhisana_lead_information" is a valid property,
    set it to the compiled_lead_summary. Otherwise, create a note afterwards.
    """
    # If property is valid and we have any summary, set it before creation
    if "dhisana_lead_information" in valid_props and compiled_lead_summary:
        properties["dhisana_lead_information"] = compiled_lead_summary
        # Then just create
        create_result = await _create_new_hubspot_contact(properties, headers)
        return create_result
    else:
        # Create the contact first, then attach a note with compiled_lead_summary (if any)
        create_result = await _create_new_hubspot_contact(properties, headers)
        if compiled_lead_summary:
            # create_result should have .id = newly created contact ID
            new_contact_id = create_result.get("id")
            if new_contact_id:
                await create_hubspot_note_for_customer(
                    customer_id=new_contact_id,
                    note=compiled_lead_summary,
                    tool_config=tool_config
                )
        return create_result
...
# ---------------------------------------------------------------------
# Original contact creation method
# ---------------------------------------------------------------------
async def _create_new_hubspot_contact(
    ...
    """
    Create a new contact in HubSpot as a NON-marketing contact.
    """
    create_url = "https://api.hubapi.com/crm/v3/objects/contacts"
    create_payload = {
        "properties": properties
        # "marketingContacts": { "marketingContact": False },
    }
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(create_url, headers=headers, json=create_payload) as resp:
            ...
            if resp.status not in (200, 201):
                raise RuntimeError(
                    f"Error creating new contact: status {resp.status}, response: {result}"
                )
            return result
...
async def _fetch_all_contact_properties(headers: Dict[str, str]) -> List[str]:
    """
    Fetch all contact property names via the HubSpot v3 properties API.
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers) as r:
            if r.status != 200:
                text = await r.text()
                raise RuntimeError(f"Error fetching contact properties: {r.status}, {text}")
            data = await r.json()
...
async def _get_contact_by_id(contact_id: str, headers: Dict[str, str]) -> Dict[str, Any]:
    """
    Fetch a single contact by ID in HubSpot (v3).
    """
    url = f"https://api.hubapi.com/crm/v3/objects/contacts/{contact_id}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers) as resp:
            ...
            if resp.status != 200:
                raise RuntimeError(
                    f"Error fetching contact ID {contact_id}: {resp.status}, {data}"
                )
            return data


# ---------------------------------------------------------------------
# Provided: create a note function
# ---------------------------------------------------------------------
async def create_hubspot_note_for_customer(
    ...
    """
    Create and attach a note (v3) to a customer (contact) in HubSpot 
    using the customer's ID or email.
    """
    HUBSPOT_API_KEY = get_hubspot_access_token(tool_config)
    if not (customer_id or email):
        raise ValueError("Either 'customer_id' or 'email' must be provided.")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # 2) Create a note object and associate it to the contact
