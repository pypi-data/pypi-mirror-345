...
from pytupli.server.db.db_handler import BaseDatabaseHandler
...
permissions_dict = {
    ...
    '/auth/signup': None if OPEN_SIGNUP_MODE else 'user_management',
    '/auth/list-users': 'user_management',
    '/auth/list-roles': 'user_management',
    '/auth/change-password': 'user_management',
    '/auth/change-roles': 'user_management',
    '/auth/delete-user': 'user_management',
...
async def validate_token(token: str, db_handler: BaseDatabaseHandler) -> User:
    """
...
async def get_user_rights(user: str | dict | User, db_handler: BaseDatabaseHandler) -> list[str]:
    """
...
async def get_entry(request: Request, db_handler: BaseDatabaseHandler):
    """
    Function to get the entry based on the submitted id from the request from the database.
    Args:
        request (Request): The request object.
        db_handler (BaseDatabaseHandler): The database handler.
    Returns:
...
async def check_authentication(
    ...
    db_handler: BaseDatabaseHandler = Depends(get_db_handler),
) -> User:
    """
    ...
    if is_list_request and 'auth' not in request.url.path:
        is_public = True

    # special rule for signup
    if request.url.path == '/auth/signup' and OPEN_SIGNUP_MODE:
        is_public = True
    ...
    if is_delete_request and entry is None:
        return user
...
async def inject_read_permission_filter(
    ...
    Args:
        ...
        db_handler (BaseDatabaseHandler): The database handler (to fetch user rights).
