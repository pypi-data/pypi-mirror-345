...
def setup_twenty_seven(options: argparse.Namespace) -> object:
    ...
    log.info(f'- Setup <00> Description ({setup.description})')
    ...
    if setup.comment:
        log.info(f'- Setup <00> Comment ({setup.comment})')
    else:
...
def main(options: argparse.Namespace) -> int:
    ...
    if cfg.comment:
        ...
        log.info(f'^ Added comment ({cfg.comment}) to ticket {c_key}; CLK={clk}')
