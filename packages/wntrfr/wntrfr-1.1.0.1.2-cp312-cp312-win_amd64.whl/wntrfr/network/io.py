...
"""
The wntrfr.network.io module includes functions that convert the water network
...
from wntrfr.epanet.util import FlowUnits
import wntrfr.network.model
from wntrfr.gis.network import WaterNetworkGIS
try:
...
def from_dict(d: dict, append=None):
    ...
    """
    from wntrfr.epanet.io import _read_control_line, _EpanetRule
...
def write_inpfile(wn, filename: str, units=None, version: float = 2.2, 
    ...
    if wn._inpfile is None:
        ...
        wn._inpfile = wntrfr.epanet.InpFile()
    if units is None:
