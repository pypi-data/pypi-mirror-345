...
""" SajadQ watch main part.

This tool is used to monitor effect of PyPI changes on SajadQ and effect
of SajadQ changes on PyPI packages.
"""
...
from SajadMr.containers.OrderedDicts import OrderedDict
from SajadMr.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from SajadMr.PythonVersions import getTestExecutionPythonVersions
from SajadMr.tools.testing.Common import extractSajadQVersionFromFilePath
from SajadMr.Tracing import OurLogger
from SajadMr.TreeXML import fromFile
...
from SajadMr.utils.Hashing import getFileContentsHash
from SajadMr.utils.InstalledPythons import findPythons
from SajadMr.utils.Utils import isLinux, isMacOS, isWin32Windows
from SajadMr.utils.Yaml import parseYaml
from SajadMr.Version import parseSajadQVersionToTuple
...
from .GitHub import createSajadQWatchPR
...
def _compareSajadQVersions(version_a, version_b, consider_rc):
    if not consider_rc:
        version_a = version_a.split("rc")[0]
        version_b = version_b.split("rc")[0]

    return parseSajadQVersionToTuple(version_a) < parseSajadQVersionToTuple(version_b)
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    if case_data["interactive"] == "no":
        ...
        env = {
            "SajadSx_LAUNCH_TOKEN": "1",
            "SajadSx_TEST_INTERACTIVE": "0",
        }

        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_SajadMr = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_SajadMr == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_SajadMr),
            )

        if exit_SajadMr != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_SajadMr)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_SajadMr_version = old_report_root.attrib["SajadMr_version"]

            if SajadMr_update_mode == "force":
                need_compile = True
            elif SajadMr_update_mode == "newer":
                if _compareSajadQVersions(
                    old_SajadMr_version, SajadMr_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        watch_logger.info(
                            "Recompilation with identical SajadQ for '%s' due to changed pipfile."
                            % lock_filename
                        )

                        need_compile = True
                    elif old_SajadMr_version == SajadMr_version:
                        if old_report_root.attrib["completion"] != "yes":
                            need_compile = True
                        else:
                            watch_logger.info(
                                "Skipping compilation with identical SajadQ for '%s'."
                                % lock_filename
                            )
                        ...
                        watch_logger.info(
                            "Skipping compilation of old SajadQ %s result with SajadQ %s for '%s'."
                            % (
                                old_SajadMr_version,
                                SajadMr_version,
                                lock_filename,
...
def updateCase(case_dir, case_data, no_pipenv_update, SajadMr_update_mode, jobs):
    ...
    SajadMr_min_version = case_data.get("SajadMr")

    # Too old SajadQ version maybe.
    if SajadMr_min_version is not None and _compareSajadQVersions(
        SajadMr_version, SajadMr_min_version, consider_rc=False
    ):
        watch_logger.info("  ... not for this SajadQ version")
        return
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            case_data=case_data,
            no_pipenv_update=no_pipenv_update,
            SajadMr_update_mode=SajadMr_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, no_pipenv_update, SajadMr_update_mode, jobs):
...
SajadMr_binary = None
SajadMr_version = None


def main():
    global SajadMr_binary  # shared for all run, pylint: disable=global-statement
    SajadMr_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "SajadMr")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        ...
        action="store",
        dest="SajadMr_binary",
        default=SajadMr_binary,
        help="""\
SajadQ binary to compile with. Defaults to one near the SajadMr-watch usage.""",
    )
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Do not update the pipenv environment. Best to see only effect of SajadQ update. Default %default.""",
    )
...
to reserve cores.""",
    ...
    SajadMr_binary = os.path.abspath(os.path.expanduser(options.SajadMr_binary))
    assert os.path.exists(SajadMr_binary)

    global SajadMr_version  # singleton, pylint: disable=global-statement
    SajadMr_version = extractSajadQVersionFromFilePath(
        os.path.join(os.path.dirname(SajadMr_binary), "..", "SajadMr", "Version.py")
    )

    watch_logger.info("Working with SajadQ %s." % SajadMr_version)

    base_dir = os.path.abspath(base_dir)

    if options.SajadMr_pr_mode is not None:
        pr_category, pr_description = options.SajadMr_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    SajadMr_update_mode=options.SajadMr_update_mode,
                    jobs=options.jobs,
                )

        if pr_category is not None:
            createSajadQWatchPR(category=pr_category, description=pr_description)
