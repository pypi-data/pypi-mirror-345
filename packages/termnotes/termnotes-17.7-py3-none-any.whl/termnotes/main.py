...
def read_note(folder, name):
  ...
  for i, line in enumerate(lines):
    ...
    if not tags_line_processed and clean_line.lower().startswith("tags:"):
      ...
      if tags_str: # Only style if there are tags
        ...
        styled_tags = [f"\033[1;35m#{tag}\033[0m" for tag in plain_tags if tag]
  ...
  # --- Markdown Styling ---
      ...
      modified_line = f"\033[1m{clean_line.lstrip('#').strip()}\033[0m"
      modified_lines.append(modified_line)
      words.extend(clean_line.lstrip('#').strip().split())
    elif clean_line.startswith("-[]"):
      modified_line = f"\033[1;31m- \033[0m{clean_line.lstrip('-[]').strip()}"
      modified_lines.append(modified_line)
      words.extend(clean_line.lstrip('-[]').strip().split())
    elif clean_line.startswith("-[+]"):
      modified_line = f"\033[1;32m+ \033[0m{clean_line.lstrip('-[+]').strip()}"
      modified_lines.append(modified_line)
      words.extend(clean_line.lstrip('-[+]').strip().split())
    elif clean_line.startswith("- "):
