...
> **Platform Support Notice:** This server currently supports macOS and Linux environments. Windows support is coming soon. Some path handling and filesystem operations may not work correctly on Windows in the current version.
...
2. Test with local system-level installation (important pre-publishing step):
   ```bash
   # First, uninstall any existing system installation 
   uv pip uninstall --system mcp-code-understanding
   
   # Clean up any leftover cache/config directories
   rm -rf ~/.cache/mcp-code-understanding ~/.config/mcp-code-understanding
   
   # Install from the current directory to system Python
   uv pip install --system .
   
   # Verify the script is available
   which mcp-code-understanding
   
   # Verify it works with your MCP client
...
5. Verify the built packages:
   ```bash
   ls dist/
   ```

6. Upload to PyPI (use TestPyPI first if unsure):
   ```bash
