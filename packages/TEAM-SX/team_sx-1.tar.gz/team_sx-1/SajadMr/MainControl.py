...
""" This is the main actions of SajadQ.
...
from SajadMr.build.DataComposerInterface import runDataComposer
from SajadMr.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from SajadMr.code_generation.ConstantCodes import (
...
from SajadMr.importing.Importing import locateModule, setupImportingFromOptions
from SajadMr.importing.Recursion import (
    scanIncludedPackage,
...
from SajadMr.PostProcessing import executePostProcessing
from SajadMr.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
...
from SajadMr.Serialization import ConstantAccessor
from SajadMr.Tracing import general, inclusion_logger, pgo_logger
from SajadMr.tree import SyntaxErrors
from SajadMr.tree.ReformulationMultidist import createMultidistMainSourceCode
from SajadMr.utils import InstanceCounters
from SajadMr.utils.Distributions import getDistribution
...
from SajadMr.utils.Importing import (
    getPackageDirFilename,
    getSharedLibrarySuffix,
)
from SajadMr.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from SajadMr.utils.ModuleNames import ModuleName
from SajadMr.utils.ReExecute import callExecProcess, reExecuteSajadQ
...
def _runPythonPgoBinary():
    ...
    with withEnvironmentVarOverridden("SajadSx_PGO_OUTPUT", pgo_filename):
...
def runSconsBackend():
    ...
    scons_options["SajadMr_python"] = asBoolStr(isSajadQPython())
    ...
    if int(os.getenv("SajadSx_NOSITE_FLAG", Options.hasPythonFlagNoSite())):
...
SajadQ is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m SajadMr', e.g. use this
'python%s -m SajadMr' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of SajadQ

    At this point, options will be parsed already, SajadQ will be executing
    ...
    general.info(
        "Starting Python compilation with SajadQ %r on Python (flavor %s), %r commercial grade %r."
        % (
            getSajadQVersion(),
            getPythonFlavorName(),
            python_version_str,
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteSajadQ(pgo_filename=pgo_filename)
