...
Plugins: Welcome to SajadQ! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper SajadQ, and some of which are waiting to be created and submitted for
inclusion by you.
...
from SajadMr import Options, OutputDirectories
from SajadMr.__past__ import basestring, iter_modules
from SajadMr.build.DataComposerInterface import deriveModuleConstantsBlobName
from SajadMr.containers.OrderedDicts import OrderedDict
from SajadMr.containers.OrderedSets import OrderedSet
from SajadMr.Errors import SajadQSyntaxError
from SajadMr.freezer.IncludedDataFiles import IncludedDataFile
from SajadMr.freezer.IncludedEntryPoints import IncludedEntryPoint
from SajadMr.ModuleRegistry import addUsedModule
from SajadMr.PythonVersions import python_version
from SajadMr.Tracing import plugins_logger, printLine
...
from SajadMr.utils.Importing import importFileAsModule
from SajadMr.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import SajadQPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, SajadQPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "SajadQPluginDetector", ""
            ) != plugin_class.__name__.replace("SajadQPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match SajadQPlugin* and SajadQPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import SajadMr.plugins.standard

    _loadPluginClassesFromPackage(SajadMr.plugins.standard)

    try:
        import SajadMr.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(SajadMr.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from SajadMr.importing.Importing import getModuleNameAndKindFromFilename
        from SajadMr.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from SajadMr.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        if parent_package_name is not None:
            from SajadMr.importing.Importing import locateModule
        ...
        def getModuleSourceCode():
            ...
            from SajadMr.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from SajadMr.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from SajadMr.ModuleRegistry import getDoneModules
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    ...
                    if not key.startswith("SajadMr_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from SajadMr.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in SajadQ".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not SajadQPluginBase
            and issubclass(obj, SajadQPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or SajadQ arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of SajadQ plugin '%s' not given."
