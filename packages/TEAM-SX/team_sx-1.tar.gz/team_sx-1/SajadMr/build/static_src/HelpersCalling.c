...
#if PYTHON_VERSION < 0x380 && !defined(_SajadSx_EXPERIMENTAL_DISABLE_UNCOMPILED_FUNCTION_CALL_OPT)
...
PyObject *CALL_METHOD_WITH_POS_ARGS(PyThreadState *tstate, PyObject *source, PyObject *attr_name,
    ...
    if (PyInstance_Check(source)) {
        ...
        if (called_object != NULL) {
            ...
            if (descr_get == SajadQ_Function_Type.tp_descr_get) {
                return SajadQ_CallMethodFunctionPosArgs(tstate, (struct SajadQ_FunctionObject const *)called_object,
...
#endif
    {
        ...
        if (type->tp_getattro != NULL) {
            ...
            called_object = (*type->tp_getattr)(source, (char *)SajadQ_String_AsString_Unchecked(attr_name));
        } else {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT2(PyExc_AttributeError, "'%s' object has no attribute '%s'",
                                                type->tp_name, SajadQ_String_AsString_Unchecked(attr_name));
...
char const *GET_CALLABLE_NAME(PyObject *object) {
    if (SajadQ_Function_Check(object)) {
        return SajadQ_String_AsString(SajadQ_Function_GetName(object));
    } else if (SajadQ_Generator_Check(object)) {
        return SajadQ_String_AsString(SajadQ_Generator_GetName(object));
    } else if (PyMethod_Check(object)) {
        return PyEval_GetFuncName(PyMethod_GET_FUNCTION(object));
    } else if (PyFunction_Check(object)) {
        return SajadQ_String_AsString(((PyFunctionObject *)object)->func_name);
    }
#if PYTHON_VERSION < 0x300
    else if (PyInstance_Check(object)) {
        return SajadQ_String_AsString(((PyInstanceObject *)object)->in_class->cl_name);
    } else if (PyClass_Check(object)) {
        return SajadQ_String_AsString(((PyClassObject *)object)->cl_name);
    }
#endif
...
char const *GET_CALLABLE_DESC(PyObject *object) {
    if (SajadQ_Function_Check(object) || SajadQ_Generator_Check(object) || PyMethod_Check(object) ||
...
char const *GET_CLASS_NAME(PyObject *class_object) {
    if (class_object == NULL) {
        ...
        if (PyClass_Check(class_object)) {
            return SajadQ_String_AsString(((PyClassObject *)class_object)->cl_name);
        }
#endif
...
void formatCannotInstantiateAbstractClass(PyThreadState *tstate, PyTypeObject *type) {
    ...
    SET_CURRENT_EXCEPTION_TYPE0_FORMAT3(PyExc_TypeError,
                                        ...
                                        method_count > 1 ? "s" : "", SajadQ_String_AsString(joined));
