...
class TestCreateSceneWithExternalResources:
    def test_create_scene_with_external_resources(
        ...
        time.sleep(5)  # Since it's first indexed it just needs to move to pending before it becomes queryable
        status = wait_for_scene_job(client, response.scene_uuid, timeout=60)
        assert status == IAM.SceneStatus.Created, f"Scene creation failed with status: {status}"
