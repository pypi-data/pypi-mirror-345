...
def get_all_fields(typename: str, typestore: Any, current_prefix: str = '', visited: Optional[Set[str]] = None) -> List[Tuple[str, str, bool]]:
    ...
    try:
        ...
        return []
         ...
         task_logger.error(f"Error accessing definition for type '{typename}': {e}", exc_info=True)
         return []
    ...
    if isinstance(field_defs, dict):
        ...
        task_logger.error(f"Unexpected structure for field definitions of type '{typename}': {type(field_defs)}.")
        field_iterator = []
    ...
    for field_info in field_iterator:
        try:
            ...
            task_logger.warning(f"Unexpected element structure in field definitions for type '{typename}': {field_info}. Skipping.")
            continue
        ...
        try:
            if not isinstance(field_type_tuple, tuple) or len(field_type_tuple) < 2:
                 task_logger.warning(f"Unexpected field_type_info format for field '{flat_name}': {field_type_tuple}. Skipping.")
                 continue
            ...
            if node_type_int == 1: # BASE
                if isinstance(type_details, (list, tuple)) and len(type_details) > 0:
                    ...
                    task_logger.warning(f"Unexpected type_details format for BASE type: {type_details}. Field: '{flat_name}'. Skipping.")
                    continue
                is_array = False
                ...
                if isinstance(type_details, str):
                    ...
                    task_logger.warning(f"Unexpected type_details format for NAME type: {type_details}. Field: '{flat_name}'. Skipping.")
                    continue
                is_array = False
            elif node_type_int == 3 or node_type_int == 4: # ARRAY or SEQUENCE
                is_array = True
                if not isinstance(type_details, (list, tuple)) or len(type_details) < 1:
                     task_logger.warning(f"Unexpected type_details format for ARRAY/SEQUENCE: {type_details}. Field: '{flat_name}'. Skipping.")
                     continue

                element_type_tuple = type_details[0]
                if not isinstance(element_type_tuple, (list, tuple)) or len(element_type_tuple) < 2:
                     task_logger.warning(f"Unexpected element_type_tuple format for ARRAY/SEQUENCE: {element_type_tuple}. Field: '{flat_name}'. Skipping.")
                     continue

                element_nodetype_int, element_details = element_type_tuple

                if element_nodetype_int == 1: # BASE element
                    if isinstance(element_details, (list, tuple)) and len(element_details) > 0:
                        element_type_name = element_details[0]
                    else:
                         task_logger.warning(f"Unexpected element_details format for ARRAY/SEQUENCE -> BASE: {element_details}. Field: '{flat_name}'. Skipping.")
                         continue
                elif element_nodetype_int == 2: # NAME element
                    if isinstance(element_details, str):
                        element_type_name = element_details
                    else:
                         task_logger.warning(f"Unexpected element_details format for ARRAY/SEQUENCE -> NAME: {element_details}. Field: '{flat_name}'. Skipping.")
                         continue
                else:
                     task_logger.warning(f"Nested ARRAY/SEQUENCE of type {element_nodetype_int} not fully handled for field '{flat_name}'. Skipping.")
                     continue
            else:
                 task_logger.warning(f"Unhandled node type integer '{node_type_int}' for field '{flat_name}'. Skipping.")
                 continue

        except (IndexError, ValueError, TypeError) as parse_err:
            task_logger.warning(f"Error parsing field type tuple for field '{flat_name}' (type: {typename}, info: {field_type_tuple}): {parse_err}. Skipping.")
            continue

        if element_type_name is None:
             task_logger.warning(f"Could not determine element type name for field '{field_name}' (type: {typename}, info: {field_type_tuple}). Skipping.")
             continue
        ...
        if is_complex:
             ...
             if nested_fields:
                 if is_array:
                      task_logger.warning(f"Field '{flat_name}' is an array of complex type '{element_type_name}'. Skipping detailed fields.")
                 else:
