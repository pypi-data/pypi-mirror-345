...
def get_available_heights(vid: str, proxies: list[str]) -> list[int]:
    """
    Fetch available video heights using the first proxy and a browser-like user agent.
    """
    opts = {
        "quiet": True,
        "no_warnings": True,
...
async def fetch_pair(vid: str, height: str, proxies: list[str]) -> dict:
    ...
    def worker():
        ...
        # verify URLs are HEAD‑OK before finishing
...
async def _amain():
    ...
    heights = get_available_heights(vid, proxies)  # Pass proxies to use for metadata fetch
