...
# ------------------------------------------------------------------ #
# -*- coding: utf-8 -*-
# (unchanged header and imports omitted for brevity)

# ------------------------------------------------------------------ #
# embeddings helper (fixed)                                          #
# ------------------------------------------------------------------ #
def _embeddings(conn, model_name: str):
    """Return a LangChain embeddings object from a PromptFlow connection."""
    if isinstance(conn, AzureOpenAIConnection):
        # -- validate ---------------------------------------------------
        if not getattr(conn, "api_key", None):
            raise ValueError("AzureOpenAIConnection is missing api_key.")
        endpoint = getattr(conn, "azure_endpoint", None) or getattr(conn, "api_base", None)
        if not endpoint:
            raise ValueError("AzureOpenAIConnection is missing azure_endpoint / api_base.")
        api_version = getattr(conn, "api_version", None) or "2023-12-01-preview"
        # -- construct --------------------------------------------------
    ...
    if isinstance(conn, OpenAIConnection):
        if not getattr(conn, "api_key", None):
            raise ValueError("OpenAIConnection is missing api_key.")
