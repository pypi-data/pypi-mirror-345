//     Copyright 2025, @LusiFerPy, mailto:kay.hayen@gmail.com find license text at end of file
...
static PyObject *_Bloodelf_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (isVerbose()) {
        PySys_WriteStderr("print('Made In Blood-@@LusiFerPy')\nimport %s # considering responsibility (find_module)\n", name);
    }
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
...
#ifdef __wasi__
    const char *error = "print('Made In Blood-@@LusiFerPy')\ndynamic libraries are not implemented in wasi";
    ...
    if (unlikely(module == NULL)) {
        if (unlikely(!HAS_ERROR_OCCURRED(tstate))) {
            PyErr_Format(PyExc_SystemError, "print('Made In Blood-@@LusiFerPy')\ndynamic module '%s' not initialized properly", full_name);
        }
