...
class IMaxNumTransfers(BaseModel):
    """
    IMaxNumTransfers
    """ # noqa: E501
    overall_max_num_transfers: NumberType = Field(description="The overall maximum number of transfers for the badgeIDs and ownershipTimes. Running tally that includes all transfers that match this approval.", alias="overallMaxNumTransfers")
    per_to_address_max_num_transfers: NumberType = Field(description="The maximum number of transfers for the badgeIDs and ownershipTimes for each to address. Running tally that includes all transfers from each unique to address that match this approval.", alias="perToAddressMaxNumTransfers")
    per_from_address_max_num_transfers: NumberType = Field(description="The maximum number of transfers for the badgeIDs and ownershipTimes for each from address. Running tally that includes all transfers from each unique from address that match this approval.", alias="perFromAddressMaxNumTransfers")
    per_initiated_by_address_max_num_transfers: NumberType = Field(description="The maximum number of transfers for the badgeIDs and ownershipTimes for each initiated by address. Running tally that includes all transfers from each unique initiated by address that match this approval.", alias="perInitiatedByAddressMaxNumTransfers")
    amount_tracker_id: StrictStr = Field(description="The ID of the approval tracker. This is the key used to track tallies.", alias="amountTrackerId")
    __properties: ClassVar[List[str]] = ["overallMaxNumTransfers", "perToAddressMaxNumTransfers", "perFromAddressMaxNumTransfers", "perInitiatedByAddressMaxNumTransfers", "amountTrackerId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMaxNumTransfers from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of overall_max_num_transfers
        if self.overall_max_num_transfers:
            _dict['overallMaxNumTransfers'] = self.overall_max_num_transfers.to_dict()
        # override the default output from pydantic by calling `to_dict()` of per_to_address_max_num_transfers
        if self.per_to_address_max_num_transfers:
            _dict['perToAddressMaxNumTransfers'] = self.per_to_address_max_num_transfers.to_dict()
        # override the default output from pydantic by calling `to_dict()` of per_from_address_max_num_transfers
        if self.per_from_address_max_num_transfers:
            _dict['perFromAddressMaxNumTransfers'] = self.per_from_address_max_num_transfers.to_dict()
        # override the default output from pydantic by calling `to_dict()` of per_initiated_by_address_max_num_transfers
        if self.per_initiated_by_address_max_num_transfers:
            _dict['perInitiatedByAddressMaxNumTransfers'] = self.per_initiated_by_address_max_num_transfers.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMaxNumTransfers from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "overallMaxNumTransfers": NumberType.from_dict(obj["overallMaxNumTransfers"]) if obj.get("overallMaxNumTransfers") is not None else None,
            "perToAddressMaxNumTransfers": NumberType.from_dict(obj["perToAddressMaxNumTransfers"]) if obj.get("perToAddressMaxNumTransfers") is not None else None,
            "perFromAddressMaxNumTransfers": NumberType.from_dict(obj["perFromAddressMaxNumTransfers"]) if obj.get("perFromAddressMaxNumTransfers") is not None else None,
            "perInitiatedByAddressMaxNumTransfers": NumberType.from_dict(obj["perInitiatedByAddressMaxNumTransfers"]) if obj.get("perInitiatedByAddressMaxNumTransfers") is not None else None,
            "amountTrackerId": obj.get("amountTrackerId")
        })
        return _obj
