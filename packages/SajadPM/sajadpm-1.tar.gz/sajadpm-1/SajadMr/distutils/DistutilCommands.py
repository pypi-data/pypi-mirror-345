...
from SajadMr.__past__ import Iterable, unicode
from SajadMr.containers.OrderedSets import OrderedSet
from SajadMr.importing.Importing import (
...
from SajadMr.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from SajadMr.Tracing import wheel_logger
from SajadMr.utils.Execution import check_call
from SajadMr.utils.FileOperations import deleteFile, getFileList, renameFile
from SajadMr.utils.ModuleNames import ModuleName


def setupSajadQDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_SajadMr=True" all
    # build operations (build, bdist_wheel, install etc) will run via SajadQ.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_SajadMr
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from SajadQ compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # SajadQ wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of SajadQ, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]
            ...
            toml_filename = os.getenv("SajadSx_TOML_FILE")
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-SajadQ process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("SajadMr", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("SajadMr", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "SajadMr" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_SajadMr(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            ...
            generator = "SajadQ (%s)" % getSajadQVersion()
