"""
The wntrfr.metrics.hydraulic module contains hydraulic metrics.
"""
...
def water_service_availability(expected_demand, demand):
    ...
    :class:`~wntrfr.metrics.hydraulic.expected_demand` method.
...
def tank_capacity(pressure, wn):
    ...
    assert isinstance(pressure, pd.DataFrame), "pressure must be a pandas DataFrame"
    assert isinstance(wn, wntrfr.network.WaterNetworkModel), "wn must be a wntr WaterNetworkModel"
