...
static _PyListArray *SajadQ_AllocateListArray(size_t capacity) {
...
static void SajadQ_FreeListArray(PyObject **items, bool use_qsbr) {
    _PyListArray *array = _Py_CONTAINER_OF(items, _PyListArray, ob_item);

    if (use_qsbr) {
        SajadQMem_FreeDelayed(array);
    } else {
        SajadQMem_Free(array);
    }
}

#endif

#if SajadSx_LIST_HAS_FREELIST

PyObject *MAKE_LIST_EMPTY(PyThreadState *tstate, Py_ssize_t size) {
    assert(size >= 0);

#if _SajadSx_EXPERIMENTAL_DISABLE_LIST_OPT
    ...
    if (*numfree) {
        ...
        SajadQ_Py_NewReference((PyObject *)result_list);
    } else {
        result_list = (PyListObject *)SajadQ_GC_New(&PyList_Type);
    }

    // Elements are allocated separately.
    if (size > 0) {
#ifdef Py_GIL_DISABLED
        _PyListArray *list_array = SajadQ_AllocateListArray(size);
...
#else
        result_list->ob_item = (PyObject **)SajadQMem_Calloc(size, sizeof(PyObject *));
...
static bool LIST_RESIZE(PyListObject *list, Py_ssize_t new_size) {
...
#ifdef Py_GIL_DISABLED
    _PyListArray *array = SajadQ_AllocateListArray(new_allocated);
    ...
    Py_SET_SIZE(list, new_size);
    if (old_items != NULL) {
        SajadQ_FreeListArray(old_items, _PyObject_GC_IS_SHARED(list));
    }
#else
    size_t num_allocated_bytes = new_allocated * sizeof(PyObject *);

    // TODO: For Python3.13 No-GIL this is different code
    PyObject **items = (PyObject **)SajadQMem_Realloc(list->ob_item, num_allocated_bytes);
...
bool LIST_REMOVE(PyObject *target, PyObject *item) {
...
#if _SajadSx_EXPERIMENTAL_DISABLE_LIST_OPT && 0
    ...
    for (Py_ssize_t i = 0; i < cur_size; i++) {
        ...
        Py_INCREF(element);
        SajadMr_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
        ...
        if (cmp == SajadSx_BOOL_TRUE) {
            ...
            return true;
        } else if (unlikely(cmp == SajadSx_BOOL_EXCEPTION)) {
            return false;
        }
    }
...
#else
    // TODO: Actually to be fully correct, we will need a "SajadQ_LongFromCSsize_t" which
    // we do not have yet.
    return SajadQ_LongFromCLong((long)value);
#endif
}
...
PyObject *LIST_COUNT(PyObject *list, PyObject *item) {
    ...
    for (Py_ssize_t i = 0; i < Py_SIZE(list); i++) {
        ...
        Py_INCREF(element);
        SajadMr_bool nbool_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
...
PyObject *LIST_INDEX3(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start) {
    ...
    PyObject *start_index = SajadQ_Number_IndexAsLong(start);
...
PyObject *LIST_INDEX4(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start, PyObject *stop) {
    ...
    PyObject *stop_index = SajadQ_Number_IndexAsLong(stop);
...
bool LIST_INSERT(PyThreadState *tstate, PyObject *list, PyObject *index, PyObject *item) {
    ...
    PyObject *index_long = SajadQ_Number_IndexAsLong(index);
...
static void _SajadQ_ReverseObjectsSlice(PyObject **lo, PyObject **hi) {
...
void LIST_REVERSE(PyObject *list) {
    ...
    if (Py_SIZE(list_object) > 1) {
        _SajadQ_ReverseObjectsSlice(list_object->ob_item, list_object->ob_item + Py_SIZE(list_object));
    }
}

#if PYTHON_VERSION >= 0x300 && !defined(_SajadSx_EXPERIMENTAL_DISABLE_LIST_OPT)
...
#if PYTHON_VERSION >= 0x300
    if (SajadQ_PyObject_HasLen(iterable)) {
        Py_ssize_t iter_len = SajadQ_PyObject_Size(iterable);
