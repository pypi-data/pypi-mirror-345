"""
*senchar.fits* contains FITS image support functions for senchar.
"""
...
def file_exists(filename: str) -> bool:
    ...
    fe = senchar.utils.make_image_filename(filename)
...
def get_keyword(filename: str, keyword: str, extension: int = 0) -> typing.Any:
    ...
    Image = senchar.utils.make_image_filename(filename)
...
def arith(
    ...
    # open Image1
    filename1 = senchar.utils.make_image_filename(filename1)
    ...
    # open Image2 and get data
        ...
        SCALAR = 0
        filename2 = senchar.utils.make_image_filename(filename2)
        numext2, fext, lext = get_extensions(filename2)
        with pyfits.open(filename2, lazy_load_hdus=False) as im2:
            if numext1 != numext2:
                im2.close()
                raise senchar.exceptions.SencharError("unequal FITS image extensions")
            if MEF:
                data2 = []
                for i in range(1, lext):
    ...
    if filename3 == "":
        filename3 = filename1
        os.remove(filename3)
    filename3 = senchar.utils.make_image_filename(filename3)
...
def combine(
    ...
    if numfiles < 2:
        raise senchar.exceptions.SencharError("two or more images are required")

    header = []  # header for output file
    dataset = []  # combined data

    for fnum, f in enumerate(file_list):
        filename = senchar.utils.make_image_filename(f)
...
# *********************************************************************************************
def _get_data_roi(senchar_roi: list = []) -> list:
    """
    Converts an senchar ROI to a numpy ROI.

    Args:
        senchar_roi: an senchar ROI list, i.e. one-based, columns first:
    ...
    if senchar_roi == []:
        roi = senchar.db.imageroi if senchar.db.get("imageroi") else []
    else:
        roi = senchar_roi
