...
impl QuicTransport {
    ...
    pub async fn connect(&self, addr: SocketAddr, client_id: u64) -> Result<(), MediaTransportError> {
        ...
        let mut client = StreamClient::new(Box::new((*self.source).clone()), client_id).map_err(|e| {
    ...
    pub async fn shutdown(&self) -> Result<(), MediaTransportError> {
        ...
        if let Some(handle) = self.client_handle.remove(&()) {
            handle.1.shutdown().await.map_err(|e| {
