...
import requests
from nebu import Processor, V1ResourceMetaRequest, V1ResourceReference
...
class Human:
    def __init__(
        ...
        if not self.human:
            print(f"Creating human {name} in namespace {namespace}")
            ...
            response.raise_for_status()
            self.human = V1Human.model_validate(response.json())
            print(f"Created human {self.human.metadata.name}")
        else:
            print(f"Found human {self.human.metadata.name}, updating if necessary")
