...
__all__ = ["ProbabilisticParetoSampler"]
...
@dataclass
class ProbabilisticParetoSampler:
    ...
    """
    # —————————————————————————————————————————————
    # Public API fields (all optional, with defaults)
    # —————————————————————————————————————————————
    weights: Optional[np.ndarray]   = None
    repulsion_weight: float         = 1.0

    # Now these are normal float fields with defaults → optional in __init__
    ...
    #@staticmethod
    def _probs_sanity_check(self, probs:float):
        probs = np.nan_to_num(probs, nan=0.0)
        if np.isclose(probs.sum(), 0.0):
            N = probs.size
            probs = np.full_like(probs, fill_value=1.0 / N, dtype=float)
        probs /= probs.sum()
        return probs
    ...
    def select(
        ...
        # 12) Cooling step
        self.counts[self.counts>0] += -self.cooling_rate
    ...
    def plot_diagnostics(
        ...
        for i, T in enumerate(samp_temps):
            ...
            idx = sampler.select(objectives, features, size=100)
            ax = axes[1, i]
