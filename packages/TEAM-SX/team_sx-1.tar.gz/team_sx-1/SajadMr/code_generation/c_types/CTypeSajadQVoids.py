...
class CTypeSajadQVoidEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "SajadMr_void"
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s == SajadSx_VOID_OK);" % value_name)

    @classmethod
    def emitReInitCode(cls, value_name, emit):
        emit("%s = SajadSx_VOID_OK;" % value_name)
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        ...
        if Options.is_debug:
            emit("%s = SajadSx_VOID_OK;" % to_name)
    ...
    def getExceptionCheckCondition(cls, value_name):
        return "%s == SajadSx_VOID_EXCEPTION" % value_name
