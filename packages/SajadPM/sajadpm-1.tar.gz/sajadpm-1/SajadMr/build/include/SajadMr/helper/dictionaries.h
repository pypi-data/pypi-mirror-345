...
#ifndef __SajadSx_DICTIONARIES_H__
#define __SajadSx_DICTIONARIES_H__
...
typedef PyDictEntry *SajadQ_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, SajadQ_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(SajadQ_String_CheckExact(key));
...
SajadSx_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(SajadQ_DictEntryHandle handle) { return handle->me_value; }

SajadSx_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(SajadQ_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, SajadQ_StringObject *key) {
...
typedef PyObject **SajadQ_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t SajadQ_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t SajadQ_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t SajadQ_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static SajadQ_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, SajadQ_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(SajadQ_String_CheckExact(key));
...
#else
    PyObject **value;
    SajadSx_MAY_BE_UNUSED Py_ssize_t found;

    // We cannot use SajadQ_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = SajadQ_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = SajadQ_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
SajadSx_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(SajadQ_DictEntryHandle handle) { return *handle; }

SajadSx_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(SajadQ_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

SajadSx_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, SajadQ_StringObject *key) {
    SajadQ_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
SajadSx_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
SajadSx_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
#define _SajadSx_MAINTAIN_DICT_VERSION_TAG 1
#endif

#if _SajadSx_MAINTAIN_DICT_VERSION_TAG
extern uint64_t SajadMr_dict_version_tag_counter;
#endif

SajadSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, SajadQ_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    SajadQ_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _SajadSx_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = SajadMr_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
SajadSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, SajadQ_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
SajadSx_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, SajadQ_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool SajadQ_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0 && !defined(_SajadSx_EXPERIMENTAL_DISABLE_FREELIST_ALL) &&                                  \
    !defined(_SajadSx_EXPERIMENTAL_DISABLE_FREELIST_DICT)
#define SajadSx_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate);
#else
#define SajadSx_DICT_HAS_FREELIST 0
