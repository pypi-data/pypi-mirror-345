...
#ifndef __SajadSx_PRELUDE_H__
#define __SajadSx_PRELUDE_H__
...
#include "SajadMr/debug_settings.h"
 ...
 */
#ifdef __GNUC__
#define SajadSx_MAY_BE_UNUSED __attribute__((__unused__))
#else
#define SajadSx_MAY_BE_UNUSED
#endif
...
#define SajadSx_USE_PYCORE_THREAD_STATE
#endif

#ifdef SajadSx_USE_PYCORE_THREAD_STATE
#undef Py_BUILD_CORE
...
static inline size_t SajadQ_static_builtin_index_get(PyTypeObject *self) { return (size_t)self->tp_subclasses - 1; }

// Changed internal type access for Python3.13
#if PYTHON_VERSION < 0x3d0
#define managed_static_type_state static_builtin_state

static inline managed_static_type_state *SajadQ_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins[SajadQ_static_builtin_index_get(self)]);
}
#else
static inline managed_static_type_state *SajadQ_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins.initialized[SajadQ_static_builtin_index_get(self)]);
}
#endif

SajadSx_MAY_BE_UNUSED static inline managed_static_type_state *SajadQ_PyStaticType_GetState(PyInterpreterState *interp,
                                                                                           PyTypeObject *self) {
    assert(self->tp_flags & _Py_TPFLAGS_STATIC_BUILTIN);
    return SajadQ_static_builtin_state_get(interp, self);
}

#define _PyStaticType_GetState(interp, self) SajadQ_PyStaticType_GetState(interp, self)
#endif
...
#define SajadSx_NO_RETURN HEDLEY_NO_RETURN

/* This is used to indicate code control flows we know cannot happen. */
#ifndef __SajadSx_NO_ASSERT__
#define SajadSx_CANNOT_GET_HERE(NAME)                                                                                   \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
#else
#define SajadSx_CANNOT_GET_HERE(NAME) abort();
#endif

#define SajadSx_ERROR_EXIT(NAME)                                                                                        \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
 ...
 */
#include <malloc.h>
#define SajadSx_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT)                                                  \
    ELEMENT_TYPE *VARIABLE_NAME = (ELEMENT_TYPE *)_alloca(sizeof(ELEMENT_TYPE) * (COUNT));
#else
#define SajadSx_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT) ELEMENT_TYPE VARIABLE_NAME[COUNT];
#endif
...
#define SajadQ_String_AsString PyString_AsString
#define SajadQ_String_AsString_Unchecked PyString_AS_STRING
#define SajadQ_String_Check PyString_Check
#define SajadQ_String_CheckExact PyString_CheckExact
SajadSx_MAY_BE_UNUSED static inline bool SajadQ_StringOrUnicode_CheckExact(PyObject *value) {
    return PyString_CheckExact(value) || PyUnicode_CheckExact(value);
}
#define SajadQ_StringObject PyStringObject
#define SajadQ_String_FromString PyString_FromString
#define SajadQ_String_FromStringAndSize PyString_FromStringAndSize
#define SajadQ_String_FromFormat PyString_FromFormat
#define PyUnicode_CHECK_INTERNED (0)
SajadSx_MAY_BE_UNUSED static Py_UNICODE *SajadQ_UnicodeAsWideString(PyObject *str, Py_ssize_t *size) {
    PyObject *unicode;
...
#else
#define SajadQ_String_AsString _PyUnicode_AsString
...
#ifdef __SajadSx_NO_ASSERT__
#define SajadQ_String_AsString_Unchecked PyUnicode_UTF8
#else
SajadSx_MAY_BE_UNUSED static char const *SajadQ_String_AsString_Unchecked(PyObject *object) {
    char const *result = PyUnicode_UTF8(object);
    assert(result != NULL);
    return result;
}
#endif
#define SajadQ_String_Check PyUnicode_Check
#define SajadQ_String_CheckExact PyUnicode_CheckExact
#define SajadQ_StringOrUnicode_CheckExact PyUnicode_CheckExact
#define SajadQ_StringObject PyUnicodeObject
#define SajadQ_String_FromString PyUnicode_FromString
#define SajadQ_String_FromStringAndSize PyUnicode_FromStringAndSize
#define SajadQ_String_FromFormat PyUnicode_FromFormat
#define SajadQ_UnicodeAsWideString PyUnicode_AsWideCharString
#endif
...
#ifdef __SajadSx_NO_ASSERT__
#define SajadQ_TypeLookup(x, y) _PyType_Lookup(x, y)
#else
SajadSx_MAY_BE_UNUSED static PyObject *SajadQ_TypeLookup(PyTypeObject *type, PyObject *name) {
...
#if PYTHON_VERSION < 0x300
#define SajadSx_MODULE_ENTRY_FUNCTION void
#else
#define SajadSx_MODULE_ENTRY_FUNCTION PyObject *
#endif
 ...
 */
#define SajadSx_CROSS_MODULE
#define SajadSx_LOCAL_MODULE static
...
#define SajadQ_GC_Track PyObject_GC_Track
#define SajadQ_GC_UnTrack PyObject_GC_UnTrack
...
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
#elif defined(_SajadSx_MODULE) && PYTHON_VERSION >= 0x370 && PYTHON_VERSION < 0x380
#define SajadQ_GC_Track PyObject_GC_Track
#define SajadQ_GC_UnTrack PyObject_GC_UnTrack
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
...
#else
#define SajadQ_GC_Track _PyObject_GC_TRACK
#define SajadQ_GC_UnTrack _PyObject_GC_UNTRACK
#endif

#if _SajadSx_EXPERIMENTAL_FAST_THREAD_GET && PYTHON_VERSION >= 0x300 && PYTHON_VERSION < 0x370
...
#if PYTHON_VERSION < 0x300
#define SajadQType_HasFeatureClass(descr) (PyType_HasFeature(Py_TYPE(descr), Py_TPFLAGS_HAVE_CLASS))
#else
#define SajadQType_HasFeatureClass(descr) (1)
#endif

// Our replacement for "PyType_IsSubtype"
extern bool SajadQ_Type_IsSubtype(PyTypeObject *a, PyTypeObject *b);

#include "SajadMr/allocator.h"
#include "SajadMr/exceptions.h"
...
typedef long SajadMr_digit;

#include "SajadMr/helpers.h"

#include "SajadMr/compiled_frame.h"

#include "SajadMr/compiled_cell.h"

#include "SajadMr/compiled_function.h"

/* Sentinel PyObject to be used for all our call iterator endings. */
extern PyObject *SajadQ_sentinel_value;

/* Value to use for __compiled__ value of all modules. */
extern PyObject *SajadQ_dunder_compiled_value;

#include "SajadMr/compiled_generator.h"

#include "SajadMr/compiled_method.h"

#if PYTHON_VERSION >= 0x350
#include "SajadMr/compiled_coroutine.h"
#endif

#if PYTHON_VERSION >= 0x360
#include "SajadMr/compiled_asyncgen.h"
#endif

#include "SajadMr/filesystem_paths.h"
#include "SajadMr/safe_string_ops.h"

#include "SajadMr/jit_sources.h"

#if _SajadSx_EXPERIMENTAL_WRITEABLE_CONSTANTS
#include "SajadMr_data_decoder.h"
#else
#define DECODE(x) assert(x)
#define UN_TRANSLATE(x) (x)
#endif

#if _SajadSx_EXPERIMENTAL_FILE_TRACING
#include "SajadMr_file_tracer.h"
#else
#if PYTHON_VERSION < 0x300
...
#if _SajadSx_EXPERIMENTAL_INIT_PROGRAM
#include "SajadMr_init_program.h"
#else
#define SajadSx_INIT_PROGRAM_EARLY(argc, argv)
#define SajadSx_INIT_PROGRAM_LATE(module_name)
#endif

#if _SajadSx_EXPERIMENTAL_EXIT_PROGRAM
#include "SajadMr_exit_program.h"
#else
#define SajadSx_FINALIZE_PROGRAM(tstate)
#endif
