...
class EntrypointClient(IClient):
    ...
    def get_agent_list(self):
        try:
            ...
            response = self.post_request(ep_url,json=data)
            if response.status_code == 200:
                logger.debug(f"get_all_public_data ok:{response.json()}")
                return response.json()["data"]
            else:
                logger.error(f"get_all_public_data failed:{response.json()}")
                return None
        except Exception as e:
            logger.exception(f"Get all public data exception occurred: {e}")

    def get_entrypoint_config(self):
        try:
            ep_url = self.server_url + "/get_entrypoint_config"
            logger.debug(f"Get server config: {ep_url}")
            data = {
                "agent_id": f"{self.agent_id}",
                "signature": self.auth_client.signature,
            }
            ...
            if response.status_code == 200:
                try:
                    ...
                    if 'config' in config:
                        if 'heartbeat_server' in config['config']:
                            ...
                            logger.debug(f"Set heartbeat server to: {self.heartbeat_server}")
                        if 'message_server' in config['config']:
                            self.message_server = config['config']['message_server']
                            logger.debug(f"Set message server to: {self.message_server}")
                ...
                logger.error(f"Get entrypoint config {ep_url} failed:{response.json()}")
    ...
    def get_agent_public_data(self):
        try:
            ...
            response = self.post_request(ep_url,json=data)
            if response.status_code == 200:
                logger.debug(f"get_agent_public_data ok:{response.json()}")
                return response.json()["data"]  # Return the data if it exists, else return None or handle it as you like
            else:
                logger.error(f"get_agent_public_data failed:{response.json()}")
                return None
        except Exception as e:
            logger.exception(f"Get agent public data exception occurred: {e}")
            return None
    ...
    def get_agent_private_data(self):
        try:
            ...
            response = self.post_request(ep_url,json=data)
            if response.status_code == 200:
                logger.debug(f"Get_agent_private_data ok:{response.json()}")
                return response.json()["data"]
            else:
                logger.error(f"Get_agent_private_data failed:{response.json()}")
                return None
        except Exception as e:
            logger.exception(f"Get agent private data exception occurred: {e}")
            return None
