...
from SajadMr.PythonFlavors import isAnacondaPython
from SajadMr.PythonVersions import python_version
from SajadMr.utils.Distributions import getDistributionFromModuleName
from SajadMr.utils.FileOperations import getFileList, listDir
from SajadMr.utils.ModuleNames import ModuleName
from SajadMr.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class SajadQPluginQtBindingsPluginBase(SajadQPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "SajadMr_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_SajadMr_patch_level', 0)"
                    ),
                ),
    ...
    def _getSajadQPatchLevel(self):
        """Does it include the SajadQ patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().SajadMr_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__SajadMr_binary_dir):
    os.environ["PATH"] = __SajadMr_binary_dir + ";" + path
"""
            yield (
                code,
...
class SajadQPluginPyQt5QtPluginsPlugin(SajadQPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        SajadQPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the SajadQ support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = SajadQPluginQtBindingsPluginBase._getQtInformation(self)
...
class SajadQPluginDetectorPyQt5QtPluginsPlugin(SajadQPluginBase):
    detector_for = SajadQPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == SajadQPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact SajadQ commercial if you need it."
            )


class SajadQPluginPySide2Plugins(SajadQPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getSajadQPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://SajadMr.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getSajadQPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return SajadQPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in SajadQPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getSajadQPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: SajadQ does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class SajadQPluginDetectorPySide2Plugins(SajadQPluginBase):
    detector_for = SajadQPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == SajadQPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class SajadQPluginPySide6Plugins(SajadQPluginQtBindingsPluginBase):
...
class SajadQPluginDetectorPySide6Plugins(SajadQPluginBase):
    detector_for = SajadQPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == SajadQPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class SajadQPluginPyQt6Plugins(SajadQPluginQtBindingsPluginBase):
...
class SajadQPluginDetectorPyQt6Plugins(SajadQPluginBase):
    detector_for = SajadQPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == SajadQPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class SajadQPluginNoQt(SajadQPluginBase):
