...
class RedisCatalog(IAsyncCatalog[RedisCatalogConfig, RedisIntegrationConfig, RedisTableIdentifier]):
    ...
    async def resolve_table_path(self, *, table_path: typing.Sequence[str]) -> typing.AsyncGenerator[RedisTableIdentifier, None]:
        if len(table_path) != 3:
            raise SyntaxError(f"Invalid table path: expected <cluster>.db<db>.<table>")  # TODO: this will be confusing because there are four parts actually

        requested_cluster_name, requested_db_id, requested_table_name = table_path

        if requested_cluster_name == "?":
            cluster_names = {integration.cluster_name for integration in self._integrations}
        else:
            if requested_cluster_name not in {integration.cluster_name for integration in self._integrations}:
                raise SyntaxError(f"Unknown cluster name: {requested_cluster_name}")

            cluster_names = {requested_cluster_name}

        try:
            table_name = RedisTableNames(requested_table_name)
        ...
        for cluster_name in cluster_names:
            ...
            if requested_db_id == "?":
                ...
                try:
                    db_id = self._parse_db_id(db_id=requested_db_id)
                except ValueError as e:
    ...
    async def fetch_all_tables(self, *, integration_config: RedisIntegrationConfig) -> typing.AsyncGenerator[TableInfo, None]:
        ...
        for db_id in range(num_databases):
            yield TableInfo(
                path=(integration_config.cluster_name, f"db{db_id}", RedisTableNames.KEYS),
