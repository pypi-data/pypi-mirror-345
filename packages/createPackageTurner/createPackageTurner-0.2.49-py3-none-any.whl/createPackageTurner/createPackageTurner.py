...
def createPackageTurner( inputClassOrFunction , packageName = "na" , packageHousePath = "na", specificPackageFolder="na", pythonAbove="3.6" , author_email='example@gmail.com', author='Joan of Arc',description="auto generated package from createPackageTurner" , readme="this package was generated by createPackageTurner using a class as input that contains functions", publishToPypi = False, pypiApiKey = "unknown", requirementsReplacement={}, overwriteRequirements="na",  relyOnOldSetupWhenPossible = True ,  linkRepo="na", linkDocumentation="na", linkSourceCode="na", linkBugTracker="na"):
    '''
    ...
    if False:
        ...
        description="auto generated package from createPackageTurner"
        readme="this package was generated by createPackageTurner using a class as input that contains functions"
        
        inputClassOrFunction = sayHello # importRelative
    ...
    def getPackageName_and_checkNameUnused():
        def getClassName_PackageName():
            className = inputClassOrFunction.__name__
    ...
    def getInfoAboutPackage():
        ...
        def getPackages( inputClassOrFunction ):
            def get_class_imports( inputClassOrFunction ): # cls = r#elativeImport
                ...
                class_file_path = inspect.getfile( inputClassOrFunction )
                ...
                return allReqs, normalImport, fromImport
            allReqs, normalImport, fromImport = get_class_imports( inputClassOrFunction )
        ...
        allReqsFiltered, normalImport, fromImport = getPackages( inputClassOrFunction )
    ...
    def dealWithUpperLayerOfPackage():
        ...
        def saveSetup( namePackage, setupLocation, thisVersion, allReqsFiltered ): # reqs      
            ...
            def getSetupText_fromOldText( setupLocation, linkDocumentation, linkSourceCode, linkBugTracker,   linkRepo):
                ...
                if author_email != 'example@gmail.com':
                    setupText = re.sub(r"author_email\s*=\s*['\"][^'\"]*['\"]", f"author_email='{author_email}'", setupText)

                if author != 'Joan of Arc':
                    setupText = re.sub(r"author\s*=\s*['\"][^'\"]*['\"]", f"author='{author}'", setupText)

                if description != "auto generated package from createPackageTurner":
                    setupText = re.sub(r"description\s*=\s*['\"][^'\"]*['\"]", f"description='{description}'", setupText)

                if readme != "this package was generated by createPackageTurner using a class as input that contains functions":
                    # Replace the `long_description` string directly if it's hardcoded (not from file), otherwise update README.md
                    setupText = re.sub(
                        r"with open\([^)]+\)\s+as fh:\s+long_description\s*=\s*fh\.read\(\)", 
    ...
    def withLowerLayerOfPackage():
        def createInit( inputClassOrFunction, innerFolder ):
            initPath = innerFolder + "/__init__.py"
            
            if inspect.ismodule( inputClassOrFunction ):
                initText = f'''
        ...
        createInit( inputClassOrFunction, innerFolder )
        
        def saveClass( inputClassOrFunction, namePackage, normalImport, fromImport ):
            class_code = inspect.getsource( inputClassOrFunction )
        ...
        saveClass( inputClassOrFunction, namePackage, normalImport, fromImport )
