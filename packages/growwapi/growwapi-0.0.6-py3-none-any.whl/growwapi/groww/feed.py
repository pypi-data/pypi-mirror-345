...
class GrowwFeed:
    ...
    def _unsubscribe_topics(self, topics: list[str]) -> dict[str, bool]:
        """
        Unsubscribe from a given topic.
        ...
        resp = {}
        for topic in topics:
            resp[topic] = self._unsubscribe(topic)
        return resp
    ...
    def unsubscribe_ltp(self, instrument_list: list[dict[str, any]]) -> dict[str, bool]:
        ...
        self._assert_valid_instruments(instrument_list)
        topics = self._get_topics(instrument_list, FeedConstants.get_live_price_topic)
        return self._unsubscribe_topics([topic.get_topic() for topic in topics])
