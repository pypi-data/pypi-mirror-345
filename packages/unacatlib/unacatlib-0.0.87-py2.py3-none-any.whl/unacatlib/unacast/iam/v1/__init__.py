...
@dataclass(eq=False, repr=False)
class ResolveUserTokenRequest(betterproto.Message):
    id_token: str = betterproto.string_field(1)
    cache_only: bool = betterproto.bool_field(2)
...
class IamServiceStub(betterproto.ServiceStub):
    async def resolve_user_token(
        self,
        resolve_user_token_request: "ResolveUserTokenRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
        ...
        return await self._unary_unary(
            "/unacast.iam.v1.IamService/ResolveUserToken",
            resolve_user_token_request,
            IamUser,
            timeout=timeout,
            deadline=deadline,
            metadata=metadata,
...
class IamServiceBase(ServiceBase):
    ...
    async def resolve_user_token(
        self, resolve_user_token_request: "ResolveUserTokenRequest"
    ) -> "IamUser":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_resolve_user_token(
        self, stream: "grpclib.server.Stream[ResolveUserTokenRequest, IamUser]"
    ) -> None:
        request = await stream.recv_message()
        response = await self.resolve_user_token(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            "/unacast.iam.v1.IamService/ResolveUserToken": grpclib.const.Handler(
                self.__rpc_resolve_user_token,
                grpclib.const.Cardinality.UNARY_UNARY,
