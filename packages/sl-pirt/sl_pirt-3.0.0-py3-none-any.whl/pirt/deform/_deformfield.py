...
from .. import interp
from .._utils import Aarray
from ..splinegrid import FD, GridInterface, calculate_multiscale_sampling
...
class DeformationField(Deformation):
    ...
    def __repr__(self):
        ...
        if self.is_identity:
            return "<DeformationField (%s) with %iD identity deformation>" % (mapping, self.ndim)
        else:
            shapestr = "x".join([("%i" % s) for s in self.field_shape])
            samplingstr = "x".join([("%1.2f" % s) for s in self.field_sampling])
            return "<DeformationField (%s) with shape %s and sampling %s>" % (mapping, shapestr, samplingstr)
    ...
    @classmethod
    def from_field_multiscale(cls, field, sampling, weights=None, injective=True, frozenedge=True, fd=None):
    ...
    @classmethod
    def from_points_multiscale(cls, image, sampling, pp1, pp2, injective=True, frozenedge=True):
