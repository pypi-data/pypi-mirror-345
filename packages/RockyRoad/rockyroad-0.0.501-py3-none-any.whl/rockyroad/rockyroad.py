...
def check_for_subscription_key():
    try:
        ...
        os.environ["OCP_APIM_SUBSCRIPTION_KEY_SERVICES"]
        ...
        export OCP_APIM_SUBSCRIPTION_KEY_SERVICES="INSERT_YOUR_SUBSCRIPTION_KEY_SERVICES"
...
class DataServicesResource(object):
    ...
    def __init__(self, *args, **kw):
        ...
        ocp_apim_subscription_key_services = kw.get("ocp_apim_subscription_key_services")
        if ocp_apim_subscription_key:
            os.environ["OCP_APIM_SUBSCRIPTION_KEY"] = ocp_apim_subscription_key
        if ocp_apim_subscription_key_services:
            os.environ["OCP_APIM_SUBSCRIPTION_KEY_SERVICES"] = ocp_apim_subscription_key_services
        check_for_subscription_key()
        
        # For localhost testing, use the same URL for both base and services
        url_parts = base_url.split('://')
        if len(url_parts) > 1:
            protocol, rest = url_parts
            # Check if this is a localhost URL
            if rest.startswith('localhost') or rest.startswith('127.0.0.1'):
                services_base = base_url
            else:
                # Create services URL by inserting '-services' before the last dot in the hostname
                hostname_parts = rest.split('.')
                hostname_parts[-2] = hostname_parts[-2] + '-services'
                services_base = protocol + '://' + '.'.join(hostname_parts)
        else:
            # No protocol specified, check if it's localhost
            if base_url.startswith('localhost') or base_url.startswith('127.0.0.1'):
                services_base = base_url
            else:
                services_base = base_url + '-services'
        
        if test:
            api_base_url = base_url + "/"
            services_api_base_url = services_base + "/"
        else:
            api_base_url = base_url + "/" + serviceName + "/" + version + "/"
            services_api_base_url = services_base + "/" + serviceName + "/" + version + "/"
...
def build(serviceName, version, base_url, ocp_apim_subscription_key=None, ocp_apim_subscription_key_services=None, **kw) -> DataServicesResource:
    ...
    try:
        ...
        return service(
            ...
            ocp_apim_subscription_key_services=ocp_apim_subscription_key_services,
