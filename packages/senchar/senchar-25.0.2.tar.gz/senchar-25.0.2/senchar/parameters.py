"""
Parameter handling tool for senchar.
...
class Parameters(object):
    ...
    def __init__(self, default_dictname: str = "senchar"):
        """
        Creates parameters tool, optionally setting default parameter dictionary name.
        """

        self.par_file = "senchar.ini"
        self.par_dict = {}

        self.default_pardict_name = default_dictname

    def read_parfile(self, parfilename: str | None = None) -> None:
        """
        ...
        if parfilename is None:
            ...
            if parfilename is None:
                senchar.exceptions.warning("Parameter file is not defined")
                return

        self.par_file = parfilename

        if not os.path.exists(parfilename):
            senchar.exceptions.warning(f"Parameter file not found: {parfilename}")
            return
    ...
    def update_pars(self) -> None:
        ...
        par_dict = self.par_dict.get(par_dictname)
        if par_dict is None:
            return
        keys = par_dict.keys()
        if keys is None:
    ...
    def get_par(self, parameter: str, subdict=None) -> typing.Any:
        ...
        try:
            attribute = db.par_table[parameter]
            ...
            try:
                value = self.par_dict[subdict][parameter]
            except KeyError:
                senchar.exceptions.warning(
    ...
    def set_par(self, parameter: str, value: typing.Any = "None", subdict=None) -> None:
        ...
        parameter = senchar.utils.dequote(parameter)
        parameter = parameter.lower()
        ...
        try:
            ...
            if numtokens < 2:
                senchar.exceptions.warning(
                    "%s not valid for parameter %s" % (attribute, parameter)
                )
                return None

        except KeyError:
            _, value = senchar.utils.get_datatype(value)

            try:
                self.par_dict[subdict][parameter] = value
            except KeyError:
                if self.par_dict.get(subdict) is None:
                    self.par_dict[subdict] = {}
                self.par_dict[subdict][parameter] = value
            return None
