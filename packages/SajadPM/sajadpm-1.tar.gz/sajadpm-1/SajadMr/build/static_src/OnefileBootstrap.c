...
#else
#define _SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
#define _SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
#define _SajadSx_ONEFILE_TEMP_BOOL 0
#define _SajadSx_ONEFILE_CHILD_GRACE_TIME_INT 5000
#define _SajadSx_ONEFILE_TEMP_SPEC "{TEMP}/onefile_{PID}_{TIME}"

#define _SajadSx_AUTO_UPDATE_BOOL 1
#define _SajadSx_AUTO_UPDATE_DEBUG_BOOL 1
#define _SajadSx_AUTO_UPDATE_URL_SPEC "https://..."

#define _SajadSx_ATTACH_CONSOLE_WINDOW 1
#endif

#if _SajadSx_ONEFILE_COMPRESSION_BOOL == 1
...
// Some handy macro definitions, e.g. unlikely and SajadSx_MAY_BE_UNUSED
#include "SajadMr/hedley.h"
...
// Out of memory error.
#if !defined(_WIN32) || _SajadSx_ONEFILE_COMPRESSION_BOOL == 1
...
#if _SajadSx_ONEFILE_COMPRESSION_BOOL == 1 && _SajadSx_ONEFILE_ARCHIVE_BOOL == 0
...
#if _SajadSx_ONEFILE_TEMP_BOOL == 0
...
#if _SajadSx_ONEFILE_ARCHIVE_BOOL == 1 && _SajadSx_ONEFILE_COMPRESSION_BOOL == 1
...
#if _SajadSx_ONEFILE_ARCHIVE_BOOL == 1

#if _SajadSx_ONEFILE_COMPRESSION_BOOL == 0
...
#if _SajadSx_ONEFILE_TEMP_BOOL == 1
...
static int waitpid_timeout(pid_t pid) {
    ...
    struct timespec timeout = {
        _SajadSx_ONEFILE_CHILD_GRACE_TIME_INT / 1000,
        (_SajadSx_ONEFILE_CHILD_GRACE_TIME_INT % 1000) * 1000,
    };
...
static void cleanupChildProcess(bool send_sigint) {
    ...
    if (handle_process != 0) {
        ...
        if (send_sigint) {
#if defined(_SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING)
...
#if _SajadSx_ONEFILE_TEMP_BOOL == 1 || 1
        SajadSx_PRINT_TRACE("Waiting for child to exit.\n");
#if defined(_WIN32)
        if (WaitForSingleObject(handle_process, _SajadSx_ONEFILE_CHILD_GRACE_TIME_INT) != 0) {
...
#endif
        SajadSx_PRINT_TRACE("Child is exited.\n");
#endif
    }

#if _SajadSx_ONEFILE_TEMP_BOOL == 1
    if (payload_created) {
#if _SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
...
BOOL WINAPI ourConsoleCtrlHandler(DWORD fdwCtrlType) {
    ...
    case CTRL_C_EVENT:
#ifdef _SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
        puts("Ctrl-C event");
#endif
...
#if _SajadSx_AUTO_UPDATE_BOOL && !defined(__IDE_ONLY__)
#include "SajadMr_onefile_auto_updater.h"
#endif

#if _SajadSx_AUTO_UPDATE_BOOL
extern bool exe_file_updatable;
#endif

#ifdef _SajadSx_ONEFILE_SPLASH_SCREEN
#ifdef __cplusplus
extern "C" {
#endif
...
int main(int argc, char **argv) {
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Entered main().");

    filename_char_t const *pattern = FILENAME_EMPTY_STR _SajadSx_ONEFILE_TEMP_SPEC;
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Unpacking payload.");
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Determining payload start position.");
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Checking header for compression.");
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Header is OK.");
...
// The 'X' stands for no compression, 'Y' is compressed, handle that.
...
#if _SajadSx_ONEFILE_SPLASH_SCREEN
    initSplashScreen();
#endif

    SajadSx_PRINT_TIMING("ONEFILE: Entering decompression.");
...
#ifdef _SajadSx_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Finishing decompression, cleanup payload.");
...
#if defined(_WIN32)
    setEnvironmentVariableFromLong("SajadSx_ONEFILE_PARENT", GetCurrentProcessId());
#else
    setEnvironmentVariableFromLong("SajadSx_ONEFILE_PARENT", (long)getpid());
#endif
...
#endif
    setEnvironmentVariable("SajadSx_ONEFILE_DIRECTORY", stripBaseFilename(binary_filename));

    setEnvironmentVariable("SajadSx_ORIGINAL_ARGV0", argv[0]);

    SajadSx_PRINT_TIMING("ONEFILE: Preparing forking of slave process.");
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Started slave process.");
    ...
    SajadSx_PRINT_TIMING("ONEFILE: Exiting.");
