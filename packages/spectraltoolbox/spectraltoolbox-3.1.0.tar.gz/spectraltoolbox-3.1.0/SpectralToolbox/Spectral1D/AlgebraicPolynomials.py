           ...
           and not self.mask[n,m]:
            ...
            if n == 0 or m == 0:
                ...
                for nn in range(ntot+1):
                    if nn % 2 == 0:
                        pi_erf_c += c[nn] * factorial2(max(0, nn-1),True)
                        for k in range(1,nn//2+1):
                            exp_c[nn-2*k+1] += c[nn] * \
                                               (factorial2(max(0,nn-1),True) \
                                                // factorial2(max(0,nn-2*k+1),True))
                    else:
                        const += c[nn] * factorial2(max(0, nn-1),True)
                ...
                for nn in range(ch2.shape[0]):
                    if nn % 2 == 0 and ch2[nn] != 0.:
                        pi_erf_c += ch2[nn] * factorial2(max(0, nn-1),True) / 2**(nn/2+1)
                        for k in range(1,nn//2+1):
                            exp_c[nn-2*k+1] += ch2[nn] * \
                                               (factorial2(max(0,nn-1),True) * 2**(nn//2-k) \
                                                / factorial2(max(0,nn-2*k+1),True)) \
                                               / 2.**(nn//2)
