...
class OptFreqSinglePoint:
    ...
    def from_toml(cls, file: str | Path) -> "OptFreqSinglePoint":
        ...
        pipeline = cls(
            directory=dir,
            cpus=config["slurm"].get("cpus", 1),
            mem_per_cpu_gb=config["slurm"].get("mem_per_cpu_gb", 1),
...
if __name__ == "__main__":
    ...
    if Path(toml_file).suffix == ".toml":
        pipe = OptFreqSinglePoint.from_toml(toml_file)
        pipe.run()
    else:
        raise ValueError(f"Unrecognized file type '{toml_file}'")
