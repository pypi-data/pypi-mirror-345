        ...
        while not found_valid_node_in_chunk and retry_count < max_retries:
            try:
                ...
                if not isinstance(content, str):
                    ...
                    human_message = HumanMessage(content="LLM response content is not a string. Retry")
                    messages.append(ai_message)
                    messages.append(human_message)
                    messages.append(HumanMessage(content=user_prompt.format(text=chunk, relationships=relations)))
                    retry_count += 1
                ...
                if not nodes_raw:
                    ...
                    human_message = HumanMessage(content="No <node> tags found in response. Retry")
                    messages.append(ai_message)
                    messages.append(human_message)
                    messages.append(HumanMessage(content=user_prompt.format(text=chunk, relationships=relations)))
                ...
                if processed_at_least_one_node:
                    ...
                    ai_message = AIMessage(content=content)
                    human_message = HumanMessage(content="No valid structure in <node> tags. Retry")
                    messages.append(ai_message)
                    messages.append(human_message)
                    messages.append(HumanMessage(content=user_prompt.format(text=chunk, relationships=relations)))
                ...
                ai_message = AIMessage(content=str(e))
                human_message = HumanMessage(content="Error during LLM invocation or processing. Retry")
                messages.append(ai_message)
                messages.append(human_message)
                messages.append(HumanMessage(content=user_prompt.format(text=chunk, relationships=relations)))
