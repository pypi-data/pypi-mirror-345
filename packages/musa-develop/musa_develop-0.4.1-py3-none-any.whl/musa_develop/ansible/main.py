...
from musa_develop.auto_gen import generate_docker_service, generate_kuae_chat_json
...
class AnsibleExecutor(BaseShellExecutor):
    ...
    def __call__(
        ...
        musa_develop_vars = (
            f"musa_develop_arguments='{musa_develop_args}'" if musa_develop_args else ""
        )
        kuae_chat_vars = (
        ...
        os.system(
            f'ansible-playbook -i {inventory_file} {playbook} -v --extra-vars "{max_model_len_vars} {tp_size_vars} {pp_size_vars} {pp_partition_vars} {host_ip_vars} {worker_num_vars} {task_name_vars} {model_path_vars} {kuae_chat_vars} {musa_develop_vars}"'
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="demo",
        type=str,
        help="""please refer to musa-develop -h""",
    )
    parser.add_argument(
        "--cluster-demo",
        dest="cluster_demo",
...
def main():
    ...
    if args.demo:
        ...
        # 2. ansible-playbook
        ansible(inventory_path, playbook_file, disable_kuae_chat=True)
        return 0
    ...
    if args.cluster_demo and args.task:
        ...
        while i < len(command_args):
            if command_args[i] == "--hostfile":
                ...
                i += 1
        musa_develop_args = " ".join(filtered)
