...
def train_unsloth_sft(message: Message[TrainingRequest]) -> TrainingResponse:
    ...
    # Try to free any meta tensors that might still be in memory
    # by releasing any cached modules or compiled functions
    # if hasattr(torch, "_dynamo"):
    #     if hasattr(torch._dynamo, "reset"):
    #         torch._dynamo.reset()

    # # Try to unload all Unsloth modules to start fresh
    # try:
    #     # Clear any cached compiled modules
    #     if hasattr(torch, "jit") and hasattr(torch.jit, "_clear_compile_cache"):
    #         torch.jit._clear_compile_cache()
    #     print("Reset PyTorch state successfully")
    # except Exception as e:
    #     print(f"Note: Could not fully reset PyTorch state: {e}")

    # Monkey patch the problematic function in unsloth_zoo
    # try:
    #     # Import the module containing the problematic function
    #     from unsloth_zoo import tokenizer_utils

    #     # Store the original function
    #     original_fix = tokenizer_utils.fix_untrained_tokens

    #     # Define safer replacement function that skips meta tensor handling
    #     def safe_fix(
    #         model, tokenizer, train_dataset, IGNORED_TOKENIZER_NAMES=[], eps=1e-16
    #     ):
    #         try:
    #             return original_fix(
    #                 model, tokenizer, train_dataset, IGNORED_TOKENIZER_NAMES, eps
    #             )
    #         except NotImplementedError as e:
    #             if "Cannot copy out of meta tensor" in str(e):
    #                 print("Skipping token correction due to meta tensor issue")
    #                 return None
    #             raise

    #     # Replace the problematic function
    #     tokenizer_utils.fix_untrained_tokens = safe_fix
    #     print("Successfully patched fix_untrained_tokens function")
    # except Exception as e:
    #     print(f"Note: Could not patch fix_untrained_tokens: {e}")
    ...
    try:
        ...
        # Monkey patch should handle init errors, remove try/except here
        # try:
        trainer = SFTTrainer(
            model=model,
        ...
        if latest_checkpoint:
            ...
            # Try to load the entire trainer_state.json
            ...
            if os.path.exists(trainer_state_path):
                ...
                print(
                    f"Warning: {trainer_state_path} not found. Proceeding without metrics from state file."
                )
        else:
            print(
                "Warning: No checkpoint found. Proceeding without metrics from state file."
            )
        ...
        # del trainer, model, tokenizer, converted_dataset
        # if hasattr(torch, "_inductor") and hasattr(torch._inductor, "caches"):
        #     torch._inductor.caches.clear()
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.ipc_collect()
        ...
        #     if m.startswith(("unsloth", "unsloth_zoo", "trl")):
