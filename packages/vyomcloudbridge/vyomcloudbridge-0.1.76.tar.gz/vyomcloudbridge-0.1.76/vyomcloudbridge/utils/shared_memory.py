...
class SharedMemoryUtil:
    ...
    def get_data(self):
        # Read length from the last 4 bytes
        length = int.from_bytes(self.shm.buf[-4:], byteorder="little")
        # Deserialize the data
        return pickle.loads(self.shm.buf[:length])

    def set_data(self, data):
        with self._lock:
            # Serialize the data
            serialized_data = pickle.dumps(data)
            if len(serialized_data) > self.shm.size - 4:
                raise ValueError(
                    f"Data too large for shared memory buffer (max {self.shm.size - 4} bytes)"
                )

            # Write data and its length
            self.shm.buf[: len(serialized_data)] = serialized_data
            length_bytes = len(serialized_data).to_bytes(4, byteorder="little")
            self.shm.buf[self.shm.size - 4 : self.shm.size] = length_bytes
