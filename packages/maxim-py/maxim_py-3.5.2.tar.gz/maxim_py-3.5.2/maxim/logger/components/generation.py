...
@deprecated(
    "Use GenerationConfigDict instead. This will be removed in a future version."
)
@dataclass
...
class Generation(BaseContainer):
    ...
    def add_message_(writer: LogWriter, id: str, message: GenerationRequestMessage):
        if "content" not in message or "role" not in message:
            logging.error(
                "Invalid message. Must have 'content' and 'role' keys. We are skipping adding this message."
            )
            return
        BaseContainer._commit_(
