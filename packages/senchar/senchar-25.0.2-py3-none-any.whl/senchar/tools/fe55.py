...
class Fe55(Tool):
    ...
    def __init__(self):
        ...
        self.plot_order = ["histogram", "hcte", "vcte"]
    ...
    def analyze(self, filename=""):
        ...
        senchar.log("Analyzing fe55 sequence")
        ...
        # get noise roi
        nroi = senchar.utils.get_image_roi()[1]
        self.noise_dn = []
        self.read_noise = []
        ...
        NumExt, first_ext, last_ext = senchar.fits.get_extensions(zerofilename)
        ...
        NumExt, first_ext, last_ext = senchar.fits.get_extensions(filename)

        senchar.log("bias correct image: %s" % os.path.basename(filename))
        zerosub = "zerosub.fits"
        senchar.fits.sub(filename, zerofilename, zerosub)
        os.remove(filename)
        os.rename(zerosub, filename)

        # overscan_correct image second
        if self.overscan_correct:
            senchar.log("overscan_correct image: %s" % os.path.basename(filename))
            senchar.fits.colbias(filename, fit_order=self.fit_order)

        self.grade = "UNKNOWN"
        senchar.log("Analyzing image %s" % os.path.basename(filename))
        ...
        if NumExt > 1:
            ...
            ext = 0
        reply = senchar.fits.get_section(filename, "DATASEC", ext)
        xedges = []
        yedges = []
        ...
        senchar.log("neighborhood_size is %d pixels" % self.neighborhood_size)
        senchar.log("Threshold is %.0f DN" % self.threshold)
        ...
        if self.gain_estimate == []:
            self.gain_estimate = senchar.db.tools["gain"].system_gain
            if self.gain_estimate == []:
                self.gain_estimate = NumExt * [1.0]

        if self.threshold == 0:
            if senchar.db.tools["bias"].is_valid:
                self.threshold = [
                    self.noise_threshold * sd for sd in senchar.db.tools["bias"].sdev
                ]
        ...
        for chan, ext in enumerate(range(first_ext, last_ext)):
            ...
            senchar.log("Analyzing channel %d " % chan)
            ...
            # show events
            if self.show_events:
                senchar.plot.plt.figure()
                senchar.plot.plt.imshow(labeled, cmap="gray")
            ...
            if len(zevents) == 0:
                senchar.log("No events in channel %d" % chan)
                self.event_data.append([0, 0, 0, 0, 0, 0, 0, 0])
            ...
            senchar.log("Removed %d of %d values" % (nevents - len(xevents), nevents))
            nevents = len(xevents)
            ...
            if self.fit_psf:
                ...
                senchar.log("Fitting gaussians for %d events" % nevents)
                with warnings.catch_warnings():  # surpress warning
                    warnings.simplefilter("ignore")
                    for i in range(nevents):
                        # if i%100==0:
                        #    senchar.log('Events remaining: %05d\r' % (nevents-i)),
                        r1 = yevents[i] - middle
            ...
            if self.fit_psf:
                ...
                senchar.log("Shape: %5.03f, %7.03f" % (shape1, shape2))
    ...
    def plot(self):
        ...
        if "events" in self.make_plots:
            fig_events = senchar.plot.plt.figure()
            ...
            for _ in range(nrows):
                for _ in range(ncols):
                    senchar.plot.plt.subplot(nrows, ncols, plotnum)
                    if self.num_chans == 1:
                        s1 = ""
                    else:
                        s1 = "Chan " + str(chan + 1)
                    senchar.plot.plt.title(s1, fontsize=medium_font)
                    ...
                    if 1:
                        ...
                        senchar.plot.plt.xlim(1, nc)
                        senchar.plot.plt.ylim(1, nr)
                        _, labels = senchar.plot.plt.xticks()
                        senchar.plot.plt.setp(labels, rotation=45)

                    if 1:
                        # mark valid events on events plot
                        senchar.plot.plt.autoscale(False)
                        senchar.plot.plt.scatter(
                            self.xevents[chan],
                            self.yevents[chan],
                            s=10,
                            facecolors="none",
            ...
            senchar.plot.save_figure(fignum, f"{self.plot_files['events']}")

        if "histogram" in self.make_plots:
            fig_hist = senchar.plot.plt.figure()
            ...
            for _ in range(nrows):
                for _ in range(ncols):
                    ...
                    senchar.plot.plt.title(s1, fontsize=medium_font)
                    ax = senchar.plot.plt.gca()
                    senchar.plot.plt.plot(self.hist_x[chan], self.hist_y[chan], "b-")
                    ...
                    # senchar.plot.plt.xlim(zmedian/2.,zmedian*2.)
                    # senchar.plot.plt.xlim(self.z[chan].min()-100, self.z[chan].max() + 200)

                    hist_max = self.xray_lines["K-alpha"] / self.system_gain[chan]
                    senchar.plot.plt.axvline(
            ...
            senchar.plot.save_figure(fignum, "%s" % self.plot_files["histogram"])
        ...
        if "cte" in self.make_plots:
            ...
            # HCTE
            fig_cte = senchar.plot.plt.figure()
            ...
            for _ in range(nrows):
                for _ in range(ncols):
                    ...
                    senchar.plot.plt.title("Chan %d" % chan)
                    ...
                    senchar.plot.plt.ylim(
                        self.z[chan].min() - 100, self.z[chan].max() + 200
                    )
                    senchar.plot.plt.xlim(1, last_col)
            ...
            senchar.plot.save_figure(fignum, "%s" % self.plot_files["hcte"])
            ...
            for _ in range(nrows):
                for _ in range(ncols):
                    ...
                    senchar.plot.plt.xlim(1, last_row)
            ...
            senchar.plot.save_figure(fignum, "%s" % self.plot_files["vcte"])
    ...
    def report(self):
        ...
        if self.grade != "UNDEFINED":
            s = f"X-Ray grade = {self.grade}  "
            lines.append(s)

        if self.spec_sigma != -1:
            s = f"Neighborhood_size = {self.neighborhood_size} pixels  "
            lines.append(s)
            s = f"Threshhold = {self.threshold:.0f} DN  "
            lines.append(s)
            s = f"TotalEvents = {sum(self.number_events):.0f}  "
            lines.append(s)

        # means for entire device
        if self.fit_psf:
            s = f"Mean FWHM = {self.mean_fwhmTotal:5.01f} microns  "
            lines.append(s)
            s = f"Mean Sigma = {self.mean_sigmaTotal:5.01f} microns  "
            lines.append(s)
        ...
        if self.system_noise_correction == []:
            ...
            lines.append(s)
            s = f"Mean system_noise_correction = {(numpy.array(self.system_noise_correction).mean()):5.01f} DN  "
        lines.append(s)
