...
class KubernetesOperations:
    ...
    def port_forward(self, pod_name: str, local_port: int, pod_port: int, 
                      namespace: str = "default", address: str = "127.0.0.1") -> Dict[str, Any]:
        """Set up port forwarding to a pod."""
        try:
            cmd = ["kubectl", "port-forward", f"pod/{pod_name}", 
                  f"--address={address}", f"{local_port}:{pod_port}", "-n", namespace]
            ...
            if process.poll() is None:
                return {
                    "status": "success",
                    "message": f"Port forwarding from {address}:{local_port} to {pod_name}:{pod_port} started",
                ...
                return {
                    "status": "error",
                    "error": f"Port forwarding failed to start: {stderr}"
                }
        except Exception as e:
            return {
                "status": "error",
                "error": f"Failed to set up port forwarding: {str(e)}"
            }
    
    def exec_command(self, pod_name: str, command: List[str], container: Optional[str] = None,
                    namespace: str = "default") -> Dict[str, Any]:
    ...
    def get_pod_logs(self, pod_name: str, container: Optional[str] = None, 
                    namespace: str = "default", tail: Optional[int] = None,
    ...
    def scale_statefulset(self, name: str, replicas: int, namespace: str = "default") -> Dict[str, Any]:
        """Scale a statefulset."""
        try:
            cmd = ["kubectl", "scale", f"statefulset/{name}", f"--replicas={replicas}", "-n", namespace]
            ...
            return {
                "status": "error",
                "error": f"Failed to scale StatefulSet: {str(e)}"
            }
