...
__date__      = '2025-May-4'                   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.5.4.html'
...
CHANGE LOG:
  ...
  Version 2.5.4:
    ...
    channel dimensionality remain unchanged from the input to the output.
...
INTRODUCTION:
    ...
    module class --- called DLStudio naturally --- and several modules of the overall
    platform.  These modules deal with Adversarial Learning, Sequence-to-Sequence
    Learning, Diffusion, Data Prediction, Text Analysis, and Transformers.
...
@tag2_vae
    ...
    A great thing about VAE is that it allows to carry out what's known as
    disentanglement learning in which the latent space captures what you believe is
    the essence in each image and have the other parts of your overall neural
    architecture capture the rest.  Subsequently, you can use the Decoder in the VAE
    to transform the essence in whichever way you want while you carry through the
    rest unchanged for the production of the final output.

    I have implemented variational autoencoding through a base class Autoencoder and
    a derived class VAE. Both of these are inner classes of the main DLStudio
    class. (That is, both these classea are defined in the file you are reading at
    the moment.) It is the Encoder and the Decoder in the base class Autoencoder that
    does the bulk of computing in VAE. What the VAE class does specifically is to
    feed the output of Autoencoder's Encoder into two nn.Linear layers for the
    learning of the mean and the log-variance of the latent distribution for the
    training dataset.  Regarding decoding, VAE's Decoder invokes what's known as the
    "reparameterization trick" for sampling the latent distribution to first
    construct a sample from the latent space, reshape the sample appropriately, and
    to then feed it into Autoencoder's Decoder.
...
THE MAIN INNER CLASSES OF THE DLStudio CLASS:
    ...
    (12) class TextClassificationWithEmbeddings
...
Examples DIRECTORY:
    ...
    (15) text_classification_with_TEXTnet.py
    ...
    (16) text_classification_with_TEXTnet_word2vec.py
    ...
    (17) text_classification_with_TEXTnetOrder2.py
    ...
    (18) text_classification_with_TEXTnetOrder2_word2vec.py
    ...
    (19) text_classification_with_GRU.py
    ...
    (20) text_classification_with_GRU_word2vec.py
...
#            plt.legend()
    ...
    class VQVAE (Autoencoder):             
        ...
        Encode-Decoder to create such a fixed-vocabulary based representation for the images.
        ...
        def __init__(self, dl_studio, encoder_in_im_size,  encoder_out_im_size, decoder_out_im_size, encoder_out_ch, num_repeats, num_codebook_vecs,
                                   codebook_vec_dim, commitment_cost, decay, path_saved_encoder, path_saved_decoder, path_saved_vector_quantizer ):
            ...
            self.decay = decay
            self.vqvae_encoder =  DLStudio.VQVAE.VQVaeEncoder(self.parent_encoder, encoder_out_im_size, encoder_out_ch)      
            self.vqvae_decoder =  DLStudio.VQVAE.VQVaeDecoder(self.parent_decoder, encoder_out_im_size, encoder_out_ch)
            self.pre_vq_conv   =  nn.Conv2d(in_channels=encoder_out_ch, out_channels=codebook_vec_dim, kernel_size=1, stride=1)
        ...
        class VQVaeEncoder(nn.Module):
            """
            For starters, I'll use the same Encoder that is in VQVAE's parent class AutoEncoder.  Am planning
            to beef it up in the next version of DLStudio.
        ...
        def run_code_for_training_VQVAE( self, vqvae, display_train_loss=False ):
            ...
            for epoch in range(self.dl_studio.epochs):                                                              
                ...
                for i, data in enumerate(self.train_dataloader):                                    
                    ...
                    decoder_out = vqvae_decoder(quantized)
        ...
        def run_code_for_evaluating_VQVAE(self, vqvae, visualization_dir = "vqvae_visualization_dir" ):
            ...
            pre_vq_conv   =  vqvae.pre_vq_conv.eval()
            ...
            pre_vq_conv.to(self.dl_studio.device)
