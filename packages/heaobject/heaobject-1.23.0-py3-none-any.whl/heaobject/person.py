from .setterlogic import id_set_setter, id_set_adder, id_sequence_remover, id_sequence_setter, id_sequence_adder, \
    id_set_remover
from .root import AbstractDesktopObject, Share, ShareImpl, Permission, View, \
    NonCreatorSuperAdminDefaultPermissionsMixin
...
class Person(NonCreatorSuperAdminDefaultPermissionsMixin, AbstractDesktopObject):
    ...
    def group_ids(self) -> list[str]:
        """
        The ids of the groups that this person belongs to. Duplicates and None ids are ignored, and the id order may
        not be preserved.
        """
    ...
    def group_ids(self, group_ids: list[str]):
        id_set_setter(self.__group_ids, group_ids)

    def add_group_id(self, group_id: str):
        id_set_adder(self.__group_ids, group_id)

    def remove_group_id(self, group_id: str):
        id_set_remover(self.__group_ids, group_id)
...
class Group(NonCreatorSuperAdminDefaultPermissionsMixin, AbstractDesktopObject):
    ...
    def role_ids(self) -> list[str]:
        """
        The ids of the roles that this group has. None ids are ignored.
        """
        return self.__role_ids.copy()

    @role_ids.setter
    def role_ids(self, role_ids: list[str]):
        id_sequence_setter(self.__role_ids, role_ids)

    def add_role_id(self, role_id: str):
        id_sequence_adder(self.__role_ids, role_id)

    def remove_role_id(self, role_id: str):
        id_sequence_remover(self.__role_ids, role_id)
