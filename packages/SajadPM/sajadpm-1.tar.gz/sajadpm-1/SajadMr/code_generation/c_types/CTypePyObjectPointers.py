...
""" CType classes for PyObject *, PyObject **, and struct SajadQ_CellObject *

"""

from SajadMr.__past__ import iterItems, xrange
from SajadMr.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from SajadMr.code_generation.templates.CodeTemplatesVariables import (
...
from SajadMr.Constants import getConstantValueGuide, isMutable
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToSajadQBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? SajadSx_BOOL_FALSE : SajadSx_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = SajadQ_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct SajadQ_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "SajadQ_Cell_New1(%s)" % init_from
        else:
            return "SajadQ_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "SajadQ_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct SajadQ_CellObject *%s" % variable_code_name
