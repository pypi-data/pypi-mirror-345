...
#ifndef PXR_TF_PY_CONTAINER_CONVERSIONS_H
#define PXR_TF_PY_CONTAINER_CONVERSIONS_H
...
struct TfPySequenceToPythonSet
{
    static PyObject* convert(ContainerType const &c)
    {
        ...
        for (const auto &elem : c) {
            PySet_Add(result, pxr::boost::python::object(elem).ptr());
        }
        return result;
    }
};
...
struct TfPyMapToPythonDict
{
    static PyObject* convert(ContainerType const &c)
    {
        return pxr::boost::python::incref(TfPyCopyMapToDictionary(c).ptr());
    }    
};
...
namespace TfPyContainerConversions {
  ...
  struct to_tuple
  {
    static PyObject* convert(ContainerType const& a)
    {
      ...
      for(const_iter p=a.begin();p!=a.end();p++) {
        result.append(pxr::boost::python::object(*p));
      }
      return pxr::boost::python::incref(pxr::boost::python::tuple(result).ptr());
    }
  };
  ...
  struct to_tuple<std::pair<First, Second> > {
    ...
    {
      pxr::boost::python::tuple result =
        pxr::boost::python::make_tuple(a.first, a.second);
      return pxr::boost::python::incref(result.ptr());
    }
  };
  ...
  struct from_python_sequence
  {
    ...
    from_python_sequence()
    {
      pxr::boost::python::converter::registry::push_back(
        &convertible,
        &construct,
        pxr::boost::python::type_id<ContainerType>());
    }
    ...
    static void* convertible(PyObject* obj_ptr)
    {
      ...
      pxr::boost::python::handle<> obj_iter(
        pxr::boost::python::allow_null(PyObject_GetIter(obj_ptr)));
    ...
    {
      for(;;i++) {
        pxr::boost::python::handle<> py_elem_hdl(
          pxr::boost::python::allow_null(PyIter_Next(obj_iter.get())));
        if (PyErr_Occurred()) {
          PyErr_Clear();
          return false;
        }
        if (!py_elem_hdl.get()) break; // end of iteration
        pxr::boost::python::object py_elem_obj(py_elem_hdl);
        pxr::boost::python::extract<container_element_type>
    ...
    {
      pxr::boost::python::handle<> obj_iter(PyObject_GetIter(obj_ptr));
      void* storage = (
        (pxr::boost::python::converter::rvalue_from_python_storage<ContainerType>*)
          data)->storage.bytes;
      ...
      for(;;i++) {
        pxr::boost::python::handle<> py_elem_hdl(
          pxr::boost::python::allow_null(PyIter_Next(obj_iter.get())));
        if (PyErr_Occurred()) pxr::boost::python::throw_error_already_set();
        if (!py_elem_hdl.get()) break; // end of iteration
        pxr::boost::python::object py_elem_obj(py_elem_hdl);
        pxr::boost::python::extract<container_element_type> elem_proxy(py_elem_obj);
  ...
  struct from_python_tuple_pair {
    ...
    from_python_tuple_pair()
    {
      pxr::boost::python::converter::registry::push_back(
        &convertible,
        &construct,
        pxr::boost::python::type_id<PairType>());
    }
    ...
    static void* convertible(PyObject* obj_ptr)
    {
      ...
      pxr::boost::python::extract<first_type> e1(PyTuple_GetItem(obj_ptr, 0));
      pxr::boost::python::extract<second_type> e2(PyTuple_GetItem(obj_ptr, 1));
    ...
    {
      void* storage = (
        (pxr::boost::python::converter::rvalue_from_python_storage<PairType>*)
          data)->storage.bytes;
      pxr::boost::python::extract<first_type>  e1(PyTuple_GetItem(obj_ptr, 0));
      pxr::boost::python::extract<second_type> e2(PyTuple_GetItem(obj_ptr, 1));
  ...
  {
    to_tuple_mapping() {
      pxr::boost::python::to_python_converter<
        ContainerType,
...
#endif // PXR_TF_PY_CONTAINER_CONVERSIONS_H
