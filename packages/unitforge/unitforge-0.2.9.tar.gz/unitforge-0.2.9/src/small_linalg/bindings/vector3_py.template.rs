...
impl Vector3Py {
    ...
    fn __setitem__(&mut self, index: usize, value: &Bound<PyAny>) -> PyResult<()> {
        ...
        for item in &mut self.data {
            if discriminant(item) != discriminant(&value_quantity) {
                *item = value_quantity.multiply(Quantity::FloatQuantity(0.)).unwrap()
            }
        }
        Ok(())
    }
            ...
            Some(unit_py) => {
                ...
                for (i, elem) in lst.iter().enumerate() {
                    values[i] = elem.extract::<f64>().map_err(|_| {
                        PyValueError::new_err(format!("Element at index {} is not a number", i))
                    })?;
                }
                Ok(Vector3Py{
                    data: [
    ...
    #[staticmethod]
    fn from_array(array: PyReadonlyArray1<f64>, unit: &Bound<PyAny>) -> PyResult<Self> {
        match array.as_slice() {
            ...
            Ok(values) => {
                ...
                let extracted_unit = Unit::from_py_any(unit).map_err(|e| PyValueError::new_err(e.to_string()))?;
                Ok(Vector3Py{
                    data: [
    ...
    fn to_array(&self, py: Python, unit: &Bound<PyAny>) -> PyResult<PyObject> {
        let extracted_unit = Unit::from_py_any(unit).map_err(|e| PyValueError::new_err(e.to_string()))?;
    ...
    fn norm(&self, py: Python) -> PyResult<PyObject> {
        fn rust_norm(data: &[Quantity; 3]) -> Option<Quantity> {
            data[0].multiply(data[0])?.add(data[1].multiply(data[1])?)?.add(data[2].multiply(data[2])?)?.sqrt()
        }
        rust_norm(&self.data)
            .ok_or_else(|| PyValueError::new_err("Failed to compute norm"))
    ...
    fn to_unit_vector(&self, py: Python) -> Self {
        ...
        Vector3Py {
            data: [
                self.data[0].divide(norm_quantity).unwrap(),
                self.data[1].divide(norm_quantity).unwrap(),
                self.data[2].divide(norm_quantity).unwrap(),
            ]
        }
    }

    fn dot_vec(&self, py: Python, other: Vector3Py) -> PyResult<PyObject> {
        fn rust_dot_vec(data_lhs: &[Quantity; 3], data_rhs: &[Quantity; 3]) -> Option<Quantity> {
            data_lhs[0].multiply(data_rhs[0])?
                .add(data_lhs[1].multiply(data_rhs[1])?)?
                .add(data_lhs[2].multiply(data_rhs[2])?)
        }
        rust_dot_vec(&self.data, &other.data)
            .ok_or_else(|| PyValueError::new_err("Failed to compute dot_vec"))
            .map(|norm_value| norm_value.to_pyobject(py))
    }

    fn cross(&self, other: Vector3Py) -> PyResult<Vector3Py> {
        fn rust_cross(data_lhs: &[Quantity; 3], data_rhs: &[Quantity; 3]) -> Option<[Quantity; 3]> {
            Some([data_lhs[1].multiply(data_rhs[2])?.sub(data_lhs[2].multiply(data_rhs[1])?)?,
                data_lhs[2].multiply(data_rhs[0])?.sub(data_lhs[0].multiply(data_rhs[2])?)?,
                data_lhs[0].multiply(data_rhs[1])?.sub(data_lhs[1].multiply(data_rhs[0])?)?])
        }
        match rust_cross(&self.data, &other.data) {
            Some(res_data) => Ok(Vector3Py { data: res_data }),
            None => Err(PyValueError::new_err("Failed to compute cross"))
        }
    }
    ...
    fn __mul__(&self, py: Python, rhs: Py<PyAny>) -> PyResult<Self> {
        ...
        Ok(Vector3Py { data: [self.data[0]
            .multiply(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot multiply given Quantities."))?, self.data[1]
            .multiply(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot multiply given Quantities."))?, self.data[2]
            .multiply(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot multiply given Quantities."))?] })
    }
    ...
    fn __truediv__(&self, py: Python, rhs: Py<PyAny>) -> PyResult<Self> {
        ...
        Ok(Vector3Py { data: [self.data[0]
            .divide(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot divide given Quantities."))?, self.data[1]
            .divide(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot divide given Quantities."))?, self.data[2]
            .divide(rhs_quantity)
            .ok_or_else(|| PyValueError::new_err("Cannot divide given Quantities."))?] })
    }
}