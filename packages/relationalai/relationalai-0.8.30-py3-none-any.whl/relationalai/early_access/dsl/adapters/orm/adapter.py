...
class ORMAdapter:
    ...
    def _add_relationships(self, md):
        ...
        complex_ucs = self._parser.complex_uniqueness_constraints()
        fact_type_to_roles = self._parser.fact_type_to_roles()
        fact_type_to_uniqueness_constraints = self._parser.fact_type_to_uniqueness_constraints()
        for fact_type, reading_orders in self._parser.fact_type_readings().items():
            with md.relationship() as rel:
                ...
                if fact_type in fact_type_to_uniqueness_constraints:
                    for uc_id in fact_type_to_uniqueness_constraints[fact_type]:
                ...
                if fact_type in complex_ucs:
                    for uc in complex_ucs[fact_type]:
                        uc_roles = []
                        for role in fact_type_to_roles[fact_type]:
                            p = md.lookup_concept(object_types[role.player].name)
