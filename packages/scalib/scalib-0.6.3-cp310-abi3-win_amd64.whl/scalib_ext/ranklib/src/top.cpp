...
extern "C" hel_result_t *helc_execute_rank(int merge_value, int nb_bins,
                                           const double *const *score_mat_init,
                                           const int *key_init) {
  return hel_execute_rank(merge_value, nb_bins, (double **)score_mat_init,
                          (int *)key_init);
}
...
extern "C" double
helc_result_get_estimation_rank_min(const hel_result_t *result) {
  ZZ estim = hel_result_get_estimation_rank_min((hel_result_t *)result);
  return conv<double>(estim);
}
extern "C" double
helc_result_get_estimation_rank_max(const hel_result_t *result) {
  ZZ estim = hel_result_get_estimation_rank_max((hel_result_t *)result);
  return conv<double>(estim);
}

extern "C" int hel_top(const double *const *log_proba, const int *real_key,
                       int nb_bin, int merge, double *min, double *est,
                       double *max) {
  ...
  result =
      hel_execute_rank(merge, nb_bin, (double **)log_proba, (int *)real_key);
