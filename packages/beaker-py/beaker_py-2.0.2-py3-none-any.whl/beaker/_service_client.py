...
class ServiceClient:
    ...
    def resolve_experiment_id(self, experiment: str | pb2.Experiment | pb2.Workload) -> str:
        ...
        if isinstance(experiment, pb2.Workload):
            if not self.beaker.workload.is_experiment(experiment):
                raise ValueError(
                    f"workload '{self.resolve_workload_id(experiment)}' is not an experiment"
                )
            return experiment.experiment.id

        # TODO: currently there's no ResolveExperimentName method.
        if "/" in experiment:
    ...
    def resolve_dataset_id(self, dataset: str | pb2.Dataset) -> str:
        ...
        # TODO: currently there's no ResolveDatasetName method.
        author_name, dataset_name = dataset.split("/", 1)
        try:
            author = self.beaker.user.get(author_name)
        except BeakerUserNotFound:
            raise BeakerDatasetNotFound(dataset)

        matches = list(
            self.beaker.dataset.list(author=author, name_or_description=dataset_name, limit=2)
        )
        if len(matches) == 1:
            return matches[0].id
        else:
    ...
    def _log_and_wait(self, retries_so_far: int, err: Exception) -> None:
        ...
        self.logger.warning(
            "Request failed with retriable error: %s\nRetrying in %d seconds...", err, retry_in
        )
        time.sleep(retry_in)
