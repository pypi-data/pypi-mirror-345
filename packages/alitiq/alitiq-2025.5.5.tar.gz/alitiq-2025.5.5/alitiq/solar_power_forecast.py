"""
alitiq's PV and Solar Power Forecast Service SDK.

This SDK provides tools for managing and interacting with alitiq's Solar Power Forecast API.
...
class alitiqSolarAPI(alitiqAPIBase):
    """
    Subclass to interact with the alitiq Solar Power Forecast API.

    This class provides methods for managing solar power plant locations, retrieving measurements, and obtaining
    ...
    def __init__(self, api_key: str) -> None:
        """
        Initialize the alitiqSolarAPI instance.

        Args:
            api_key (str): API key for authentication.
        """
        super().__init__(Services.SOLAR_POWER_FORECAST, api_key)

    def create_location(
        self, locations: Union[SolarPowerPlantModel, List[SolarPowerPlantModel]]
    ) -> str:
        """
        Create a new location (solar power plant) by sending the data to the API.

        Args:
            locations (Union[SolarPowerPlantModel, List[SolarPowerPlantModel]]):
                A single SolarPowerPlantModel instance or a list of such instances.
    ...
    def post_measurements(
        self, measurements: Union[PvMeasurementForm, List[PvMeasurementForm]]
    ) -> str:
        """
        Push new PV measurements to the API.

        Args:
            measurements (Union[PvMeasurementForm, List[PvMeasurementForm]]):
                A single PvMeasurementForm instance or a list of such instances.
    ...
    def list_locations(self) -> pd.DataFrame:
        """
        List all solar power plant locations associated with the account.
    ...
    def get_forecast(
        ...
        """
        Retrieve the power forecast for a specific solar power plant.
