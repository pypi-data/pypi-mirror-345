...
class ValueType(ConstrainedValueType):
    ...
    def build_scalar_variable(self, args, kwargs):
        if len(args) == 0:
            raise Exception(f"Unexpected error in ValueType {self.name()}")

        if len(kwargs) != 0:
            raise Exception(
                f"Cannot use keyword arguments when instantiating a type parameter for ValueSubtype {self.name()}")
    ...
    def instantiate(self, *tup):
        ...
        if len(tup) != len(cvars):
            raise Exception(
                f"ValueType {self.name()} constructor expects {len(cvars)} arguments but was instantiated with {len(tup)}")
