...
class DataTransformer:
    ...
    7) output_info -> stores dimension and output activations of columns (ie tanh for numeric, softmax for categorical)
    ...
    def __init__(self, train_data=pd.DataFrame, categorical_list=None, mixed_dict=None, n_clusters=10, eps=0.005,
                 random_state=None):
    ...
    def get_metadata(self):
        ...
        for index in range(self.train_data.shape[1]):
            ...
            if index in self.categorical_columns:
                ...
                meta.append({"name": index, "type": "mixed", "min": column.min(), "max": column.max(),
                             "modal": self.mixed_columns[index]})
            else:
                meta.append({"name": index, "type": "continuous", "min": column.min(), "max": column.max(), })
        ...
        for id_, info in enumerate(self.meta):
            if info['type'] == "continuous":
                ...
                gm = BayesianGaussianMixture(
                    n_components=self.n_clusters, weight_concentration_prior_type='dirichlet_process',
                ...
                gm1 = BayesianGaussianMixture(
                    ...
                    n_init=1, random_state=self._random_state)
                gm2 = BayesianGaussianMixture(
                    n_components=self.n_clusters, weight_concentration_prior_type='dirichlet_process',
                    weight_concentration_prior=0.001, max_iter=100,
                    n_init=1, random_state=self._random_state)
                
                # first bgm model is fit to the entire data only for the purposes of obtaining a normalized value of
                # any particular categorical mode
                gm1.fit(data[:, id_].reshape([-1, 1]))
                
                # main bgm model used to fit the continuous component and serves the same purpose as with purely
                # numeric columns
                filter_arr = []
                ...
                # similarly keeping only relevant modes with higher weight than eps and are used to fit strictly
                # continuous data
    ...
    def transform(self, data):
        ...
        for id_, info in enumerate(self.meta):
            ...
            if info['type'] == "continuous":
                ...
                # storing the mode for each data point by sampling from the probability mass distribution across all
                # modes based on fitted bgm model
                ...
                # obtaining the normalized values based on the appropriately selected mode and clipping to ensure
                ...
                # based on the means and stds of the chosen modes for categorical components, their respective values
                ...
                # transforming continuous component of mixed columns similar to purely numeric columns using second
                # fitted bgm model
                current = current.reshape([-1, 1])
                ...
                # for categorical columns, standard one-hot-encoding is applied where categories are in descending
                # order of frequency by default
    ...
    def inverse_transform(self, data):
        ...
        for id_, info in enumerate(self.meta):
            if info['type'] == "continuous":
                ...
                # obtaining approriate means and stds as per the appropriately selected mode for each data point
                # based on fitted bgm model
                ...
                # moving to the next set of columns in the raw generated data in correspondance to original column
                # information
                ...
                for idx in range(len(data)):
                    ...
                    if p_argmax[idx] < len(info['modal']):
                        ...
                        # in case of continuous mode being selected, similar inverse-transform for purely numeric
                        # values is applied
