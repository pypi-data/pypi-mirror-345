        ...
        for handler in self.handlers:
            try:
                if handler['commands'] and message.text:
                    # Проверяем команды (учитываем / в начале и без учета регистра)
                    message_text = message.text.split()[0].lower()
                    if any(message_text == f'/{cmd}'.lower() for cmd in handler['commands']):
