...
def getSupportedPythonVersions():
    """Officially supported Python versions for SajadQ."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for SajadQ."""
...
def getTargetPythonDLLPath():
    ...
    from SajadMr.Options import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from SajadMr.utils.Utils import getLaunchingSajadQProcessEnvironmentValue

    return getLaunchingSajadQProcessEnvironmentValue("SajadSx_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For SajadQ, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from SajadMr.utils.FileOperations import getDirectoryRealPath
