            ...
            try:
                ...
                if order.capa.numero_pedido_externo is not None and order.capa.numero_pedido_externo != '':
                    ...
                    if check_non_integrated_order_existence(db_config,
                        ...
                        if log:
                            send_log(job_config.get('job_name'), job_config.get('enviar_logs'), True,
                                     f'Log de integração do pedido - {q_order.pedido_oking_id}: {log}', 'error', 'internaliza_pedido_job', 'PEDIDO')
                        if sp_success:
                            send_log(job_config.get('job_name'), job_config.get('enviar_logs'), False,
            ...
            try:
                ...
                if q_order.numero_pedido_externo is not None and q_order.numero_pedido_externo != '':  # Pedido integrado anteriormente
                    ...
                    if check_order_existence(db_config, order.id):  # Pedido existente no banco semaforo
                        ...
                        if log:
                            send_log(job_config.get('job_name')
                                     , job_config.get('enviar_logs'), True,
...
def insert_temp_order_okvendas(job_config: dict, order: OrderOkvendas, db_config: DatabaseConfig, job_type: str) -> bool:
    step = ''
            ...
            if existent_client is None:
                ...
                if db_cpf_cnpj != client_cpf_cnpj:
                    send_log(job_config.get('job_name')
                             , job_config.get('enviar_logs')
                             , True
                             , f"Inconsistência: cliente ID {client_id} possui CPF/CNPJ {db_cpf_cnpj} mas era esperado {client_cpf_cnpj}, Pedido [{order.id}] "
                             , 'error'
                             , job_type  # Nome do job dinâmico
                             , f'PEDIDO [{str(order.id)}]'
                            )
                    raise Exception(f"Inconsistência: cliente ID {client_id} possui CPF/CNPJ {db_cpf_cnpj} mas era esperado {client_cpf_cnpj}")
                ...
                if order.capa.numero_pedido_externo is not None and order.capa.numero_pedido_externo != '':
                    ...
                    if check_order_existence(db_config, int(order.capa.pedido_oking_id)):
                        ...
                        if check_non_integrated_order_existence(db_config,
                            ...
                            if log:
                                send_log(job_config.get('job_name'), job_config.get('enviar_logs'), True,
                                         f'Log de integração do pedido - {q_order.pedido_oking_id}: {log}', 'error', 'duplicar_pedido_internalizado_job', 'DUPLICARPEDIDO')
                            if sp_success:
                                send_log(job_config.get('job_name'), job_config.get('enviar_logs'), False,
                                         f'Chamadas das procedures executadas com sucesso, protocolando pedido...',
                                         'info',
                            ...
                            if inserted:
                                ...
                                if log:
                                    send_log(job_config.get('job_name'), job_config.get('enviar_logs'), True,
                                             f'Log de integração do pedido - {q_order.pedido_oking_id}: {log}', 'error', 'duplicar_pedido_internalizado_job',
                                             'DUPLICARPEDIDO')
                                if sp_success:
                                    send_log(job_config.get('job_name'), job_config.get('enviar_logs'), False,
                                             f'Chamadas das procedures executadas com sucesso, protocolando pedido...',
