...
import GooeyEx.gui.events as events
from GooeyEx.gui.components.filtering.prefix_filter import PrefixSearch
...
from GooeyEx.python_bindings import constants, types as t
...
class FilterableDropdown(Dropdown):
    ...
    gooey_options = {"placeholder": str, "empty_message": str, "max_size": str}
    ...
    def getWidget(self, parent, *args, **options):
        ...
        # classes is broken.
        self.model = FilterableDropdownModel(
            self._meta["choices"], self._options, listeners=[self.interpretState]
        )
    ...
    def estimateBestSize(self):
        ...
        self.listbox.SetMaxSize(
            (-1, min(maxHeight, len(self.model.suggestions) * rowHeight))
        )
        self.listbox.SetMinSize(
            (-1, min(maxHeight, len(self.model.suggestions) * rowHeight))
        )
