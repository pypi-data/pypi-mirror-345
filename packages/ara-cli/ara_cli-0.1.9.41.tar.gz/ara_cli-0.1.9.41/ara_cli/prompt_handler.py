...
class LLMSingleton:
    ...
    def get_instance():
        if LLMSingleton._instance is None:
            config = ConfigManager().get_config()
            llm_config = config.llm_config
            first_model = next(iter(llm_config))
            LLMSingleton(first_model)
        return LLMSingleton._instance

    @staticmethod
    def set_model(model_name):
        if model_name == LLMSingleton._model:
            return LLMSingleton._instance
        LLMSingleton(model_name)
    ...
    def get_model():
        ...
        return LLMSingleton._model

    @staticmethod
    def _strip_model_name(model):
