...
mod test_force {
    ...
    fn test_inf() {
        ...
        assert!(f.get_value().is_infinite());
        assert!(!f.get_value().is_sign_negative());
    }
...
mod test_connections {
    ...
    fn test_multiply_distance_with_force() {
        ...
        assert!(approx_eq((d * f).to_raw(), m_sol.to_raw(), 1E-10));
        assert!(approx_eq((f * d).to_raw(), m_sol.to_raw(), 1E-10));
    }
