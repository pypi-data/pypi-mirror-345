...
fn main() {
    if env::var("CARGO_FEATURE_PYO3").is_err() {
        return;
    }
    ...
    let quantity_structs = extract_quantity_struct_names_from_dir(quantities_dir, &quantity_macro_re);


    write_quantity_code(&Path::new(&out_dir).join("quantities.rs"), &quantity_structs, quantities_template_path);
    write_vector_code(&Path::new(&out_dir).join("vector3_py.rs"), &quantity_structs, vector_template_path);
    write_matrix_code(&Path::new(&out_dir).join("matrix3_py.rs"), &quantity_structs, matrix_template_path);
    write_module_definition(&Path::new(&out_dir).join("python_module_definition.rs"), &quantity_structs);
}
...
fn write_quantity_code(dest_path: &PathBuf, quantity_structs: &Vec<(String, String)>, quantities_template_path: String) {
    ...
    let div_with_self_to_f64_macro_re = Regex::new(r"impl_div_with_self_to_f64!\(\s*(\w+)\)").unwrap();
    ...
    for (struct_name, content) in quantity_structs {
        ...
        to_quantity_variants += &format!("            Unit::{}Unit(unit) => Quantity::{}Quantity({}::new(value, *unit)),\n", struct_name, struct_name, struct_name);
        ...
        mul_matches += &format!(
            "                (FloatQuantity(v_lhs), {0}Quantity(v_rhs)) => Some({0}Quantity(*v_lhs * *v_rhs)),\n",
            struct_name
        );
        div_matches += &format!(
            "            ({0}Quantity(v_lhs), FloatQuantity(v_rhs)) => Some({0}Quantity(v_lhs / v_rhs)),\n",
            struct_name
        );
        add_matches += &format!(
            "            ({0}Quantity(v_lhs), {0}Quantity(v_rhs)) => Some({0}Quantity(v_lhs + v_rhs)),\n",
            struct_name
        );
        sub_matches += &format!(
            "            ({0}Quantity(v_lhs), {0}Quantity(v_rhs)) => Some({0}Quantity(v_lhs - v_rhs)),\n",
        ...
        for caps in mul_macro_re.captures_iter(content) {
            ...
            mul_matches += &format!(
                "                ({0}Quantity(v_lhs), {1}Quantity(v_rhs)) => Some({2}Quantity(*v_lhs * *v_rhs)),\n",
                lhs, rhs, result
            );
        }
        ...
        for caps in mul_with_self_macro_re.captures_iter(content) {
            ...
            mul_matches += &format!(
                "                ({0}Quantity(v_lhs), {0}Quantity(v_rhs)) => Some({1}Quantity(*v_lhs * *v_rhs)),\n",
                lhs, result
            );
        }
        ...
        for caps in div_with_self_to_f64_macro_re.captures_iter(content) {
            let lhs = &caps[1];
            div_matches += &format!(
                "            ({0}Quantity(v_lhs), {0}Quantity(v_rhs)) => Some(FloatQuantity(v_lhs / v_rhs)),\n",
                lhs
            );
        }
        ...
        for caps in sqrt_macro_re.captures_iter(content) {
            ...
            let operand = &caps[1];
            sqrt_matches += &format!(
                "            Quantity::{}Quantity(v) => Some(Quantity::{}Quantity(v.sqrt())),\n",
                operand, result
            );
        }
        first = false
    ...
    f.write_all(generated.as_bytes()).expect("Could not write quantities.rs");
}

fn write_vector_code(dest_path: &PathBuf, quantity_structs: &Vec<(String, String)>, vector_template_path: String) {
    let template = fs::read_to_string(vector_template_path)
        .expect("Failed to read vector3_py.template.rs");
    let mut raw_interfaces = String::new();
    for struct_data in quantity_structs {
        let struct_name = &struct_data.0;
        let lowercase_name = camel_to_snake(struct_name);
        raw_interfaces += &format!("\n    pub fn from_raw_{}(raw: Vector3<{}>) -> Self {{", lowercase_name, struct_name);
        raw_interfaces += "\n        Self {";
        raw_interfaces += &format!("\n            data: [Quantity::{}Quantity(raw[0]), Quantity::{}Quantity(raw[1]), Quantity::{}Quantity(raw[2])]", struct_name, struct_name, struct_name);
        raw_interfaces += "\n        }";
        raw_interfaces += "\n    }\n";
        raw_interfaces += &format!("\n    pub fn into_raw_{}(self) -> Result<Vector3<{}>, String> {{", lowercase_name, struct_name);
    ...
    f.write_all(generated.as_bytes()).expect("Could not write vector3_py.rs");
}

fn write_matrix_code(dest_path: &PathBuf, quantity_structs: &Vec<(String, String)>, vector_template_path: String) {
    let template = fs::read_to_string(vector_template_path)
        .expect("Failed to read matrix3_py.template.rs");
    let mut raw_interfaces = String::new();
    for struct_data in quantity_structs { // ToDo: Rewrite raw interface for Matrix3Py
        let struct_name = &struct_data.0;
        let lowercase_name = camel_to_snake(struct_name);
        raw_interfaces += &format!("\n    pub fn from_raw_{}(raw: Matrix3<{}>) -> Self {{", lowercase_name, struct_name);
        raw_interfaces += "\n        Self {";
        ...
        raw_interfaces += "\n        }";
        raw_interfaces += "\n    }\n";
        raw_interfaces += &format!("\n    pub fn into_raw_{}(self) -> Result<Matrix3<{}>, String> {{", lowercase_name, struct_name);
    ...
    f.write_all(generated.as_bytes()).expect("Could not write matrix3_py.rs");
}
