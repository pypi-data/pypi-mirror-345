...
class AgentID(abc.ABC):
    ...
    def __init__(self, id:str,app_path:str,aid_path:str,seed_password:str,ca_client,ep_url):
        super().__init__()       
        ...
        self.db_manager = DBManager(self.app_path,id)
        
    def get_app_path(self):
        return self.app_path
    
    def get_agent_public_path(self):
        path = os.path.join(self.app_path,"aid",self.id,"public")
        return path

    def get_agent_private_path(self):
        path = os.path.join(self.app_path,"aid",self.id,"private")
        return path

    def online(self):
        log_debug("initialzing entrypoint server")
        self.entry_client = EntrypointClient(self.id, self.ep_url,self.aid_path,self.seed_password)
        self.entry_client.initialize()

        log_debug("initialzing heartbeat server")
        self.heartbeat_client = HeartbeatClient(
            self.id, self.entry_client.get_heartbeat_server(),self.aid_path,self.seed_password
        )
        self.heartbeat_client.initialize()

        self.group_manager = GroupManager(
            self.id, self.entry_client.get_message_server(),self.aid_path,self.seed_password
        )
    ...
    def send_message(self, sessionId: str,to_aid_list: list,  message: Union[AssistantMessageBlock, list[AssistantMessageBlock], dict],ref_msg_id: str="",message_id:str=""):
        # 处理对象转换为字典
    ...
    def get_agent_public_data(self,agentid:str):
        return self.entry_client.get_agent_public_data(agentid)
    
    def sync_public_files(self):
        aid_public_path = os.path.join(self.aid_path,self.id,"public")
        self.entry_client.sync_public_files(aid_public_path)
        
    def get_agent_profile_data(self):
        path = os.path.join(self.aid_path,self.id,"public","agentprofile.json")
        try:
    ...
    def save_public_file(self,file_path:str,filename:str):
        self.entry_client.save_public_file(file_path,filename)
    ...
    def delete_public_file(self,file_path:str):
        try:
            if os.path.exists(file_path):
                ...
                self.entry_client.delete_public_file(file_path)
            else:
    ...
    def get_agent_private_data(self):
        return self.entry_client.get_agent_private_data()
...
class AgentCP(_AgentCP):
    ...
    def __init__(self, agent_data_path, certificate_path:str = "", seed_password:str = "",debug = False):
        ...
        os.path.exists(self.app_path) or os.makedirs(self.app_path)
        #if seed_password == "" or seed_password is None:
        ...
        self.aid_path = os.path.join(certificate_path, 'aid')
    ...
    def __build_url(self, aid: str):
        ...
        self.ca_client = CAClient("https://ep."+end_str,self.aid_path,self.seed_password)
    ...
    def load_aid(self, agent_id: str) -> AgentID:
        ...
        try:
            ...
            if self.__aid_is_not_exist(agent_id):  # 检查返回结果是否有效
                ...
                return None
            aid = AgentID(agent_id,self.app_path,self.aid_path,self.seed_password,self.ca_client,self.ep_url)
    ...
    def get_guest_aid(self,ep_url: str):
        self.ca_client = CAClient("https://ep."+ep_url,self.aid_path,self.seed_password)
    ...
    def create_aid(self, ap: str,agent_name: str) -> AgentID:
        ...
        if not self.__aid_is_not_exist(agent_name+"."+ap):
            return self.load_aid(agent_name+"."+ap)
            
        self.ca_client = CAClient("https://ep."+ap,self.aid_path,self.seed_password)
        self.ep_url = "https://ep."+ap
        agent_id = self.__build_id(agent_name)
        # 确保证书目录存在
        cert_dir = os.path.join(self.aid_path, agent_id)
