    ...
    def convert_excel_to_pdf(self,filename,file_storage_path):
        ...
        if file_extension == '.xls' or file_extension == '.xlsx':
            ...
            try:
                ...
                styleN.fontSize = 7.5     # Slightly smaller to fit more text
                styleN.leading = 9
                ...
                # Format all cells using Preformatted (not Paragraph)
                wrapped_data = []
                for row in data:
                    wrapped_row = []
                    for cell in row:
                        clean_cell = str(cell).replace('\n', ' ').strip()
                        wrapped_row.append(Preformatted(clean_cell, styleN))
                    wrapped_data.append(wrapped_row)

                # Dynamically calculate column widths in points
                max_widths = []
                for col in range(num_cols):
                    ...
                    max_widths.append(max_pixel_width + 5)  # Add padding

                # Scale to fit usable width exactly
                total_width_pts = sum(max_widths)
                scaling_factor = usable_width / total_width_pts if total_width_pts else 1
