...
class Header(object):
    ...
    def set_header(self, object_name: str, order: int = -1):
        ...
        if object_name not in list(
            senchar.db.headers.keys()
        ):  # do not change order if already exists
            if order == -1:
                senchar.db.headerorder.append(object_name)
            else:
                senchar.db.headerorder.insert(order, object_name)

        senchar.db.headers[object_name] = self  # may replace an existing object
    ...
    def set_title(self, title):
        ...
        # special case
        title = "senchar focal plane" if title == "Focalplane" else title
        self.title[0] = (
            "=================================================================="
        )
    ...
    def set_keyword_string(self, keystring):
        ...
        # allow for quotes
        tokens = senchar.utils.parse(keystring)
    ...
    def read_file(self, filename=""):
        ...
        if not os.path.exists(filename):
            senchar.exceptions.SencharError(f"Header file not found: {filename}")
        ...
        with open(filename, "r") as f1:
            for line in f1.readlines():
                ...
                typ, val = senchar.utils.get_datatype(value)
