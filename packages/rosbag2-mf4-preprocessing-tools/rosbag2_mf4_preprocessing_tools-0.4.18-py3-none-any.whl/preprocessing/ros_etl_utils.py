...
def get_all_fields(typename: str, typestore: Any, current_prefix: str = '', visited: Optional[Set[str]] = None) -> List[Tuple[str, str, bool]]:
    ...
    try:
        ...
        field_defs = msg_def.fields
         ...
         task_logger.warning(f"Type '{typename}' definition in typestore lacks 'fields' attribute or is not a complex type.")
         return []
    except KeyError:
        task_logger.warning(f"Type '{typename}' definition not found in typestore during field recursion (KeyError).")
        return []
    ...
    if isinstance(field_defs, dict):
        ...
        task_logger.error(f"Unexpected structure for field definitions of type '{typename}': {type(field_defs)}. Cannot iterate fields.")
        field_iterator = []

    for field_info in field_iterator:
        try:
            field_name, field_type_info = field_info
        except (TypeError, ValueError):
            task_logger.warning(f"Unexpected element structure in field definitions for type '{typename}': {field_info}. Skipping element.")
            continue
        ...
        if isinstance(field_type_info, tuple):
            if len(field_type_info) > 0:
                ...
                if isinstance(base_type_info, tuple):
                     if len(base_type_info)>0 and isinstance(base_type_info[0], str): element_type_name = base_type_info[0]
                     is_array = True
