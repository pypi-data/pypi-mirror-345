...
@_rpc_groups(["Annotations"])
@_log_call
def create_annotation(user: User, resource_id: int, data: dict) -> dict:
    """
    ...
    annotation = Annotation.objects.create(
        resource=resource_instance, owner=user, data=data
    )
...
@_rpc_groups(["Annotations"])
@_log_call
def update_annotation(user: User, annotation_id: int, data: dict) -> dict:
    """
    Updates an annotation, returning the serialized annotation
    """
    annotation_instance = Annotation.objects.filter(
        pk=annotation_id, owner=user
    ).first()
