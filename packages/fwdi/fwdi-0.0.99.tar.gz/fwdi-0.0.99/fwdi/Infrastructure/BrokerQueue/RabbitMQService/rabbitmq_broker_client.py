...
from ....Application.Abstractions.base_broker_client import BaseBrokerClient
...
class RabbitMQBrokerClient(BaseBrokerClient):
    def __init__(self, name_queue:str, credential:dict, host:str='localhost', port:int=5672):
        ...
        if credential['username'] and credential['password']:
            self.__credentials = pika.PlainCredentials(credential['username'], credential['password'])
    ...
    def create_consumer(cls:type[_CLS_], 
        ...
        new_instance = cls(name_queue, host, port)
        new_instance.mode = TypeBrokerClient.Consumer
        
        if cls.is_queue_exists(name_queue, host, port):
            new_instance.__init_executor_connection()
    ...
    def is_queue_exists(name_queue:str, host:str, port:int)->bool:
        try:
            connection_consumer:pika.BlockingConnection = pika.BlockingConnection(pika.ConnectionParameters(host=host, port=port))
