    ...
    def sign_in(self) -> Union[dict, None]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                aid_path = os.path.join(self.app_path, 'aid',self.agent_id,self.agent_id+".key")
                with open(aid_path, "rb") as f:
                    private_key = serialization.load_pem_private_key(
                        f.read(),
                        password=None
                    )
                aid_path = os.path.join(self.app_path, 'aid',self.agent_id,self.agent_id+".crt")
                with open(aid_path, "rb") as f:
