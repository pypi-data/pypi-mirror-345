...
from GooeyEx.rewx.widgets import mount as _mount
from GooeyEx.rewx.widgets import update as _update
    ...
    try:
        ...
        if isclass(vdom["type"]) and issubclass(vdom["type"], Component):
            ...
            if sizer is None:
                ...
                pool = {
                    f"__index_{index}": child.GetWindow()
                    for index, child in enumerate(sizer.GetChildren())
                }
                for index, child in enumerate(vdom["props"].get("children", [])):
...
class Component:
    ...
    def patch_component(cls, dom, vdom):
        ...
        if (
            hasattr(dom, "_instance")
            and type(dom._instance).__name__ == vdom["type"].__name__
        ):
