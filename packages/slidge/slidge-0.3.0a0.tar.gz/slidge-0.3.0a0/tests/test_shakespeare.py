...
class TestContact(ClearSessionMixin, SlidgeTest):
    ...
    def setUp(self):
        ...
        with self.xmpp.store.session() as orm:
            ...
            orm.add(user)
            orm.execute(delete(Contact))
            orm.commit()
        romeo = self.get_romeo_session()
        romeo.logged = True
        romeo.contacts.ready.set_result(True)
...
class TestCarbon(ClearSessionMixin, SlidgeTest):
    ...
    def setUp(self):
        ...
        with self.xmpp.store.session() as orm:
            user = GatewayUser(
                jid=JID("romeo@shakespeare.lit").bare,
                legacy_module_data={"username": "romeo", "city": ""},
                preferences={"sync_avatar": True, "sync_presence": True},
            )
            orm.add(user)
    ...
    def test_carbon_send_file(self):
        ...
        async def get_url(self, att, *a, **k):
            return False, att.path, "URL"
        ...
        with tempfile.NamedTemporaryFile("w+") as f:
            ...
            f.seek(0)
            self.run_coro(
                juliet.send_file(Path(f.name), legacy_msg_id=446, carbon=True)
            )
