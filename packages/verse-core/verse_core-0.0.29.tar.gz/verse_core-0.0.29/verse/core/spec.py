...
class SpecBuilder:
    ...
    def build_provider_spec(
        ...
        cnamespace, _ = component_name.split(".", 1)
        kind = Kind.CUSTOM if cnamespace == "custom" else Kind.BASE
        py_path = SpecBuilder._get_provider_path(component_name, provider_name)
        provider = Loader.load_class(py_path, Provider)
        title = " ".join(
            word.capitalize() for word in provider_name.split("_")
        )
    ...
    def build_component_spec(
        ...
        cnamespace, cpackage = component_name.split(".", 1)
        if cnamespace == "custom":
            py_path = f"{cpackage}.component"
            kind = Kind.CUSTOM
        else:
            py_path = f"{ROOT_PACKAGE_NAME}.{cnamespace}.{cpackage}.component"
            kind = Kind.BASE
        component = Loader.load_class(py_path, Component)
        title = " ".join(word.capitalize() for word in cpackage.split("_"))
        ...
        provider_names = SpecBuilder._get_provider_names(py_path)
        providers = []
        for provider_name in provider_names:
            try:
                providers.append(
                    self.build_provider_spec(component_name, provider_name)
                )
                types = SpecBuilder._combine_types_list(
                    types,
    ...
    @staticmethod
    def _get_provider_path(component_name: str, provider_name: str) -> str:
        cnamespace, cpackage = component_name.split(".", 1)
        if cnamespace == "custom":
            py_path = f"{cpackage}"
            py_path = f"{py_path}.providers.{provider_name}"
        else:
            py_path = f"{ROOT_PACKAGE_NAME}.{cnamespace}.{cpackage}"
            py_path = f"{py_path}.providers.{provider_name}"
        return py_path

    @staticmethod
    def _get_provider_names(component_package: str) -> list[str]:
        component_path = SpecBuilder._get_component_path(component_package)
    ...
    @staticmethod
    def _get_component_path(component_package: str) -> str:
        spec = importlib.util.find_spec(component_package)
    ...
    def _get_provider_parameters_custom_types(
        component_name: str, provider_name: str
    ) -> list[Any]:
        py_path = SpecBuilder._get_provider_path(component_name, provider_name)
        try:
            provider = Loader.load_class(py_path, Provider)
    ...
    def _is_operation(method) -> bool:
        return callable(method) and hasattr(method, "operation")
