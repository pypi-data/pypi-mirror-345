...
# -----------------------------------------------------------------------------
# OPTIONAL: si quisieras reutilizar la validación de constraints
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# EJEMPLO de una constraint que podrías usar:
# (Para que sirva como referencia en el caso "validate_crossover")
# -----------------------------------------------------------------------------
def constraint_check_same_size(structA, structB):
    """
    Example constraint that checks if both structures
    have the same number of atoms (already used above
    but provided as a demonstration).
    """
    return (
        len(structA.AtomPositionManager.atomLabelsList)
        == len(structB.AtomPositionManager.atomLabelsList)
    )
...
def _layer_index(z_coords, layer_thickness=1.2):
    """
    Return an integer layer ID for every atom, counting from the
    most negative z upward.  'layer_thickness' is the maximum gap
    in Å that still belongs to the same crystallographic plane.
    """
...
def crossover_top_layer_swap(structA, structB, n_layers=1, constraints=None):
    """
    Exchange the top-most *n_layers* between the two slabs.
    Useful for exploring different surface terminations.
    """
...
def crossover_surface_rotation(structA, structB, angle_max=60.0, constraints=None):
    """
    Rotate the upper half of A by a random angle (≤ angle_max°)
    about the surface normal and graft it onto B (and vice-versa).
    Tests different in-plane registries, mimicking grain-boundary
    like twists in homoepitaxy.
    """
    if constraints and not validate_crossover(structA, structB, constraints):
        return None, None
...
def crossover_adsorbate_swap(structA, structB, adsorbate_set, constraints=None):
    """
    Exchange all atoms whose labels are in 'adsorbate_set'
    (e.g. {'H', 'O', 'CO'}).  The slab backbone remains intact.
    """
    if constraints and not validate_crossover(structA, structB, constraints):
        return None, None
...
def crossover_surface_vacancies(structA, structB, vac_prob=0.3, constraints=None):
    """
    Creates complementary surface vacancies: with probability
...
def crossover_inplane_shuffle(structA, structB, shuffle_sigma=0.35, top_n_layers=2, constraints=None):
    """
    Keep parent A's slab but apply a small 2-D Gaussian shuffle to
    the top *top_n_layers*; childB receives the complementary
    distortion from B.  Encourages local configurational search on
    terraces without destroying stoichiometry.
    """
    if constraints and not validate_crossover(structA, structB, constraints):
        return None, None
