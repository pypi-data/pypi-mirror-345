...
class Credentials(root.NonCreatorSuperAdminDefaultPermissionsMixin, root.AbstractDesktopObject):
    """
    ...
    def expiration(self) -> datetime | None:
        ...
        future than the current value of the maximum_lifespan property will raise a ValueError. A None expiration may
...
class AWSCredentials(Credentials):
    ...
    def state(self) -> State:
        ...
        AWSCredentials.State.MANAGED. This property is read-only and should not be overridden."""
        return self.__state

    @property
    def lifespan_class(self) -> CredentialsLifespanClass:
        """The credentials' lifespan, by default CredentialsLifespanClass.LONG_LIVED. Long-term AWS credentials are
        LONG_LIVED. Temporary credentials are SHORT_LIVED. Setting this property to SHORT_LIVED will automatically set
        temporary to True, and setting it to LONG_LIVED will automatically set temporary to False. Setting this
        property to SHORT_LIVED will also set the for_presigned_url property to False."""
        match self.__state:
    ...
    def temporary(self) -> bool:
        ...
        this property to False sets the lifespan_class property to LONG_LIVED. Setting it to True sets the
        lifespan_class property to SHORT_LIVED and sets the for_presigned_url property to False. Likewise, setting the
        lifespan_class property to LONG_LIVED sets this property to False, and setting the lifespan_class property to
        SHORT_LIVED sets this property to True. AWSCredentials cannot be both temporary and managed."""
        match self.__state:
            case AWSCredentials.State.DEFAULT | AWSCredentials.State.MANAGED:
                return False
            case AWSCredentials.State.TEMPORARY:
    ...
    def for_presigned_url(self) -> bool:
        ...
        property to True also sets the managed property to True and the state property to MANAGED."""
    ...
    @property
    def role(self) -> str | None:
        """The role ARN string. If overridden, you must also override the role_arn property."""
    ...
    def aws_role_name(self) -> str | None:
        """The role name extracted from the role property."""
