...
from relationalai.early_access.dsl.bindings import binding_models
...
def export_models(ontology_paths) -> list[ModelObject]:
    ...
    for vt in standard_value_types.values():
        data_types_list.append(DataType(id=vt.guid(), name=vt.name()))
            ...
            if isinstance(obj, models.Model):
                ...
                for vt in obj._value_types.values():
                    ...
                    value_types_list.append(
                        ValueType(id=vt.guid(), name=vt.name(), data_type=decl.guid(), data_type_name=decl.name()))

                for et in obj._entity_types.values():
                    entity_types_list.append(
                        EntityType(id=et.guid(), name=et.name(), domain=list(map(lambda d: d.name(), et._domain))))
                    ...
                    for role in rel._rolemap.values():
                        rel_roles.append(
                            Role(id=role.guid(), name=role.name(), role_player=role.player().guid(),
                                 role_player_name=role.player().name(),
                ...
                for a in obj._subtype_arrows:
                    subtype_arrows_list.append(
                        SubtypeArrow(id=a.guid(), start=a.start.guid(), end=a.end.guid(), start_name=a.start.name(),
                                     end_name=a.end.name()))
        ...
        if isinstance(obj, binding_models.BindingModel):
            ...
            data_transformer_list = list(map(
                lambda kv: DataTransformer(
                    name=kv[0],
                    sourceType=kv[1][0].name().upper(),
                    targetType=kv[1][1].name().upper()
                ),
            ...
            entity_map_list = list(map(
                lambda kv: EntityMapRef(
                    ...
                    entityType=kv.entity_type.qualified_name(),
