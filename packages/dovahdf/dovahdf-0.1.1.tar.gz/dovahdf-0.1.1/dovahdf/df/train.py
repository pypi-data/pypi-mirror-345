...
#New Function, for MY GAMMA clearml
def run_df_train(cfg_path: str, data_dir: str, base_dir: str, pretrained: str = "deepfilternet2"):
    """
    Run DeepFilterNet training directly as a function (without argparse).
    Args:
        cfg_path: Path to dataset config file.
        data_dir: Path to dataset directory.
        base_dir: Output directory for logs, checkpoints, etc.
        pretrained: Pretrained model name to use as initialization (default: "deepfilternet2").
    """
    global should_stop, debug, state, log_timings

    if not os.path.isfile(cfg_path):
        raise FileNotFoundError(f"Dataset config not found at {cfg_path}")
    ...
    debug = False
    log_level = "INFO"
    init_logger(file=os.path.join(base_dir, "train.log"), level=log_level, model=base_dir)
    config_file = os.path.join(base_dir, "config.ini")
    config.load(config_file)
    
    # 设置预训练模型名称到配置中
    if pretrained:
        config.overwrite("train", "model", pretrained)
    ...
    signal.signal(signal.SIGUSR1, get_sigusr1_handler(base_dir))
    ...
    model, epoch = load_model(
        ...
        mask_only=mask_only,
        train_df_only=train_df_only,
        # 这里不直接传入pretrained参数，因为load_model函数会从config中读取
    )
    ...
    dataloader = DataLoader(
        ...
        log_timings=log_timings,
        global_sampling_factor=config("GLOBAL_DS_SAMPLING_F", 1.0, float, section="train"),
        snrs=config("DATALOADER_SNRS", [-5, 0, 5, 10, 20, 40], Csv(int), section="train"),
        gains=config("DATALOADER_GAINS", [-6, 0, 6], Csv(int), section="train"),
        log_level=log_level,
    )
    ...
    wds = setup_wds(len(dataloader))
    try:
        log_model_summary(model, verbose=debug)
    except Exception as e:
        logger.warning(f"Failed to print model summary: {e}")
    if jit:
        model = torch.jit.script(model)
    ...
    for epoch_idx in range(epoch, max_epochs):
        train_loss = run_epoch(
            model=model,
            epoch=epoch_idx,
            loader=dataloader,
        ...
        log_metrics(f"[{epoch_idx}] [train]", {"loss": train_loss})
        write_cp(model, "model", checkpoint_dir, epoch_idx + 1)
        write_cp(opt, "opt", checkpoint_dir, epoch_idx + 1)
        losses.reset_summaries()
        ...
        write_cp(
            model, "model", checkpoint_dir, epoch_idx + 1, metric=val_criteria, cmp=val_criteria_rule
        )
        log_metrics(f"[{epoch_idx}] [valid]", {"loss": val_loss})
    ...
    logger.info("Finished training (function call)")
