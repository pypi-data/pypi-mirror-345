...
class Replica(BaseEntity):
    def __init__(
        ...
        assert (
            ...
            == 0
        ), f"Number of layers ({self._model_config.num_layers}) is not divisible by the number of pipeline stages ({self._replica_config.num_pipeline_stages})"
        assert (
            self._model_config.embedding_dim % self._replica_config.tensor_parallel_size
            == 0
        ), f"Embedding dimension ({self._model_config.embedding_dim}) is not divisible by the tensor parallel size ({self._replica_config.tensor_parallel_size})"
    ...
    @property
    def num_kv_parallel_groups(self) -> int:
        return self._replica_config.kv_parallel_size
