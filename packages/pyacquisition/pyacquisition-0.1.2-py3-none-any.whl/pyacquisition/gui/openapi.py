...
class Method:
    ...
    def parameters(self) -> list:
        ...
        """
        return {param['name']: MethodParameter(param['name'], param) for param in self.details.get("parameters", [])}
    ...
    def responses(self) -> dict:
        ...
        return {status_code: Response(status_code, details) for status_code, details in responses.items()}
...
class Component:
    ...
    def attributes(self) -> dict:
        ...
        return {name: ComponentAttribute(name, details) for name, details in properties.items()}
        ...
        for path in self.paths.values():
            for param in path.get.parameters.values():
                if param.contains_reference:
                    ...
                    param.schema = self.component(param.schema["$ref"].split('/')[-1]).details
                else:
        ...
        for path in self.paths.values():
            for response in path.get.responses.values():
                if "$ref" in response.schema:
                    ...
                    response.schema = self.component(response.schema["$ref"].split('/')[-1]).details
