...
from slide.analysis.stats.tests import (
    StatResult,
...
class Stats:
    def __init__(self, window_matrix: csr_matrix, annotations_matrix: csr_matrix) -> None:
        """
        ...
        Args:
            ...
            annotations_matrix (csr_matrix): A sparse matrix representing annotations.
        """
        self.window_matrix = window_matrix  # Shape: (num_windows, num_labels)
        self.annotations_matrix = annotations_matrix  # Shape: (num_terms, num_labels)
        self.num_windows, self.num_labels = self.window_matrix.shape
        self.num_terms = self.annotations_matrix.shape[0]
        self.overlap_matrix = (
            self.window_matrix @ self.annotations_matrix.T
        ...
        self.term_sizes = np.asarray(
            self.annotations_matrix.sum(axis=1)
    ...
    def run(self, test: str = "poisson", null_distribution: str = "input") -> StatResult:
        ...
        Args:
            ...
            null_distribution (str): Null model basis, 'input' or 'annotations'. Defaults to 'input'.
        ...
        try:
            return dispatch[test](self.window_matrix, self.annotations_matrix, null_distribution)
        except KeyError:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        sparse_density_annotations = (
            self.annotations_matrix.nnz / self.annotations_matrix.size
            if self.annotations_matrix.size > 0
            else 0.0
        )
        return {
            ...
            "sparse_density_annotations": sparse_density_annotations,
        }
