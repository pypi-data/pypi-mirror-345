...
def _get_flattened_fields(msg_type_name: str, typestore) -> List[str]:
    ...
    flat_logger = logging.getLogger(f"{__name__}._get_flattened_fields_v3")
    ...
    def get_fields_recursive(current_type_name: str, current_prefix: str = '', visited: Optional[Set[str]] = None):
        ...
        if path_identifier in visited:
            flat_logger.warning(f"Cycle detected or already visited path: {path_identifier}. Stopping recursion for this branch.")
            return
        ...
        try:
            ...
            if not hasattr(type_definition, 'fields'):
                 flat_logger.debug(f"Type '{current_type_name}' definition via get_msgdef has no 'fields' attribute.")
                 if not current_prefix:
                     flat_logger.info(f"Message type '{current_type_name}' has no fields defined.")
                 return
            ...
            flat_logger.error(f"Error getting definition for '{current_type_name}' via get_msgdef: {e}", exc_info=True)
            return
        ...
        for field_name, field_type_info in fields_definition:
            ...
            if isinstance(field_type_info, tuple) and len(field_type_info) == 2:
                 ...
                 if type_category in [1, 2]:
                     is_array = True
                     if isinstance(type_details, tuple) and len(type_details) > 0 and isinstance(type_details[0], str):
                         base_type_name = type_details[0]
                     else:
                ...
                flat_logger.warning(f"Unexpected field_type_info format for field '{field_name}' in '{current_type_name}': {field_type_info}")
    ...
    flat_logger.debug(f"Final unique flattened fields for {msg_type_name}: {unique_field_names}")
