...
from mypy_boto3_s3.type_defs import PublicAccessBlockConfigurationTypeDef, VersioningConfigurationTypeDef, TagTypeDef
...
async def get_all_buckets(request: web.Request) -> web.Response:
    ...
    if cache_value is not None:
        bucket_dict_list: list[DesktopObjectDict] = cache_value[0]
        perms: list[list[Permission]] =  cache_value[1]
        attribute_perms: list[dict[str, list[Permission]]] = cache_value[2]
    else:
        async with DesktopObjectActionLifecycle(request=request,
            ...
            async with aws.S3ClientContext(request=request, credentials=credentials) as s3_client:
                try:
                    ...
                    bucket_coros: list[Coroutine[Any, Any, tuple[AWSBucket, dict[str, list[Permission]]]]] = []
                    ...
                    for buck, attr_perms in buck_list:
                        perms.append(buck.shares[0].permissions)
                        attribute_perms.append(attr_perms)
                        bucket_dict_list.append(buck.to_dict())
        ...
        if cached_value is not None:
            ...
            async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
                try:
                    ...
                    head_tasks: list[asyncio.Task[bool]] = []
                    for bucket in resp['Buckets']:
                        # filter out inaccessible buckets
                        bucket_name = bucket['Name']

                        async def _check_bucket_access_async(bucket_name: str, s3_client) -> bool:
                            func = partial(_check_bucket_access, bucket_name, s3_client)
                            loop = asyncio.get_running_loop()
                            return await loop.run_in_executor(None, func)

                        head_tasks.append(asyncio.create_task(_check_bucket_access_async(bucket_name, s3_client)))
                        ...
                        bucket_item.add_user_share(share)
                    ...
                    # Gather results from head tasks (maintaining order)
                    access_results = await asyncio.gather(*head_tasks, return_exceptions=True)

                    # Filter accessible buckets
                    filtered_result = [ result[i] for i, accessible in enumerate(access_results) if accessible]
                    filtered_permissions = [permissions[i] for i, accessible in enumerate(access_results) if accessible]
                    filtered_attribute_permissions = [ attribute_permissions[i] for i, accessible in enumerate(access_results) if accessible ]
                    # Update cache and response
                    request.app[HEA_CACHE][cache_key] = (filtered_result, filtered_permissions, filtered_attribute_permissions)
...
async def __get_bucket(request: web.Request, volume_id: str, s3_client: S3Client, cache,
    ...
    if context:
        async def _get_all_attribute_permissions(b: AWSBucket):
            nonlocal attr_perms
            attr_perms = await b.get_all_attribute_permissions(context)

        async_bucket_methods.append(_get_all_attribute_permissions(b))
...
def _check_bucket_access(bucket_name: str, s3_client: S3Client) -> bool:
    ...
    try:
        ...
        if e.response['Error']['Code'] == '403':
            logger.debug( f"The bucket {bucket_name} is not accessible {e}")
