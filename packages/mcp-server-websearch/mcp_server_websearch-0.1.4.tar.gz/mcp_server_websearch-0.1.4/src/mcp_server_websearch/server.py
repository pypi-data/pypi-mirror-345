...
class WebSearchInput(BaseModel):
    ...
    query: str = Field(
        description="Search keywords, similar to using a search engine like Google. Use 1–3 short phrases separated by spaces."
    )
    image: bool = Field(
    ...
    language: str = Field(
        default="zh-TW", description="Search language."
    )
    categories: list[str] = Field(
        default=[], description="Search categories (optional: general, news, etc)."
    )
