...
def is_request_malicious(environ):
    ...
    for rule_name, rule in RULES.items():
        if rule.get("enabled", False):
            ...
            try:
                if pattern and target == "query_string" and re.search(pattern, query_string, re.IGNORECASE):
                    log_blocked_request(environ, rule_name)
                    return True
                elif pattern and target == "user_agent" and re.search(pattern, user_agent, re.IGNORECASE):
                    log_blocked_request(environ, rule_name)
                    return True
