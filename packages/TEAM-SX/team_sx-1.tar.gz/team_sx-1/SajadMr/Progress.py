...
from SajadMr.utils.ThreadedExecutor import RLock
...
class SajadQProgressBar(object):
    def __init__(self, iterable, stage, total, min_total, unit):
        ...
        self.tqdm = tqdm(
            ...
            leave=False,
            dynamic_ncols=True,
            bar_format="جار التجميع {percentage:3.1f}%",
        )
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = SajadQProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withSajadQDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class SajadQDownloadProgressBar(tqdm):
        ...
        with SajadQDownloadProgressBar(*args, **kwargs) as progress_bar:
