...
class AsrWsClient:
    ...
    def signature_auth(self, data):
        ...
        header_dicts['Authorization'] = 'HMAC256; access_token="{}"; mac="{}"; h="{}"'.format(self.token,str(mac, 'utf-8'), auth_headers)
        return header_dicts
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        async with websockets.connect(
            self.ws_url, 
            additional_headers=header,  # Changed from extra_headers to additional_headers
            max_size=1000000000
        ) as ws:
