...
from GooeyEx.python_bindings.types import Success, Failure, Try, InvalidChoiceException
from GooeyEx.python_bindings.argparse_to_json import is_subparser
from GooeyEx.util.functional import lift, identity, merge
from GooeyEx.gui.constants import VALUE_PLACEHOLDER
from GooeyEx.python_bindings.constants import Events
from GooeyEx.python_bindings.coms import decode_payload
from GooeyEx.gui.constants import RADIO_PLACEHOLDER
...
def collect_errors(
    parser, error_registry: Dict[str, Exception], args: Dict[str, Try]
) -> Dict[str, str]:
    """
...
def collect_mutex_errors(parser, args: Dict[str, Try]):
    ...
    mutexes_missing = {
        dest
        for dest, v in args.items()
        if isinstance(v, Success) and v.value == RADIO_PLACEHOLDER
    }

    return {
        dest: "One of these must be provided"
