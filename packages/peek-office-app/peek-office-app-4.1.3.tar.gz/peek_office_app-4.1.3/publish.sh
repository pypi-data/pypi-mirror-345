...
function updateFileVers() {
    ...
    then
        ...
        # e.g.
        echo "Updating peek dependencies for release"
        sed -i 's#0\.0\.\*,>=0\.0\.0#'${VER}'#g' pyproject.toml
        ...
        sed -i 's/0\.0\.\*,>=0\.0\.0/'"${VERSION_MAJOR}"'.'"${VERSION_MINOR}"'.\*,>='"${VER}"'/g' pyproject.toml
    fi
}
...
# Make sure the package file name has hyphens not underscores
for file in dist/*.gz
do
  mv "$file" "${file//_/-}"
done
