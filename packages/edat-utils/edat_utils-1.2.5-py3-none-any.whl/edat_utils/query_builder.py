            ...
            if isinstance(value, dict):
                for key_dict in list(value.keys()):
                    ...
                    match key:
                        ...
                        case 'like':
                            where.append(
                                'lower(' + key_dict + ')' + ' LIKE ' + "lower('%" + eval(value_dict) + "%')")
                        case 'likeTranslate':
                            _termo = "translate(lower('%" + eval(
                                value_dict) + "%'),'áàâãäåaaaÁÂÃÄÅAAAÀéèêëeeeeeEEEÉEEÈìíîïìiiiÌÍÎÏÌIIIóôõöoooòÒÓÔÕÖOOOùúûüuuuuÙÚÛÜUUUUçÇñÑýÝ','aaaaaaaaaAAAAAAAAAeeeeeeeeeEEEEEEEiiiiiiiiIIIIIIIIooooooooOOOOOOOOuuuuuuuuUUUUUUUUcCnNyY')"
                        ...
                        case 'termsListTranslate':
                            ...
                            terms = [word.strip().replace("'", '') for word in value_dict.split()]
                            print(f'terms={terms}')
                            for term in terms:
                                print(f'term={term}')
                                search_pattern = f"%{term.lower()}%"
                                normalized_pattern = (
                                    f"translate(lower('{search_pattern}'),"
                                    f"'áàâãäåaaaÁÂÃÄÅAAAÀéèêëeeeeeEEEÉEEÈìíîïìiiiÌÍÎÏÌIIIóôõöoooòÒÓÔÕÖOOOùúûüuuuuÙÚÛÜUUUUçÇñÑýÝ',"
                                    f"'aaaaaaaaaAAAAAAAAAeeeeeeeeeEEEEEEEiiiiiiiiIIIIIIIIooooooooOOOOOOOOuuuuuuuuUUUUUUUUcCnNyY')"
                                )
                                condition = (
                                    f"lower(translate({key_dict},"
                                    f"'áàâãäåaaaÁÂÃÄÅAAAÀéèêëeeeeeEEEÉEEÈìíîïìiiiÌÍÎÏÌIIIóôõöoooòÒÓÔÕÖOOOùúûüuuuuÙÚÛÜUUUUçÇñÑýÝ',"
                                    f"'aaaaaaaaaAAAAAAAAAeeeeeeeeeEEEEEEEiiiiiiiiIIIIIIIIooooooooOOOOOOOOuuuuuuuuUUUUUUUUcCnNyY')) "
                                    f"LIKE {normalized_pattern}"
                                )
                                _where.append(condition)

                            final_where = ' OR '.join(_where)
                            print(final_where)
                            where.append(final_where)
