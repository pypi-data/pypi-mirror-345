...
"""
This is the main program of SajadQ, it checks the options and then translates
...
def getLaunchingSajadQProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run SajadQ",
            "Error",
    ...
    getLaunchingSajadQProcessEnvironmentValue("SajadSx_SITE_FILENAME")

    SajadMr_binary_name = getLaunchingSajadQProcessEnvironmentValue("SajadSx_BINARY_NAME")
    if SajadMr_binary_name is not None:
        sys.argv[0] = SajadMr_binary_name

    SajadMr_pythonpath_ast = getLaunchingSajadQProcessEnvironmentValue(
        "SajadSx_PYTHONPATH_AST"
    )

    if SajadMr_pythonpath_ast is not None:
        ...
        sys.path = [SajadMr_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingSajadQProcessEnvironmentValue("SajadSx_PYTHONPATH")
        )
    else:
        ...
        # from "SajadMr.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time SajadQ is run. This kills any attempt at
    ...
    if needs_re_execution:
        from SajadMr.utils.ReExecute import reExecuteSajadQ  # isort:skip

        # Does not return
        reExecuteSajadQ(pgo_filename=None)
    ...
    # module of SajadMr package and renamed to where we can get at easily for
    # other uses.
    __import__("SajadMr").getLaunchingSajadQProcessEnvironmentValue = (
        getLaunchingSajadQProcessEnvironmentValue
    )

    from SajadMr import Options  # isort:skip
    ...
    from SajadMr.plugins.Plugins import activatePlugins
    ...
    if "SajadSx_NAMESPACES" in os.environ:
        # Restore the detected name space packages, that were force loaded in
        # site.py, and will need a free pass later on
        from SajadMr.importing.PreloadedPackages import setPreloadedPackagePaths

        setPreloadedPackagePaths(ast.literal_eval(os.environ["SajadSx_NAMESPACES"]))
        del os.environ["SajadSx_NAMESPACES"]

    if "SajadSx_PTH_IMPORTED" in os.environ:
        # Restore the packages that the ".pth" files asked to import.
        from SajadMr.importing.PreloadedPackages import setPthImportedPackages

        setPthImportedPackages(ast.literal_eval(os.environ["SajadSx_PTH_IMPORTED"]))
        del os.environ["SajadSx_PTH_IMPORTED"]

    if "SajadSx_USER_SITE" in os.environ:
        from SajadMr.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["SajadSx_USER_SITE"]))

        del os.environ["SajadSx_USER_SITE"]

    # Now the real main program of SajadQ can take over.
    from SajadMr import MainControl  # isort:skip
...
if __name__ == "__main__":
    _SajadMr_package_home = getLaunchingSajadQProcessEnvironmentValue(
        "SajadSx_PACKAGE_HOME"
    )
    if _SajadMr_package_home is not None:
        sys.path.insert(0, _SajadMr_package_home)

        import SajadMr  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _SajadMr_package_home
        del sys.path[0]
