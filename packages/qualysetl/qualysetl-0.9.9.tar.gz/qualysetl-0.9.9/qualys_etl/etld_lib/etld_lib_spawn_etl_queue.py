...
from qualys_etl.etld_lib import etld_lib_logging_queue_writer
    ...
    try:
        with open(lock_file, 'wb+') as lock_program_fcntl:  # If locked, exit.
            ...
            if log_dir.is_dir():
                ...
                listener_proc = multiprocessing.Process(target=etld_lib_logging_queue_writer.listener, args=(log_queue, log_file_path))
                listener_proc.start()
                sys.stdout = etld_lib_logging_queue_writer.QueueWriter(log_queue)
                sys.stderr = etld_lib_logging_queue_writer.QueueWriter(log_queue)
