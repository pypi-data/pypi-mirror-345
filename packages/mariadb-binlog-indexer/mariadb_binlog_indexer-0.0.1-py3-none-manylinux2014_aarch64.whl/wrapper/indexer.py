...
class Indexer:
    def __init__(
        ...
        self.indexer_lib = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "lib", "indexer"
        )
        ...
        if search_str:
            for binlog, row_ids in result.items():
                parquet_file_path = os.path.join(
                    self.base_path, f"queries_{binlog}.parquet"
                )
    ...
    def _execute_query(
        self, source: Literal["db", "parquet"], query: str, params: list[str] = []
    ):
        result = []
