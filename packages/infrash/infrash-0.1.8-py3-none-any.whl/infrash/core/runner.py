...
class Runner:
    ...
    def is_running_pid(self, pid: int) -> bool:
        ...
        """
        try:
            # W systemach Unix możemy sprawdzić proces za pomocą sygnału 0
            # Nie wysyła on faktycznego sygnału, ale sprawdza, czy proces istnieje
            os.kill(pid, 0)
            return True
        except OSError:
            # Proces nie istnieje
            ...
            logger.error(f"Błąd podczas sprawdzania procesu {pid}: {str(e)}")
            return False

    def _get_process_info(self, path: str) -> Optional[Dict[str, Any]]:
        """
        Pobiera informacje o procesie dla danej ścieżki.
    ...
    def is_running(self, path: str) -> bool:
        ...
        # Jeśli mamy informacje o procesie, sprawdzamy czy jest uruchomiony
        if process_info and 'pid' in process_info:
            return self.is_running_pid(process_info['pid'])
