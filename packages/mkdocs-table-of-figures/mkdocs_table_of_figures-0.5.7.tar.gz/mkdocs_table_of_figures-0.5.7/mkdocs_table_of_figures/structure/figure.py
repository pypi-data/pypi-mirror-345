...
class Figure:
    ...
    # Constructors
    def __init__(self, index: int, figure_type: FigureType, caption_description: str, page: Page, match: Optional[Match] = None):
        self.index = index
        self.figure_type = figure_type
        self.caption_description = caption_description
        self.page = page
        self.match = match
...
class FiguresCollection(Collection[Figure]):
    ...
    """
    def page_figures(self, page: Page) -> 'FiguresCollection':
        figures = [figure for figure in self if figure.page == page]
        figures.sort(key=lambda f: f.index) # Ensure figure are sorted by position in the page
        return FiguresCollection(figures)
