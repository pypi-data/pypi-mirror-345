...
def generate_ensamble(path: str, source: Optional[str] = None, forces_tag: Optional[str] = None, 
    ...
    if operation[0].lower() == 'c':
        ...
        ens.compare_ensembles_abs(temperature_max=T, reference_potentials=mu, reference_state=list(mu.keys())[0], print_results=True, save_figures=True,save_data=True)
        return 
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    """
    structure_list = ['VASP', 'OUTCAR', 'xyz', 'traj', 'cif', 'AIMS', 'gen', 'POSCAR', 'ASE', 'PDB', 'DUMP', 'LAMMPS', 'metadata']
    
    parser.add_argument('--path', '--f',type=str, default='.', help='Path to the files directory')
