...
from GooeyEx.gui.components.util.wrapped_static_text import AutoWrappedStaticText
...
from GooeyEx.util.functional import getin, flatmap, indexunique
...
class ConfigPage(ScrolledPanel):
    ...
    def getName(self, group):
        ...
        """
        name = group["name"]
        return _(name) if name in {"optional_args_msg", "required_args_msg"} else name
    ...
    def getFormState(self) -> List[FormField]:
        return [widget.getUiState() for widget in self.reifiedWidgets]
    ...
    def getErrors(self):
        ...
        return {
            state["meta"]["dest"]: state["error"] for state in states if state["error"]
        }
    ...
    def setErrors(self, errorMap: Mapping[str, str]):
        ...
        widgetsByDest = {
            v._meta["dest"]: v
            for k, v in self.widgetsMap.items()
    ...
    def makeGroup(self, parent, thissizer, group, *args):
        ...
        if group_description:
            description = AutoWrappedStaticText(
                parent, label=group_description, target=boxSizer
            )
            description.SetForegroundColour(
                getin(group, ["options", "description_color"])
            )
        ...
        if (
            not getin(group, ["options", "show_border"], False)
            and group["name"]
    ...
    def chunkWidgets(self, group):
        ...
        for index, item in enumerate(group["items"]):
            ...
            if (
                len(subgroup) == getin(group, ["options", "columns"], 2)
    ...
    def reifyWidget(self, parent, item):
        ...
        from GooeyEx.gui.components import widgets
