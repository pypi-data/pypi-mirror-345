...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-lvol', help='Max logical volume per storage node', type=int,
    ...
    parser.add_argument('--nodes-per-socket', help='number of each node to be added per each socket.',
                                       type=int, default=1, dest='nodes_per_socket', required=False)
    parser.add_argument('--sockets-to-use',
                                       help='The system socket to use when adding the storage nodes', type=str,
                                       default='0', dest='sockets_to_use', required=False)
    ...
    parser.add_argument('--upgrade',
                                       help='Upgrade',
                                       type=bool, action='store_true', dest='upgrade', required=False)
    ...
    if args.upgrade:
        ...
        generate_automated_deployment_config(
            ...
            sockets_to_use=args.sockets_to_use,
            nodes_per_socket=args.nodes_per_socket,
            pci_allowed=args.pci_allowed,
            pci_blocked=args.pci_blocked
        )