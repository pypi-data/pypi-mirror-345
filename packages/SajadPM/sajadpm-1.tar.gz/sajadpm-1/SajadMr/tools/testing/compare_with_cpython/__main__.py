...
""" Tool to compare output of CPython and SajadQ.
...
from SajadMr.OptionParsing import getSajadQProjectOptions
...
from SajadMr.tools.testing.OutputComparison import compareOutput
from SajadMr.Tracing import my_print
from SajadMr.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from SajadMr.utils.FileOperations import deleteFile
from SajadMr.utils.Importing import getSharedLibrarySuffix
from SajadMr.utils.Timing import StopWatch
from SajadMr.utils.Utils import isMacOS
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.getenv("SajadSx_HASH_SALT", "")
...
def main():
    ...
    trace_command = (
        hasArg("trace_command") or os.getenv("SajadSx_TRACE_COMMANDS", "0") != "0"
    )
    ...
    extra_options = os.getenv("SajadSx_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "SajadSx" in os.environ:
        # Would need to extract which "python" this is going to use.
        assert not coverage_mode, "Not implemented for binaries."

        SajadMr_call = [os.environ["PYTHON"], os.environ["SajadSx"]]
    else:
        if comparison_mode:
            SajadMr_call = [
                os.environ["PYTHON"],
                "-m",
                "SajadMr.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates SajadQ to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses SajadSx_EXTRA_OPTIONS still.
    ...
    # Now build the command to run SajadQ.
    if not two_step_execution:
        ...
        SajadMr_cmd = SajadMr_call + extra_options + ["--run", filename]

        if no_site:
            SajadMr_cmd.insert(len(SajadMr_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            SajadMr_cmd1 = SajadMr_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            SajadMr_cmd1 = SajadMr_call + extra_options + ["--mode=standalone", filename]
        else:
            SajadMr_cmd1 = SajadMr_call + extra_options + [filename]

        if no_site:
            SajadMr_cmd1.insert(len(SajadMr_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        SajadMr_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        SajadMr_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("SajadQ command", SajadMr_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(SajadMr_package_dir):
                process = createProcess(command=SajadMr_cmd)

            stdout_SajadMr, stderr_SajadMr = process.communicate()
            exit_SajadMr = process.returncode

            if checkNoPermissionError(stdout_SajadMr) and checkNoPermissionError(
                stderr_SajadMr
            ):
                break

            my_print("Retrying SajadMr exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("SajadQ command 1:", SajadMr_cmd1)

        for _i in range(5):
            with withPythonPathChange(SajadMr_package_dir):
                stdout_SajadMr1, stderr_SajadMr1, exit_SajadMr1 = executeProcess(
                    SajadMr_cmd1
                )
...
%s
"""
                        % (SajadMr_cmd1, python_path_used, stderr_SajadMr1)
                    )

                exit_SajadMr = exit_SajadMr1
                stdout_SajadMr, stderr_SajadMr = stdout_SajadMr1, stderr_SajadMr1
                stdout_SajadMr2 = b"not run due to compilation error:\n" + stdout_SajadMr1
                stderr_SajadMr2 = stderr_SajadMr1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(SajadMr_cmd2[0][:-4] + ".cmd"):
                        SajadMr_cmd2[0] = SajadMr_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("SajadQ command 2:", SajadMr_cmd2)

                    # Need full manual control
                    process = createProcess(command=SajadMr_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "SajadQ compiled program", process.pid
                            ),
                        )

                    stdout_SajadMr2, stderr_SajadMr2 = process.communicate()
                    stdout_SajadMr = stdout_SajadMr1 + stdout_SajadMr2
                    stderr_SajadMr = stderr_SajadMr1 + stderr_SajadMr2
                    exit_SajadMr = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_SajadMr in (-11, -6) and sys.platform != "nt":
                        SajadMr_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=SajadMr_cmd2
                        )

                        callProcess(SajadMr_cmd2, shell=False)
                else:
                    exit_SajadMr = exit_SajadMr1
                    stdout_SajadMr, stderr_SajadMr = stdout_SajadMr1, stderr_SajadMr1
    ...
    stop_watch.stop()
    SajadMr_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(SajadMr_package_dir):
            stderr_SajadMr = decryptOutput(project_options, stderr_SajadMr)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(SajadMr_package_dir):
            stdout_SajadMr = decryptOutput(project_options, stdout_SajadMr)

    if not silent_mode:
        displayOutput(stdout_SajadMr, stderr_SajadMr)

        if coverage_mode:
            assert not stdout_SajadMr
            assert not stderr_SajadMr
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_SajadMr2 if two_step_execution else stdout_SajadMr,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_SajadMr2 if two_step_execution else stderr_SajadMr,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_SajadMr

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_SajadMr:d} (SajadQ)""".format(
                        exit_cpython=exit_cpython, exit_SajadMr=exit_SajadMr
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.getenv("SajadSx_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_SajadMr in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            SajadMr_cmd.insert(len(SajadMr_cmd) - 1, "--debugger")

            with withPythonPathChange(SajadMr_package_dir):
                callProcess(SajadMr_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(SajadMr_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(SajadMr_cmd2[0] + ".away"):
                        os.unlink(SajadMr_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(SajadMr_cmd2[0], SajadMr_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs SajadQ." % (cpython_time, SajadMr_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


SajadMr_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, SajadMr_package_dir)
