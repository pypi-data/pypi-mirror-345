...
class ValueSubtype(ConstrainedValueType):
    ...
    def build_scalar_variable(self, args, kwargs):
        if len(args) == 0:
            raise Exception(f"Unexpected error in ValueSubtype {self.name()}")
    ...
    def build_relation_variable(self, args, kwargs):
        ...
        if len(args) > 1:
            raise Exception(f"Cannot declare type variable of ValueSubtype {self.name()} by supplying more than a type")
    ...
    def head_display(self):
        ...
        head = ", ".join(headvars)
        return f"value type {self.name()} <: {parent.name()}({head})"
