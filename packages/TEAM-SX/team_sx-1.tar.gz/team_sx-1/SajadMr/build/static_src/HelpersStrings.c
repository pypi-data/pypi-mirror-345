...
PyObject *STRING_FROM_CHAR(unsigned char c) {
    ...
    return SajadQ_String_FromStringAndSize(s, 1);
}
...
#if PYTHON_VERSION < 0x300 && defined(_SajadSx_FULL_COMPAT)
...
PyObject *BUILTIN_ORD(PyObject *value) {
    ...
    return SajadQ_PyInt_FromLong(result);
}
...
static void _SajadQUnicode_FastCopyCharacters(PyObject *to, Py_ssize_t to_start, PyObject *from, Py_ssize_t from_start,
                                              Py_ssize_t how_many) {
...
static int _SajadQUnicode_modifiable(PyObject *unicode) {
...
#if PYTHON_VERSION < 0x3c0
static PyObject *_SajadQUnicode_New(Py_ssize_t length) {
    assert(length != 0);
    ...
    _PyUnicode_WSTR(unicode) = (Py_UNICODE *)SajadQObject_Malloc(new_size);
...
static PyObject *_SajadQUnicode_resize_copy(PyObject *unicode, Py_ssize_t length) {
    if (_PyUnicode_KIND(unicode) != PyUnicode_WCHAR_KIND) {
        ...
        _SajadQUnicode_FastCopyCharacters(copy, 0, unicode, 0, copy_length);

        return copy;
    } else {
        PyObject *w = _SajadQUnicode_New(length);
        if (unlikely(w == NULL)) {
            return NULL;
        }
...
#if PYTHON_VERSION < 0x3c0
static PyObject *_SajadQUnicode_resize_compact(PyObject *unicode, Py_ssize_t length) {
    ...
    if (unlikely(new_unicode == NULL)) {
        SajadQ_Py_NewReference(unicode);
...
static int _SajadQUnicode_resize_inplace(PyObject *unicode, Py_ssize_t length) {
    assert(!PyUnicode_IS_COMPACT(unicode));
...
static int _SajadQUnicode_resize(PyObject **p_unicode, Py_ssize_t length) {
    ...
    if (!_SajadQUnicode_modifiable(unicode)) {
        PyObject *copy = _SajadQUnicode_resize_copy(unicode, length);
        if (unlikely(copy == NULL)) {
            return -1;
        }
        Py_DECREF(*p_unicode);
    ...
    if (PyUnicode_IS_COMPACT(unicode)) {
        PyObject *new_unicode = _SajadQUnicode_resize_compact(unicode, length);
    ...
    return _SajadQUnicode_resize_inplace(unicode, length);
}
#else

#ifndef __SajadSx_NO_ASSERT__
static void _SajadQ_unicode_fill_invalid(PyObject *unicode, Py_ssize_t old_length) {
...
static PyObject *_SajadQUnicode_resize_compact(PyObject *unicode, Py_ssize_t length) {
    ...
    // assert(_SajadQ_unicode_modifiable(unicode));
    ...
    PyObject *new_unicode = (PyObject *)PyObject_Realloc(unicode, new_size);
    if (unlikely(new_unicode == NULL)) {
        SajadQ_Py_NewReferenceNoTotal(unicode);
...
#ifndef __SajadSx_NO_ASSERT__
    _SajadQ_unicode_fill_invalid(unicode, old_length);
#endif
...
PyObject *UNICODE_CONCAT(PyThreadState *tstate, PyObject *left, PyObject *right) {
    ...
    _SajadQUnicode_FastCopyCharacters(result, 0, left, 0, left_len);
    _SajadQUnicode_FastCopyCharacters(result, left_len, right, 0, right_len);
...
bool UNICODE_APPEND(PyThreadState *tstate, PyObject **p_left, PyObject *right) {
    ...
    if (_SajadQUnicode_modifiable(left) && PyUnicode_KIND(right) <= PyUnicode_KIND(left) &&
        !(PyUnicode_IS_ASCII(left) && !PyUnicode_IS_ASCII(right))) {
        if (unlikely(_SajadQUnicode_resize(p_left, new_len) != 0)) {
            return false;
        }

        _SajadQUnicode_FastCopyCharacters(*p_left, left_len, right, 0, right_len);
    } else {
        Py_UCS4 max_char = PyUnicode_MAX_CHAR_VALUE(left);
        Py_UCS4 max_char2 = PyUnicode_MAX_CHAR_VALUE(right);
        ...
        _SajadQUnicode_FastCopyCharacters(res, 0, left, 0, left_len);
        _SajadQUnicode_FastCopyCharacters(res, left_len, right, 0, right_len);
...
PyObject *SajadQUnicode_FromWideChar(wchar_t const *str, Py_ssize_t size) {
#if PYTHON_VERSION < 0x300
    if (size == -1) {
        size = wcslen(str);
    }
...
PyObject *BUILTIN_UNICODE3(PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    if (encoding == NULL) {
        encoding_str = NULL;
    } else if (SajadQ_String_Check(encoding)) {
        encoding_str = SajadQ_String_AsString_Unchecked(encoding);
    }
#if PYTHON_VERSION < 0x300
        ...
        encoding_str = SajadQ_String_AsString_Unchecked(uarg2);
    }
#endif
    else {
    ...
    if (errors == NULL) {
        errors_str = NULL;
    } else if (SajadQ_String_Check(errors)) {
        errors_str = SajadQ_String_AsString_Unchecked(errors);
    }
#if PYTHON_VERSION < 0x300
        ...
        errors_str = SajadQ_String_AsString_Unchecked(uarg3);
    }
#endif
    else {
