...
import pandas as pd
from typing import Generic, TypeVar, List, Any, Optional, Union, Tuple, Callable, ForwardRef, Dict, Iterator, Type
...
class DF(Generic[Scheme], pd.DataFrame):    # TODO override main funcs
    def __init__(self, df: Union[pd.DataFrame, Type[BaseModel], Dict, List, 'Doc', 'Docs']):
        if df is None:
            super().__init__(df)
            return
...
class DFS(Generic[Unpack[Schemes]]):
    ...
    def init(self, *dfs: Union[str, pd.DataFrame, Type[BaseModel], Dict, List], nested: bool = False) -> 'DFS':
    ...
    def __add_jdf(self, df: Union[str, pd.DataFrame, Type[BaseModel], Dict, List], type) -> None:
    ...
    def __init(self, dfs: List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]], nested: bool = False) -> None:
...
class Sink(Generic[Unpack[Schemes]]):   # FIXME inside
    ...
    def init(self, *list_data: List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]) -> 'Sink':
    ...
    def __init(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]]) -> None:
    ...
    def __init_DFS(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]], type: Any) -> None:
        for data in list_data:
            self.__data.append(type().init(*data))

    def __init_DF(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]], type: Any) -> None:
        for data in list_data:
            self.__data.append(type(*data))

    def __init_Docs(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]], type: Any) -> None:
        for data in list_data:
            self.__data.append(type(*data).init())

    def __init_Doc(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]], type: Any) -> None:
        for data in list_data:
            self.__data.append(type(*data).init())

    def __init_common(self, list_data: List[List[Union[str, pd.DataFrame, Type[BaseModel], Dict, List]]], types: Optional[Any]) -> None:
...
class Docs(Generic[Scheme]):
    ...
    @cache
    def parse(self, *, recursive: bool = False) -> Union[List[Doc[Scheme]], List[Union[Scheme, Type[BaseModel], Dict]]]:
        if recursive:
