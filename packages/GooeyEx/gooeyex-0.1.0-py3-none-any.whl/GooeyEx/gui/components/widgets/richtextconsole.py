...
class RichTextConsole(wx.richtext.RichTextCtrl):
    ...
    def __init__(self, parent):
        super(wx.richtext.RichTextCtrl, self).__init__(
            parent, -1, "", style=wx.richtext.RE_MULTILINE | wx.richtext.RE_READONLY
        )
        ...
        for index, hex in enumerate(kColorList):
            ...
            self.actionsMap[escSeq] = lambda bindedColor=wxcolor: self.BeginTextColour(
                bindedColor
            )
    ...
    def onMouseWheel(self, event):
        if (
            event.GetModifiers() == 2
            and event.GetWheelAxis() == wx.MOUSE_WHEEL_VERTICAL
        ):
