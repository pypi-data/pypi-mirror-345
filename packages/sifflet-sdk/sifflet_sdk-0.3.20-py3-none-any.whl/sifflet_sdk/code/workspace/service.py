...
class WorkspaceService:
    ...
    @exception_handler
    def delete_workspace_by_id(self, id: UUID, dry_run: bool) -> WorkspaceApplyResponseDto:
        response: WorkspaceApplyResponseDto = self.api_instance.delete_workspace(id, dry_run)
        logger.info("Workspace deleted.")
        return response

    @exception_handler
    def delete_workspace_by_file_name(self, workspace_file_name: Path, dry_run: bool) -> WorkspaceApplyResponseDto:
        _, workspace = self.get_workspace_content(workspace_file_name)
        return self.delete_workspace_by_id(workspace.id, dry_run)
    ...
    def apply_workspace(
        ...
        response = self.api_instance.apply_workspace(
            workspace_id,
            changes,
            dry_run=dry_run,
