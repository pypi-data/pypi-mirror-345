...
def is_enum(
    ...
    """
    return (
        PythonValidator.is_instance(cls, Enum) or
        PythonValidator.is_subclass(cls, Enum) or
        PythonValidator.is_subclass(cls, YTAEnum)
    )
...
def is_valid(
    ...
    """
    return (
        PythonValidator.is_instance(name_or_value_or_enum, enum) or
...
def from_name_or_value_or_enum(
    ...
    raises an Exception if not.
    """
    if PythonValidator.is_instance(name_or_value_or_enum, enum):
...
def get_enum_from_value(
    ...
    try:
        ...
        if PythonValidator.is_list(first.value):
            enum_type = 'list'
            if PythonValidator.is_instance(first.value[0], Enum):
...
def get_all(
    ...
    if (
        not PythonValidator.is_instance(enum, Enum) and
        not PythonValidator.is_subclass(enum, Enum)
    ):
...
def get_values(
    ...
    for enum in enums:
        if PythonValidator.is_list(enum.value):
            if PythonValidator.is_instance(enum.value[0], Enum):
                # It is a list of Enums, so use Enum values
