...
from GooeyEx.gui import events, processor
...
class TestProcessor(unittest.TestCase):
    ...
    def test_all_interrupts_halt_process(self):
        ...
        try:
            for sig in signals:
                print("sig", sig)
                processor = ProcessController(
                    None, None, False, "utf-8", True, shutdown_signal=sig
                )
    ...
    def test_ignore_sigint_family_signals(self):
        ...
        for program, sig in zip(programs, signals):
            ...
            process = processor = ProcessController(
                None, None, False, "utf-8", True, shutdown_signal=sig, testmode=True
            )
            process.run(cmd)
            # super-duper important sleep so that the
            # signal is actually received by the child process
