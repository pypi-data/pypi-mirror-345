...
from . import FD, Aarray, PointSet, DeformationFieldForward, DeformationFieldBackward, diffuse, gaussfun
...
def create_random_deformation_gaussian(im, amplitude=1, min_sigma=10, nblobs=50, seed=None):
    ...
    for dimension in range(len(im.shape)):
        ...
        field = Aarray(im.shape, fill=0.0, dtype="float32")
        ...
        for iter in range(nblobs):
            ...
            # Create patch
            patch = gaussfun.gaussiankernel2(sigma, 0, 0)
    ...
    deform = DeformationFieldBackward.from_field_multiscale(fields, gridsampling, injective=True, frozenedge=True)
    # Apply a bit of Gaussian diffusion
    fields = []
    for field in deform:
        fields.append(diffuse(field, 1.0))

    # Done
    return DeformationFieldBackward(*fields)


def create_random_deformation(im, amplitude=20, scale=50, n=50, frozenedge=True, mapping="backward", seed=None):
    ...
    # Init pointsets
    pp1 = PointSet(fd.ndim)
    pp2 = PointSet(fd.ndim)
    ...
    if mapping.lower() == "forward":
        DeformationField = DeformationFieldForward
    elif mapping.lower() == "backward":
        DeformationField = DeformationFieldBackward
    else:
    ...
    deform = DeformationField.from_points_multiscale(fd, gridsampling, pp1, pp2, injective=True, frozenedge=frozenedge)
...
class RandomDeformations:
    ...
    def __init__(self, im, amplitude=20, scale=50, n=50, frozenedge=True, mapping="backward", seed=None):
    ...
    def __getitem__(self, index):
        ...
        if index not in self._deforms.keys():
            ...
            deform = create_random_deformation(
                self._fd, self._amplitude, self._scale, self._n, self._frozenedge, self._mapping, seed
            )
