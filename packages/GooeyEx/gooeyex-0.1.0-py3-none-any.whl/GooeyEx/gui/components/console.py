...
class Console(wx.Panel):
    ...
    def evtUrl(self, event):
        if event.MouseEvent.LeftUp():
            ...
            webbrowser.open(
                event.GetString() or self.textbox.GetRange(event.URLStart, event.URLEnd)
            )
        event.Skip()
    ...
    def getFontFace(self):
        ...
        userFace = (
            self.buildSpec["terminal_font_family"] or self.defaultFont.GetFaceName()
        )
        return "" if self.buildSpec["monospace_display"] else userFace
