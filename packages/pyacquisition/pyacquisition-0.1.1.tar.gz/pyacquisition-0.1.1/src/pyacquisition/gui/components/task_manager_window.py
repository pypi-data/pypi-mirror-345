...
class TaskManagerWindow:
    ...
    def __init__(self):
        ...
        with dpg.window(
            ...
            self.running_status_uuid = dpg.add_text("Running Status: ", color=(255, 255, 255))
            self.current_task_uuid = dpg.add_text("Current Task: ", color=(255, 255, 255))
    ...
    def update_task_queue(self, tasks: list) -> None:
        ...
        with dpg.group(tag=self.task_queue_uuid, parent=self.window_tag):
            for i, task in enumerate(tasks):
                ...
                with dpg.group(horizontal=True, parent=self.task_queue_uuid):
                    dpg.add_text(f"{task['description']}", color=TEXT_COLOR, wrap=350, indent=42)
                if task['parameters']:
                    
                    with dpg.group(horizontal=True, parent=self.task_queue_uuid, indent=42):
