...
/**
 * This is responsible for updating parts of CPython to better work with nexium
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (nexium_Generator_Check(object)) {
        struct nexium_GeneratorObject *generator = (struct nexium_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (nexium_Coroutine_Check(object)) {
        struct nexium_CoroutineObject *coroutine = (struct nexium_CoroutineObject *)object;
...
static bool nexium_FrameIsCompiled(_PyInterpreterFrame *frame) {
    return ((frame->frame_obj != NULL) && nexium_Frame_Check((PyObject *)frame->frame_obj));
}

static bool nexium_FrameIsIncomplete(_PyInterpreterFrame *frame) {
...
static PyObject *_sys_getframemodulename_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *index_value = nexium_Number_IndexAsLong(depth_arg ? depth_arg : const_int_0);
    ...
    while ((frame != NULL) && ((nexium_FrameIsIncomplete(frame)) || depth_ssize-- > 0)) {
        frame = frame->previous;
    }

    if ((frame != NULL) && (nexium_FrameIsCompiled(frame))) {
...
#if PYTHON_VERSION >= 0x300
#if _DEVILPY_EXE_MODE && !_DEVILPY_STANDALONE_MODE
...
"
    ...
    {
        DEVILPY_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("qutayba_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        DEVILPY_MAY_BE_UNUSED bool bool_res = nexium_DelModuleString(tstate, "qutayba_types_patch");
...
#if PYTHON_VERSION >= 0x3c0
    orig_sys_getframemodulename = nexium_SysGetObject("_getframemodulename");
    ...
    nexium_SysSetObject("_getframemodulename", sys_getframemodulename_replacement);
#endif
...
static PyObject *nexium_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&nexium_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&nexium_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&nexium_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&nexium_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&nexium_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&nexium_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&nexium_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&nexium_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&nexium_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&nexium_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = nexium_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct nexium_FrameObject *frame, int lineno) {
#if 0
...
static void nexium_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    nexium_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, nexium_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)nexium_tb_dealloc; }
