...
def main(port: int, transport: str) -> int:
    ...
    if transport == "sse":
        try:
            ...
            logger.error("SSE dependencies (starlette, uvicorn) not installed. Please install with 'pip install mcp-chess[sse]' or 'pip install starlette uvicorn'")
            return 1
        ...
        async def handle_sse(request):
            async with sse.connect_sse(
                ...
                # Assuming FastMCP handles its own run logic internally
                # If direct stream handling is needed, mcp.app would be used here.
                # For FastMCP, running uvicorn should be sufficient as FastMCP integrates with FastAPI/Starlette
                # The lifespan manager should handle initialization.
                pass # Uvicorn manages the app lifecycle
            # Returning Response() might be necessary depending on Starlette version/setup
            return Response() # Added to ensure a response is returned


        # FastMCP typically returns a FastAPI/Starlette app instance
        # Let's assume mcp.app exists or mcp can be mounted
        # If mcp itself is the app:
        starlette_app = mcp.app # Assuming FastMCP provides a .app attribute

        # Add SSE routes to the existing FastMCP app
        starlette_app.add_route("/sse", endpoint=handle_sse, methods=["GET"])
        starlette_app.mount("/messages/", app=sse.handle_post_message)


        logger.info(f"Running SSE server on http://0.0.0.0:{port}")
        ...
        async def arun():
            ...
            async with stdio_server() as streams:
                # FastMCP likely handles its own run/initialization via its lifespan manager
                # We might need to adapt this if FastMCP requires explicit run call with streams
                # Assuming FastMCP automatically starts listening on stdio when run this way.
                # If not, we might need: await mcp.run(streams[0], streams[1], ...)
                # For now, let's assume the lifespan context handles it.
                # We need a way to keep the process alive while stdio_server manages IO.
                # A simple way is to wait indefinitely or until an exit signal.
                 await anyio.sleep_forever() # Keep running until interrupted
