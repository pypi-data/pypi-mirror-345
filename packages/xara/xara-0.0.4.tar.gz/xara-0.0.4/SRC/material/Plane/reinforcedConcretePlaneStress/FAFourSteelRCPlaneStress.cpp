...
OPS_Export void * OPS_ADD_RUNTIME_VPV(OPS_FAFourSteelRCPlaneStressMaterial)
{
  if (numFAFourSteelRCPPlaneStressMaterials == 0) {
    ...
    opslog << "FAFourSteelRCPPlaneStress unaxial material - Written by J.Zhong, Thomas T.C. Hsu and Y.L. Mo - Copyright@2009\n";
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid uniaxialMaterial FAFourSteelRCPPlaneStress tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial FAFourSteelRCPPlaneStress tag: " << tag << endln;
    return 0;	
  }

  numData = 6;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial FAFourSteelRCPPlaneStress tag:" << tag << endln;
    return 0;
  }

  numData = 12;
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data FAFourSteelRCPPlaneStress tag:" << tag << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nFAFourSteelRCPPlaneStress tag: " << tag << endln;
    return 0;
  }
  ...
  //now create the FAFourSteelRCPPlaneStress
  theMaterial = new FAFourSteelRCPlaneStress (tag, 
					      rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "FAFourSteelRCPPlaneStress tag: " << tag << endln;
    return 0;
  }
...
FAFourSteelRCPlaneStress ::FAFourSteelRCPlaneStress (int      tag, 
						     double   RHO,
						     UniaxialMaterial *s1,
						     UniaxialMaterial *s2,
						     UniaxialMaterial *s3,
						     ...
						     double   E,
						     double   EPSC0) :
  NDMaterial(tag, ND_TAG_FAFourSteelRCPlaneStress), rho(RHO), 
  ...
  fpc(FPC), fy(FY), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[1] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for steel3\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for steel4\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[4] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for concrete1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[5] == 0 ) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for concrete2\n";
    exit(-1);
  }
	...
	(theResponses[6] == 0) || (theResponses[7] == 0)) {
      opserr << " FAFourSteelRCPLaneStress::FAFourSteelRCPlaneStress - failed to set appropriate materials tag:" << tag << endln;
      exit(-1);
    }
...
FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress()
:NDMaterial(0, ND_TAG_FAFourSteelRCPlaneStress), strain_vec(3),
...
double FAFourSteelRCPlaneStress::getRho(void)
{
	return rho;
}


int FAFourSteelRCPlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int FAFourSteelRCPlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& FAFourSteelRCPlaneStress::getTangent(void)
{
...
int FAFourSteelRCPlaneStress::commitState(void)
{
    for (int i=0; i<6; i++)
	{
...
int FAFourSteelRCPlaneStress::revertToLastCommit(void)
{
...
int FAFourSteelRCPlaneStress::revertToStart(void)
{
  int i;
  for (i=0; i<6; i++)
    {
...
NDMaterial* FAFourSteelRCPlaneStress::getCopy(void)
{
	/*
	FAFourSteelRCPlaneStress *clone;
	clone = new FAFourSteelRCPlaneStress();
	*clone = *this;
	return clone;
	//*/

	FAFourSteelRCPlaneStress* theCopy =
		new FAFourSteelRCPlaneStress( this->getTag(), 
...
NDMaterial* FAFourSteelRCPlaneStress::getCopy(const char *type)
{
    /*
	FAFourSteelRCPlaneStress *clone;
	clone = new FAFourSteelRCPlaneStress();
	*clone = *this;
	return clone;
	//*/
	FAFourSteelRCPlaneStress* theModel =
	new FAFourSteelRCPlaneStress( this->getTag(), 
           rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], theMaterial[4], theMaterial[5],
		   angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4, fpc, fy, E0, epsc0 );
...
void FAFourSteelRCPlaneStress::Print(OPS_Stream &s, int flag )
{
	s << "\n\tFAFourSteelRCPlaneStress, material id: " << this->getTag() << endln;
    /*
	s << "\tRho: " << rho << endln;
...
int FAFourSteelRCPlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
    ...
    if (res < 0) {
      opserr << "WARNING FAFourSteelRCPlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
      return res;
	}	      
    ...
    if (res < 0) {
      opserr << "WARNING FAFourSteelRCPlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
      return res;
    }
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (res < 0) {
	opserr << "FAFourSteelRCPlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
	return res;
      }
    }	
...
int FAFourSteelRCPlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelRCPlaneStress::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelRCPlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[6];
    if (theMaterial == 0) {
      opserr << "FAFourSteelRCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "FAFourSteelRCPlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "FAFourSteelRCPlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "FAFourSteelRCPlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int FAFourSteelRCPlaneStress::determineTrialStress(void)
{ 
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "FAReinforceConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
...
double FAFourSteelRCPlaneStress::getAngleError(double inputCita)
{
	double pi = 3.14159265359;
	double outputCita;
...
double FAFourSteelRCPlaneStress::getPrincipalStressAngle(double inputAngle)
{   
	...
	if (steelStatus == 1)
      ...
      {
    	opserr << "FAReinforceConcretePlaneStress::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
      }
	...
	if ( v12*v21==1.0 )
	{
		opserr << "FAReinforceConcretePlaneStress::getPrincipalStressAngle: failure to get matrix [V]!\n";
	...
	double ita = 1.0; // set ita to 1.0 for simpification
