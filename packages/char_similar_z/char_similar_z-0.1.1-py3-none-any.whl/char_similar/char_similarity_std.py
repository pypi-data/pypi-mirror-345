...
def sim_fourangle(char1, char2, code=4):
    ...
    if char1_f4 and char2_f4:
        same_count = sum(1 for cf1, cf2 in zip(char1_f4[:code], char2_f4[:code]) if cf1 == cf2)
...
def cal_sim_by_shape(
    ...
    # 得分*权重
    result = (
        score_fourangle * rate_fourangle
        + score_component * rate_component
        + score_frequency * rate_frequency
        + score_number * rate_number
        + score_stroke * rate_stroke
        + score_struct * rate_struct
        + score_order * rate_order
    )
    rate_all = rate_fourangle + rate_component + rate_frequency + rate_number + rate_stroke + rate_struct + rate_order
    result = round(result / rate_all, rounded)
    return result


def cal_sim_by_pinyin(
    char1,
    char2,
    rounded=4,
    code=4,
