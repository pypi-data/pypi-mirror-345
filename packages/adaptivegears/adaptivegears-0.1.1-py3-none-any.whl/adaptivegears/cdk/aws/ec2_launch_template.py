...
from rebelmouse_sdk.aws_ec2_keypair import AwsEc2KeyPair
from .config import NetworkConfig, Tags
...
class AwsEc2LaunchTemplate:
    def __init__(
        ...
        instance_keypair: AwsEc2KeyPair | str | None = None,
        ...
        if isinstance(instance_keypair, AwsEc2KeyPair):
            self.keypair = instance_keypair
        else:
            self.keypair = AwsEc2KeyPair(
                self.stack,
                self.ns,
                tags=self.tags,
            )
