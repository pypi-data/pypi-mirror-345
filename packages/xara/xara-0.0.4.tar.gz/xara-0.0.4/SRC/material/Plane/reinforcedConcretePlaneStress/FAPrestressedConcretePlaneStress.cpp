...
OPS_Export void * OPS_ADD_RUNTIME_VPV(OPS_FAPrestressedConcretePlaneStressMaterial)
{
  if (numFAPrestressedConcretePlaneStressMaterials == 0) {
    numFAPrestressedConcretePlaneStressMaterials++;
    opslog << "FAPrestressedConcretePlaneStress unaxial material - Written by J.Zhong, Thomas T.C. Hsu and Y.L. Mo - Copyright@2009\n";
  }
  ...
  fpc(FPC), fy1(FY1), fy2(FY2), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocatio    
  if ( theMaterial[1] == 0 ) {
    opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed to get a copy for concrete1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed to get a copy for concrete2\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) ||
      ...
      opserr << " FAPrestressedConcretePlaneStress::FAPrestressedConcretePlaneStress - failed to set appropriate materials tag: " << tag << "\n";
      exit(-1);
  }
...
:NDMaterial(0, ND_TAG_FAPrestressedConcretePlaneStress), strain_vec(3),
...
double FAPrestressedConcretePlaneStress::getRho(void)
{
	return rho;
}


int FAPrestressedConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int FAPrestressedConcretePlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAPrestressedConcretePlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAPrestressedConcretePlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& FAPrestressedConcretePlaneStress::getTangent(void)
{
	return tangent_matrix;
}

const Vector& FAPrestressedConcretePlaneStress::getStress(void)
{
...
const Vector& FAPrestressedConcretePlaneStress :: getStrain()
{
	return strain_vec;
}
    

const Vector& FAPrestressedConcretePlaneStress::getCommittedStress(void)
{
    return stress_vec;
}


const Vector& FAPrestressedConcretePlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int FAPrestressedConcretePlaneStress::commitState(void)
{
    for (int i=0; i<4; i++)
	{
...
int FAPrestressedConcretePlaneStress::revertToLastCommit(void)
{
...
int FAPrestressedConcretePlaneStress::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
    {
...
NDMaterial* FAPrestressedConcretePlaneStress::getCopy(void)
{
	/*
	...
	*clone = *this;
	...
	FAPrestressedConcretePlaneStress* theCopy =
		new FAPrestressedConcretePlaneStress( this->getTag(), 
...
NDMaterial* FAPrestressedConcretePlaneStress::getCopy(const char *type)
{
    /*
	FAPrestressedConcretePlaneStress *clone;
	clone = new FAPrestressedConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
	FAPrestressedConcretePlaneStress* theModel =
		new FAPrestressedConcretePlaneStress( this->getTag(), 
           rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rou1, rou2, pstrain, fpc, fy1, fy2, E0, epsc0 );
...
void FAPrestressedConcretePlaneStress::Print(OPS_Stream &s, int flag )
{
...
int FAPrestressedConcretePlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
    ...
    if (res < 0) {
      opserr << "WARNING FAPrestressedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
      return res;
	}	      
    ...
    if (res < 0) {
       opserr << "WARNING FAPrestressedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
   ...
   for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "FAPrestressedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
   }	
...
int FAPrestressedConcretePlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAPrestressedConcretePlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FAPrestressedConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "FAPrestressedConcretePlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "FAPrestressedConcretePlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "FAPrestressedConcretePlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int FAPrestressedConcretePlaneStress::determineTrialStress(void)
{ 
  ...
  if ( fabs(Tstrain[0]-Tstrain[1]) < 1e-7 )
	...
	{
	  opserr << "FAPrestressedConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
...
double FAPrestressedConcretePlaneStress::getAngleError(double inputCita)
{
  double pi = 3.14159265359;
  double outputCita;
...
double FAPrestressedConcretePlaneStress::getPrincipalStressAngle(double inputAngle)
{   
  ...
  if ( (epslonOne > 0.0) && (epslonTwo > 0.0) )
    ...
    {
      opserr << "FAPrestressedConcretePlaneStress::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
    }
  ...
  miu21 = v21; // record the value for output in screen
	...
	if ( v12*v21==1.0 )
	{
		opserr << "FAPrestressedConcretePlaneStress::getPrincipalStressAngle: failure to get matrix [V]!\n";
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "FAPrestressedConcretePlaneStress::getPrincipalStressAngle: Failure to calculate principal stress direction\n";
