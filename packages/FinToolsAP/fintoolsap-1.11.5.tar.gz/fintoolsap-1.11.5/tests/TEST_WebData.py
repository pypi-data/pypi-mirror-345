...
def main():
    ...
    df = WD.getData(tickers=['AAPL', 'MSFT', 'F', 'GE'], freq = 'M')
    
    df = df.set_index('date')
    
    f, a = plt.subplots(2, 2, figsize=(12, 8), tight_layout=True)
    df[df.ticker == 'AAPL'].eps.plot(ax=a[0, 0])
    a[0, 0].set_title('AAPL')
    df[df.ticker == 'MSFT'].eps.plot(ax=a[0, 1])
    a[0, 1].set_title('MSFT')
    df[df.ticker == 'F'].eps.plot(ax=a[1, 0])
    a[1, 0].set_title('F')
    df[df.ticker == 'GE'].eps.plot(ax=a[1, 1])
    a[1, 1].set_title('GE')
