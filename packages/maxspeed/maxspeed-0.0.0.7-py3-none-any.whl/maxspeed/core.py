...
alphabet = "АКРУ"
#alphabet = sorted(alphabet) # чтобы начинались с AAA  
length = 5  
...
def truth_table():
    """Копирует код для генерации таблицы истинности в буфер обмена"""
    code = """
print('z', 'w', 'y', 'x')
for z in range(0, 2):
    for w in range(0, 2):
...
def info():
    ...
    print("- count_system(): Копирует код для работы с вычислением примера")
    print("- n_and_r_numbers(): Копирует код для работы с поиском числа")
    print("- truth_table(): Копирует код для работы с таблицей истинности")
