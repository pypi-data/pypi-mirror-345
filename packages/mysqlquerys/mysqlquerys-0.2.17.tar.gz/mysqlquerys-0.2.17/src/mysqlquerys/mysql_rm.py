...
class DataBase:
    ...
    def drop_table_list(self, tableList):
        ...
        while tableList:
            for i, tab in enumerate(tableList):
                ...
                # query = "SELECT table_name FROM information_schema.KEY COLUMN USAGE " \
                #         "WHERE table_schema = {} AND referenced table_name = {}".format(self.db_name, tab)
                # print(query)
                # cur.execute(query)
    ...
    #     FILE.close()
    #
    # def backup_profile_without_files(self, tables, output_sql_file, user_id):
    #     data = ""
    #     cur = self.db.cursor()
    #     for table in tables:
    ...
    #         for row in cur.fetchall():
    #             # data += "INSERT INTO `" + str(table) + "` VALUES("
    #             data += "INSERT INTO {} ({}) VALUES(".format(table, export_cols_query)
    #             first = True
    #             for field in row:
    #                 if not first:
    #                     data += ', '
    ...
    # def backup_profile_without_files(self, tableName, colDict, origTab):
    #     cur = self.db.cursor()
    #     selectQuery = ''
    #     for colName, alias in colDict.items():
    #         selectQuery += '{} {}, '.format(colName, alias)
...
if __name__ == '__main__':
    ...
    output_dir = r'C:\_Development\Diverse\pypi\radu\cheltuieli\src\cheltuieli\static\backup_profile\000000001'
    db.backup_profile_with_files(tables_dict, user_id=1, output_dir=output_dir, export_files=True)
    # db.drop_table_list(list(tables_dict.keys()))
    #
    # testrrrr = Table(conf.credentials, 'users')
    # # src_file = r"C:\_Development\Diverse\pypi\radu\cheltuieli\src\cheltuieli\static\backup_profile\000000001\2024_12_18__13_11_36_user_name.sql"
    # # print(db.run_sql_file(src_file))
