...
class Device(Login):
	...
	def __init__(self):
		...
		if not '/static' in request.path:
			...
			with self('before', **{'cookies': request.cookies,
								   ...
								   'X-Key-Signature': request.headers.get('X-Key-Signature', str()) or 
									 		  	  	  request.form.get('X-Key-Signature', str())}) as tcp:
				if tcp.response in (401, 403):
					#: The response from the service side may require the user to 
					#: logout if the device keys do not match the server side or 
					#: if data manipulation is detected.
					if worker.is_authenticated:
						worker.logout()
					abort(tcp.response)
				elif tcp.response in (417, 410):
					#: The server may require the user to re-authenticate if the 
					...
					#: the transient key. 
					self.__xsid   = tcp.response.get('xsid', bytes())
					self.__screen = tcp.response.get('screen', bytes())
		...
		with self('validate', **{'screen': screen,
			...
			if response := tcp.response:
				if not response.get('status'):
					...
					#: should be revoked imidietly, even if javascript redirect
					#: to logout view fails.
					if worker.is_authenticated:
						worker.logout()
				return response
		return dict(status=False)
	...
	def screen(self):
		...
		"""
		return binascii.hexlify(self.__screen).decode()
	...
	def xsid(self):
		...
		"""
		return binascii.hexlify(self.__xsid).decode()
	...
	def nonce(self) -> str:
		...
		if not self.__nonce:
			...
			with self('nonce') as tcp:
				if tcp.response:
					if nonce := tcp.response.get('nonce'):
						self.__nonce = nonce
						if signature := tcp.response.get('signature'):
