...
class Authentication:  # noqa: D101
    ...
    def auth_header(
        ...
        if self.client.config.org_name:
            headers.update({"nv-ngc-context": json.dumps({"orgName": self.client.config.org_name})})
        return headers
    ...
    def get_starfleet_kas_session_key(self, starfleet_kas_email):
        ...
        try:
            return (
                ...
                Authentication._token_cache["starfleet_kas_session_key"][1],
            )
        except AuthenticationException:
            return None
