...
class BaseRepository:
    ...
    def _build_filter_conditions(self, filter_dict: Dict) -> List[expression.BinaryExpression]:
        conditions = []
            ...
            for condition in filter_dict["conditions"]:
                if isinstance(condition, dict):
                    ...
                    if "operator" in condition and "conditions" in condition:
                        nested_result = self._build_filter_conditions(condition)
                        if nested_result:
                    ...
                    # Handle comparison operators
                        ...
                        try:
                            ...
                            nested_conditions.append(
                                self._apply_comparison_operator(condition["field"], comparison_op, condition["value"])
                            )
                        except ValueError:
    ...
    def _build_sort_orders(self, sort_orders: List[SortOrder]) -> List[expression.UnaryExpression]:
        orders = []
    ...
    def read_by_options(
        ...
        with self.session_factory() as session:
            try:
                ...
                if filter_dict:
                    filter_conditions = self._build_filter_conditions(filter_dict)
                ...
                if sort_orders and len(sort_orders) > 0:
                    query = query.order_by(*self._build_sort_orders(sort_orders))
                else:
        ...
        with self.session_factory() as session:
            try:
                ...
                if not hasattr(self.model, attr):
                    raise ValidationError(detail=f"Field '{attr}' does not exist on model {self.model.__name__}")
                ...
                if not query:
                    raise NotFoundError(detail=f"not found {attr} : {value}")
                return query
            except SQLAlchemyError as e:
                raise ValidationError(detail=str(e))
