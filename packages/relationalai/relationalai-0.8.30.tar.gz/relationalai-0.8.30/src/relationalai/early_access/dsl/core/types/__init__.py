...
from relationalai.early_access.dsl.core.constraints.scalar import entails
...
class AbstractValueType(Type):
    ...
    def root_nominal_type(self):
        raise Exception(f"Cannot determine root nominal type of AbstractValueType {self.name()}")
    ...
    def subtype_of(self, T2):
        if self.nominal():
            ...
            if self.unconstrained():
                return self.name == T2.name

            if T2.unconstrained():
                u2 = self.root_unconstrained_type() # type: ignore
                return T2.name == u2.name
        ...
        # [[cset1]] NOT subseteq [[cset2]] if there exists a c in cset2 that is not entailed
        # by cset1
        #
        for c in cset2:
            if not entails(cset1, cset2[c]):
                return False
