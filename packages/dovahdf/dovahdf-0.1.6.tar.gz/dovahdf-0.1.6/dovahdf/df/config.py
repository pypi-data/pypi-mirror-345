...
class Config:
    ...
    def load(
        ...
        if path is None and "DF_CONFIG_PATH" in os.environ and os.path.isfile(os.environ["DF_CONFIG_PATH"]):
            path = os.environ["DF_CONFIG_PATH"]
            logger.debug(f"Using config path from environment: {path}")

        # 尝试Docker环境中的固定路径
        if path is None:
            docker_path = "/workspace/AudioClarity/models/DeepFilterNet2/config.ini"
            if os.path.isfile(docker_path):
                path = docker_path
                logger.debug(f"Found config at Docker path: {path}")
        ...
        if path is None:
            possible_paths = [
                ...
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../models/DeepFilterNet2/config.ini"),
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../../models/DeepFilterNet2/config.ini")
            ]
            for p in possible_paths:
                if os.path.isfile(p):
                    path = p
                    logger.debug(f"Found config at default location: {path}")
                    break
        ...
        if path is not None and os.path.isfile(path):
            ...
            logger.warning(f"No configuration file found or loaded. Using defaults.")
...
try:
    ...
    for p in possible_paths:
        if p and os.path.isfile(p):
            config.load(p, allow_defaults=True, allow_reload=True)
            break
