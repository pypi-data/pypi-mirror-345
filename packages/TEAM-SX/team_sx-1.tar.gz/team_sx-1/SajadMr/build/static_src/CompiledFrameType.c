...
#if _DEBUG_REFCOUNTS
int count_active_SajadQ_Frame_Type = 0;
int count_allocated_SajadQ_Frame_Type = 0;
int count_released_SajadQ_Frame_Type = 0;
#endif
...
static PyMemberDef SajadQ_Frame_members[] = {
...
#else
#define SajadQ_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_SajadQ_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct SajadQ_FrameObject *frame = (struct SajadQ_FrameObject *)self;
...
static int _SajadQ_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadQ_Frame_get_exc_type(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadQ_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadQ_Frame_get_exc_value(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadQ_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadQ_Frame_get_restricted(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadQ_Frame_get_locals(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct SajadQ_FrameObject *SajadMr_frame = (struct SajadQ_FrameObject *)self;
    if (SajadMr_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &SajadMr_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &SajadMr_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = SajadQ_GetCodeVarNames(SajadQ_GetFrameCodeObject(SajadMr_frame));

        char const *w = SajadMr_frame->m_type_description;
        char const *t = SajadMr_frame->m_locals_storage;

        while (*w != 0) {
            switch (*w) {
            case SajadSx_TYPE_DESCRIPTION_OBJECT:
            case SajadSx_TYPE_DESCRIPTION_OBJECT_PTR: {
            ...
            case SajadSx_TYPE_DESCRIPTION_CELL: {
                struct SajadQ_CellObject *value = *(struct SajadQ_CellObject **)t;
                assert(SajadQ_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct SajadQ_CellObject *);

                break;
            }
            case SajadSx_TYPE_DESCRIPTION_NULL: {
                break;
            }
            case SajadSx_TYPE_DESCRIPTION_BOOL: {
                int value = *(int *)t;
                t += sizeof(int);
                switch ((SajadMr_bool)value) {
                case SajadSx_BOOL_TRUE: {
                    DICT_SET_ITEM(result, *var_names, Py_True);
                    break;
                }
                case SajadSx_BOOL_FALSE: {
...
static PyObject *_SajadQ_Frame_get_lineno(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct SajadQ_FrameObject *frame = (struct SajadQ_FrameObject *)self;
    return SajadQ_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_SajadQ_Frame_get_trace(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadQ_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
#if !defined(_SajadSx_DEPLOYMENT_MODE) && !defined(_SajadSx_NO_DEPLOYMENT_FRAME_USELESS_SET_TRACE)
    if (value == Py_None) {
        return 0;
    } else {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in SajadQ, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_SajadQ_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadQ_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in SajadQ");
    return -1;
}

static PyObject *_SajadQ_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadQ_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(SajadQ_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in SajadQ");
    return -1;
}
#endif

static PyGetSetDef SajadQ_Frame_tp_getset[] = {
    {(char *)"f_locals", _SajadQ_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _SajadQ_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _SajadQ_Frame_get_trace, _SajadQ_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _SajadQ_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _SajadQ_Frame_get_exc_traceback, _SajadQ_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _SajadQ_Frame_get_exc_type, _SajadQ_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _SajadQ_Frame_get_exc_value, _SajadQ_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _SajadQ_Frame_get_trace_lines, _SajadQ_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _SajadQ_Frame_get_trace_opcodes, _SajadQ_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *SajadQ_Frame_tp_repr(struct SajadQ_FrameObject *SajadMr_frame) {
    assert(SajadQ_Frame_CheckExact((PyObject *)SajadMr_frame));
    CHECK_OBJECT((PyObject *)SajadMr_frame);
    assert(_PyObject_GC_IS_TRACKED(SajadMr_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = SajadQ_GetFrameCodeObject(SajadMr_frame);
    return SajadQ_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", SajadMr_frame,
                                    code_object->co_filename, SajadQ_GetFrameLineNumber(SajadMr_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = SajadQ_GetFrameCodeObject(SajadMr_frame);
    return SajadQ_String_FromFormat("<compiled_frame object for %s at %p>",
                                    SajadQ_String_AsString(code_object->co_name), SajadMr_frame);
#else
    return SajadQ_String_FromFormat("<compiled_frame object at %p>", SajadMr_frame);
#endif
}

static void SajadQ_Frame_tp_clear(struct SajadQ_FrameObject *frame) {
    if (frame->m_type_description) {
        ...
        while (*w != 0) {
            ...
            case SajadSx_TYPE_DESCRIPTION_OBJECT:
            case SajadSx_TYPE_DESCRIPTION_OBJECT_PTR:
            case SajadSx_TYPE_DESCRIPTION_NILONG: {
                PyObject *value = *(PyObject **)t;
                CHECK_OBJECT_X(value);
...
static struct SajadQ_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void SajadQ_Frame_tp_dealloc(struct SajadQ_FrameObject *SajadMr_frame) {
#if _DEBUG_REFCOUNTS
    count_active_SajadQ_Frame_Type -= 1;
    count_released_SajadQ_Frame_Type += 1;
#endif
    ...
    SajadQ_GC_UnTrack(SajadMr_frame);

    PyFrameObject *frame = &SajadMr_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    assert(SajadQ_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    SajadQ_Frame_tp_clear(SajadMr_frame);

    if (Py_REFCNT(SajadMr_frame) > 0) {
        Py_SET_REFCNT(SajadMr_frame, Py_REFCNT(SajadMr_frame) - 1);
        if (Py_REFCNT(SajadMr_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(SajadMr_frame, SajadMr_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, SajadMr_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static int SajadQ_Frame_tp_traverse(struct SajadQ_FrameObject *frame, visitproc visit, void *arg) {
    assert(SajadQ_Frame_CheckExact((PyObject *)frame));
    ...
    while (w != NULL && *w != 0) {
        ...
        default:
            SajadSx_CANNOT_GET_HERE("invalid type description");
            assert(false);
        }
...
static PyObject *SajadQ_GetFrameGenerator(struct SajadQ_FrameObject *SajadMr_frame) {
#if PYTHON_VERSION < 0x3b0
    return SajadMr_frame->m_frame.f_gen;
#else
    return SajadMr_frame->m_generator;
#endif
}

static PyObject *SajadQ_Frame_clear(struct SajadQ_FrameObject *frame, PyObject *unused) {
    assert(SajadQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (SajadQ_Frame_IsExecuting(frame)) {
    ...
    PyObject *f_gen = SajadQ_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (SajadQ_Generator_Check(f_gen)) {
            struct SajadQ_GeneratorObject *generator = (struct SajadQ_GeneratorObject *)f_gen;
            SajadQ_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadQ_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (SajadQ_Coroutine_Check(f_gen)) {
            struct SajadQ_CoroutineObject *coroutine = (struct SajadQ_CoroutineObject *)f_gen;
            SajadQ_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadQ_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (SajadQ_Asyncgen_Check(f_gen)) {
            struct SajadQ_AsyncgenObject *asyncgen = (struct SajadQ_AsyncgenObject *)f_gen;
            SajadQ_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadQ_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t SajadQ_Frame_GetSize(struct SajadQ_FrameObject *frame) {
    assert(SajadQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *SajadQ_Frame_sizeof(struct SajadQ_FrameObject *frame, PyObject *unused) {
    assert(SajadQ_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct SajadQ_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef SajadQ_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)SajadQ_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)SajadQ_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject SajadQ_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct SajadQ_FrameObject),
    1,
    (destructor)SajadQ_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)SajadQ_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)SajadQ_Frame_tp_traverse,  // tp_traverse
    (inquiry)SajadQ_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    SajadQ_Frame_methods,                    // tp_methods
    SajadQ_Frame_members,                    // tp_members
    SajadQ_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(SajadQ_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(SajadQ_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(SajadQ_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(SajadQ_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(SajadQ_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(SajadQ_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(SajadQ_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(SajadQ_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(SajadQ_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(SajadQ_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(SajadQ_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(SajadQ_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(SajadQ_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(SajadQ_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(SajadQ_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(SajadQ_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(SajadQ_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(SajadQ_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(SajadQ_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(SajadQ_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(SajadQ_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(SajadQ_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(SajadQ_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(SajadQ_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(SajadQ_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    SajadQ_PyType_Ready(&SajadQ_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct SajadQ_FrameObject, m_frame) == 0);
}

static struct SajadQ_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_SajadQ_Frame_Type += 1;
    count_allocated_SajadQ_Frame_Type += 1;
#endif
    ...
    struct SajadQ_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct SajadQ_FrameObject, SajadQ_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    SajadQ_SetFrameGenerator(result, NULL);

    SajadQ_Frame_MarkAsNotExecuting(result);
#endif
...
struct SajadQ_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct SajadQ_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct SajadQ_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(SajadQ_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(SajadQ_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        SajadSx_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("SajadMr_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        SajadSx_MAY_BE_UNUSED bool bool_res = SajadQ_DelModuleString(tstate, "SajadMr_empty_function");
    ...
    if (result == NULL) {
        PyErr_PrintEx(0);
        SajadSx_CANNOT_GET_HERE("Failed to create code object");
    }
...
void SajadQ_Frame_AttachLocals(struct SajadQ_FrameObject *frame_object, char const *type_description, ...) {
    assert(SajadQ_Frame_CheckExact((PyObject *)frame_object));
    ...
    PRINT_FORMAT("Attaching to frame 0x%lx %s\n", frame_object,
                 SajadQ_String_AsString(PyObject_Repr((PyObject *)SajadQ_Frame_GetCodeObject(&frame_object->m_frame))));
#endif
    ...
    while (*w != 0) {
        ...
        case SajadSx_TYPE_DESCRIPTION_CELL: {
            struct SajadQ_CellObject *value = va_arg(ap, struct SajadQ_CellObject *);
            assert(SajadQ_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct SajadQ_CellObject *));
            ...
            // Py_SET_REFCNT((struct SajadQ_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case SajadSx_TYPE_DESCRIPTION_NULL: {
            SajadSx_MAY_BE_UNUSED void *value = va_arg(ap, struct SajadQ_CellObject *);
        ...
        case SajadSx_TYPE_DESCRIPTION_NILONG: {
            SajadMr_ilong value = va_arg(ap, SajadMr_ilong);
    ...
    assert(t - frame_object->m_locals_storage <= SajadQ_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)SajadQ_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, SajadQ_String_AsString(current_repr), Py_REFCNT(current),
                     SajadQ_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)SajadQ_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct SajadQ_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, SajadQ_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)SajadQ_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
