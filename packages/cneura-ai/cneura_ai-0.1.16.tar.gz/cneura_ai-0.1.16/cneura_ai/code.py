...
class CodeGenerator:
    ...
    def generate_test_script(self, code: str) -> str:
        ...
        from script import code #  Assume the class is in script.py
        ...
        - The test script must be execute as normal python script like this `python test.py`. 
        - You need to include this snippet to the end of the test script.
        ```python
        if __name__ == '__main__':
            unittest.main()
        ```
        ...
        if not data:
            raise ValueError("The data key not found on llm response")
        return self.parse_code(data) if data else "No valid test code returned from LLM."
