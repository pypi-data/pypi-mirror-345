...
Implementation of scope nesting in pycroscope.

This module is responsible for mapping names to their values in pycroscope. Variable lookup happens
mostly through a series of nested dictionaries. When pycroscope sees a reference to a name inside a
...
- Multiple assignments to the same name result in :class:`pycroscope.value.MultiValuedValue`
- Globals are represented as :class:`pycroscope.value.ReferencingValue`, and name lookups for such
  names are delegated to the :class:`pycroscope.value.ReferencingValue`\'s scope
...
class Composite(NamedTuple):
    """A :class:`pycroscope.value.Value` with information about its
...
class FunctionScope(Scope):
    ...
    will produce a :class:`pycroscope.value.MultiValuedValue` because it does not know whether the
    ...
    dictionary, which maps each local variable to a list of active definition nodes. When pycroscope
    ...
    After pycroscope visits all branches, it runs the `combine_subscopes` operation on all of the
    ...
    a naive approach would infer that `x` is ``None`` at (a). To take care of this case, pycroscope
...
class StackedScopes:
    ...
    def get(self, varname: Varname, node: Node, state: VisitorState) -> Value:
        ...
        Returns :data:`pycroscope.value.UNINITIALIZED_VALUE` if the name is not defined in any known
        scope.
