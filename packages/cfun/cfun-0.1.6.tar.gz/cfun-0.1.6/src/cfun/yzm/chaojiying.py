...
class Chaojiying_Client(object):
    ...
    def parse_response(self, res: dict | str) -> list:
        """
        解析超级鹰返回的字符串格式
        :param res: 超级鹰返回的字符串 或者字典格式
        :return: 解析后的列表  [{"name": "之", "coordinates": [207, 115]}, {"name": "成", "coordinates": [158, 86]}, {"name": "人", "coordinates": [126, 44]}]
        eg：
        input_str = '之,207,115|成,158,86|人,126,44'
    ...
    def get(self, image_path, codetype=9800):
        """
        根据图片和题目类型获取超级鹰识别结果，
        暂时只针对 codetype=9800 进行处理，其他类型的暂未测试
