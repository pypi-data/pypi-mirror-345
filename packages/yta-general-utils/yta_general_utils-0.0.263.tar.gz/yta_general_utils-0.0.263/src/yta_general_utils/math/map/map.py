...
class Map:
    ...
    def _get_pair_of_coordinates_index_from_normalized_global_d(
        ...
        """
        return int(d // self._get_pair_of_coordinates_d()) if d != 1.0 else len(self.pairs_of_coordinates) - 1
    ...
    def _get_normalized_local_d_from_normalized_global_d(
        ...
        if d != 1.0:
            d = (
                ...
                if pair_of_coordinates_index > 0 else d / pair_of_coordinates_d
            )
    ...
    def plot(self):
        ...
        # Draw nodes
        x_vals = [coordinate.x for coordinate in self.coordinates]
        y_vals = [coordinate.y for coordinate in self.coordinates]
