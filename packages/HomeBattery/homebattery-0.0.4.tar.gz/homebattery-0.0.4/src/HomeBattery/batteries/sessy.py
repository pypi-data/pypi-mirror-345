...
class SessyInverter(Battery):
    ...
    def __init__(
        ...
        if manual_mode:
            self.set_active_strategy(PowerStrategy.API)
    ...
    async def _get_power_status(self):
        ...
        try:
            async with self._client.get(endpoint) as response:
                ...
                if sessy_data:
                    self._soc = sessy_data.get('state_of_charge')
        ...
        try:
            async with self._client.get(endpoint) as response:
                ...
                if phase_data:
                    ...
                    self._voltage[f"phase{phase}"] = phase_voltage
    ...
    async def get_phase_voltage(self, phase: int):
        ''' Get the voltage of a specific phase of the inverter'''
    ...
    async def get_phase_current(self, phase: int):
        ''' Get the active current of a specific phase of the inverter'''
        if phase not in [1, 2, 3]:
            raise ValueError("Invalid phase input, must be 1, 2 or 3")
    ...
    async def set_power_setpoint(self, setpoint: int):
        ''' Set the power setpoint of the battery system'''
    ...
    async def set_current_setpoint(self, setpoint: int):
        ''' Set the current setpoint of the battery system'''
