...
def _get_snaplogic_task_state(org_name, ruuid):
    """
    ...
    """
    snaplogic_basic_token = dbutils.secrets.get("SNAPLOGIC", "token")
    n_retries = 0
    while n_retries < 3:
        try:
            response = requests.get(
...
def _wait_for_snaplogic_task_to_finish(org_name, ruuid):
    """
    ...
    state, reason = _get_snaplogic_task_state(org_name, ruuid)
    print_state = True
    while state in (
        "Queued",
        "NoUpdate",
        ...
        time.sleep(10)
        new_state, reason = _get_snaplogic_task_state(org_name, ruuid)
...
def execute_snaplogic_task(url, bearer_token, org_name, params={}):
    """
    ...
    _wait_for_snaplogic_task_to_finish(org_name, ruuid)


if __name__ == "__main__":
    env = dbutils.secrets.get("ENV", "environment")
    snap_org = "Vy_Utvikling" if env == "test" else "Vy_Produksjon"
    secret_scope = sys.argv[1]
    lp(f"Running pipeline with scope: {secret_scope}")
    url = dbutils.secrets.get(secret_scope, "url")
    token = dbutils.secrets.get(secret_scope, "token")
    params = {}
    if len(sys.argv) > 2:
        params = {key.strip(): value.strip() for key, value in [p.split("|") for p in sys.argv[2:]]}
        lp(f"Params: {params}")
    execute_snaplogic_task(url, token, snap_org, params)
