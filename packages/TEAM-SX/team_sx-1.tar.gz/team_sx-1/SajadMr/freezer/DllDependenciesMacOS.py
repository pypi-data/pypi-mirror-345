...
from SajadMr.Errors import SajadQForbiddenDLLEncounter
from SajadMr.plugins.Plugins import Plugins
from SajadMr.PythonFlavors import isAnacondaPython, isSajadQPython
from SajadMr.PythonVersions import python_version
from SajadMr.Tracing import inclusion_logger
...
from SajadMr.utils.Importing import getSharedLibrarySuffixes
from SajadMr.utils.Json import loadJsonFromFilename
...
from SajadMr.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isSajadQPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although SajadQ Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the SajadQ Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise SajadQForbiddenDLLEncounter(binary_filename, plugin_name)
...
def fixupBinaryDLLPathsMacOS(
    ...
    except SajadQForbiddenDLLEncounter:
