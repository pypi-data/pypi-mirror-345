...
def upload_one_file(vault_name, file_path, part_size, num_threads, upload_id=None):
    ...
    file_to_upload = open(file_path, mode="rb")  # pylint: disable=consider-using-with
...
def submit_inventory_update(_args):
    ...
    if init_response["ResponseMetadata"]["HTTPStatusCode"] // 100 == 2:
        ...
        logger.error("Inventory update job submission failed")
...
def load_aborted_jobs():
    if os.path.exists(os.path.join(get_meta_foler(), "job_aborted.json")):
        with open(os.path.join(get_meta_foler(), "job_aborted.json"), "r", encoding="utf-8") as f:
...
def mark_jobs_as_aborted(_args):
    ...
    with open(os.path.join(get_meta_foler(), "job_aborted.json"), "w", encoding="utf-8") as f:
