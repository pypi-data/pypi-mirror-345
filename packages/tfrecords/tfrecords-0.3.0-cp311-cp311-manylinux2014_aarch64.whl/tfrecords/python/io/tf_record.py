...
from ...lib.tfrecords_lib import record as _pywrap_record_io
...
class TFRecordOptions(object):
  ...
  def __init__(self,
    ...
    self.compression_type =  self.get_compression_type_string(compression_type)
  ...
  def get_compression_type_string(cls, options):
    ...
    if not options:
      ...
      return cls.compression_type_map[options]
    elif options.upper() in TFRecordOptions.compression_type_map.values():
...
def tf_record_iterator(path, options=None,skip_bytes=0,with_share_memory=False):
  """An iterator that read the records from a TFRecords file.

  Args:
    path: The path to the TFRecords file.
    options: (optional) A TFRecordOptions object.
    with_share_memory: (optional) open file with share memrory
  ...
  return _pywrap_record_io.RecordIterator(path, compression_type,skip_bytes,with_share_memory)


def tf_record_random_reader(path, options=None,with_share_memory=False):
  ...
  """
  compression_type = TFRecordOptions.get_compression_type_string(options)
  return _pywrap_record_io.RandomRecordReader(path,compression_type,with_share_memory)
...
class TFRecordWriter(_pywrap_record_io.RecordWriter):
  ...
  def write_batch(self,record_list):
    super(TFRecordWriter, self).write_batch(record_list)
