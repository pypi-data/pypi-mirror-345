...
def _get_flattened_fields(msg_type_name: str, typestore) -> List[str]:
    ...
    flat_logger = logging.getLogger(f"{__name__}._get_flattened_fields_v5")
        ...
        for field_name, field_type_info in fields_definition:
            ...
            try:
                if isinstance(field_type_info, str):
                    ...
                    is_array = False
                elif isinstance(field_type_info, tuple) and len(field_type_info) == 2:
                    type_details = field_type_info[0]
                    type_category = field_type_info[1]

                    if type_category in [1, 2]: # Array or bounded sequence
                        is_array = True
                        # type_details = ((Nodetype, ElementTypeInfo), Size)
                        if isinstance(type_details, tuple) and len(type_details) == 2:
                            nodetype_enum, element_type_info = type_details
                            # element_type_info = ('typename', 0) for primitive or 'typename' string for complex
                            if isinstance(element_type_info, tuple) and len(element_type_info) > 0 and isinstance(element_type_info[0], str):
                                base_type_name = element_type_info[0] # Primitive array element
                            elif isinstance(element_type_info, str):
                                base_type_name = element_type_info # Complex array element
                            else:
                    ...
                    elif type_category == 0: # Plain type
                        # type_details = 'typename' (string)
                ...
                is_complex = base_type_name in typestore.types and base_type_name not in known_primitives
                ...
                if is_complex:
                    ...
                    # Only add if base_type_name was successfully determined
                    if not base_type_name.startswith('unknown'):
                         all_flat_fields.add(flat_name)
                    else:
                         # Log the fact that we couldn't resolve it but are skipping adding it
                         flat_logger.debug(f"Skipping field '{flat_name}' due to unresolved base type '{base_type_name}'.")
    ...
    if len(unique_field_names) < 70: # Slightly increased limit for debug log
