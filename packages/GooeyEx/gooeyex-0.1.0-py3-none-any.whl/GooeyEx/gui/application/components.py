            ...
            [
                ...
                [
                    ...
                    [
                        c.StaticText,
                        {
                            "label": present_time(self.props["timing"]),
                            "flag": wx.LEFT,
                            "wx_name": "timing",
...
def ProgressSpinner(props):
    return wsx(
        [
            c.Block,
            {"flag": wx.EXPAND, "show": props["show"]},
            [c.Gauge, {"flag": wx.EXPAND, "value": -1, "size": (-1, 4)}],
    ...
    return wsx(
        [
            ...
            [
                c.StaticText,
                {
                    "label": "Whoops! You have some errors which must be corrected",
...
def RSidebar(props):
    return wsx(
        [
            ...
            [
                c.StaticLine,
                {"style": wx.LI_VERTICAL, "flag": wx.EXPAND, "min_size": (1, -1)},
            ],
            *[
                [
        ...
        [
            ...
            *[
                [
                    c.NotebookItem,
                    {
                        "title": props["options"][i],
                        "selected": props["activeSelection"] == i,
                    },
                    [
                        TabbedConfigPage if props["tabbed_groups"] else ConfigPage,
                        {
...
class TitleText(Component):
    ...
    def render(self):
        return wsx(
            [
                c.StaticText,
                {**self.props, "label": self.props["label"], "ref": self.ref},
            ]
        )
...
def config(element, parent):
    return update(
        element, ConfigPage(parent, element["props"]["config"], {"contents": []})
    )
...
def tabbedconfig(element, parent):
    return update(
        element, TabbedConfigPage(parent, element["props"]["config"], {"contents": []})
    )
