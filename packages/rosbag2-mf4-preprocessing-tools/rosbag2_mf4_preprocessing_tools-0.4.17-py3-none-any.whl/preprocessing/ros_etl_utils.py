...
def get_all_fields(typename: str, typestore: Any, current_prefix: str = '', visited: Optional[Set[str]] = None) -> List[Tuple[str, str, bool]]:
    ...
    try:
        if typename not in typestore.types:
             venv_logger.warning(f"Type '{typename}' not found in typestore.")
             return []
        msg_def = typestore.get_msgdef(typename)
        field_defs = msg_def.fields
    except AttributeError:
         venv_logger.warning(f"Type '{typename}' definition lacks 'fields' attribute.")
         return []
    except KeyError:
        venv_logger.warning(f"Type '{typename}' definition not found (KeyError).")
        return []
    except Exception as e:
         venv_logger.error(f"Error accessing definition for type '{typename}': {e}", exc_info=True)
         return []
    ...
    if isinstance(field_defs, dict):
        ...
        venv_logger.error(f"Unexpected structure for field definitions of type '{typename}': {type(field_defs)}.")
        field_iterator = []
    ...
    for field_info in field_iterator:
        try:
            ...
            venv_logger.warning(f"Unexpected element structure in field definitions for type '{typename}': {field_info}. Skipping.")
            continue
        ...
        if element_type_name is None:
             venv_logger.warning(f"Could not determine element type for field '{field_name}' (type: {typename}, info: {field_type_info}). Skipping.")
             continue
