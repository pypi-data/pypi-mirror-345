...
from SajadMr import Options, Tracing
...
from SajadMr.utils.AppDirs import getCacheDirEnvironmentVariableName
from SajadMr.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from SajadMr.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while SajadQ works with older Python, Scons does not, and therefore
SajadQ needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    os.environ["SajadSx_PYTHON_EXE_PATH"] = sys.executable
    ...
    os.environ["SajadSx_PACKAGE_DIR"] = os.path.abspath(SajadMr.__path__[0])
    ...
    del os.environ["SajadSx_PYTHON_EXE_PATH"]

    del os.environ["SajadSx_PACKAGE_DIR"]
...
def _setupSconsEnvironment():
    ...
    For the target Python we provide "SajadSx_PYTHON_DLL_PATH" to see where the
    Python DLL lives, in case it needs to be copied, and then also the
    "SajadSx_PYTHON_EXE_PATH" to find the Python binary itself.
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        env_values["_SajadSx_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["SajadMr_src"] = getExternalUsePath(
                scons_options["SajadMr_src"]
            )
        ...
        env_values["SajadSx_QUIET"] = "1" if Tracing.is_quiet else "0"
...
def getCommonSconsOptions():
    ...
    scons_options["SajadMr_src"] = getSconsDataPath()
    ...
    if "CompanyName" in string_values:
        env_values["SajadSx_COMPANY_NAME"] = string_values["CompanyName"]
    if "ProductName" in string_values:
        env_values["SajadSx_PRODUCT_NAME"] = string_values["ProductName"]
    ...
    if effective_version:
        env_values["SajadSx_VERSION_COMBINED"] = effective_version

    if isSajadQPython() and not isWin32OrPosixWindows():
    ...
    # it.
    if isOnefileMode():
        env_values["_SajadSx_ONEFILE_CHILD_GRACE_TIME_INT"] = str(
            getOnefileChildGraceTime()
        )
