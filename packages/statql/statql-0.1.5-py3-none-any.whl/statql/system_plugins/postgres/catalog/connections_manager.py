...
class PostgresConnectionsManager:
    ...
    def _build_dsn(self, *, db_identifier: DatabaseIdentifier) -> str:
        ...
        password = SecretsManager.get_secret(secret_name=config.password_secret_name)
        return f"postgresql://{config.user}:{password}@{config.host}:{config.port}/{db_identifier.db_name}"
