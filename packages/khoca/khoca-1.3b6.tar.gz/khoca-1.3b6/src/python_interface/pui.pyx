...
cdef extern from "../planar_algebra/smith.h":
	cdef void set_pari_active_before(bool pari_active_before)
	cdef void pari_backup()
	cdef void pari_rollback()
...
cdef class PyComplexStack(object):
	...
	cdef bool pari_active_before

	def __init__(self, pari_active):
		r"""
		Initialize the wrapper and record if Pari is already running.
		"""
		self.pari_active_before = pari_active
	...
	def __enter__(self):
		...
		stack = NULL
		set_pari_active_before(self.pari_active_before)
		if self.pari_active_before:
			pari_backup()
		sig_on()
