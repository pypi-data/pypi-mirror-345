...
class PSS1830 :
    ...
    def enter_login_prompt(self) :
        ...
        for i in range(10) :
            ...
            if 'No Active ECs'.lower() in str(new_data).lower()  :
                raise Exception(f"No Active ECs found in {self.neip}")
    ...
    def loginToGmre(self) :
        ...
        if not self.gmrePrompt :
            ...
            raise Exception(f'catching gmre.prompt failure')
    ...
    def activateGmreRelease(self , release ) :
        ...
        if 'unsuccessfully configured' in result.lower() :
            raise Exception(f'Network Element {self.neip} activation failed. due to {result}')
