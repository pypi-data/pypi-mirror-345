...
def compare_fin_indicator_china(tickers,fsdates,facecolor='papayawhip',font_size='16px'):
    ...
    if fsdf is None:
        print("  #Error(compare_fin_summary_china): none record found for above tickers on",fsdates[0])
        print("  Reasons: either wrong tickers or dates, or blocked by data source, try later")
        return None        
    if len(fsdf) == 0:
        print("  #Warning(compare_fin_summary_china): zero recrod found for above tickers on",fsdates[0])
        print("  Reasons: either wrong tickers or dates, or blocked by data source, try later")
        return None 
