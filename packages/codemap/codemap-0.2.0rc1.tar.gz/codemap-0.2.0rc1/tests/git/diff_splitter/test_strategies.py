...
class TestSemanticSplitStrategy:
	...
	def test_split_unrelated_files(
		...
		"""Test splitting a diff where files are unrelated and dissimilar."""
		# Create a diff with unrelated files
		diff_content = (
			"diff --git a/file1.py b/file1.py\n"
			"--- a/file1.py\n+++ b/file1.py\n"
			"@@ +1 @@\n+ content1\n"
		...
		git_diff = GitDiff(files=["file1.py", "docs/guide.md"], content=diff_content, is_staged=True)

		# Expect two separate chunks
		chunks = semantic_strategy.split(git_diff)

		assert len(chunks) == 2
		assert chunks[0].files == ["file1.py"]
		assert chunks[1].files == ["docs/guide.md"]
		# Check descriptions generated by the side_effect mock
		assert chunks[0].description == "Desc for file1.py"
		assert chunks[1].description == "Desc for docs/guide.md"
