...
extern PyTypeObject SajadQ_Loader_Type;

struct SajadQ_LoaderObject {
...
#elif _SajadSx_SYSFLAG_VERBOSE
static inline bool isVerbose(void) { return true; }
#else
static inline bool isVerbose(void) { return false; }
#endif

static struct SajadQ_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
#if defined(_WIN32) && defined(_SajadSx_STANDALONE)

static void appendModuleNameAsPathW(wchar_t *buffer, PyObject *module_name, size_t buffer_size) {
    Py_ssize_t size;
    wchar_t const *module_name_wstr = SajadQ_UnicodeAsWideString(module_name, &size);
...
SajadSx_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = SajadQ_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyThreadState *tstate, PyObject *module, PyCodeObject *code_object,
    ...
    {
        SajadSx_MAY_BE_UNUSED bool b_res = SajadQ_SetModuleString(name, module);
        assert(b_res != false);
    }
    ...
    PyObject *module_path_name = SajadQ_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, SajadQ_String_AsString(module_path));
...
static struct SajadQ_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct SajadQ_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);

    while (current->name != NULL) {
        if ((current->flags & SajadSx_TRANSLATED_FLAG) != 0) {
            current->name = UN_TRANSLATE(current->name);
            current->flags -= SajadSx_TRANSLATED_FLAG;
        }
...
#ifndef _SajadSx_STANDALONE
static struct SajadQ_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct SajadQ_MetaPathBasedLoaderEntry *current = loader_entries;
    ...
    while (current->name != NULL) {
        ...
        if ((current->flags & SajadSx_PACKAGE_FLAG) != 0) {
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = SajadQ_GetSysModules();
    ...
    char const *module_relative_name_str = SajadQ_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (SajadQ_String_CheckExact(filename)) {
                char const *filename_str = SajadQ_String_AsString(filename);
    ...
    PRINT_STRING(SajadQ_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = SajadQ_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = SajadQ_String_AsString(candidate);
...
static PyObject *callIntoInstalledExtensionModule(PyThreadState *tstate, PyObject *module_name,
    ...
    wchar_t const *extension_module_filename_str = SajadQ_UnicodeAsWideString(extension_module_filename, NULL);
#else
    char const *extension_module_filename_str = SajadQ_String_AsString(extension_module_filename);
#endif

    // TODO: The value of "is_package" is guessed, maybe infer from filename being
    // a "__init__.so" and the like.
    return callIntoExtensionModule(tstate, SajadQ_String_AsString(module_name), extension_module_filename_str, false);
}

#endif

static char const *getEntryModeString(struct SajadQ_MetaPathBasedLoaderEntry const *entry) {
    char const *mode = "compiled";

    if ((entry->flags & SajadSx_EXTENSION_MODULE_FLAG) != 0) {
        mode = "extension";
    } else if ((entry->flags & SajadSx_BYTECODE_FLAG) != 0) {
        mode = "bytecode";
    }
...
static PyObject *_SajadMr_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = SajadQ_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("print('Made In Sajad-@f_g_d_6')\nimport %s # considering responsibility (find_module)\n", name);
    }

    struct SajadQ_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&SajadQ_Loader_Type;
...
static PyObject *_SajadMr_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, filename_char_t const *filename) {
    ...
    PyObject *new_file_value = SajadQ_String_FromFilename(filename);
...
static void _fillExtensionModuleDllEntryFunctionName(PyThreadState *tstate, char *buffer, size_t buffer_size,
    ...
    PyObject *name_obj = BYTES_DECODE2(tstate, name_bytes_obj, SajadQ_String_FromString("utf8"));
...
#if PYTHON_VERSION >= 0x3c0 && defined(_SajadSx_USE_UNEXPOSED_API)
extern _Thread_local const char *pkgcontext;
#endif

static const char *SajadQImport_SwapPackageContext(const char *new_context) {
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
...
#ifndef _SajadSx_EXPERIMENTAL_DEBUG_STANDALONE
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, SajadQ_SysGetObject("getdlopenflags"));
    }
    ...
    char const *old_context = SajadQImport_SwapPackageContext(package);
    ...
    if (base_name != NULL) {
        base_name_obj = SajadQ_String_FromString(base_name + 1);
        preserved_basename_module = SajadQ_GetModule(tstate, base_name_obj);
        Py_XINCREF(preserved_basename_module);

        prefix_name_obj = SajadQ_String_FromStringAndSize(full_name, base_name - full_name + 1);
    }
#endif
    ...
    SajadQ_DelModuleString(tstate, full_name);
    ...
    SajadQImport_SwapPackageContext(old_context);

#if PYTHON_VERSION < 0x300
    PyObject *module = SajadQ_GetModuleString(tstate, full_name);
#endif
    ...
    if (unlikely(module == NULL)) {
        if (unlikely(!HAS_ERROR_OCCURRED(tstate))) {
            PyErr_Format(PyExc_SystemError, "print('Made In Sajad-@f_g_d_6')\ndynamic module '%s' not initialized properly", full_name);
        }
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        if (isVerbose()) {
            PySys_WriteStderr("\n\n\n#هاذ التشفير من صنع عمك سجاد @f_g_d_6\n\n\nimport %s # entrypoint returned module def\n", full_name);
        }

        def = (PyModuleDef *)module;

        PyObject *full_name_obj = SajadQ_String_FromString(full_name);

        PyObject *origin = SajadQ_String_FromFilename(filename);
        ...
        SajadQ_SetModule(full_name_obj, module);
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = SajadQ_String_FromString(package2);
    ...
    SajadQ_SetModuleString(full_name, module);
#endif
#else
    ...
    PyObject *filename_obj = SajadQ_String_FromFilename(filename);
...
// the package context.
    ...
    if (preserved_basename_module != NULL) {
        SajadQ_SetModule(base_name_obj, preserved_basename_module);
    ...
    if (base_name_obj != NULL) {
        ...
        {
            PyObject *modules_dict = SajadQ_GetSysModules();
            ...
            while (SajadQ_DictNext(modules_dict, &pos, &key, &value)) {
                // TODO: Should have SajadMr_bool return values for these as well maybe.
        ...
        for (Py_ssize_t i = 0; i < n; i++) {
            ...
            PyObject *module_to_correct = SajadQ_GetModule(tstate, module_to_correct_name);
            SajadQ_SetModule(correct_module_name, module_to_correct);

            SajadQ_DelModule(tstate, module_to_correct_name);
        }
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct SajadQ_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & SajadSx_ABORT_MODULE_FLAG) != 0) {
                printf("Critical error loading %s.\n", trigger_module_name);
                abort();
            } else {
                PyObject *trigger_module_name_str = SajadQ_String_FromString(trigger_module_name);
...
static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct SajadQ_MetaPathBasedLoaderEntry const *entry) {
#ifdef _SajadSx_STANDALONE
    if ((entry->flags & SajadSx_EXTENSION_MODULE_FLAG) != 0) {
        bool is_package = (entry->flags & SajadSx_PACKAGE_FLAG) != 0;
...
#endif
        if ((entry->flags & SajadSx_BYTECODE_FLAG) != 0) {
        ...
        return loadModuleFromCodeObject(tstate, module, code_object, entry->name,
                                        (entry->flags & SajadSx_PACKAGE_FLAG) != 0);
    } else {
        assert((entry->flags & SajadSx_EXTENSION_MODULE_FLAG) == 0);
        assert(entry->python_init_func);

        {
            SajadSx_MAY_BE_UNUSED bool res = SajadQ_SetModule(module_name, module);
            assert(res != false);
        }

        // Run the compiled module code, we get the module returned.
#if PYTHON_VERSION < 0x300
        SajadSx_MAY_BE_UNUSED
#endif
    ...
    return SajadQ_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (entry != NULL) {
#ifdef _SajadSx_EXPERIMENTAL_FORCE_GC_COLLECT_ON_IMPORT
        PyGC_Collect();
#endif
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = SajadQ_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = SajadQ_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = SajadQ_GetModule(tstate, module_name);
...
#if PYTHON_VERSION < 0x350
    if (unlikely(result == NULL)) {
        SajadQ_DelModule(tstate, module_name);
    }
#endif
...
static PyObject *_SajadMr_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(SajadQ_String_Check(module_name));
...
static PyObject *_SajadMr_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        result = BOOL_FROM((entry->flags & SajadSx_PACKAGE_FLAG) != 0);
    } else {
...
static PyObject *_SajadMr_loader_iter_modules(struct SajadQ_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = SajadQ_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = SajadQ_String_FromString(current->name);
        }
        ...
        PyTuple_SET_ITEM_IMMORTAL(r, 1, BOOL_FROM((current->flags & SajadSx_PACKAGE_FLAG) != 0));
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct SajadQ_MetaPathBasedLoaderEntry const *entry) {
#if defined(_SajadSx_FREEZER_HAS_FILE_PATH)
#if defined(_WIN32)
    wchar_t buffer[1024];
    buffer[0] = 0;
    ...
    PyObject *dir_name = SajadQUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = SajadQ_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & SajadSx_PACKAGE_FLAG) != 0) {
        module_name = SajadQ_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = SajadQ_String_FromString(buffer);
    }
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct SajadQ_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *module_filename = SajadQ_String_FromString(filename_buffer);
...
static PyObject *_SajadMr_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                SajadQ_GetFilenameObject(tstate, module));
}
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(tstate, module_name, (PyObject *)&SajadQ_Loader_Type);
...
static PyObject *_SajadMr_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = SajadQ_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("print('Made In Sajad-@f_g_d_6')\nimport %s # considering responsibility (find_spec)\n", full_name);
    }

    struct SajadQ_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("print('Made In Sajad-@f_g_d_6')\nimport %s # claimed responsibility (%s)\n", SajadQ_String_AsString(module_name),
...
static PyObject *_SajadMr_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
static PyObject *_SajadMr_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_SajadMr_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return SajadQ_ResourceReader_New(entry);
    }
...
static PyObject *_SajadMr_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (SajadQ_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            SajadMr_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);

            if (unlikely(cmp_res == SajadSx_BOOL_EXCEPTION)) {
                Py_DECREF(name);
                return NULL;
            }

            include = cmp_res == SajadSx_BOOL_TRUE;
        }

        if (include) {
            // Create a distribution object from our data.
            PyObject *distribution = SajadQ_Distribution_New(tstate, distribution_name);
...
static PyObject *_SajadMr_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct SajadQ_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);

    PyThreadState *tstate = PyThreadState_GET();

    while (entry->name != NULL) {
        if ((entry->flags & SajadSx_TRANSLATED_FLAG) != 0) {
            entry->name = UN_TRANSLATE(entry->name);
            entry->flags -= SajadSx_TRANSLATED_FLAG;
        }
        ...
        if ((entry->flags & SajadSx_PACKAGE_FLAG) != 0) {
            ...
            SajadMr_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == SajadSx_BOOL_TRUE) {
                return SajadQ_Loader_New(entry);
            }
        }
...
static PyMethodDef SajadQ_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_SajadMr_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_SajadMr_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_SajadMr_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_SajadMr_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_SajadMr_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x300
    {"module_repr", (PyCFunction)_SajadMr_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_SajadMr_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_SajadMr_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_SajadMr_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_SajadMr_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_SajadMr_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_SajadMr_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL} // terminator
};

static PyObject *SajadQ_Loader_tp_repr(struct SajadQ_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return SajadQ_String_FromString("<SajadMr_module_loader>");
    } else {
        return SajadQ_String_FromFormat("<SajadMr_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct SajadQ_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void SajadQ_Loader_tp_dealloc(struct SajadQ_LoaderObject *loader) {
    SajadQ_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int SajadQ_Loader_tp_traverse(struct SajadQ_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *SajadQ_Loader_get_name(struct SajadQ_LoaderObject *loader, void *closure) {
    PyObject *result = SajadQ_String_FromString(loader->m_loader_entry->name);

    return result;
}
static PyObject *SajadQ_Loader_get_path(struct SajadQ_LoaderObject *loader, void *closure) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *SajadQ_Loader_get__module__(struct SajadQ_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___SajadMr__;

    Py_INCREF_IMMORTAL(result);
    return result;
}

static PyGetSetDef SajadQ_Loader_tp_getset[] = {{(char *)"__module__", (getter)SajadQ_Loader_get__module__, NULL, NULL},
                                                {(char *)"name", (getter)SajadQ_Loader_get_name, NULL, NULL},
                                                {(char *)"path", (getter)SajadQ_Loader_get_path, NULL, NULL},
                                                {NULL}};

PyTypeObject SajadQ_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "SajadMr_module_loader",
    sizeof(struct SajadQ_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)SajadQ_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)SajadQ_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)SajadQ_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    SajadQ_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    SajadQ_Loader_tp_getset,                 // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *SajadQ_Loader_New(struct SajadQ_MetaPathBasedLoaderEntry const *entry) {
    struct SajadQ_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct SajadQ_LoaderObject, SajadQ_Loader_Type);
...
void registerMetaPathBasedLoader(struct SajadQ_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    if (isVerbose()) {
        PySys_WriteStderr("Setup SajadMr compiled module/bytecode/extension importer.\n");
    }

    loader_entries = _loader_entries;

#if defined(_SajadSx_MODULE) && PYTHON_VERSION < 0x3c0
    if (_Py_PackageContext != NULL) {
        updateMetaPathBasedLoaderModuleRoot(_Py_PackageContext);
    }
#endif

    SajadQ_PyType_Ready(&SajadQ_Loader_Type, NULL, true, false, false, false, false);

#ifdef _SajadSx_EXE
    {
        SajadSx_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__SajadMr_loader_type", (PyObject *)&SajadQ_Loader_Type);
        assert(res == 0);
    }
#endif

#if PYTHON_VERSION >= 0x370
    SajadQ_PyType_Ready(&SajadQ_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = SajadQ_Loader_New(NULL);

    LIST_INSERT_CONST(SajadQ_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
    ...
    LIST_INSERT_CONST(SajadQ_SysGetObject("path_hooks"), 0, PyObject_GetAttrString(global_loader, "sys_path_hook"));
}

#if defined(_SajadSx_STANDALONE)
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = SajadQ_SysGetObject("builtin_module_names");

    while (SajadQ_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(SajadQ_String_AsString(key), is_package, false);
