...
PROPERTY_MAPPING = {
    ...
    "user_linkedin_url": "linkedinbio",  # or "linkedin_url" if you prefer
    # etc.
}
...
async def update_crm_contact_record_function(
    ...
    tags: Optional[List[str]] = None,  # optional tags to store
    tool_config: Optional[List[Dict]] = None
) -> Dict[str, Any]:
    """
    ...
    # 2) Convert contact_values to valid HubSpot properties
    ...
    for k, v in contact_values.items():
        ...
        if k in PROPERTY_MAPPING:
            ...
            # Only set if the mapped_name is a valid contact property
            if mapped_name in valid_props:
                hubspot_props[mapped_name] = v

    # If user provided tags, store them in "my_tags" (only if it's valid in your portal)
    if tags:
        if "my_tags" in valid_props:
            hubspot_props["my_tags"] = ";".join(tags)

    # 3) If is_update=True, try to find an existing contact; else create
    if is_update:
        ...
        if found_contact_id:
            ...
            return await _create_new_hubspot_contact(hubspot_props, headers)
    else:
...
async def _create_new_hubspot_contact(
    ...
    (We simply do NOT include 'marketingContacts' or set marketingContact: false.)
    """
    create_url = "https://api.hubapi.com/crm/v3/objects/contacts"

    # By default, newly created contacts in some portals might be marketing contacts automatically.
    # If you need to explicitly set them as non-marketing, you can do so by:
    # {
    #   "marketingContacts": {
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(create_url, headers=headers, json=create_payload) as resp:
            ...
            if resp.status not in (200, 201):
                raise RuntimeError(f"Error creating new contact: status {resp.status}, response: {result}")
            return result


# Example helper if you haven't already defined it:
async def _fetch_all_contact_properties(headers: Dict[str, str]) -> List[str]:
    """
    Fetch all contact property names via the HubSpot v3 properties API.
    Filter out any read-only or irrelevant ones as desired.
    """
