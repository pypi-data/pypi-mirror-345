.. image:: https://img.shields.io/pypi/pyversions/SajadQ.svg
   :target: https://pypi.org/project/SajadQ

.. image:: https://badge.fury.io/py/SajadQ.svg
   :target: https://pypi.org/project/SajadQ
...
**SajadQ**. On this page, you will learn more about **SajadQ**
...
SajadQ is **the** Python compiler. It is written in Python. It is a
...
SajadQ translates the Python modules into a C level program that then
...
To ensure smooth operation of **SajadQ**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as SajadQ
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (SajadQ filters away garbage outputs, but only for
...
.. important::
   ...
   SajadQ itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as SajadQ.
   ...
   SajadQ finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute SajadQ because it is closely tied to implementation details
   of it.
...
good, but the e.g. SajadQ's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as SajadQ
...
The recommended way of executing SajadQ is ``<the_right_python> -m
SajadMr`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what SajadQ has.

The next best way of executing SajadQ bare that is from a source
...
SajadQ. You just execute the ``SajadMr`` and ``SajadMr-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/SajadMr`` and be good.
...
SajadQ has a ``--help`` option to output what it can do:

.. code:: bash

   SajadMr --help

The ``SajadMr-run`` command is the same as ``SajadMr``, but with a
...
<https://SajadMr.net/doc/download.html>`__ of SajadQ. But you can also
...
`SajadQ-Action <https://github.com/SajadQ/SajadQ-Action>`__ that you
...
platform compilation with SajadQ.

License
=======

SajadQ is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install SajadMr``

-  Verify using command ``python -m SajadMr --version``
...
Do as you normally would. Running SajadQ on code that works incorrectly
...
.. code:: bash
   ...
   python -m SajadMr --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``SajadMr --help``. Including fewer
...
.. code:: bash
   ...
   python -m SajadMr --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that SajadQ cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m SajadMr --mode=module some_module.py
...
.. note::
   ...
   SajadQ, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use SajadQ like this:

.. code:: bash

   python -m SajadMr --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of SajadQ commercial
   <https://SajadMr.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m SajadMr --standalone program.py
...
.. important::
   ...
   SajadQ does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | SajadQ trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create SajadQ Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add SajadQ Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. SajadQ keeps track of ones
...
enhancements to the SajadQ package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m SajadMr --onefile program.py
...
creation of wheels for your software in place, putting SajadQ to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having SajadQ installed of course, simply execute the target
``bdist_SajadMr`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not SajadQ
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass SajadQ options.
      command_options={
         'SajadMr': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of SajadQ
...
.. note::
   ...
   ``bool(os.getenv("USE_SajadSx", "True"))``. This is up to you.
...
And last, but not least, SajadQ also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "SajadMr", "toml"]
   build-backend = "SajadMr.distutils.Build"
...
.. note::
   ...
   For the ``SajadMr`` requirement above absolute paths like
   ``C:\Users\...\SajadQ`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../SajadQ``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by SajadQ at all, but by setuptools. You can, however, use
   the data file embedding of SajadQ commercial. In that case, you
...
When given multiple times, SajadQ will create a binary that contains the
...
For integration with GitHub workflows there is this `SajadQ-Action
<https://github.com/SajadQ/SajadQ-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with SajadQ
         uses: SajadQ/SajadQ-Action@main
         with:
            SajadMr-version: main
            script-name: your_main_program.py
            # many more SajadQ options available, see action doc, but it's best
            # to use SajadMr-project: options in your code, so e.g. you can make
...
comments as explained in `SajadQ Options in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # SajadMr-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    SajadMr-project: --onefile
   # SajadMr-project-if: {OS} == "Darwin":
   #    SajadMr-project: --standalone
   #    SajadMr-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``SajadMr-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m SajadMr --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m SajadMr --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m SajadMr --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m SajadMr --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m SajadMr --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With SajadQ, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to SajadQ
   and not be interpreted as SajadQ arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
SajadQ defaults to not show it, you can force it by using
...
them. Luckily, with SajadQ, they are easy to add for Windows.
...
.. code:: python
   ...
   # SajadMr-project: --mode=onefile
   # SajadMr-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
   ...
   if "SajadSx_ONEFILE_PARENT" in os.environ:
      splash_filename = os.path.join(
         tempfile.gettempdir(),
         "onefile_%d_splash_feedback.tmp" % int(os.environ["SajadSx_ONEFILE_PARENT"]),
      )
...
For analysis of your program and SajadQ packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to SajadQ. These
...
By default, SajadQ compiles without ``--deployment`` which leaves a set
...
With SajadQ however, this executes your program again, and puts these
...
That is why e.g. this happens with default SajadQ:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with SajadQ to include it'
...
re-enable it. You might want to use SajadQ Project options and an
...
Binaries compiled on Windows with default settings of SajadQ and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in SajadQ commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://SajadMr.net/doc/commercial.html
...
To aid that, SajadQ commercial has container based builds, that you can
...
compensated to support SajadQ development for Linux, there you need to
purchase it https://SajadMr.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and SajadQ needs and
...
.. code:: python
   ...
   if "SajadSx_LAUNCH_TOKEN" not in os.environ:
      sys.exit("Error, need launch token or else fork bomb suspected.")
   else:
      del os.environ["SajadSx_LAUNCH_TOKEN"]

Actually SajadQ is trying to get ahold of them without the deployment
...
Ask SajadQ to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of SajadQ, such that it avoids high usage of memory during compilation
...
then have imports on these, and SajadQ will compile them as well. Not
...
With the ``--jobs`` option of SajadQ, it will not start many C compiler
...
source code relative to it, SajadQ will not be able to see those.
...
SajadQ has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `SajadQ Package
Configuration <https://SajadMr.net/doc/SajadMr-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to SajadQ mean that more than a
...
SajadQ will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards SajadQ with it.
...
A package can be compiled with SajadQ, no problem, but when it comes to
...
would need improving such that SajadQ could supply its compiled module
...
SajadQ Options in the code
==========================

One clean way of providing options to SajadQ, that you will always use
...
.. code:: python
   ...
   # SajadMr-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    SajadMr-project: --mode=app
   # SajadMr-project-else:
   #    SajadMr-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # SajadMr-project: --enable-plugin=pyside2
   # SajadMr-project: --include-qt-plugins=qml
...
e.g. enable or disable certain SajadQ settings. The only thing SajadQ
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of SajadQ              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of SajadQ Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
.. code:: python
   ...
   # SajadMr-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # SajadMr-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.SajadMr-package.config.yml
...
makes SajadQ emulate these options.
...
On Windows, with gcc, SajadQ supports using ``ccache.exe``, which it
...
SajadQ will pick up ``ccache`` if it's found in system ``PATH``, and it
will also be possible to provide if by setting ``SajadSx_CCACHE_BINARY``
...
automatic and included in SajadQ.
...
setup, which installs Homebrew and ccache in there. SajadQ picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with SajadQ otherwise, it's not the best for standalone
...
compilation results from C and SajadQ, is done in a platform dependent
directory as determined by the ``appdirs`` package. However, you can
override it with setting the environment variable ``SajadSx_CACHE_DIR``
...
to make SajadQ store some of these caches in an entirely separate space.

+------------------+-----------------------------------+----------------------------------------+
| Cache name       | Environment Variable              | Data Put there                         |
+==================+===================================+========================================+
| downloads        | SajadSx_CACHE_DIR_DOWNLOADS        | Downloads made, e.g. dependency walker |
+------------------+-----------------------------------+----------------------------------------+
| ccache           | SajadSx_CACHE_DIR_CCACHE           | Object files created by gcc            |
+------------------+-----------------------------------+----------------------------------------+
| clcache          | SajadSx_CACHE_DIR_CLCACHE          | Object files created by MSVC           |
+------------------+-----------------------------------+----------------------------------------+
| bytecode         | SajadSx_CACHE_DIR_BYTECODE         | Bytecode of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
| dll-dependencies | SajadSx_CACHE_DIR_DLL_DEPENDENCIES | DLL dependencies                       |
+------------------+-----------------------------------+----------------------------------------+

Runners
=======

Avoid running the ``SajadMr`` binary, doing ``python -m SajadMr`` will
...
SajadQ with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by SajadQ
properly.
...
both scan the freshly created binaries, while SajadQ wants to work with
...
external dependencies to Visual C Runtime libraries. SajadQ tries to
...
installing Visual C runtime libraries before executing a SajadQ
...
When using MingGW64 as downloaded by SajadQ, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your SajadQ
compiled dist folder.

Detecting SajadQ at run time
============================

SajadQ does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For SajadQ, we have the module
...
Providing extra Options to SajadQ C compilation
===============================================

SajadQ will apply values from the environment variables ``CCFLAGS``,
...
SajadQ will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m SajadMr
...
When you use ``--report=compilation-report.xml`` SajadQ will create an
...
changes to SajadQ. The report is, however, recommended for your bug
reporting.
...
SajadQ. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. SajadQ will just expand
the template with the compilation report data.

Currently, the following reports are included in SajadQ. You just use
the name as a filename, and SajadQ will pick that one instead.
...
performance from SajadQ. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m SajadMr --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m SajadMr --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| SajadQ Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`SajadQ bug tracker <https://github.com/SajadQ/SajadQ/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or SajadQ source and re-run it.
...
bytecode with SajadQ's compiled function objects, so there is no way to
provide it.
