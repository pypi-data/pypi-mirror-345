...
relap_labels = {
    ...
    "emass": "Estimate of mass error (kg)",
    ...
    "rktpow3d": "Total reactor power (W)",
    ...
    "systmb": "Total mass of boron (kg) from state equation",
    "systmc": "Total mass of steam, water, and noncondensable (kg) from continuity equation",
    "systmf": "Total mass of liquid (kg) from state equation",
    "systmfc": "Total mass of liquid (kg) from continuity equation",
    "systmg": "Total mass of vapor (steam plus noncondensable) (kg) from state equation",
    "systmgc": "Total mass of vapor (steam plus noncondensable) (kg) from continuity equation",
    "systmn": "Total mass of noncondensables (kg) from state equation",
    "systmnc": "Total mass of noncondensables (kg) from continuity equation",
    "systms": "Total mass of steam, water, and noncondensable (kg) from state equation",
    "testda": "An array, testda, of one hundred quantities",
    "time": "Time (s)",
    "timeof": "Time of trip (s)",
    "tmass": "Total mass of water, steam, and noncondensables (kg)",
    "tothgen": "Total hydrogen generation from metal-water reactions (kg)",
    "vnmaxvme": "volume having the maximum volume mass error",
    "acpgtg": "Accumulator vapor specific heat (J/kg•K)",
    "acpnit": "Accumulator noncondensable specific heat (J/kg•K)",
    "acqtank": "Total energy transport in the accumulator (W)",
...
def message(message: str, typ='WARNING'):
    ...
    :typ: the type ERROR or WARNING or INFO
    """
    print(36*'-'+typ+(44-len(typ))*'-')
...
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def read_case(restart: str, cases=None):
    """
    :restart: [str] case name without extension
    :cases: (optional) [str] name of the case folder or a [list] with cases
    :returns: a DataFrame with the case data
    :USAGE: your_case = read_case('restart', cases)

    """
    if isinstance(cases, str):
        dat_file = './'+cases+'/' + restart + '.dat'
        if not os.path.isfile(dat_file):
           message('File '+ dat_file +' does not exist.', 'ERROR--read_case')
        cases_df = pd.read_csv(dat_file)
    else :
        cas = []
        for item in cases:
            dat_file = './'+item+'/' + restart + '.dat'
            if not os.path.isfile(dat_file):
               message('File '+ dat_file +' does not exist.', 'ERROR--read_case')
            case = pd.read_csv(dat_file)
            cas.append(case)
        cases_df = pd.concat(cas, keys=cases)
    return cases_df
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def read_variables(input_variables_file):
    ...
    if type(input_variables_file) == str:
        if not os.path.isfile(input_variables_file):
           message('File '+ input_variables_file +' does not exist.', 'ERROR--read_variables')
        variables = pd.read_csv(input_variables_file,sep='\s+', comment='%')
    else :
        variables = input_variables_file
    general_options = ['no', 'top_left', 'top_right', 'mid_left', 'mid_right', 'bottom_left', 'bottom_right']
    if 'general' in variables.columns:
        for item in variables['general']:
            if item not in general_options:
                message(item + ' is not a possible option for the general column in variables', 'WARNING--read_variables')
    columns = ['alphanum', 'numeric', 'star', 'var_tag', 'figure_tag', 'unit',
       'timei', 'timee', 'ymin', 'ymax', 'general']
    cols = []
    for i, item in enumerate(variables.columns):
        if item == 'experiment_tag':
            warning='You used and old label "'+ item + '" but this is no longer used. '
            warning2='It has been automatically updated to the new tag "var_tag"'
            message(warning + warning2, 'WARNING--read_variables')
            item = 'var_tag'
        elif item not in columns:
            warning='You labelled a column "'+ item + '" but this is not a possible column label.\n'
            warning3 ='There might be unexpected behaviour'
            message(warning + warning3, 'WARNING--read_variables')
        cols.append(item)
...
def make_strip(input_variables_file, strip_filename: str, form="fmtout"):
    ...
    if form != 'csv' and form != 'fmtout' :
        message('argument 1 should be either csv or fmtout', 'WARNING--make_strip')
        if relap_scdap():
            form = 'csv'
            message('Argument 1 is set to csv', 'WARNING--make_strip')
        else:
            form = 'fmtout'
            message('Argument 1 is set to fmtout', 'WARNING--make_strip')
    ...
    with open(strip_filename, 'w+') as st:
        ...
        for index, plot in variables.iterrows():
            ...
            st.write(str(plot['alphanum']) + ' ')
            st.write(str(plot['numeric']) + ' ')
            # for word in plot[0:2]:
                # st.write(str(word) + ' ')
            st.write('\n')
        st.write('. \n')
        st.close()
        message('Your strip file ' + strip_filename + ' has been created', 'INFO--make_strip')
    return variables
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
...
def run_strip(strip_file: str, restart: str, case_folder=''):
    ...
    if case_folder != '':
       case_folder = './' + case_folder + '/'
    if not os.path.isfile(case_folder + restart + '.r'):
       message('File '+ case_folder + restart + '.r does not exist.', 'ERROR--run_strip')
    # exe = relapwin_loc + '/' + relap_exe
    if path_to_exe is None:
        message('You cannot use this function without a valid relap executable', 'ERROR--run_strip')
    exe = path_to_exe()
    if relap_scdap() :
        arg1 = ' -i ' + strip_file
        ...
        arg4 = ''
        message("relap_scdap() is used", 'INFO--run_strip')
    else:
        arg1 = ' -i ' + strip_file
    ...
    try :
        ...
        pass
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def run_case(case_name: str, restart: bool=False, background: bool=False):
    """ Runs a case

    :case_name: Full path to the input file
    :restart: Boolean, True if this is a restart case, False if it is a normal case
    :background: Boolean, to run the case in the background
    :returns: runs the case

    """
    if path_to_exe is None:
        message('You cannot use this function without a valid relap executable', 'ERROR--run_case')
    exe = path_to_exe()
    inp = ' -i ' + case_name + '.i'
    out = ' -o ' + case_name + '.o' 
    rest = ' -r ' + case_name + '.r' 
    if relap_scdap() :
        water_tables = ''
        message("relap_scdap() is used", 'INFO--run_case')
    else:
        water_tables = ' -Z ' + relapwin_loc() + '/tpfh2onew'
    if background:
        end='> '+ case_name + '.out  &'
        message('case '+ case_name + ' running on the background, stdout saved in "'\
                + case_name + '.out"', 'INFO--run_case')
    else :
        end=''
    try :
        os.remove(case_name + '.o')
    except OSError :
        pass
    if not restart:
        try :
            os.remove(case_name + '.r')
        except OSError :
            pass
    cmd = exe + inp + out + rest + water_tables + end
    with Popen(cmd, stdout=PIPE, shell=True, bufsize=1, universal_newlines=True) as p:
        for line in p.stdout:
            print(line, end='')
    pass
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def extract_data(input_variables_file: str, restart: str, case_folder=''):
    ...
    if relap_scdap():
        ...
        noms_codi = ['time']
        noms_codi.extend(variables[0].loc[:, 'var_tag'])
        noms_descrip = ['Time']
        ...
        noms_codi = ['time']
        noms_codi.extend(variables.loc[:,'var_tag'])
        noms_descrip = ['Time']
    ...
    for item in case_paths:
        ...
        case.to_csv('./'+item+'/' + restart + '.dat', float_format='%1.4e',
                       index=False)
...
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_var(variable: str, variables_df: pd.DataFrame, cases_df: pd.DataFrame,
             case_folder = list(), y_limits: tuple=(0,0), x_limits: tuple=(0,0),
             dataExp: pd.DataFrame = None , label: str=''):
    ...
    :variables_df: Dataframe containing the variables and properties
                         of each variable
    :cases_df: DataFrame containing the data cases
    :case_folder: (optional) name of the case folder [str] or a [list] with cases, 
                  if not entered it takes all cases in cases_df
    :y_limits: (optional) limits in the y axis
    :x_limits: (optional) limits in the x axis
    :dataExp: (optional) name of the DataFrame containing the experimental data
    ...
    """
    if variable not in list(variables_df['var_tag']):
        message(variable + ' is not in the list of variables and will not be plotted',
                'WARNING--plot_var')
        return
    is_multiindex = isinstance(cases_df.index, pd.MultiIndex)
    is_one_case = not is_multiindex or type(case_folder) == str 
    if case_folder == list() and is_multiindex :
        case_folder = list(cases_df.index.levels[0]) 
    print('making plot for', variable)
    # plt.figure(variable)
    fig, ax = plt.subplots()
    i = variables_df[variables_df['var_tag'] == variable].index.values[0]
    tag = variables_df['var_tag'][i] 
    if type(dataExp) == str :
        message('you first need to load the experimental data to a DataFrame','ERROR--plot_var')
    if dataExp is not None :
        tim_column = dataExp.columns[0]
        if tag in dataExp.columns :
            ax.plot(dataExp[tim_column], dataExp[tag],
                     linewidth=0.75,color='k',label='Experimental')
    if is_one_case:
        if is_multiindex:
            ax.plot(cases_df.loc[case_folder]['time'], cases_df.loc[case_folder][tag], linewidth=0.75)
        else :
            ax.plot(cases_df['time'], cases_df[tag], linewidth=0.75)
    else :
        for j in case_folder :
            ax.plot(cases_df.loc[j]['time'], cases_df.loc[j][tag], linewidth=0.75, label=j)
    # plt.ylabel(variable.replace('_',' ') + ' (' + variables_df['unit'][i] + ')')
    # if label == '' : label = relap_labels.get(variables_df['alphanum'][i])
    if label == '' : 
        label = variables_df['figure_tag'][i].replace('_', ' ') + ' (' + variables_df['unit'][i] + ')'
    if x_limits[0] == 0 and x_limits[1] == 0 :
        if 'timei' in variables_df.columns :
            ax.set_xlim(float(variables_df['timei'][i]), float(variables_df['timee'][i]))
    else :
        ax.set_xlim(x_limits)
    # ax.set(ylabel =label, xlabel ='Time (s)', title = variable.replace('_',' '))
    ax.set(ylabel =label, xlabel ='Time (s)')
    if y_limits[0] == 0 and y_limits[1] == 0 :
        try :
    ...
    return fig, ax 
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_all(variables_df: pd.DataFrame, cases_df: pd.DataFrame,
             case_folder= list(), dataExp: pd.DataFrame = None):

    """
    :variables_df: DataFrame containing the requested variables
    :cases_df: DataFrame containing all results
    :case_folder: (optional) name of the case folder [str] or a [list] with cases, 
                  if not entered it takes all cases in cases_df
    :dataExp: (optional) name of the DataFrame containing the experimental data

    :returns: Plots all items extracted for the list of cases

    """
    for item in variables_df['var_tag'] :
        plot_var(item, variables_df, cases_df, case_folder=case_folder, dataExp=dataExp)
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot(alpha: str, numeric: str, restart: str, case_folder=''):
    ...
    if type(case_folder) == str:
        case_folder = [case_folder]
    cases = []
    for item in case_folder:
        if not os.path.isfile('./' + item + '/' + restart + '.r'):
            message('File ./'+ item + '/' + restart + '.r does not exist.', 'ERROR--plot')
        try :
            os.remove('./' + item + '/' + 'stripf')
        except OSError :
            pass
        if relap_scdap():
            if item != '':
                run_strip('../' + 'dummy_strip.i', restart, item)
            else:
                run_strip('dummy_strip.i', restart, item)
        else:
            run_strip('dummy_strip.i', restart, item)
        cas = read_stripf(item)
        # cases.append(pd.DataFrame(cas.transpose()))
        cases.append(cas)
        what = alpha +'-'+ numeric
        plt.plot(cas['time-0'],cas[what],linewidth=0.75,label=item)
    ...
    if show_plot() :
        ...
        plt.savefig(figures_loc() + what + '.' + figures_type(), format=figures_type(),
                    dpi=300,bbox_inches='tight')
        print(67*'-')
        print("plot has been saved as:", figures_loc() + what + '.' + figures_type())
        print(67*'-')
    plt.close()
    if len(cases) > 1:
        return pd.concat(cases,keys=case_folder)
    else :
        return cases[0]
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_profile(alpha: str, nodes: list, time: float, restart: str, case_folder=''):

    """ Plots a profile at a particular time for the given variables

    :alpha: [str] type of variable to be plotted
    :nodes: [list] list with the numeric part of the variable
    :time: [float or list] the time (or list of times) at which you want to plot the profile
    :restart: [str] the restart code name
    :case_folder: [str or list] (optional) case or cases to be plotted
    :returns: [DataFrame] with the data extracted
        - it plots the specified variable for all specified cases
    ...
    with open('dummy_strip.i', 'w+') as st:
        ...
        for j, item in enumerate(nodes):
            word = 1001 + j 
            st.write(str(word) + ' ' + alpha + ' ' + item + '\n')
        st.write('. \n')
    if type(case_folder) == str :
        case_folder = [case_folder]
    if type(time) == float :
        time = [time]
    cases = pd.DataFrame(index=nodes) 
    for item in case_folder:
        if not os.path.isfile('./' + item + '/' + restart + '.r'):
            message('File ./'+ item + '/' + restart + '.r does not exist.', 'ERROR--plot_profile')
        try :
            os.remove('./' + item + '/' + 'stripf')
        except OSError :
            pass
        if relap_scdap():
            ...
            run_strip('dummy_strip.i', restart, item)
        cas = read_stripf(item)
        time_column = cas['time-0'][:].to_numpy()
        for tim in time:
            closest_index = np.argmin(np.abs(time_column - tim))
            closest_time = time_column[closest_index]
            # Print warning if the closest time isn't an exact match
            if closest_time != tim:
               message(f"Warning: Exact time {tim}s not found. Using closest time {closest_time}s instead.",
                       'WARNING--plot_profile')
            # Extract variable values for the closest time
            variables = list(cas.loc[closest_index])
            cases[f"{item}_{closest_time:.0f}"] = variables[1:]
            # Plot the profile
            plt.plot(range(1, len(variables)), variables[1:], marker='o',
                     linestyle='-', label=f"case: {item}, time: {closest_time:.0f}")
    plt.xlabel('Node')
    if alpha in relap_labels:
       plt.ylabel(relap_labels.get(alpha))
    # plt.title(f"Profile of {alpha} at time = {closest_time}")
    plt.legend()
    plt.tight_layout()
    fig_name = figures_loc() + alpha + '_profile.' + figures_type()
    plt.savefig(fig_name, format=figures_type(), dpi=300,bbox_inches='tight')
    print(67*'-')
    print("plot has been saved as:", figures_loc() + alpha + '_profile.' + figures_type())
    print(67*'-')
    ...
    if len(cases) > 1:
        ...
        return cases[0]
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def general_fig(variables: pd.DataFrame, cases_df: pd.DataFrame, 
                dataExp: pd.DataFrame=None , annotations=[], labels=[],
                x_limits: tuple=(0,0), label_case: str='UPC',fig_name: str=''):
    """

    :variables: Dataframe containing all the requested variables and properties
    :cases_df: DataFrame containing all results: cases_df.loc['base'] or cases_df if only one case is there
    :dataExp: DataFrame containing the experimental data
    :annotations: (optional) location of the tags for each line
    :labels: (optional) list of labels for each y_axes
    :x_limits: (optional) limits in the x axis
    :label_case: (optional) Title of the case

    :USAGE:
           rp.general_fig(variables, cases_df['base'])
           your_name, your_axes = rp.general_fig(variables, cases_df['base'])
        :with annotations:

           rp.general_fig(variables, cases_df['base'], data_test62,
                         [[300., 15030000.0],
                         [7500.0, 8000000.0],
                         [100.0,5500000.0],
                         [23930.06, 1445.9],
    ...
    """
    if isinstance(cases_df.index, pd.MultiIndex):
        message('Your data file has more than one case, use general_fig_all_cases ' + \
                'if you want to plot the general figure for each case or ' + \
                'input cases_df.loc["case"] for a single case', 'WARNING--general_fig')
        return
    dataCalc0 = np.array(list(cases_df.to_records(index=False)))
    if dataExp is not None :
        if type(dataExp) == str and dataExp != '' :
            message('For the experimental data, you first need to load the experimental' + \
                    ' data to an Ndarray with genfromtxt method ' + \
                    'The general figure will not be generated', 'WARNING--general_fig')
            return
    if x_limits[0] == 0 and x_limits[1] == 0 :
        if 'timei' in variables.columns :
            initial_time = variables.loc[1,'timei']
            final_time   = variables.loc[1,'timee']
        else :
            message('For the general figure, you need to specify the time interval. ' + \
                    'The general figure will not be generated', 'WARNING--general_fig')
            return
    else : 
        initial_time = x_limits[0]
        final_time   = x_limits[1]
    ticks        = final_time/10
    try : 
        initial_index = np.where(dataCalc0['time'] >= initial_time)[0][0]
    except : 
        message('the calculation time did not reach the initial plotting time. ' + \
                'The general figure will not be generated', 'WARNING--general_fig')
        return
    ...
    message('annotation location coordinates are the following', 'INFO--general_fig')
    for i in range(0,3):
        l = 0
        ...
        for k, plot in plotted.iterrows():
            ...
            time_coord = (dataCalc0['time'][initial_index
                    + np.argmax(dataCalc0[plot['var_tag']][initial_index:final_index])])
            time_var = max(dataCalc0[plot['var_tag']][initial_index:final_index])
            if time_coord <= initial_time or time_coord >= final_time :
                time_coord = 0.0
                time_var = dataCalc0[plot['var_tag']][initial_index]
            ...
            if dataExp is not None : 
                l_style = ':'
                tim_column = dataExp.columns[0]
                if plot['var_tag'] in dataExp.columns:
                   axes[i].plot(dataExp[tim_column],dataExp[plot['var_tag']], linewidth=1,
                           color=colors[l], alpha=1.0, linestyle='-',
                           label=label_exp)
                   label_exp = '_nolegend_'
            axes[i].plot(dataCalc0['time'],dataCalc0[plot['var_tag']], linewidth=1,
                    color=colors[l], alpha=1.0, linestyle= l_style,label=label)
            axes[i].annotate(plot['figure_tag'].replace('_',' '), xy = anot,
        ...
        for k, plot in plotted.iterrows():
            ...
            if dataExp is not None :
               l_style = ':'
               if plot['var_tag'] in dataExp.dtype.names :
                  ax2.plot(dataExp[tim_column], dataExp[plot['var_tag']], linewidth=1,
                          color=colors[l], alpha=1.0, linestyle='-',
                          label=label_exp)
                  label_exp = '_nolegend_'
            ax2.plot(dataCalc0['time'], dataCalc0[plot['var_tag']], linewidth=1,
                    color=colors[l], alpha=1.0, linestyle= l_style,label=label)
            ax2.annotate(plot['figure_tag'].replace('_', ' '), xy=anot, xycoords='data',
    ...
    if len(fig_name) > 0 : fig_name = '_' + fig_name
    f.savefig(figures_loc() + 'general'+ fig_name +'.'+ figures_type(), format=figures_type(), dpi=300)
    message('plot has been saved as: '+figures_loc() + 'general.' + figures_type(),'INFO')
    if show_plot() : plt.show()
    return f, axes

def general_fig_all_cases(variables: pd.DataFrame, cases_df: pd.DataFrame, 
                dataExp: pd.DataFrame=None , annotations=[], labels=[],
                x_limits: tuple=(0,0), label_case: str='UPC'):
    """
    ...
    :USAGE:
           rp.general_fig_all_cases(variables, cases_df)
        :with annotations:

    :returns: A figure with the general evolution of the transient for each case in cases_df

    """
    cases = list(cases_df.index.levels[0]) 
    for item in cases:
        message('Generating the general plot for case '+ item,'INFO--general_fig_all_cases')
        general_fig(variables, cases_df.loc[item], dataExp, annotations, labels,
                    x_limits, label_case, item)

def plot_variables(variable: list, variables_df, cases_df, ylabel='',
                   y_limits: tuple=(0,0), x_limits: tuple=(0,0)):
    ...
    :variables_df: Dataframe containing the variables and properties
                         of each variable
    :cases_df: DataFrame containing the data for the particular case:
               cases_df.loc['base'] or cases_df if the DataFrame has one case
    :ylabel: (optional) the ylabel
    :y_limits: (optional) limits in the y axis
    :x_limits: (optional) limits in the x axis

    :USAGE:
       rp.plot_variables(['SGTR_MF', 'REL_VLV_MF'], variables, cases_df.loc['base'])

    :output: a plot with the two variables listed

    """
    if isinstance(cases_df.index, pd.MultiIndex):
        message('Remember that cases_df should be an DataFrame containing the data' + \
                'for one particular case,\n ex.: cases_df.loc["base"] \n' + \
                'Plot for variables:' + variable[0] + ', ' + variable[1] + \
                '... will not be plotted', 'WARNING--plot_variables')
        return
    variables = []
    for var in variable:
        if var not in list(variables_df['var_tag']):
            message(var + ' is not in the list of variables and will not be plotted',
                    'WARNING--read_variables')
        else : 
            variables.append(var)
    if len(variables) == 0:
        message('Plot for variables:' + variable[0] + ', ' + variable[1] + \
                '... will not be plotted', 'WARNING--plot_variables')
        return
    message('making plot for' + variables[0] + ' to ' + variables[-1],
            'INFO--plot_variables')
    plt.figure(variables[0] + '_' + variables[-1])
    for var in variables :
        i = variables_df[variables_df['var_tag'] == var].index.values[0]
        plt.plot(cases_df['time'], cases_df[variables_df['var_tag'][i]],
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(figures_loc() + variables[0] +'_'+ variables[-1] +'.'+ figures_type(),
            format= figures_type(), dpi=300, bbox_inches='tight')
    message('plot has been saved as: '+ figures_loc() + variables[0] +'_'+ \
            variables[-1] +'.'+ figures_type(),'INFO--plot_variables')
    if show_plot() : plt.show()
    plt.close()


def append_variable(dataframe, var_tag: str, unit: str,
...
def print_ss(cases_df: pd.DataFrame, dataExp: pd.DataFrame=None, variables= False, end_ss = 0, length = 50.):
    """ Prints a table for steady state values
    TODO: 
    - Print the result in a text file

    :case: [DataFrame] specific case or cases for which you want to make the table
    ...
    :variables: (optional) list of variables to add to the table. Default is False and 
             all variables are shown
    :end_ss: (optional) end time of the steady state
    :length: Time length to be averaged as steady state value
    :returns: the DataFrame with the results and prints on screen the table of steady state
    ...
    if relap_scdap():
        ...
        key = 'time'
    if type(dataExp) == str :
        message('you first need to load the experimental data to a DataFrame','ERROR--print_ss')
    if not variables:
        try :
            variables = list(cases_df.columns)
        except AttributeError:
            message('You should input a pd.Dataframe, the steady state will not be printed', 'WARNING')
    vars = [key]
    vars.append(variables)
    cases_ss = cases_df[variables]
    if dataExp is not None :
        dataExp = dataExp.rename(columns={dataExp.columns[0]: "time"})
        for column in cases_ss.columns:
            if column not in dataExp.columns:
                # cases_ss.drop(column, axis=1, inplace=True)
                dataExp[column] = 123456789
        for column in dataExp.columns:
            if column not in cases_ss.columns:
                dataExp.drop(column, axis=1, inplace=True)
        dataExp.index = pd.MultiIndex.from_product([["experimental"], dataExp.index])
        cases_ss = pd.concat([cases_ss, dataExp])

    cases_ss = cases_ss[(cases_ss[key] >= end_ss - length) & (cases_ss[key] <= end_ss)]
    cases_ss.drop(key, axis=1, inplace=True)

    if is_multiindex:
        cases_ss = cases_ss.groupby(level=0).apply(lambda x: x.mean())
        cases_ss = cases_ss.transpose()
        cases_ss = cases_ss.replace(123456789, '-')
        print(tabulate(cases_ss, headers='keys', tablefmt='github',numalign='left',floatfmt=".2e"))
    else :
        cases_ss = cases_ss.apply(lambda x: x.mean())
        data = list(cases_ss.items())  # or cc.to_dict().items()^M
        print(tabulate(data, headers=['Parameter', 'Value'], tablefmt='github',numalign='left',floatfmt=".2e"))
    print(80*'-')
    return cases_ss
...
def mypie(data: list, labels: list, title='',unit: str='%'):
    ...
    plt.close('all')
    return fig, ax
def interactive_plot(data: pd.DataFrame, name: str='interactive_plot',
                     dataExp: pd.DataFrame = None, x_limits: tuple=(0,0)):
    """TODO: Docstring for interactive_plot.

    :data: DataFrame with the data
    :name: name of the html file to be created
    :dataExp: (optional) name of the DataFrame containing the experimental data
    :x_limits: (optional) limits in the x axis
    :returns: an interactive plot in html

    """
    base_colors = ['k','mediumblue','red','green','tab:orange','tab:purple',
            'tab:brown','tab:pink','tab:olive','tab:cyan', 'tab:grey']
    all_data = {}
    if type(dataExp) == str :
        message('you first need to load the experimental data to a DataFrame','ERROR--interactive_plot')
    if dataExp is not None :
        all_data['Experimental'] = np.array(list(dataExp.to_records(index=False)))
    if isinstance(data.index, pd.MultiIndex):
       cases = list(data.index.levels[0]) 
       for item in cases:
           all_data[item] = np.array(list(data.loc[item].to_records(index=False)))
    else :
        all_data['base_case'] = np.array(list(data.to_records(index=False)))
    colors = dict(zip(cases, base_colors[:len(cases)]))
    variables = list(data.columns)
    cases_json = json.dumps(list(all_data.keys()))
    dat_json = json.dumps({key: value.tolist() for key, value in all_data.items()})
    variables_index_json = json.dumps({variable: index for index, variable in enumerate(variables[1:])})
    colors_json = json.dumps(colors)
    variables_json = json.dumps(variables)
    ranges = x_limits
    if x_limits[0] == 0 and x_limits[1] == 0 :
        ranges = [data.time.min(), data.time.max()]
    ...
    <body>
        ...
        <div>
            ...
            <select id="variable-dropdown" onchange="updatePlot()">
                {}
            </select>
        </div>
        <div id="plot"></div>
        ...
        <script>
            ...
            function updatePlot() {{
                var selectedVariable = document.getElementById("variable-dropdown").value;
                ...
                if (selectedVariable !== 'time') {{
                    ...
                    cases.forEach(function(group) {{
                        var xValues = dat[group].map(function(row) {{ return row[0]; }});
                        var yValues = dat[group].map(function(row) {{ return row[variablesIndex[selectedVariable]+1]; }});

                        if (!yValues.every(function(value) {{ return value === 0; }})) {{
                            var trace = {{
                                x: xValues,
                                y: yValues,
                                mode: 'lines',
                    ...
                    var layout = {{
                        hovermode:'closest',
                        title: variables[variablesIndex[selectedVariable]+1],
                        xaxis: {{ title: "Time (s)", range: ranges }},
                        yaxis: {{ title: variables[variablesIndex[selectedVariable]+1] }},
                        showlegend: true,  
                        height: 600
                    }};
    ...
    '''
    with open(figures_loc() + name + '.html', 'w') as f:
        f.write(html_code.format(
            '\n'.join(f'<option value="{variable}">{variable}</option>' for variable in variables[1:]),
            cases_json, dat_json, variables_index_json, colors_json, variables_json, ranges_json))
    
    message('The interactive plot has been saved as: '+name+'.html','INFO--interactive_plot')
    pass
