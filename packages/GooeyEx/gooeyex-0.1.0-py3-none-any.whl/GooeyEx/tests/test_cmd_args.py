...
from GooeyEx.python_bindings import cmd_args
...
class TextCommandLine(unittest.TestCase):
    ...
    def test_default_overwritten(self):
        ...
        argdefault = next(
            action for action in parser._actions if action.dest == "arg"
        ).default
    ...
    def test_required_not_enforced(self):
        ...
        argnnargs = next(
            action for action in parser._actions if action.dest == "argn"
        ).nargs
        self.assertEqual(argnnargs, "+")
        argpnargs = next(
            action for action in parser._actions if action.dest == "argp"
        ).nargs
        self.assertEqual(argpnargs, None)
        mutexrequired = next(
            mutex for mutex in parser._mutually_exclusive_groups
        ).required
    ...
    def test_cmd_args_subparser(self):
        ...
        subparseraction = next(
            action for action in parser._actions if action.dest == "subparser"
        )
        argAdefault = next(
            action
            for action in subparseraction.choices["A"]._actions
            if action.dest == "argA"
        ).default
        self.assertEqual(argAdefault, 1)
        argBdefault = next(
            action
            for action in subparseraction.choices["B"]._actions
            if action.dest == "argB"
        ).default
