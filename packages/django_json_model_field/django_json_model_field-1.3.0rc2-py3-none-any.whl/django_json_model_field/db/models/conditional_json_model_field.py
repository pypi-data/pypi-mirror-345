...
class ConditionalJSONModelField(BaseJSONModelField):
    ...
    def __init__(
        self,
        selector_field: str,
        model_map: dict[Any, Type[JSONModel] | JSONModelState],
        ...
        **kwargs,
        ...
        model_map = {key: reconstruct(value) for key, value in model_map.items()}
        self.model_map = {
    ...
    def _deconstruct_compat(self, *, cloned: bool = False):
        ...
        model_map = {
            key: JSONModelState.from_model(value)
            for key, value in self._model_map.items()
        }
        # sort the keys to ensure consistent ordering in migrations
        ordered_keys = sorted(model_map.keys())
        ordered_map = OrderedDict((key, model_map[key]) for key in ordered_keys)
        args = [self.selector_field_name, ordered_map, *args]
        kwargs.update(selector_data_key=self._selector_data_key)
        return name, path, args, kwargs

    def _deconstruct_compat_is_reconstructed(self) -> bool:
        return any(model for model in self.model_map.values() if self._deconstruct_compat_model_is_reconstructed(model))
    ...
    def _get_db_prep_value_from_model(
        self,
        instance: "JSONModel",
        connection: BaseDatabaseWrapper,
        prepared=False
    ) -> dict:
        data = super()._get_db_prep_value_from_model(instance, connection, prepared)
