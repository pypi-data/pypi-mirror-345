...
class AccLuminosityAnalyzer:
    ...
    def __init__(
        ...
        accel_mode_by_year: Optional[dict] = None,
    ):
        ...
        self.accel_mode_by_year = accel_mode_by_year
        ...
        self.cms_energy_label = self.format_cms_energy()
    ...
    def get_energy_per_nucleon_scale_factor(self, amodetag: str):
        if amodetag not in ["IONPHYS", "PAPHYS"]:
            raise ValueError("Incorret bril accelerator mode tag")
        res = self.LEAD_SCALE_FACTOR
        if amodetag == "PAPHYS":
            res = math.sqrt(res)
        return res

    def format_cms_energy(self, include_units=True):
        cms_energy = 2 * self.beam_energy
        cms_energy_str = "???"

        if self.bril_amodetag == "PROTPHYS":
            width = 1 if self.year in (2022, 2023, 2024) else 0
            cms_energy_str = "%.*f" % (width, 1.0e-3 * cms_energy)
        elif self.bril_amodetag in ["IONPHYS", "PAPHYS", "ALLIONS"]:
            amodetag = self.accel_mode_by_year[self.year] if self.bril_amodetag == "ALLIONS" else self.bril_amodetag
            scale_factor = self.get_energy_per_nucleon_scale_factor(amodetag)
            cms_energy_str = "%.2f" % (1.0e-3 * scale_factor * cms_energy)
            if include_units and self.bril_amodetag == "ALLIONS":
                cms_energy_str = PARTICLE_TYPES[amodetag] + " " + cms_energy_str

        if include_units:
            if self.bril_amodetag in ("PROTPHYS", "IONPHYS"):
                cms_energy_str += " TeV"
            else:
    ...
    def plot_acc_lumi_by_day(self, save_pdf: bool = False):
        ...
        for t in leg.get_texts():
            t.set_font_properties(self.FONT_PROPS_TICK_LABEL)
        ...
        # Update fig
        suptitle = (
            f"CMS Integrated Luminosity, {self.particle_type}, {self.year}, {self.sqrt_s} {self.cms_energy_label}"
        )
        ...
        zoom = 0.8
        xy_offset = (2.0, -3.0)
        logo_img_path = resources.files(assets) / "cms_logo.png"
        logo_data = Image.open(logo_img_path)
        fig_dpi = ax.get_figure().dpi
        fig_size = ax.get_figure().get_size_inches()
        zoom_factor = 0.1 / 1.2 * fig_dpi * fig_size[0] / np.shape(logo_data)[0]
        zoom_factor *= zoom
