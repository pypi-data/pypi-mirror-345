...
from capibara_model.core._model import CapibaraModel as BaseCapibaraModel
from capibara_model.utils import LanguageUtils
...
class TPUDataLoader:
    ...
    def _parse_example(self, example: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        # Procesar bytes usando LanguageUtils
        input_data = LanguageUtils.text_to_bytes(
            parsed['input_bytes'].numpy().decode('utf-8'),
            max_length=self.config.seq_length
        )
        target_data = LanguageUtils.text_to_bytes(
            parsed['target_bytes'].numpy().decode('utf-8'),
            max_length=self.config.seq_length
        )
...
def train_step(state: train_state.TrainState, batch: Dict) -> tuple:
    ...
    def loss_fn(params):
        ...
        return loss * jax.lax.rsqrt(jax.device_count())
...
def validate(state: train_state.TrainState, data_loader: TPUDataLoader) -> float:
    ...
    # Función JITeada para un paso de validación
