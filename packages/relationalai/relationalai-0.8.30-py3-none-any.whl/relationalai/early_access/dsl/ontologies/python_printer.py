...
from relationalai.early_access.dsl import Model, ExternalRelation, ValueType, ValueSubtype, CsvTable, Relationship
...
class PythonPrinter(Printer):
    _mandatory_roles: OrderedSet[AbstractRole] = OrderedSet()
    _single_unique_roles: OrderedSet[AbstractRole] = OrderedSet()
    _composite_preferred_identifiers: OrderedSet[Unique] = OrderedSet()
    _internal_preferred_identifier_roles: OrderedSet[AbstractRole] = OrderedSet()
    _relationship_uniqueness_constraints: dict[Relationship, OrderedSet[Unique]] = defaultdict(OrderedSet)
    ...
    def _handle_entity_types(self, model: Model) -> None:
        for name in model.entity_types_map().keys():
            self._print_nl(f"{name} = EntityType('{name}')")
    ...
    def _handle_csv_tables(self, model: Model) -> None:
        for name, table in model.bindable_tables().items():
            if isinstance(table, CsvTable):
                schema_name = f"{name}_schema"
                table_name = name.upper()
                self._handle_csv_table_schema(schema_name, table)
                self._print_nl(f"{table_name} = CsvTable('{name}', {schema_name})")
                self._handle_csv_table_data(table_name, table)
                self._nl()

    def _handle_csv_table_schema(self, schema_name, table: CsvTable):
        self._print_nl(f"{schema_name} = {{")
        for column_name, column in table.columns().items():
            self._indent_print_nl(1, f"'{column_name}': {self._get_type(column.metadata.datatype)},")
        self._print_nl("}")

    def _handle_csv_table_data(self, table_name, table: CsvTable):
        data_name_prefix = table_name.lower()
        for idx, csv_data in enumerate(table.csv_data):
            data_name = f"{data_name_prefix}_{idx}"
            self._print_nl(f'{data_name} = """{csv_data}"""')
            self._print_nl(f"{table_name}.data({data_name})")
