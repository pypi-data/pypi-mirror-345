    ...
    def __init__(self, *args, **kw):
        ...
        if len(url_parts) > 1:
            ...
            if rest.startswith('localhost') or rest.startswith('127.0.0.1'):
                ...
                # Insert -services after the first part of the hostname
                hostname_parts = rest.split('/')  # Split by / to handle any path components
                domain_parts = hostname_parts[0].split('.')  # Split the hostname by dots
                domain_parts[0] = domain_parts[0] + '-services'  # Add -services to first part
                hostname_parts[0] = '.'.join(domain_parts)  # Rejoin the hostname
                services_base = protocol + '://' + '/'.join(hostname_parts)  # Reconstruct full URL
        else:
...
def build(serviceName, version, base_url, ocp_apim_subscription_key=None, ocp_apim_services_subscription_key=None, **kw) -> DataServicesResource:
