...
from labels.parsers.package_information.dotnet import (
    NugetCatalogEntry,
    get_nuget_package,
)
...
def _update_advisories(package: Package) -> None:
    pkg_advisories = advisories.get_vulnerabilities("nuget", package.name, package.version)
    if pkg_advisories:
...
def complete_package(package: Package) -> Package:
    ...
    package.licenses = validate_licenses([nuget_licenses]) if nuget_licenses else []
