    ...
    def hlan_checkurl_shutdown_bj(
        ...
        *,
        ...
        #某些pve中可能需要写 python3.10 -c xxx
            ...
            @reboot python3 -c "from dknovautils import *; HomeLanUtils.hlan_checkurl_shutdown_bj(cmd='pve_stop_all',verbose=False,dry_run=Fasle);" >>/root/pve_stop_all.log

            @reboot echo `date -Is` "after run checkurl-shutdown" >>/root/start.log

            @reboot echo `date -Is` `python3 --version` >>/root/test_cron.log
        ...
        debug_cmd = (
            "#"
            + r""" python3 -c "from dknovautils import *; HomeLanUtils.hlan_checkurl_shutdown_bj(cmd='pve_stop_all',verbose=False,dry_run=Fasle);"  """
        )
