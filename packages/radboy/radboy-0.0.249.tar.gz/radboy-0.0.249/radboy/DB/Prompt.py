...
{Fore.grey_70}+Tax({i.Tax}) w/o CRV({i.CRV})*Total({round(total,2)}){Fore.slate_blue_1}\n=TaxNoCRVTotal({tax})+TotalPrice({round(total*i.Price,ROUNDTO)})=NetPrice({round(total*i.Price,ROUNDTO)+tax})
{Fore.grey_70}+Tax({i.Tax}) w/ CRV({i.CRV})*Total({round(total,2)}){Fore.slate_blue_1}\n=TaxCRVTotal({tax_crv})+TotalPrice({round(total*i.Price,ROUNDTO)})=NetPrice({round(total*i.Price,ROUNDTO)+tax_crv})
