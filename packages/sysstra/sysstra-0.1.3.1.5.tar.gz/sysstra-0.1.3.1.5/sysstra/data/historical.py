...
def fetch_option_candles(underlying, start_date, end_date, option_type, strike_price, expiry="near", granularity=1, timestamp=None):
    ...
    try:
        ...
        request_data = {"underlying": underlying, "from_date": str(start_date),
                        ...
                        "expiry": expiry, "granularity": granularity, "timestamp": str(timestamp)}
...
def fetch_option_candles_by_symbol(symbol, start_date, end_date, granularity=1):
    """ Function to Fetch Options Trade Data """
    try:
        headers = {"x-api-key": api_key, "content-type": "application/json"}
        request_data = {"symbol": symbol, "from_date": start_date, "to_date": end_date, "granularity": granularity}
...
def fetch_crypto_candles(symbol, start_date, end_date, granularity=1):
    """ Function to fetch candles for the respective date """
    try:
        headers = {"x-api-key": api_key, "content-type": "application/json"}
        request_data = {"symbol": symbol, "from_date": start_date, "to_date": end_date,
...
def fetch_forex_candles(symbol, start_date, end_date, granularity=1):
    """ Function to fetch candles for the respective date """
    try:
        headers = {"x-api-key": api_key, "content-type": "application/json"}
        request_data = {"symbol": symbol, "from_date": start_date, "to_date": end_date,
...
if __name__ == '__main__':
    ...
    candle = fetch_option_candle_by_timestamp(underlying_symbol="SENSEX",  strike_price=80200, option_type="CE", expiry="near",
