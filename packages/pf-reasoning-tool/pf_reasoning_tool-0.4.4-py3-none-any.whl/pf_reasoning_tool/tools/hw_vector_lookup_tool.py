...
FAISS and Chroma.  Imports are lazy, so missing libraries do not block other tools.
Requires an AzureOpenAIConnection or OpenAIConnection for embeddings.
'''
...
# -------------------------------------------------------------------------- #
# Vectorstore factory (imports on demand)                                    #
# -------------------------------------------------------------------------- #
...
def _get_store(
    ...
    *,
    ...
    if b == 'azure_search':
        try:
            AzureSearch = import_module(
                'langchain_community.vectorstores.azuresearch'
            ).AzureSearch
        except ModuleNotFoundError:
            raise ModuleNotFoundError(
                'AzureSearch vectorstore not found.  Install with '
                '"pip install pf-reasoning-tool[azure]"'
            )
    ...
    if b == 'weaviate':
        try:
            Weaviate = import_module(
                'langchain_community.vectorstores.weaviate'
            ).Weaviate
        except ModuleNotFoundError:
            raise ModuleNotFoundError(
                'Weaviate vectorstore not found.  Install with '
                '"pip install pf-reasoning-tool[weaviate]"'
            )
    ...
    if b == 'faiss':
        try:
            ...
            raise ModuleNotFoundError(
                'FAISS vectorstore not found.  Install with '
                '"pip install pf-reasoning-tool[faiss]"'
            )
    ...
    if b == 'chroma':
        try:
            Chroma = import_module(
                'langchain_community.vectorstores.chroma'
            ).Chroma
        except ModuleNotFoundError:
            raise ModuleNotFoundError(
                'Chroma vectorstore not found.  Install with '
                '"pip install pf-reasoning-tool[chroma]"'
            )
