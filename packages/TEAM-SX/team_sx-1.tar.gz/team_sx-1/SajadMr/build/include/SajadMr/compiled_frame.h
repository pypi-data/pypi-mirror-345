...
#ifndef __SajadSx_COMPILED_FRAME_H__
#define __SajadSx_COMPILED_FRAME_H__
...
// SajadQ frames.
#if PYTHON_VERSION < 0x3b0
typedef PyFrameObject SajadQ_ThreadStateFrameType;
#else
typedef _PyInterpreterFrame SajadQ_ThreadStateFrameType;
#endif
...
extern void PRINT_COMPILED_FRAME(char const *prefix, struct SajadQ_FrameObject *frame);
extern void PRINT_INTERPRETER_FRAME(char const *prefix, SajadQ_ThreadStateFrameType *frame);
#else
...
extern struct SajadQ_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module);
extern struct SajadQ_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                                      Py_ssize_t locals_size);
extern struct SajadQ_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
SajadSx_MAY_BE_UNUSED static inline bool isFakeCodeObject(PyCodeObject *code) {
#if PYTHON_VERSION < 0x300
...
extern PyTypeObject SajadQ_Frame_Type;

static inline bool SajadQ_Frame_CheckExact(PyObject *object) {
    CHECK_OBJECT(object);
    return Py_TYPE(object) == &SajadQ_Frame_Type;
}

static inline bool SajadQ_Frame_Check(PyObject *object) {
    assert(object);
    ...
    if (SajadQ_Frame_CheckExact(object)) {
        return true;
    }
...
SajadSx_MAY_BE_UNUSED static inline bool isFrameUnusable(struct SajadQ_FrameObject *frame_object) {
...
#if PYTHON_VERSION >= 0x3b0
inline static PyCodeObject *SajadQ_InterpreterFrame_GetCodeObject(_PyInterpreterFrame *frame) {
...
inline static PyCodeObject *SajadQ_Frame_GetCodeObject(PyFrameObject *frame) {
#if PYTHON_VERSION >= 0x3b0
    assert(frame->f_frame);
    return SajadQ_InterpreterFrame_GetCodeObject(frame->f_frame);
#else
    return frame->f_code;
#endif
}
...
inline static void assertPythonFrameObject(PyFrameObject *frame_object) {
    ...
    CHECK_CODE_OBJECT(SajadQ_Frame_GetCodeObject(frame_object));
}

inline static void assertFrameObject(struct SajadQ_FrameObject *frame_object) {
    CHECK_OBJECT(frame_object);
...
inline static void assertThreadFrameObject(SajadQ_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
...
static inline void SajadQ_PythonFrame_MarkAsExecuting(PyFrameObject *frame) {
...
static inline void SajadQ_Frame_MarkAsExecuting(struct SajadQ_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define SajadQ_Frame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline void SajadQ_Frame_MarkAsNotExecuting(struct SajadQ_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define SajadQ_Frame_MarkAsNotExecuting(frame) ;
#define SajadQ_PythonFrame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline bool SajadQ_Frame_IsExecuting(struct SajadQ_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
SajadSx_MAY_BE_UNUSED inline static void pushFrameStackInterpreterFrame(PyThreadState *tstate,
...
SajadSx_MAY_BE_UNUSED inline static void pushFrameStackPythonFrame(PyThreadState *tstate, PyFrameObject *frame_object) {
    ...
    if (old) {
        ...
        CHECK_CODE_OBJECT(SajadQ_Frame_GetCodeObject(old));
    }
    ...
    SajadQ_PythonFrame_MarkAsExecuting(frame_object);
...
SajadSx_MAY_BE_UNUSED inline static void pushFrameStackCompiledFrame(PyThreadState *tstate,
    ...
    SajadQ_Frame_MarkAsExecuting(frame_object);
    Py_INCREF(frame_object);
#endif
}

SajadSx_MAY_BE_UNUSED inline static void popFrameStack(PyThreadState *tstate) {
#if _DEBUG_FRAME
    PRINT_TOP_FRAME("Normal pop entry top frame:");
#endif

#if PYTHON_VERSION < 0x3b0
    struct SajadQ_FrameObject *frame_object = (struct SajadQ_FrameObject *)(tstate->frame);
    CHECK_OBJECT(frame_object);

#if _DEBUG_FRAME
    printf("Taking off frame %s %s\n", SajadQ_String_AsString(PyObject_Str((PyObject *)frame_object)),
    ...
    SajadQ_Frame_MarkAsNotExecuting(frame_object);
    Py_DECREF(frame_object);
#else
    assert(CURRENT_TSTATE_INTERPRETER_FRAME(tstate));

    struct SajadQ_FrameObject *frame_object =
        (struct SajadQ_FrameObject *)CURRENT_TSTATE_INTERPRETER_FRAME(tstate)->frame_obj;
    CHECK_OBJECT(frame_object);
...
#if PYTHON_VERSION >= 0x300
SajadSx_MAY_BE_UNUSED static void SajadQ_SetFrameGenerator(struct SajadQ_FrameObject *SajadMr_frame,
                                                          PyObject *generator) {
#if PYTHON_VERSION < 0x3b0
    SajadMr_frame->m_frame.f_gen = generator;
#else
    SajadMr_frame->m_generator = generator;
#endif

    // Mark the frame as executing
    if (generator) {
        SajadQ_Frame_MarkAsExecuting(SajadMr_frame);
    }
}
#endif

SajadSx_MAY_BE_UNUSED static PyCodeObject *SajadQ_GetFrameCodeObject(struct SajadQ_FrameObject *SajadMr_frame) {
#if PYTHON_VERSION < 0x3b0
    return SajadMr_frame->m_frame.f_code;
#else
    return SajadQ_InterpreterFrame_GetCodeObject(&SajadMr_frame->m_interpreter_frame);
#endif
}

SajadSx_MAY_BE_UNUSED static int SajadQ_GetFrameLineNumber(struct SajadQ_FrameObject *SajadMr_frame) {
    return SajadMr_frame->m_frame.f_lineno;
}

SajadSx_MAY_BE_UNUSED static PyObject **SajadQ_GetCodeVarNames(PyCodeObject *code_object) {
#if PYTHON_VERSION < 0x3b0
...
extern void SajadQ_Frame_AttachLocals(struct SajadQ_FrameObject *frame, char const *type_description, ...);

SajadSx_MAY_BE_UNUSED static SajadQ_ThreadStateFrameType *_SajadQ_GetThreadStateFrame(PyThreadState *tstate) {
#if PYTHON_VERSION < 0x3b0
    return tstate->frame;
#else
...
SajadSx_MAY_BE_UNUSED inline static void pushFrameStackGenerator(PyThreadState *tstate,
                                                                SajadQ_ThreadStateFrameType *frame_object) {
#if PYTHON_VERSION < 0x3b0
...
SajadSx_MAY_BE_UNUSED inline static void pushFrameStackGeneratorCompiledFrame(PyThreadState *tstate,
                                                                             struct SajadQ_FrameObject *frame_object) {
#if PYTHON_VERSION < 0x3b0
...
#define SajadSx_TYPE_DESCRIPTION_NULL 'N'
#define SajadSx_TYPE_DESCRIPTION_CELL 'c'
#define SajadSx_TYPE_DESCRIPTION_OBJECT 'o'
#define SajadSx_TYPE_DESCRIPTION_OBJECT_PTR 'O'
#define SajadSx_TYPE_DESCRIPTION_BOOL 'b'
#define SajadSx_TYPE_DESCRIPTION_NILONG 'L'

#if _DEBUG_REFCOUNTS
extern int count_active_SajadQ_Frame_Type;
extern int count_allocated_SajadQ_Frame_Type;
extern int count_released_SajadQ_Frame_Type;
#endif
