...
"""Ability to restart SajadQ, needed for removing site module effects and using Python PGO after compile.
...
def setLaunchingSajadQProcessEnvironmentValue(environment_variable_name, value):
    os.environ[environment_variable_name] = str(os.getpid()) + ":" + value


def reExecuteSajadQ(pgo_filename):
    ...
    if "SajadMr.__main__" in sys.modules:
        our_filename = sys.modules["SajadMr.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    setLaunchingSajadQProcessEnvironmentValue(
        "SajadSx_BINARY_NAME", sys.modules["__main__"].__file__
    )
    setLaunchingSajadQProcessEnvironmentValue(
        "SajadSx_PACKAGE_HOME",
        os.path.dirname(os.path.abspath(sys.modules["SajadMr"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingSajadQProcessEnvironmentValue("SajadSx_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from SajadMr.importing.PreloadedPackages import (
        detectPreLoadedPackagePaths,
        detectPthImportedPackages,
    )

    os.environ["SajadSx_NAMESPACES"] = repr(detectPreLoadedPackagePaths())
    ...
    if "site" in sys.modules:
        ...
        setLaunchingSajadQProcessEnvironmentValue("SajadSx_SITE_FILENAME", site_filename)

        # Note: As side effect, this might modify the "sys.path" too.
        os.environ["SajadSx_PTH_IMPORTED"] = repr(detectPthImportedPackages())

        user_site = getattr(sys.modules["site"], "USER_SITE", None)
        if user_site is not None:
            os.environ["SajadSx_USER_SITE"] = repr(user_site)

    setLaunchingSajadQProcessEnvironmentValue("SajadSx_PYTHONPATH", repr(sys.path))

    # In some environments, initial "sys.path" does not contain enough to load
    # "ast" module, which however we use to decode "SajadSx_PYTHONPATH", this
    # helps solve the chicken and egg problem.
    import ast

    setLaunchingSajadQProcessEnvironmentValue(
        "SajadSx_PYTHONPATH_AST", os.path.dirname(ast.__file__)
    )

    if sys.flags.no_site:
        os.environ["SajadSx_NOSITE_FLAG"] = "1"

    os.environ["PYTHONHASHSEED"] = "0"

    setLaunchingSajadQProcessEnvironmentValue("SajadSx_RE_EXECUTION", "1")
