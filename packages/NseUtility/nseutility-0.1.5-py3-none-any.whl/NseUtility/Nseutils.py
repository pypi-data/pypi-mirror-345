...
class NseUtils:
    ...
    def get_corporate_announcement(self, from_date_str: str = None, to_date_str: str = None):
        ...
        # Set date range from today's midnight to current time
        today = datetime.now().strftime("%d-%m-%Y")  # Today's date in required format
        from_date_str = today  # Start from today's midnight
        to_date_str = today  # End at the current time today
        ...
        try:
            ...
            corp_announcement = corp_announcement.rename(columns=column_mapping)

            # Reorder columns
            column_order = [
                "SYMBOL", "COMPANY NAME", "INDUSTRY", "SUBJECT",
                "DETAILS", "ATTACHMENT", "BROADCAST DATE/TIME"
            ]
            corp_announcement = corp_announcement[column_order]
        ...
        try:
            ...
            if 'data' in data and isinstance(data['data'], list):
                ...
                columns = ['key', 'index', 'indexSymbol', 'last', 'variation', 'percentChange', 'open', 'high', 'low',
                        ...
                        'perChange30d', 'perChange365d', 'previousDay', 'oneWeekAgo', 'oneMonthAgo', 'oneYearAgo']
    ...
    def get_corporate_announcement_all(self):
        # Define headers similar to the working script
        headers = {
        ...
        try:
            ...
            # List to store the announcement data
            announcements = []

            # Iterate through each item in the RSS feed
            for entry in feed.entries:
                try:
                    # Extract fields similar to the working script
                    title = entry.title
                    link = entry.link
                    published = entry.published
                    full_description = entry.description

                    # Split the description into subject and details
                    parts = full_description.split(":", 1)
                    subject = parts[0].strip() if len(parts) > 1 else full_description
                    ...
                    announcements.append({
                        ...
                        "SUBJECT": description,
                        "ATTACHMENT": link,
                        "BROADCAST DATE/TIME": published
                    })
                except Exception as e:
                    print(f"Error processing RSS entry: {e}")
                    continue

            # Convert the list of announcements to a DataFrame
            corp_announcement = pd.DataFrame(announcements)

            # If no data is found, return None
            if corp_announcement.empty:
                print("No announcements found in the RSS feed.")
                return None

            # Reorder columns
            column_order = [
                "COMPANY NAME", "SUBJECT","DETAILS",
                "ATTACHMENT", "BROADCAST DATE/TIME"
            ]
            corp_announcement = corp_announcement[column_order]
            ...
            print(f"Error fetching RSS feed: {re}")
            return None
        except Exception as e:
            print(f"Unexpected error while fetching or parsing RSS feed: {e}")
            return None
    ...
    def get_active_contracts(self, symbol: str, expiry_date: str = None):
        ...
        try:
            ...
            print(f"Error fetching option chain data: {str(e)}")
            return None
    ...
    def get_press_releases(self, from_date_str: str = None, to_date_str: str = None, filter: str = None):
        ...
        try:
            ...
            url = f"https://www.nseindia.com/api/press-release?fromDate={from_date_str}&toDate={to_date_str}"
            response = self.session.get(url, headers=self.headers, cookies=ref.cookies.get_dict(), timeout=10)
            response.raise_for_status()
            ...
            for item in json_data:
                ...
                subject_clean = BeautifulSoup(subject_raw, "html.parser").get_text(separator=' ').strip()
