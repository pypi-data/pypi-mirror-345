...
def update_user_setting(widget = None, setting = None, val = None, callback = None):
    ''' Update user conf by passing an arbitrary user.conf pointer (?) aka 'setting' (exe. ' conf.user['task_commands']['email']['status']) and a new value for it '''

    print("update user_settings")
    print(setting)
...
class SettingsWindow(Gtk.Window):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.set_icon_from_file(os.path.abspath('icon/icon-0.svg'))
        ...
        # Operations notebook page  
        self.operations_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
        self.notebook.append_page(self.operations_box,Gtk.Label('Operations'))
        
        reconcile_db_button = Gtk.Button(label="Reconcile sessions database with tasks database")
        reconcile_db_button.connect("clicked", utils.db_cleanup)
        self.operations_box.add(reconcile_db_button)
        ...
        self.operations_box.add(print_time_totals_button)
    ...
    def show_time_targets(self,category="tasks"):
        ...
        for id, data in conf.user['time_targets'][category].items():
            ...
            item_data = utils.db_get_item_by_id(id,category)
            if not item_data:
                label.set_markup("<b>Error</b>: '"+str(id)+"' didn't match anything " )
            else:     
                # print('Show time target',item_data)
                label.set_markup("<b>"+item_data['label']+"</b> "+str(data['type'])+ " "+str(data['value'])+" minutes per "+str(data['within_value'])+" "+str(data['within_unit']) )
    ...
    def show_task_commands(self):
        ...
        for id, command_data in conf.user['task_commands'].items():
            ...
            task_data = utils.db_get_item_by_id(id,"tasks")
            if not task_data:
...
class EditAddTargetDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, category, id = None):
        ...
        box.add(Gtk.Label(label=category))
    ...
    def on_response(self, widget, response, category):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            save_user_settings()
            utils.reindex_one(utils.db_get_item_by_id(self.item_id))
