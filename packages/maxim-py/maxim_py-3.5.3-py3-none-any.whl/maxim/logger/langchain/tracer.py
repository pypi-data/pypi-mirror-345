...
logging = logging.getLogger("MaximSDK")


# 20 minutes
DEFAULT_TIMEOUT = 60 * 20

_langchain_tracer_containers_ctx_var: ContextVar[Dict[str, Container]] = ContextVar(
    "maxim_ctx_langchain_tracer_containers", default={}
)


def get_langchain_tracer_containers(id: str) -> Optional[Container]:
    """
    Get the current Langchain tracer containers from the context
    """
    return _langchain_tracer_containers_ctx_var.get().get(id)


def set_container_in_langchain_tracer_containers(id: str, container: Container):
    """
    Add a container to the current Langchain tracer containers in the context
    """
    containers = _langchain_tracer_containers_ctx_var.get()
    containers[id] = container
    _langchain_tracer_containers_ctx_var.set(containers)


def remove_container_from_langchain_tracer_containers(id: str):
    """
    Remove a container from the current Langchain tracer containers in the context
    """
    containers = _langchain_tracer_containers_ctx_var.get()
    containers.pop(id)
    _langchain_tracer_containers_ctx_var.set(containers)
...
class MaximLangchainTracer(BaseCallbackHandler):
    ...
    def __get_container_from_metadata(self, run_id: UUID) -> Container:
        ...
        if container is None:
            ...
            container = get_langchain_tracer_containers(str(run_id))
    ...
    def __get_container(
        ...
        if parent_run_id is None:
            ...
            container = get_langchain_tracer_containers(str(parent_run_id))
        return container
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        logging.debug("[MaximSDK: Langchain] on_llm_start called")
        ...
        if container is None:
            logging.error("[MaximSDK] Couldn't find a container for generation")
            return
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        logging.debug("[Langchain] on_chat_model_start called")
        ...
        if kwargs.get("parent_run_id", None) is None:
            generation_name = maxim_metadata.generation_name if maxim_metadata else None
        ...
        if container.parent() is None:
            set_container_in_langchain_tracer_containers(str(run_id), container)
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> Any:
        ...
        logging.debug("[MaximSDK: Langchain] on_llm_end called")
        run_id = kwargs.get("run_id", None)
        ...
        if container.parent() is None:
            container.end()
            remove_container_from_langchain_tracer_containers(str(run_id))
    ...
    def on_llm_error(
        ...
        """Run when LLM errors."""
        logging.debug("[Langchain] on_llm_error called")
        run_id = kwargs.get("run_id", None)
        parent_run_id = kwargs.get("parent_run_id", None)
        container = self.__get_container(run_id, parent_run_id)
        if container is None:
    ...
    def on_retriever_start(
        ...
        **kwargs: Any,
        ...
        """
        logging.debug("[Langchain] on_retriever_start called")
        retrieval_id = str(run_id)
        retrieval_config = RetrievalConfig(id=retrieval_id)
        container = self.__get_container(run_id, parent_run_id)
        if container is None:
            logging.error("[MaximSDK] Couldn't find a container for retrieval")
            return
        if not container.is_created():
            container.create()
    ...
    def on_retriever_end(
        ...
        **kwargs: Any,
        ...
        logging.debug("[Langchain] on_retriever_end called")
        container = self.__get_container(run_id, parent_run_id)
        if container is None:
            logging.error("[MaximSDK] Couldn't find a container for retrieval")
            return
    ...
    def on_chain_start(
        ...
        try:
            ...
            if container is None:
                logging.error("[MaximSDK] Couldn't find a container for chain")
                return
            ...
            set_container_in_langchain_tracer_containers(
                str(run_id),
                SpanContainer(
            ...
            logging.error("[MaximSDK] Error while processing chain_start %s", e)
    ...
    def on_chain_end(self, outputs: Union[str, Dict[str, Any]], **kwargs: Any) -> Any:
        try:
            ...
            logging.error("[MaximSDK] Failed to parse chain-end: %s", e)

    def on_chain_error(
        self, error: Union[Exception, BaseException, KeyboardInterrupt], **kwargs: Any
    ) -> Any:
        logging.info("[MaximSDK] on_chain_error called")
    ...
    def on_custom_event(
        ...
        **kwargs: Any,
        ...
        container = self.__get_container(run_id, kwargs.get("parent_run_id", None))
        if container is None:
            logging.error("[MaximSDK] Couldn't find a container for event")
            return
        final_tags = {
            tag.split(":")[0]: tag.split(":")[1] for tag in (tags or []) if ":" in tag
        }
    ...
    def on_agent_action(
        ...
        **kwargs: Any,
        ...
        logging.warning("[MaximSDK] On agent action is not supported")
    ...
    def on_tool_start(
        ...
        try:
            ...
            container = self.__get_container(run_id, parent_run_id)
            if container is None:
                logging.error("[MaximSDK] Couldn't find a container for this tool call")
                return
            if not container.is_created():
                container.create()
            ...
            logging.error("[MaximSDK] Failed to parse tool-start: %s", e)
    ...
    def on_tool_end(self, output: Any, **kwargs: Any) -> Any:
        try:
            ...
            container = self.__get_container(run_id, parent_run_id)
            if container is None:
                logging.error("[MaximSDK] Couldn't find a container for tool call")
                return
            ...
            logging.error("[MaximSDK] Failed to parse tool-end: %s", e)
    ...
    def on_tool_error(
        ...
        try:
            ...
            container = self.__get_container(run_id, kwargs.get("parent_run_id", None))
            if container is None:
                logging.error("[MaximSDK] Couldn't find a container for tool_call")
                return
    ...
    def on_text(self, text: str, **kwargs: Any) -> Any:
        logging.warning("[MaximSDK] Text models are not supported by Maxim callback")
