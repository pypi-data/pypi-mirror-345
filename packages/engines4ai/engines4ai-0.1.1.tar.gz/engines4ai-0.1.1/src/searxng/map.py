...
from searxng.searxng import SearXNG
...
def map_engine(query: str, pageno: int = 1) -> List[MapData]:
    engine = SearXNG()

    try:
        results = engine.search(query=query, categories="map", pageno=pageno)["results"]
        maps = []
