...
@deprecated("This class will be removed in a future version. Use ConfigDict instead.")
@dataclass
class Config:
    """
...
class Maxim:
    ...
    def __init__(self, config: Union[Config, ConfigDict] = Config()):
        ...
        """
        # ASCII art for MAXIM AI logo
        self.ascii_logo = "[MaximSDK] Initializing Maxim AI(v3.5.5)"
    ...
    def __sync_entities(self):
        logging.debug("[MaximSDK] Syncing prompts and folders")
        if not self.prompt_management:
            return
        ...
        self.__syncFolders()
        logging.debug("[MaximSDK] Syncing completed")

    def __sync_prompts(self):
        logging.debug("[MaximSDK] Syncing prompts")
        try:
            prompts = self.maxim_api.get_prompts()
            logging.debug(f"[MaximSDK] Found {len(prompts)} prompts")
            for prompt in prompts:
                try:
                    ...
                    logging.error(f"[MaximSDK] Error while syncing prompts: {err}")
    ...
    def __sync_prompt_chains(self):
        logging.debug("[MaximSDK] Syncing prompt Chains")
        try:
            prompt_chains = self.maxim_api.get_prompt_chains()
            logging.debug(f"[MaximSDK] Found {len(prompt_chains)} prompt chains")
    ...
    def __syncFolders(self):
        logging.debug("[MaximSDK] Syncing folders")
        try:
            folders = self.maxim_api.get_folders()
            logging.debug(f"[MaximSDK] Found {len(folders)} folders")
            for folder in folders:
                try:
                    ...
                    logging.error(f"[MaximSDK] Error while syncing folders: {err}")
                    if self.raise_exceptions:
                        raise err
        except Exception as err:
            logging.error(f"[MaximSDK] Error while syncing folders: {err}")
    ...
    def cleanup(self):
        ...
        time.sleep(2)
        logging.debug("[MaximSDK] Cleaning up Maxim sync thread")
        self.is_running = False
        for logger in self.__loggers.values():
            logger.cleanup()
        logging.debug("[MaximSDK] Cleanup done")
