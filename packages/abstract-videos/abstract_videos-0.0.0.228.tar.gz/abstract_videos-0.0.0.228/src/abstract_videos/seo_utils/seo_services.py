...
def eatAllQuotes(string):
    """Remove all types of quotes from a string."""
    return eatAll(string, ['"', "'", '`'])

def get_seo_description(description=None, keywords=None, keyword_length=None, desc_length=None):
    """Generate an SEO-friendly description without quotes."""
    keyword_length = keyword_length or 3
    desc_length = desc_length or 300
    
    # Ensure description is clean of quotes
    description = eatAllQuotes(description) if description else ""
    
    # Format keywords as a clean, comma-separated string
    if isinstance(keywords, list):
        keywords_str = ', '.join(keywords[:keyword_length])
    ...
    seo_desc = f"{description} Explore {keywords_str}".strip()
    
    # Truncate to desc_length and remove any remaining quotes
    seo_description = eatAllQuotes(get_from_list(seo_desc, length=desc_length))
...
def get_title_tags_description(title=None, keywords=None, summary=None, filename=None, title_length=None, summary_length=None, keyword_length=None, desc_length=None, description=None):
    """Generate SEO title, keywords string, description, and tags with clean formatting."""
    summary_length = summary_length or 150
    keyword_length = keyword_length or 3
    title_length = title_length or 70
    desc_length = desc_length or 300
    
    # Use description or summary, clean of quotes
    summary_desc = eatAllQuotes(get_from_list(description or summary, length=summary_length))
    
    # Generate SEO title
    seo_title = eatAllQuotes(get_seo_title(title=title, keywords=keywords, filename=filename, title_length=title_length, description=description))
    ...
    seo_description = get_seo_description(description=summary_desc, keywords=keywords, keyword_length=keyword_length, desc_length=desc_length)
    
    # Generate SEO tags, excluding generic terms
    seo_tags = [kw for kw in (keywords if isinstance(keywords, list) else []) if kw.lower() not in ['video', 'audio', 'file']]
