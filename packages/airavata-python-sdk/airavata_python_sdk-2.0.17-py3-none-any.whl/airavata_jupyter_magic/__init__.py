...
def list_remote_files(remot_rt: str, remot_path: str) -> list[str]:
    """
    List files in a remote runtime
...
def copy_data(line: str):
    ...
    if source_runtime == "local":
        ...
        source_matches = list_remote_files(source_runtime, source_path)
        for source_file in source_matches:
            target_file = Path(target_path) / source_file
            os.makedirs(target_file.parent.as_posix(), exist_ok=True)
            pull_remote(source_runtime, source_file, target_file.as_posix())
    else:
