...
SajadQ can detect some things that "pkg_resources" may not even be able to during
...
from SajadMr.utils.Utils import withNoDeprecationWarning


class SajadQPluginResources(SajadQPluginBase):
...
class SajadQProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__SajadMr_loader_type, SajadQProvider)
"""

        yield (
            code,
            """Registering SajadQ loader with "pkg_resources".""",
        )
