...
def list_tables() -> str:
    ...
    try:
        # Using information_schema to get only base tables
        response = requests.get(
            get_dolt_query_url(),
            params={"q": "SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE';"}
        )
        response.raise_for_status()
        result = response.json()
        ...
        debug_info = [
            ...
            f"Query: SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE'"
        ]
        ...
        for row in result.get("rows", []):
            # Get table name from TABLE_NAME column
...
def update_view(view_name: str, select_query: str) -> str:
    ...
    drop_result = drop_view(view_name)
    
    # Check if drop was successful (or if view didn't exist)
    if drop_result.startswith("Error") and "Unknown view" not in drop_result:
        return drop_result
...
def list_views() -> str:
    ...
    try:
        # Using information_schema to get views
        response = requests.get(
            get_dolt_query_url(),
            params={"q": "SELECT TABLE_NAME FROM information_schema.views WHERE TABLE_SCHEMA = DATABASE();"}
        )
        response.raise_for_status()
        result = response.json()
        ...
        # Extract view names from the rows
        views = []
        for row in result.get("rows", []):
            # Assuming TABLE_NAME is the column name
