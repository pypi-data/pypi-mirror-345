...
class LRQ600():
    """
    Class to handle the Zaber LRQ600 XY stage.
    ...
    def getPos(self):
        ...
        if not successx or not successy:
            ...
            return self.getUserCoords(np.array([self.toDistance(int(replyx.data)),0.601/self.scale-self.toDistance(int(replyy.data))]))
    ...
    def moveTo(self,pos):
        ...
        [successy,reply] = self.execute(self.yAxis,self.cmd["moveTo"]+"%d"%self.toMicrosteps(0.601/self.scale-coords[1]))
        if not successx or not successy:
