...
from relationalai.early_access.dsl.core.constraints.scalar import reduce_constraints, unsatisfiable
...
class ConstrainedValueType(AbstractValueType, ScalarExprBuilder, LogicBuilder):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        constraints = self.denotes()
        if unsatisfiable(constraints):
            raise Exception(f"Value type {self.name()} has an unsatisfiable set of type constraints")
    ...
    def constraint_display(self):
        ...
        constraints = [f"   {c.pprint()}" for c in self.verifies().values()]
        if exists_ppr != "":
            return exists_ppr + "\n" + "\n".join(constraints)
        else:
            return "\n".join(constraints)
    ...
    def contains(self, val):
        ...
        if self.signature_arity() != 1:
            raise Exception(
                f"NOT IMPLEMENTED: Check containment of values in ({self.name()}) which is not unary")
    ...
    def datatype(self):
        if not self.unary():
            raise Exception(
                f"Cannot infer an ORM datatype from AbstractValueType {self.name()}, which is not a "
    ...
    def interpret_type_atom(self, ac):
        ...
        if len(args) > 1:
            raise Exception(f"Cannot apply ValueSubtype constraint ({T.name()}) to more than one TypeVariable")
        ...
        if isinstance(tv, BasicTypeVariable):
            if T.nominal():
                raise Exception(
                        f"Argument mismatch when using type {T.name()} as an AtomicConstraint in context of ValueType {self.name()}")

            if T.root_unconstrained_type() != tv.decl.root_unconstrained_type():
                raise Exception(
                    f"Applying type constraint {T.name()} to variable of incompatible unconstrained "
            ...
            for i in range(len(ttup)):
                if not isinstance(tvtup[i], TypeVariable):
                    raise Exception(
                        f"When using type {T.name()} as an AtomicConstraint in context of ValueType {self.name()}, {i}-th "
                ...
                if tup_basic_type != tvtup_basic_type:
                    raise Exception(
                        f"Applying basic type constraint {T.name()} to variable of incompatible unconstrained "
                        f"type {tvtup_basic_type.pprint()}")

                vmap[id(ttup[i])] = tvtup[i]

        rv = {}
        constraints = T.verifies()
    ...
    def value(self):
        if self.signature_arity() != 1:
            raise Exception(
                f"Cannot refer to singleton value type variable for non-unary AbstractValueType {self.name()}")
        else:
            return self.typevar_tuple()

    # Returns that subset of those ScalarConstraints that this ConstrainedValueType
    # denotes that we actually need to verify to ensure that values in its signature
    # satisfy the constraints.
    #
    def verifies(self):
        return reduce_constraints(self.denotes())
