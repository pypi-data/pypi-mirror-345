...
def bf16vf16(size=1024, perf_threshold= 1.5, runs=5):
    ...
    bfok = (bf16_time <= f32_time * perf_threshold)
    print(f"HLO check passed?         {hlo_ok}")
    print(f"BF16/FP32 time ratio:     {bf16_time / f32_time:.2f}  (≤ {perf_threshold})")
...
def load(model_name='jaco-bro/Dia-1.6B', dtype='float16'):
...
def main():
    ...
    model = load(args.model, dtype=dtype)
