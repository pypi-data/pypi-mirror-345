...
def extract_one(proxy_uri, vid, height):
    ...
    audio_fmt = next((f for f in fmts if f.get("acodec","")!="none"), None)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=len(proxies)) as pool:
        futures = {
            pool.submit(extract_one, p, args.video_id, args.height): p
            for p in proxies
        }
    ...
    print("Error: Failed to fetch any valid URL pair", file=sys.stderr)
    sys.exit(1)
