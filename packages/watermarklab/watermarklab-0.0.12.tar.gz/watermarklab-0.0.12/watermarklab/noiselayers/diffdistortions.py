...
__all__ = ["Identity", "GaussianBlur", "MedianFilter",
           ...
           "Dropout", "Cropout", "Crop", "Contrast",
           "Hue", "Brightness", "Saturation", "Resize",
           "Rotate", "UnsharpMasking", "MotionBlur",
...
class SpeckleNoise(BaseDiffNoiseModel):
    """
...
class ColorQuantization(BaseDiffNoiseModel):
    """
    ...
    def __init__(self, factor: int = 16, max_step: int = 1000, noisename: str = "ColorQuantization"):
        ...
        """
        super().__init__(noisename)  # Assuming BaseDiffNoiseModel inherits from nn.Module
...
class ChromaticAberration(BaseDiffNoiseModel):
    """
...
class GammaCorrection(BaseDiffNoiseModel):
    """
...
class PoissonNoise(BaseDiffNoiseModel):
    """
...
class MotionBlur(BaseDiffNoiseModel):
    """
    ...
    - max_step: int - Maximum number of steps for dynamic factor scaling.
    - noisename: str - Name of the noise model (inherited from BaseDiffNoiseModel).
    """
    ...
    def test(self, stego_img: Tensor, cover_img: Tensor = None, factor=None) -> Tensor:
        """
        Core computation for motion blur with a specified kernel size.
        ...
        if factor is None:
            factor = self.factor
        # Ensure factor is an odd integer for valid kernel size
        ...
        kernel[kernel_size // 2, :] = 1.0 / kernel_size  # Horizontal line, normalized
        kernel = kernel.view(1, 1, kernel_size, kernel_size)  # Shape: (1, 1, K, K)
        kernel = kernel.expand(stego_img.shape[1], 1, kernel_size, kernel_size)  # Shape: (C, 1, K, K)
        ...
        # Clip values to the valid range [0, 1] (differentiable operation)
        noised_img = torch.clamp(noised_img, 0, 1)
...
class UnsharpMasking(BaseDiffNoiseModel):
...
class Crop(BaseDiffNoiseModel):
    ...
    def test(self, marked_img: Tensor, cover_img: Tensor = None, crop_ratio: float = None) -> Tensor:
        ...
        if self.mode == "cover_pad" and cover_img is not None:
            noised_img = marked_img * crop_mask + (1 - crop_mask) * cover_img  # Use cover_img for padding
        else:
            noised_img = marked_img * crop_mask + (1 - crop_mask) * self.constant  # Use constant value for padding
    ...
    def random_cropout_mask(self, marked_img: Tensor, crop_ratio: float) -> Tensor:
        ...
        mask = torch.zeros((N, C, H, W), dtype=torch.float32, device=marked_img.device)  # Initialize the mask with ones
        crop_ratio = 1 - crop_ratio
        for i in range(N):
            num_pixels = int(H * W * crop_ratio)
            width = random.randint(num_pixels // H, W)
            height = num_pixels // width
            rect_w = random.randint(0, W - width)
            rect_h = random.randint(0, H - height)
            mask[i, :, rect_h:rect_h + height, rect_w:rect_w + width] = 1.0
...
class Cropout(BaseDiffNoiseModel):
    ...
    def random_cropout_mask(self, marked_img: Tensor, remain_ratio: float) -> Tensor:
        """
        Generate a mask for Cropout operation, where a rectangular region in the middle of the image is cropped out.
        ...
        Args:
            marked_img (Tensor): Input tensor of shape (N, C, H, W), where N is the batch size,
                                 C is the number of channels, H is the height, and W is the width.
            remain_ratio (float): The ratio of the image area that should remain visible.
        ...
        for i in range(N):
            num_pixels = int(H * W * remain_ratio)
            width = random.randint(num_pixels // H, W)
            height = num_pixels // width
            rect_w = random.randint(0, W - width)
            rect_h = random.randint(0, H - height)
...
class JpegMask(BaseDiffNoiseModel):
...
class JpegPolynomial(BaseDiffNoiseModel):
...
class JpegFourier(BaseDiffNoiseModel):
