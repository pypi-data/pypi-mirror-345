...
class GenericCRUD(metaclass=MetaLogger, object=LOGGER_CRUD_CODE_OBJECT):
    ...
    def __init__(self, *, default_schema_name: str,
        ...
        self.default_column_name = default_column_name or generate_id_column_name(self.default_table_name)
    ...
    def insert(self, *, schema_name: str = None, table_name: str = None, data_dict: dict = None,
        ...
        try:
            ...
            if commit_changes:
                self.connection.commit()  # TODO: test if it works when schema_name != self.default_schema_name
    ...
    def upsert_with_select_clause(self, *, schema_name: str = None, table_name: str = None, view_table_name: str = None,
        ...
        result_dict = {}
        if not data_dict:
            self.logger.warning(log_message="GenericCRUD.upsert_with_select_clause: data_dict is empty")
            return result_dict
    ...
    # TODO Why we need to send "select_clause_value" if we provided "default_select_clause_value" in the constructor. Added "= None" to try and resolve it.
    ...
    def get_test_entity_id(self, *, entity_name: str = None, insert_function: callable, insert_kwargs: dict = None,
        ...
        test_entity_id = self.select_one_value_by_column_and_value(
            ...
            , order_by=select_clause_value ) # To bring alway the 1st is_test_data so the test can relay on the value.
    ...
    def create_column(self, schema_name: str, table_name: str, column_name:str, data_type: str, after_column:str = None, default_value: str = None):
        query = 'ALTER TABLE ' + schema_name + "." + table_name + " ADD COLUMN " + column_name + " " + data_type + " NULL"
    ...
    def foreach(self, where, limit, function, id_column_name, select_function: callable = None, **kwargs):
        ...
        result = select_function(where=where, limit=limit, select_clause_value="*")  # select_clause_value="test_mysql_id, number, name") # noqa
        ...
        for r in result:
            # print(f"normal r:{r}")
            r_our = OurObject(id_column_name=id_column_name, **r)
            kwargs["object"] = r_our

            # print(f"our r:{r_our}")
            # response = function(r_our)
            response = function(**kwargs)
            responses_dict[id_column_name] = response
    ...
    def get_view_name(self, schema_name: str) -> str:
        ...
        if self.default_view_with_deleted_and_test_data:
            ...
            view_table_name = replace_view_with_table(view_table_name=view_table_name)  # select_clause_value=select_clause_value)
