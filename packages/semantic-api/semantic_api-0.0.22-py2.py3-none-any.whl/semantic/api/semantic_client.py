...
class semanticClient:
  ...
  async def query_primitive(self, args: PipeArg):
      stringified= json.dumps(args)
      msg('query_primitive args: '+json.dumps(args, indent=6))
  ...
  async def createDbForSchemaId(self):
    result = await self.semantic_config('createDbForSchemaId', None)
    greenMsg('createDbForSchemaId: '+json.dumps(result, indent=6))
    return result.get('doc')

  async def schemaQuery(self):
    result = await self.semantic_config('schemaQuery', None)
    greenMsg('createDbForSchemaId: '+json.dumps(result, indent=6))
    return result.get('doc')
 
  async def insertFromServerPath(self, path: str):
              
    greenMsg("insert_from_sever_path topic:   "+service_request_topic(getDbId(self.schemaId)))

    '''
        note this is considered a service request, not a config request
    '''
    pipe_args: PipeArg = {
      "schema": self.schemaId,
      "topic": service_request_topic(getDbId(self.schemaId)),
      "doc": {"@type": "insertFromServerPath"},
      "path": path,
      "frame": None 
    }

    result = await self.query_primitive(pipe_args)
    greenMsg('insertFromServerPath: '+json.dumps(result, indent=6))
    return result.get('doc')
