...
def manual_overview(
    ...
    """
    Build an overview prompt (directory tree + file contents).

    Args:
        focus_directories:
            Paths you want to scan.  If *None* or empty, the entire
            *project_root* is scanned.

        manual_ignore_files:
            Extra filename or glob patterns to ignore in addition to the
            defaults in ``atlaz.old_overview.ignore_utils.append_default_ignores``.

        project_root:
            The root of the project you want to scan.  Defaults to the current
            working directory.  (Useful when your code is run from a sub-dir
            such as ``tests/``.)

    Returns:
        A single string you can feed straight to an LLM.
    """
    # 1. Decide what we consider “project root”.
    project_root = Path(project_root) if project_root else Path.cwd()

    # 2. Decide which directories to walk.
    if not focus_directories:              # catches None *and* []
        focus_directories = [str(project_root)]

    manual_ignore_files = manual_ignore_files or []

    # 3. Fabricate a fake script path *inside* the project so that
    #    script_path.parent.parent.parent  ==>  project_root
    fake_script_path = (
        project_root / "__atlaz_dummy__" / "__atlaz_dummy__" / "__atlaz_dummy__.py"
    )
