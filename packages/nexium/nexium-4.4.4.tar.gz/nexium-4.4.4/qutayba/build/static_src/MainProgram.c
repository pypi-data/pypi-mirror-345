...
#define DEVILPY_MAIN_MODULE_NAME "__main__"
#define DEVILPY_MAIN_IS_PACKAGE_BOOL false
#define _DEVILPY_ATTACH_CONSOLE_WINDOW 1
#if defined(__APPLE__)
#define _DEVILPY_MACOS_BUNDLE_MODE 1
#endif
#endif

// It doesn't work for MinGW64 to update the standard output handles early on,
// so make a difference here.
#if defined(DEVILPY_FORCED_STDOUT_PATH) || defined(DEVILPY_FORCED_STDERR_PATH)
#if defined(__MINGW64__) || defined(__MINGW32__)
#define DEVILPY_STANDARD_HANDLES_EARLY 0
#else
#define DEVILPY_STANDARD_HANDLES_EARLY 1
#endif
#else
#define DEVILPY_STANDARD_HANDLES_EARLY 0
#endif

#if defined(_WIN32) && (defined(_DEVILPY_ATTACH_CONSOLE_WINDOW) || defined(_DEVILPY_HIDE_CONSOLE_WINDOW))
#include "HelpersConsole.c"
#endif

// We are open to having this defined otherwise, this is a default only.
#if defined(_WIN32) && defined(DEVILPY_COMPANY_NAME) && defined(DEVILPY_PRODUCT_NAME) &&                                 \
    !defined(DEVILPY_APP_MODEL_USER_ID)
#define DEVILPY_APP_MODEL_USER_ID L"" DEVILPY_COMPANY_NAME L"." DEVILPY_PRODUCT_NAME
#endif

#if defined(_WIN32) && defined(DEVILPY_APP_MODEL_USER_ID)
...
static void prepareFrozenModules(void) {
    ...
    struct _frozen *merged =
        (struct _frozen *)malloc(sizeof(struct _frozen) * (_DEVILPY_FROZEN + pre_existing_count + 1));
...
static void prepareStandaloneEnvironment(void) {
    ...
    DEVILPY_PRINTF_TRACE("main(): Binary dir is %s\n", binary_directory);
...
static void restoreStandaloneEnvironment(void) {
    ...
    // DEVILPY_PRINTF_TRACE("Final PySys_GetPath is 's'.\n", PySys_GetPath());
#elif PYTHON_VERSION < 0x370
    PySys_SetPath(getBinaryDirectoryWideChars(true));
    Py_SetPath(getBinaryDirectoryWideChars(true));
    // DEVILPY_PRINTF_TRACE("Final Py_GetPath is '%ls'.\n", Py_GetPath());
#endif

#ifdef _DEVILPY_EXPERIMENTAL_DUMP_PY_PATH_CONFIG
...
#if PYTHON_VERSION >= 0x300 && _DEVILPY_NATIVE_WCHAR_ARGV == 0
...
static void PRINT_REFCOUNTS(void) {
    ...
    PRINT_FORMAT("Compiled Functions: %d | %d | %d (module/class ownership may occur)\n",
                 count_active_nexium_Function_Type, count_allocated_nexium_Function_Type,
                 count_released_nexium_Function_Type);
    PRINT_FORMAT("Compiled Generators: %d | %d | %d\n", count_active_nexium_Generator_Type,
                 count_allocated_nexium_Generator_Type, count_released_nexium_Generator_Type);
#if PYTHON_VERSION >= 0x350
    PRINT_FORMAT("Compiled Coroutines: %d | %d | %d\n", count_active_nexium_Coroutine_Type,
                 count_allocated_nexium_Coroutine_Type, count_released_nexium_Coroutine_Type);
    PRINT_FORMAT("Compiled Coroutines Wrappers: %d | %d | %d\n", count_active_nexium_CoroutineWrapper_Type,
                 count_allocated_nexium_CoroutineWrapper_Type, count_released_nexium_CoroutineWrapper_Type);

    PRINT_FORMAT("Compiled Coroutines AIter Wrappers: %d | %d | %d\n", count_active_nexium_AIterWrapper_Type,
                 count_allocated_nexium_AIterWrapper_Type, count_released_nexium_AIterWrapper_Type);
#endif
#if PYTHON_VERSION >= 0x360
    PRINT_FORMAT("Compiled Asyncgen: %d | %d | %d\n", count_active_nexium_Asyncgen_Type,
                 count_allocated_nexium_Asyncgen_Type, count_released_nexium_Asyncgen_Type);
    PRINT_FORMAT("Compiled Asyncgen Wrappers: %d | %d | %d\n", count_active_nexium_AsyncgenValueWrapper_Type,
                 count_allocated_nexium_AsyncgenValueWrapper_Type, count_released_nexium_AsyncgenValueWrapper_Type);
    PRINT_FORMAT("Compiled Asyncgen Asend: %d | %d | %d\n", count_active_nexium_AsyncgenAsend_Type,
                 count_allocated_nexium_AsyncgenAsend_Type, count_released_nexium_AsyncgenAsend_Type);
    PRINT_FORMAT("Compiled Asyncgen Athrow: %d | %d | %d\n", count_active_nexium_AsyncgenAthrow_Type,
                 count_allocated_nexium_AsyncgenAthrow_Type, count_released_nexium_AsyncgenAthrow_Type);
#endif

    PRINT_FORMAT("Compiled Frames: %d | %d | %d (cache usage may occur)\n", count_active_nexium_Frame_Type,
                 count_allocated_nexium_Frame_Type, count_released_nexium_Frame_Type);
    PRINT_FORMAT("Compiled Cells: %d | %d | %d (function ownership may occur)\n", count_active_nexium_Cell_Type,
                 count_allocated_nexium_Cell_Type, count_released_nexium_Cell_Type);
...
static int HANDLE_PROGRAM_EXIT(PyThreadState *tstate) {
    ...
    if (HAS_ERROR_OCCURRED(tstate)) {
        ...
        while (tstate->curexc_traceback) {
            ...
            if (0 == strcmp(PyUnicode_AsUTF8(nexium_Frame_GetCodeObject(frame)->co_filename),
...
#endif
        DEVILPY_FINALIZE_PROGRAM(tstate);
...
static PyObject *EXECUTE_MAIN_MODULE(PyThreadState *tstate, char const *module_name, bool is_package) {
    DEVILPY_INIT_PROGRAM_LATE(module_name);
...
#if _DEVILPY_PLUGIN_WINDOWS_SERVICE_ENABLED
#include "qutayba_windows_service.h"

// Callback from Windows Service logic.
void SvcStartPython(void) {
    PyThreadState *tstate = PyThreadState_GET();

    EXECUTE_MAIN_MODULE(tstate, DEVILPY_MAIN_MODULE_NAME, DEVILPY_MAIN_IS_PACKAGE_BOOL);

    DEVILPY_PRINT_TIMING("SvcStartPython() Python exited.")

    int exit_code = HANDLE_PROGRAM_EXIT(tstate);

    // TODO: Log exception and call ReportSvcStatus

    DEVILPY_PRINT_TIMING("SvcStartPython(): Calling Py_Exit.");
    Py_Exit(exit_code);
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (i == 1) {
            ...
            if (strcmpFilename(argv[i], FILENAME_EMPTY_STR "install") == 0) {
                DEVILPY_PRINT_TRACE("main(): Calling plugin SvcInstall().");

                SvcInstall();
                DEVILPY_CANNOT_GET_HERE("main(): SvcInstall must not return");
            }
#endif
        }
...
#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_SELF_EXECUTION)
...
#if defined(_DEVILPY_ONEFILE_MODE) && defined(_WIN32)

static long onefile_ppid;

DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    DEVILPY_PRINT_TRACE("Onefile parent monitoring starts.");
    ...
    DEVILPY_PRINT_TRACE("Onefile parent monitoring causes KeyboardInterrupt.");
    PyErr_SetInterrupt();

    Sleep(_DEVILPY_ONEFILE_CHILD_GRACE_TIME_INT);

    DEVILPY_PRINT_TRACE("Onefile parent monitoring hard kills after ignored KeyboardInterrupt.");
    ExitProcess(1);
...
#if DEVILPY_FORCED_STDOUT_NULL_BOOL
    if (strcmp(name, "stdout") == 0) {
        return true;
    }
#endif

#if DEVILPY_FORCED_STDERR_NULL_BOOL
    if (strcmp(name, "stderr") == 0) {
        return true;
    }
#elif defined(DEVILPY_FORCED_STDERR_PATH) || defined(DEVILPY_FORCED_STDERR_NONE_BOOL)
    if (strcmp(name, "stderr") == 0) {
        return false;
    }
#endif

    PyObject *sys_std_handle = nexium_SysGetObject(name);
...
static void setStdinHandle(PyThreadState *tstate, PyObject *stdin_file) {
    ...
    nexium_SysSetObject("stdin", stdin_file);
...
static void setStdoutHandle(PyThreadState *tstate, PyObject *stdout_file) {
    ...
    nexium_SysSetObject("stdout", stdout_file);
...
static void setStderrHandle(PyThreadState *tstate, PyObject *stderr_file) {
    ...
    nexium_SysSetObject("stderr", stderr_file);
...
#if DEVILPY_STANDARD_HANDLES_EARLY == 0
#if defined(DEVILPY_FORCED_STDOUT_PATH) || defined(DEVILPY_FORCED_STDERR_PATH)
#ifdef _WIN32
static PyObject *getExpandedTemplatePath(wchar_t const *template_path) {
    ...
    return nexiumUnicode_FromWideChar(filename_buffer, -1);
}
#else
static PyObject *getExpandedTemplatePath(char const *template_path) {
    ...
    return nexium_String_FromString(filename_buffer);
}
#endif
#endif
#endif
...
// reason.
#if DEVILPY_STANDARD_HANDLES_EARLY == 1 && PYTHON_VERSION >= 0x370
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Early handles.");
#if defined(DEVILPY_FORCED_STDOUT_PATH) || defined(DEVILPY_FORCED_STDERR_PATH)
    PyObject *args = MAKE_DICT_EMPTY(tstate);

    DICT_SET_ITEM(args, const_str_plain_encoding, nexium_String_FromString("utf-8"));
    DICT_SET_ITEM(args, const_str_plain_line_buffering, Py_True);

#if defined(DEVILPY_FORCED_STDOUT_PATH)
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Forced stdout update.");
    {
        PyObject *sys_stdout = nexium_SysGetObject("stdout");
...
#if defined(DEVILPY_FORCED_STDERR_PATH)
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Forced stderr update.");
    {
        PyObject *sys_stderr = nexium_SysGetObject("stderr");
    ...
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Done with early handles.");
#endif

#if DEVILPY_STANDARD_HANDLES_EARLY == 0
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Late handles.");
#if defined(DEVILPY_FORCED_STDOUT_PATH)
    {
#if defined(_WIN32)
        PyObject *filename = getExpandedTemplatePath(L"" DEVILPY_FORCED_STDOUT_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(DEVILPY_FORCED_STDOUT_PATH);
#endif
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" DEVILPY_FORCED_STDERR_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(DEVILPY_FORCED_STDERR_PATH);
#endif
    ...
    {
#if defined(_WIN32)
        PyObject *devnull_filename = nexium_String_FromString("NUL:");
#else
        PyObject *devnull_filename = nexium_String_FromString("/dev/null");
#endif

        DEVILPY_PRINT_TRACE("setInputOutputHandles(): Considering stdin.");

        if (shallSetOutputHandleToNull("stdin")) {
            DEVILPY_PRINT_TRACE("setInputOutputHandles(): Set stdin to NULL file.");
        ...
        DEVILPY_PRINT_TRACE("setInputOutputHandles(): Considering stdout.");

        if (shallSetOutputHandleToNull("stdout")) {
            DEVILPY_PRINT_TRACE("setInputOutputHandles(): Set stdout to NULL file.");
        ...
        DEVILPY_PRINT_TRACE("setInputOutputHandles(): Considering stderr.");

        if (shallSetOutputHandleToNull("stderr")) {
            DEVILPY_PRINT_TRACE("setInputOutputHandles(): Set stderr to NULL file.");
...
#if DEVILPY_FORCED_STDOUT_NONE_BOOL
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Forcing stdout to None.");
    setStdoutHandle(tstate, Py_None);
#endif

#if DEVILPY_FORCED_STDERR_NONE_BOOL
    DEVILPY_PRINT_TRACE("setInputOutputHandles(): Forcing stderr to None.");
...
static void nexium_Py_Initialize(void) {
#if PYTHON_VERSION > 0x350 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_ALLOCATORS)
    initnexiumAllocators();
#endif

#if PYTHON_VERSION < 0x380 || defined(_DEVILPY_EXPERIMENTAL_OLD_PY_INITIALIZE)
    Py_Initialize();
#else
    ...
    DEVILPY_MAY_BE_UNUSED _PyRuntimeState *runtime = &_PyRuntime;
    ...
    // Need to disable frozen modules, nexium can handle them better itself.
...
// too.
    ...
    DEVILPY_PRINT_TIMING("nexium_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
// short lived.
#if SYSFLAG_ISOLATED
    nexium_SysSetObject("path", PyList_New(0));
#endif
#endif
...
#if DEVILPY_STANDARD_HANDLES_EARLY == 1
#if defined(_WIN32)
...
static void nexium_at_exit(void) { DEVILPY_PRINT_TIMING("nexium_at_exit(): Called by C exit()"); }
#endif

#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_SEGFAULT)
#include <signal.h>
static void qutayba_segfault_handler(int sig) {
    puts("nexium: A segmentation fault has occurred. This is highly unusual and can");
    puts("have multiple reasons. Please check https://qutayba.net/info/segfault.html");
    puts("for solutions.");
...
#if _DEVILPY_NATIVE_WCHAR_ARGV == 1
...
PyObject *getOriginalArgv0Object(void) {
    ...
    return nexium_String_FromFilename(original_argv0);
}
...
static int nexium_Main(int argc, native_command_line_argument_t **argv) {
#if defined(_DEVILPY_HIDE_CONSOLE_WINDOW)
    hideConsoleIfSpawned();
#endif

    // Installer a segfault handler that outputs a helpful message.
#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, qutayba_segfault_handler);
#endif

#ifdef _DEVILPY_EXPERIMENTAL_DUMP_C_TRACEBACKS
    INIT_C_BACKTRACES();
    DUMP_C_BACKTRACE();
#endif
    // Trace when the process exits.
#if defined(_DEVILPY_EXPERIMENTAL_SHOW_STARTUP_TIME)
    atexit(nexium_at_exit);
#endif

    // Attach to the parent console respecting redirection only, otherwise we
    // cannot even output traces.
#if defined(_WIN32) && defined(_DEVILPY_ATTACH_CONSOLE_WINDOW)
    inheritAttachedConsole();
#endif

#if defined(_WIN32) && defined(DEVILPY_APP_MODEL_USER_ID)
    setCurrentProcessExplicitAppUserModelID(DEVILPY_APP_MODEL_USER_ID);
#endif

// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _DEVILPY_NATIVE_WCHAR_ARGV == 0
    original_argv0 = argv[0];
#if !defined(_DEVILPY_ONEFILE_MODE)
...
#if defined(DEVILPY_FORCED_STDOUT_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_OUTPUT_HANDLE, stdout, L"" DEVILPY_FORCED_STDOUT_PATH);
#else
    changeStandardHandleTarget(stdout, DEVILPY_FORCED_STDOUT_PATH);
#endif
#endif
#if defined(DEVILPY_FORCED_STDERR_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_ERROR_HANDLE, stderr, L"" DEVILPY_FORCED_STDERR_PATH);
#else
    changeStandardHandleTarget(stderr, DEVILPY_FORCED_STDERR_PATH);
#endif
#endif
#if defined(DEVILPY_FORCED_STDIN_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_INPUT_HANDLE, stdin, L"" DEVILPY_FORCED_STDIN_PATH);
#else
    changeStandardHandleTarget(stdin, DEVILPY_FORCED_STDIN_PATH);
#endif
#endif
#endif
    ...
    DEVILPY_PRINT_TIMING("main(): Entered.");
    DEVILPY_INIT_PROGRAM_EARLY(argc, argv);
...
#if _DEVILPY_STANDALONE_MODE
    DEVILPY_PRINT_TIMING("main(): Prepare standalone environment.");
    prepareStandaloneEnvironment();
#endif

#if _DEVILPY_FROZEN > 0
    DEVILPY_PRINT_TIMING("main(): Preparing frozen modules.");
...
#ifdef DEVILPY_PYTHON_STATIC
    DEVILPY_PRINT_TIMING("main(): Preparing static modules.");
...
#if defined(_DEVILPY_ONEFILE_MODE)
    {
        environment_char_t const *parent_original_argv0 = getEnvironmentVariable("DEVILPY_ORIGINAL_ARGV0");
        ...
        if (parent_original_argv0 != NULL) {
            ...
            unsetEnvironmentVariable("DEVILPY_ORIGINAL_ARGV0");
        }
    }
#endif

#if PYTHON_VERSION >= 0x300 && _DEVILPY_NATIVE_WCHAR_ARGV == 0
    DEVILPY_PRINT_TRACE("main(): Calling convertCommandLineParameters.");
    orig_argv = convertCommandLineParameters(argc, argv);
#elif PYTHON_VERSION < 0x300 && _DEVILPY_NATIVE_WCHAR_ARGV == 1
    DEVILPY_PRINT_TRACE("main(): Calling getCommandLineToArgvA.");
...
// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _DEVILPY_NATIVE_WCHAR_ARGV == 1
    original_argv0 = argv[0];
#if PYTHON_VERSION >= 0x300 && !defined(_DEVILPY_ONEFILE_MODE)
    ...
    DEVILPY_PRINT_TRACE("main(): Calling setCommandLineParameters.");
    setCommandLineParameters(argc, argv);

    /* For Python installations that need the home set, we inject it back here. */
#if defined(PYTHON_HOME_PATH)
#if PYTHON_VERSION < 0x300
    DEVILPY_PRINT_TRACE("main(): Prepare run environment '" PYTHON_HOME_PATH "'.");
    Py_SetPythonHome(PYTHON_HOME_PATH);
#else
    DEVILPY_PRINTF_TRACE("main(): Prepare run environment '%S'.\n", L"" PYTHON_HOME_PATH);
    ...
    DEVILPY_PRINT_TRACE("main(): Disabling Python warnings.");
    {
#if PYTHON_VERSION >= 0x300
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    DEVILPY_PRINT_TIMING("main(): Calling nexium_Py_Initialize to initialize interpreter.");
    nexium_Py_Initialize();

    PyThreadState *tstate = PyThreadState_GET();

#if _DEVILPY_STANDALONE_MODE
    DEVILPY_PRINT_TRACE("main(): Restore standalone environment.");
    restoreStandaloneEnvironment();
#else
    {
        environment_char_t const *python_path_cstr = getEnvironmentVariable("DEVILPY_PYTHONPATH");

        if (python_path_cstr != NULL) {
            PyObject *python_path_str = nexium_String_FromFilename(python_path_cstr);
#ifdef _WIN32
            ...
            nexium_SysSetObject("path", python_path_list);

            unsetEnvironmentVariable("DEVILPY_PYTHONPATH");
        }
    }
#endif
    ...
    PyList_SetSlice(nexium_SysGetObject("path"), 0, 1, PyTuple_New(0));
#endif

    /* Initialize the built-in module tricks used and builtin-type methods */
    DEVILPY_PRINT_TRACE("main(): Calling _initBuiltinModule().");
    _initBuiltinModule();

    /* Initialize the Python constant values used. This also sets
     * "sys.executable" while at it.
     */
    DEVILPY_PRINT_TIMING("main(): Calling createGlobalConstants().");
    createGlobalConstants(tstate);
    DEVILPY_PRINT_TIMING("main(): Returned createGlobalConstants().");

    /* Complex call helpers need "__main__" constants, even if we only
     * go into "__parents__main__" module as a start point.
     */
    DEVILPY_PRINT_TIMING("main(): Calling createMainModuleConstants().");
    createMainModuleConstants(tstate);
    DEVILPY_PRINT_TIMING("main(): Returned createMainModuleConstants().");

    DEVILPY_PRINT_TRACE("main(): Calling _initBuiltinOriginalValues().");
...
#if PYTHON_VERSION < 0x300
    PyStructSequence_SET_ITEM(nexium_SysGetObject("flags"), 9, const_int_0);
#else
    PyStructSequence_SetItem(nexium_SysGetObject("flags"), 6, const_int_0);
#endif
#endif

    /* Initialize the compiled types of nexium. */
    ...
    DEVILPY_PRINT_TRACE("main(): Calling enhancePythonTypes().");
    enhancePythonTypes();

    DEVILPY_PRINT_TRACE("main(): Calling patchTypeComparison().");
    patchTypeComparison();

    DEVILPY_PRINT_TRACE("main(): Calling patchTracebackDealloc().");
    patchTracebackDealloc();

#ifndef DEVILPY_USE_PYCORE_THREAD_STATE
    /* Allow to override the ticker value, to remove checks for threads in
     * CPython core from impact on benchmarks. */
    char const *ticker_value = getenv("DEVILPY_TICKER");
    ...
    if (needs_stdout_attaching) {
        PyObject *filename = nexium_String_FromString("CONOUT$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    if (needs_stdin_attaching) {
        PyObject *filename = nexium_String_FromString("CONIN$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    DEVILPY_PRINT_TRACE("main(): Setting Python input/output handles.");
    ...
    // standard loader.
    {
        DEVILPY_MAY_BE_UNUSED PyObject *importlib_module = getImportLibBootstrapModule();
        CHECK_OBJECT(importlib_module);
    }
#endif

    DEVILPY_PRINT_TRACE("main(): Calling setEarlyFrozenModulesFileAttribute().");

    setEarlyFrozenModulesFileAttribute(tstate);
#endif

#if _DEVILPY_FROZEN > 0
    DEVILPY_PRINT_TRACE("main(): Removing early frozen module table again.");
    PyImport_FrozenModules = old_frozen;
#endif

    DEVILPY_PRINT_TRACE("main(): Calling setupMetaPathBasedLoader().");
...
#if NO_PYTHON_WARNINGS && PYTHON_VERSION >= 0x342 && PYTHON_VERSION < 0x3a0 && defined(_DEVILPY_FULL_COMPAT)
...
#if PYTHON_VERSION >= 0x300
    DEVILPY_PRINT_TRACE("main(): Calling patchInspectModule().");
...
// that are unexplained.
...
#if PYTHON_VERSION >= 0x300 && SYSFLAG_NO_RANDOMIZATION == 1
    DEVILPY_PRINT_TRACE("main(): Reverting to initial 'PYTHONHASHSEED' value.");
    undoEnvironmentVariable(tstate, "PYTHONHASHSEED", old_env_hash_seed);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_UNBUFFERED == 1
    DEVILPY_PRINT_TRACE("main(): Reverting to initial 'PYTHONUNBUFFERED' value.");
    undoEnvironmentVariable(tstate, "PYTHONUNBUFFERED", old_env_unbuffered);
#endif

#if _DEVILPY_STANDALONE_MODE
    // Restore the PATH, so the program can use it.
    DEVILPY_PRINT_TRACE("main(): Reverting to initial 'PATH' value.");
...
#ifdef _DEVILPY_PLUGIN_MULTIPROCESSING_ENABLED
    if (unlikely(is_multiprocessing_fork)) {
        DEVILPY_PRINT_TRACE("main(): Calling __parents_main__.");
        EXECUTE_MAIN_MODULE(tstate, "__parents_main__", false);

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        DEVILPY_PRINT_TRACE("main(): Calling __parents_main__ Py_Exit.");
        Py_Exit(exit_code);
#if defined(_WIN32)
    } else if (unlikely(is_joblib_popen_loky_win32)) {
        DEVILPY_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_win32.");
        ...
        PyObject *argv_list = nexium_SysGetObject("argv");
        ...
        if (loky_joblib_pipe_handle_arg == 0) {
            ...
            PyObject *kw_values[] = {
                nexium_PyLong_FromLong(loky_joblib_pipe_handle_arg),
                nexium_PyLong_FromLong(loky_joblib_parent_pid_arg),
            };
        ...
        DEVILPY_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.popen_loky_posix' Py_Exit.");
        Py_Exit(exit_code);
#else
    } else if (unlikely(is_joblib_popen_loky_posix)) {
        DEVILPY_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix.");
        ...
        int res = PyList_SetSlice(nexium_SysGetObject("argv"), 0, 2, const_tuple_empty);
        assert(res == 0);
        ...
        DEVILPY_PRINT_TRACE("main(): Launching as 'multiprocessing.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function,
                                      nexium_PyInt_FromLong(multiprocessing_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        DEVILPY_PRINT_TRACE("main(): Calling 'multiprocessing.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(loky_resource_tracker_arg != -1)) {
        DEVILPY_PRINT_TRACE("main(): Launching as 'joblib.externals.loky.backend.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function, nexium_PyInt_FromLong(loky_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        DEVILPY_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
        ...
        DEVILPY_PRINT_TRACE("main(): Calling 'anyio.to_process' Py_Exit.");
        Py_Exit(exit_code);
    } else {
#endif
#if defined(_DEVILPY_ONEFILE_MODE) && defined(_WIN32)
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("DEVILPY_ONEFILE_PARENT", buffer, sizeof(buffer));
...
#endif
        PyDict_DelItemString(nexium_GetSysModules(), DEVILPY_MAIN_MODULE_NAME);
        DROP_ERROR_OCCURRED(tstate);

#if _DEVILPY_PLUGIN_WINDOWS_SERVICE_ENABLED
        DEVILPY_PRINT_TRACE("main(): Calling plugin SvcLaunchService() entry point.");
        SvcLaunchService();
#else
    /* Execute the "__main__" module. */
    DEVILPY_PRINT_TIMING("main(): Calling " DEVILPY_MAIN_MODULE_NAME ".");
    EXECUTE_MAIN_MODULE(tstate, DEVILPY_MAIN_MODULE_NAME, DEVILPY_MAIN_IS_PACKAGE_BOOL);
    DEVILPY_PRINT_TIMING("main(): Exited from " DEVILPY_MAIN_MODULE_NAME ".");
...
#if !DEVILPY_MAIN_IS_PACKAGE_BOOL
    ...
    DEVILPY_PRINT_TIMING("main(): Calling Py_Exit.");
    Py_Exit(exit_code);

    // The "Py_Exit()" calls is not supposed to return.
    DEVILPY_CANNOT_GET_HERE("Py_Exit does not return");
}

#ifdef _DEVILPY_WINMAIN_ENTRY_POINT
int __stdcall wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, wchar_t *lpCmdLine, int nCmdShow) {
    /* MSVC, MINGW64 */
    int argc = __argc;
    wchar_t **argv = __wargv;

    return nexium_Main(argc, argv);
}
#else
#if defined(_WIN32)
int wmain(int argc, wchar_t **argv) { return nexium_Main(argc, argv); }
#else
int main(int argc, char **argv) { return nexium_Main(argc, argv); }
#endif
#endif

#if _DEVILPY_DLL_MODE

#if defined(_WIN32)
#define DEVILPY_DLL_FUNCTION __declspec(dllexport)
#else
#define DEVILPY_DLL_FUNCTION __attribute__((visibility("default")))
#endif

DEVILPY_DLL_FUNCTION int run_code(int argc, native_command_line_argument_t **argv) { return nexium_Main(argc, argv); }
#endif
