...
from GooeyEx.gui.util.quoting import quote
from GooeyEx.python_bindings.types import EnrichedItem, FormField
from GooeyEx.gui.constants import VALUE_PLACEHOLDER, RADIO_PLACEHOLDER
from GooeyEx.util.functional import assoc, associnMany
...
def textArea(metadata, value):
    if metadata["commands"] and value:
        return "{} {}".format(
            metadata["commands"][0], quote(value.encode("unicode_escape"))
        )
    else:
