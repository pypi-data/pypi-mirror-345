...
from GooeyEx.python_bindings.types import Try, Success, Failure
from GooeyEx.python_bindings.coms import deserialize_inbound
...
def communicate(cmd, encoding) -> Try:
    ...
    try:
        ...
        if out and proc.poll() == 0:
            ...
            return Failure(
                CalledProcessError(proc.returncode, cmd, output=out, stderr=err)
            )
