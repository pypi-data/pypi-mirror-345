...
class PaidClient:
    ...
    def create_agent(self, agent: CreateAgentRequest) -> Agent:
        ...
        try:
            ...
            request_data = asdict(agent)
            
            # Map agentAttribute to ProductAttribute for the API request
            if 'agentAttribute' in request_data and request_data['agentAttribute']:
                request_data['ProductAttribute'] = request_data.pop('agentAttribute')
                # Also map agentId to productId in each attribute
    ...
    def update_agent(self, agent_id: str, data: UpdateAgentRequest) -> Agent:
        ...
        try:
                ...
                for attr in request_data['ProductAttribute']:
                    ...
                    if 'pricing' in attr:
                        ...
                        if 'pricePoints' in pricing:
                            pricing['PricePoints'] = pricing.pop('pricePoints')
                        if 'pricingModel' in pricing:
                            pricing['pricingModel'] = pricing.pop('pricingModel')
                        if 'billingFrequency' in pricing:
                            pricing['billingFrequency'] = pricing.pop('billingFrequency')
                        if 'chargeType' in pricing:
                            pricing['chargeType'] = pricing.pop('chargeType')
                        if 'eventName' in pricing:
                            pricing['eventName'] = pricing.pop('eventName')
            ...
            # Map field names back for the response
            if 'data' in data and 'attributes' in data['data']:
                if 'ProductAttribute' in data['data']['attributes']:
                    data['data']['attributes']['agentAttribute'] = data['data']['attributes'].pop('ProductAttribute')
                    for attr in data['data']['attributes']['agentAttribute']:
