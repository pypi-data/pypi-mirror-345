...
import os
import json
from ubuntuwebserver import config  # no circular import now
import psutil
...
def scan_network():
    ...
    for iface in get_active_interfaces():
        try:
            print(f"[INFO] Scanning {config.SUBNET} on interface: {iface}")
            nm.scan(hosts=config.SUBNET, arguments=f"-sn -e {iface}")
