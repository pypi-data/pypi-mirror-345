...
class AgentServicer:
    ...
    def listening_user(self): 
        ...
        for user in list(self.user_reply_time_dic.keys()):  # 先复制 key 的列表
            if (datetime.now() - self.user_reply_time_dic[user]["real_time"]).total_seconds() / 60 > 10:
                try:
                    self.call("close_chat", user)
                    del self.user_reply_time_dic[user]
                except Exception as e:
                    logger.debug(f"关闭聊天失败: {e}")
    ...
    def on_error(self, e): 
        ...
        WhisperTools_Qywx.send_to_error_robot(f"{self._kf_name}循环出现异常：第{self._error_count}次。({e}, {e.__traceback__.tb_lineno})")
    ...
    def report_modify_address_sended(self, order_id, user_id, service_detail):
        self._report("更改地址", order_id, user_id, service_detail)
    def report_logistic_error(self, order_id, user_id, service_detail):
        self._report("物流异常", order_id, user_id, service_detail)
    def report_delivery_error(self, order_id, user_id, service_detail):
        self._report("错发漏发", order_id, user_id, service_detail)
    def report_product_error(self, order_id, user_id, service_detail):
        self._report("产品投诉", order_id, user_id, service_detail)
    def report_transfer_fail(self, user_id, service_detail):
        self._report("转人工失败", "", user_id, service_detail)
    
    def _report(self, type, order_id, user_id, service_detail):
        myDB = WhisperDB()
        
        # 直接使用参数化查询，确保 JSON 格式正确存储
        query = """
            INSERT INTO `openai_service_manage`
            (`type`, `shop`, `order_id`, `user_id`, `detail`, `kf_name`) 
            VALUES (%s, %s, %s, %s, %s, %s);
        """
        
        # 使用参数化，避免 SQL 注入，并确保 JSON 格式正确存入数据库
        myDB.query(query, (type, self.get_shop_name(), order_id, user_id, json.dumps(service_detail, ensure_ascii=False), self.get_kf_name()))
        
        myDB.commit()
        myDB.close()
        WhisperTools_Qywx.send_to_kf_robot(self, f"{self.get_shop_name()}店铺的'{user_id}'提交了‘{type}’的工单，请及时处理！")
