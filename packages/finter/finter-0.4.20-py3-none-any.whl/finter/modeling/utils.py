...
class DataFrameFilter:
    ...
    def match_common_column(filter_df, target_df, slice_length=None):
        ...
        for sliced_col, target_cols in col_mapping.items():
            for target_col in target_cols:
                temp_df = pd.DataFrame(filter_df[sliced_col].values, 
    ...
    def compare_column_length(filter_df, target_df):
        filter_df_col_length = len(filter_df.columns[0]) if len(filter_df.columns) > 0 else 0
        target_df_col_length = len(target_df.columns[0]) if len(target_df.columns) > 0 else 0
        return filter_df_col_length == target_df_col_length


    @staticmethod
    def slice_masking(slice_filter_df, value, mode='include'):
        pattern_mask = pd.DataFrame(False, index=slice_filter_df.index, columns=slice_filter_df.columns)
        
        for col in slice_filter_df.columns:
            valid_cells = slice_filter_df[col].notna()
            if valid_cells.any():
                sliced_values = slice_filter_df.loc[valid_cells, col].astype(str).str[:len(value)]
                matches = sliced_values == value
                
                if matches.any():
                    pattern_mask.loc[matches.index[matches], col] = True
        
        result_mask = pd.DataFrame(np.nan, index=slice_filter_df.index, columns=slice_filter_df.columns)
        if mode == 'include':
    ...
    def match_column_reindex(result_mask, filter_df, target_df, slice_length=None):
        if not DataFrameFilter.compare_column_length(filter_df, target_df):
            matched_mask = DataFrameFilter.match_common_column(result_mask, target_df, slice_length)
    ...
    @classmethod
    def by_boolean(cls, boolean_df, target_df, tobe_masked, mode='include', slice_length=None):
        target = target_df.copy()
        boolean = boolean_df.copy()
        
        boolean_str = cls.data_to_str(boolean)
        
        result_mask = pd.DataFrame(np.nan, index=boolean_str.index, columns=boolean_str.columns)
        ...
        final_mask = cls.match_column_reindex(result_mask, boolean_df, target_df, slice_length)
    ...
    def by_gics(cls, gics_df, target_df, tobe_masked, mode='include', slice_length=6):
        ...
        final_mask = cls.match_column_reindex(result_mask, gics_df, target_df, slice_length)
