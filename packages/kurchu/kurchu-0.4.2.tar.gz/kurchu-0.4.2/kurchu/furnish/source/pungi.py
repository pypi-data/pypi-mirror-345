...
class SourcePungi(SourceBase):
	"""
	Implements furnishing the pungi compose source
	"""

	def __init__(self, source_config: dict, output_dir: Path) -> None:
		super().__init__(source_config, output_dir)
		self.id_file = "COMPOSE_ID"
		self.path_to_id = f"{self.target_subdir}/{self.id_file}"
		log.info(f"Identifying source ID for {self.name}")
		pungicomposeid_file = open(file=output_dir / "compose" / self.path_to_id, mode="rt")
		self.source_id = pungicomposeid_file.read()
