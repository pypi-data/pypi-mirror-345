...
def readme(session: nox.Session) -> None:
    ...
    with open("README.md", "r+", encoding="utf8") as f:
        help_message = session.run("heatmap", "-h", "-vvv", silent=True)
        help_codeblock = f"\n\n```console\n{help_message}```\n\n"
...
def release(session: nox.Session) -> None:
    ...
    """
    with open("heatmap_cli/__init__.py", "r", encoding="utf8") as f:
        tree = ast.parse(f.read())
        current_version = None
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign) and len(node.targets) == 1:
                target, value = node.targets[0], node.value
                if target.id == "__version__" and isinstance(  # type: ignore
                    value, ast.Constant
                ):
        ...
        _search_and_replace(
            "heatmap_cli/__init__.py", str(before_version), after_version
        )
...
def _search_and_replace(file: str, search: str, replace: str) -> None:
