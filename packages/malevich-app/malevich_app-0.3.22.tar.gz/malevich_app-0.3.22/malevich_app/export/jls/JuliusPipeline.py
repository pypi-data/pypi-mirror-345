...
from malevich_app.export.secondary.helpers import get_collection_by_id
...
class JuliusPipeline:
    def __init__(self, pipeline: InitPipeline, registry, logs_buffer: io.StringIO, storage: Optional['LocalStorage'] = None, fail_storage: Optional[FailStorage] = None):
        ...
        self.__collections_holder = JuliusCollectionHolder(pipeline.operationId, pipeline.secret, C.DAG_HOST_PORT(pipeline.dagHost), self.__logs_buffer, pipeline.pipeline.pullCollectionPolicy, self.__common_cfg.schemes_aliases, registry.internal_schemes, storage)
        ...
        self.__tracker = JuliusTracker(C.DAG_HOST_PORT(pipeline.dagHost), is_local=storage is not None)
        self.__check()
    ...
    def __construct(self) -> Dict[str, JuliusPipelineItem]:
        ...
        for bind_id, entity in pipeline.processors.items():     # TODO set also loopArguments, loopConditions
            ...
            japp = self.__registry.create_app(self.__init, bind_id, self.__common_cfg,  pipeline_app, app_cfg, local_dfs=self.__collections_holder.local_dfs)
            ...
            res[bind_id] = JuliusPipelineItem(japp)
        for bind_id, entity in pipeline.conditions.items():     # FIXME copypaste, TODO set also loopArguments
            ...
            app_cfg = get_app_cfg(entity.cfg, self.__logs_buffer)
            japp = self.__registry.create_app(self.__init, bind_id, self.__common_cfg, pipeline_app, app_cfg, local_dfs=self.__collections_holder.local_dfs)     # TODO set task_id
    ...
    async def __run_fail(self, julius_app, run_id: str, bind_id: str, iteration: int, is_processor: bool, err_msg: str, collections: List[List[Union[LocalCollection, ObjectCollection, CompositeCollection, List[Union[LocalCollection, ObjectCollection, CompositeCollection]]]]]):
        if self.__fail_storage is not None:
            try:
                self.__fail_storage.save(julius_app, self.__init.operationId, run_id, bind_id, iteration, is_processor, err_msg, julius_app._cfg_dict_if_need(), collections)
            except:
    ...
    async def __run(self, run_id: str, iteration: int, items: Dict[str, JuliusPipelineItem], bind_id: str) -> bool:
        ...
        if pipeline_item.japp.get_operation() == EntityType.CONDITION:
            try:
                ...
                await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, False, PipelineStructureUpdate(), branch=False)
                raise ex

            ok, is_true = await pipeline_item.run_condition(args)
            if not ok:  # is_true - str traceback
                self.__local_log(run_id, bind_id, iteration, "fail")
                await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, ok, PipelineStructureUpdate(), branch=True)
                await self.__run_fail(pipeline_item.japp, run_id, bind_id, iteration, False, is_true, interpret_colls)
                return ok
            ...
            await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, ok, struct_update.to_scheme(), branch=is_true)
        else:  # processor
            try:
                args, interpret_colls = await self.__collections_holder.interpret(pipeline_item.japp, collections, pipeline_item.japp.get_cfg())
            except BaseException as ex:
                await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, False, PipelineStructureUpdate())
                raise ex

            ok, colls = await pipeline_item.run_processor(args)
            if not ok:  # colls - str traceback
                self.__local_log(run_id, bind_id, iteration, "fail")
                await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, ok, PipelineStructureUpdate())
                await self.__run_fail(pipeline_item.japp, run_id, bind_id, iteration, True, colls, interpret_colls)
                return ok
            ...
            if pipeline_item.japp.is_stream():
                ...
                if streams_by_bind_id.get(bind_id) is not None:
                    ...
                    streams_by_bind_id[bind_id] = colls
                    ...
                    for save_res in to_save:
                        if save_res.index is None:
                            for i, result in enumerate(colls):
                                saves.append(save_real_collection(result, pipeline_item.japp, save_res.name, index=i, group_name=save_res.name))
                        else:
                            i = save_res.index
                            if i < len(colls):
                                saves.append(save_real_collection(colls[i], pipeline_item.japp, save_res.name, index=i, group_name=save_res.name))
                            else:
            ...
            await self.__tracker.finished(self.__init.operationId, run_id, bind_id, iteration, True, struct_update.to_scheme(), colls=[] if pipeline_item.japp.is_stream() else colls)

        if len(tasks) > 0:
            res = all(await gather(*tasks))
            return res
        else:
            return True
    ...
    async def init_run(self, init: InitRun) -> Optional[bool]:
        ...
        for bind_id, item in self.__items.items():
            ...
            new_item = JuliusPipelineItem(new_j_app)
    ...
    async def run(self, run_id: str, iteration: int, bind_id_run: str, data: Optional[Dict[str, Tuple[str, ...]]], conditions: Optional[Dict[str, bool]], structure_update: PipelineStructureUpdate, bind_id_iterations: Optional[Dict[str, int]]) -> bool:
        ...
        if data is None:
            ...
            self.__set_collections_for_iteration(run_id, max(bind_id_iterations.values()) + 1)
            self.__collections_holder.set_collection_external(run_id, data, bind_id_iterations)
        ...
        if len(data) == 0 and len(conditions) == 0:
            ...
            for bind_id in self.__items:
                if bind_id not in run_wait_bind_ids and bind_id not in run_wait_bind_ids_conds and bind_id not in run_alt_wait_bind_ids:
                    ...
                    runs.append(self.__run(run_id, iteration, self.__run_items[run_id], bind_id))
        else:
            for bind_id, coll_ids in data.items():
                ...
                tasks, _ = self.__run_next(run_id, bind_id_iteration, self.__run_items[run_id], bind_id, None, with_update=False)
            ...
            for bind_id, cond_value in conditions.items():
                ...
                tasks, _ = self.__run_next(run_id, bind_id_iteration, self.__run_items[run_id], bind_id, cond_value, with_update=False)
                runs.extend(tasks)
    ...
    def set_index(self, run_id: str, index: int):
        for item in self.__run_items[run_id].values():
            item.japp.set_index(index)
    ...
    def delete_run(self, run_id: str):
        ...
        self.__run_items.pop(run_id, None)
