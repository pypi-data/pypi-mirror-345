...
2. Context-based disambiguation enhancement:
   ...
   - Specific Contextual Disambiguation for "Гандурас/гандурас":
     (A) DEFAULT BEHAVIOR: Treat "гандурас" (any case) as POTENTIALLY offensive due to its common use as a slur, often phonetically related to or used interchangeably with "пидарас".
     (B) NON-OFFENSIVE CONTEXT (DO NOT FLAG): If the surrounding text clearly indicates a reference to the country Honduras (e.g., mentions "страна", "государство", "Центральная Америка", "Тегусигальпа", travel contexts, geography discussions, political news about the country, imports/exports, specific cultural references to Honduras), treat it as a proper noun referring to the country and therefore STRICTLY NON-OFFENSIVE. Capitalization ("Гандурас") is a strong indicator for the country but its absence doesn't automatically mean it's an insult if other context is present.
     (C) OFFENSIVE CONTEXT (FLAG AS HIGHLY OFFENSIVE): If "гандурас" is used as a direct address to a person ("ты гандурас", "эй, гандурас"), as explicit name-calling, appears within a clearly insulting phrase aimed at an individual or group, or is used in a context of strong negative sentiment/aggression lacking any geographical/political indicators, then FLAG it as HIGHLY OFFENSIVE. The likely intended normalized form in this context is often equivalent to "пидарас" or a similar severe insult.
     (D) Confidence Scoring: Assign high confidence (e.g., >0.85) to the offensive classification ONLY when the context strongly supports the insulting usage (as per C) AND simultaneously lacks credible evidence for the country reference (as per B). If clear country context exists, the confidence score for being offensive must be effectively zero (or below a very low threshold like 0.1). In ambiguous cases without strong pointers either way, assign a medium/low confidence score and potentially list "Country Name" as an alternative interpretation.
...
9. Reversed String Obfuscation (including variations):
   ...
   - Explicitly detect variations of reversed forms using character substitutions (Leet speak, homoglyphs, script mixing, symbols, spacing).
   - Apply high confidence score when a reversed profane word is strongly indicated. Context override requires very strong evidence for innocence.
   - Examples MUST include (but are not limited to):
     - "сарадип" and its variations:
     ...
     - "хол" and its variations:
       - "хол" → "лох" (Exact reverse)
       - "xol" / "xoл" → "лох" (Mixed script/Latin x)
       - "x0l" / "х0л" → "лох" (Leet speak '0' for 'о')
       - "х о л" → "лох" (Space insertion)
     - "чиб" and its variations:
       - "чиб" → "бич" (Exact reverse - note context dependency, prioritize offensive if unclear)
       - "4иб" / "chиб" / "чи6" → "бич" (Leet speak '4', Latin 'ch', Leet speak '6')
       - "4ib" / "ch1b" → "бич" (Leet speak/Latin variations)
       - "ч и б" → "бич" (Space insertion)
     - "омч" and its variations:
       - "омч" → "чмо" (Exact reverse)
       - "0мч" / "oмч" / "ом4" → "чмо" (Leet speak '0', Latin 'o', Leet speak '4')
       - "0m4" / "om4" → "чмо" (Leet speak/Latin variations)
       - "о м ч" → "чмо" (Space insertion)
