...
class TwincatHandler(BaseHandler):
    ...
    def __init__(self, config: TwincatConfig, base_dir: Path, **kwargs: Any) -> None:
        ...
        _logger.info(f"Config Paths from File: {config.paths}")
        paths:List[Path] = []

        for path in config.paths:
            _purePath = Path(PureWindowsPath(path))
            _logger.info(f"path: {_purePath}")
            _logger.info(f"type: {_purePath.__class__}")
            if _purePath.resolve().is_file():
                paths.append(_purePath.resolve())
                _logger.info(f"filepath: {_purePath.resolve()} is appended and resolved")
            else:
                _logger.info(f"filepath: {_purePath.resolve()} is not appended and resolved")
