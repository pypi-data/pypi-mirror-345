...
def load_constants(file):
    with pkg_resources.files(constants).joinpath(file).open("rb") as f:
        return np.load(f)
...
class BIFROST_nD:
    ...
    def RT_2_CA(self):
        ...
        channel = 3*(self.data["RING"] // 2) + self.add_data
    ...
    def pixel_split(self):
        ...
        bins = np.digitize(L_x, ds)
        
        self.data["D_tube"] = bins // 100 + 1
        self.data["d_i"] = bins % 100
        
        # Flip second tube
        self.data.loc[self.data["D_tube"] == 2, "d_i"] = 99 - self.data.loc[self.data["Tube"] == 2, "d_i"]   
    ...
    def E_f_k_f(self):
        ...
        phi = load_constants("phi.npy")
        
        a4 = float(self.parameters["a4"])
        ...
        E = Es[C-1, A-1, T-1, d]
        L = L[C-1, A-1, T-1, d]
        phi = np.deg2rad(phi[C-1, A-1, T-1, d])
    ...
    def psc_timing(self):
        ...
        psc_v = float(self.parameters["psc1speed"])
        psc_offset_1 = float(self.parameters["psc1phase"])
        psc_offset_2 = float(self.parameters["psc2phase"])
        
        psc_off = (psc_offset_1 + psc_offset_2) / (720*psc_v)
        psc_opening = (psc_offset_2 - psc_offset_1+170) / (360*psc_v)
    ...
    def Q_hw(self):
        ...
        final_data["Q_x"] = self.data["k_ix"] - self.data["k_fx"]
        final_data["Q_y"] = self.data["k_iy"] - self.data["k_fy"]
        final_data["Q_z"] = self.data["k_iz"] - self.data["k_fz"]
        final_data["hbar_omega"] = self.data["E_i"] - self.data["E_f"]
        final_data["Intensity"] = self.data["Intensity"]
...
class BIFROST_Res(BIFROST_nD):
    ...
    def pixel_resolution(self, save_folder=None, save_file=None, threshold=1, plot=False, data_save=False):
        if save_folder == None:
        ...
        for c in range(1, 10):
            ...
            for a in range(1, 6):
                ...
                for t in range(1, 4):
                    for d in range(100):
                        data_i = self.data[(self.data["channel"] == c) & (self.data["analyzer"] == a) & (self.data["TUBE"] == t) & (self.data["d_i"] == d)]
                        ...
                        mean, cov = weighted_cov(data_i[["dQx", "dQy", "dQz", "dhbar_w"]].to_numpy(), data_i["Intensity"].to_numpy())
    ...
    def resolution_ellipse(self, channel, analyzer, tube, pixel):
        ...
        cov = Res_i[["sigma_Qx_2", "sigma_Qx_Qz", "sigma_Qz_2", "sigma_Qx_hw", "sigma_Qz_hw", "sigma_hw_2"]].to_numpy()
...
class Theory_Res:
    ...
    def resolution(self, channel, analyzer, tube, pixel):
        ...
        L2s = [1.189,1.314,1.418,1.52,1.622]
        delta_phi = [0.86,0.77,0.72,0.67, 0.62]
        Ef = E_fs[analyzer-1]
        Ei = Ef + self.hw
        ...
        delta_phi = np.deg2rad(delta_phi[analyzer-1])
        ...
        sigma = np.diag(np.array([0.01/2.355, 0.002/2.355, np.sqrt(self.dt**2+2e-5**2),0.0001/d, np.deg2rad(3/60)/2.355, 0.025/2.355, delta_phi/2.355]))
...
def compare_resolution(folder, stt, channel, analyzer, tube, pixel, plot=True, save_file=None):
    """
    ...
    theory = Theory_Res(data.dt, Res["hw_m"].values[0], stt)
    ...
    if plot:
        ...
        for k in range(3):
            ...
            plot_cov_ellipse(sim_cov[np.ix_([i, j], [i, j])], sim_mean, ax=ax, edgecolor='blue', facecolor='none')
            plot_cov_ellipse(theory_cov[np.ix_([i, j], [i, j])], sim_mean, ax=ax, edgecolor='red', facecolor='none')
            ax.scatter(sim_mean[i], sim_mean[j], color='blue', label="Simulated")
            ax.set_xlabel(labels[i])
            ax.set_ylabel(labels[j])
            
        plt.suptitle(f"Resolution hw={Res['hw_m'].values[0]:.2f} dt={data.dt:.2f} stt={stt} {channel}-{analyzer}-{tube}-{pixel}")
        axs[0].legend()
        plt.tight_layout()
    ...
    D_b = np.log(det_sum/np.sqrt(det_sim*det_theory))
    
    print(f"Bhattacharyya distance: {D_b:.2f}")
