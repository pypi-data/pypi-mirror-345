...
class CodeGenerator:
    ...
    def synthesize(self, rules: dict) -> str:
        ...
        prompt = (
            ...
            "Write the complete Python class based strictly on these rules.\n"
        ...
        response = self.llm.query([system_prompt, prompt], self.SYTHESIZE_SCHEMA)
        if not response.get("success", False):
            raise ValueError(response.get("error", "LLM ERROR"))
    ...
    def debug(self, code: str, instructions: str) -> str:
        ...
        response = self.llm.query(prompt, self.DEBUG_SCHEMA)
        if not response.get("success", False):
            raise ValueError(response.get("error", "LLM ERROR"))
    ...
    def generate_test_script(self, code: str) -> str:
        ...
        response = self.llm.query(prompt, self.TESTSCRIPT_SCHEMA)
        if not response.get("success", False):
            raise ValueError(response.get("error", "LLM ERROR"))
