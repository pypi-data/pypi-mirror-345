...
class IIncrementedBalances(BaseModel):
    ...
    increment_ownership_times_by: NumberType = Field(description="The amount to increment the owned times by after each transfer. Incompatible with approvalDurationFromNow.", alias="incrementOwnershipTimesBy")
    approval_duration_from_now: NumberType = Field(description="The number of unix milliseconds to approve starting from now. Incompatible with incrementOwnershipTimesBy.", alias="approvalDurationFromNow")
    __properties: ClassVar[List[str]] = ["startBalances", "incrementBadgeIdsBy", "incrementOwnershipTimesBy", "approvalDurationFromNow"]
