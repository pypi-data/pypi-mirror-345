...
class Template:
    def cfmt(self,line,n=4):
        ...
        lastblock=x[-1]
        x[0]=f"{Fore.orange_red_1}{first}{Fore.cyan}{second}{Style.reset}"
        for num,i in enumerate(x):
            if num not in [0,len(x)-1]:
                if num % 2 == 0:
                    x[num]=f"{Fore.cyan}{x[num]}{Style.reset}"
                else:
                    x[num]=f"{Fore.light_steel_blue}{x[num]}{Style.reset}"
        if len(line) >= n:
            x[-1]=f"{Fore.light_red}{lastblock[:-1]}{Fore.dark_goldenrod}{lastblock[-1]}{Style.reset}"
        else:
            x[-1]=f"{Fore.cyan}{lastblock[:-1]}{Fore.dark_goldenrod}{lastblock[-1]}{Style.reset}"
        cformat=''.join(x)
        return cformat
...
"""
    ...
    def seeShortRaw(self):
        msg=f'''Short Data - {self.Name}[Name][EID={self.EntryId}] ({self.rebar()}[UPC]:{self.cfmt(self.Code)}[SHELF/TAG/CIC])|{self.CaseCount}[CaseCount]|{self.Price}[Price]'''
        return msg

    def rebar(self,barcode=None,steps=4):
        try:
            if barcode == None:
    ...
    def seeShort(self):
        rebar=self.rebar()
        msg=f''' {Fore.slate_blue_1}{Style.underline}{'-'*5}{Style.reset}{Style.bold} Short Data {Style.reset}{Fore.slate_blue_1}{Style.underline}{'-'*5}{Fore.light_magenta}Price Per Unit: {Fore.slate_blue_1}{self.Price}{Style.reset}
{Fore.light_yellow}{self.Name} [{Fore.misty_rose_3}EID={Fore.pale_green_1b}{self.EntryId}{Fore.light_yellow}] ({Fore.light_magenta}{rebar}{Fore.light_magenta}[{Fore.spring_green_3a}UPC{Style.reset}{Fore.light_magenta}]:{Fore.light_red}{self.cfmt(self.Code)}[{Fore.orange_red_1}SHELF/TAG/CIC]{Fore.light_red}){Style.reset}'''
        return msg
...
class UniqueRecieptIdInfo(BASE,Template):
    ...
    StoreName=Column(String,default='No Store Applied')
    StoreNumber=Column(String,default="NaN")
    StoreAddress=Column(String,default='2 NoWay Crt, NopeLand Bay, NowhereLand 00000-0000')
    DTOE=Column(DateTime,default=datetime.now())
    StoreDirector=Column(String,default='Brady Nunsen Inexsistancia')
    ...
    Cashier=Column(String,default="Jane Doe")
