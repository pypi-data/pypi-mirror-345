...
class fixify:
  ...
  def inlineEdit(self, filepath, start_line, end_line=None, explain = False, prompt = "Explain in easy language"):
        if ("/" in filepath):
        ...
        older_code = self.edit_in_file(filepath=filepath, start_line=start_line, end_line=end_line)
        ...
        if (explain):
          ...
          and new code:
          {corrected_code}
          Your work is to explain the difference between both code and tell user why new code is better
          Dont print in md format print in plain paragraph but with correct format and break in lines
          {prompt}
          """
          output = template.setLLM(msg)
          print(output) 
