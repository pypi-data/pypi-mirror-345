...
from SajadMr.__past__ import subprocess
...
class SajadQCalledProcessError(subprocess.CalledProcessError):
...
def check_output(*popenargs, **kwargs):
    ...
    if exit_code:
        ...
        raise SajadQCalledProcessError(exit_code, cmd, output=output, stderr=stderr)
...
def wrapCommandForDebuggerForExec(command, debugger=None):
    ...
    if debugger is None:
        debugger = os.getenv("SajadSx_DEBUGGER_CHOICE")
