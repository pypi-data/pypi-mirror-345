...
#include "FAReinforcedConcretePlaneStress.h"
...
OPS_Export void * OPS_ADD_RUNTIME_VPV(OPS_FAReinforcedConcretePlaneStressMaterial)
{
  if (numFAReinforcedConcretePlaneStressMaterials == 0) {
    numFAReinforcedConcretePlaneStressMaterials++;
    opslog << "FAReinforcedConcretePlaneStress unaxial material - Written by J.Zhong, Thomas T.C. Hsu and Y.L. Mo - Copyright@2009\n";
  }
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial FAReinforcedConcretePlaneStress matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? angle1? angle2? rou1? rou2? fpc? fy? E0? epsc0?>\n";
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid tag nDMaterial FAReinforcedConcretePlaneStress tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial FAReinforcedConcretePlaneStress tag:" << tag << endln;
    return 0;	
  }

  numData = 4;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial FAReinforcedConcretePlaneStress tag: " << tag << endln;
    return 0;
  }

  numData = 8;
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data FAReinforcedConcretePlaneStress tag: " << tag << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nFAReinforcedConcretePlaneStress tag: " << tag << endln;
    return 0;
  }
  ...
  //now create the FAReinforcedConcretePlaneStress
  theMaterial = new FAReinforcedConcretePlaneStress (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "FAReinforcedConcretePlaneStress: " << tag << endln;
    return 0;
  }
...
double FAReinforcedConcretePlaneStress::getRho(void)
{
	return rho;
}


int FAReinforcedConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
const Vector& FAReinforcedConcretePlaneStress::getStress(void)
{
...
const Vector& FAReinforcedConcretePlaneStress::getCommittedStress(void)
{
  return stress_vec;
}


const Vector& FAReinforcedConcretePlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int FAReinforcedConcretePlaneStress::commitState(void)
{
  for (int i=0; i<4; i++)
    {
...
int FAReinforcedConcretePlaneStress::revertToLastCommit(void)
{
...
int FAReinforcedConcretePlaneStress::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
    {
...
NDMaterial* FAReinforcedConcretePlaneStress::getCopy(void)
{
  /*
    FAReinforcedConcretePlaneStress *clone;
    clone = new FAReinforcedConcretePlaneStress();
    *clone = *this;
    return clone;
  //*/
  
  FAReinforcedConcretePlaneStress* theCopy =
    new FAReinforcedConcretePlaneStress( this->getTag(), 
					 rho, 
					 theMaterial[0], 
					 theMaterial[1], 
					 theMaterial[2], 
...
NDMaterial* FAReinforcedConcretePlaneStress::getCopy(const char *type)
{
    /*
	FAReinforcedConcretePlaneStress *clone;
	clone = new FAReinforcedConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
	FAReinforcedConcretePlaneStress* theModel =
		new FAReinforcedConcretePlaneStress( this->getTag(), 
...
void FAReinforcedConcretePlaneStress::Print(OPS_Stream &s, int flag )
{
	s << "\n\tFAReinforcedConcretePlaneStress, material id: " << this->getTag() << endln;
    /*
	s << "\tRho: " << rho << endln;
...
int FAReinforcedConcretePlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "FAReinforcedConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }	
...
int FAReinforcedConcretePlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FAReinforcedConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "FAReinforcedConcretePlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "FAReinforcedConcretePlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "FAReinforcedConcretePlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int FAReinforcedConcretePlaneStress::determineTrialStress(void)
{ 
  ...
  if ( fabs(Tstrain[0]-Tstrain[1]) < 1e-7 )
	...
	{
	  opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
  ...
  if ( status == 0 ) // does not get converged after iteration
    {
      ...
      // if ( minError > 0.05 )
      //    opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: Warning, failure to get converged principal stress direction\n";
    } 
...
double FAReinforcedConcretePlaneStress::getAngleError(double inputCita)
{
	double pi = 3.14159265359;
	double outputCita;
...
double FAReinforcedConcretePlaneStress::getPrincipalStressAngle(double inputAngle)
{   
    ...
    */
	...
	if (steelStatus == 1)
      ...
      {
    	opserr << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
      }
    ...
    if ( v12*v21==1.0 )
      {
	opserr << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: failure to get matrix [V]!\n";
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: Failure to calculate principal stress direction\n";
