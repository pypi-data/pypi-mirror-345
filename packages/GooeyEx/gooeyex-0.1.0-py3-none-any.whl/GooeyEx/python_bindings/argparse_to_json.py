...
from GooeyEx.util.functional import merge, getin, identity, assoc
from GooeyEx.gui.components.options.validators import validators
from GooeyEx.gui.components.options.validators import collect_errors
...
def item_default():
    ...
    return {
        ...
        "help_color": (
            constants.COLOR_GREY_5 if use_dark_mode else constants.COLOR_GREY_100
        ),
        "full_width": False,
...
def process(parser, widget_dict, options, group_defaults):
    ...
    raw_action_groups = [
        extract_groups(group, group_defaults)
...
def reapply_mutex_groups(mutex_groups, action_groups):
    ...
    return [
        group.update({"items": swap_actions(group["items"])}) or group
...
def categorize2(groups, widget_dict, options):
    ...
    defaults = {
        ...
        "description_color": (
            constants.COLOR_GREY_10 if use_dark_mode else constants.COLOR_GREY_100
        ),
    }
...
def is_version(action):
    return isinstance(action, _VersionAction) or issubclass(
        type(action), _VersionAction
    )
...
def is_standard(action):
    ...
    """
    boolean_actions = (_StoreConstAction, _StoreFalseAction, _StoreTrueAction)
    return (
        not action.choices
...
def is_flag(action):
    ...
    return any(
        list(map(lambda Action: isinstance(action, Action), action_types))
    ) or issubclass(
        type(action), (_StoreTrueAction, _StoreFalseAction, _StoreConstAction)
    )
...
def build_radio_group(mutex_group, widget_group, options):
    ...
    return {
        ...
        "data": {
            ...
            "widgets": list(
                categorize(mutex_group._group_actions, widget_group, options)
            ),
        },
    }
...
def choose_cli_type(action):
    return "positional" if action.required and not action.option_strings else "optional"
...
def clean_list_defaults(default_values):
    ...
    """
    wrapped_values = (
        [default_values] if isinstance(default_values, str) else default_values
    )
