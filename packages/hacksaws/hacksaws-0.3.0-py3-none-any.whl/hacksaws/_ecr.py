...
def _do_login(context: _configs.Context, registry: str):
    ...
    cmd = [
        "podman" if context.args.podman else "docker",
        "login",
        f"--username={user}",
        "--password-stdin",
        registry,
...
def logout(
    context: _configs.Context, aws_account: _configs.AwsAccount, check: bool = True
):
    """Logs out of the registry for the given AWS account."""
    for registry in aws_account.ecr_registries:
        cmd = ["podman" if context.args.podman else "docker", "logout", registry]
