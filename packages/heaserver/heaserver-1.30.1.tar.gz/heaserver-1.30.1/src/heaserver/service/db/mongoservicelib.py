...
from ..heaobjectsupport import new_heaobject_from_type, RESTPermissionGroup, HEAServerPermissionContext
...
from heaobject.root import DesktopObject, DesktopObjectDict, desktop_object_from_dict, Permission, DesktopObjectTypeVar
...
async def get_dict(request: Request, collection: str, volume_id: str | None = None,
                   mongo: Mongo | None = None) -> DesktopObjectDict | None:
    """
    ...
    """
    return await _get_dict(request, collection, volume_id, mongo=mongo)
    ...
    if cached_value is not None:
        ...
        async with MongoContext(request, volume_id) as mongo:
            ...
            if result is not None:
                ...
                permitted = await RESTPermissionGroup.GETTER_PERMS.has_any(obj_, context)
                if not permitted:
                    return None
...
async def get(request: Request, collection: str, volume_id: str | None = None) -> Response:
    ...
    async with MongoContext(request, volume_id) as mongo:
        context = mongo.get_default_permission_context(request)
        result = await _get_dict(request, collection, volume_id, mongo=mongo, context=context)
        if result is None:
...
async def get_by_name_dict(request: Request, collection: str,
                           volume_id: str | None = None, mongo: Mongo | None = None) -> DesktopObjectDict | None:
    """
    Gets the requested desktop object as a desktop object dict.
    ...
    if cached_value is not None:
        ...
        async with MongoContext(request, volume_id) as mongo:
            async def fetch_from_mongo(mongo: Mongo) -> DesktopObjectDict | None:
                context = mongo.get_default_permission_context(request)
                result = await mongo.get(request, collection, var_parts='name', context=context)
...
async def get_all(request: Request,
    ...
    if mongoattributes is None and cached_value is not None:
        ...
        async with MongoContext(request, volume_id) as mongo:
            context = mongo.get_default_permission_context(request)
            gen = _get_all_desktop_objects_gen(request, collection,
...
async def get_all_dict(request: Request,
    ...
    if mongoattributes is None and cached_value is not None:
        ...
        l: list[DesktopObjectDict] = []
        gen = get_all_desktop_objects_gen(request, collection, volume_id=volume_id,
...
async def put(request: Request, collection: str, type_: type[DesktopObjectTypeVar], volume_id: str | None = None,
    ...
    async with MongoContext(request, volume_id) as mongo:
        ...
        try:
            ...
            result = await mongo.put(request, obj_, collection, context=context)  # if lacks permissions or object is not in database, then updates no records.
...
def get_all_desktop_objects_gen(request: Request, collection: str, *,
                      ...
                      mongo: Mongo | None = None) -> AsyncGenerator[DesktopObject, None]: ...
...
def get_all_desktop_objects_gen(request: Request, collection: str, *,
                      ...
                      mongoattributes: Any | None = None,
                      sort: dict[str, Literal[-1, 1]] | None = None,
                      mongo: Mongo | None = None) -> AsyncGenerator[DesktopObjectTypeVar, None]: ...
...
async def get_all_desktop_objects_gen(request: Request, collection: str, *,
                      ...
                      mongo: Mongo | None = None) -> AsyncGenerator[DesktopObject, None]:
    """
    Gets an async generator of all desktop objects. Does no caching.
    ...
    """
    async for obj in _get_all_desktop_objects_gen(request, collection, type_=type_, volume_id=volume_id,
                                                  mongoattributes=mongoattributes, sort=sort, mongo=mongo):
        yield obj
...
async def get_all_desktop_objects(request: Request,
                      ...
                      volume_id: str | None = None,
                      mongoattributes: Any | None = None,
                      sort: dict[str, Literal[-1, 1]] | None = None) -> list[DesktopObject]:
    """
    Gets a list of all desktop objects matching the query. Does no caching.
...
async def _get_dict(request: Request, collection: str, volume_id: str | None = None, mongo: Mongo | None = None,
...
async def _get_all_desktop_objects_gen(request: Request, collection: str, *,
                      type_: type[DesktopObject] | None = None,
                      volume_id: str | None = None,
                      mongoattributes: Any | None = None,
                      sort: dict[str, Literal[-1, 1]] | None = None,
    ...
    async def fetch_objects(mongo: Mongo) -> AsyncGenerator[DesktopObject, None]:
        ...
        try:
            async for desktop_object_dict in gen:
                obj = desktop_object_from_dict(desktop_object_dict, type_=type_)
