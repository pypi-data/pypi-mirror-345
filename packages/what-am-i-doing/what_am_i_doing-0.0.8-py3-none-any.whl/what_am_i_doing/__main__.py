...
# print('sys.path with os.path.dirname(__file__) added',sys.path)
...
class Application(Gtk.Application):
    def __init__(self, *args, **kwargs):
        ...
        self.indicator = appindicator.Indicator.new(conf.app_name, os.path.abspath('icon/icon-0.png'), appindicator.IndicatorCategory.APPLICATION_STATUS)
    ...
    #     print('do_comand_line options', options)
        ...
        if(self.is_running == False):
            ...
            if afk_time > 120:
                ...
                if 'target' in self.session:
                    ...
                    t['percent'] = ( (t['starting_value'] + minutes) / t['value'] ) * 100
                    print('target',t)
    ...
    def tick(self):
        ...
        # check for suspend indicated gap in tick intervals
        time_since_last_tick = time_difference(self.session['start_time'])  - self.session['duration']
        if time_since_last_tick > 10:
            print(time_since_last_tick, "secons since las tick, Probably just woke from suspend ")
            print(self.session)
        ...
        if(self.is_running == True):
            ...
            indicator.set_icon_full(os.path.abspath('icon/icon-0.png'),label)
    ...
    def start_task(self, w = None, task_data_or_id = None, transfer_current_session_time = False):
        ...
        # TODO: copy this to save change in session 
        ...
        # if 'command' in task_data['data']:
        #     thread = threading.Thread(target=self.run_task_command,args=(task_data['data']['command'],))
        #     print("found task command:", task_data['data']['command'])
        #     thread.start()
