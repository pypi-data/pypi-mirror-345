...
def get_updates_per_repo(repo_list: typing.List[str] = [], max_wait: int = 14) -> typing.Dict[str, list]:
    """
    ...
    When updates available: {'libudev1', 'udev'}, {'repo-oss'}
    When updates available: {'repo-oss': ['libudev1', 'udev']}
    """
    per_repo_updates = {}
    repo_args = []
    ...
    try:
        ...
        zypp_run = run(
            ["zypper", "-x", "--non-interactive", "list-updates"] + repo_args,
            capture_output=True,
        ...
        logger.error(f"Error fetching updates from all, or indicated repos: {e}")
...
def local_changelog(package: str, titles_only: bool = False, max_wait: int = 1) -> str:
    """
...
def fetch_rpm_header(url: str, end: str, retries: int = 2, backoff_factor: int = 2, auth: SimpleCredential = None):
    """
    ...
    """
    logger.debug(f"++ CALLED fetch_rpm_header(url={url}, end={end}, auth={auth is not None})")
    for attempt in range(retries):  # [0 1] for retries = 2
        try:
            ...
            if not auth:
                ...
                response = requests.get(
                    url, headers={'Range': f'bytes=0-{end}'}, auth=(auth.username, auth.password), timeout=(3.5, 10)
                )
            ...
            if response.status_code == 401:  # Unauthorized
                logger.error("Repo Authorization failure, ensure credentials are in keyring for this user.")
            return None
        # Catch all:
...
def get_zypper_repo_cache_filenames(repository_list=[]) -> typing.Dict[str, str]:
    """
    ...
    repodata_paths: typing.Dict[str, str] = {repo: f"/var/cache/zypp/raw/{repo}/repodata/" for repo in repository_list}
...
def get_packages_zypp_cache_tree(xml_tree: et.ElementTree, pkg_list, regex_enabled: bool = False) -> dict:
    """
    ...
    xml_ns = root.tag.split('}')[0].strip('{')  # "http://linux.duke.edu/metadata/common"
    packages_info: dict = {}
    package_older: bool
    ...
    for package in root.findall('doc:package', namespaces={'doc': xml_ns}):
        ...
        for ver_info in package[FORMAT_INDEX].findall("./rpm:provides/rpm:entry[1]",
            ...
            ver: str = ver_info.get('ver')
            rel: str = ver_info.get('rel')
            info_dict["ver"]: str = ver
            info_dict["rel"]: str = rel
            ver_rel: str = f"{ver}-{rel}"
            logger.debug(f"++ Found ({pkg_name}) ver-rel = {ver_rel}")
            if pkg_name in packages_info.keys():  # Ignore current version if saved is newer
                saved_ver_rel: str = f"{packages_info[pkg_name]['ver']}-{packages_info[pkg_name]['rel']}"
                try:
                    zypp_vcmp_run = run(
                        ["zypper", "versioncmp", saved_ver_rel, ver_rel],
        ...
        for field in package[FORMAT_INDEX].findall(  # One 'rpm:header-range' field per package
...
def get_zypper_changelog(repo_list=[], pkg_list=[], only_updates: bool = True,
                         ...
                         output_format: str | None = None) -> dict | None:
    """
    Library invocation: The importing agent should call this procedure.
    ...
    :param output_format: "dict" for dictionary, None (default) for stdout via print().
    :return: Determined by output_format; default is None.
    """
    ...
    if only_updates:
        ...
        per_repo_updates = get_updates_per_repo(repo_list)
    ...
    cache_files_dict = get_zypper_repo_cache_filenames(repo_list)
    enabled_repos_dict = get_zypper_repo_dict()
    if not all_repos:
        enabled_repos_dict = {item: value for item, value in enabled_repos_dict.items() if item in repo_list}
    ...
    for cached_repo_alias, zypp_cache_file in cache_files_dict.items():
        logger.debug(f"Considering cached repo ({cached_repo_alias}), file {zypp_cache_file}")
        url: str = ""
        ...
        if only_updates:
            ...
            if not pkg_intersection:
                logger.debug("-- No intersection with installable updates, packages filter, and this repo.")
                continue
            else:
        ...
        with tempfile.NamedTemporaryFile() as zypp_cache_tempfile:
            try:
                unzstd_run = run(
                    ["unzstd", zypp_cache_file, "--quiet", "--force", "-o", zypp_cache_tempfile.name],
        ...
        cached_pkg_info = get_packages_zypp_cache_tree(tree, pkg_list_per_repo,
        ...
        try:
            repo_auth = get_credential(service_name=f"zypper-changelog-lib/{cached_repo_alias}", username=None)
        ...
        # Greater fidelity in reporting would be nice, i.e. NoKeyringError
            ...
            logger.debug(f"KeyringError - for repo user/pass from service `zypper-changelog-lib`. Skipping repo.: {e}")
            continue
        if not repo_auth:
        ...
        for pkg_name, pkg_info in cached_pkg_info.items():
            ...
            rpm_header_content = fetch_rpm_header(rpm_header_url, pkg_info["header-end"], auth=repo_auth)
            ...
            for cname, ctime, ctext in zip(
                ...
                cname = decode_if_bytes(cname)
                ctext = decode_if_bytes(ctext)
                ...
                if titles_only:
                    ...
                    changelog += f"* {dt} {cname}\n{ctext}\n\n"
            # Package name 'header' before changelog output.
            pkg_ver_rel: str = pkg_info['ver'] + "-" + pkg_info['rel']
            match output_format:
                case None:
            ...
            if only_updates:
                local = local_changelog(pkg_name, titles_only)
                ...
                match output_format:
                    case None:
                        print(changelog)
                    case "dict":
                        changelog_dict[pkg_name] = changelog.split('\n')
...
def main():
    ...
    parser.add_argument(
        ...
        help="Comma separated list (no spaces) of packages to consider. Default is all installed packages."
    )
    parser.add_argument(
        "-r", "--repos", dest="repos",
        help="Comma separated list (no spaces) of repository aliases to consider. Default is all enabled repositories."
    )
    parser.add_argument(
        "-of", "--output-format", dest="outf",
        help="Output format: machine readable output, e.g. 'dict' for Python library use. Default is stdout prints for CLI use."
    )
    parser.add_argument(
