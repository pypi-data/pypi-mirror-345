...
from ...models.company.assets.chat_assets import SaleAssignedToChat, AssignedAssetToChat, ChatAssetType, ContactPointAssignedToChat
from ...models.chat.scheduled_messages import ScheduledMessageStatus
from ...models.utils.types.identifier import StrObjectId
from ...models.utils.custom_exceptions.custom_exceptions import AssetAlreadyAssigned, NotFoundError, MessageAlreadyInChat, MetaErrorNotification, ChatAlreadyAssigned
...
class ChatService:
    ...
    def add_tag(chat : Chat, tag : Tag, execution_context: ExecutionContext) -> AssignedAssetToChat:
        ...
        """
        if next((tag for tag in chat.client.tags if tag.asset_id == tag.id), None) is not None:
    ...
    def remove_tag(chat : Chat, tag_id : StrObjectId, tag : Tag, execution_context: ExecutionContext) -> AssignedAssetToChat:
        ...
        try:
            ...
            raise NotFoundError(message=f"Tag with id {tag_id} not found in chat {chat.id}")

    @staticmethod
    def add_workflow_link(chat : Chat, link : LinkItem, flow:FlowPreview, execution_context: ExecutionContext) -> FlowStateAssignedToChat:
        """
        Add a tag to the chat.
        """
        state = FlowStateAssignedToChat.from_link(link, execution_context)
    ...
    @staticmethod
    def add_contact_point(chat : Chat, execution_context: ExecutionContext, contact_point : ContactPoint) -> ContactPointAssignedToChat:
        """
        ...
        assigned_asset = ContactPointAssignedToChat(
            ...
            assigned_by=execution_context.executor.id,
            source_id=contact_point.source.id
        )
        bisect.insort(chat.client.contact_points, assigned_asset)
        ChatService.add_central_notification_from_text(chat=chat, body=f"TouchPoint de la fuente {contact_point.source.name} agregado al chat {chat.id} por {execution_context.executor.name}", subtype=MessageSubtype.CONTACT_POINT_ADDED)
        return assigned_asset

    @staticmethod
    def update_contact_point(chat : Chat, execution_context: ExecutionContext, contact_point : ContactPoint) -> ContactPoint:
        """
        Update a contact point for the chat.
        """
        ChatService.add_central_notification_from_text(chat=chat, body=f"TouchPoint de la fuente {contact_point.source.name} actualizado en el chat {chat.id} por {execution_context.executor.name}", subtype=MessageSubtype.CONTACT_POINT_UPDATED)
        return contact_point

    @staticmethod
    def delete_contact_point(chat : Chat, execution_context: ExecutionContext, contact_point_id : StrObjectId, contact_point : ContactPoint) -> ContactPointAssignedToChat:
        """
        ...
        try:
            ...
            ChatService.add_central_notification_from_text(chat=chat, body=f"TouchPoint de la fuente {contact_point.source.name} eliminado del chat {chat.id} por {execution_context.executor.name}", subtype=MessageSubtype.CONTACT_POINT_DELETED)
            return assigned_asset_to_remove
        except StopIteration:
            raise NotFoundError(message=f"TouchPoint de la fuente {contact_point.source.name} not found in chat {chat.id}")
    ...
    def update_message_status(chat : Chat, message_id : str, status : Status, error_details : Optional[ErrorDetail] = None ) -> ChattyMessage:
        ...
        if message is None:
            ...
            raise NotFoundError(message=f"Message with id {message_id} not found in chat {chat.id}")
    ...
    @staticmethod
    def add_central_notification_from_text(chat : Chat, body:str, subtype:MessageSubtype = MessageSubtype.SYSTEM, content_status:CentralNotificationStatus = CentralNotificationStatus.INFO) -> CentralNotification:
        """
        Add a central notification to the chat.
        """
        central_notification = CentralNotificationFactory.from_notification_body(notification_body=body, subtype=subtype, content_status=content_status)
    ...
    def archive_chat(chat : Chat, execution_context: ExecutionContext, current_agent:Optional[User]) -> Chat:
        ...
        if chat.area == Area.ARCHIVED:
            raise ValueError(f"Chat {chat.id} is already archived")
    ...
    def assign_chat(chat : Chat, agent_destination:User, execution_context: ExecutionContext, current_agent:Optional[User]) -> Chat:
        ...
        if current_agent is None: #No current agent - So its an assignment
            ...
            raise ChatAlreadyAssigned(f"Chat {chat.id} already assigned to {agent_destination.name}")
    ...
    def desassign_chat(chat : Chat, execution_context:ExecutionContext, current_agent:User) -> Chat:
        ...
        if chat.area != Area.WITH_AGENT:
            raise ValueError(f"Chat {chat.id} is not assigned to an agent")
    ...
    def read_chat(chat : Chat, execution_context:ExecutionContext) -> Chat:
        ...
        if chat.is_read_status:
            raise ValueError(f"Chat {chat.id} is already read")
    ...
    def unread_chat(chat : Chat, execution_context:ExecutionContext) -> Chat:
        ...
        if not chat.is_read_status:
            raise ValueError(f"Chat {chat.id} is not read")
    ...
    def block_chat(chat: Chat, execution_context: ExecutionContext) -> Chat:
        ...
        if chat.area == Area.BLOCKED:
            raise ValueError(f"Chat {chat.id} is already blocked")
