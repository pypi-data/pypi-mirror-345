#########################
 nexium Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
nexium, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so nexium can turn a ``ctypes`` binding
...
For nexium we use semantic versioning, initially with a leading zero
...
nexium top level works like this:

-  ``qutayba.tree.Building`` outputs node tree

-  ``qutayba.optimization`` enhances it as best as it can

-  ``qutayba.finalization`` prepares the tree for code generation

-  ``qutayba.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``qutayba.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``qutayba.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``qutayba_bool`` and ``qutayba_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for nexium
***************************************************
...
``.vscode`` in your nexium checkout. If you are not familiar with
...
In nexium we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with nexium, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-qutayba-source`` is
...
format call nexium source code.
...
These rules should generally be adhered when working on nexium code.
...
There is a tool ``bin/autoformat-qutayba-source`` which is to apply
...
they must be lower case, like e.g. ``qutayba`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``qutayba.code_generation`` the code generation packages are located,
while the main interface is ``qutayba.code_generation.CodeGeneration``
...
built-ins is highly discouraged within nexium source code. Using them is
...
readability and with nexium as a compiler, there won't be any
...
But of course, nexium is the project to free us from what is faster and
...
For nexium the choice is list contractions as these are more easily
...
that has no use, and in nexium, every API is internal API. One exception
...
``autoformat-qutayba-source`` does this for us.
...
******************************
 nexium "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto nexium
   repository <https://github.com/nexium/nexium> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/nexium.git
      cd nexium
      git remote add upstream https://github.com/nexium/nexium.git
...
`this location <https://qutayba.net/apidoc>`__ .
...
future, nexium itself will gain the ability to present its findings in a
...
This section describes how to run nexium tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if nexium is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if nexium
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if nexium
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if nexium
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         nexium does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if nexium
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles nexium with nexium, and
                         then nexium with the compile nexium and compares the
...
the nexium git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that nexium must do this itself, because the
...
this. These tests make sure that the errors of nexium and CPython are
...
Compile nexium with nexium
==========================

And there is the "compile itself" or "reflected" test. This test makes
nexium compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of nexium into an extension module and
all of nexium into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because nexium uses a lot of packages and imports between them.
...
parts are published as `nexium API <https://qutayba.net/apidoc>`__ and
...
Please find `here
<https://github.com/nexium/nexium/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/nexium/nexium/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/nexium-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/nexium-Logo-Horizontal.png
      :alt: nexium Logo
...
.. code:: rest
   ...
   .. image:: doc/images/nexium-Logo-Vertical.png
      :alt: nexium Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``qutayba/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/nexium-Logo-Symbol.svg doc/images/nexium-Logo-Symbol.png
   convert -background none doc/Logo/nexium-Logo-Vertical.svg doc/images/nexium-Logo-Vertical.png
   convert -background none doc/Logo/nexium-Logo-Horizontal.svg doc/images/nexium-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of nexium is decided here
...
nexium does not involve Scons in its user interface at all; Scons is
purely used internally. nexium itself, being pure Python, will run
without any build process just fine.

nexium simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which nexium again
...
.. note::
   ...
   ``qutayba.MainControl`` module.

For interfacing to Scons, there is the module
``qutayba.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of nexium.
...
modules is just one of them. It runs outside of nexium process scope,
...
-  ``qutayba_src``
   ...
   Where do the include files and static C parts of nexium live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``qutayba.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where nexium tries to help
...
The search for modules used is driven by ``qutayba.importing.Importing``
module.

-  Quoting the ``qutayba.importing.Importing`` documentation:
   ...
   End quoting the ``qutayba.importing.Importing`` documentation.
...
The decision making and caching are located in the ``qutayba.tree``
package, in modules ``qutayba.tree.Recursion`` and
``qutayba.tree.ImportCache``. Each module is only considered once (then
...
``qutayba/build/static_src/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the nexium provided
...
This left nexium with the strange problem, of how to emulate that.
...
The choice of nexium is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``qutayba.nodes.FrameNodes.StatementsFrame`` is
...
Representation in nexium
------------------------

So a slot in nexium typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs nexium do all the hard work.
...
Other overloads do not currently exist in nexium, but through the
...
In principle, every slot needs to be dealt with in nexium, and it is
...
Currently, nexium uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of nexium, created all constants for the whole program
...
This transformation is performed at tree building already, so nexium
...
In nexium we have the concept of an outline, and therefore we can make
...
explicit and comparisons in nexium to be internally always about two
operands only.
...
Therefore in nexium this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by nexium.
...
Quoting the ``qutayba.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in nexium have no condition attached anymore, so while loops are
...
End quoting the ``qutayba.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in nexium converted so that the code generation for ``print``
...
Obviously, optimization of nexium can decide, that e.g. should ``a`` or
...
nexium there is only a ``try``, which then has blocks to handle
...
compile time, nexium will not generate the code the leads to the
...
Add interfacing to C code, so nexium can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in nexium
...
Again, we wouldn't want to create the list. Therefore nexium avoids
...
This leads to nexium in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into nexium unless necessary, so
...
one design goal of nexium was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so nexium is using it,
   and of course one day nexium ought to be able to make no difference
   in performance for it.

To nexium though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   qutayba --experimental=jinja_generated_add ...
...
.. code:: C
   ...
   #ifdef _DEVILPY_EXPERIMENTAL_JINJA_GENERATED_ADD
...
experimental flag is given on the command line to nexium.

The testing of nexium is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to nexium
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when nexium is
executing.
...
nexium to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and nexium can work without it
...
.. note::
   ...
   with ``@nexium-pushers`` first and get a green light.
...
the actual compiler code of nexium.
...
like ``autoformat-qutayba-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # qutayba-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from qutayba.tools.testing.Common import checkReferenceCount
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In nexium
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, nexium will become able to determine it has to be the
...
https://github.com/nexium/nexium/blob/develop/Developer_Manual.rst

And the current PDF under: https://qutayba.net/doc/Developer_Manual.pdf
