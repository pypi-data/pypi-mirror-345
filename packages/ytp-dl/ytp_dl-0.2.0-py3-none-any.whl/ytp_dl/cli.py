...
def get_available_heights(vid: str, proxies: list[str]) -> list[int]:
    """
    Try each proxy in turn to fetch metadata; if all proxies fail,
    retry indefinitely (brute‑force) with no delay.
    """
    ...
    while True:
        for proxy in proxies:
            ...
            try:
                ...
                # Only log the extractor error itself, never the proxy URI
                logger.warning(str(e))
                last_exc = e
                continue

        # exhausted this round of proxies—retry immediately
        logger.info("All proxies failed to fetch metadata; retrying immediately…")
...
async def fetch_pair(vid: str, height: str, proxies: list[str]) -> dict:
    ...
    then pick the best formats and return the URLs.
    """
...
async def _amain():
    ...
    # build proxy list once
    proxies = [p.strip() for p in args.proxy.split(",") if p.strip()]
    if not proxies:
        sys.exit("Error: no proxies provided via -p")

    # metadata lookup with infinite retries
    heights = get_available_heights(vid, proxies.copy())
