...
class Progression:
    ...
    def values(self):
        ...
        if self._values is None:
            start, end = (self.end, self.start) if self.start > self.end else (self.start, self.end)
            ...
            values = [
                ...
                for value in values
            ] if self.start > self.end else values
    ...
    def rate_function(self, value: RateFunctionArgument):
    ...
    def __init__(
        ...
        if not NumberValidator.is_number(start) or not NumberValidator.is_number(end):
            raise Exception('The provided "start" and "end" parameter must be numbers.')

        if not NumberValidator.is_positive_number(n) or n < 2:
            raise Exception('The provided "n" parameter is not a positive number greater or equal than 2.')
    ...
    def get_n_equally_distributed_normalized_values(
        ...
        normalized) values between the 'start' and 'end'
        (included). The 'n' value must be equal or greater
        than 2.
        ...
        return np.linspace(0.0, 1.0, n).tolist() if n > 0 else []
    ...
    def get_n_normalized_values(
        ...
        """
        Get a list containing 0.0 and 1.0 limits and 'n' - 2
        values in between those 'start' and 'end' limits
        according to the provided 'rate_function'.

        If n = 6 and rate_function = ease_in_quad:
        - [0.0, 0.04, 0.16, 0.36, 0.64, 1.0]
        """
        if not NumberValidator.is_positive_number(n) or not n >= 2:
            raise Exception('The provided "n" parameter is not a positive number greater or equal than 2.')
