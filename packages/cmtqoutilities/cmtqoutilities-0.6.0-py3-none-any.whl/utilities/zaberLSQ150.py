...
class LSQ150():
    """
    Class to handle the Zaber LSQ150 XY stage.
    ...
    def __init__(self,port,switchAxes=False,units='mm'):
        """
        ...
        switchAxes: bool
          By default set to False. If set true, the x- and y-axis get switched.
        ...
        # the origin is always save in terms of the absolute stage coordiante system
        ...
        self.microstepSize = 0.09921875e-6
        # set origin of coordinate system
        self.origin = np.array([0,0])
        # switch the axis x <-> y
        self.switchAxis = switchAxes
        # axis identifiers
        self.xAxis = 1
        self.yAxis = 2
        if switchAxes:
    ...
    def execute(self,axis,content):
        ...
        axis: int
          Axis specifier. Has to be either self.xAxis, or self.yAxis.
    ...
    def rotMat(self,angle):
        ...
        Rotation matrix to rotate the coordinate system.
    ...
    def rotateCoordSysToRefPoint(self,pos):
        """
        Rotate coordinate system around its current origin such that the angle between the line defined by the origin and the point pos with respect to the x-axis
        ...
        """
        if pos[0]**2 > 0:
            desiredAngle = np.mod((np.arctan(pos[1]/pos[0])-(np.sign(pos[0])-1)*np.pi/2),2*np.pi)
        else:
            desiredAngle = np.mod(np.sign(pos[1])*np.pi/2,2*np.pi)
        actPos = self.getPos()
        if actPos[0]**2 > 0:
            actAngle = np.mod((np.arctan(actPos[1]/actPos[0])-(np.sign(actPos[0])-1)*np.pi/2),2*np.pi)
        else:
            actAngle = np.mod(np.sign(actPos[1])*np.pi/2,2*np.pi)
        return self.rotateCoordSys(actAngle-desiredAngle)
    
    # rotate coordiante system
    def rotateCoordSys(self,angle):
        """
        Rotate coordinate system around its current origin by angle.
    ...
    def getPos(self):
        ...
        if not successx or not successy:
            ...
            return self.getUserCoords(np.array([self.toDistance(int(replyx.data)),0.151/self.scale-self.toDistance(int(replyy.data))]))
    
    # move to absolute value pos. Pos is with respect to the current coordinate system.
    def moveTo(self,pos):
        """
        Move to the absolute position pos. Works with repsect to current coordinate system.
        
        Parameters
        ==========
        
        pos: 2-value array or list
          x and y position of the desired position in units of self.units.
        ...
        [successx,reply] = self.execute(self.xAxis,self.cmd["moveTo"]+"%d"%self.toMicrosteps(coords[0]))
        [successy,reply] = self.execute(self.yAxis,self.cmd["moveTo"]+"%d"%self.toMicrosteps(0.151/self.scale-coords[1]))
        if not successx or not successy:
            self.printMsg("Could not nove to requested position!")
    
    # move relative to current position
    def moveRel(self,diff):
        """
        Move relative from the current position by diff. Works with repsect to current coordinate system.
        
        Parameters
        ==========
        
        pos: 2-value array or list
          x and y distance to be made in units of self.units.
        ...
        """
        diffCoords = self.getStageCoords(diff)-self.origin
        [successx,reply] = self.execute(self.xAxis,self.cmd["moveRel"]+"%d"%self.toMicrosteps(diffCoords[0]))
        [successy,reply] = self.execute(self.yAxis,self.cmd["moveRel"]+"%d"%self.toMicrosteps(-diffCoords[1]))
        if not successx or not successy:
            self.printMsg("Could not nove to requested position!")
    
    # select a coordinate system
    def setCoordSys(self,key='abs',origin=np.zeros(2)):
        """
        ...
        origin: 2-value list or np.array
