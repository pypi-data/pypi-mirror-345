...
class WorkerPool:
    ...
    def update_jobs_status():
        ...
        pipeline = [
            {  # 1. Find WAITING jobs with due date
                '$match': {
                    'status': {'$in': ['NEW', 'WAITING']},
                    'date': {'$lt': now}
                }
            },
            {  # 2. Lookup dependencies' statuses
                '$lookup': {
            ...
            {  # 3. Project a field that's true if all deps['status'] == 'COMPLETED'
                '$addFields': {
                    'all_deps_completed': {
                        '$cond': [
                            # If there are no dependencies, treat as all completed.
                            {'$gt': [{'$size': '$dependencies'}, 0]},
                            {
                                '$allElementsTrue': {
                                    '$map': {
                                        'input': "$deps",
            ...
            {  # 4. Keep only ready jobs
                '$match': {'all_deps_completed': True}
            },    
            # 5. Project only _id (or other needed fields)
            {
    ...
    def fetch_job(workername, njobs=1):
        ...
        filter_query = {
            ...
            'status': 'PENDING'  # Only fetch jobs that are in 'WAITING' status
        }
