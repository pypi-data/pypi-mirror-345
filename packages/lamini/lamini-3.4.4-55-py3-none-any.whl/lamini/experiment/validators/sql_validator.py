...
class SQLValidator(BaseSQLValidator):
    ...
    def __call__(self, prompt_obj, debug=False):
        ...
        valid_autofixes = set(
            ["extract_sql", "fix_statement_count", "fix_column", "fix_dates"]
        )
        autofixes = valid_autofixes - set(self.skip_autofixes)

        if len(autofixes) + len(self.skip_autofixes) > len(valid_autofixes):
            raise Exception(
                f"Valid autofix options are {valid_autofixes}. You have used {self.skip_autofixes}"
            )
        ...
        if len(autofixes) > 0:
            ...
            if (
                not can_explain_query_plan
                and "fix_statement_count" in autofixes
                and query
            ):
            ...
            if not can_explain_query_plan and "fix_column" in autofixes and query:
                query = sql_autofix.fix_invalid_col(
                    query, self.col_table_map, self.db_type
                )
        ...
        result = self.process_results(prompt_obj)
