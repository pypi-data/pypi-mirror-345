    ...
    for (int j = 0; j < repeat_group_certs_count; j++) {
      ...
      if (x509_name_out) {
        ...
        if (lpDN) {
          ...
          throw RACFuError(std::string("X509_NAME_oneline failed"));
        }
        X509_NAME_free(x509_name_out);
      } else {
        throw RACFuError(std::string("d2i_X509_NAME failed"));
      }
      ...
      if (x509_cert) {
        ...
        if (x509_name_out) {
          ...
          throw RACFuError(std::string("X509_get_issuer_name failed"));
        }
...
void KeyringPostProcessor::convertASN1TIME(ASN1_TIME *t, char *p_buf,
  ...
  if (rc <= 0) {
    throw RACFuError(std::string("ASN1_TIME_print failed or wrote no data."));
  }
    ...
    if (EVP_DigestInit_ex(context.get(), EVP_sha256(), NULL)) {
      if (EVP_DigestUpdate(context.get(), p_cert, len_cert)) {
        if (EVP_DigestFinal_ex(context.get(), hash, &hash_len)) {
          ...
          throw RACFuError(std::string("EVP_DigestFinal_ex failed"));
      } else
        throw RACFuError(std::string("EVP_DigestUpdate failed"));
    } else
      throw RACFuError(std::string("EVP_DigestInit_ex failed"));
