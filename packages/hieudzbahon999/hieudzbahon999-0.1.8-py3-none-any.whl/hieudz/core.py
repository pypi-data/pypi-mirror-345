...
def chatai(mess: str) -> None:
    """
    Gửi tin nhắn đến Gemini API và hiển thị phản hồi được định dạng đẹp.

    Args:
        mess (str): Tin nhắn gửi đến Gemini.
    """
    # URL của Gemini API (dùng gemini-2.0-flash)
    ...
    if response_text is None:
        print("Tất cả API keys đều không hoạt động. Vui lòng kiểm tra lại.")
        return

    # Định dạng và hiển thị phản hồi
    print("\n=== Phản hồi từ Gemini AI ===")

    # Tách các phần của phản hồi (code và giải thích)
    ...
    for part in parts:
        ...
        if part.startswith('```python') and part.endswith('```'):
            ...
            # In từng dòng của code với thụt lề
            for line in code.split('\n'):
                print(f"    {line}")
        else:
            # Đây là phần giải thích hoặc văn bản khác
            if in_code_block:
                in_code_block = False
                print("\n[Giải thích]")
            # Xử lý tiêu đề Markdown (##, ###, v.v.)
            if part.startswith('**') and part.endswith('**'):
                # Tiêu đề
                title = part[2:-2].strip()
                print(f"\n[{title}]")
            else:
                # Văn bản thường
