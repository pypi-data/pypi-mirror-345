...
class BigQueryClient:
    ...
    def sql2gcs(self, sql: str,
        ...
        try:
            ...
            if wait_for_completion:
                ...
                extract_job.result(timeout=timeout)  # This waits for the job to finish and raises an exception if fails
    ...
    def erd(self, table_list, output_file="erd.gv", format="pdf", dpi=300, max_width=50, max_height=50):
        """
        ...
        Args:
            ...
            format (str, optional): Output format ('png', 'svg', 'pdf'). Defaults to "pdf".
        ...
        erd.attr(
            ...
            ratio="fill",
            margin="0.3",
            rankdir='LR',  # LR is better for entity relationships
            concentrate='false', 
            splines='ortho',
            overlap='false',
            fontname='Arial',
            bgcolor='#E8E8E8',  # Light gray background like a grid
            style='filled',
            # Add grid pattern
            bgcolorlist='#E8E8E8;#DFDFDF',
            gradientangle='45',
            compound='true'
        )
        ...
        # Track detected relationships for drawing later
        relationships = []
        ...
        for table_ref in table_list:
            try:
                ...
                # For complex diagrams, limit columns shown
                if is_complex and len(table.schema) > 10:
                    # Show only key columns and a sample of others
                    key_fields = [f for f in table.schema if f.name.endswith('_id') or f.name == 'id']
                    other_fields = [f for f in table.schema if not (f.name.endswith('_id') or f.name == 'id')]
                    
                    # Limit other fields to keep diagram readable
                    if len(other_fields) > 8:
                        other_fields = other_fields[:8]
                        
                    display_fields = key_fields + other_fields
                else:
                    display_fields = table.schema
                
                # Create styled HTML table similar to the example image                
                label = f'''<<TABLE BORDER="1" CELLBORDER="1" CELLSPACING="0" CELLPADDING="3" BGCOLOR="white">
                <TR><TD COLSPAN="3" BGCOLOR="#555555" PORT="header"><FONT COLOR="white" POINT-SIZE="12"><B>{simple_table_id}</B></FONT></TD></TR>'''
                
                # Add PK indicator column if we have IDs
                has_pk = any(f.name == 'id' or f.name.endswith('_id') for f in display_fields)
                if has_pk:
                    label += f'''<TR><TD WIDTH="30">PK</TD><TD WIDTH="30">type</TD><TD>name</TD></TR>'''
                else:
                    label += f'''<TR><TD WIDTH="30">type</TD><TD COLSPAN="2">name</TD></TR>'''
                
                # Add columns as rows with compact styling
                for schema_field in display_fields:
                    field_name = schema_field.name
                    field_type = schema_field.field_type.lower()  # bq types like INT64, STRING, etc.
                    
                    # Determine field styling
                    is_pk = field_name == 'id'
                    is_fk = field_name.endswith('_id') and not is_pk
                    is_date = field_type.startswith('date') or 'time' in field_type or 'timestamp' in field_type
                    
                    # Set colors similar to example (blue for IDs, red for dates)
                    name_color = ''
                    if is_fk:
                        name_color = ' COLOR="#0000FF"'  # Blue for foreign keys
                    elif is_date:
                        name_color = ' COLOR="#FF0000"'  # Red for dates
                    
                    # Create row based on whether we're showing PK column
                    if has_pk:
                        pk_mark = '<B>★</B>' if is_pk else ''
                        label += f'''<TR>
                        <TD ALIGN="CENTER">{pk_mark}</TD>
                        <TD ALIGN="LEFT" PORT="type_{field_name}"><FONT POINT-SIZE="9">{field_type}</FONT></TD>
                        <TD ALIGN="LEFT" PORT="{field_name}"><FONT POINT-SIZE="9"{name_color}><B>{field_name}</B></FONT></TD>
                        </TR>'''
                    else:
                        label += f'''<TR>
                        <TD ALIGN="LEFT" PORT="type_{field_name}"><FONT POINT-SIZE="9">{field_type}</FONT></TD>
                        <TD COLSPAN="2" ALIGN="LEFT" PORT="{field_name}"><FONT POINT-SIZE="9"{name_color}><B>{field_name}</B></FONT></TD>
                        </TR>'''
                
                # If we truncated columns, add an indicator
                if len(table.schema) > len(display_fields):
                    hidden_count = len(table.schema) - len(display_fields)
                    colspan = "3" if has_pk else "2"
                    label += f'''<TR><TD COLSPAN="{colspan}" BGCOLOR="#F9F9F9" ALIGN="CENTER">
                    <FONT POINT-SIZE="8" COLOR="#777777">... {hidden_count} more columns</FONT></TD></TR>'''
                
                label += '</TABLE>>'
                
                erd.node(table_id, label, margin='0.2')
            except Exception as e:
            ...
            for column in table.schema:
                ...
                if column.name.endswith("_id") and column.name != 'id':
                    ...
                    for ref_table_id in table_objects:
                        ...
                        # If we find a matching table name
                        if ref_table_name == referenced_table.lower():
                            # Look for primary key in referenced table
                            target_column = None
                            
                            # Try to find id field in target table
                            for ref_column in table_objects[ref_table_id].schema:
                                if ref_column.name == 'id' or ref_column.name == ref_table_name + '_id':
                                    target_column = ref_column.name
                                    break
                            
                            # Fall back to first column if no id found
                            if not target_column and table_objects[ref_table_id].schema:
                                target_column = table_objects[ref_table_id].schema[0].name
                            
                            # Store relationship for drawing
                            if target_column:
                                relationships.append({
                                    'source_table': table_id,
                                    'source_column': column.name,
                                    'target_table': ref_table_id,
        ...
        for rel in relationships:
            ...
            # Draw arrow similar to example (simple arrow line)
            erd.edge(
                source, 
                target,
                color='#000000',
