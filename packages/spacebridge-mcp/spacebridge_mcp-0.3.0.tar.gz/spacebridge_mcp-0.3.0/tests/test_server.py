...
async def test_get_issue_tool_handler_success():
    ...
    with patch("spacebridge_mcp.server.spacebridge_client", mock_client_instance):
        # Call the handler
        result_data: Dict[str, Any] = await get_issue_tool_handler(issue=issue_id) # Use 'issue'

    # Assert the method on the mocked global client was called
    mock_client_instance.get_issue.assert_called_once_with(issue=issue_id) # Assert with 'issue'
...
async def test_get_issue_tool_handler_not_found():
    ...
    with patch("spacebridge_mcp.server.spacebridge_client", mock_client_instance):
        ...
        with pytest.raises(Exception, match="Simulated client error"):
            await get_issue_tool_handler(issue=issue_id) # Use 'issue'

    # Assert the method on the mocked global client was called
    # Client get_issue expects 'issue' kwarg
    mock_client_instance.get_issue.assert_called_once_with(issue=issue_id)
...
async def test_search_issues_handler_context_priority(
    ...
    with patch("spacebridge_mcp.server.spacebridge_client", mock_client_instance):
        # Call handler using its signature: org, project, status, labels, assignee, priority
        result: SearchIssuesOutput = await search_issues_handler(
            query=query,
            search_type=search_type,
            org=tool_context.get("org"),
            project=tool_context.get("proj"),
            status=None, # Pass None for filters in this test case
            labels=None,
            assignee=None,
            priority=None,
        )

    # Assert the client method was called with the correctly prioritized context and filters
    # Client method expects org_name/project_name and the filters
...
async def test_create_issue_handler_new(mock_openai_client_instance):
    ...
    with patch("spacebridge_mcp.server.spacebridge_client", mock_sb_client_instance):
        # Call handler using its signature: org, project, labels, assignee, priority, status, similarity_search
        result: CreateIssueOutput = await create_issue_handler(
            ...
            priority=None,
            status=None,
            similarity_search=True, # Explicitly pass default
        )

    # Assert client methods were called with the prioritized context (Tool > Startup)
    # Search call uses org_name/project_name; handler doesn't pass filters here
    mock_sb_client_instance.search_issues.assert_called_once_with(
        ...
        # status, labels, assignee, priority are NOT passed by handler here
    )
    # Create call uses org_name/project_name and expects other args (even if None)
    mock_sb_client_instance.create_issue.assert_called_once_with(
        title=title,
        description=description,
        org_name=tool_context.get("org"),
        project_name=tool_context.get("proj"),
        labels=None, # Client create_issue expects List[str] or None
        # Pass other fields client expects if provided in handler call
        assignee=None,
        priority=None,
        status=None,
    )
...
async def test_create_issue_handler_duplicate_found(mock_openai_client_instance):
    ...
    # Assert search was called with prioritized context (Tool > Startup)
    # Client method expects org_name/project_name; handler doesn't pass filters here
    mock_sb_client_instance.search_issues.assert_called_once_with(
        query=f"{title}\n\n{description}",
        search_type="similarity",
        org_name=tool_context.get("org"),
...
async def test_create_issue_handler_not_duplicate(mock_openai_client_instance):
    ...
    # Client method expects org_name/project_name and other args (even if None)
    mock_sb_client_instance.create_issue.assert_called_once_with(
        title=title,
        description=description,
        org_name=expected_call_context.get("org"), # Client expects org_name
        project_name=expected_call_context.get("proj"), # Client expects project_name
        labels=None,
        # Add other fields client expects if necessary
    )
    # Assert OpenAI was called
    mock_openai_client_instance.chat.completions.create.assert_called_once()
...
async def test_update_issue_handler_no_fields():
    ...
    # Call handler with only issue identifier - Use 'issue', remove org/proj
    result: UpdateIssueOutput = await update_issue_handler(
        issue=issue_id, # Use 'issue'
        # No update fields (title, desc, status, etc.) provided
    )
