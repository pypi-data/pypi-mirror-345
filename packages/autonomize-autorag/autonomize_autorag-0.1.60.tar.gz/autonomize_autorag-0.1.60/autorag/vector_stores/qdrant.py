...
class QdrantVectorStore(VectorStore):
    ...
    def __init__(
        ...
        enable_https_string = os.getenv("QDRANT_ENABLE_HTTPS", "")
        enable_https: bool = True
        if enable_https_string.lower() == "false":
            enable_https = False
        ...
        **kwargs: Any,
        ...
        if use_client_provided_id:
            ...
            for metadata in metadatas:
                ...
                input_id = metadata.pop("id")
                is_valid_id: bool = False
                if input_id:
                    is_valid_id = self._is_valid_qdrant_id(input_id)
                if not is_valid_id:
                    raise VectorStoreDataMismatchException(
                        "Error in id field If client provided id is to be used for QDRANT points it should be unsigned 64 bit integer or UUID string"
                    )
                ids.append(input_id)  # type: ignore
