...
from SajadMr import TreeXML
from SajadMr.__past__ import unicode
from SajadMr.build.DataComposerInterface import getDataComposerReportValues
from SajadMr.build.SconsUtils import readSconsErrorReport
from SajadMr.code_generation.ConstantCodes import getDistributionMetadataValues
from SajadMr.containers.OrderedSets import OrderedSet
from SajadMr.freezer.IncludedDataFiles import getIncludedDataFiles
from SajadMr.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from SajadMr.freezer.Standalone import getRemovedUsedDllsInfo
from SajadMr.importing.Importing import getPackageSearchPath
from SajadMr.importing.Recursion import getRecursionDecisions
...
from SajadMr.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from SajadMr.plugins.Plugins import getActivePlugins
...
from SajadMr.Tracing import ReportingSystemExit, reports_logger
...
from SajadMr.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    SajadMr_version = getSajadQVersion()
    SajadMr_commercial_version = getCommercialVersion() or "not installed"

    SajadMr_aborted = aborted

    SajadMr_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["SajadMr_aborted"]:
        completion = "yes"
    elif report_input_data["SajadMr_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["SajadMr_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["SajadMr_exception"][1].code
    elif report_input_data["SajadMr_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["SajadMr_exception"][1].code
        )
        exit_message = report_input_data["SajadMr_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "SajadMr-compilation-report",
        SajadMr_version=report_input_data["SajadMr_version"],
        SajadMr_commercial_version=report_input_data["SajadMr_commercial_version"],
...
_crash_report_filename = "SajadMr-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of SajadQ but
