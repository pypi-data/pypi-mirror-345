...
def validate_template_name(ctx, param, value):
    ...
    if "/" in value or "\\" in value:
        raise click.BadParameter(f"Invalid template name: {value}. Template name cannot contain slashes.")
...
def parse_variable(variable_str):
    if "=" not in variable_str:
        raise click.BadParameter(f"Invalid variable format: {variable_str}. Expected format: name=value")
...
@click.option("--variable", "-v", multiple=True, help="Define a variable for the template (format: name=type)")
...
@click.option("--force/--no-force", default=False, help="Force creation even if project already exists")
@click.option("--variable", "-v", multiple=True, help="Override variables for the template (format: name=value)")
def create(template_name, project_name, force, variable):
    ...
    if project_dir.exists():
        if not force:
            raise click.BadParameter(f"Project directory already exists: {project_name}")
