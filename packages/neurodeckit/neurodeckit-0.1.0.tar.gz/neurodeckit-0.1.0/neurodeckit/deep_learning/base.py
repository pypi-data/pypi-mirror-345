...
class SkorchNet2:
    ...
    def __call__(self, *args, **kwargs):
        ...
        # 提取dtype、seed参数并从net_params中移除
        dtype = net_params.pop('dtype', 'float64')
        seed = net_params.pop('seed', 42)
        patience = net_params.pop('patience', 50)
        ...
        # 确保net_params中只包含default_net_params中的关键字参数
        net_params = {k: v for k, v in net_params.items() if k in default_net_params}
        
        # 更新默认参数
        default_net_params.update(net_params)

        # 计算T_max
        T_max_value = default_net_params['max_epochs'] - 1

        # 添加回调函数
        default_net_params['callbacks'] = [
            (
                "train_acc",
                EpochScoring(
                    "accuracy",
            ...
            ("lr_scheduler", LRScheduler(policy="CosineAnnealingLR", T_max=T_max_value)),
            ("estoper", EarlyStopping(patience=patience)),
            (
                "checkpoint",
                Checkpoint(dirname=f"checkpoints/{str(id(model))}"),
            ),
        ]
