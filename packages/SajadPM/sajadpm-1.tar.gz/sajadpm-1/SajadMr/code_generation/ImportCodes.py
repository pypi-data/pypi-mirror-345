...
from SajadMr.HardImportRegistry import isHardModule, isHardModuleDynamic
from SajadMr.nodes.LocalsScopes import GlobalsDictHandle
...
def generateConstantSysVersionInfoCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        ...
        emit("""%s = SajadQ_SysGetObject("%s");""" % (value_name, "version_info"))
...
def getImportModuleNameHardCode(
    ...
    if module_name == "sys":
        emit("""%s = SajadQ_SysGetObject("%s");""" % (to_name, import_name))
        needs_release = False
