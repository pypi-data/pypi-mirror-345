...
class TestFrameRegistryExtras(unittest.TestCase):
    def test_str_and_repr(self):
        cs = FrameRegistry.from_directions(Direction.UP, Direction.RIGHT, Direction.BACKWARD)
    ...
    def test_pickle_roundtrip(self):
        ...
        cs = FrameRegistry.from_directions(Direction.FORWARD, Direction.LEFT, Direction.DOWN)
        s = pickle.dumps(cs)
        cs2 = pickle.loads(s)
...
class TestTransformExtras2(unittest.TestCase):
    ...
    def test_convert_to_input_transformed(self):
        """
        ...
        tr = Transform.from_values(translation=np.array([5,0,0]), coordinate_system=cs1)
...
class TestTransformExtras(unittest.TestCase):
    ...
    def test_matmul_frame_mismatch(self):
        ...
        A = Transform.from_values(translation=np.array([1,0,0]), coordinate_system=cs1)
        B = Transform.from_values(translation=np.array([0,2,0]), coordinate_system=cs2)
    ...
    def test_convert_to_reexpress_input(self):
        """
    ...
    def test_roundtrip_inverse(self):
        ...
        # inv stays unchanged
        np.testing.assert_allclose(inv.matrix @ inv2.matrix, np.eye(4), atol=1e-8)
        ...
        np.testing.assert_allclose(inv_ip.matrix @ orig_ip, np.eye(4), atol=1e-8)
...
class TestFrameRegistry(unittest.TestCase):
    ...
    def test_rotation_to_identity_and_det(self):
        ...
        frames = all_frames()
        n = len(frames)
        print(f"Testing rotation_to across {n} valid frames → {n} valid frames ({n*n} pairs)")
        tol = 1e-16
        ...
        for i, (tag1, cs1) in enumerate(frames):
            for j, (tag2, cs2) in enumerate(frames):
                ...
                for vec_name in ("forward", "left", "up"):
                    ...
                    mapped = R @ v1
                    self.assertTrue(np.allclose(mapped, v2, atol=tol), f"[{i}->{j}] mapping {vec_name} failed:\n{tag1}→{tag2}, R@{vec_name} = {mapped}, expected {v2}")

                # 2) determinant
                detR = np.linalg.det(R)
                expected = +1.0 if tag1 == tag2 else -1.0
                self.assertTrue(np.isclose(detR, expected, atol=tol), f"[{i}->{j}] det(R)={detR}, expected {expected} (tags {tag1}->{tag2})")
