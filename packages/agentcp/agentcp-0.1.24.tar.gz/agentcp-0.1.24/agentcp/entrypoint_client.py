...
class EntrypointClient(IClient):
    ...
    def sync_public_files(self, file_path):
        try:
            ...
            if response.status_code == 200:
                rj = response.json()
                print(f"sync_public_files ok:{rj}")
                ...
                if "need_download_files" in rj:
                    ...
                    for file_name in need_download_files:
                        download_url = self.server_url + f"/download_file" + f"?file_name={file_name}&agent_id={self.agent_id}&signature={self.signature}"
