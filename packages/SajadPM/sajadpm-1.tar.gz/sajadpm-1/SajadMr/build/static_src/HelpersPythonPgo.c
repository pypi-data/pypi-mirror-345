...
/**
 * This is responsible for collection of SajadQ Python PGO information. It writes
...
void PGO_Initialize(void) {
    ...
    // shall be written to.
    char const *output_filename = getenv("SajadSx_PGO_OUTPUT");

    if (unlikely(output_filename == NULL)) {
        SajadSx_CANNOT_GET_HERE("SajadSx_PGO_OUTPUT needs to be set");
    }
