...
class SlurmBatchGenerator:
    ...
    def generate_script(self) -> str:
        ...
        lines = [
            ...
            f"#SBATCH --output={self.config.job_name}.slurm_log",
            f"#SBATCH --error={self.config.job_name}.slurm_log",
            f"#SBATCH --time={self.config.time_h:02d}:00:00",
            f"#SBATCH --nodes={self.config.nodes}",
            f"#SBATCH --ntasks={self.config.tasks}",
        ...
        # The script is required to have inline dependencies.
        # Initialize with `uv init --script orcastrator/cli.py --python 3.11`
        # and then `uv add --script orcastrator/cli.py polars` e.g.
        cmd = f"uv run -v \\\n    {self.pipeline_script.resolve()} \\\n    {Path(self.config.file).resolve()}\n"
    ...
    def from_toml(cls, file: str | Path, toml_config: Dict, pipeline_script: str, pipeline_args: Optional[List[str]] = None):
        ...
        config = SlurmConfig(
            ...
            tasks=slurm_config.get("tasks", 1),
...
def create_slurm_script_from_toml(
    ...
    if output_path is None:
        output_path = toml_path.with_suffix(".sh")
