...
class BigQueryClient:
    ...
    def erd(self, table_list, output_file="erd.gv"):
        ...
        erd.attr('graph', rankdir='LR')
        erd.attr('node', shape='box')
        erd.attr('edge', arrowhead='open')
        ...
        for table_ref in table_list:
            try:
                ...
                columns = [f"{schema_field.name} ({schema_field.field_type})" for schema_field in table.schema]
                erd.node(table_id, f"{table_id}\n{', '.join(columns)}")
            except Exception as e:
                ...
                if column.name.endswith("_id") or column.name.endswith("_key"):
                    ...
                    for ref_table_id in table_objects:
                        ...
                        if ref_table_name.lower() == referenced_table.lower():
                            erd.edge(table_id, ref_table_id, label=column.name)
