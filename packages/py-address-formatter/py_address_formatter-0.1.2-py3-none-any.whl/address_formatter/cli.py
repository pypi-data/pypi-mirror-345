...
def parse_args(args=None):
    ...
    # Determine the program name from the entry point
    prog_name = os.path.basename(sys.argv[0])

    # Default to the new name if not called directly
    if prog_name == "__main__.py":
        prog_name = "py-address-formatter"
...
def main() -> None:
    """Run the CLI."""
    # Check if we're being called with the old name for backward compatibility
    prog_name = os.path.basename(sys.argv[0])
    if prog_name == "address-formatter":
        print("Warning: The 'address-formatter' command is deprecated. Please use 'py-address-formatter' instead.")
