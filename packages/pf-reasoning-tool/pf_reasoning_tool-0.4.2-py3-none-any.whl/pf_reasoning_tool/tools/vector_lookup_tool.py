...
FAISS, Chroma.  Requires an AzureOpenAIConnection or OpenAIConnection for
embeddings.  Single quotes, two spaces.
'''
...
def _get_store(
    ...
    if b == 'faiss':
        ...
        p = Path(conn.index_path)
        if not p.exists():
            raise FileNotFoundError(f'FAISS index not found: {p}')
        return FAISS.load_local(folder_path=str(p), embeddings=embeddings)
    if b == 'chroma':
...
def hw_vector_lookup(
    ...
    '''
    HW Vector Lookup Tool – returns the top-k similar docs with optional scores.
    '''
    ...
    for doc, score in docs_scores:
        record = {'text': doc.page_content, 'metadata': doc.metadata}
        if include_scores:
            record['score'] = score
