...
class FloPyAgent():
    ...
    def getActionType(self, ENVTYPE):
        """
        Determine the action type based on the environment type string.

        This method inspects the `ENVTYPE` string to identify whether the action
        space of the environment is discrete or continuous. It looks for specific
        substrings within `ENVTYPE` to make this determination:
        - If `'-d'` is found, the action type is considered 'discrete'.
        - If `'-c'` is found, the action type is considered 'continuous'.
        If neither substring is present, the method prints an error message and
        terminates the program.

        Args:
            ENVTYPE (str): A string representing the environment type. Expected to
                contain '-d' for discrete or '-c' for continuous action spaces.

        Returns:
            str: The action type, either `'discrete'` or `'continuous'`.

        Raises:
            SystemExit: If `ENVTYPE` does not contain either '-d' or '-c', the
    ...
    def initializeDQNAgent(self):
        """Initialize the agent to perform Deep Double Q-Learning (DDQN).

        This method sets up the main and target neural network models used for DDQN,
        initializes the replay memory buffer, sets the initial exploration rate (epsilon),
        and prepares the counter for updating the target network. It includes a workaround
        related to a known Keras predict function bug (https://github.com/keras-team/keras/issues/6462).
        ...
        Raises:
            KeyError: If 'ENVTYPE' key is missing in self.envSettings.
            KeyError: If required hyperparameters like 'REPLAYMEMORYSIZE' or 'EPSILONINITIAL' are missing in self.hyParams.

        Example:
            >>> agent = YourAgentClass()
            >>> agent.envSettings = {'ENVTYPE': '3s-d'}
            >>> agent.hyParams = {'REPLAYMEMORYSIZE': 1000, 'EPSILONINITIAL': 0.95}
            >>> agent.initializeDQNAgent()
        """
    ...
    def initializeGeneticAgents(self):
        """Initialize an ensemble of agents for genetic algorithm-based optimization.

        This method creates and configures agents based on genetic algorithm parameters.
        Depending on the `KEEPMODELHISTORY` setting, it either:
        - Processes agents in parallel chunks using multiprocessing (if True), or
        - Initializes mutation history tracking for sequential agent creation (if False).

        Args:
            None (uses instance attributes from class initialization)

        Returns:
            None

        Raises:
            KeyError: If required keys ('KEEPMODELHISTORY', 'NAGENTSPARALLEL', etc.)
                are missing in `self.envSettings` or `self.hyParams`.

        Example:
            >>> agent = GeneticAgent()
            >>> agent.envSettings = {
            ...     'KEEPMODELHISTORY': True,
            ...     'NAGENTSPARALLEL': 4,
            ...     'MODELNAME': 'ga_v1'
            ... }
            >>> agent.hyParams = {'NAGENTS': 100, 'NGENERATIONS': 50}
            >>> agent.initializeGeneticAgents()
            # Initializes 100 agents using 4 parallel processes
        """
    ...
    def setSeeds(self):
        ...
        This method sets the seed for numpy, the Python random module,
        and TensorFlow to ensure consistent behavior in stochastic operations.
        ...
        Raises:
            KeyError: If 'SEEDAGENT' key is missing in `self.envSettings`.

        Examples:
            >>> agent = YourAgentClass()
            >>> agent.envSettings = {'SEEDAGENT': 42}
            >>> agent.setSeeds()
        """
    ...
    def runAgentsGeneticSingleRun(self, agentCount):
        ...
        for step in range(self.hyParams['NAGENTSTEPS']):
            if env.actionType == 'discrete':
                ...
                # print('actionIdx', actionIdx, len(self.actionSpace))
                action = self.actionSpace[actionIdx]
...
# class FloPyEnv(EnvCompatibility):
class FloPyEnv(gym.Env):
        ...
        if self.initWithSolution:
            ...
            if self.actionType == 'discrete':
                ...
                if self.ENVTYPE == '3s-c':
                    ...
                    self.distanceMax = 98.
                    self.rewardMax = 1000.
                self.action_space = gym.spaces.Box(0.0, 1.0, shape=(self.actionSpaceSize,), dtype=np.float32)
    ...
    def seed(self, seed=None):
        numpySeed(seed)
        # Add other RNG seeding as needed
    ...
    # Example feature extractor: flatten and normalize an observation
    ...
    def create_single_hash(self, observations):
        """
        Create a single hash from a list of observations.

        Parameters:
        - observations: list of raw observations (e.g., numpy arrays)
        - feature_extractor: function to convert observations into feature vectors

        Returns:
        - single_hash: a single hash string representing the list of observations
        """
        import numpy as np
        import hashlib
        ...
        # Flatten all features into a 1D array
        flat_features = features.flatten()
        # Convert to bytes for hashing
        feature_bytes = flat_features.tobytes()
        # Compute SHA-256 hash of the bytes
        single_hash = hashlib.sha256(feature_bytes).hexdigest()
    ...
    def step(self, action, teardownOnFinish=True):
        ...
        # QUICKFIX as this metric is not included, yet
        # !!!
        ...
        if self.ENVTYPE in ['0s-c']:
            ...
            if self.done:
                ...
                print('DEBUG ENVHASH', self.envhash)
            ...
            return self.observationsVectorNormalized, reward, self.done, self.truncated, self.info
            ...
            # import numpy as np
            # print('SHAPE', np.shape(list(self.observations)))
            # print('DEBUG', self.observations[0, 0, 0])
            # observations = self.observationsVectorToDict(list(self.observations))
            observations = self.observationsVectorToDict(self.observations)
    ...
    def defineEnvironment(self, seed=None):
        ...
        if self.actionType == 'continuous':
            if self.ENVTYPE in ['1s-c', '1r-c', '2s-c', '2r-c']:
        ...
        if hasattr(self, 'actionSpace'):
            ...
            self.actionSpaceSize = len(self.actionSpaceIndividual)
        # print(self.actionSpaceSize, self.actionSpace)
    ...
    def render(self, mode='human', dpi=120):
        ...
        if self.ENVTYPE in ['0s-c']:
            ...
            # self.fig.set_size_inches(7, 7)
            # self.fig.savefig('D:\\forGitHub_' + str(self.timeStep) + '.png')
    ...
    def play(self, env=None, ENVTYPE='1s-d', seed=None, returnReward=False, verbose=False):
        ...
        for self.timeSteps in range(self.NAGENTSTEPS):
            if not self.done:
                ...
                t0step = time()
                observations, reward, self.done, truncated, _ = self.env.step(action)
