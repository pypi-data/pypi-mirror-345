...
class HeartbeatClient:
        ...
        while self.is_sending_heartbeat and self.is_running:
            try:
                ...
                if current_time_ms > (self.last_hb + self.heartbeat_interval):
                    logger.debug(f'send heartbeat message to {self.server_ip}:{self.port}')
                ...
                logger.exception(f"Heartbeat send error: {e}")
    ...
    def _receive_messages(self):
        while self.is_running:
            try:
                ...
                logger.exception(f"Receive message exception: {e}")
    ...
    def online(self):
        ...
        logger.info(f"UDP socket bound to {self.local_ip}:{self.local_port}")
        ...
        logger.info("Successfully went online")
    ...
    def get_online_status(self,aids):
        try:
            ...
            if response.status_code == 200:
                print("get_online_status ok",response.json())
                return response.json()["data"]
            else:
                print(f"get_on_line_status failed:{response.json()}")
                return []
        except Exception as e:
            print(f"get_on_line_status in exception: {e}")
            return []