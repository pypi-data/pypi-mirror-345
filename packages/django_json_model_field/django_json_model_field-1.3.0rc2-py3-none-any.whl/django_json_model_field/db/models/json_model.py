...
class JSONModelBase(type):
    ...
    def add_to_class(cls, name, value):
        if isinstance(value, DateField):
            cls._monkey_patch_date_field(value)

        ModelBase.add_to_class(cls, name, value)

    def _monkey_patch_date_field(cls, field: DateField):
        """
        Timezone offsets gets stripped off for mysql since datetime fields are stored as UTC in the database. However,
        since the underlying "column" is part of a JSON object, the timezone information is lost when the object is
        deserialized.
        """

        og_to_python = field.to_python

        def to_python(value):
            value = og_to_python(value)
            if value is None or not settings.USE_TZ or timezone.is_aware(value):
                return value
    ...
    def deconstruct(cls):
        return JSONModelState.from_model(cls)
...
class JSONModel(metaclass=JSONModelBase):
    ...
    def _init_fields(self, initial: dict) -> None:
        ...
        for field in self._meta.get_fields():
            value = field.to_python(initial[field.name] if field.name in initial else field.get_default())
    ...
    def __eq__(self, other: Any) -> bool:
        # Regular Django models compare by PK, which is not applicable for JSONModel
        # Compare type and field values instead
        # Use "is" instead of isinstance to avoid comparing with subclasses
        return (
            type(self) is type(other)
            and all(getattr(self, field.name) == getattr(other, field.name) for field in self._meta.get_fields())
        )

    def __repr__(self):
        fields = [field for field in self._meta.get_fields() if hasattr(self, field.name)]
        fields_repr = ", ".join(f"{field.name}={getattr(self, field.name)!r}" for field in fields)
