...
LingmoObject {
            ...
            Column {
                ...
                move: Transition {
                    NumberAnimation {
                        ...
                        duration: LingmoTheme.animationEnabled ? 333 : 0
                    }
                }
        ...
        property Component lingmo_style: Rectangle {
            ...
            border.color: {
                if (LingmoTheme.dark) {
                    switch (_super.type) {
                    case mcontrol.const_success:
                        return Qt.rgba(56 / 255, 61 / 255, 27 / 255, 1)
                    case mcontrol.const_warning:
                        return Qt.rgba(66 / 255, 53 / 255, 25 / 255, 1)
                    case mcontrol.const_info:
                        return Qt.rgba(38 / 255, 39 / 255, 39 / 255, 1)
                    case mcontrol.const_error:
                        return Qt.rgba(67 / 255, 39 / 255, 38 / 255, 1)
                    }
                    return Qt.rgba(1, 1, 1, 1)
                } else {
                    switch (_super.type) {
            ...
            Row {
                ...
                LingmoIcon {
                    ...
                    iconColor: {
                        if (LingmoTheme.dark) {
                            switch (_super.type) {
                            case mcontrol.const_success:
                                return Qt.rgba(108 / 255, 203 / 255,
                                               95 / 255, 1)
                            case mcontrol.const_warning:
                                return Qt.rgba(252 / 255, 225 / 255, 0 / 255, 1)
                            case mcontrol.const_info:
                                return LingmoTheme.primaryColor
                            case mcontrol.const_error:
                                return Qt.rgba(255 / 255, 153 / 255,
                                               164 / 255, 1)
                            }
                            return Qt.rgba(1, 1, 1, 1)
                        } else {
                ...
                LingmoIconButton {
                    ...
                    iconColor: LingmoTheme.dark ? Qt.rgba(
                                                      222 / 255, 222 / 255,
                                                      222 / 255,
                                                      1) : Qt.rgba(97 / 255, 97 / 255, 97 / 255, 1)
    ...
    function showSuccess(text, duration = 1000, moremsg) {
        return mcontrol.create(mcontrol.const_success, text, duration,
                               moremsg ? moremsg : "")
    }
    function showInfo(text, duration = 1000, moremsg) {
        return mcontrol.create(mcontrol.const_info, text, duration,
                               moremsg ? moremsg : "")
    }
    function showWarning(text, duration = 1000, moremsg) {
        return mcontrol.create(mcontrol.const_warning, text, duration,
                               moremsg ? moremsg : "")
    }
    function showError(text, duration = 1000, moremsg) {
        return mcontrol.create(mcontrol.const_error, text, duration,
                               moremsg ? moremsg : "")
    }
