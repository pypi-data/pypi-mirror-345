...
class IApprovalTrackerDoc(BaseModel):
    """
    
    """ # noqa: E501
    collection_id: StrictStr = Field(alias="collectionId")
    approval_id: StrictStr = Field(description="The approval ID", alias="approvalId")
    amount_tracker_id: StrictStr = Field(description="The amount tracker ID of the approval.", alias="amountTrackerId")
    ...
    tracker_type: StrictStr = Field(description="The type of tracker to check \"overall\", \"to\", \"from\", or \"initiatedBy\".", alias="trackerType")
    approved_address: StrictStr = Field(description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.", alias="approvedAddress")
    ...
    num_transfers: NumberType = Field(description="The number of transfers. Is an incrementing tally.", alias="numTransfers")
    amounts: List[IBalance] = Field(description="A tally of the amounts transferred for this approval.")
    __properties: ClassVar[List[str]] = ["collectionId", "approvalId", "amountTrackerId", "approvalLevel", "approverAddress", "trackerType", "approvedAddress", "_docId", "_id", "numTransfers", "amounts"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IApprovalTrackerDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of num_transfers
        if self.num_transfers:
            _dict['numTransfers'] = self.num_transfers.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IApprovalTrackerDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "trackerType": obj.get("trackerType"),
            "approvedAddress": obj.get("approvedAddress"),
            "_docId": obj.get("_docId"),
            "_id": obj.get("_id"),
            "numTransfers": NumberType.from_dict(obj["numTransfers"]) if obj.get("numTransfers") is not None else None,
            "amounts": [IBalance.from_dict(_item) for _item in obj["amounts"]] if obj.get("amounts") is not None else None
        })
        return _obj
