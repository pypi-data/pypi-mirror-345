...
from relationalai.early_access.metamodel import ir, builtins, types, factory as f
from relationalai.early_access.metamodel.types import Any, Symbol
...
class Compiler:
    ...
    def compile_head(self, ctx:CompilerContext, head:Atom, original_name:str,
        ...
        return f.output([(var.name, var) for var in sig_vars]) if name == 'output' \
    ...
    def _compile_aggregation(self, ctx, left:Aggregation, right:RelVariable) -> ir.Aggregate:
        ...
        group = lookup_args - vargs
        items.add(f.aggregate(builtins.builtin_relations_by_name[left._method], vargs[:-1], group.list, [vargs[-1], result_var]))
        return f.logical(list(items), [result_var, *group.list])
