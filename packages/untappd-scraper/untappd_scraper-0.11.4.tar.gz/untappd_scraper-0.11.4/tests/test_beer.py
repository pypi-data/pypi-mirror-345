...
from untappd_scraper.beer import Beer, extract_checkin_elements, extract_checkin_elements_bs4
from untappd_scraper.web import url_of
from ut_types.checkin import WebActivityBeer
...
def test_beer(beer: Beer) -> None:
    ...
    assert result
    assert result.name
    assert result.name == result.beer_name
    assert result.brewery
    assert result.beer_id
    assert result.brewery_slug
