...
class BaseJSONModelField(MigrationCompatFieldMixin, JSONField):
    """
    ...
    def get_db_prep_value(self, value, connection, prepared=False):
        ...
        if isinstance(value, JSONModel):
            data = self._get_db_prep_value_from_model(value, connection, prepared)
            return super().get_db_prep_value(data, connection, prepared)

        return super().get_db_prep_value(value, connection, prepared)

    def _get_db_prep_value_from_model(self, instance: JSONModel, connection, prepared=False) -> dict:
        """
        ...
        return {
            field.name: field.get_db_prep_value(field.value_from_object(instance), connection, prepared=prepared)
    ...
    def __copy__(self):
        # see implementation of Field.__copy__ for why this is needed for Field subclasses
        # the __class__ assignment doesn't work due to the MRO structure, including the ABC metaclass, so this
        # workaround uses a class definition that includes the same bases as the "Empty" placeholder.
        obj = _Empty()
...
class _Empty(MigrationCompatFieldMixin, JSONField):
