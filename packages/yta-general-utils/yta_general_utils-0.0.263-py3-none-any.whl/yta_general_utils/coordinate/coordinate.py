...
class Coordinate:
    ...
    """
    _normalized_position: tuple[float, float] = None
    """
    ...
    def normalized_position(self):
        ...
        [0, 1] range.
        """
        if self._normalized_position is None:
    ...
    def __init__(self, position: Union[tuple[int, int], list[int, int]], is_normalized: bool = False):
    ...
    @staticmethod
    def generate(n: int = 1, lower_limit: Union[tuple[int, int], list[int, int]] = (0, 0), upper_limit: Union[tuple[int, int], list[int, int]] = (1920, 1080)):
        """
        Generate 'n' coordinates with random values
        between the 'lower_limit' and the 'upper_limit' for
        ...
        """
        if not NumberValidator.is_number_between(n, 1, 100):
            raise Exception(f'The provided "amount" parameter "{str(n)}" is not a number between 1 and 100.')
