...
class MutationCrossoverHandler:
    ...
    def clamp_and_renormalize(self, probabilities: np.ndarray, *,inplace: bool = False) -> np.ndarray:
        """
        Efficiently clamp probabilities to [self._min_prob, self._max_prob] 
        and normalize them so their sum is 1.

        Uses in-place operations (via `np.clip(..., out=...)` and `arr /= total`) 
        to avoid unnecessary temporaries.

        Parameters
        ----------
        probabilities : np.ndarray
            1D array of non-negative unnormalized probabilities.
        inplace : bool, optional
            If True, mutate `probabilities` directly; otherwise, operate on a copy.

        Returns
        -------
        np.ndarray
            The clamped (and normalized) array of probabilities.
        """
        # Decide whether to work on the original array or a copy
        arr = probabilities if inplace else probabilities.copy()

        # Clamp in-place to [min_prob, max_prob]
        np.clip(arr, self._min_prob, self._max_prob, out=arr)

        # Compute total and normalize if above threshold
        total = arr.sum()
        if total > 1e-12:
            arr /= total
    ...
    def _combined_rate(self, generation:int, objectives:np.array, temperature:float) -> np.array:
        """
        Compute the per-individual mutation rate based on fitness objectives and temperature scaling.

        This method calculates a base rate from the fitness factor of each individual,
        scales it by an initial mutation rate, converts to integers, and enforces a minimum rate.

        Parameters
        ----------
        generation : int
            Index of the current generation (included for interface consistency).
        objectives : np.ndarray
            1D array of fitness objective values for each individual (shape (N,)).
        temperature : float
            Temperature factor controlling selection pressure and rate scaling.

        Returns
        -------
        np.ndarray
            1D integer array (shape (N,)) of mutation rates for each individual,
            with all values at least `self._min_mutation_rate`.
        """
                        ...
                        for ea, d in zip(effective_alpha, delta):
                            ...
                            if d < 0:
                                for crossover in meta.get("crossover_idx", []):
                                    self._crossover_probabilities[crossover] *= (1.0 + ea)
                                    self._crossover_success_counts[crossover] += 1
                            else:
                                for crossover in meta.get("crossover_idx", []):
                                    self._crossover_probabilities[crossover] *= (1.0 - ea)
                                    self._crossover_unsuccess_counts[crossover] += 1
    ...
    def penalization(self, container, process='hash_colition'):
        """
