...
class TableOfFiguresPlugin(BasePlugin[PluginConfig]):
    ...
    def on_nav(self, nav, config: MkDocsConfig, files: MkDocsFiles):
        def process_nav_item(item: MkDocsSection|MkDocsPage|MkDocsLink):
            # Recursively process childs of Section item
            ...
            # Only work on Page item that are Markdown
            elif isinstance(item, MkDocsPage) and hasattr(item, 'file') and item.file and item.file.src_path.endswith('.md'):
                # It's a Page
                self.logger.debug(f'Processing nav item (page): {item}')
                process_page(item)
            # Skip other types of items (mostly Link and non-markdown Page)
            else:
        ...
        def process_page(page: MkDocsPage):
            ...
            for _, match, figure_type in matches:
                ...
                new_figure = Figure(self.figures.size + 1, figure_type, match.group('caption'), page)
                self.figures.append(new_figure)
                self.logger.debug(f'New figure added to collection: {new_figure.page_uri}[{match.start()}:{match.end()}] - {new_figure.figure_type_name} "{new_figure.caption_description}"')
    ...
    def on_page_markdown(self, markdown: str, page: MkDocsPage, config: MkDocsConfig, files: MkDocsFiles) -> str | None:
        ...
        # Retrieve all figures matches in the page content and order them by occurrence
        matches = []
        for figure_type in self.figure_types:
            for match in re.finditer(figure_type.pattern, markdown):
                if not any(existing_match.start() < match.end() and match.start() < existing_match.end() for existing_match in matches):
                    matches.append(match)
        matches.sort(key=lambda x: x.start())

        checkpoint_markdown = markdown
        try:
            # Adding figure to page
            for index, figure in enumerate(self.figures.page_figures(page)):
                # Add match to figure object
                figure.match = matches[index]

                # Check that figure content in markdown content haven't been tempered with
                if figure.match_content != markdown[figure.match_start + offset:figure.match_end + offset]:
                ...
                [self.logger.debug(f'| - {line}') for line in figure.match_content.splitlines()]
                ...
                offset += len(figure.replacement) - len(figure.match_content)
        ...
        for directive in self.directives.filter_by_page(page):
            figures_relpath = [os.path.relpath(figure.uri, start=os.path.dirname(directive.context.page_uri)) for figure in self.figures]
            replacement = self.builder.build({'figures': self.figures, 'figures_relpath': figures_relpath, 'directive': directive, 'translations': self.translations, 'config': config})
