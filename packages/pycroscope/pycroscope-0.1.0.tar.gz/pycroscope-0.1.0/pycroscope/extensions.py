...
Extensions to the type system supported by pycroscope. These can be imported at runtime and used in
user code.
...
class CustomCheck:
    ...
    return value is equivalent to that of :meth:`pycroscope.value.Value.can_assign`.

    A simple example is :class:`LiteralOnly`, which is also exposed by pycroscope
    itself::
    ...
    - ``walk_values()`` should yield all ``TypeVar`` objects contained in the check,
      wrapped in a :class:`pycroscope.value.TypeVarValue`.
    - ``substitute_typevars()`` takes a map from ``TypeVar`` to
      :class:`pycroscope.value.Value` objects and returns a new ``CustomCheck``.
...
class NoAny(CustomCheck):
    ...
    allowed_sources: Container["AnySource"] = field(
        default_factory=lambda: frozenset({pycroscope.value.AnySource.unreachable})
    )
    ...
    def can_assign(self, value: "Value", ctx: "CanAssignContext") -> "CanAssign":
        if self.deep:
            ...
            vals = pycroscope.value.flatten_values(value)
        for subval in vals:
            if self._is_disallowed(subval):
                return pycroscope.value.CanAssignError(f"Value may not be {subval}")
        return {}

    def _is_disallowed(self, value: "Value") -> bool:
        return (
            isinstance(value, pycroscope.value.AnyValue)
...
def reveal_type(value: _T) -> _T:
    ...
    Calling this function will make pycroscope print out the argument's
...
def patch_typing_overload() -> None:
    ...
    and have it be recognized by pycroscope.
...
class EnumName:
    ...
    def __class_getitem__(self, enum_cls: type[enum.Enum]) -> Any:
        return Annotated[str, pycroscope.annotated_types.EnumName(enum_cls)]
