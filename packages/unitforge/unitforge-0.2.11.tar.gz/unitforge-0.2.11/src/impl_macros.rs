...
macro_rules! impl_quantity {
    ($type:ident, $unit:ty, $display_unit:expr) => {
        #[cfg(feature = "pyo3")]
        use pyo3::{
            basic::CompareOp, pymethods, types::PyType, Bound, IntoPy, Py, PyAny, PyRef, PyResult, exceptions::PyValueError, PyObject
        };
        #[cfg(feature = "pyo3")]
        use $crate::Quantity;
        ...
        impl $type {
            fn __mul__(lhs: PyRef<Self>, rhs: Py<PyAny>) -> PyResult<PyObject> {
                ...
                let rhs_quantity = Quantity::from_py_any(rhs_ref).map_err(|e| PyValueError::new_err(e.to_string()))?;
                let lhs_quantity = Quantity::from_py_any(lhs_ref).map_err(|e| PyValueError::new_err(e.to_string()))?;
                match lhs_quantity.multiply(rhs_quantity) {
                    Some(value) => Ok(value.to_pyobject(py)),
                    None => Err(PyValueError::new_err("Multiplication of given objects is not possible."))
                }
            }
            ...
            fn __truediv__(lhs: PyRef<Self>, rhs: Py<PyAny>) -> PyResult<PyObject> {
                ...
                let rhs_quantity = Quantity::from_py_any(rhs_ref).map_err(|e| PyValueError::new_err(e.to_string()))?;
                let lhs_quantity = Quantity::from_py_any(lhs_ref).map_err(|e| PyValueError::new_err(e.to_string()))?;
                match lhs_quantity.divide(rhs_quantity) {
                    Some(value) => Ok(value.to_pyobject(py)),
                    None => Err(PyValueError::new_err("Division of given objects is not possible."))
                }
            }
        ...
        impl fmt::Display for $type {
            fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
                let base_unit = $display_unit;
                let unit_name = if base_unit.name() == "°" {
                    "°".to_string()
                } else {
                    format!(" {}", base_unit.name())
                };
                let value = self.to(base_unit);
                let rounded = if value.is_zero() {
                    value
                } else {
                    let didgits = value.log10().ceil() as i32;
                    let rounding_multiplier = 10_f64.powi(3 - didgits);
                    (value * rounding_multiplier).round() as f64 / rounding_multiplier as f64
                };

                write!(f, "{}{}", rounded, unit_name)
            }
        }
        ...
        impl PartialEq<Self> for $type {
            fn eq(&self, other: &Self) -> bool {
                self.power == other.power && self.multiplier == other.multiplier
            }
        }
