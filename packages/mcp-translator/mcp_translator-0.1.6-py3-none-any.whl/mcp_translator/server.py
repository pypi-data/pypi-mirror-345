...
mcp = FastMCP(
    ...
    dependencies=["sqlite3", "pydantic", "google-generativeai", "mammoth"] # Add google-generativeai and mammoth
)
    ...
    try:
        if file_extension == ".pdf":
            ...
            try:
             ...
             logger.info(f"Processing {file_extension} file with mammoth: {absolute_file_path}")
             try:
                 with open(absolute_file_path, "rb") as docx_file:
                     result = mammoth.convert_to_markdown(docx_file)
                     text = result.value # The generated markdown
                 logger.info(f"Successfully converted {file_extension} to Markdown (Length: {len(text)})")
             except Exception as doc_e:
                 logger.error(f"Error converting {file_extension} file {absolute_file_path}: {doc_e}")
                 return {"error": f"Could not process {file_extension} file: {doc_e}"}

        else: # Assuming plain text otherwise
...
def main(chunk_size: int, model: str) -> int: # Renamed model_name to model
    """Starts the Translator Server."""
    global CHUNK_SIZE, GEMINI_MODEL_NAME # Allow modification of globals
    CHUNK_SIZE = chunk_size
    GEMINI_MODEL_NAME = model # Use the passed argument
