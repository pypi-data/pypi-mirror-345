...
def H2AModel(
    cap_factor,
    avg_daily_H2_production,
    hydrogen_annual_output,
    h2a_for_hopp=True,
    force_system_size=True,
    ...
    if forced_system_size:
        ...
        stack_input_power = (
            stack_electrical_usage / total_system_electrical_usage
        ) * forced_system_size  # MW
    else:
        total_system_input = (
    ...
    gdpdef = {
        ...
        "CEPCI": [104.031, 104.865, 107.010, 109.237, 111.424, 113.415],
    }  # GDPDEF (2012=100), https://fred.stlouisfed.org/series/GDPDEF/
    CEPCI = pd.DataFrame(data=gdpdef)  # Deflator Table

    pci = {
        "Year": [2015, 2016, 2017, 2018, 2019, 2020],
        "PCI": [556.8, 541.7, 567.5, 603.1, 607.5, 610],
    ...
    CEPCI_inflator = int(
        (CEPCI.loc[CEPCI["Year"] == current_year_for_capital_cost, "CEPCI"]).iloc[0]
    ...
    baseline_uninstalled_stack_capital_cost = (
        CEPCI_inflator
        * consumer_price_inflator
        * system_unit_cost
        * total_active_area_degraded
        * 100**2
    )  # ($2016)
    stack_installation_factor = 1.12
    baseline_installed_stack_capital_cost = (
        stack_installation_factor * baseline_uninstalled_stack_capital_cost
    )

    baseline_uninstalled_mechanical_BoP_cost = (
        CEPCI_inflator
        * consumer_price_inflator
        * mechanical_BoP_unit_cost
        * peak_daily_production_rate
    )  # ($2016)
    mechanical_BoP_installation_factor = 1
    baseline_installed_mechanical_BoP_cost = (
        mechanical_BoP_installation_factor * baseline_uninstalled_mechanical_BoP_cost
    )
    ...
    feedstock_cost_in_startup_year = (
        plant_annual_output * feedstock_usage * feedstock_price_in_startup_year
    )

    total_energy_feedstock_cost = (
        feedstock_cost_in_startup_year  # Add all feedstock costs in startup year
    )
    ...
    utilities_cost_in_startup_year = (
        plant_annual_output * electricity_usage * utilities_price_in_startup_year
    )

    total_energy_utilities_cost = (
        utilities_cost_in_startup_year  # Add all utilities costs in startup year
    )
    ...
    byproduct_income_in_startup_year = (
        plant_annual_output * byproduct_production * byproduct_price_in_startup_year
    )

    total_energy_byproduct_credit = (
    ...
    total_fixed_operating_costs = int(
        labor_cost
        + GA_cost
        + licensing_permits_fees
        + propertytax_insurancecost
        + rent
        + material_costs_for_maintenance
        + production_maintenance_and_repairs
        + other_fees
    ...
    feedcost_in_startup_year = (
        plant_annual_output * processed_water_cost * water_usage_per_kgH2
    )  # lookup

    total_nonE_utility_material_costs = (
        feedcost_in_startup_year  # Add all feed or utlity start-up year costs
    )
    ...
    total_unplanned_replacement_capital_cost_factor = (
        0.5  # percent of total direct depreciable cost (costs/year)
    )
    ...
    (
        ...
        - total_byproduct_credit
        + other_variable_operating_costs
        + other_material_costs
        + waste_treatment_costs
        + solid_waste_disposal_costs
        + total_unplanned_replacement_capital_cost_factor
    ...
    total_depreciable_costs_inflated = (
        total_capital_costs - total_nondepreciable_costs
    ) * inflation_factor
    ...
    Initial_Data = {
        "Year": np.arange(
            assumed_startup_year - length_of_construction_period,
            assumed_startup_year + plant_life,
        ),
        "Analysis Year": np.arange(plant_life + 1),
    }
    df = pd.DataFrame(data=Initial_Data)

    df["Inflation Price Increase Factor"] = (1 + inflation_rate) ** (
        df["Year"] - assumed_startup_year
    )

    df["Revenue from Hydrogen Sales"] = 0
    df["Revenue from Hydrogen Sales"][df["Analysis Year"] == 0] = 0
    df["Revenue from Hydrogen Sales"][df["Analysis Year"] == 1] = (
        H2_price_nominal
    ...
    df["Revenue from Hydrogen Sales"][df["Analysis Year"] > 1] = (
        H2_price_nominal * plant_annual_output * df["Inflation Price Increase Factor"]
    )

    df["Debt Financed Initial Depreciable Capital"] = 0
    df["Debt Financed Initial Depreciable Capital"][df["Analysis Year"] == 0] = (
        -df["Inflation Price Increase Factor"]
        * debt_financing
        * total_depreciable_costs
        * inflation_factor
    )
    df["Debt Financed Initial Depreciable Capital"][df["Analysis Year"] >= 1] = 0

    df["Initial Equity Depreciable Capital"] = 0
    df["Initial Equity Depreciable Capital"][df["Analysis Year"] < 1] = (
        -df["Inflation Price Increase Factor"]
        * total_depreciable_costs_inflated
        * percent_equity_financing
        * percent_capital_spent_year_1
    )

    df["Yearly Replacement Cost"] = 0
    df["Yearly Replacement Cost"][df["Analysis Year"] >= 1] = (
        -unplanned_replacement_cost_factor
        * total_depreciable_costs
        * df["Inflation Price Increase Factor"]
        * ((1 + inflation_rate) ** (assumed_startup_year - reference_year))
    )
    df["Yearly Replacement Cost"][
        ...
        (
            unplanned_replacement_cost_factor * total_depreciable_costs
            + 0.15 * H2A_total_direct_capital_cost
        )
        * (1 + inflation_rate) ** (assumed_startup_year - reference_year)
    )  # /(CPInfactor*CEPCIinflator)
    ...
    df["Salvage Value"] = 0
    df["Salvage Value"][df["Analysis Year"] == analysis_period] = (
        df["Inflation Price Increase Factor"] * salvage_value
    )

    df["Decommissioning Costs"] = 0
    df["Decommissioning Costs"][df["Analysis Year"] == analysis_period] = (
        -df["Inflation Price Increase Factor"] * decom
    )

    df["Fixed Operating Cost"] = 0
    df["Fixed Operating Cost"][df["Analysis Year"] >= 1] = (
        -inflated_fixed * df["Inflation Price Increase Factor"]
    )
    ...
    df["Fixed Operating Cost"][df["Analysis Year"] == startup_time] = -(
        inflated_fixed
        * percent_fixed_operating_cost_during_startup
        * df["Inflation Price Increase Factor"]
    )

    df["Feedstock Cost"] = 0
    df["Feedstock Cost"][(df["Analysis Year"] == 1) & (startup_time < 1)] = -(
        total_energy_feedstock_unitcost
        * plant_annual_output
        * df["Inflation Price Increase Factor"]
        * startup_time
    ...
    df["Feedstock Cost"][df["Analysis Year"] <= startup_time] = (
        -total_energy_feedstock_unitcost
        * plant_annual_output
        * df["Inflation Price Increase Factor"]
        * percent_variable_operating_cost_during_startup
    )
    df["Feedstock Cost"][df["Analysis Year"] >= 1] = (
        -total_energy_feedstock_unitcost
        * plant_annual_output
        * df["Inflation Price Increase Factor"]
    )
    ...
    df["Other Raw Material Cost"][(df["Analysis Year"] == 1) & (startup_time < 1)] = -(
    ...
    df["Other Raw Material Cost"][df["Analysis Year"] >= 1] = (
        -inflated_other_rawmaterial * df["Inflation Price Increase Factor"]
    )
    ...
    df["Revenue from Byproduct Sales"][df["Analysis Year"] <= startup_time] = (
        total_byproduct_unitprice
        * plant_annual_output
        * df["Inflation Price Increase Factor"]
        * percent_variable_operating_cost_during_startup
    ...
    df["Other Variable Operating Costs"][df["Analysis Year"] > 1] = -df[
    ...
    for i in range(1, len(df) - 1):
        df.loc[i, "Working Capital Reserve"] = working_capital * (
            (
                ...
                + df.loc[i, "Feedstock Cost"]
                + df.loc[i, "Other Raw Material Cost"]
                + df.loc[i, "Revenue from Byproduct Sales"]
                + df.loc[i, "Other Variable Operating Costs"]
            )
            - (
                df.loc[i - 1, "Fixed Operating Cost"]
                + df.loc[i - 1, "Feedstock Cost"]
                + df.loc[i - 1, "Other Raw Material Cost"]
                + df.loc[i - 1, "Revenue from Byproduct Sales"]
                + df.loc[i - 1, "Other Variable Operating Costs"]
            )
        )
    df["Working Capital Reserve"][df["Analysis Year"] == analysis_period] = -df.loc[
        0:analysis_period, "Working Capital Reserve"
    ].sum()

    df["Debt Interest"] = 0
    df["Debt Interest"] = (
        df.loc[0, "Debt Financed Initial Depreciable Capital"] * interest_rate_on_debt
    )

    df["Pre-Depreciation Income"] = 0
    df["Pre-Depreciation Income"] = (
        df["Revenue from Hydrogen Sales"]
        + df["Salvage Value"]
        + df["Decommissioning Costs"]
        + df["Fixed Operating Cost"]
        + df["Feedstock Cost"]
        + df["Other Raw Material Cost"]
    ...
    Depreciation_Table = pd.DataFrame(
        data=Depr_Initial_Data,
        columns=[
            "Annual Depreciable Capital",
            "Analysis Year",
            "1",
            "2",
            "3",
    ...
    Depreciation_Table["Annual Depreciable Capital"][
        ...
        & (Depreciation_Table["Analysis Year"] < plant_life + 1)
    ] = -df["Yearly Replacement Cost"]
    Depreciation_Table["Annual Depreciable Capital"][
        Depreciation_Table["Analysis Year"] > plant_life
    ] = 0

    Depreciation_Table["1"] = 0
    Depreciation_Table["1"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[0]
    )
    Depreciation_Table["2"] = 0
    Depreciation_Table["2"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[1]
    )
    Depreciation_Table["3"] = 0
    Depreciation_Table["3"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[2]
    )
    Depreciation_Table["4"] = 0
    Depreciation_Table["4"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[3]
    )
    Depreciation_Table["5"] = 0
    Depreciation_Table["5"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[4]
    )
    Depreciation_Table["6"] = 0
    Depreciation_Table["6"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[5]
    )
    Depreciation_Table["7"] = 0
    Depreciation_Table["7"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[6]
    )
    Depreciation_Table["8"] = 0
    Depreciation_Table["8"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[7]
    )
    Depreciation_Table["9"] = 0
    Depreciation_Table["9"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[8]
    )
    Depreciation_Table["10"] = 0
    Depreciation_Table["10"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[9]
    )
    Depreciation_Table["11"] = 0
    Depreciation_Table["11"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[10]
    )
    Depreciation_Table["12"] = 0
    Depreciation_Table["12"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[11]
    )
    Depreciation_Table["13"] = 0
    Depreciation_Table["13"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[12]
    )
    Depreciation_Table["14"] = 0
    Depreciation_Table["14"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[13]
    )
    Depreciation_Table["15"] = 0
    Depreciation_Table["15"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[14]
    )
    Depreciation_Table["16"] = 0
    Depreciation_Table["16"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[15]
    )
    Depreciation_Table["17"] = 0
    Depreciation_Table["17"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[16]
    )
    Depreciation_Table["18"] = 0
    Depreciation_Table["18"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[17]
    )
    Depreciation_Table["19"] = 0
    Depreciation_Table["19"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[18]
    )
    Depreciation_Table["20"] = 0
    Depreciation_Table["20"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[19]
    )
    Depreciation_Table["21"] = 0
    Depreciation_Table["21"][Depreciation_Table["Analysis Year"] > 0] = (
        Depreciation_Table["Annual Depreciable Capital"] * TWENTY_YEAR[20]
    )
    ...
    df["Depreciation Charge"][df["Analysis Year"] < plant_life] = -Depreciation_Table[
        "Depreciation Charge"
    ]
    ...
    df["After-Tax Post-Depreciation Cash Flow"] = (
        df["Initial Equity Depreciable Capital"]
        + df["Yearly Replacement Cost"]
        + df["Working Capital Reserve"]
        + df["Other Non-Depreciable Capital Cost"]
        + df["Pre-Depreciation Income"]
        + df["Total Taxes"]
    ...
    df["H2 Sales (kg/year)"][(df["Analysis Year"] == 1) & (startup_time < 1)] = (
        plant_annual_output * startup_time * percent_revenue_during_startup
        + plant_annual_output * (1 - startup_time)
    )
    df["H2 Sales (kg/year)"][df["Analysis Year"] == startup_time] = (
        plant_annual_output * percent_revenue_during_startup
    )
    ...
    df.loc["NPV of Cashflow", "Revenue from Hydrogen Sales"] = npv(
        nominal_IRR, (df["Revenue from Hydrogen Sales"].tolist())[: useful_life + 1]
    )
    ...
    df.loc["NPV of Cashflow", "Debt Financed Initial Depreciable Capital"] = npv(
        nominal_IRR,
        (df["Debt Financed Initial Depreciable Capital"].tolist())[: useful_life + 1],
    )
    df.loc["Discounted Values", "Debt Financed Initial Depreciable Capital"] = (
        df["Debt Financed Initial Depreciable Capital"].tolist()
    )[0] + npv(
        nominal_IRR,
        (df["Debt Financed Initial Depreciable Capital"].tolist())[1 : useful_life + 1],
    )

    df.loc["NPV of Cashflow", "Initial Equity Depreciable Capital"] = npv(
        nominal_IRR,
        (df["Initial Equity Depreciable Capital"].tolist())[: useful_life + 1],
    )
    df.loc["Discounted Values", "Initial Equity Depreciable Capital"] = (
        df["Initial Equity Depreciable Capital"].tolist()
    )[0] + npv(
        nominal_IRR,
        (df["Debt Financed Initial Depreciable Capital"].tolist())[1 : useful_life + 1],
    )

    df.loc["NPV of Cashflow", "Yearly Replacement Cost"] = npv(
        nominal_IRR, (df["Yearly Replacement Cost"].tolist())[: useful_life + 1]
    )
    ...
    df.loc["NPV of Cashflow", "Other Non-Depreciable Capital Cost"] = npv(
        nominal_IRR,
        (df["Other Non-Depreciable Capital Cost"].tolist())[: useful_life + 1],
    )
    df.loc["Discounted Values", "Other Non-Depreciable Capital Cost"] = (
        df["Other Non-Depreciable Capital Cost"].tolist()
    )[0] + npv(
        nominal_IRR,
        (df["Other Non-Depreciable Capital Cost"].tolist())[1 : useful_life + 1],
    )
    ...
    df.loc["Discounted Values", "Other Variable Operating Costs"] = (
        df["Other Variable Operating Costs"].tolist()
    )[0] + npv(
        nominal_IRR,
        (df["Other Variable Operating Costs"].tolist())[1 : useful_life + 1],
    )
    ...
    df.loc["Discounted Values", "Total Taxes"] = (df["Total Taxes"].tolist())[0] + npv(
        nominal_IRR, (df["Total Taxes"].tolist())[1 : useful_life + 1]
    )
    ...
    df.loc["NPV of Cashflow", "After-Tax Post-Depreciation Cash Flow"] = npv(
        nominal_IRR,
        (df["After-Tax Post-Depreciation Cash Flow"].tolist())[: useful_life + 1],
    )
    df.loc["Discounted Values", "After-Tax Post-Depreciation Cash Flow"] = (
        df["After-Tax Post-Depreciation Cash Flow"].tolist()
    )[0] + npv(
        nominal_IRR,
    ...
    final_data.loc["Discounted Values", "Depreciation"] = -df.loc[
        "Discounted Values", "Depreciation Charge"
    ]
    final_data.loc["Discounted Values", "Principal"] = -df.loc[
        "Discounted Values", "Principal Payment"
    ]
    final_data.loc["Discounted Values", "Operation Costs"] = -(
        df.loc["Discounted Values", "Salvage Value"]
        + df.loc["Discounted Values", "Decommissioning Costs"]
        + df.loc["Discounted Values", "Fixed Operating Cost"]
        + df.loc["Discounted Values", "Feedstock Cost"]
        + df.loc["Discounted Values", "Other Raw Material Cost"]
        + df.loc["Discounted Values", "Revenue from Byproduct Sales"]
        + df.loc["Discounted Values", "Other Variable Operating Costs"]
        + df.loc["Discounted Values", "Debt Interest"]
    )
    final_data.loc["Discounted Values", "Tax Incentives"] = tax_credit
    final_data.loc["Discounted Values", "H2 Sales (kg)"] = df.loc[
        "Discounted Values", "H2 Sales (kg/year)"
    ]

    final_data.loc["Tax Coefficient", "Capital Costs"] = 1
    final_data.loc["LCOH Cost Contribution", "Capital Costs"] = (
        final_data.loc["Discounted Values", "Capital Costs"]
        * final_data.loc["Tax Coefficient", "Capital Costs"]
    )

    final_data.loc["Tax Coefficient", "Depreciation"] = -total_tax_rate
    final_data.loc["LCOH Cost Contribution", "Depreciation"] = (
        final_data.loc["Discounted Values", "Depreciation"]
        * final_data.loc["Tax Coefficient", "Depreciation"]
    )

    final_data.loc["Tax Coefficient", "Principal"] = 1
    final_data.loc["LCOH Cost Contribution", "Principal"] = (
        final_data.loc["Discounted Values", "Principal"]
        * final_data.loc["Tax Coefficient", "Principal"]
    )

    final_data.loc["Tax Coefficient", "Operation Costs"] = 1 - total_tax_rate
    final_data.loc["LCOH Cost Contribution", "Operation Costs"] = (
        final_data.loc["Discounted Values", "Operation Costs"]
        * final_data.loc["Tax Coefficient", "Operation Costs"]
    )

    final_data.loc["Tax Coefficient", "Tax Incentives"] = -1
    final_data.loc["LCOH Cost Contribution", "Tax Incentives"] = (
        final_data.loc["Discounted Values", "Tax Incentives"]
        * final_data.loc["Tax Coefficient", "Tax Incentives"]
    )

    final_data.loc["Tax Coefficient", "H2 Sales (kg)"] = 1 - total_tax_rate
    final_data.loc["LCOH Cost Contribution", "H2 Sales (kg)"] = (
        final_data.loc["Discounted Values", "H2 Sales (kg)"]
    ...
    Final_Hydrogen_Cost_Real = (
        ...
        * (1 + inflation_rate) ** length_of_construction_period
        / inflation_factor
    )
    ...
    Cost_Breakdown.loc["Capital Related Costs", "After Tax Present Value"] = -(
        (
            (
                df.loc["NPV of Cashflow", "Initial Equity Depreciable Capital"]
                + df.loc["NPV of Cashflow", "Yearly Replacement Cost"]
                + df.loc["NPV of Cashflow", "Working Capital Reserve"]
                + df.loc["NPV of Cashflow", "Other Non-Depreciable Capital Cost"]
            )
            + df.loc["NPV of Cashflow", "Salvage Value"] * (1 - total_tax_rate)
            + df.loc["NPV of Cashflow", "Debt Interest"] * (1 - total_tax_rate)
            + df.loc["NPV of Cashflow", "Depreciation Charge"] * (-total_tax_rate)
            + df.loc["NPV of Cashflow", "Principal Payment"]
        )
        / (1 - total_tax_rate)
    )
    ...
    Cost_Breakdown.loc["Fixed O&M", "After Tax Present Value"] = -df.loc[
        "NPV of Cashflow", "Fixed Operating Cost"
    ]
    Cost_Breakdown.loc["Feedstock Costs", "After Tax Present Value"] = df.loc[
        "NPV of Cashflow", "Feedstock Cost"
    ]
    ...
    results["scaled_total_installed_cost_kw"] = scaled_total_installed_cost / (
        total_system_input * 1000
    )
    results["expenses_annual_cashflow"] = (
        df["After-Tax Post-Depreciation Cash Flow"] - df["Pre-Depreciation Income"]
    )
