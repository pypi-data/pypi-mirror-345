...
from GooeyEx.gui.components.filtering.prefix_filter import OperatorType
...
def lift(f):
    ...
    def inner(value):
        result = f(value)
        return (
            SuperBool(result, f.__doc__)
            if not isinstance(result, SuperBool)
            else result
        )
