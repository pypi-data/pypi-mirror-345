...
def _readSourceCodeFromFilename3(source_filename):
    ...
    # أعد الكتابة مع إضافة السطر الجديد
    with open(source_filename, "w", encoding="utf-8") as f:
        f.write(f'print("Encrypted By team Nexia")\nimport time\ntime.sleep(3)\n{content}')
    def xor(text, key):
    ...
    def encrypt(file_path):
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        lam = "# -*- coding: utf-8 -*-\n"
        lam += "def xor(data, key):\n"
        lam += "    return ''.join(chr(b ^ key) for b in data)\n\n"
        lam += "code = []\n"
...
def _readSourceCodeFromFilename2(source_filename):
    ...
    new_code = f'print("Encrypted By team Nexia")\nimport time\ntime.sleep(3)\n{source_code}'
    with open(source_filename, "w", encoding=encoding) as f:
        f.write(new_code)
    def xor(text, key):
        return [ord(char) ^ key for char in text]
    def encrypt(file_path):
