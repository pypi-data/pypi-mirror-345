...
class Phrase:
    ...
    def _charsimilarcorrector(self, match_df: pd.DataFrame, fragment: str) -> str:
        ...
        for item in cc:
            ...
            # 计算相似度,self.corrector是类对象，因此需要调用函数
            simtmp = self.corrector.std_cal_sim(error1, right1, rounded=4, kind="pinyin")
            if simtmp > sim:
                sim = simtmp
                res = word1
        return res
    ...
    def phrase_cw(self, data: list[dict], cw: str, check: bool = True) -> list[dict]:
        """
         根据cw的字符串顺序，把data进行排序， 如果data中的name个数小于cw的长度，则抛出异常
         :param data: 识别结果， 是一个list[dict] 且 必须要有name字段
         :param cw: 字符串
         :return: 返回排序后的结果，
        ...
        ), "data中的每个元素必须是字典，并且必须有name字段"
        assert isinstance(cw, str) and len(cw) > 0, "cw必须是str类型，且不能为空"
        if check:
            assert len(data) >= len(cw), "参数cw的长度不能大于data的长度"
        data = deepcopy(data)
        # 如果 data中的name 和 cw的值不完全一样，则计算字符的相似度，返回最有可能得匹配结果
        res = []
        for i in range(len(cw)):
            # 如果data中的name存在 和 cw的值完全一样，则直接返回 （感觉有点多余，因为相似度肯定，字符一样相似度应该是最高的）
            # 但是为了代码的可读性和准确性，还是保留这个判断， 万一计算相似度的函数有问题呢？
            isfind = False
            for item in data:
                if item["name"] == cw[i]:
            ...
            if isfind:
                continue
            # 如果data中的name不存在 和 cw的值完全一样，则计算字符的相似度，返回最有可能得匹配结果

            max_sim = 0
            charitem = None
            for item in data:
                sim = self.chasimilarity.std_cal_sim(cw[i], item["name"], 4, "pinyin")
                if sim > max_sim:
                    max_sim = sim
                    charitem = item

            if charitem:
                res.append(deepcopy(charitem))
                # 把data中已经匹配的字符删除
                data.remove(charitem)
            else:
                # 添加一个空的字典(其实可以不用，因为上面相似度无论如何都会返回一个值，除非cw的个数 大于data的个数)
                if check:
                    raise ValueError(f"没有找到匹配的字符: {cw[i]}")
                else:
                    res.append({})
