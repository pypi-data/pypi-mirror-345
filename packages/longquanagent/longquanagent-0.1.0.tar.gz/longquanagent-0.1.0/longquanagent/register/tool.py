...
class ToolManager:
    ...
    TOOLS_PATH: Path = Path(os.path.join(absolute_lib_path.parent, 'tool_template', 'tools.json'))
    _registered_functions: Dict[str, Callable] = {}

    @staticmethod
    def load_tools() -> Dict[str, Any]:
        """Load existing tools from JSON"""
        file_path = ToolManager.TOOLS_PATH
...
class ToolRegister:
    @classmethod
    def register_function(cls, llm: Union[ChatTogether, BaseLanguageModel, BaseChatOpenAI], module_path: str) -> None:
        ...
        try:
            if os.path.isfile(module_path):
                ...
                absolute_lib_path = Path(os.path.dirname(os.path.abspath(__file__)))
                destination_path = Path(os.path.join(absolute_lib_path.parent, 'tools', module_path.name))
                shutil.copy2(module_path, destination_path)
                module_path=f"longquanagent.tools.{destination_path.name.split('.')[0]}"
    ...
    @classmethod
    def tool_calling(cls, llm: Union[ChatTogether, BaseLanguageModel, BaseChatOpenAI], task: str) -> Any:
        ...
        tool_data = cls.extract_json(response)
