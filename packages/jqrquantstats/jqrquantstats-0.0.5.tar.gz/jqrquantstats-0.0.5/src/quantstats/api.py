...
def build_data(returns: pd.DataFrame | pd.Series, rf=0.0, benchmark=None, nperiods=None) -> "Data":
    """
    ...
    Returns:
        Data: A Data object containing processed returns and benchmark data.
    ...
    if benchmark is None:
        return Data(returns=_calculate_excess_returns(returns_df, rf, nperiods=nperiods))
    ...
    # Process returns with benchmark
        ...
        return Data(returns=excess_returns, benchmark=excess_benchmark)
...
class Data:
    ...
    def resample(self, resample: str = "YE", compounded: bool = False) -> "Data":
        """
        ...
        Returns:
            Data: A new Data object containing the resampled returns.
        """
        ...
        return Data(returns=frame[self.names], benchmark=frame.get("Benchmark", None))
    ...
    def copy(self) -> "Data":
        ...
        """
        try:
            return Data(returns=self.returns.copy(), benchmark=self.benchmark.copy())
        except AttributeError:
            # Handle case where benchmark is None
            return Data(returns=self.returns.copy())
    ...
    def head(self, n: int = 5) -> "Data":
        ...
        return Data(returns=all_data[self.names], benchmark=all_data.get("Benchmark", None))
...
class Plots:
    ...
    # ):
    #     """
    #     Creates a time series line plot for financial data.
    #
    #     This function visualizes time series data as lines, making it easy to track
    #     performance over time. It's particularly useful for plotting returns, prices,
    #     or other financial metrics.
    #
    #     Args:
    #         df (pd.DataFrame): DataFrame containing time series data with dates as index.
    #         title (str, optional): Title of the plot. Defaults to "Returns".
    #         percent (bool, optional): Whether to format y-axis as percentages. Defaults to True.
    #         log_scale (bool, optional): Whether to use logarithmic scale for y-axis. Defaults to False.
    #         lw (float, optional): Line width for the plots. Defaults to 1.5.
    #         ylabel (str, optional): Label for y-axis. Defaults to "".
    #         fontname (str, optional): Font family to use. Defaults to "Arial".
    #         subtitle (bool, optional): Whether to include date range as subtitle. Defaults to True.
    #
    #     Returns:
    #         plotly.graph_objects.Figure: A Plotly figure object containing the time series plot.
    #
    #     Example:
    #         >>> import pandas as pd
    #         >>> df = pd.DataFrame(...)
    #         >>> fig = plot_timeseries(df, title="Portfolio Returns")
    #         >>> fig.show()
    #     """
    #     # Get color palette and styling parameters
    #     colors, _, alpha = Plots._get_colors()
    #
    #     # Create figure
    #     fig = go.Figure()
    #
    ...
    #                 mode="lines",
    #                 name=col,
    #                 line=dict(color=colors[(i + 1) % len(colors)], width=lw),  # Cycle through colors
    #                 opacity=alpha,
    #                 # Format hover text based on whether data is percentage or not
    #                 hovertemplate=(
    #                     f"<b>{col}</b><br>%{{y:.2%}}<extra></extra>"
    #                     if percent
    ...
    #         )
    #
    #     # Add horizontal line at y=0 as reference
    ...
    #     )
    #
    #     # Configure axes with softer gridlines
    ...
    def plot_returns_bars(self):
        ...
        Args:
            data (Data): A Data object containing returns data to plot.
        ...
        Example:
            >>> from quantstats.api import Data
            >>> import pandas as pd
            >>> returns = pd.DataFrame(...)
    ...
    def plot_snapshot(self, title="Portfolio Summary", compounded=True, log_scale=False):
        ...
        Args:
            data (Data): A Data object containing returns data.
    ...
    def plot_longest_drawdowns(
        ...
        """
        Creates a plot highlighting the longest drawdown periods.

        This function visualizes cumulative returns with the longest drawdown periods
        highlighted, making it easy to identify and analyze significant market downturns.

        Args:
            data (Data): A Data object containing returns data.
            periods (int, optional): Number of drawdown periods to highlight. Defaults to 5.
            lw (float, optional): Line width for the plots. Defaults to 1.5.
            fontname (str, optional): Font family to use. Defaults to "Arial".
            title (str, optional): Title of the plot. If None, defaults to "Returns".
            ylabel (bool, optional): Whether to show y-axis label. Defaults to True.
            subtitle (bool, optional): Whether to include date range as subtitle. Defaults to True.
            compounded (bool, optional): Whether to use compounded returns. Defaults to True.

        Returns:
            plotly.graph_objects.Figure: A Plotly figure object containing the drawdown plot.
        ...
        Example:
            ...
            >>> returns = pd.DataFrame(...)
            >>> data = Data(returns=returns)
            >>> fig = plot_longest_drawdowns(data, periods=3)
            >>> fig.show()
        """
        # Get color palette
        colors, _, _ = Plots._get_colors()
        ...
        # Extract returns for the first asset
        returns = self.data.returns[self.data.names[0]]

        # Calculate drawdowns
        dd = self.data.drawdown(compounded=compounded)[self.data.names[0]]

        # Get drawdown details and find longest periods
        ...
        if subtitle:
            subtitle_text = f"{returns.index[0].strftime('%d %b %Y')} - {returns.index[-1].strftime('%d %b %Y')}"
