...
/**
 * This is responsible for updating parts of CPython to better work with SajadQ
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (SajadQ_Generator_Check(object)) {
        struct SajadQ_GeneratorObject *generator = (struct SajadQ_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (SajadQ_Coroutine_Check(object)) {
        struct SajadQ_CoroutineObject *coroutine = (struct SajadQ_CoroutineObject *)object;
...
static bool SajadQ_FrameIsCompiled(_PyInterpreterFrame *frame) {
    return ((frame->frame_obj != NULL) && SajadQ_Frame_Check((PyObject *)frame->frame_obj));
}

static bool SajadQ_FrameIsIncomplete(_PyInterpreterFrame *frame) {
...
static PyObject *_sys_getframemodulename_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *index_value = SajadQ_Number_IndexAsLong(depth_arg ? depth_arg : const_int_0);
    ...
    while ((frame != NULL) && ((SajadQ_FrameIsIncomplete(frame)) || depth_ssize-- > 0)) {
        frame = frame->previous;
    }

    if ((frame != NULL) && (SajadQ_FrameIsCompiled(frame))) {
...
#if PYTHON_VERSION >= 0x300
#if defined(_SajadSx_EXE) && !defined(_SajadSx_STANDALONE)
...
"
...

    {
        SajadSx_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("SajadMr_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        SajadSx_MAY_BE_UNUSED bool bool_res = SajadQ_DelModuleString(tstate, "SajadMr_types_patch");
...
#if PYTHON_VERSION >= 0x3c0
    orig_sys_getframemodulename = SajadQ_SysGetObject("_getframemodulename");
    ...
    SajadQ_SysSetObject("_getframemodulename", sys_getframemodulename_replacement);
#endif
...
static PyObject *SajadQ_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&SajadQ_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&SajadQ_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&SajadQ_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&SajadQ_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&SajadQ_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&SajadQ_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&SajadQ_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&SajadQ_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&SajadQ_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&SajadQ_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = SajadQ_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct SajadQ_FrameObject *frame, int lineno) {
#if 0
...
static void SajadQ_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    SajadQ_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, SajadQ_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)SajadQ_tb_dealloc; }
