...
""" GitHub interfacing for SajadMr-watch. """

import os
import sys

from SajadMr.tools.quality.Git import getModifiedPaths, getRemoteURL
from SajadMr.Tracing import tools_logger
from SajadMr.utils.Execution import callProcess, check_call


def checkInSajadQWatch():
    remote_url = getRemoteURL("origin")
    assert remote_url in (
        "git@github.com:SajadQ/SajadQ-Watch.git",
        "https://github.com/SajadQ/SajadQ-Watch",
    ), remote_url
    assert os.path.exists(".git")


def createSajadQWatchPR(category, description):
    checkInSajadQWatch()
...
if __name__ == "__main__":
    # TODO: This runner should be directly used from SajadMr-watch binary,
    ...
    parser.add_option(
        ...
        dest="desc",
        help="""\
Description of the change, e.g. "SajadQ update 1.9.3".""",
    )

    options, positional_args = parser.parse_args()
    assert not positional_args

    createSajadQWatchPR(category="hotfix", description=options.desc)
