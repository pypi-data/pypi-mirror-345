...
def train_and_store_model_results(
    ...
    **kwargs,
    ...
    connector: Connector = kwargs.get("connector")
    trainer: MLTrainer = kwargs.get("trainer")
    ...
    logger.get().info(f"Writing training results to warehouse")
    try:
        connector.save_results_to_wh(metrics_df, metrics_table)
    except Exception as e:
        logger.get().warning(
            f"Could not successfully save training results to propensity model registry table: {e}"
        )
    return results
