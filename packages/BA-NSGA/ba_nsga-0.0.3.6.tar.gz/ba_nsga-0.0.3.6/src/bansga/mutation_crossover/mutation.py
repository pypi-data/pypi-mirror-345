...
def random_rattle(structure, species:list=[], std:float=1, constraints:list=[], seed:int=42, verbose:bool=False, components:list=[0,1,2]):
    """
    Randomly rattle atom positions for the specified species and components.
    
    Parameters
    ----------
    structure : object
        The structure containing atom positions and labels. It is assumed that
        the structure has an attribute `AtomPositionManager` with `atomLabelsList`
        and a method `rattle`.
    species : list, optional
        A single species (as a string) or a list of species. If empty, all atoms are used.
    std : float, optional
        Standard deviation of the random displacement.
    constraints : list, optional
        A list of constraints used by the `validate` function.
    seed : int, optional
        Random seed for reproducible results.
    verbose : bool, optional
        If True, detailed output can be provided.
    components : list, optional
        A list specifying the components to rattle. Each element may be either an integer
        (0 for x, 1 for y, 2 for z) or a string ('x', 'y', 'z').
    
    Returns
    -------
    structure : object or None
        The modified structure if any atom indices pass the validation, otherwise None.
    atom_indices : np.ndarray or None
        The array of filtered atom indices on which rattle was applied, or None.
    """
...
def change_atom_type(structure, ID_initial, ID_final, N:int=1, constraints:list=[], verbose:bool=False, ):
    ...
    if isinstance(ID_initial, str):     
        ...
        atom_indices = np.where(
            np.isin(np.array(structure.AtomPositionManager.atomLabelsList), np.array(ID_initial) )
        )[0]
    else:
        raise ValueError("ID_initial must be either a string or a list of strings.")
...
def remove_atom_groups(structure, species, N=1, constraints: list=[]):
    ...
    if isinstance(species, str):
        ...
        partition.containers = partition.generate_configurational_space(values=values, verbose=verbose)
...
def mutation_add(species: list, bound:list =None, collision_tolerance:float=2.0, constraints: list=[]):
    def func(structure):
        structure = add_species(structure=structure, species=species, bound=bound, collision_tolerance=collision_tolerance, constraints=constraints)
        return structure
