...
""" Command line options of SajadQ.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "SajadMr.Options" values is not in
...
from SajadMr.PythonFlavors import getPythonFlavorName
from SajadMr.PythonVersions import isPythonWithGil
from SajadMr.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from SajadMr.utils.FileOperations import getFileContentByLine
from SajadMr.utils.Utils import (
    getArchitecture,
...
from SajadMr.Version import getCommercialVersion, getSajadQVersion

# Indicator if we were called as "SajadMr-run" in which case we assume some
# other defaults and work a bit different with parameters.
_SajadMr_binary_name = os.path.basename(sys.argv[0])
if _SajadMr_binary_name == "__main__.py":
    _SajadMr_binary_name = "%s -m SajadMr" % os.path.basename(sys.executable)
is_SajadMr_run = _SajadMr_binary_name.lower().endswith("-run")

if not is_SajadMr_run:
    usage_template = (
...
parser = makeOptionsParser(usage=usage_template % _SajadMr_binary_name)
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run SajadQ, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run SajadQ, most
...
Python binary to use for Scons. Otherwise SajadQ can
use what you run SajadQ with, or find Python installation,
...
and SajadQ will then find it and include it and all the modules found below that
...
and SajadQ will then find it and include it in the binary or extension module
...
with "SajadMr-onefile-unpack" rather than a stream that only the onefile
...
warnings_group = parser.add_option_group("Control the warnings to be given by SajadQ")
...
warnings_group.add_option(
    ...
    help="""\
Allow SajadQ to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the SajadQ website. The mnemonic is the part
...
debugger is automatically chosen unless specified by name with the SajadSx_DEBUGGER_CHOICE
...
remove bloat, add hidden dependencies. Check the SajadQ Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by SajadQ. This is
...
created by SajadQ. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in SajadQ, do not use for
...
development_group = parser.add_option_group("SajadQ Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for SajadQ development only. Takes
...
Note: This is experimental and not working with standalone modes of SajadQ yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of SajadQ commercial.
...
intended for developing plugins and SajadQ package configuration. Use e.g.
...
def _considerPluginOptions(logger):
    ...
    from SajadMr.plugins.Plugins import (
...
def getSajadQProjectOptions(logger, filename_arg, module_mode):
    """Extract the SajadQ project options.

    Note: This is used by SajadQ project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of SajadQ handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)SajadMr-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'SajadMr-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'SajadMr-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'SajadMr-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'SajadMr-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'SajadMr-project-else' not currently allowed after nested SajadMr-project-if.",
                    )
...
def _considerGithubWorkflowOptions(phase):
    ...
    for key, value in json.loads(os.environ["SajadSx_WORKFLOW_INPUTS"]).items():
        if not value:
            continue
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getSajadQProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
pass them to the compiled program execution."""
    ...
    return is_SajadMr_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from SajadMr.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from SajadMr.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from SajadMr.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from SajadMr.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.edit_module_code:
        from SajadMr.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from SajadMr.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from SajadMr.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
