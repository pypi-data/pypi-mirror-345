...
class AsyncInMemorySettingsRegistry(
    ...
    def __init__(
        ...
        settings_menu: Callable[[str], Awaitable[SettingsMenuType]] | SettingsMenuType,
    ...
    async def available_settings(self) -> SettingsMenuType:
        ...
        if not self._dataset_name:
            raise ValueError("Dataset name is required for callable settings menu.")
        return await self._settings_menu(self._dataset_name)
