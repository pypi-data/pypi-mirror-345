...
class FastPluggy(GlobalRegistry):
    ...
    def load_app(self):
      #  self.modules = {}
        ...
        #self.module_manager.create_module_menu_entries(self.menu_manager)
        ...
        self.module_manager.execute_all_module_hook(hook_name='after_template_setup')
    ...
    def include_core_route(self):
        ...
        if self.settings.admin_enabled:
            self.app.include_router(home_router, dependencies=auth_dependencies)

            admin_deps = auth_dependencies + [require_role("fp_admin")]

            from fastpluggy.core.routers.admin import admin_router
            self.app.include_router(admin_router, dependencies=admin_deps, prefix=self.settings.fp_admin_base_url)

            from fastpluggy.core.routers.settings import app_settings_router
            self.app.include_router(app_settings_router, dependencies=admin_deps, prefix=self.settings.fp_admin_base_url)

            from fastpluggy.core.routers.base_module import base_module_router
            self.app.include_router(base_module_router, dependencies=admin_deps, prefix=self.settings.fp_admin_base_url)

        # if self.auth_manager:
        #    self.app.include_router(user_router, dependencies=auth_dependencies, )  # Add user router
        self.app.include_router(execute_router, dependencies=auth_dependencies, )

        self.app.include_router(app_static_router, )
    ...
    def get_folder_by_module_type(self, type_module: str) -> str:
        """
        Helper function to retrieve the manager based on the type of module.
        """
