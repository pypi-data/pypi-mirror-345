...
def stack_forecast(ds: xr.Dataset | str | list[str] | pd.DataFrame) -> xr.Dataset:
    ...
    LEAD_DIM = "lead"
    use_dask = False  # future enhancement: lazy-lad
    if isinstance(ds, xr.Dataset) or isinstance(ds, xr.DataArray):
        ...
        if use_dask:
            return stack_forecast(xr.open_dataset(ds, decode_timedelta=True))  # lazy-load w/ dask
        else:
            return stack_forecast(xr.load_dataset(ds, decode_timedelta=True))  # immediate load
