...
class TestGooeyFilterableDropdown(unittest.TestCase):
    ...
    def test_input_spawns_popup(self):
        ...
        with instrumentGooey(parser) as (app, frame, gapp):
            ...
            self.assertTrue(
                dropdown.model.suggestionsVisible, dropdown.listbox.IsShown()
            )
    ...
    def test_arrow_key_selection_cycling(self):
        ...
        for actions in scenarios:
            ...
            with instrumentGooey(parser) as (app, frame, gapp):
                ...
                self.assertEqual(
                    dropdown.model.suggestionsVisible, initial.expectVisible
                )
                self.assertEqual(
                    dropdown.model.displayValue, initial.expectedDisplayValue
                )
                self.assertEqual(
                    dropdown.model.selectedSuggestion, initial.expectedSelection
                )
                ...
                for action in actions:
                    ...
                    self.assertEqual(
                        dropdown.model.displayValue, action.expectedDisplayValue
                    )
                    self.assertEqual(
