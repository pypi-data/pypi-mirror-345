...
class UnconstrainedNumericType(UnconstrainedValueType):
    ...
    def boolean(self): return self.name() == 'Boolean'
    ...
    #
    def decimal(self): return self.name() == 'Decimal' or self.name() == 'Float'

    def integer(self): return self.name() == 'Integer' or self.name() == 'BigInteger'
...
class UnsupportedUnconstrainedValueType(UnconstrainedValueType):
    ...
    def root_unconstrained_type(self):
        raise Exception(f"Unsupported basic type {self.name()} cannot be the base type of any value type")
