    ...
    if load_adapter:
        ...
        if adapters:
            ...
            if needs_loading:
                ...
                print("copying adapter", adapter_to_load.uri, adapter_path)

                time_start_copy = time.time()
                bucket.copy(adapter_to_load.uri, adapter_path)
        ...
        if "content" in msg and isinstance(msg["content"], list):
            for item in msg["content"]:
                if (
                    ...
                    img_data = item["image"]
                    prefix_single = "data:image/png;base64,"
                    prefix_double = prefix_single + prefix_single
                    if isinstance(img_data, str):
                        if img_data.startswith(prefix_double):
                            print("Detected and removing double prefix")
                            item["image"] = img_data[len(prefix_double) :]
                        elif img_data.startswith(prefix_single):
                            print("Detected and removing single prefix")
                            item["image"] = img_data[len(prefix_single) :]
