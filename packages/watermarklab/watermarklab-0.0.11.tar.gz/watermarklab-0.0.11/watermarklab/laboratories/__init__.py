...
class WLab:
    ...
    def _encode(self, watermark_model: BaseWatermarkModel, dataloader: DataLoader):
        ...
        for index, (cover_list, secret_list, img_indexes, iter_indexes) in tar:
            ...
            result = watermark_model.embed(cover_list, secret_list)
            for stego, cover, secret, img_index, iter_index in zip(result.stego_img, cover_list, secret_list,
                                                                   img_indexes,
                                                                   iter_indexes):
    ...
    def _addnoise(self, watermark_model: BaseWatermarkModel,
        ...
        for noise_model in self.noise_models:
            ...
            for factor in noise_model.factors:
                ...
                tar = tqdm(zip(cover_stego_paths_pair[0], cover_stego_paths_pair[1]),
                           desc=f"Model {watermark_model.modelname} Applying Noise Models", total=total_len)
                for cover_img_paths, stego_img_paths in tar:
                    for cover_img_path, stego_img_path in zip(cover_img_paths, stego_img_paths):
                        for parent in Path(cover_img_path).parents:
                            if parent.name == "images":
                                relative_path = str(Path(cover_img_path).parent.relative_to(parent))
                                break
                        else:
                            raise ValueError("Path does not contain 'images'")
                        parts = relative_path.split("/")
                        img_index = int(parts[0].split("_")[1])
                        iter_index = int(parts[1].split("_")[1])
                        noised_img_save_dir = os.path.join(save_dir, relative_path)
                        os.makedirs(noised_img_save_dir, exist_ok=True)
                        stego_img = np.float32(Image.open(stego_img_path))
                        cover_img = np.float32(Image.open(cover_img_path))
                        noised_img = noise_model.noisemodel.test(stego_img, cover_img, factor)
                        Image.fromarray(noised_img.astype(np.uint8)).save(
                            os.path.join(noised_img_save_dir, "noised.png"))
                        info = {
                        ...
                        with open(os.path.join(noised_img_save_dir, "info.json"), 'w') as f:
    ...
    def test(self, watermark_model: BaseWatermarkModel, dataloader: DataLoader, mode: str = "default") -> dict:
        ...
        robustness_result = self._testrobustness(watermark_model, img_pair_paths[0], noise_info)
    ...
    def _testrobustness(self, watermark_model: BaseWatermarkModel,
        ...
        for info in tqdm(noise_info, desc="Testing Robustness"):
            ...
            for cover_path in flattened_cover_paths:
                for parent in Path(cover_path).parents:
                    if parent.name == "images":
                        relative_path = str(Path(cover_path).parent.relative_to(parent))
                        break
                else:
                    raise ValueError("Path does not contain 'images'")
                parts = relative_path.split("/")
                ...
                noise_img_dir = os.path.join(
                    ...
                    info["noisename"], str(info["factor"]), f"image_{img_index}", f"iter_{iter_index}"
                )
                ...
                if os.path.exists(info_json_path):
                    ...
                    for metric in self.robustnessmetrics:
                        ...
                        try:
                            metric_value = metric.test(org_secret, ext_secret)
                            robustness_result[noise_type]["factors"][str(factor)][image_key][metric_name].append(
                                metric_value
                            )
                        except Exception as e:
                            print(
                                f"Error calculating {metric_name} for image {image_key}, iteration {iter_index + 1}: {e}"
                            )
                else:
                    print(
                        f"info.json ({info_json_path}) not found for image {image_key}, iteration {iter_index + 1}")
            ...
            for cover_path, stego_path in zip(cover_paths, stego_paths):
                ...
                for metric in self.vqmetrics:
                    try:
                        ...
                        visual_quality_result[metric.metric_name].append(metric_value)
                    except Exception as e:
                        print(
                            f"Error calculating {metric.metric_name} for cover: {cover_path} and stego: {stego_path}: {e}")
