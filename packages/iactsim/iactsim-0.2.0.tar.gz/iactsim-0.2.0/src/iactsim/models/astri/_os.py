...
class AstriOpticalSystem(OpticalSystem):
    ...
    def __init__(self):
        ...
        #################################################
        # M2 buffle (should be updated if M2 is tilted)
        #################################################
        m2_buffle_pos = self.m2.position.copy()
        m2_buffle_pos[2] += self.m2.sagitta(900)
        m2_buffle_surface_1 = FlatSurface(
            position = m2_buffle_pos,
            half_aperture = 1070.,
        ...
        buffle_dz = 10
        m2_buffle_surface_2 = CylindricalSurface(
            position = m2_buffle_pos-np.asarray([0,0,0.5*buffle_dz]),
            radius = 1070.,
            height = buffle_dz,
            surface_type=SurfaceType.OPAQUE,
            has_bottom=False,
        ...
        super().__init__(surfaces=[self.m1,self.m2,self._dummy_window,self.fp,camera_body,central_tube, m2_buffle_surface_1, m2_buffle_surface_2], name='ASTRI-OS')
    ...
    def camera_window_parameters(self):
        """Camera window parameterization:
            ...
            - layers thickness
            - layers separation
            - focus shift (positive along the pointing direction)
