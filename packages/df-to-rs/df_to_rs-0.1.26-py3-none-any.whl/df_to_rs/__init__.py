...
class df_to_rs:
    ...
    def upsert_to_redshift(self, df, dest_table, upsert_columns, clear_dest_table=False):
        ...
        try:
            ...
            print(f"Total records: {len(df)}, Records with NULL keys: {len(df_with_nulls)}, Records without NULL keys: {len(df_without_nulls)}")
            ...
            if clear_dest_table:
                ...
                if not df_without_nulls.empty:
                    print("Deleting matching rows (without NULL keys) from main table...")
                    ...
                    print(f"Time taken for non-NULL deletion: {int(time.time() - start_time)} seconds")
                
                # Process NULL records with optimized condition
                if not df_with_nulls.empty:
                    print("Deleting matching rows (with NULL keys) from main table...")
                    ...
                    """
                    cursor.execute(delete_query)
                    print(f"Time taken for NULL deletion: {int(time.time() - start_time)} seconds")
