...
try:
	# https://pypi.python.org/pypi/fast_diff_match_patch
	import fast_diff_match_patch
	def default_differ(text1, text2):
		f = fast_diff_match_patch.diff
	...
	try:
		...
		warnings.warn("xml_diff using slower difflib.SequenceMatcher, install fast_diff_match_patch or diff-match-patch")
		import difflib
		def default_differ(text1, text2):
			diff = difflib.SequenceMatcher(a=text1, b=text2, autojunk=False)
			for (tag, i1, i2, j1, j2) in diff.get_opcodes():
				if tag == "equal":
					...
					raise ValueError("got {}".format(tag))
...
def perform_diff(differ, text1, text2, word_separator_regex):
	...
	def text_to_words(text):
		if not word_separator_regex:
			# Don't map words to Unicode code points. Just do a character-by-character diff.
			for c in text: word_map[c] = c
			return text
