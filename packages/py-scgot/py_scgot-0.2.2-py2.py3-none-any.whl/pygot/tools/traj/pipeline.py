...
def post_process(adata, embedding_key, kernel, pseudotime_key, cell_type_key=None, single_branch_detect=True):
    ...
    ot_ct_tree, _, _ = topological_tree(adata, embedding_key=embedding_key, cell_type_key=cell_type_key, time_key=pseudotime_key, start_cell_type=adata.obs[cell_type_key].tolist()[adata.uns['ot_ct_root']])
...
def fit_velocity_model_without_time(
    ...
    First it use `pygot.tl.traj.determine_source_states` to determine source cell and
    corresponding pseudotime. Because pseudotime can not be transform into real time directly,
    it use minimum-spanning tree to detect the developing structure and adjust pseudotime according 
    to different lineage. Then it auto-determine the split number and split dataset into k portions.
     
    After that, it train velocity model with generated time label and use continuity equation to infer
    the time conditional distribution to update time label. By doing so, the velocity model will be
    trained iteratively, and converge in 2-3 iteration.
    ...
    kernel: 'dpt' or 'palantir' (default: 'dpt')
    ...
    model = _fit_velocity_model(adata, embedding_key,  device=device, **kwargs)
    return model
