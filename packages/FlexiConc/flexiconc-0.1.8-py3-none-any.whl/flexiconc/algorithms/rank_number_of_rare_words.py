...
def rank_number_of_rare_words(conc, **args) -> dict:
    ...
    df_freq = conc.root.concordance().resources.get("frequency_list", freq_list_name)
    if p_attr not in df_freq.columns or "f" not in df_freq.columns:
    ...
    freq_map = dict(zip(df_freq["_key"], df_freq["f"]))
    ...
    tokens["_key"] = key_tok
    tokens["f"]    = tokens["_key"].map(freq_map)
    tokens["rank"] = tokens["_key"].map(rank_map)

    mask = tokens["f"].isna()

    if threshold is not None:
        freq_mask = tokens["f"] < threshold
        mask |= freq_mask
