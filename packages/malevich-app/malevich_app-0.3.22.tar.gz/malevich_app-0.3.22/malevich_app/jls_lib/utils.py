...
from typing import Optional, Any, List, Dict, Union, Tuple, TypeVar, Generic, Callable
import pandas as pd
import numpy as np
...
def _dict_with_class(base):
    ...
    assert issubclass(cl, BaseModel), "app_cfg extension: class should be subclass of BaseModel"
...
class Context(Generic[MinimalCfg]):
    class _DagKeyValue:
        ...
        def __init__(self, operation_id: str, run_id: Optional[str], dag_host_port: str, secret: str, local_storage: Optional['LocalStorage']):
        ...
        def __get_bytes2(self, key: str) -> bytes:
            operation = OperationWithRunWithKey(operationId=self.__operation_id, runId=self.__run_id, key=key, hostedAppId=C.APP_ID, secret=self.__secret)
            return asyncio.run(send_post_dag(operation.model_dump_json(), GET_KEYS_VALUES_RAW(self.__dag_host_port), raw_res=True,
                                       headers={'Content-type': 'application/json', 'Accept': 'application/python-pickle'}, operation_id=self.__operation_id))
        ...
        def get(self, keys: List[str]) -> Dict[str, Any]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), GET_KEYS_VALUES(self.__dag_host_port), operation_id=self.__operation_id))
        ...
        def get_all(self) -> Dict[str, Any]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), GET_KEYS_VALUES_ALL(self.__dag_host_port), operation_id=self.__operation_id))
            return self.__fix_get2(res)
        ...
        async def __update(self, keys_values: Dict[str, Any]):
            ...
            for k, v in keys_values.items():
                if isinstance(v, bytes):
                    ...
                    sends.append(send_post_dag(v, url, headers={'Content-type': 'application/python-pickle', 'Accept': 'application/json'}, operation_id=self.__operation_id))    # TODO secret
                else:
            ...
            for k, v in keys_values.items():
                if isinstance(v, bytes):
                    ...
                    if C.WS is not None:
                        ...
                        url = f"{POST_KEYS_VALUES_RAW(self.__dag_host_port)}/{k}/{C.APP_ID}/{self.__operation_id}{run_id_part}"
                    asyncio.run(send_post_dag(v, url, headers={'Content-type': 'application/python-pickle', 'Accept': 'application/json'}, operation_id=self.__operation_id))     # TODO secret
                else:
                    main_keys_values[k] = v
            if len(main_keys_values) > 0:
                operation = KeysValues(operationId=self.__operation_id, data=main_keys_values, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
                asyncio.run(send_post_dag(operation.model_dump_json(), POST_KEYS_VALUES(self.__dag_host_port), operation_id=self.__operation_id))
        ...
        def clear(self) -> None:
            ...
            operation = OperationWithRun(operationId=self.__operation_id, runId=self.__run_id, hostedAppId=C.APP_ID, secret=self.__secret)
            asyncio.run(send_delete_dag(operation.model_dump_json(), DELETE_KEYS_VALUES_ALL(self.__dag_host_port), operation_id=self.__operation_id))
    ...
    class _ObjectStorage:
        def __init__(self, operation_id: str, dag_host_port: str, secret: str, single_pod: bool, local_storage: Optional['LocalStorage']):
            self.__operation_id = operation_id
            self.__dag_host_port = dag_host_port
            self.__secret = secret
        ...
        def get_keys(self, local: bool = False, all_apps: bool = False) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), GET_OBJ_STORAGE_KEYS(self.__dag_host_port), operation_id=self.__operation_id))
            return res["result"]
        ...
        def get(self, keys: List[str], force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), GET_OBJ_STORAGE(self.__dag_host_port), operation_id=self.__operation_id))
            return res["result"]
        ...
        def get_all(self, local: bool = False, force: bool = False, all_apps: bool = True) -> List[str]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), GET_OBJ_STORAGE_ALL(self.__dag_host_port), operation_id=self.__operation_id))
            return res["result"]
        ...
        def update(self, keys: List[str], presigned_expire: Optional[int] = -1) -> Dict[str, str]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), POST_OBJ_STORAGE(self.__dag_host_port), operation_id=self.__operation_id))
            return res["result"]
        ...
        def presigned(self, keys: List[str], expire: Optional[int] = None) -> Dict[str, str]:
            ...
            res = asyncio.run(send_post_dag(operation.model_dump_json(), POST_PRESIGNED_OBJ_STORAGE(self.__dag_host_port), operation_id=self.__operation_id))
            return res["result"]
        ...
        def delete(self, keys: List[str]) -> None:
            ...
            asyncio.run(send_delete_dag(operation.model_dump_json(), DELETE_OBJ_STORAGE(self.__dag_host_port), operation_id=self.__operation_id))
    ...
    """context for run"""
    def __init__(self, operation_id: str = None, run_id: str = None, app_id: str = None, app_cfg: Dict[str, Any] = None, msg_url: str = None, email: Optional[str] = None, dag_host_port: str = None, secret: str = None, single_pod: bool = None, indexes: int = None, login: str = None, japp_logs_buffer: io.StringIO = None, local_storage: Optional['LocalStorage'] = None, logger_fun: Optional[Callable[[str, Optional[str], Optional[str], bool], logging.Logger]] = None, *, initialize: bool = True):
        ...
        self.email = email
        self.dag_key_value = Context._DagKeyValue(operation_id, run_id, dag_host_port, secret, local_storage)
        self.object_storage = Context._ObjectStorage(operation_id, dag_host_port, secret, single_pod, local_storage)
        self.__secret = secret
        self.__single_pod = single_pod
        ...
        if not self.__single_pod:
            ...
            if C.IS_EXTERNAL:
                ...
                if data is not None and len(data) > 0:
                    ...
                    asyncio.run(send_post_dag(data, url, headers={'Content-type': 'application/octet-stream'}, operation_id=self.__operation_id))    # TODO secret
            else:
                ...
                asyncio.run(send_post_dag(settings.model_dump_json(), SYNCHRONIZE(self.__dag_host_port), operation_id=self.__operation_id))
            ...
            if C.IS_EXTERNAL:
                ...
                if data is not None and len(data) > 0:
                    if C.WS is not None:
                        ...
                        url = self.__objects_url(all_runs)
                    await send_post_dag(data, url, headers={'Content-type': 'application/octet-stream'}, operation_id=self.__operation_id)   # TODO secret
            else:
                settings = SynchronizeSettings(operationId=self.__operation_id, runId=run_id, paths=paths, hostedAppId=C.APP_ID, secret=self.__secret)
                await send_post_dag(settings.model_dump_json(), SYNCHRONIZE(self.__dag_host_port), operation_id=self.__operation_id)
    ...
    def msg(self, data: Union[str, Dict], url: Optional[str] = None, headers: Optional[Dict[str, str]] = None, wait: bool = False, wrap: bool = True, with_result: bool = False):
        ...
        """
        return asyncio.run(self.async_msg(data, url, headers, wait, wrap, with_result))
    ...
    def email_send(self, message: str, subject: Optional[str] = None, type: str = "gmail") -> None:
        ...
        try:
            asyncio.run(send_post_dag(msg.model_dump_json(), MESSAGE(self.__dag_host_port), raw_res=True, operation_id=self.__operation_id)) # FIXME
        except:
    ...
    def get_scale_part(self, jdf: JDF) -> JDF:
        ...
        if isinstance(jdf, DFS) or isinstance(jdf, Sink):
    ...
    def as_object(self, paths: Dict[str, str], path_prefix: Optional[str] = None, *, dir: Optional[str] = None, allow_update_dir: bool = True) -> OBJ:
        if dir is None:
            dir = str(uuid4())
