...
export function createTopButtons(
  ...
  uiButton.addEventListener('click', () => {
    ...
    // Get the proxy port from settings
    const settings = getSettings();
    const port = settings.proxyPort || 3000;

    // Get the current window's protocol and host
    ...
    // Format the URL using the proxy endpoint and port
    const formattedUrl = `${baseUrl}/proxy/${port}`;
    console.log('Opening UI at URL:', formattedUrl);
