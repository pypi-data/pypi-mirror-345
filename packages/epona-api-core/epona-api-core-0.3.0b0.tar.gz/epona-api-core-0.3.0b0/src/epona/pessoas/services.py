...
from src.epona.common import create_suuid
from .models import InfoContato, Pessoa
from .schemas import (
    InfoContatoPayloadSchema,
    InfoContatoResponseSchema,
    PessoaPayloadSchema,
    PessoaResponseSchema,
    PessoaSchema
)

env = os.getenv("EVIRONMENT", "dev")

async def save(payload: PessoaPayloadSchema, user: UserSchema) -> str:
...
async def update(payload: PessoaPayloadSchema, user: UserSchema) -> Union[int, None]:
    ...
    while attempt < 3:
        ...
        try:
            ...
            logging.error(f"Falha ao atualizar pessoa. Tentativa: {attempt}/3.\n{str(err)}")
...
async def save_xlsx(wb: Workbook, user: UserSchema) -> List[PessoaResponseSchema]:
    ...
    for pessoa in pessoas:
        ...
        if result:
            armazenadas.append(
                PessoaResponseSchema(**{"id": result, "suuid": result, **pessoa.dict()}))
    return armazenadas


async def get(pk: str, user: UserSchema) -> Union[PessoaSchema, None]:
    """retorna informações de uma pessoa ou empresa filtrada por id"""
    pessoa = await Pessoa.filter(suuid=pk, client_id=user.client_id).first()
    if not pessoa:
        return None
...
async def find_principal(
    ...
    empresa = await Pessoa.filter(suuid=pk, client_id=user.client_id).first()
    if not empresa:
        return None
    if empresa.matriz:
...
def _make_contato_response(contato: InfoContato) -> InfoContatoResponseSchema:
    contato_dict = dict(contato)
    contato_dict["id"] = contato.suuid
    contato_dict["pessoa_id"] = contato.pessoa_suuid
    return InfoContatoResponseSchema(**contato_dict)


async def save_contato(
    pk: str, payload: InfoContatoPayloadSchema, user: UserSchema
    ...
    while attempt < 3:
        ...
        try:
            if pk:
                info_contato = await InfoContato.filter(suuid=pk, client_id=user.client_id).first()
                if not info_contato:
                    return None
                info_contato.email = payload.email
                info_contato.telefone = payload.telefone
                info_contato.tipo = payload.tipo
                info_contato.created_at = \
                    datetime.fromisoformat(str(info_contato.created_at)[:23])  # precisao de milesimos
            else:
            ...
            logging.error(f"Falha na tentativa de salvar contato: {attempt}/3\n: {str(err)}")
            await asyncio.sleep(attempt * 5)
