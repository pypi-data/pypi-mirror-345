...
class FixedAssets:
    ...
    def backup_excel_file():
        shutil.copy(
            src=r'V:\Accounting\Work\ОТДЕЛ ЗАТРАТ\Основные средства и НМА\Амортизация\2024\!Амортизация_ помесячно_БУ_2024.xlsb',
            dst=r'V:/Findep/Incoming/test/DevOps/Fixed assets/DataSource_Backups/!Амортизация_помесячно_БУ_2024_' + datetime.now().strftime(
                "%d-%m-%Y") + '.xlsb'
        )

    def read_data_from_excel_file(self):
        df = ExcelParser(self.define_excel_file_name(), self.Month_name, "Класс").read_data()
        ...
        pattern: Pattern[str] = re.compile(r'(\bTMA\w*)|(\bXW\w*)|(\bKM\w*)|(\bZ94\w*)|(\bVIN\s202\w*)|(\bVIN\s000\w*)')
...
if __name__ == '__main__':
    ...
    # print(x.read_data_from_excel_file())
    x.backup_excel_file()
