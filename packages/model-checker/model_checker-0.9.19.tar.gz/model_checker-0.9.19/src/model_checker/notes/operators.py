...
class EssenceOperator(syntactic.Operator):
    ...
    The truth conditions involve three requirements:
        1. Any fusion of verifiers for A and B must verify B
    ...
    def true_at(self, leftarg, rightarg, eval_point):
        ...
        semantics = self.semantics          # Import the semantics
        N = semantics.N                     # Import the number of atomic states
        x = z3.BitVec("true_essence_y", N)  # Z3 BitVector variable of length N
        y = z3.BitVec("true_essence_y", N)  # Z3 BitVector variable of length N
        return z3.And(
            # Condition 1: Fusing verifiers of A and B yields a verifier of B
            # For any states x and y,
            ForAll(
                [x, y],
                z3.Implies(
                    ...
                    # Then the fusion x.y verifies B
                    semantics.extended_verify(semantics.fusion(x, y), rightarg, eval_point)
                ),
            ),
            # Condition 2: Every verifier of B has a part that verifies A
            # For every state x,
            ForAll(
                [x],
                z3.Implies(
            ...
            # Condition 3: Every falsifier of A is also a falsifier of B
            # For every state x,
            ForAll(
                [x],
                z3.Implies(
    ...
    def false_at(self, leftarg, rightarg, eval_point):
        ...
        return z3.Or(
            # Condition 1: There exist verifiers of A and B whose fusion does not verify B
            # There exist states x and y,
            Exists(
                [x, y],
                z3.And(
                    ...
                    # And y verifies B
                    semantics.extended_verify(y, rightarg, eval_point),
                    # But their fusion does not verify B
                    z3.Not(semantics.extended_verify(semantics.fusion(x, y), rightarg, eval_point))
                ),
            ),
            # Condition 2: There exists a verifier of B that has no part verifying A
            # There exists a state x,
            Exists(
                [x],
                z3.And(
            ...
            # Condition 3: There exists a falsifier of A that is not a falsifier of B
            # There exists a state x,
            Exists(
                [x],
                z3.And(
