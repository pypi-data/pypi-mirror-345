    ...
    try:
        ...
        for step in ["optimization", "frequency", "single_point"]:
            if step in config:
                ...
                # Extract blocks
                step_blocks = config[step].get("blocks", [])
                blocks.append(step_blocks)
                logger.debug(f"{step.capitalize()} blocks: {step_blocks}")
            else:
                # If a step is missing, use the previous step's keywords and blocks
                if keywords:
                    ...
                    logger.debug(
                        f"{step.capitalize()} step not defined, using previous blocks"
                    )
                else:
                    logger.error(
            ...
            if "molecules" in config:
                ...
                for idx, molecule in enumerate(config["molecules"], 1):
                    ...
                    process_molecule(
                        ...
                        keywords=keywords,
                        blocks=blocks,  # Pass blocks to processor
                        cpus=cpus,
...
def process_molecule(
    ...
    molecule_dir: Path,
    keywords: List[List[str]],
    blocks: List[List[str]],  # New parameter
    cpus: int,
    mem_per_cpu_gb: int,
    scratch_base_dir: Path,
    overwrite: bool,
    ...
    run_opt_freq_sp(
        ...
        xyz_file=xyz_file,
        keywords=keywords,
        blocks=blocks,  # Pass blocks to pipeline
        cpus=cpus,
        mem_per_cpu_gb=mem_per_cpu_gb,
...
blocks = ["%TDDFT NROOTS 64 TDA FALSE DONTO TRUE END"]
