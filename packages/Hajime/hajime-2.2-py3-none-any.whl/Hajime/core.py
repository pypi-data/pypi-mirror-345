...
class Hajime:
    def __init__(self, database=None):
        ...
        self.scripts_folder = "scripts"  # Added scripts folder
        self.middlewares = []
        self.sessions = {}
        self.ws_routes = {}
        self.templates_cache = {}  
        self.static_cache = {}
        self.scripts_cache = {}  # Added cache for scripts folder
        if database:
        ...
        self.preload_scripts_files()  # Added preloading for scripts folder
    ...
    def preload_static_files(self):
        ...
        svelte_files = [f for f in self.static_cache.keys() if f.endswith('.svelte')]
        print(f"🚀 Loaded {len(js_files)} JavaScript files: {', '.join(js_files)}")
        print(f"🔥 Loaded {len(svelte_files)} Svelte files: {', '.join(svelte_files)}")

    def preload_scripts_files(self):
        """Preloads all script files from scripts folder into memory."""
        scripts_dir = os.path.join(os.getcwd(), self.scripts_folder)
        if os.path.exists(scripts_dir):
            for root, _, files in os.walk(scripts_dir):
                for file in files:
                    file_path = os.path.join(root, file)
                    relative_path = os.path.relpath(file_path, scripts_dir)
                    with open(file_path, "rb") as f:
                        self.scripts_cache[relative_path] = f.read()
        print("📜 All script files preloaded!")
        # Add a debug log to check for JS files
        js_files = [f for f in self.scripts_cache.keys() if f.endswith('.js')]
        svelte_files = [f for f in self.scripts_cache.keys() if f.endswith('.svelte')]
        print(f"🚀 Loaded {len(js_files)} JavaScript files from scripts: {', '.join(js_files)}")
        print(f"⚛️ Loaded {len(svelte_files)} Svelte files from scripts: {', '.join(svelte_files)}")
    ...
    def template(self, filename, **context):
        ...
        # Process function calls for JavaScript and Svelte files FIRST
        js_function_pattern = r'{{\s*js_include\([\'"](.+?)[\'"]\)\s*}}'
        template = re.sub(js_function_pattern, lambda m: self.include_js(m.group(1)), template)
        
        svelte_function_pattern = r'{{\s*svelte_include\([\'"](.+?)[\'"]\)\s*}}'
        template = re.sub(svelte_function_pattern, lambda m: self.include_svelte(m.group(1)), template)
    ...
    def get_script_path(self, script_file):
        """Check if script exists in scripts folder or static folder and return proper path"""
        if script_file in self.scripts_cache:
            return f"/scripts/{script_file}"
        elif script_file in self.static_cache:
            return f"/static/{script_file}"
        else:
            print(f"Warning: Script '{script_file}' not found in either scripts or static folders")
            # Return scripts path as default since that was likely the intent
            return f"/scripts/{script_file}"
    ...
    def serve_static(self, path, start_response):
        ...
        if relative_path in self.static_cache:
            ...
            # Add Svelte MIME type support
            elif relative_path.endswith('.svelte'):
    ...
    def serve_scripts(self, path, start_response):
        """Serve script files instantly from scripts cache."""
        relative_path = path.replace('/scripts/', '', 1)

        if relative_path in self.scripts_cache:
            mime_type, _ = mimetypes.guess_type(relative_path)
            # Ensure JS files have proper MIME type
            if relative_path.endswith('.js'):
                mime_type = 'application/javascript'
            # Add Svelte MIME type support
                ...
                mime_type = 'text/javascript'
            start_response("200 OK", [('Content-Type', mime_type or 'application/octet-stream')])
            return [self.scripts_cache[relative_path]]
        else:
            start_response("404 Not Found", [('Content-Type', 'text/plain')])
            return [b"404 Not Found"]
    ...
    def include_js(self, js_file):
        ...
        return f'<script src="{self.get_script_path(js_file)}"></script>'
        
    def include_svelte(self, svelte_file):
        """Helper to generate script tag for Svelte files"""
        return f'<script src="{self.get_script_path(svelte_file)}" type="module"></script>'
    ...
    def __call__(self, environ, start_response):
        ...
        if path.startswith("/scripts/"):
            return self.serve_scripts(path, start_response)
    ...
    def redirect(self, location, status_code=302):
        ...
        status = status_text.get(status_code, "302 Found")
