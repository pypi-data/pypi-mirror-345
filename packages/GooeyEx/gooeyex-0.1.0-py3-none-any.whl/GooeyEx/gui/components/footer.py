...
class Footer(wx.Panel):
    ...
    def updateTimeRemaining(self, *args, **kwargs):
        ...
        if elapsed_time_value is None:
            ...
            self.time_remaining_text.SetLabel(
                f"{elapsed_time_value}<{estimate_time_remaining}"
            )
            return
        else:
    ...
    def _init_components(self):
        self.cancel_button = self.button(
            _("cancel"), wx.ID_CANCEL, event_id=events.WINDOW_CANCEL
        )
        self.stop_button = self.button(_("stop"), wx.ID_OK, event_id=events.WINDOW_STOP)
        self.start_button = self.button(
            _("start"), wx.ID_OK, event_id=int(events.WINDOW_START)
        )
        self.close_button = self.button(
            _("close"), wx.ID_OK, event_id=int(events.WINDOW_CLOSE)
        )
        self.restart_button = self.button(
            _("restart"), wx.ID_OK, event_id=int(events.WINDOW_RESTART)
        )
        self.edit_button = self.button(
            _("edit"), wx.ID_OK, event_id=int(events.WINDOW_EDIT)
        )
    ...
    def _do_layout(self):
        ...
        h_sizer.Add(
            self.progress_bar, 1, wx.ALIGN_LEFT | wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 20
        )
    ...
    def button(self, label=None, style=None, event_id=-1):
        return wx.Button(
            parent=self, id=event_id, size=(90, -1), label=label, style=style
        )
