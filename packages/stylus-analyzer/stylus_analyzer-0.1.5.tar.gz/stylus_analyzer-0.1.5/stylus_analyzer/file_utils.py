...
def get_rust_parser():
    ...
    if _RUST_PARSER is None:
        try:
            ...
            # Check if tree-sitter-rust exists, if not, create it
            if not os.path.exists(rust_dir):
                os.makedirs(rust_dir, exist_ok=True)
                # Copy the necessary files from the installed tree-sitter-rust package
                installed_rust_dir = pkg_resources.resource_filename('tree_sitter_rust', '')
                if os.path.exists(installed_rust_dir):
                    for item in ['src', 'grammar.js', 'package.json', 'binding.gyp']:
                        src = os.path.join(installed_rust_dir, item)
                        dst = os.path.join(rust_dir, item)
