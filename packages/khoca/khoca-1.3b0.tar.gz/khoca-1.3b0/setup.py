...
try:
    ...
    if 'SAGE_LOCAL' in SAGE_ENV:
        sage_local = SAGE_ENV['SAGE_LOCAL']
        print('using SAGE_LOCAL: %s' % sage_local)
except ImportError:
    pass

if sage_local:
    sage_local_include = join(sage_local, 'include')
    sage_local_lib = join(sage_local, 'lib')
    include_dirs += [sage_local_include]
    library_dirs += [sage_local_lib]
