...
def total_background(l, b, date, time, Sun_sp_wl = wavelenght_newguey2003, Sun_sp_fx = flux_newguey2003, V_wl = wavelenght_band_V, V_tr = trancparency_band_V, wavelenght_airglow = wavelenght_kp, intensity_airglow = intensity_kp, wavelenght_atmosphere = wavelenght_modtran_kp, transparency_atmosphere = trancparency_modtran_kp, venus_albedo_wl = venus_alb_wl, venus_albedo_rf = venus_alb_rf, mars_albedo_wl = mars_alb_wl, mars_albedo_rf = mars_alb_rf, jupiter_albedo_wl = jupiter_alb_wl, jupiter_albedo_rf = jupiter_alb_rf, saturn_albedo_wl = saturn_alb_wl, saturn_albedo_rf = saturn_alb_rf):
    ...
    Параметры:
        ...
        wavelenght_atmosphere (ndarray, optional): Массив длин волн для модели атмосферы (в нм).
                                                      По умолчанию используется `wavelenght_modtran_kp`.
        transparency_atmosphere (ndarray, optional): Массив значений прозрачности атмосферы.
...
def total_background_on_Earth(lat_obs, lon_obs, h, zenith_angle, azimuth_angle, date, time, wl_tr = [None], sp_tr = [None], Sun_sp_wl = wavelenght_newguey2003, Sun_sp_fx = flux_newguey2003, V_wl = wavelenght_band_V, V_tr = trancparency_band_V, wavelenght_airglow = wavelenght_kp, intensity_airglow = intensity_kp, wavelenght_atmosphere = wavelenght_modtran_kp, transparency_atmosphere = trancparency_modtran_kp, venus_albedo_wl = venus_alb_wl, venus_albedo_rf = venus_alb_rf, mars_albedo_wl = mars_alb_wl, mars_albedo_rf = mars_alb_rf, jupiter_albedo_wl = jupiter_alb_wl, jupiter_albedo_rf = jupiter_alb_rf, saturn_albedo_wl = saturn_alb_wl, saturn_albedo_rf = saturn_alb_rf):
    """
    ...
    Параметры:
        lat_obs (float): Широта наблюдателя (в градусах).
        lon_obs (float): Долгота наблюдателя (в градусах).
        h (float): Высота наблюдателя над уровнем моря (в метрах).
        zenith_angle (float): Зенитный угол направления наблюдения (в градусах).
        azimuth_angle (float): Азимутальный угол направления наблюдения (в градусах, от севера по часовой стрелке).
        date (str): Дата наблюдения в формате 'YYYY-MM-DD' (например, '2023-11-03').
        time (str): Время наблюдения в формате 'HH:MM:SS' (например, '12:00:00').
    ...
    tot_gal_wl, tot_gal_sp = total_background(l, b, date, time, Sun_sp_wl, Sun_sp_fx, V_wl, V_tr, wavelenght_airglow, intensity_airglow, wavelenght_atmosphere, transparency_atmosphere, venus_albedo_wl, venus_albedo_rf, mars_albedo_wl, mars_albedo_rf, jupiter_albedo_wl, jupiter_albedo_rf, saturn_albedo_wl, saturn_albedo_rf)
