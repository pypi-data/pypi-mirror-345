...
from SajadMr import Options
from SajadMr.__past__ import iter_modules, unicode
from SajadMr.containers.Namedtuples import makeNamedtupleClass
from SajadMr.containers.OrderedSets import OrderedSet
from SajadMr.freezer.IncludedDataFiles import (
    decodeDataFileTags,
...
from SajadMr.freezer.IncludedEntryPoints import (
    makeDllEntryPoint,
    makeExeEntryPoint,
)
from SajadMr.ModuleRegistry import (
...
from SajadMr.PythonFlavors import (
    isAnacondaPython,
    isDebianPackagePython,
    isSajadQPython,
)
from SajadMr.PythonVersions import (
    getTestExecutionPythonVersions,
    python_version,
    python_version_full_str,
    python_version_str,
)
from SajadMr.Tracing import plugins_logger
from SajadMr.utils.AppDirs import getAppdirsModule
from SajadMr.utils.Distributions import (
    getDistributionFromModuleName,
    getDistributionName,
    isDistributionCondaPackage,
)
from SajadMr.utils.Execution import (
    SajadQCalledProcessError,
    check_output,
...
from SajadMr.utils.Importing import getSharedLibrarySuffix, isBuiltinModuleName
...
from SajadMr.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from SajadMr.utils.SlotMetaClasses import getMetaClassBase
from SajadMr.utils.StaticLibraries import getSystemStaticLibPythonPath
...
def _getPackageData(package_name, resource, default=None):
    ...
    from SajadMr.utils.PackageResources import getPackageData
...
class SajadQPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """SajadQ base class for all plugins.

    Derive your plugin from "SajadQPluginBase" please.
    For instructions, see https://github.com/SajadQ/SajadQ/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt SajadQ's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by SajadQ. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the SajadQ command line. However, some plugins are always enabled
    and invisible to the user.

    SajadQ comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "SajadMr --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which SajadQ
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon SajadQ start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from SajadMr.importing.Importing import locateModules
...
""" % {
        ...
        except SajadQCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
