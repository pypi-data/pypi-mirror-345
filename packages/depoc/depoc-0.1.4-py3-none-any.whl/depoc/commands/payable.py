    ...
    if ctx.invoked_subcommand is None:
        ...
        if response := _handle_response(service, limit=limit, page=page):
            ...
            if not all:
                ...
                console.rule(
                    '[bold]OUTSTANDING PAYABLES',
                    align='center',
                    style='bold',
                )
            else:
                page_summary(response)
                console.rule('ALL PAYABLES', align='center', style='none')
            ...
            format_total = f'[yellow]R${total_payable:,.2f}'
            message = f'\n{'[bold]💵 Total to be paid: ' + format_total}\n'
            console.print(message)

@payable.command
def create() -> None:
    ''' Create payable. '''
    panel = Panel('[bold]+ ADD NEW PAYABLE')
    console.print(panel)
    ...
    service = client.payables.create
...
@click.argument('id')
def get(id: str) -> None:
    ''' Retrieve an specific payable. '''
    service = client.payables.get
...
def settle(id: str, amount: float, account: str) -> None:
    ...
    service = client.payable_settle.create
...
def search(
    ...
    service = client.payables.filter
