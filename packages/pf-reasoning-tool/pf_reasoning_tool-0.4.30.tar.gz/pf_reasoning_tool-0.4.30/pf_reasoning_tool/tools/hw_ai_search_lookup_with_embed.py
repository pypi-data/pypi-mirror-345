# -*- coding: utf-8 -*-
"""
HW AI Search Lookup Tool (Internal Embedding - Manual Input v2 - AttributeError Fixed)
====================================================================================
...
FIXED AttributeError: 'SearchClient' object has no attribute 'index_name'.
"""
...
# Removed: SearchIndexClient, SearchFieldDataType, traceback (no longer needed)
...
# ------------------------------------------------------------------ #
# 6.  Main search execution logic (FIXED AttributeError)             #
# ------------------------------------------------------------------ #
def _execute_search(
    query_text: str,
    ...
    index_name: str, # <<<--- ADDED index_name as a parameter HERE
) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Executing search on index '{index_name}': Type='{query_type}', Query='{query_text[:50]}...'") # Use index_name here
    ...
    if needs_vec:
        if vector is None:
             raise RuntimeError("Internal Error: Vector was required but is None before search.")
    ...
    # --- Execute Search ---
    # <<<--- CORRECTED LOG LINE uses index_name variable --- >>>
    logger.debug(f"Sending search request to index '{index_name}' with params (vector omitted): "
                 f"{ {k: v for k, v in params.items() if k != 'vector_queries'} }")
    try:
        results = search_client.search(**params)
        results_list = list(results)
        logger.info(f'Search successful for index {index_name}, received {len(results_list)} results.') # Use index_name here
    except Exception as e:
        logger.error(f'Search request failed for index {index_name}: {e}', exc_info=True) # Use index_name here
        logger.error(f"Failed Search Params (vector omitted): { {k: v for k, v in params.items() if k != 'vector_queries'} }")
        # Hints using index_name variable
    ...
    for doc_dict in results_list:
        ...
        metadata = {
            ...
            if k != text_field and (not vector_field or k != vector_field) and not k.startswith('@search.')
        }
...
def hw_ai_search_lookup_with_embed(
    ...
    """
    (Manual Input v2 - AttrFix) Performs lookup on Azure AI Search, embedding queries internally.
    Requires manual input for index name, text field, vector field, and semantic config.
    """
    run_id = os.getenv("PROMPTFLOW_RUN_ID", "local")
    logger.info(f"Starting HW AI Search Lookup Tool (Manual Input v2 - Run ID: {run_id}).")
    ...
    all_results = []
    # --- Threading logic updated to pass index_name ---
    search_func_partial = partial(
        ...
        index_name=index_name, # <<<--- PASSED index_name HERE
    )
    ...
    with ThreadPoolExecutor() as executor:
        ...
        results_map = {} # Store results keyed by original query to maintain order
        for future in tasks:
            ...
            try:
                ...
                logger.error(f"Search task for query {query_preview} failed in thread: {type(e).__name__} - {e}", exc_info=False) # exc_info=False to avoid duplicate stack trace if already logged
