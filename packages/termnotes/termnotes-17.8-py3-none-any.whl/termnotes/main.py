...
class MarkupAwareHighlighter(RegexHighlighter):
  """
  Custom highlighter that avoids highlighting within Rich markup tags.
  """
  base_style = ""
  highlights = [
    r"(?P<url>(?<!\[)\b(https?://|www\.)\S+(?!\]))",  # Avoid URLs within [...]
    r"(?P<number>(?<!\[)\b\d+(\.\d*)?\b(?!\]))",     # Avoid numbers within [...]
    r"(?P<email>(?<!\[)\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b(?!\]))", # Avoid emails within [...]
  ]

highlighter = MarkupAwareHighlighter()
...
def read_note(folder, name):
  ...
  content_for_display = Text("\n".join(modified_lines))
  highlighter.highlight(content_for_display)
