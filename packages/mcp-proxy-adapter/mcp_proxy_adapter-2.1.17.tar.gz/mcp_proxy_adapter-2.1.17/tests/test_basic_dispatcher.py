...
@pytest.mark.asyncio
async def test_register_and_execute_command(test_dispatcher):
    ...
    result = await test_dispatcher.execute("test", a=5)
    assert result == 6  # 5 + 1 (default value)
    
    # Execute command with all parameters
    result = await test_dispatcher.execute("test", a=5, b=3)
    assert result == 8  # 5 + 3

@pytest.mark.asyncio
async def test_command_not_found(test_dispatcher):
    """Test handling of non-existent command"""
    with pytest.raises(CommandNotFoundError):
        await test_dispatcher.execute("non_existent_command")

@pytest.mark.asyncio
async def test_command_execution_error(test_dispatcher):
    """Test handling of error during command execution"""
    with pytest.raises(CommandExecutionError):
        await test_dispatcher.execute("error")
...
@pytest.mark.asyncio
async def test_params_command(test_dispatcher):
    """Test command accepting params dictionary"""
    test_params = {"key1": "value1", "key2": 123}
    result = await test_dispatcher.execute("params", **test_params)
    assert result == test_params

@pytest.mark.asyncio
async def test_help_command(test_dispatcher):
    """Test built-in help command"""
    # Request list of all commands
    result = await test_dispatcher.execute("help")
