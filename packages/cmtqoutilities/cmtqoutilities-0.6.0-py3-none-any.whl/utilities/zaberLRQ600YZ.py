...
class LRQ600():
    ...
    def home(self):
        ...
        if not successy or not successx:
...
#                 xx=0.601/self.scale-xx
    ...
    def moveTo(self, newPosition):
        if self.invert[1]:
            yy=0.601/self.scale-newPosition[1]
        else:
            yy=newPosition[1]
        if self.invert[0]:
            xx=0.601/self.scale-newPosition[0]
        else:
            xx=newPosition[0]
    ...
    def stop(self):
        ...
        [successy,replz] = self.execute(self.yAxis,self.cmd["stop"])
        if not successx or not successy:
