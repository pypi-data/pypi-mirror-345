...
from polylith_cli.polylith.cli import build, create, options, test
from typer import Exit, Option, Typer
from typing_extensions import Annotated
app = Typer()
app.add_typer(create.app, name='create', help='Commands for creating a workspace, bases, components and projects.')
app.add_typer(build.app, name='build', help='For Package & Dependency Management tools without support for plugins or build hooks.')
app.add_typer(test.app, name='test', help='Commands for tests.')
...
def enriched_with_lock_file_data(root: Path, project_data: dict, is_verbose: bool) -> dict:
    try:
...
def enriched_with_lock_files_data(root: Path, projects_data: List[dict], is_verbose: bool) -> List[dict]:
...
def check_command(strict: Annotated[bool, options.strict]=False, verbose: Annotated[bool, options.verbose]=False, quiet: Annotated[bool, options.quiet]=False, directory: Annotated[str, options.directory]='', alias: Annotated[str, options.alias]=''):
    ...
    cli_options = {'verbose': verbose, 'short': False, 'quiet': quiet, 'strict': strict, 'alias': str.split(alias, ',') if alias else []}
    ...
    libs_result = commands.check.check_libs_versions(filtered_projects, all_projects_data, cli_options)
    if not result or not libs_result:
        raise Exit(code=1)

@app.command('diff')
def diff_command(since: Annotated[str, Option(help='Changed since a specific tag.')]='', short: Annotated[bool, options.short]=False, bricks: Annotated[bool, Option(help='Print changed bricks.')]=False, deps: Annotated[bool, Option(help='Print bricks that depend on changes. Use with --bricks.')]=False):
...
@app.command('sync')
def sync_command(strict: Annotated[bool, options.strict]=False, quiet: Annotated[bool, options.quiet]=False, directory: Annotated[str, options.directory]='', verbose: Annotated[bool, options.verbose]=False):
...
@app.command('deps')
def deps_command(directory: Annotated[str, options.directory]='', brick: Annotated[str, options.brick]=''):
