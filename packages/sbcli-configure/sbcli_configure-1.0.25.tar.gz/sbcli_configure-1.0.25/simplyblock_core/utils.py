...
def calculate_minimum_sys_memory(max_prov, total):
    minimum_sys_memory = (250 * 1024 * 1024) * 1.1 * convert_size(max_prov, 'TiB') + (
            constants.EXTRA_SYS_MEMORY * total)
...
def get_nvme_pci_devices():
    try:
        ...
        pci_addresses = [line.split()[0] for line in output.strip().splitlines()]
...
def bind_to_nvme_driver(pci_address):
    ...
    try:
        with open(new_id_path, "w") as f:
            f.write(pci_address)
        logger.info(f"Bound {pci_address} to {NVME_DRIVER}.")
    except Exception as e:
        logger.error(f"Failed to bind {pci_address} to {NVME_DRIVER}: {e}")


def detect_nvmes():
    pci_addresses = get_nvme_pci_devices()
...
def generate_configs(max_lvol, max_prov, sockets_to_use, nodes_per_socket, ssd_pcie):
    system_info = {}
    nodes_config = {"nodes": []}
    ...
    nvmes = detect_nvmes()
    if ssd_pcie:
        ssd_pcie = ssd_pcie.split(',')
        # Normalize SSD PCI addresses and user PCI list
        ssd_pci_set = set(item["pci_address"] for item in nvmes.values())
        user_pci_set = set(
            addr if len(addr.split(":")[0]) == 4 else f"0000:{addr}"
            for addr in ssd_pcie
        )
        ...
        # Filter the original list to include only matching items
        filtered_ssd_pcis = {key: item for key, item in nvmes.items() if item["pci_address"] in user_pci_set}
        nvmes = filtered_ssd_pcis
