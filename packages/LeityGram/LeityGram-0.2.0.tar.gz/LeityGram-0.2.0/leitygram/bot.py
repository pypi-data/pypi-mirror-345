...
async def process_message(self, message_data: Dict[str, Any]):
    ...
    logger.debug(f"Registered handlers: {self.handlers}")
    ...
    if message.text and message.text.startswith('/'):
        command = message.text.split()[0].lower()  # Получаем команду
        command = command.split('@')[0]  # Удаляем имя бота если есть
    ...
    if message.text and message.text.startswith('/'):
        await message.reply("Команда не распознана. Введите /help для списка команд.")
