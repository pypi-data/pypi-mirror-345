...
def _extract_ros_data_logic(config: Dict[str, Any], subfolder_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        typestore = Stores.get(f'ros2_{ros_distro}')
        venv_logger.info(f"Using base typestore: ROS2_{ros_distro.upper()}")
    except KeyError:
        venv_logger.error(f"Unsupported ROS distro '{ros_distro}' for typestore.")
        return None
