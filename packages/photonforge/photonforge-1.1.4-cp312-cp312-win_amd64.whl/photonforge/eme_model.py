...
class _EMEModelRunner:
    def __init__(
        ...
        if self.mode_remap:
            ...
            for name in port_groups[0] + port_groups[1]:
                monitor = ports[name].to_tidy3d_monitor(simulation.freqs, name="M")
...
class EMEModel(Model):
    ...
    def __init__(
        ...
        subpixel: Union[tidy3d.SubpixelSpec, bool] = True,
    ...
    def __deepcopy__(self, memo: dict = None) -> "EMEModel":
        return EMEModel(
            self.eme_grid_spec,
            self.medium,
    ...
    def __eq__(self, other: Any) -> bool:
        return isinstance(other, EMEModel) and self.as_bytes == other.as_bytes
    ...
    def data_file_for(self, component: Component) -> Optional[pathlib.Path]:
        result = EMEModel._data_cache.get(component.as_bytes)
        if not result:
            raise RuntimeError(
    ...
    def get_simulation(
        ...
        zmax = -1e30
        for name in port_groups[0] + port_groups[1]:
            monitor = component_ports[name].to_tidy3d_monitor(frequencies, name="M")
    ...
    def start(
        ...
        **kwargs,
        ...
        if isinstance(result.runners[0], tidy3d_model._Tidy3DTaskRunner):
            EMEModel._data_cache[component.as_bytes] = result.runners[0].data_file()
    ...
    def as_bytes(self) -> bytes:
        ...
        indices = {"eme_grid_spec": (0, -1)}
        models = [tidy3d_model._tidy3d_to_bytes(self.eme_grid_spec)]

        if isinstance(self.grid_spec, tidy3d.GridSpec):
            indices["grid_spec"] = (len(models), -1)
            models.append(tidy3d_model._tidy3d_to_bytes(self.grid_spec))
        ...
        if isinstance(self.subpixel, tidy3d.SubpixelSpec):
            ...
            models.append(tidy3d_model._tidy3d_to_bytes(self.subpixel))
        elif self.subpixel is not None:
            obj["subpixel"] = self.subpixel

        if self.medium is not None:
            indices["medium"] = (len(models), -1)
            models.append(tidy3d_model._tidy3d_to_bytes(self.medium))

        if len(self.monitors) > 0:
            indices["monitors"] = (len(models), len(models) + len(self.monitors))
            models.extend(tidy3d_model._tidy3d_to_bytes(m) for m in self.monitors)

        if len(self.structures) > 0:
            indices["structures"] = (len(models), len(models) + len(self.structures))
            models.extend(tidy3d_model._tidy3d_to_bytes(s) for s in self.structures)
