...
def run_enhance(
    ...
    """
    Run enhancement as a function call rather than via command line.
    
    Args:
        noisy_audio_files: List of noisy audio files to enhance
        output_dir: Directory where to save enhanced files
        model_base_dir: Path to model directory (or model name for pretrained models)
        post_filter: Enable post-filtering
        compensate_delay: Add padding to compensate for STFT/ISTFT delay
        atten_lim: Attenuation limit in dB
        suffix: Add model suffix to output filenames
        noisy_dir: Alternative to noisy_audio_files - directory with noisy files
        log_level: Log verbosity level
        epoch: Which checkpoint to use
        no_df_stage: Skip the DF stage
    """
    # Create an args-like object to maintain compatibility with main()
    class Args:
        pass
    
    args = Args()
    args.noisy_audio_files = noisy_audio_files if isinstance(noisy_audio_files, list) else [noisy_audio_files]
    args.output_dir = output_dir
    args.model_base_dir = model_base_dir
    args.pf = post_filter
    args.compensate_delay = compensate_delay
    args.atten_lim = atten_lim
    args.suffix = suffix
    args.noisy_dir = noisy_dir
    args.log_level = log_level
    args.epoch = epoch
    args.no_df_stage = no_df_stage
    
    # Save current directory and change to model directory if needed
    original_dir = os.getcwd()
    if model_base_dir and os.path.isdir(model_base_dir):
        os.chdir(os.path.dirname(model_base_dir))
    
    try:
        # Run the original main function with our args
        main(args)
    finally:
