...
class fixify:
  ...
  def create_cli_file(self):
    import os
    cli_file_path = os.path.join(os.getcwd(), "fixify", "cli.py")
    
    # Check if the cli.py already exists
    if not os.path.exists(cli_file_path):
        with open(cli_file_path, "w") as file:
            file.write("""
import click
from fixify.core import fixify  # Assuming fixify class is in core.py
...
if __name__ == '__main__':
    cli()
            """)
        print(f"cli.py file created at {cli_file_path}")
    else:
        print(f"cli.py already exists at {cli_file_path}")
