...
class RuntimeEnvironment:
    ...
    def __init__(self, logger: Logger = None):
        ...
        self.pip_executable = os.path.join(self.venv_path, 'Scripts', 'pip') if os.name == 'nt' else os.path.join(self.venv_path, 'bin', 'pip')
    ...
    def install_package(self, package_name: str, version: str = None):
        ...
        Args:
            ...
            version: Optional specific version string (e.g., '==1.2.3', '>=1.0').
                     Note: uv might have slightly different specifier syntax nuances
                     compared to pip sometimes, but standard ones usually work.
        """
        # Check if uv is callable first (optional but good practice)
        try:
             subprocess.run(['uv', '--version'], check=True, capture_output=True)
             self.logger.info("uv command found.")
        except (subprocess.CalledProcessError, FileNotFoundError):
             self.logger.error("uv command not found on PATH. Cannot install packages dynamically this way.")
             return False
        ...
        if version:
            ...
            if not all(c.isalnum() or c in ['.', '=', '<', '>', '!', '~'] for c in version): # Added ~ for compatible releases
                self.logger.error(f"Error: Potentially invalid characters in version specifier for uv: {version}")
        ...
        try:
            ...
            self.logger.debug(f"Install STDERR:\n{process.stderr}") # uv might output progress here
            ...
            try:
                ...
                self.logger.info(f"Module '{module_name_import}' reloaded/available.")
                 ...
                 self.logger.warning(f"Could not import '{module_name_import}' immediately after install. May require script restart.")
            ...
            self.logger.error(f"Error installing {package_spec} using uv; command failed.")
            self.logger.error(f"Return Code: {e.returncode}")
            self.logger.error(f"STDOUT:\n{e.stdout}")
            self.logger.error(f"STDERR:\n{e.stderr}") # This should contain the uv error
            return False
        except Exception as e:
            self.logger.error(f"An unexpected error occurred during uv installation setup: {e}")
            return False
    ...
    def construct_environment(self, environment_info: Dict[str, Any]):
        ...
        if "pip" in environment_info:
            ...
            if isinstance(pip_info, list):
                for package in pip_info:
                    ...
                    elif isinstance(package, dict): # Tuple maybe ? 
        ...
        if "env_variables" in environment_info:
            ...
            if isinstance(env_vars, dict):
                ...
                self.logger.error("Environment variables information is not a dictionary.")
        # if working directory is specified download the data (TODO)
