...
def run_stream(cmd: list[str], outfile_name: str | None = None) -> int:
    """
    Run a subprocess, streaming every stdout/stderr line
    prefixed with “[download]”, and at the end emit
    “[download_complete] <outfile_name>” if provided.
...
def get_available_heights(vid: str, proxies: list[str]) -> list[int]:
    """
    Fetch available video heights, retrying with different proxies until success.
    """
    while True:
        for proxy in proxies:
            ...
            try:
                ...
                print(f"Proxy {proxy} failed: {e}. Trying next...")
                time.sleep(2)  # Wait 2 seconds before trying the next proxy
...
def parse_args():
    p = argparse.ArgumentParser(
        prog="ytp-dl",
        usage="ytp-dl -o OUTDIR -p PROXY [--download-proxy DOWNLOAD_PROXY] [--audio] URL [HEIGHT]",
    )
    p.add_argument(
        "-o",
        "--output-dir",
    ...
    p.add_argument(
        ...
        dest="proxy",
        required=True,
        help="Comma‑separated proxy URIs for metadata fetching",
    )
    p.add_argument(
        "-d",
        "--download-proxy",
        dest="dlproxy",
        default="",
        help="Optional single proxy URI used ONLY for aria2c downloads",
    )
    p.add_argument(
        "-a",
        "--audio",
...
async def fetch_pair(vid: str, height: str, proxies: list[str]) -> dict:
    """
    Race one fetch per proxy until one succeeds, with a delay between attempts.
    """
    ...
    def worker():
        ...
        vids = [
            f
            for f in fmts
            if isinstance(f.get("height"), int) and f["height"] <= int(height)
        ]
        if not vids:
            time.sleep(2)
            return schedule()
...
async def _amain():
    ...
    try:
        ...
        safe = (
            "".join(c for c in pair["title"] if c.isalnum() or c.isspace())
            .strip()[:150]
            or f"media_{vid}"
        )
        ...
        # Helper to translate proxy URI to aria2c flags
        def proxy_flags(uri: str) -> list[str]:
            if not uri:
                return []
            return ["--all-proxy="+uri] if not uri.startswith("socks") else ["--proxy="+uri]

        dl_flags = proxy_flags(args.dlproxy)
        ...
        if args.audio:
            ...
            if dl_flags:
                aria2c_cmd = dl_flags + aria2c_cmd
            run_stream(aria2c_cmd, outfile_name=f"{safe}.mp3")

            cover_args: list[str] = []
            thumb = pair.get("thumbnail")
            if thumb and head_ok(thumb):
                cover_path = TMP_DIR / "cover.jpg"
                resp = requests.get(thumb, timeout=TIMEOUT)
        ...
        if dl_flags:
            audio_cmd = dl_flags + audio_cmd

        await asyncio.gather(
            asyncio.to_thread(run_stream, video_cmd, vtmp.name),
            asyncio.to_thread(run_stream, audio_cmd, atmp.name),
        )

        final = outdir / f"{safe}.{ext}"
        run_stream(
            [str(ffmpeg), "-i", str(vtmp), "-i", str(atmp), "-c", "copy", "-y", str(final)],
