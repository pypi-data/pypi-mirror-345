...
from robot.libraries.BuiltIn import BuiltIn, keyword, RobotNotRunningError
    ...
    def __init__(self, options: dict):
        ...
        for result in results[1:]:
            ...
            if raw_result.type == TestResultType.Load:
                ...
                self.scenario_dict[raw_result.description].timeseries = timeseries_list
    ...
    def get_overall_status(self) -> str:
        ...
        for scenario_name, scenario in self.scenario_dict.items():
            if len(scenario_name.split(".")) == 2:
                overall_status.append(json.loads(scenario.get_overall_status()))
        return json.dumps(overall_status, indent=2, ensure_ascii=False)
