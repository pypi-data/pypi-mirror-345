...
class Squad:
    ...
    def deploy(self, guardrails: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        # Log guardrails for debugging
        if guardrails:
            logger.info(f"Squad deployment with guardrails: {list(guardrails.keys())}")
        ...
        for i, operation in enumerate(self.operations):
            ...
            # Apply guardrails to operation instructions if provided
            if guardrails:
                # Apply template variables from guardrails
                processed_instructions = operation.instructions
                for key, value in guardrails.items():
                    placeholder = "{" + key + "}"
                    if placeholder in processed_instructions:
                        processed_instructions = processed_instructions.replace(placeholder, str(value))
                        logger.debug(f"Applied guardrail '{key}' to operation {i+1}")

                # Handle select syntax: {variable, select, option1:value1|option2:value2|...}
                select_pattern = r'\{([^,]+),\s*select,\s*([^}]+)\}'
                select_matches = re.finditer(select_pattern, processed_instructions)

                for match in select_matches:
                    var_name = match.group(1).strip()
                    options_str = match.group(2).strip()

                    # Only process if the variable is in guardrails
                    if var_name in guardrails:
                        selected_value = str(guardrails[var_name])
                        replacement = ""
                        ...
                        for option in options:
                            if ':' in option:
                                ...
                                if option_key == selected_value:
                                    replacement = option_value.strip()
                                    break

                        if replacement:
                            processed_instructions = processed_instructions.replace(match.group(0), replacement)
                            logger.debug(f"Applied select guardrail '{var_name}' with value '{selected_value}' to operation {i+1}")

                # Update operation with processed instructions
                if processed_instructions != operation.instructions:
                    operation.instructions = processed_instructions
                    logger.info(f"Applied guardrails to operation {i+1}")
            ...
            # Apply guardrails to expert attributes if provided
            if guardrails and operation.expert:
                expert = operation.expert

                # Apply template variables to expert attributes
                for attr in ['specialty', 'objective', 'background']:
                    if hasattr(expert, attr):
                        original_value = getattr(expert, attr)
                        if original_value and isinstance(original_value, str):
                            ...
                            select_pattern = r'\{([^,]+),\s*select,\s*([^}]+)\}'
                            select_matches = re.finditer(select_pattern, processed_value)
                            ...
                            for match in select_matches:
                                ...
                                if var_name in guardrails:
                                    ...
                                    if replacement:
                                        processed_value = processed_value.replace(match.group(0), replacement)

                            # Update attribute if changed
                            if processed_value != original_value:
                                setattr(expert, attr, processed_value)
                                logger.debug(f"Applied guardrails to expert {attr}")
