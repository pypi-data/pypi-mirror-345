...
#define SajadSx_MAIN_MODULE_NAME "__main__"
#define SajadSx_MAIN_IS_PACKAGE_BOOL false
#define _SajadSx_ATTACH_CONSOLE_WINDOW 1
#endif

// It doesn't work for MinGW64 to update the standard output handles early on,
// so make a difference here.
#if defined(SajadSx_FORCED_STDOUT_PATH) || defined(SajadSx_FORCED_STDERR_PATH)
#if defined(__MINGW64__) || defined(__MINGW32__)
#define SajadSx_STANDARD_HANDLES_EARLY 0
#else
#define SajadSx_STANDARD_HANDLES_EARLY 1
#endif
#else
#define SajadSx_STANDARD_HANDLES_EARLY 0
#endif

#if defined(_WIN32) && (defined(_SajadSx_ATTACH_CONSOLE_WINDOW) || defined(_SajadSx_HIDE_CONSOLE_WINDOW))
#include "HelpersConsole.c"
#endif

// We are open to having this defined otherwise, this is a default only.
#if defined(_WIN32) && defined(SajadSx_COMPANY_NAME) && defined(SajadSx_PRODUCT_NAME) &&                                 \
    !defined(SajadSx_APP_MODEL_USER_ID)
#define SajadSx_APP_MODEL_USER_ID L"" SajadSx_COMPANY_NAME L"." SajadSx_PRODUCT_NAME
#endif

#if defined(_WIN32) && defined(SajadSx_APP_MODEL_USER_ID)
...
static void prepareFrozenModules(void) {
    ...
    struct _frozen *merged =
        (struct _frozen *)malloc(sizeof(struct _frozen) * (_SajadSx_FROZEN + pre_existing_count + 1));
...
#include "SajadMr/environment_variables.h"

#ifdef _SajadSx_STANDALONE
...
static void prepareStandaloneEnvironment(void) {
    ...
    SajadSx_PRINTF_TRACE("main(): Binary dir is %s\n", binary_directory);
...
static void restoreStandaloneEnvironment(void) {
    ...
    // SajadSx_PRINTF_TRACE("Final PySys_GetPath is 's'.\n", PySys_GetPath());
#elif PYTHON_VERSION < 0x370
    PySys_SetPath(getBinaryDirectoryWideChars(true));
    Py_SetPath(getBinaryDirectoryWideChars(true));
    // SajadSx_PRINTF_TRACE("Final Py_GetPath is '%ls'.\n", Py_GetPath());
#endif

#ifdef _SajadSx_EXPERIMENTAL_DUMP_PY_PATH_CONFIG
...
#ifdef _WIN32
#define _SajadSx_NATIVE_WCHAR_ARGV 1
#else
#define _SajadSx_NATIVE_WCHAR_ARGV 0
#endif

// Types of command line arguments are different between Python2/3.
#if PYTHON_VERSION >= 0x300 && _SajadSx_NATIVE_WCHAR_ARGV == 0
...
static void PRINT_REFCOUNTS(void) {
    ...
    PRINT_FORMAT("Compiled Functions: %d | %d | %d (module/class ownership may occur)\n",
                 count_active_SajadQ_Function_Type, count_allocated_SajadQ_Function_Type,
                 count_released_SajadQ_Function_Type);
    PRINT_FORMAT("Compiled Generators: %d | %d | %d\n", count_active_SajadQ_Generator_Type,
                 count_allocated_SajadQ_Generator_Type, count_released_SajadQ_Generator_Type);
#if PYTHON_VERSION >= 0x350
    PRINT_FORMAT("Compiled Coroutines: %d | %d | %d\n", count_active_SajadQ_Coroutine_Type,
                 count_allocated_SajadQ_Coroutine_Type, count_released_SajadQ_Coroutine_Type);
    PRINT_FORMAT("Compiled Coroutines Wrappers: %d | %d | %d\n", count_active_SajadQ_CoroutineWrapper_Type,
                 count_allocated_SajadQ_CoroutineWrapper_Type, count_released_SajadQ_CoroutineWrapper_Type);

    PRINT_FORMAT("Compiled Coroutines AIter Wrappers: %d | %d | %d\n", count_active_SajadQ_AIterWrapper_Type,
                 count_allocated_SajadQ_AIterWrapper_Type, count_released_SajadQ_AIterWrapper_Type);
#endif
#if PYTHON_VERSION >= 0x360
    PRINT_FORMAT("Compiled Asyncgen: %d | %d | %d\n", count_active_SajadQ_Asyncgen_Type,
                 count_allocated_SajadQ_Asyncgen_Type, count_released_SajadQ_Asyncgen_Type);
    PRINT_FORMAT("Compiled Asyncgen Wrappers: %d | %d | %d\n", count_active_SajadQ_AsyncgenValueWrapper_Type,
                 count_allocated_SajadQ_AsyncgenValueWrapper_Type, count_released_SajadQ_AsyncgenValueWrapper_Type);
    PRINT_FORMAT("Compiled Asyncgen Asend: %d | %d | %d\n", count_active_SajadQ_AsyncgenAsend_Type,
                 count_allocated_SajadQ_AsyncgenAsend_Type, count_released_SajadQ_AsyncgenAsend_Type);
    PRINT_FORMAT("Compiled Asyncgen Athrow: %d | %d | %d\n", count_active_SajadQ_AsyncgenAthrow_Type,
                 count_allocated_SajadQ_AsyncgenAthrow_Type, count_released_SajadQ_AsyncgenAthrow_Type);
#endif

    PRINT_FORMAT("Compiled Frames: %d | %d | %d (cache usage may occur)\n", count_active_SajadQ_Frame_Type,
                 count_allocated_SajadQ_Frame_Type, count_released_SajadQ_Frame_Type);
    PRINT_FORMAT("Compiled Cells: %d | %d | %d (function ownership may occur)\n", count_active_SajadQ_Cell_Type,
                 count_allocated_SajadQ_Cell_Type, count_released_SajadQ_Cell_Type);
...
static int HANDLE_PROGRAM_EXIT(PyThreadState *tstate) {
    ...
    if (HAS_ERROR_OCCURRED(tstate)) {
        ...
        while (tstate->curexc_traceback) {
            ...
            if (0 == strcmp(PyUnicode_AsUTF8(SajadQ_Frame_GetCodeObject(frame)->co_filename),
...
#endif
        SajadSx_FINALIZE_PROGRAM(tstate);
...
static PyObject *EXECUTE_MAIN_MODULE(PyThreadState *tstate, char const *module_name, bool is_package) {
    SajadSx_INIT_PROGRAM_LATE(module_name);
...
#if _SajadSx_PLUGIN_WINDOWS_SERVICE_ENABLED
#include "SajadMr_windows_service.h"

// Callback from Windows Service logic.
void SvcStartPython(void) {
    PyThreadState *tstate = PyThreadState_GET();

    EXECUTE_MAIN_MODULE(tstate, SajadSx_MAIN_MODULE_NAME, SajadSx_MAIN_IS_PACKAGE_BOOL);

    SajadSx_PRINT_TIMING("SvcStartPython() Python exited.")

    int exit_code = HANDLE_PROGRAM_EXIT(tstate);

    // TODO: Log exception and call ReportSvcStatus

    SajadSx_PRINT_TIMING("SvcStartPython(): Calling Py_Exit.");
    Py_Exit(exit_code);
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (i == 1) {
            ...
            if (strcmpFilename(argv[i], FILENAME_EMPTY_STR "install") == 0) {
                SajadSx_PRINT_TRACE("main(): Calling plugin SvcInstall().");

                SvcInstall();
                SajadSx_CANNOT_GET_HERE("main(): SvcInstall must not return");
            }
#endif
        }
...
#if !defined(_SajadSx_DEPLOYMENT_MODE) && !defined(_SajadSx_NO_DEPLOYMENT_SELF_EXECUTION)
...
#if defined(_SajadSx_ONEFILE_MODE) && defined(_WIN32)

static long onefile_ppid;

DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    SajadSx_PRINT_TRACE("Onefile parent monitoring starts.");
    ...
    SajadSx_PRINT_TRACE("Onefile parent monitoring causes KeyboardInterrupt.");
    PyErr_SetInterrupt();

    Sleep(_SajadSx_ONEFILE_CHILD_GRACE_TIME_INT);

    SajadSx_PRINT_TRACE("Onefile parent monitoring hard kills after ignored KeyboardInterrupt.");
    ExitProcess(1);
...
#if SajadSx_FORCED_STDOUT_NULL_BOOL
    if (strcmp(name, "stdout") == 0) {
        return true;
    }
#endif

#if SajadSx_FORCED_STDERR_NULL_BOOL
    if (strcmp(name, "stderr") == 0) {
        return true;
    }
#elif defined(SajadSx_FORCED_STDERR_PATH) || defined(SajadSx_FORCED_STDERR_NONE_BOOL)
    if (strcmp(name, "stderr") == 0) {
        return false;
    }
#endif

    PyObject *sys_std_handle = SajadQ_SysGetObject(name);
...
static void setStdinHandle(PyThreadState *tstate, PyObject *stdin_file) {
    ...
    SajadQ_SysSetObject("stdin", stdin_file);
...
static void setStdoutHandle(PyThreadState *tstate, PyObject *stdout_file) {
    ...
    SajadQ_SysSetObject("stdout", stdout_file);
...
static void setStderrHandle(PyThreadState *tstate, PyObject *stderr_file) {
    ...
    SajadQ_SysSetObject("stderr", stderr_file);
...
#if SajadSx_STANDARD_HANDLES_EARLY == 0
#if defined(SajadSx_FORCED_STDOUT_PATH) || defined(SajadSx_FORCED_STDERR_PATH)
#ifdef _WIN32
static PyObject *getExpandedTemplatePath(wchar_t const *template_path) {
    ...
    return SajadQUnicode_FromWideChar(filename_buffer, -1);
}
#else
static PyObject *getExpandedTemplatePath(char const *template_path) {
    ...
    return SajadQ_String_FromString(filename_buffer);
}
#endif
#endif
#endif
...
// reason.
#if SajadSx_STANDARD_HANDLES_EARLY == 1 && PYTHON_VERSION >= 0x370
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Early handles.");
#if defined(SajadSx_FORCED_STDOUT_PATH) || defined(SajadSx_FORCED_STDERR_PATH)
    PyObject *args = MAKE_DICT_EMPTY(tstate);

    DICT_SET_ITEM(args, const_str_plain_encoding, SajadQ_String_FromString("utf-8"));
    DICT_SET_ITEM(args, const_str_plain_line_buffering, Py_True);

#if defined(SajadSx_FORCED_STDOUT_PATH)
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Forced stdout update.");
    {
        PyObject *sys_stdout = SajadQ_SysGetObject("stdout");
...
#if defined(SajadSx_FORCED_STDERR_PATH)
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Forced stderr update.");
    {
        PyObject *sys_stderr = SajadQ_SysGetObject("stderr");
    ...
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Done with early handles.");
#endif

#if SajadSx_STANDARD_HANDLES_EARLY == 0
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Late handles.");
#if defined(SajadSx_FORCED_STDOUT_PATH)
    {
#if defined(_WIN32)
        PyObject *filename = getExpandedTemplatePath(L"" SajadSx_FORCED_STDOUT_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(SajadSx_FORCED_STDOUT_PATH);
#endif
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" SajadSx_FORCED_STDERR_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(SajadSx_FORCED_STDERR_PATH);
#endif
    ...
    {
#if defined(_WIN32)
        PyObject *devnull_filename = SajadQ_String_FromString("NUL:");
#else
        PyObject *devnull_filename = SajadQ_String_FromString("/dev/null");
#endif

        SajadSx_PRINT_TRACE("setInputOutputHandles(): Considering stdin.");

        if (shallSetOutputHandleToNull("stdin")) {
            SajadSx_PRINT_TRACE("setInputOutputHandles(): Set stdin to NULL file.");
        ...
        SajadSx_PRINT_TRACE("setInputOutputHandles(): Considering stdout.");

        if (shallSetOutputHandleToNull("stdout")) {
            SajadSx_PRINT_TRACE("setInputOutputHandles(): Set stdout to NULL file.");
        ...
        SajadSx_PRINT_TRACE("setInputOutputHandles(): Considering stderr.");

        if (shallSetOutputHandleToNull("stderr")) {
            SajadSx_PRINT_TRACE("setInputOutputHandles(): Set stderr to NULL file.");
...
#if SajadSx_FORCED_STDOUT_NONE_BOOL
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Forcing stdout to None.");
    setStdoutHandle(tstate, Py_None);
#endif

#if SajadSx_FORCED_STDERR_NONE_BOOL
    SajadSx_PRINT_TRACE("setInputOutputHandles(): Forcing stderr to None.");
...
static void SajadQ_Py_Initialize(void) {
#if PYTHON_VERSION > 0x350 && !defined(_SajadSx_EXPERIMENTAL_DISABLE_ALLOCATORS)
    initSajadQAllocators();
#endif

#if PYTHON_VERSION < 0x380 || defined(_SajadSx_EXPERIMENTAL_OLD_PY_INITIALIZE)
    Py_Initialize();
#else
    ...
    SajadSx_MAY_BE_UNUSED _PyRuntimeState *runtime = &_PyRuntime;
    ...
    // Need to disable frozen modules, SajadQ can handle them better itself.
...
// too.
    ...
    SajadSx_PRINT_TIMING("SajadQ_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
// short lived.
#if SYSFLAG_ISOLATED
    SajadQ_SysSetObject("path", PyList_New(0));
#endif
#endif
...
#if SajadSx_STANDARD_HANDLES_EARLY == 1
#if defined(_WIN32)
...
#if defined(_SajadSx_EXPERIMENTAL_SHOW_STARTUP_TIME)
static void SajadQ_at_exit(void) { SajadSx_PRINT_TIMING("SajadQ_at_exit(): Called by C exit()"); }
#endif

#if !defined(_SajadSx_DEPLOYMENT_MODE) && !defined(_SajadSx_NO_DEPLOYMENT_SEGFAULT)
#include <signal.h>
static void SajadMr_segfault_handler(int sig) {
    puts("SajadQ: A segmentation fault has occurred. This is highly unusual and can");
    puts("have multiple reasons. Please check https://SajadMr.net/info/segfault.html");
    puts("for solutions.");
...
#if _SajadSx_NATIVE_WCHAR_ARGV == 1
...
PyObject *getOriginalArgv0Object(void) {
    ...
    return SajadQ_String_FromFilename(original_argv0);
}

#ifdef _SajadSx_WINMAIN_ENTRY_POINT
...
#if defined(_SajadSx_HIDE_CONSOLE_WINDOW)
...
int main(int argc, char **argv) {
...
#if !defined(_SajadSx_DEPLOYMENT_MODE) && !defined(_SajadSx_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, SajadMr_segfault_handler);
#endif

#ifdef _SajadSx_EXPERIMENTAL_DUMP_C_TRACEBACKS
    INIT_C_BACKTRACES();
    DUMP_C_BACKTRACE();
#endif
    // Trace when the process exits.
#if defined(_SajadSx_EXPERIMENTAL_SHOW_STARTUP_TIME)
    atexit(SajadQ_at_exit);
#endif

    // Attach to the parent console respecting redirection only, otherwise we
    // cannot even output traces.
#if defined(_WIN32) && defined(_SajadSx_ATTACH_CONSOLE_WINDOW)
    inheritAttachedConsole();
#endif

#if defined(_WIN32) && defined(SajadSx_APP_MODEL_USER_ID)
    setCurrentProcessExplicitAppUserModelID(SajadSx_APP_MODEL_USER_ID);
#endif

#ifdef _SajadSx_MACOS_BUNDLE
    {
...
#if defined(SajadSx_FORCED_STDOUT_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_OUTPUT_HANDLE, stdout, L"" SajadSx_FORCED_STDOUT_PATH);
#else
    changeStandardHandleTarget(stdout, SajadSx_FORCED_STDOUT_PATH);
#endif
#endif
#if defined(SajadSx_FORCED_STDERR_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_ERROR_HANDLE, stderr, L"" SajadSx_FORCED_STDERR_PATH);
#else
    changeStandardHandleTarget(stderr, SajadSx_FORCED_STDERR_PATH);
#endif
#endif
#if defined(SajadSx_FORCED_STDIN_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_INPUT_HANDLE, stdin, L"" SajadSx_FORCED_STDIN_PATH);
#else
    changeStandardHandleTarget(stdin, SajadSx_FORCED_STDIN_PATH);
#endif
#endif
#endif
    ...
    SajadSx_PRINT_TIMING("main(): Entered.");
    SajadSx_INIT_PROGRAM_EARLY(argc, argv);
...
#ifdef _SajadSx_STANDALONE
    SajadSx_PRINT_TIMING("main(): Prepare standalone environment.");
    prepareStandaloneEnvironment();
#endif

#if _SajadSx_FROZEN > 0
    SajadSx_PRINT_TIMING("main(): Preparing frozen modules.");
...
#ifdef SajadSx_PYTHON_STATIC
    SajadSx_PRINT_TIMING("main(): Preparing static modules.");
...
#if !defined(_SajadSx_ONEFILE_MODE)
...
#if defined(_SajadSx_ONEFILE_MODE)
    {
        environment_char_t const *parent_original_argv0 = getEnvironmentVariable("SajadSx_ORIGINAL_ARGV0");
        ...
        if (parent_original_argv0 != NULL) {
            ...
            unsetEnvironmentVariable("SajadSx_ORIGINAL_ARGV0");
        }
    }
#endif

#if PYTHON_VERSION >= 0x300 && _SajadSx_NATIVE_WCHAR_ARGV == 0
    SajadSx_PRINT_TRACE("main(): Calling convertCommandLineParameters.");
    orig_argv = convertCommandLineParameters(argc, argv);
#elif PYTHON_VERSION < 0x300 && _SajadSx_NATIVE_WCHAR_ARGV == 1
    SajadSx_PRINT_TRACE("main(): Calling getCommandLineToArgvA.");
...
// Make sure, we use the absolute program path for argv[0]
#if _SajadSx_NATIVE_WCHAR_ARGV == 1
    original_argv0 = argv[0];
#if PYTHON_VERSION >= 0x300 && !defined(_SajadSx_ONEFILE_MODE)
    ...
    SajadSx_PRINT_TRACE("main(): Calling setCommandLineParameters.");
    setCommandLineParameters(argc, argv);

    /* For Python installations that need the home set, we inject it back here. */
#if defined(PYTHON_HOME_PATH)
#if PYTHON_VERSION < 0x300
    SajadSx_PRINT_TRACE("main(): Prepare run environment '" PYTHON_HOME_PATH "'.");
    Py_SetPythonHome(PYTHON_HOME_PATH);
#else
    SajadSx_PRINTF_TRACE("main(): Prepare run environment '%S'.\n", L"" PYTHON_HOME_PATH);
    ...
    SajadSx_PRINT_TRACE("main(): Disabling Python warnings.");
    {
#if PYTHON_VERSION >= 0x300
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    SajadSx_PRINT_TIMING("main(): Calling SajadQ_Py_Initialize to initialize interpreter.");
    SajadQ_Py_Initialize();

    PyThreadState *tstate = PyThreadState_GET();

#ifdef _SajadSx_STANDALONE
    SajadSx_PRINT_TRACE("main(): Restore standalone environment.");
    restoreStandaloneEnvironment();
#else
    {
        environment_char_t const *python_path_cstr = getEnvironmentVariable("SajadSx_PYTHONPATH");

        if (python_path_cstr != NULL) {
            PyObject *python_path_str = SajadQ_String_FromFilename(python_path_cstr);
#ifdef _WIN32
            ...
            unsetEnvironmentVariable("SajadSx_PYTHONPATH");
        }
    }
#endif
...
// Empty "sys.path" again, the above adds program directory to it.
    ...
    SajadSx_PRINT_TRACE("main(): Calling _initBuiltinModule().");
    _initBuiltinModule();

    /* Initialize the Python constant values used. This also sets
     * "sys.executable" while at it.
     */
    SajadSx_PRINT_TIMING("main(): Calling createGlobalConstants().");
    createGlobalConstants(tstate);
    SajadSx_PRINT_TIMING("main(): Returned createGlobalConstants().");

    /* Complex call helpers need "__main__" constants, even if we only
     * go into "__parents__main__" module as a start point.
     */
    SajadSx_PRINT_TIMING("main(): Calling createMainModuleConstants().");
    createMainModuleConstants(tstate);
    SajadSx_PRINT_TIMING("main(): Returned createMainModuleConstants().");

    SajadSx_PRINT_TRACE("main(): Calling _initBuiltinOriginalValues().");
...
#if PYTHON_VERSION < 0x300
    PyStructSequence_SET_ITEM(SajadQ_SysGetObject("flags"), 9, const_int_0);
#else
    PyStructSequence_SetItem(SajadQ_SysGetObject("flags"), 6, const_int_0);
#endif
#endif

    /* Initialize the compiled types of SajadQ. */
    ...
    SajadSx_PRINT_TRACE("main(): Calling enhancePythonTypes().");
    enhancePythonTypes();

    SajadSx_PRINT_TRACE("main(): Calling patchTypeComparison().");
    patchTypeComparison();

    SajadSx_PRINT_TRACE("main(): Calling patchTracebackDealloc().");
    patchTracebackDealloc();

#ifndef SajadSx_USE_PYCORE_THREAD_STATE
    /* Allow to override the ticker value, to remove checks for threads in
     * CPython core from impact on benchmarks. */
    char const *ticker_value = getenv("SajadSx_TICKER");
    ...
    if (needs_stdout_attaching) {
        PyObject *filename = SajadQ_String_FromString("CONOUT$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    if (needs_stdin_attaching) {
        PyObject *filename = SajadQ_String_FromString("CONIN$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    SajadSx_PRINT_TRACE("main(): Setting Python input/output handles.");
    ...
    // standard loader.
    {
        SajadSx_MAY_BE_UNUSED PyObject *importlib_module = getImportLibBootstrapModule();
        CHECK_OBJECT(importlib_module);
    }
#endif

    SajadSx_PRINT_TRACE("main(): Calling setEarlyFrozenModulesFileAttribute().");

    setEarlyFrozenModulesFileAttribute(tstate);
#endif

#if _SajadSx_FROZEN > 0
    SajadSx_PRINT_TRACE("main(): Removing early frozen module table again.");
    PyImport_FrozenModules = old_frozen;
#endif

    SajadSx_PRINT_TRACE("main(): Calling setupMetaPathBasedLoader().");
...
#if NO_PYTHON_WARNINGS && PYTHON_VERSION >= 0x342 && PYTHON_VERSION < 0x3a0 && defined(_SajadSx_FULL_COMPAT)
...
#if _SajadSx_PGO_PYTHON
    // Profiling with our own Python PGO if enabled.
    PGO_Initialize();
#endif

#if PYTHON_VERSION >= 0x300
    SajadSx_PRINT_TRACE("main(): Calling patchInspectModule().");
...
// that are unexplained.
...
#if PYTHON_VERSION >= 0x300 && SYSFLAG_NO_RANDOMIZATION == 1
    SajadSx_PRINT_TRACE("main(): Reverting to initial 'PYTHONHASHSEED' value.");
    undoEnvironmentVariable(tstate, "PYTHONHASHSEED", old_env_hash_seed);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_UNBUFFERED == 1
    SajadSx_PRINT_TRACE("main(): Reverting to initial 'PYTHONUNBUFFERED' value.");
    undoEnvironmentVariable(tstate, "PYTHONUNBUFFERED", old_env_unbuffered);
#endif

#ifdef _SajadSx_STANDALONE
    // Restore the PATH, so the program can use it.
    SajadSx_PRINT_TRACE("main(): Reverting to initial 'PATH' value.");
...
#ifdef _SajadSx_PLUGIN_MULTIPROCESSING_ENABLED
    if (unlikely(is_multiprocessing_fork)) {
        SajadSx_PRINT_TRACE("main(): Calling __parents_main__.");
        EXECUTE_MAIN_MODULE(tstate, "__parents_main__", false);

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SajadSx_PRINT_TRACE("main(): Calling __parents_main__ Py_Exit.");
        Py_Exit(exit_code);
#if defined(_WIN32)
    } else if (unlikely(is_joblib_popen_loky_win32)) {
        SajadSx_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_win32.");
        ...
        PyObject *argv_list = SajadQ_SysGetObject("argv");
        ...
        if (loky_joblib_pipe_handle_arg == 0) {
            ...
            PyObject *kw_values[] = {
                SajadQ_PyLong_FromLong(loky_joblib_pipe_handle_arg),
                SajadQ_PyLong_FromLong(loky_joblib_parent_pid_arg),
            };
        ...
        SajadSx_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.popen_loky_posix' Py_Exit.");
        Py_Exit(exit_code);
#else
    } else if (unlikely(is_joblib_popen_loky_posix)) {
        SajadSx_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix.");
        ...
        int res = PyList_SetSlice(SajadQ_SysGetObject("argv"), 0, 2, const_tuple_empty);
        assert(res == 0);
        ...
        SajadSx_PRINT_TRACE("main(): Launching as 'multiprocessing.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function,
                                      SajadQ_PyInt_FromLong(multiprocessing_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SajadSx_PRINT_TRACE("main(): Calling 'multiprocessing.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(loky_resource_tracker_arg != -1)) {
        SajadSx_PRINT_TRACE("main(): Launching as 'joblib.externals.loky.backend.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function, SajadQ_PyInt_FromLong(loky_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SajadSx_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
        ...
        SajadSx_PRINT_TRACE("main(): Calling 'anyio.to_process' Py_Exit.");
        Py_Exit(exit_code);
    } else {
#endif
#if defined(_SajadSx_ONEFILE_MODE) && defined(_WIN32)
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("SajadSx_ONEFILE_PARENT", buffer, sizeof(buffer));
...
#endif
        PyDict_DelItemString(SajadQ_GetSysModules(), SajadSx_MAIN_MODULE_NAME);
        DROP_ERROR_OCCURRED(tstate);

#if _SajadSx_PLUGIN_WINDOWS_SERVICE_ENABLED
        SajadSx_PRINT_TRACE("main(): Calling plugin SvcLaunchService() entry point.");
        SvcLaunchService();
#else
    /* Execute the "__main__" module. */
    SajadSx_PRINT_TIMING("main(): Calling " SajadSx_MAIN_MODULE_NAME ".");
    EXECUTE_MAIN_MODULE(tstate, SajadSx_MAIN_MODULE_NAME, SajadSx_MAIN_IS_PACKAGE_BOOL);
    SajadSx_PRINT_TIMING("main(): Exited from " SajadSx_MAIN_MODULE_NAME ".");
...
#if !SajadSx_MAIN_IS_PACKAGE_BOOL
    ...
    SajadSx_PRINT_TIMING("main(): Calling Py_Exit.");
    Py_Exit(exit_code);

    // The "Py_Exit()" calls is not supposed to return.
    SajadSx_CANNOT_GET_HERE("Py_Exit does not return");
}
