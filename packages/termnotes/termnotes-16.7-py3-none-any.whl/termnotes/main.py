...
def read_note(folder, name):
  ...
  for i, line in enumerate(lines):
      ...
      # --- Markdown Styling ---
          ...
          modified_lines.append(modified_line)
          words.extend(clean_line.lstrip('- ').strip().split())
      elif url_pattern.search(clean_line):  # Check if the line contains a URL
          modified_line = url_pattern.sub(lambda match: f"[link={match.group(1)}]{match.group(1)}[/link]", clean_line)
          modified_lines.append(modified_line)
