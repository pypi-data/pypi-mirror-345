...
from .const import BASE_APIS, logger
from .db import db
from .dependencies import set_global_user
from .generic.api import GenericApi
...
class FastAPIReactToolkit:
    ...
    app: FastAPI = None
    apis: list[ModelRestApi | GenericApi] = None
    ...
    async def _associate_permission_with_api(self, session: AsyncSession | Session):
        for api in self.apis:
            ...
            for permission in new_permissions:
                ...
                await safe_call(session.execute(permission_api_stmt))
    ...
    def _mount_template_folder(self):
        ...
        def index(request: Request):
            try:
                base_path = (Setting.BASE_PATH or request.scope["root_path"]).rstrip(
                    "/"
                ) + "/"
    ...
    def _init_lifespan(self):
        ...
        async def lifespan(app: FastAPI):
            ...
            # Run save_data on all generic sessions
            for api in self.apis:
                if isinstance(api, GenericApi):
                    api.datamodel.session().save_data()
