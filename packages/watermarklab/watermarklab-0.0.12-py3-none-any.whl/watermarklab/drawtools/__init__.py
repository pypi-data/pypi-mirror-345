    ...
    for result in combined_results:
        ...
        for metric in metrics:
            if metric in visualqualityresult:
                ...
                mean = np.mean(data)
                std = np.std(data)
                row_data.append(f"{mean:.4f} ± {std:.4f}")  # Format as mean ± std
            else:
...
def draw_visual_compare(load_dir: List[str], save_dir: str, img_index: int = 1, iter_index: int = 1,
                        noise_factor_order: List[int] = None,
                        no_noise: bool = False, x_align: float = -0.02, y_align: float = 0.02,
                        secret_scale: float = 0.3,
                        label_fontsize: int = 20):
    img_index = max(1, img_index)
    iter_index = max(1, iter_index)

    num_methods = len(load_dir)
    num_columns = 3

    noise_params_list = []
    if not no_noise:
        for method_path in load_dir:
            noise_params_path = os.path.join(method_path, 'noise', 'noise_params.json')
            with open(noise_params_path, 'r') as f:
                noise_params_list.append(json.load(f))
        num_columns += len(noise_params_list[0])

    fig = plt.figure(figsize=((num_columns + 1) * 3, num_methods * 3))  # 加1列显示方法名
    gs = gridspec.GridSpec(num_methods, num_columns + 1, width_ratios=[0.1] + [1] * num_columns, wspace=x_align,
                           hspace=y_align)

    column_titles = ["Cover", "Stego", "Residual"]
    if not no_noise:
        for noise_param in noise_params_list[0]:
            noisename = noise_param['noisename']
            factorsymbol = noise_param['factorsymbol']
            noisesymbol = noise_param.get('noisesymbol', '')
            column_titles.append(f"{noisename} ${noisesymbol}={factorsymbol}$")

    for method_idx, method_path in enumerate(load_dir):
        method_name = os.path.basename(method_path)

        cover_path = os.path.join(method_path, f"images/image_{img_index}/iter_{iter_index}/cover.png")
        stego_path = os.path.join(method_path, f"images/image_{img_index}/iter_{iter_index}/stego.png")
        residual_path = os.path.join(method_path, f"images/image_{img_index}/iter_{iter_index}/residual.png")
        secret_path = os.path.join(method_path, f"images/image_{img_index}/iter_{iter_index}/secret.bmp")

        cover_img = np.float32(Image.open(cover_path))
        stego_img = np.float32(Image.open(stego_path))
        residual_img = np.float32(Image.open(residual_path))

        stego_h, stego_w = stego_img.shape[:2]
        secret_resized = np.float32(
            Image.open(secret_path).resize((int(stego_w * secret_scale), int(stego_h * secret_scale)),
                                           resample=Image.NEAREST))

        # 方法名列（竖着显示）
        ax_name = fig.add_subplot(gs[method_idx, 0])
        ax_name.text(0.5, 0.5, method_name, rotation=90, va='center', ha='center', fontsize=label_fontsize)
        ax_name.axis('off')

        # Cover
        ax = fig.add_subplot(gs[method_idx, 1])
        ax.imshow(np.uint8(cover_img))
        ax.axis('off')
        if method_idx == 0:
            ax.set_title(column_titles[0], fontsize=label_fontsize)

        # Stego + Secret
        ax = fig.add_subplot(gs[method_idx, 2])
        stego_with_secret = stego_img.copy()
        stego_with_secret[-secret_resized.shape[0]:, -secret_resized.shape[1]:] = secret_resized
        ax.imshow(np.uint8(stego_with_secret))
        ax.axis('off')
        if method_idx == 0:
            ax.set_title(column_titles[1], fontsize=label_fontsize)

        # Residual
        ax = fig.add_subplot(gs[method_idx, 3])
        ax.imshow(np.uint8(residual_img))
        ax.axis('off')
        if method_idx == 0:
            ax.set_title(column_titles[2], fontsize=label_fontsize)

        # Noise 图
        if not no_noise:
            noise_params = noise_params_list[method_idx]
            if noise_factor_order is None:
                noise_factor_order = [-1] * len(noise_params)
            for noise_col_idx, noise_param in enumerate(noise_params):
                noisename = noise_param['noisename']
                factorsymbol = noise_param['factorsymbol'].replace("$", "")
                factors = sorted(noise_param['factors'])
                chosen_factor = factors[noise_factor_order[noise_col_idx]]

                noise_path = os.path.join(method_path,
                                          f"noise/{noisename}/{chosen_factor}/image_{img_index}/iter_{iter_index}/noised.png")
                ext_secret_path = os.path.join(method_path,
                                               f"noise/{noisename}/{chosen_factor}/image_{img_index}/iter_{iter_index}/ext_secret.bmp")

                noise_img = np.float32(Image.open(noise_path))
                ext_secret_resized = np.float32(
                    Image.open(ext_secret_path).resize((int(stego_w * secret_scale), int(stego_h * secret_scale)),
                                                       resample=Image.NEAREST))

                ax = fig.add_subplot(gs[method_idx, 4 + noise_col_idx])
                noise_with_secret = noise_img.copy()
                noise_with_secret[-ext_secret_resized.shape[0]:, -ext_secret_resized.shape[1]:] = ext_secret_resized

                ax.imshow(np.uint8(noise_with_secret))
                ax.axis('off')
                if method_idx == 0:
                    ax.set_title(f"{noisename}\n${factorsymbol}={chosen_factor}$", fontsize=label_fontsize)

    plt.tight_layout(pad=0.5)

    pdf_path = os.path.join(save_dir, f"visual_comparison_image_{img_index}_iter_{iter_index}.pdf")
    plt.savefig(pdf_path, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved visual comparison to {pdf_path}")
