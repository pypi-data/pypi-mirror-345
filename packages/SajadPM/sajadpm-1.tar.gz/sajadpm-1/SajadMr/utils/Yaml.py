...
from SajadMr.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get SajadQ package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.SajadMr-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "SajadMr.plugins.standard",
                "stdlib2.SajadMr-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "SajadMr.plugins.standard",
                "stdlib3.SajadMr-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "SajadMr.plugins.commercial", "commercial.SajadMr-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for SajadQ package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "SajadMr-package-config-schema.json",
    )
