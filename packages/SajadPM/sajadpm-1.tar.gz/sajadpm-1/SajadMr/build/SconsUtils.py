...
from SajadMr.__past__ import basestring, unicode
from SajadMr.containers.OrderedDicts import OrderedDict
from SajadMr.Progress import enableThreading
from SajadMr.PythonFlavors import isTermuxPython
...
from SajadMr.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def _enableFlagSettings(env, name, experimental_flags):
    for flag_name in experimental_flags:
        ...
        if value:
            env.Append(CPPDEFINES=[("_SajadSx_%s" % flag_name, value)])
        else:
            env.Append(CPPDEFINES=["_SajadSx_%s" % flag_name])
...
def createEnvironment(
    ...
    env.SajadMr_python = getArgumentBool("SajadMr_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.SajadMr_src, "static_src", sub_path)
...
def createDefinitionsFile(source_dir, filename, definitions):
    for env_name in os.environ["_SajadSx_BUILD_DEFINITIONS_CATALOG"].split(","):
    ...
    with openTextFile(build_definitions_filename, "w", encoding="utf8") as f:
        for key, value in sorted(definitions.items()):
            if key == "_SajadSx_BUILD_DEFINITIONS_CATALOG":
                continue
    ...
    if not _linker_arch_determined:
        if os.name == "nt":
            ...
            _linker_arch = _getBinaryArch(
                binary=os.environ["SajadSx_PYTHON_EXE_PATH"], mingw_mode=mingw_mode
            )
...
b) Using "--mingw64" lets SajadQ download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_SajadMr_temp.pyd" if env.module_mode else "_SajadMr_temp.exe",
        )
