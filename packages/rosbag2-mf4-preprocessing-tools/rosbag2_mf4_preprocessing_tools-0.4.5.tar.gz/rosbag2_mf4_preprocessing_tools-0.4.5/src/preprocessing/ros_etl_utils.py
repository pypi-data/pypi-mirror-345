...
def _get_flattened_fields(msg_type_name: str, typestore) -> List[str]:
    ...
    def get_fields_recursive(current_type_name: str, prefix: str = ''):
        if current_type_name in processed_types:
            flat_logger.warning(f"Detected recursion for type '{current_type_name}', stopping expansion at prefix '{prefix}'.")
            if prefix:
                 flat_field_names.append(prefix.rstrip('.') + "__recursive")
            return

        processed_types.add(current_type_name)

        try:
            if current_type_name not in typestore.types:
                 flat_logger.error(f"Type '{current_type_name}' not found in typestore.types dictionary.")
                 if prefix:
                     flat_field_names.append(prefix.rstrip('.') + "__unknown_type")
                 processed_types.remove(current_type_name) 
                 return

            type_entry = typestore.types[current_type_name]
            if isinstance(type_entry, (tuple, list)) and len(type_entry) == 2:
                _consts, fields_list = type_entry
            else:
                flat_logger.warning(f"Unexpected structure for type '{current_type_name}' in typestore.types: {type(type_entry)}. Cannot extract fields.")
                if prefix:
                     flat_field_names.append(prefix.rstrip('.') + "__type_structure_error")
                processed_types.remove(current_type_name) 
                return

            for field_name, field_type_info in fields_list:
                full_name = f"{prefix}{field_name}"
                base_type_name = ''
                is_array = False

                if isinstance(field_type_info, tuple):
                    if isinstance(field_type_info[0], tuple): # Nested tuple like (('typename', ...), size)
                        base_type_name = field_type_info[0][0]
                    elif isinstance(field_type_info[0], str): # Simple tuple like ('typename', size)
                         base_type_name = field_type_info[0]
                    else:
                         flat_logger.warning(f"Unexpected field_type_info[0] format for {full_name}: {field_type_info[0]}")
                         flat_field_names.append(full_name + "__error_type_format")
                         continue
                    is_array = True
                elif isinstance(field_type_info, str):
                    base_type_name = field_type_info
                else:
                     flat_logger.warning(f"Unexpected field_type_info format for {full_name}: {field_type_info}")
                     flat_field_names.append(full_name + "__error_type_format")
                     continue

                is_complex = base_type_name in typestore.types

                if is_complex:
                    field_suffix = "__array_complex" if is_array else "__complex"
                    flat_field_names.append(full_name + field_suffix)
                else:
                    field_suffix = "__array" if is_array else ""
                    flat_field_names.append(full_name + field_suffix)

        except KeyError:
            flat_logger.error(f"KeyError accessing fields for type '{current_type_name}' in typestore.")
            if prefix:
                 flat_field_names.append(prefix.rstrip('.') + "__key_error")
        except Exception as e:
            flat_logger.error(f"Error getting fields for type '{current_type_name}': {e}", exc_info=True)
            if prefix:
                 flat_field_names.append(prefix.rstrip('.') + "__exception")
        finally:
            if current_type_name in processed_types:
                processed_types.remove(current_type_name)

    get_fields_recursive(msg_type_name)
    flat_field_names.extend(['timestamp_s', 'timestamp_ns'])
    flat_logger.debug(f"Flattened fields for {msg_type_name}: {flat_field_names}")
