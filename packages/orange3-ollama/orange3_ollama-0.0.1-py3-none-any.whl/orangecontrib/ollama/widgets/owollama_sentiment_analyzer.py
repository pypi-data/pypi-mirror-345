...
class SentimentWorker(QThread):
    ...
    def run(self):
        ...
        for i, text in enumerate(self.corpus.get_column(self.column)):
            ...
            prompt = f"{self.context}\n\nDetermine the sentiment of the following text as Positive, Negative, or Neutral. Respond with one word only.\n\n{text}"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    sentiments.append("[Error in response]")
        ...
        self.corpus = self.corpus.add_column(StringVariable(name="Sentiment"), sentiments, to_metas=True)
...
class OWOllamaSentimentAnalyzer(OWWidget):
    ...
    class Outputs:
        out_table = Output("Sentiment Corpus", Table)
    ...
    def layout_control_area(self):
        ...
        self.summarize_button = QPushButton("Analyze Sentiment")
    ...
    def layout_main_area(self):
        ...
        self.context_box.setPlaceholderText("Add context for sentiment prompt...")
        ...
        self.mainArea.layout().addWidget(QLabel("Text Feature to Analyze:"))
    ...
    def run_summary_thread(self):
        ...
        self.worker = SentimentWorker(self.in_corpus.copy(), self.selected_column, host, port, model, context)
...
if __name__ == "__main__":
    ...
    WidgetPreview(OWOllamaSentimentAnalyzer).run(sample_corpus)
