...
def generate_tree(root_dir: Path, combined_ignore_spec: pathspec.PathSpec) -> str:
    ...
    logger.debug(f"Generating directory tree for {root_dir} (ignores applied, includes not applied to tree structure itself)...")
    ...
    def _build_tree_recursive(current_dir: Path, prefix: str):
        ...
        logger.debug(f"Filtered down to {len(filtered_items)} items in {current_dir} for tree display (based on ignores only).")
...
def scan_and_filter_files(
    ...
    logger.debug(f"Ignore spec patterns being used: {[p.pattern for p in combined_ignore_spec.patterns if hasattr(p, 'pattern')] if combined_ignore_spec else 'None'}")
            ...
            try:
                if item.is_relative_to(root_dir):
                    ...
                    if include_spec:
                        ...
                        logger.debug(f"Checking file against include patterns: Path='{relative_path_str}', Included={should_include}")
    ...
    if include_spec and not included_files:
         logger.info("Include patterns were specified, but no files matched after applying both include and ignore rules.")
    elif not included_files:
         logger.info("No files found to include after applying ignore rules (no include patterns were specified or matched).")
    else:
