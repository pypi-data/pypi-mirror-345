...
@dataclass
class PooledConnection(AsyncWsSurrealConnection):
    """
    ...
    """
    connection: Union[AsyncWsSurrealConnection, AsyncHttpSurrealConnection]
...
class SurrealDBConnectionPool:
    ...
    async def _acquire_connection(self) -> Union[AsyncWsSurrealConnection,AsyncHttpSurrealConnection]:
        """
        ...
        Returns:
            Union[AsyncWsSurrealConnection, AsyncHttpSurrealConnection]: A Surreal connection from the pool
    ...
    async def _execute_schema(self, db: Union[AsyncWsSurrealConnection, AsyncHttpSurrealConnection]):
        """
        Execute a SurQL schema file on a database connection.

        Reads the entire schema file and executes it as a single query.
        SurrealDB handles multiple statements separated by semicolons within one query.

        Args:
            db: The asynchronous SurrealDB connection instance.

        Raises:
            FileNotFoundError: If the schema file cannot be found.
            PermissionError: If the schema file cannot be read due to permissions.
            Exception: For database errors during schema execution or other issues.
                       (Ideally, catch specific surrealdb errors if the library provides them).
        """
        logger.info(f"Attempting to execute schema from: {self.schema_file}")
        try:
            # Open with explicit encoding, UTF-8 is standard for code/schema files
            ...
            if not schema_script.strip():
                logger.warning(f"Schema file '{self.schema_file}' is empty or contains only whitespace. Skipping execution.")
                return # Nothing to execute

            # Execute the entire script in one go
            # SurrealDB server will parse and execute statements sequentially
            logger.debug(f"Executing schema script from '{self.schema_file}'...")
            ...
            logger.error(f"Permission denied when reading schema file: {self.schema_file}")
            raise
        except Exception as e:
            # Log the type of exception for better debugging
            logger.error(f"Unexpected error executing schema from {self.schema_file}: {type(e).__name__} - {e}")
