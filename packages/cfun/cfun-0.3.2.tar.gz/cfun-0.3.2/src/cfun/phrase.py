...
class Phrase:
    ...
    def _best_match_from_regex(
        ...
        #     patterns.extend(self._generate_regex_patterns(combo, n))
    ...
    def _charsimilarcorrector(self, match_df: pd.DataFrame, fragment: str) -> str:
        ...
        for _index, row in match_df.iterrows():
            word = row["word"]
        ...
        for item in cc:
            ...
            simtmp = self.corrector.std_cal_sim(
                error1, right1, rounded=4, kind="pinyin"
            )
            if simtmp > sim:
                sim = simtmp
                res = word1
    ...
    def get_yuxu(self, fragment: str) -> tuple[str, str, bool]:
        ...
        if matched == "":
            matched, match_df = self._best_match_from_regex(combinations, n=1)
    ...
    def phrase_cw(
        ...
        要求 data 中的每个元素都必须是包含指定字段（默认是 "name"）的字典。
        ...
        for i in range(len(cw)):
            ...
            # 优先尝试精准匹配
            for item in data:
                if item[key] == target_char:
                    res.append(deepcopy(item))
                    isfind = True
                    data.remove(item)  # 把data中已经匹配的字符删除
                    break
            if isfind:
                continue
            ...
            charitem = None
            for item in data:
                sim = self.chasimilarity.std_cal_sim(
                    target_char, item[key], 4, "pinyin"
                )
                if sim > max_sim:
                    max_sim = sim
                    charitem = item
            ...
            if charitem:
                ...
                if check:
                    raise ValueError(f"没有找到匹配的字符: {target_char}")
                else:
                    res.append({})
