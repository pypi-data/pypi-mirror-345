import time, threading, os, cloudpickle, inspect, asyncio
...
def make_hashable(obj):
    ...
    if isinstance(obj, dict):
        # Sort items by key to ensure consistent hash for same dict content
        return tuple(sorted((k, make_hashable(v)) for k, v in obj.items()))
    # Check for sets
    if isinstance(obj, set):
        return tuple(sorted(make_hashable(e) for e in obj))
    # Assume obj is already hashable (int, float, str, bool, None, bytes, frozenset, etc.)
    # If it's not hashable here, a TypeError will be raised later, which is expected.
    return obj
