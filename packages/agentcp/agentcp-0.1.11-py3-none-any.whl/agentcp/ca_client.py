...
import requests
import datetime
from agentcp.log import log_error, logger
...
class CAClient:
    def __init__(self, ca_server, app_path,timeout=30):
        ...
        self.ca_server = self.ca_server + "/api/entrypoint"
    ...
    def __save_csr_to_file_guest(self, csr, filename):
        try:
            # 确保目录存在
            file_dir = os.path.dirname(filename)
            os.makedirs(file_dir, exist_ok=True)
    ...
    def __save_private_key_to_file_guest(self, name, private_key):
        try:
            aid_path = os.path.join(self.aid_path,name,'private')
            # 确保目录存在
            os.makedirs(aid_path, exist_ok=True)
            aid_path = os.path.join(aid_path, name + ".key")
            # 假设 private_key 是 PEM 格式的字符串
    ...
    def get_guest_aid(self):
        try:           
            ...
            path = os.path.join(self.aid_path,local_guest_aid,'private',local_guest_aid+".crt")
            ...
            if response.status_code == 200:
                ...
                if "guest_aid" in rjs and "key" in rjs and "cert" in rjs and "entrypoint" in rjs:
                    print(f"sign_guest_cert ok:{rjs}")
                    ...
                    # 保存证书文件
                    aid_path = os.path.join(self.aid_path, guest_aid, 'private')
                    ...
                    print(f"sign_guest_cert failed:{rjs}")
                    return None
            else:
                print(f"sign_guest_cert failed:{response.status_code} - {response.json().get('error', '')}")
                return None
        except Exception as e:
            import traceback
            print(f"获取访问身份失败: {e}")
    ...
    def resign_csr(self,agent_id) -> bool:
        path = os.path.join(self.aid_path,agent_id,'private',agent_id+".crt")
