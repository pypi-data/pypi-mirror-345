...
import wntrfr.sim.hydraulics
from wntrfr.sim.solvers import NewtonSolver, SolverStatus
import wntrfr.sim.results
import numpy as np
import warnings
import time
import sys
...
from wntrfr.network import Junction, Pipe, Valve, Pump, Tank, Reservoir, LinkStatus, WaterNetworkModel, Link
from wntrfr.sim.network_isolation import check_for_isolated_junctions, get_long_size
from wntrfr.sim.aml.aml import VarDict, ParamDict
from wntrfr.sim.aml.expr import Var, Param
from wntrfr.network.controls import (AndCondition, Comparison, Control, ControlAction,
...
class _Diagnostics(object): # pragma: no cover
    ...
    def compare_link_sol(self):
        ...
        if isinstance(start_node, wntrfr.network.Junction):
        ...
        if isinstance(end_node, wntrfr.network.Junction):
    ...
    def store_var_values_in_network(self):
        # self.mode = self._wn.options.hydraulic.demand_model
        wntrfr.sim.hydraulics.store_results_in_network(self.wn, self.model) #, self.mode)
...
class _ValveSourceChecker(Observer):
    ...
    def update(self, action: BaseControlAction):
        ...
        if val != self._previous_values[(obj, attr)]:
            self._needs_compute = True
            if val == wntrfr.network.LinkStatus.Closed:
...
class WNTRSimulator(WaterNetworkSimulator):
    ...
    def _compute_next_timestep_and_run_presolve_controls_and_rules(self, first_step):
        ...
        while cnt < len(presolve_controls_to_run) or self._rule_iter * self._wn.options.time.rule_timestep <= self._wn.sim_time:
            if cnt >= len(presolve_controls_to_run):
                ...
                if not first_step:
                    wntrfr.sim.hydraulics.update_tank_heads(self._wn)
                self._rule_iter += 1
    ...
    def run_sim(self, solver=NewtonSolver, backup_solver=None, solver_options=None,
        ...
        ----------
        solver: object
            :py:class:`~wntrfr.sim.solvers.NewtonSolver` or Scipy solver
        backup_solver: object
            :py:class:`~wntrfr.sim.solvers.NewtonSolver` or Scipy solver
        solver_options: dict
            See :py:class:`~wntrfr.sim.solvers.NewtonSolver` for possible options
        ...
        self.mode = self._wn.options.hydraulic.demand_model
        self._model, self._model_updater = wntrfr.sim.hydraulics.create_hydraulic_model(wn=self._wn, HW_approx=HW_approx)
        ...
        node_res, link_res = wntrfr.sim.hydraulics.initialize_results_dict(self._wn)
        results = wntrfr.sim.results.SimulationResults()
        ...
        if first_step:
            wntrfr.sim.hydraulics.update_network_previous_values(self._wn)
        ...
        while True:
            ...
            if not first_step and not resolve:
                wntrfr.sim.hydraulics.update_tank_heads(self._wn)
            wntrfr.sim.hydraulics.update_model_for_controls(self._model, self._wn, self._model_updater, self._change_tracker)
            wntrfr.sim.models.param.source_head_param(self._model, self._wn)
            wntrfr.sim.models.param.expected_demand_param(self._model, self._wn)
            ...
            if solver_status == 0:
                ...
                results.error_code = wntrfr.sim.results.ResultsStatus.error
            ...
            wntrfr.sim.hydraulics.store_results_in_network(self._wn, self._model)
            ...
            if isinstance(self._report_timestep, (float, int)):
                if self._wn.sim_time % self._report_timestep == 0:
                    wntrfr.sim.hydraulics.save_results(self._wn, node_res, link_res)
        ...
        wntrfr.sim.hydraulics.get_results(self._wn, results, node_res, link_res)
    ...
    def _get_isolated_junctions_and_links(self):
        ...
        wntrfr.sim.hydraulics.update_model_for_isolated_junctions_and_links(self._model, self._wn, self._model_updater,
