...
def choose_windows_interface():
    import pcap
    """
    Returns a dict mapping each NPF device string to an (empty) IP list.
    read_from_interface_windows() will be handed the NPF name directly,
    so it never has to “fall back” to the first device.
    """
    devs = pcap.findalldevs()
    return { dev: {'ipv4': [], 'ipv6': []} for dev in devs }
...
##TODO remove me, just windows testing
