...
from malevich_app.export.secondary.InitFunState import InitFunState
...
from malevich_app.export.jls.helpers import get_params_names, run_func, Object, PreContext, is_async
...
class JuliusApp:
    ...
    def _init(self, operation_id: str, app_id: str, task_id: Optional[str] = None, extra_collections_from: Optional[Dict[str, List[str]]] = None):
    ...
    def set_context(self):
        ...
        if len(self.__context_funcs) == 0:
            context = Context(self.__operation_id, self.run_id, self.app_id, self.app_cfg, self.__cfg.msg_url, self.__cfg.email, self.dag_host_port, self.secret, self._single_pod, self.state.scale, self._login, self.logs_buffer, self._local_storage, self.__logger_fun)
            ...
            for name, context_fun in self.__context_funcs.items():
                if context is None:
                    context = context_fun(self.__operation_id, self.run_id, self.app_id, self.app_cfg, self.__cfg.msg_url, self.__cfg.email, self.dag_host_port, self.secret, self._single_pod, self.state.scale, self._login, self.logs_buffer, self._local_storage, self.__logger_fun)
                    context.common = common
    ...
    async def init_all(self) -> bool:
        try:
            await gather(*[init_fun.run() for init_fun in self.__init_funs])
            return True
        except:
            self.logs_buffer.write(f"init failed: {format_exc_skip(skip=15)}\n")
            return False
    ...
    async def init_all_first(self) -> bool:
        try:
            ...
            self.logs_buffer.write(f"start init failed: {format_exc_skip(skip=15)}\n")
            return False
    ...
    async def __input(self, *args):
        try:
            res = await run_func(self.__input_fun, *args, cpu_bound=self.__input_cpu_bound, logs_buffer=self.logs_buffer)
    ...
    async def __processor(self, *args):
        try:
            res = await run_func(self.__processor_fun, *args, cpu_bound=self.__processor_cpu_bound, logs_buffer=self.logs_buffer)
    ...
    async def __output(self, *args):
        try:
            res = await run_func(self.__output_fun, *args, cpu_bound=self.__output_cpu_bound, logs_buffer=self.logs_buffer)
    ...
    async def __condition(self, *args):
        try:
            res = await run_func(self.__condition_fun, *args, cpu_bound=self.__condition_cpu_bound, logs_buffer=self.logs_buffer)
    ...
    async def run(self, *args) -> Tuple[bool, Any]:
        ...
        with redirect_stdout(self.logs_buffer):
        ...
        if self.__operation == EntityType.PROCESSOR and res[0] and res[1] is None:
            ...
            res = (False, "processor function should return dataframes, not None")
        return res
