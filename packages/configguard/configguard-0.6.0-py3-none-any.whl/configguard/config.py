...
# Date: 2025-05-04 (Revised based on user request for key naming and version in values save)
...
class ConfigGuard(MutableMapping):
    ...
    Key naming for 'full' saves/exports: '__version__', '__schema__', '__settings__'.
    Key naming for 'values' saves: '__version__' and setting/section keys.
    """

    # --- Constants for standard keys ---
    VERSION_KEY = "__version__"  # Used in schema, values save, full save/export
    SCHEMA_KEY = "__schema__"    # Used in full save/export
    VALUES_KEY = "__settings__"  # Used in full save/export
    ...
    def __init__(
        ...
        # Use the defined constant VERSION_KEY here
        ...
        if self._config_path and self._handler:
            try:
                # Initial load attempt using the configured handler and path.
                # Note: Implicit saving after migration is NOT done here.
                # User should call save() explicitly if they want to update the file.
                self.load()
    ...
    def _load_schema_definition(
        ...
        if isinstance(schema_input, dict):
            ...
            try:
                ...
                # Check if the loaded result looks like a 'full' save structure (old or new keys)
                # This handles cases where a schema might have been saved using save(mode='full')
                ...
                if is_new_full:
                     raw_schema_inner = load_result.get(self.SCHEMA_KEY)
                     version = load_result.get(self.VERSION_KEY)
                     ...
                     if isinstance(raw_schema_inner, dict):
                         ...
                         if version: raw_schema[self.VERSION_KEY] = version
                     else:
                          raise SchemaError(f"Schema file {schema_path} has '{self.SCHEMA_KEY}' but it's not a dictionary.")
                elif is_old_full:
                     ...
                     if isinstance(raw_schema_inner, dict):
                         raw_schema = raw_schema_inner
                         if version: raw_schema[self.VERSION_KEY] = version # Use new key internally
                     else:
                          raise SchemaError(f"Schema file {schema_path} has 'schema' (old format) but it's not a dictionary.")
    ...
    def load(self, filepath: typing.Optional[typing.Union[str, Path]] = None) -> None:
        ...
        Note: If migration occurs, this method does *not* automatically save the updated
        configuration back to the file. Call `save()` explicitly if needed.
        ...
        try:
            ...
            # It internally handles checking for new (__version__, __schema__, __settings__)
            # and old (version, schema, values) keys for 'full' mode detection.
            loaded_data: LoadResult = current_handler.load(load_path)

            # Extract data from LoadResult (keys are consistent regardless of file format)
            ...
            if loaded_version_str is not None:
                # File contains version info, could be 'full' save or 'values' with version
                # Note: Handler load() should now detect version in values-only files too if present
                load_mode_desc = f"file contains version info (v{loaded_version_str})"
                try:
                ...
                # Treat as if migrating from an older version
                allow_migration = True

            # Apply values using the recursive helper method
            log.info(f"Applying loaded values from '{load_mode_desc}' file...")
            # If loading a basic 'values' file (no version), `allow_migration` is True,
            # meaning unknown keys from the file will be ignored.
    ...
    def _apply_and_migrate_values(
        ...
        for name, current_item in current_container.items():
            ...
            if isinstance(current_item, ConfigSetting):
                ...
                if name in loaded_values_level:
                    ...
                    # --- Type Coercion Logic (based on loaded schema if available) ---
            ...
            # --- Process schema-defined ConfigSections ---
                ...
                nested_loaded_schema = None # Initialize

                # Try to get nested schema from loaded schema (new and old keys)
                if loaded_schema_level and isinstance(loaded_schema_level.get(name), dict):
                     ...
                     nested_loaded_schema = nested_loaded_schema_section.get("schema") # Standard key
                     if not isinstance(nested_loaded_schema, dict):
                          nested_loaded_schema = None # Reset if not a dict
                ...
                if isinstance(nested_loaded_values, Mapping):
                    ...
                    self._apply_and_migrate_values(
                        ...
                        loaded_schema_level=nested_loaded_schema, # Pass down nested schema
                        allow_migration=allow_migration,
                    ...
                    # Ensure defaults within the section (recursion with empty loaded_values)
                    self._apply_and_migrate_values(
                        current_container=current_item._settings,
                        current_schema_level=current_schema_level.get(name, {}).get("schema", {}),
                        loaded_values_level={},
        ...
        for key in unknown_or_migrated_keys:
            ...
            elif allow_migration: # Includes cases where file had no version
                skipped_migration += 1
                log.warning(
                    f"Migration/Skip: Item '{full_unknown_path}' (value: {unknown_value!r}) loaded from file is not present in current instance version ({self.version}). Skipping."
                )
    ...
    def save(
        ...
        Args:
            ...
            mode: Specifies what to save.
                  - 'values': Saves only the current configuration key-value pairs,
                              prepended with a '__version__' key indicating the instance version.
                              The structure depends on the handler (e.g., simple dict for JSON/YAML/TOML,
                              flat key-value for SQLite).
                  - 'full': Saves the instance version, schema definition, and values using the
                            keys '__version__', '__schema__', and '__settings__'.
        ...
        # --- Prepare data payload based on mode ---
        # NOTE: The structure prepared here might differ from the final file structure,
        # as the handler is responsible for interpreting the mode and payload.
        # However, based on the request, we construct the *intended* final structure
        # here and rely on the handler (ideally updated) to save it directly.

        config_values = self.get_config_dict()
        schema_definition = self.get_instance_schema_definition()

        data_to_serialize: typing.Any # The structure the handler should attempt to save
        # The standard payload passed *to* the handler's save method, containing all components.
        standard_handler_payload = {
            # Use standard key names for the payload passed *to* the handler
            "instance_version": self.version,
            "schema_definition": schema_definition,
            "config_values": config_values,
            # Add the constants for the new keys so handler knows what to use
            "__version_key__": self.VERSION_KEY,
            "__schema_key__": self.SCHEMA_KEY,
            "__values_key__": self.VALUES_KEY,
        }

        # --- Determine the final structure based on mode ---
        # This part deviates from the pure handler-based approach to match the request
        # to define the output structure in config.py. A cleaner approach would be
        # for the handler to construct these structures based on the mode and the
        # standard_handler_payload.

        if mode == "full":
            # Structure using the new keys
            data_to_serialize = {
                self.VERSION_KEY: self.version,
                self.SCHEMA_KEY: schema_definition,
                self.VALUES_KEY: config_values,
            }
            log.debug("Prepared 'full' data structure for saving.")
            # In a handler-centric approach, we would just pass standard_handler_payload
            # and the handler would build this structure.

        elif mode == "values":
            # Structure is version key + config values
            data_to_serialize = {
                self.VERSION_KEY: self.version,
                **config_values # Unpack config values alongside version
            }
            log.debug("Prepared 'values' data structure (with version) for saving.")
            # In a handler-centric approach, we would pass standard_handler_payload
            # and the handler would build this structure.

        else: # Should be caught earlier, but defensively
             raise ValueError(f"Internal error: Invalid mode '{mode}' encountered.")


        try:
            # --- Call handler's save method ---
            # Pass the standard payload so handler has all info, AND the mode.
            # The handler MUST now correctly interpret the mode and use the constants
            # from the payload (__version_key__, etc.) to construct the correct output file format.
            # --- Correction based on request interpretation ---
            # The request implies config.py dictates the final format to be saved.
            # This requires modifying the handler interface or making assumptions.
            # Let's assume the handler's save method is modified to directly accept
            # the `data_to_serialize` structure *instead* of the standard payload
            # when called from here. This is NOT the recommended design pattern.
            # A better approach is keeping standard_handler_payload and modifying handlers.

            # --- MODIFIED CALL (Based on request interpretation, less ideal design) ---
            # We will pass the already constructed data_to_serialize directly.
            # This assumes the handler's save method signature/logic is adapted,
            # or it has a way to differentiate this non-standard payload.
            # For simplicity of just rewriting config.py, we make this assumption.
            # The existing handler interface expects the standard payload.
            # Let's revert to passing the standard payload and assume handlers are updated.
    ...
    def get_config_dict(self) -> dict:
        ...
        Does NOT include the '__version__' key.
        """
        config_dict = {}
    ...
    def export_schema_with_values(self) -> dict:
        ...
        Returns:
            ...
            The '__settings__' dict maps names to {'schema': ..., 'value': ...}.
        ...
        settings_export = {}
        # Use internal recursive helper to populate settings_export
        self._export_level(self._settings, settings_export)

        # Construct the final export dictionary using the defined keys
        full_export = {
            self.VERSION_KEY: self.version,
            self.SCHEMA_KEY: self.get_instance_schema_definition(),
            self.VALUES_KEY: settings_export,  # Use the populated settings dict
        }
    ...
    def _export_level(
        ...
        for name, item in container.items():
            if isinstance(item, ConfigSetting):
                export_target[name] = {
                    "schema": item.schema.to_dict(), # Use standard schema key name here
                    "value": item.value,
                }
            elif isinstance(item, ConfigSection):
            ...
            # Handle dynamically added values within a dynamic section
            else:
                 # This else assumes the container is from a dynamic ConfigSection's _settings
                 export_target[name] = {
                      "schema": { # Provide a minimal placeholder schema
    ...
    def import_config(self, data: Mapping, ignore_unknown: bool = True) -> None:
        ...
        try:
            ...
            # The method handles dynamic sections correctly based on the flag.
            self._apply_and_migrate_values(
                current_container=self._settings,
                current_schema_level=self._instance_schema_definition,
            ...
            # Don't re-raise, allow partial import maybe? Or should we? Re-raise for now.
            raise
    ...
    # --- Magic methods (No changes needed for nesting/dynamic/versioning logic here) ---
    ...
    def __setattr__(self, name: str, value: typing.Any) -> None:
        ...
        known_internals = {
            ...
            "_autosave",
            "loaded_file_version",
            # Add constants here if they become instance attributes
        }
    ...
    def __delitem__(self, key: str) -> None:
        ...
        # Allow deletion only if the top level itself could be dynamic (which it can't be currently)
