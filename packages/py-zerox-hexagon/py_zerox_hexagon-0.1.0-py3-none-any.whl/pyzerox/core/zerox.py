...
async def zerox(
    ...
    **kwargs
    ...
    :param file_path: The path or URL to the PDF file, image file, or a list of file paths to process.
    :type file_path: str or List[str], optional
    ...
    with tempfile.TemporaryDirectory() as temp_dir_:
        ...
        for path in file_paths:
            # Download the file. Get file name.
            local_path = await download_file(file_path=path, temp_dir=temp_directory)
            if not local_path:
                continue  # Skip if file download fails
            ...
            if not (is_pdf or is_image):
                warnings.warn(f"Skipping unsupported file type: {file_ext}. Supported types are PDF and common image formats.")
                continue
            ...
            if is_pdf:
                ...
                pdf_images = await convert_pdf_to_images(local_path=local_path, temp_dir=temp_directory)
                images.extend(pdf_images)
            else:  # is_image
                # For a single image, just use it directly
                images.append(local_path)

        if not images:
            raise ValueError("No valid PDF or image files found in the provided paths.")

        # Use the first file's name as base filename
        raw_file_name = os.path.splitext(os.path.basename(file_paths[0]))[0]
