...
from verse.core import Component, Context, Operation, Provider
from verse.interface.api import API
...
class NextJS(Provider):
    ...
    def _get_api(self) -> API | None:
        component = self.component or self.get_component().component
        if self._api:
            return self._api
        ...
        if self.api_component is not None:
            ...
            from verse.interface.api import API
            from verse.interface.api.providers.default import Default

            api_component = API(component=component)
            api_component.attach_provider(
                Default(
                    host=self.api_host,
                    port=self.api_port,
