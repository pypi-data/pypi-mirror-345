...
from johnsnowlabs.utils.pip_utils import (
    install_standard_pypi_lib,
    install_licensed_pypi_lib,
    get_pip_lib_version,
)
        ...
        if cls.pypi_name:
            if cls.is_py4j and settings.enforce_versions:
                ...
                if cls.name == ProductName.hc.value and suite.hc and suite.hc.py_lib:
                    ...
                    cls.name == ProductName.ocr.value and suite.ocr and suite.ocr.py_lib
                ):
                    return install_standard_pypi_lib(
                        os.path.join(settings.py_dir, suite.ocr.py_lib.file_name),
                        cls.py_module_name,
                        python_path=py_path,
                        upgrade=upgrade,
                        re_install=re_install,
                    ...
                    cls.name == ProductName.nlp.value and suite.nlp and suite.nlp.py_lib
                ):
                    return install_standard_pypi_lib(
                        os.path.join(settings.py_dir, suite.nlp.py_lib.file_name),
                        cls.py_module_name,
                        python_path=py_path,
                        upgrade=upgrade,
                        re_install=re_install,
            ...
            if secrets and cls.licensed:
                ...
                return install_licensed_pypi_lib(
                    secrets=secrets,
