...
for the Deye inverter. It includes methods to set power and current setpoints,
...
class DeyeModbusInverter(ModbusBattery):
    '''Deye modbus inverter class
    this class is used to communicate with a deye inverter through the
    solarman data logger(LSE-3)
    ...
    def __init__(
        ...
        super().__init__(data_logger_ip, data_logger_port, timeout, capacity,
    ...
    def _set_discharge_setpoint(self, setpoint: int):
        # Set discharge power setpoint
        self.write_register(
            self.HoldingRegisters.SellModeKWPoint6.value, setpoint)
        # Set SOC setpoint to 5% to discharge
        self.write_register(
            self.HoldingRegisters.SellModeBattCapacity6.value, 5)

    def _set_charge_setpoint(self, setpoint: int):
        voltage = self.read_holding_register(
            self.HoldingRegisters.BatteryVoltage.value, 1)

        # TODO:: voltage changes slightly when charging so this is not
        # accurate, try using grid power limit register or reguraly update the
        # charge current
        ...
        self.write_register(
            self.HoldingRegisters.MaxChargeCurrent.value, max_charge_current)
        # Set SOC setpoint to 100% to charge
        self.write_register(
            self.HoldingRegisters.SellModeBattCapacity6.value, 100)
    ...
    def _init_power_setpoint_registers(self):
        # set TimeOfUse to 0x00FF
        # this enables the time of use settings for all days of the week
        self.write_register(
            self.HoldingRegisters.TimeOfUse.value, 0x00FF)

        self.write_register(
            self.HoldingRegisters.SellTimePoint1.value, 100)
        self.write_register(
            self.HoldingRegisters.SellTimePoint2.value, 100)
        self.write_register(
            self.HoldingRegisters.SellTimePoint3.value, 100)
        self.write_register(
            self.HoldingRegisters.SellTimePoint4.value, 100)
        self.write_register(
            self.HoldingRegisters.SellTimePoint5.value, 100)
        self.write_register(
            self.HoldingRegisters.SellTimePoint6.value, 100)
        ...
        self.write_register(
            self.HoldingRegisters.ChargeModeEnable6.value, 1)
    ...
    def get_active_power_total(self):
        ...
        Returns:
            float: the active power of the inverter
        '''
        return self.read_holding_register(
            self.HoldingRegisters.ActivePower.value, 1)

    def get_active_current_total(self):
        '''Gets the active total current of the inverter

        Returns:
            float: the active current of the inverter
        '''
    ...
    def get_battery_soc(self):
        ...
        '''
        return self.read_holding_register(
            self.HoldingRegisters.BatterySOC.value, 1)
    ...
    def get_phase_voltage(self, phase: int):
        ...
        if register:
            return self.read_holding_register(register, 1)
        else:
