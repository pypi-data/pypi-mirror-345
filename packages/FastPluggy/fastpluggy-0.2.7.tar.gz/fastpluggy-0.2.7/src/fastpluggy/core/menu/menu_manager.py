...
class MenuManager:
    ...
    def get_menu(self, menu_type: str, current_url: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for index, item in enumerate(self.menus[menu_type]):
            ...
            #if index > 0:
            #    result_item["divider_before"] = True
...
def get_plugin_state_for_route(
    ...
    for module in modules:
        ...
        if isinstance(plugin_router, APIRouter) and plugin_router is not None:
            for route in plugin_router.routes:
                # Only consider standard HTTP routes
                if isinstance(route, APIRoute):
                    # route.path_regex is a compiled regex matching the full path
