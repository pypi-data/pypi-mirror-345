...
def virtual_port_spec(num_modes: int = 1, classification: str = "optical") -> PortSpec:
    ...
    if classification == "optical":
        ...
        virtual = PortSpec(
            "Virtual spec (electrical)", 1, (0, 0), num_modes, voltage_path=[(-0.1, 0), (0.1, 0)]
        )
    cached = _virtual_port_specs.get((classification, num_modes))
    if cached != virtual:
        _virtual_port_specs[classification, num_modes] = virtual
        cached = virtual
    return cached
...
def grid_layout(
    objects: Sequence[Union[Component, Reference, Circle, Path, Polygon, Rectangle]],
    gap: Union[float, Sequence[float]] = 0,
    shape: Optional[Sequence[int]] = None,
    align_x: Optional[Literal["left", "right", "center", "origin"]] = "center",
    align_y: Optional[Literal["bottom", "top", "center", "origin"]] = "center",
    direction: Literal[
...
def pack_layout(
    ...
    sorting: Optional[Literal["best", "area"]] = "best",
