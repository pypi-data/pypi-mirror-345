...
async def fetch_pair(vid: str, height: str, proxies: list[str]) -> dict:
    """
    Exactly as original: race one fetch per proxy until one succeeds,
    then return the info dict containing video_url/audio_url, etc.
    """
...
async def _amain():
    ...
    def proxy_flags(uri: str) -> list[str]:
        if not uri:
            return []
        return ["--proxy="+uri] if uri.startswith("socks") else ["--all-proxy="+uri]

    dl_flags = proxy_flags(args.dlproxy)

    # Cleanup / recreate tmp
    if TMP_DIR.exists():
    ...
    try:
        # Fetch metadata by racing proxies until one works
        ...
        if args.audio:
            ...
            # Download audio with optional proxy
            run_stream(
                [
                    *dl_flags,
                    str(aria2c),
            ...
            # Fetch & resize thumbnail if present
            cover_args: list[str] = []
        ...
        # Download video & audio in parallel with optional proxy
