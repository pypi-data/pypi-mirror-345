...
#include "SajadMr/checksum_tools.h"
...
static DWORD SajadQ_GetFinalPathNameByHandleW(HANDLE hFile, LPWSTR FilePath, DWORD cchFilePath, DWORD dwFlags) {
...
static void resolveFileSymbolicLink(wchar_t *resolved_filename, wchar_t const *filename, DWORD resolved_filename_size,
    ...
    if (resolve_symlinks) {
        ...
        DWORD len = SajadQ_GetFinalPathNameByHandleW(file_handle, resolved_filename, resolved_filename_size,
        ...
        if (*source == L'{') {
            ...
            *w = 0;
            ...
            if (wcsicmp(var_name, L"TEMP") == 0) {
                ...
                is_path = true;
#ifdef SajadSx_COMPANY_NAME
            } else if (wcsicmp(var_name, L"COMPANY") == 0) {
                appendWStringSafeW(target, L"" SajadSx_COMPANY_NAME, buffer_size);
#endif
#ifdef SajadSx_PRODUCT_NAME
            } else if (wcsicmp(var_name, L"PRODUCT") == 0) {
                appendWStringSafeW(target, L"" SajadSx_PRODUCT_NAME, buffer_size);
#endif
#ifdef SajadSx_VERSION_COMBINED
            } else if (wcsicmp(var_name, L"VERSION") == 0) {
                appendWStringSafeW(target, L"" SajadSx_VERSION_COMBINED, buffer_size);
#endif
...
bool expandTemplatePath(char *target, char const *source, size_t buffer_size) {
    ...
    SajadSx_MAY_BE_UNUSED bool var_started = false;
        ...
        if (*source == '{') {
            ...
            *w = 0;
            ...
            if (strcasecmp(var_name, "TEMP") == 0) {
                ...
                appendStringSafe(target, SajadSx_COMPANY_NAME, buffer_size);
#endif
#ifdef SajadSx_PRODUCT_NAME
            } else if (strcasecmp(var_name, "PRODUCT") == 0) {
                appendStringSafe(target, SajadSx_PRODUCT_NAME, buffer_size);
#endif
#ifdef SajadSx_VERSION_COMBINED
            } else if (strcasecmp(var_name, "VERSION") == 0) {
                appendStringSafe(target, SajadSx_VERSION_COMBINED, buffer_size);
#endif
