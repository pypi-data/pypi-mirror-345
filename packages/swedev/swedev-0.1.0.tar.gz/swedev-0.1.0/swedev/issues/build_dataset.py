...
def create_instance(repo: Repo, pull: dict) -> dict:
    ...
    logger.critical(f"Extracted patches and problem statement for PR {pull['number']} in {repo.repo.full_name}")
    logger.critical(f"Patch: {patch[:100]}")
    logger.critical(f"Test patch: {test_patch[:100]}")
    return {
        ...
        with open(output, write_mode) as output:
            for ix, line in enumerate(open(pr_file)):
                ...
                if not is_valid_pull(pull):
                    ...
                    print(f"Skipping invalid PR {pull['number']}")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--token", type=str, default='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', help="GitHub token")
