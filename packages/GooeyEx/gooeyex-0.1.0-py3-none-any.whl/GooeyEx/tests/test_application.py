...
class TestGooeyApplication(unittest.TestCase):
    ...
    def testFullscreen(self):
        ...
        for shouldShow in [True, False]:
            with self.subTest("Should set full screen: {}".format(shouldShow)):
                with instrumentGooey(parser, fullscreen=shouldShow) as (
                    app,
                    frame,
                    gapp,
                ):
    ...
    def testGooeyRequestsConfirmationWhenShowStopWarningModalTrue(self, mockModal):
        ...
        Case = namedtuple(
            "Case",
            ["show_warning", "shouldSeeConfirm", "userChooses", "shouldHaltProgram"],
        )
        testcases = [
            Case(
                show_warning=True,
                shouldSeeConfirm=True,
                userChooses=True,
                shouldHaltProgram=True,
            ),
            Case(
                show_warning=True,
                shouldSeeConfirm=True,
                userChooses=False,
                shouldHaltProgram=False,
            ),
            Case(
                show_warning=False,
                shouldSeeConfirm=False,
                userChooses="N/A",
                shouldHaltProgram=True,
            ),
        ]
        ...
        for case in testcases:
            ...
            parser = self.basicParser()
            with instrumentGooey(parser, show_stop_warning=case.show_warning) as (
                app,
                frame,
                gapp,
            ):
    ...
    def testTerminalColorChanges(self):
        ...
        for expectedColor in expectedColors:
            with instrumentGooey(parser, terminal_panel_color=expectedColor) as (
                app,
                frame,
                gapp,
            ):
    ...
    def testFontWeightsGetSet(self):
        ...
        for weight in [constants.FONTWEIGHT_LIGHT, constants.FONTWEIGHT_BOLD]:
            parser = self.basicParser()
            with instrumentGooey(parser, terminal_font_weight=weight) as (
                app,
                frame,
                gapp,
            ):
