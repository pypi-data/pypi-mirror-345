#!/usr/bin/env python3
"""
LLM Hardware Client for Docker Environment

This script creates a client that connects to the Ollama LLM and the hardware server
to enable natural language control of virtual hardware devices.
"""
...
logger = logging.getLogger("LLMHardwareClient")

# Get environment variables
HARDWARE_SERVER_HOST = os.getenv('HARDWARE_SERVER_HOST', 'hardware-server')
HARDWARE_SERVER_PORT = int(os.getenv('HARDWARE_SERVER_PORT', '8080'))
...
class MCPHardwareController:
    ...
    def __init__(self, host: str = HARDWARE_SERVER_HOST, port: int = HARDWARE_SERVER_PORT):
        """
        Initialize the MCP hardware controller.
        
        Args:
            host: Hardware server hostname
            port: Hardware server port
        """
        self.host = host
        self.port = port
    ...
    async def connect(self):
        ...
        # Create a simple server command that connects to the hardware server
...
async def main():
    ...
    controller = MCPHardwareController(HARDWARE_SERVER_HOST, HARDWARE_SERVER_PORT)
    ...
    try:
        ...
        system_message = (
            "You are a helpful assistant that controls virtual hardware devices through an MCP server. "
        ...
        print("\nVirtual Hardware Control Assistant\n")
        print("Type 'exit' or 'quit' to end the conversation.")
        print("Type your commands in natural language to control virtual hardware.\n")
