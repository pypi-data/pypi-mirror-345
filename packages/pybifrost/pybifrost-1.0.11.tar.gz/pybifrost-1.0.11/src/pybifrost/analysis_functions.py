...
class BIFROST_nD:
    ...
    def E_f_k_f(self):
        Es = load_constants("consts/Es_fit.npy")
        L = load_constants("consts/dL.npy")
        phi = load_constants("consts/dphi.npy")
...
class BIFROST_Res(BIFROST_nD):
    ...
    def __init__(self, folder, remove=True, reread=False, plot=False):
...
class Theory_Res:
    ...
    def resolution(self, channel, analyzer, tube, pixel):
        ...
        sigma = np.diag(np.array([
            0.01/2.355, 
            0.002/2.355, 
            np.sqrt(self.dt**2+2e-5**2),
            0.0001/d, 
            np.deg2rad(3/60)/2.355, 
            0.025/2.355, 
            delta_phi/2.355]))
...
def compare_resolution(folder, stt, hw, channel, analyzer, tube, pixel, plot=True, save_file=None, reread=False):
    ...
    print(f"Simulated resolution: {sim_mean}, {sim_cov}")
