...
class TopNGrams(TopFeatureExtractor):
    ...
    def __filter_out_very_unlikely(self, malware_dataset, experiment):
        ...
        print(
            f"Extracting n-grams from a randomly selected set of {subsample} samples from the training set"
        )
        subprocess.call(
    ...
    def __compute_ig_for_likely_ones(self, malware_dataset, experiment):
        ...
        df_ig.loc["benign", df_ig.columns] = df[df["sha256"].isin(list(df_ig.columns))][
            "family"
        ]
