    ...
    for i in range(0, len(optimizer)):
        ...
        for j in range(0, len(objectivefunc)):
            ...
            if enable_parallel and NumOfRuns > 1:
                ...
                sol.best = objf(best_result.bestIndividual)
                ...
                for k in range(0, NumOfRuns):
                    ...
                    if Export_details:
                        ...
                        with open(ExportToFile, "a", newline="\n") as out:
                            ...
                            best_fitness = getattr(x, 'best_score', None)
                            if best_fitness is None:
                                best_fitness = func_details[0](x.bestIndividual)
                ...
                best_result_idx = numpy.argmin([objf(selector(optimizer[i], func_details, PopulationSize, Iterations).bestIndividual) for _ in range(NumOfRuns)])
                ...
                sol.optimizer = optimizer[i]
                sol.objfname = objectivefunc[j]
                sol.best = func_details[0](best_result.bestIndividual)
                sol.bestIndividual = best_result.bestIndividual
                sol.best_score = sol.best
                sol.convergence = best_result.convergence
