...
class AWSEC2:
    ...
    def clone_instance(self, model_name: str, clone_names: list):
        """
        Clone a model instance (by Name) into new instances with given names.
        Each new instance will have COMPUTERNAME set to its Name, available in Ubuntu shell.
        """
        ...
        for inst in instances:
            ...
            inst.reload()
            print(f"Instance {inst.id} with Name={ [t['Value'] for t in (inst.tags or []) if t['Key']=='Name'][0] } is running.")
        return instances
