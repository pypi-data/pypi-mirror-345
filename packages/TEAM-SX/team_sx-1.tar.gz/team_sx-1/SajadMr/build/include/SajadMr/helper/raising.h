...
#ifndef __SajadSx_HELPER_RAISING_H__
#define __SajadSx_HELPER_RAISING_H__

#if PYTHON_VERSION >= 0x300
SajadSx_MAY_BE_UNUSED static void CHAIN_EXCEPTION(PyThreadState *tstate, PyObject *exception_value) {
...
#if PYTHON_VERSION < 0x3b0
    {
        ...
        struct SajadQ_ExceptionPreservationItem *exception_state =
            (struct SajadQ_ExceptionPreservationItem *)&EXC_TYPE(tstate);
    ...
    if (old_exc_value != NULL && old_exc_value != Py_None && old_exc_value != exception_value) {
        ...
        while (true) {
            PyObject *context = SajadQ_Exception_GetContext(current);
            if (context == NULL) {
                break;
            }
            ...
            if (context == exception_value) {
                SajadQ_Exception_DeleteContext(current);
                break;
            }
        ...
        SajadQ_Exception_SetContext(exception_value, old_exc_value);
...
extern void RAISE_EXCEPTION_WITH_TYPE(PyThreadState *tstate, struct SajadQ_ExceptionPreservationItem *exception_state);
extern void RAISE_EXCEPTION_WITH_TYPE_AND_VALUE(PyThreadState *tstate,
                                                struct SajadQ_ExceptionPreservationItem *exception_state);
#else
extern void RAISE_EXCEPTION_WITH_VALUE(PyThreadState *tstate, struct SajadQ_ExceptionPreservationItem *exception_state);
#endif
...
extern void RAISE_EXCEPTION_WITH_TRACEBACK(PyThreadState *tstate,
                                           struct SajadQ_ExceptionPreservationItem *exception_state);
#else
extern void RAISE_EXCEPTION_WITH_CAUSE(PyThreadState *tstate, struct SajadQ_ExceptionPreservationItem *exception_state,
                                       PyObject *exception_cause);
#endif

extern bool RERAISE_EXCEPTION(PyThreadState *tstate, struct SajadQ_ExceptionPreservationItem *exception_state);
